// IW8 GSC SOURCE
// Generated by https://github.com/xensik/gsc-tool

cumulative_damage_expire_time()
{
    scripts\cp_mp\utility\script_utility::registersharedfunc( "cargo_truck_mg", "spawnCallback", ::cumulative_damage_to_chopper_boss );
    cargo_truck_mp_initmines();
    cargo_truck_mp_initspawning();
    scripts\mp\vehicles\vehicle_oob_mp::vehicle_oob_mp_registeroutoftimecallback( "cargo_truck_mg", scripts\mp\trials\mp_trl_gunslinger_memory_create_script::cruisepredator_detachplayerfromintro );
}

cargo_truck_mp_initspawning()
{
    var_0 = scripts\cp_mp\vehicles\vehicle_spawn::vehicle_spawn_getleveldataforvehicle( "cargo_truck_mg", 1 );
    var_0.advanced_supply_drop_marker_sfx = scripts\cp_mp\vehicles\vehicle_spawn::_id_127AA;
}

cargo_truck_mp_initmines()
{
    var_0 = scripts\cp_mp\vehicles\vehicle_mines::vehicle_mines_getleveldataforvehicle( "cargo_truck_mg", 1 );
    var_0.frontextents = 165;
    var_0.backextents = 168;
    var_0.leftextents = 57;
    var_0.rightextents = 57;
    var_0.bottomextents = 35;
    var_0.distancetobottom = 50;
    var_0.loscheckoffset = ( 0, 0, 70 );
}

cumulative_damage_to_chopper_boss( var_0, var_1 )
{
    var_2 = scripts\mp\trials\mp_trl_gunslinger_memory_create_script::createvisualsinfo( var_0, var_1 );

    if ( isdefined( var_2 ) && scripts\cp_mp\vehicles\vehicle_spawn::vehicle_spawn_gamemodesupportsrespawn() )
        var_2.ondeathrespawn = ::cumulative_damage_monitor;

    return var_2;
}

cumulative_damage_monitor()
{
    thread cargo_truck_mp_waitandspawn();
}

cargo_truck_mp_waitandspawn()
{
    var_0 = scripts\cp_mp\vehicles\vehicle_tracking::getvehiclespawndata( self );
    var_1 = spawnstruct();
    scripts\cp_mp\vehicles\vehicle_tracking::copyvehiclespawndata( var_0, var_1 );
    var_2 = spawnstruct();
    var_3 = scripts\cp_mp\vehicles\vehicle_spawn::_id_127B2( "cargo_truck_mg", var_1, var_2 );
}
