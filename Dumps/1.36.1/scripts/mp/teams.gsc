...
init()
{
    ...
    level.multiteambased = level.teamnamelist.size > 2 || scripts\mp\utility\game::getgametype() == "br" && scripts\mp\menus::_id_11EB7();
    level.teamdata = [];
    ...
    {
        scripts\mp\utility\teams::lbravo_spawner_jammer1( var_9 );
    ...
    if ( getdvarint( "scr_debug_teams", 0 ) == 1 )
        level thread findgunsmithattachments();
}
...
createoperatorcustomization()
{
    ...
    var_10.clothtype = laser_trap_structs( var_9 );
    var_10.superfaction = getoperatorsuperfaction( var_8 );
    var_10.execution = getoperatorexecution( var_8 );
    var_10.has_intel = laser_trap_triggers( var_8 );
    var_10.has_keycard = laser_traps( var_8 );
    var_10.suit = var_3;
    var_10.rebuild = 0;
    var_10.model_parts = player_exposed_to_chopper_boss( var_1 );

    if ( scripts\mp\utility\game::getgametype() == "br" )
    {
        var_11 = laser_trap_control_station_interaction_array( var_9 );

        if ( var_11 != "" )
            var_10.checkformatchend = var_11;
    }

    var_12 = spawnstruct();
    var_12.apc = lethal_boxes( "apc" );
    var_12.binoculars_checkexpirationtimer = lethaldelayallows( "apc" );
    var_12.bot_ctf_my_team_flag_is_picked_up = lethal_boxes( "atv" );
    var_12.createquestobjicon = lethal_boxes( "cargo_truck" );
    var_12.playerjailtimeout = lethal_boxes( "jeep" );
    var_12.questenabled = lethal_boxes( "little_bird" );
    var_12._id_12101 = lethal_boxes( "tac_rover" );
    var_12._id_1214B = lethal_boxes( "tank_east" );
    var_12._id_1214C = lethaldelayallows( "tank_east" );
    var_12._id_12156 = lethal_boxes( "tank_west" );
    var_12._id_12157 = lethaldelayallows( "tank_west" );
    var_12.bestplayer = lethal_crate_spawn( "apc", 4 );
    var_12.bot_ctf_player_always_attacker = lethal_crate_spawn( "atv", 6 );
    var_12.createrectangularzonebasedonent = lethal_crate_spawn( "cargo_truck", 8 );
    var_12.playerkilled_washitbyvehicle = lethal_crate_spawn( "jeep", 10 );
    var_12.questinstance = lethal_crate_spawn( "little_bird", 12 );
    var_12._id_12102 = lethal_crate_spawn( "tac_rover", 14 );
    var_12.bot_dd_ai_director_update = lethal_equipmentmaskoffsets( "atv" );
    var_12._id_12106 = lethal_equipmentmaskoffsets( "tac_rover" );
    var_12.race_ui_lap = lethal_equipmentmaskoffsets( "little_bird" );
...
lookupcurrentoperator( var_0 )
{
    ...
    if ( getdvarint( "scr_forceHeadlessCustomization", 1 ) == 1 && !isagent( self ) && self isplayerheadless() )
    {
        ...
        if ( !isdefined( self.matchdata_logvictimkillevent ) )
        {
            foreach ( var_9 in var_7 )
            {
                var_10 = getarraykeys( level.operatorcustomization[var_9] );

                if ( !isdefined( level.matchrules_initialammo ) )
                    level.matchrules_initialammo = [];

                if ( !isdefined( level.matchrules_initialammo[var_9] ) || level.matchrules_initialammo[var_9] > var_10.size )
                    level.matchrules_initialammo[var_9] = 0;

                for ( var_11 = var_10[level.matchrules_initialammo[var_9]]; isdefined( var_11 ) && ( var_11 == "default_western" || var_11 == "default_eastern" ); var_11 = var_10[level.matchrules_initialammo[var_9]] )
                {
                    level.matchrules_initialammo[var_9] = level.matchrules_initialammo[var_9] + 1;

                    if ( level.matchrules_initialammo[var_9] > var_10.size )
                        level.matchrules_initialammo[var_9] = 0;
                }
            ...
            self.matchdata_logvictimkillevent = 1;
        }
...
lookupcurrentoperatorskin( var_0 )
{
    ...
    else
    {
        if ( getdvarint( "scr_forceHeadlessCustomization", 1 ) == 1 && !isagent( self ) && self isplayerheadless() )
        {
            ...
            {
                if ( !isdefined( level.matching_correct_bomb_wire_pair ) )
                    onteamproximitybecameinvalidplayer();

                var_5 = level.matching_correct_bomb_wire_pair[var_1]["curIndex"];
                level.playercustomizationdata[var_3][var_0].operatorskinindex = level.matching_correct_bomb_wire_pair[var_1]["lootIDs"][var_5];
                level.matching_correct_bomb_wire_pair[var_1]["curIndex"] = level.matching_correct_bomb_wire_pair[var_1]["curIndex"] + 1;

                if ( level.matching_correct_bomb_wire_pair[var_1]["curIndex"] >= level.matching_correct_bomb_wire_pair[var_1]["maxIndex"] )
                    level.matching_correct_bomb_wire_pair[var_1]["curIndex"] = 0;
            }
        }
...
getoperatorcustomization()
{
    ...
    if ( isdefined( level.round_smoke_molotov_logic ) )
        [var_0, var_1] = [[ level.round_smoke_molotov_logic ]]( self, var_0, var_1 );
...
checkforafk()
{
    ...
    if ( istrue( self.mark_as_bomb_vest_controller_holder ) )
        return;
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" && istrue( self.checkpoint_fx_set ) )
    {
        self.checkpoint_fx_set = undefined;
        var_10 = 30;
    }
    else if ( var_11 > 0 )
        var_10 = var_11;
    ...
    if ( scripts\mp\utility\game::isroundbased() && scripts\mp\utility\game::getgametype() != "ctf" )
    {
        ...
        if ( var_0 && !isgamebattlematch() && !istrue( self.ammorestock_disableusefortime ) )
        {
            self.ammorestock_disableusefortime = 1;
            self.pers["roundsAFK"]++;

            if ( self.pers["roundsAFK"] > 1 )
                level thread playertimestamp( self );
        }
...
{
    level endon( "game_ended" );
    scripts\engine\utility::_id_128FF( "death_or_disconnect", "joined_team" );
...
{
    level endon( "game_ended" );
    scripts\engine\utility::_id_12900( "death_or_disconnect", "joined_team", "joined_squad" );
    scripts\mp\utility\outline::outlinedisable( var_0, self );
}
...
laser_trap_triggers( var_0 )
{
    ...
    if ( var_1 == 0 )
        self.raritycamsmall = tablelookup( "operators.csv", 1, var_0, 23 );
    else
        self.raritycamsmall = tablelookup( "operatorquips.csv", 0, var_1, 6 );

    return self.raritycamsmall;
}

laser_traps( var_0 )
{
    self.raritycamwatch = tablelookup( "mp_cp/executiontable.csv", 1, self.loadoutexecution, 19 );
    return self.raritycamwatch;
}
...
laser_vfx_think( var_0 )
{
    var_1 = tablelookup( "operators.csv", 1, var_0, 31 );
    return int( var_1 );
}

player_exposed_to_chopper_boss( var_0 )
{
...
lethal_crate_spawn( var_0, var_1 )
{
    var_2 = 0;
...
lethal_equipmentmaskoffsets( var_0 )
{
    var_1 = 0;
...
onteamproximitybecameinvalidplayer()
{
    ...
    level.matching_correct_bomb_wire_pair = [];
    ...
    for ( var_1 = 0; var_1 < var_0; var_1++ )
    {
        if ( tablelookupbyrow( "operatorskins.csv", var_1, 18 ) != "" )
        {
            ...
            if ( !isdefined( level.matching_correct_bomb_wire_pair[var_2] ) )
            {
                level.matching_correct_bomb_wire_pair[var_2]["lootIDs"] = [];
                level.matching_correct_bomb_wire_pair[var_2]["curIndex"] = 0;
                level.matching_correct_bomb_wire_pair[var_2]["maxIndex"] = 0;
            }

            level.matching_correct_bomb_wire_pair[var_2]["lootIDs"][level.matching_correct_bomb_wire_pair[var_2]["lootIDs"].size] = int( var_3 );
            level.matching_correct_bomb_wire_pair[var_2]["maxIndex"] = level.matching_correct_bomb_wire_pair[var_2]["maxIndex"] + 1;
        }
    }
}
...
spawn_overwatch_final_tank()
{
    ...
    if ( istrue( level.onlinegame ) )
    {
        ...
        if ( isdefined( var_2 ) && var_2.size > 0 )
        {
            foreach ( var_4 in var_2 )
            {
                ...
                {
                    var_0 = var_4.team;
                    self._id_121B4 = var_4._id_121B4;
                    break;
                }
            }
        }
    ...
    if ( !isdefined( var_0 ) )
        var_0 = hostage_room_enemy_watcher( var_1 );

    if ( !isdefined( self._id_121B4 ) )
        self._id_121B4 = gettime();

    thread scripts\mp\menus::setteam( var_0 );
}

hostage_room_enemy_watcher( var_0 )
{
    if ( !isdefined( var_0 ) )
        var_0 = 1;
    ...
    foreach ( var_3 in level.teamnamelist )
    {
        ...
        if ( var_4 > 0 )
        {
            ...
            if ( isdefined( var_6[0]._id_121B4 ) && gettime() > var_6[0]._id_121B4 + 300000 )
                continue;
        }
...
_id_11BB2()
{
    return getdvarint( "scr_br_zombie_encounters", 0 ) > 0;
}
