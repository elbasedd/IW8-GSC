...
init()
{
    ...
    level.nuke_expl_struct = scripts\cp_mp\utility\game_utility::kiosksearchradiusidealmax( "nuke_expl_pos" );
}
...
tryusenukefromstruct( var_0 )
{
    ...
    else if ( !istrue( level.respawntagvisibility ) )
    ...
    if ( !istrue( var_4 ) || level.gameended )
    {
        ...
        level.setarenaomnvar = undefined;
        var_0 notify( "killstreak_finished_with_deploy_weapon" );
        return 0;
    }
    ...
    if ( var_1 != 1 && !istrue( level.respawntagvisibility ) )
        scripts\common\utility::_id_12456( level.remove_old_wheelsons, "nuke", self.origin );
...
{
    if ( !isdefined( level.set_up_rear_minigun ) )
    {
        soundsettimescalefactorfromtable( "nuke" );
        level.set_up_rear_minigun = 1;
    }

    level endon( "game_ended" );
    var_0 waittill( "disconnect" );
    set_showing_bomb_wire_pair_to_player();
}
...
nuke_startlaunchsequence( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7 )
{
    ...
    if ( var_1.streakname != "nuke_select_location" )
    {
        ...
        level thread set_scriptable_states();
    ...
    if ( isdefined( level.nukeinfo.player ) )
    {
                ...
                if ( [[ scripts\cp_mp\utility\script_utility::getsharedfunc( "nuke", "cankill" ) ]]( var_5, var_3 ) )
                {
                    if ( isplayer( var_5 ) )
                    {
                        ...
                        if ( scripts\cp_mp\utility\script_utility::issharedfuncdefined( "player", "isReallyAlive" ) )
                        {
                            ...
                            {
                                if ( !istrue( var_5.watchforentervehicle ) )
                                {
        ...
        if ( !istrue( level.respawntagvisibility ) )
        {
            if ( isdefined( level.nukeinfo.player ) && isdefined( var_1 ) )
                level.nukeinfo.player scripts\cp_mp\utility\killstreak_utility::updatesmokinggunhud( var_1 );
        }
    }
...
{
    level endon( "game_ended" );
    level thread set_ui_omnvar_for_relics( 1 );
...
set_ui_omnvar_for_relics( var_0 )
{
    level endon( "game_ended" );
...
set_up_chopper_boss()
{
    ...
    level thread set_unloadtype_at_end_path();
}
...
nuke_startnukedeathfx()
{
    ...
    if ( !istrue( self.watchforentervehicle ) )
    {
