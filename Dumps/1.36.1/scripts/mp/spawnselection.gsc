...
init()
{
    ...
    scripts\mp\utility\spawn_event_aggregator::registeronplayerspawncallback( ::player_give_killstreak );
}
...
getspawnpoint()
{
    ...
    if ( isdefined( self.selectedspawnarea ) )
    {
        if ( issubstr( self.selectedspawnarea, "squad" ) )
        {
            ...
            if ( isdefined( var_3.vehicle ) )
            {
                ...
                self._id_127E6 = var_3.vehicle.vehiclename;
            }
    ...
    if ( istrue( level.usesquadspawn ) && istrue( self.squadspawnconfirmed ) )
    {
        ...
        if ( isdefined( var_14 ) && isdefined( self.squadindex ) && self.team == var_14.team && self.squadindex == var_14.squadindex )
        {
            ...
            var_14 thread scripts\mp\utility\points::little_bird_mg_deletenextframe( "squad_spawn" );
            thread scripts\mp\utility\points::little_bird_mg_deletenextframe( "squad_spawn_self" );
            self._id_12988 = 1;
        }
    }
...
refreshplayerspawnareaomnvars()
{
    ...
    thread handleweaponreloadammodrop();
    ...
    for (;;)
    {
        ...
        foreach ( var_3 in level.availablespawnlocations[self.team] )
        {
            ...
            if ( isdefined( var_6.objectivekey ) )
                var_8 = istrue( var_6.new_col_map );
            else if ( issubstr( var_3, "vehicle" ) )
            {
                ...
                {
                    var_8 = istrue( var_11.new_col_map );
                    var_9 = mark_phone_guy_once_all_spawned( var_11, self );
...
{
    return level.objectives[var_0].ownerteam != var_1 || level.objectives[var_0].stalemate || level.objectives[var_0].captureblocked || istrue( level.objectives[var_0]._id_11E3E ) || level.objectives[var_0].claimteam != "none" && level.objectives[var_0].claimteam != var_1;
}
...
_id_127D2( var_0 )
{
    ...
    {
        var_2 = gettime();
        var_3 = istrue( var_0.closenukecrate );
        var_4 = scripts\cp_mp\utility\weapon_utility::islockedonto( var_0 );
        var_5 = scripts\cp_mp\vehicles\vehicle_occupancy::_id_12767( var_0 );
...
mark_phone_guy_once_all_spawned( var_0, var_1 )
{
...
spawnselectionthink( var_0 )
{
    ...
    if ( istrue( level._id_11C24 ) )
        thread scripts\mp\gametypes\arm.gsc::_id_127A7();

    if ( istrue( level._id_11C23 ) )
        thread scripts\mp\gametypes\arm.gsc::_id_127A8();
...
watchforafk()
{
    ...
    if ( isdefined( level._id_11E51 ) )
        wait( level._id_11E51 );
    else
        wait 60.0;
    ...
    for (;;)
    {
        ...
        if ( var_0 == "tac_ops_spawn_area_selected" )
        {
            ...
            foreach ( var_4 in level.spawnselectionlocations )
            {
                ...
                {
                    _id_125FA( var_4[self.team].typeref, self, self.team );

                    if ( isdefined( var_4[self.team].objectivekey ) && isobjectiveindanger( var_4[self.team].objectivekey, self.team ) )
                        break;

                    if ( isdefined( var_4[self.team].typeref ) && issubstr( var_4[self.team].typeref, "vehicle" ) && istrue( var_4[self.team].dynamicent.new_col_map ) )
                        break;
            ...
            if ( !isdefined( var_2 ) )
            {
                var_6 = lb_mg_dmg_factor_landing_gear( self.team, self.squadindex );
                ...
                foreach ( var_4 in var_6 )
                {
                    if ( isdefined( var_4.dynamicent ) && var_4.dynamicent getentitynumber() == var_1 )
                    {
                        ...
                        var_9 = spawnstruct();
                        var_9 = scripts\mp\spawnscoring::createdevguientryforkidnapper( var_9, var_4.dynamicent );

                        if ( !isdefined( var_9._id_11E41 ) )
                        {
                            var_4.dynamicent.set_relic_fastbleedout = gettime();
                            break;
                        }
...
_id_125EB( var_0 )
{
    if ( !isdefined( self.lootcachesearchparams ) )
        self.lootcachesearchparams = [];

    var_1 = 65536;
    var_2 = 131072;
    var_3 = 262144;
    var_4 = 524288;
    self.lootcachesearchparams["rightmouseup"] = istrue( self.lootcachesearchparams["rightmouseup"] ) || ( var_0 & var_1 ) > 0;
    self.lootcachesearchparams["leftmouseup"] = istrue( self.lootcachesearchparams["leftmouseup"] ) || ( var_0 & var_2 ) > 0;
    self.lootcachesearchparams["activate"] = istrue( self.lootcachesearchparams["activate"] ) || ( var_0 & var_3 ) > 0;
    self.lootcachesearchparams["gostand"] = istrue( self.lootcachesearchparams["gostand"] ) || ( var_0 & var_4 ) > 0;
...
watchendconditions()
{
    ...
    var_0 = scripts\engine\utility::_id_12912( "tac_ops_map_selection_valid", "tac_ops_map_changed", "tac_ops_map_cleared", "spawned_player", "tac_ops_map_game_ended", "tac_ops_map_nuke" );
...
endcondition_focuschanged( var_0 )
{
    ...
    foreach ( var_3 in level.spawnselectionlocations )
    {
        ...
        {
            thread playershowarenastartobjectivetext( var_3[self.team].anchorentity.origin );
    ...
    if ( !isdefined( var_1 ) )
    {
        var_5 = lb_mg_dmg_factor_landing_gear( self.team, self.squadindex );

        foreach ( var_3 in var_5 )
        {
            if ( isdefined( var_3.dynamicent ) && var_3.dynamicent getentitynumber() == var_0 )
            {
                thread playershowarenastartobjectivetext( var_3.dynamicent.origin );
                var_1 = var_3.typeref;
                break;
            }
        }
    ...
    if ( issubstr( var_1, "vehicle" ) )
    {
        ...
        thread relic_mythic_modifyplayerdamage( var_1 );
        var_8 = tablelookuprownum( "mp/spawnSelectionMapData.csv", 0, var_1 );
        self setclientomnvar( "ui_tom_previous_selection", var_8 );
        return;
    }
...
playershowarenastartobjectivetext( var_0 )
{
    ...
    if ( !isdefined( level._id_12AD7 ) )
        level._id_12AD7 = max( getdvarint( "NOLPSOTORP", 20000 ) * 0.5 / 1000, 1.0 );

    for (;;)
    {
        self predictstreampos( var_0, 1 );
        wait( level._id_12AD7 );
    }
}
...
lb_mg_dmg_factor_landing_gear( var_0, var_1 )
{
    var_2 = [];
...
evaluatesquadspawn()
{
    ...
    for ( var_4 = 0; var_4 < 4; var_4++ )
    {
        ...
        var_5.mp_m_speedball_patch = undefined;
        var_3[var_4] = var_5;
    }
    ...
    for (;;)
    {
        ...
        var_11 = lb_mg_dmg_factor_landing_gear( self.team, self.squadindex );
        ...
        for ( var_4 = 0; var_4 < 4; var_4++ )
        {
            ...
            {
                var_15 = istrue( var_9.new_col_map );
                var_16 = var_1[var_4];
                ...
                if ( !isdefined( var_3[var_4].mp_m_speedball_patch ) || var_3[var_4].mp_m_speedball_patch != var_16 )
                {
                    var_3[var_4].mp_m_speedball_patch = var_16;
            ...
            if ( isdefined( var_3[var_4].mp_m_speedball_patch ) && var_3[var_4].mp_m_speedball_patch != -1 )
            {
                var_3[var_4].mp_m_speedball_patch = -1;
...
handlesquadspawncycle()
{
    ...
    for (;;)
    {
        var_0 = scripts\engine\utility::_id_12907( "cyclePos", "cycleNeg" );
...
player_give_killstreak()
{
    thread path_data_ordered();

    if ( getdvarint( "scr_squad_spawn_debug", 0 ) == 1 )
        thread scripts\mp\spawnscoring::_id_11EBF();
}
...
path_data_ordered()
{
    ...
    for (;;)
    {
        if ( issquadspawnable( self ) )
            self.new_col_map = issquadmateindanger( self );
...
issquadmateindanger( var_0 )
{
    ...
    if ( isdefined( var_0.prestreaminglocation ) && var_0.prestreaminglocation + 3000 > var_1 )
        return 1;
    ...
    {
        if ( istrue( var_0.vehicle.closenukecrate ) )
            return 1;
    ...
    if ( isdefined( var_0 ) && isdefined( var_0.set_relic_fastbleedout ) && var_0.set_relic_fastbleedout + 1000 > var_1 )
        return 1;
...
timeuntilspawnmessaging( var_0, var_1 )
{
    ...
    if ( var_0 > 0 )
    {
        ...
        scripts\engine\utility::_id_12919( var_0, "force_spawn" );
    }
}
...
getclosestavailablespawnlocation()
{
    ...
    else
    {
        ...
        {
            var_7 = lb_mg_dmg_factor_landing_gear( self.team, self.squadindex );
        ...
        foreach ( var_1 in level.teamnamelist )
        {
            foreach ( var_3 in level.availablespawnlocations[var_1] )
            {
                ...
                if ( isdefined( var_4.objectivekey ) )
                    var_4.new_col_map = isobjectiveindanger( var_4.objectivekey, var_1 );
            }
        }
        ...
        foreach ( var_1 in level.teamnamelist )
        {
            ...
            foreach ( var_4 in level.availablespawnlocations[var_1] )
            {
                if ( !issubstr( var_4, "HQ" ) && issubstr( var_4, "fob" ) )
                {
                    ...
                    level.objectives[var_17]._id_11E3E = var_14;
                    level.objectives[var_17]._id_11E3F = gettime();
                }
            }
        }
...
_id_125FA( var_0, var_1, var_2 )
{
    var_3 = level.spawnselectionlocations[var_0][var_2].objectivekey;

    if ( !isdefined( var_3 ) || !isdefined( level.objectives[var_3] ) || !isdefined( level.objectives[var_3]._id_11E3F ) || level.objectives[var_3]._id_11E3F == gettime() )
        return;
    ...
    level.objectives[var_3]._id_11E3E = var_13;
    level.objectives[var_3]._id_11E3F = gettime();
}
