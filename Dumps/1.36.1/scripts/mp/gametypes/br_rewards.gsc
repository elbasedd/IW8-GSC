...
transitionplayersoutofac130cinematic( var_0 )
{
    ...
    if ( isdefined( level.br_level ) )
    {
        if ( var_4 > 0 )
            ...
            var_21 = computedropbagpositions( var_2, level.dropbagstruct.clusters, level.br_level.br_mapbounds, var_18, var_19, 2000, var_17, 128 );
    }
    ...
    if ( isdefined( var_21 ) )
    {
        for ( var_22 = 0; var_22 < var_21.size; var_22++ )
        {
            ...
            if ( isdefined( var_21[var_22].node ) )
            {
                ...
                var_11.transition_to_next_section = var_24.droplocations[var_25].origin;
                continue;
            }

            var_11.transition_to_next_section = var_21[var_22].origin;
        }

        level.transitionac130tomovinganim = 1;
        level thread _id_11E04( var_1 );
    }

    level thread relic_healthpacks_think();
}
...
_id_11E03( var_0, var_1 )
{
    destroy_vehicles( self.team, 1 );
    var_2 = 4096;

    if ( istrue( self.play_nags_from_array ) )
        var_2 = 10000;

    var_3 = scripts\cp_mp\killstreaks\airdrop::dropbrloadoutcrate( self.team, var_0 + ( 0, 0, var_2 ), var_0 + ( 0, 0, 512 ) );
    var_3 endon( "death" );
    _id_122D0( var_3 );
    enabledropbagobjective( var_3 );
    defendkill( var_3 );
    var_4 = [];

    foreach ( var_6 in level.teamdata[self.team]["alivePlayers"] )
    {
        if ( isdefined( var_6 ) && !var_6 scripts\mp\gametypes\br_public.gsc::isplayeringulag() )
        {
            var_4[var_4.size] = var_6;
            var_6 thread scripts\mp\hud_message::showsplash( "br_airdrop_incoming" );
        }
    }

    if ( !scripts\mp\gametypes\br_public.gsc::playerexecutionsenable() )
        scripts\mp\gametypes\br_public.gsc::choosebestpropforkillcam( "dropbag_incoming", self.team, 1 );

    if ( scripts\mp\utility\game::getgametype() == "br" )
    {
        scripts\mp\gametypes\br_analytics.gsc::carrier_cleanup( self, var_0, var_1, var_3 );
        thread lossendgame( var_3 );
    }
}
...
lossendgame( var_0 )
{
    ...
    while ( isdefined( var_0 ) )
    {
        ...
        if ( abs( var_2[2] ) < 0.01 )
        {
            ...
            if ( var_5["fraction"] == 1 )
                var_0 scripts\cp_mp\killstreaks\airdrop::dropoff_sound_playerwm_handler( var_0.origin, ( 0, 0, -10 ) );
            else
                break;
        }
...
_id_11E04( var_0 )
{
    foreach ( var_2 in var_0 )
    {
        ...
        var_3 = var_2.transition_to_next_section;
        ...
        if ( isdefined( level.br_circle ) && isdefined( level.br_circle.safecircleent ) )
        {
            ...
            else if ( istrue( level._id_12659 ) && isnavmeshloaded() )
                var_3 = getclosestpointonnavmesh( var_3 );

            if ( istrue( level._id_12642 ) )
                var_3 = lastactivateinstruct( var_3 );

            if ( scripts\mp\outofbounds::ispointinoutofbounds( var_3 ) )
            {
                var_4 = scripts\mp\gametypes\br_circle.gsc::getsafecircleorigin();
                var_7 = scripts\mp\gametypes\br_c130.gsc::_id_11CA8( var_4, var_3 );
        ...
        var_8 = keepviewingthe747( 1, 0, 0, 0, 0 );
        var_2 _id_11E03( var_3, var_8 );
        waitframe();
    }
}

lastactivateinstruct( var_0 )
{
    if ( isdefined( level.getlowermessageindex ) )
    {
        if ( !player_is_pressing_attack( var_0, level.getlowermessageindex ) )
        {
            var_1 = juggernaut_kills_tracker( var_0, level.getlowermessageindex );
...
player_is_pressing_attack( var_0, var_1 )
{
...
juggernaut_kills_tracker( var_0, var_1 )
{
    var_2 = undefined;
    var_3 = undefined;
...
spawndropbagonlanding()
{
    ...
    if ( isdefined( var_0 ) )
    {
        var_3 = keepviewingthe747( 0, 0, 0, 1, 0 );
        _id_11E03( var_0.origin, var_3 );
        level thread relic_healthpacks_think();
    }
}
...
_id_122D0( var_0 )
{
    level thread _id_122D1( var_0, self );
}
...
force_var_to_array( var_0 )
{
    ...
    foreach ( var_3 in var_0 )
    {
        ...
        var_1 = var_3;
        transitionplayersoutofac130cinematic( 0 );
    }
}

audio_player_start_mud_loop()
{
    if ( scripts\mp\gametypes\br_gametypes.gsc::play_station_closed_vo( "dropbag" ) )
        return;
...
keepviewingthe747( var_0, var_1, var_2, var_3, var_4 )
{
    if ( istrue( var_0 ) )
        return 0;
