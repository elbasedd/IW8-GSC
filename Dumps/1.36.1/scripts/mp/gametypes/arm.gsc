...
main()
{
    ...
    level.giveachievementpilotkill = getdvarint( "scr_arm_enableLittleBirdMG", 0 );
    ...
    level._id_1215C = getdvarint( "scr_arm_tankRespawnTime", 120 );
    level.binoculars_cansee = getdvarint( "scr_arm_apcRespawnTime", 120 );
    level.longdialoguecooldown = getdvarint( "scr_arm_longDialogueCooldown", 1 );
    level.respawntags = getdvarint( "scr_arm_mercyEndingNuke", 1 );
    level.respawntagsfreed = getdvarint( "scr_arm_mercyEndingTime", 30 );
    level.setherodropscriptable = getdvarint( "scr_arm_numNonRallyVehicles", 25 );
    level.get_fake_digit_from_pool = getdvarint( "scr_arm_disablePersonalNuke", 0 );
    level.spawn_overwatch_soldiers_01 = getdvarint( "scr_personalNukeCostOverride", 30 );
    level._id_11C24 = getdvarint( "scr_arm_showTeamTanks", 1 );
    level._id_11C23 = getdvarint( "scr_arm_showTeamLittleBirds", 1 );
    level._id_11E51 = getdvarint( "scr_arm_spawnSelectionAFKTime", 120 );
    level.relic_punchbullets_fire_fists = getdvarint( "scr_arm_manualAdjustLittleBirdLocs", 1 );
    level.getactiveteamcount = getdvarint( "scr_arm_dontSpawnJeep", 0 );
    level.getactiveforteam = getdvarint( "scr_arm_dontSpawnCargoTruck", 0 );
    level._id_1265B = getdvarint( "scr_arm_usePingSystem", 1 );

    if ( istrue( level._id_11C24 ) )
    {
        level._id_122D3 = [];
        level._id_122D3["axis"] = [];
        level._id_122D3["allies"] = [];
    }

    if ( istrue( level._id_11C23 ) )
    {
        level._id_122D2 = [];
        level._id_122D2["axis"] = [];
        level._id_122D2["allies"] = [];
        level._id_122D2["untouched"] = [];
    }
...
onstartgametype()
{
    ...
    if ( istrue( level.respawntags ) )
        level thread set_track_operational_status();

    gettruegroundposition();

    if ( istrue( level._id_1265B ) )
...
gettruegroundposition()
{
    ...
    scripts\cp_mp\utility\game_utility::vehicle_occupancy_mp_updateriotshield( "vehicle_volume", "script_noteworthy" );
    scripts\cp_mp\utility\game_utility::vehicle_occupancy_mp_updateriotshield( "vehicle_volume_simplified", "script_noteworthy" );
    scripts\cp_mp\utility\game_utility::vehicle_occupancy_mp_updateriotshield( "super", "script_noteworthy" );
    scripts\cp_mp\utility\game_utility::vehicle_occupancy_mp_updateriotshield( "militarybase", "script_noteworthy" );
    scripts\cp_mp\utility\game_utility::vehicle_occupancy_mp_updateriotshield( "location_volume", "targetname" );
    scripts\cp_mp\utility\game_utility::vehicle_occupancy_mp_updateriotshield( "locale_area_trigger", "targetname" );
    scripts\cp_mp\utility\game_utility::vehicle_occupancy_mp_updateriotshield( "shadow_blocker", "targetname" );
    scripts\cp_mp\utility\game_utility::vehicle_occupancy_setfriendlystatusdirty( "door_prison_cell_metal_mp", 1 );
    scripts\cp_mp\utility\game_utility::vehicle_occupancy_setfriendlystatusdirty( "veh8_mil_air_acharlie130", 1 );
    ...
    scripts\cp_mp\utility\game_utility::vehicle_occupancy_mp_takeriotshield( "light" );
    scripts\cp_mp\utility\game_utility::vehicle_occupancy_mp_takeriotshield( "trigger_use_touch" );

    if ( isdefined( level.localeid ) && level.localeid == "locale_6" )
        scripts\cp_mp\utility\game_utility::vehicle_occupancy_mp_updateriotshield( "locale_8", "script_noteworthy" );
...
updatedomscores()
{
    ...
    level thread scripts\mp\spawnselection::_id_125CB();
...
updatescores()
{
    ...
    var_5 = scripts\mp\gamescore::connect_circlar_path();
    ...
    var_8 = scripts\mp\gamescore::connect_circlar_path();

    if ( var_5 != var_8 )
        scripts\mp\gamescore::toggle_in_use( var_8, 1, var_5 );
}
...
runobjectives( var_0 )
{
    ...
    if ( istrue( level.useobjectives ) )
    {
        ...
        foreach ( var_2 in level.allfobs )
        {
            ...
            if ( istrue( level.init_subway_cars ) )
                var_2._id_11E6A = scripts\mp\gametypes\obj_dom.gsc::_id_11E6B;
    ...
    hackfixcameras();
    level thread audio_player_spawn_mud_loop();
}
...
objective_manageobjectivesintrovisibility()
{
    ...
    scripts\mp\objidpoolmanager::objective_playermask_hidefromall( level.gw_objstruct.axishqloc.gunbutt.objidnum );
    scripts\mp\objidpoolmanager::objective_playermask_hidefromall( level.gw_objstruct.allieshqloc.gunbutt.objidnum );
...
sethqmarkerobjective()
{
    ...
    var_2.lockupdatingicons = 1;
    level.gw_objstruct.axishqloc.gunbutt = var_2;
    level.uncapturableobjectives[level.uncapturableobjectives.size] = var_2;
    ...
    scripts\mp\objidpoolmanager::update_objective_setbackground( var_2.objidnum, 7 );
    var_2.lockupdatingicons = 1;
    level.gw_objstruct.allieshqloc.gunbutt = var_2;
    level.uncapturableobjectives[level.uncapturableobjectives.size] = var_2;
...
spawnselection_showenemyhq()
{
    ...
    if ( self.team == "allies" )
    {
        ...
        scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( level.gw_objstruct.axishqloc.gunbutt.objidnum, self );
    }
    else
    {
        scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( level.gw_objstruct.allieshqloc.gunbutt.objidnum, self );
    ...
    if ( self.team == "axis" )
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( level.gw_objstruct.allieshqloc.gunbutt.objidnum, self );
    else
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( level.gw_objstruct.axishqloc.gunbutt.objidnum, self );
}
...
docratedropsmoke( var_0, var_1, var_2 )
{
    ...
    if ( isdefined( var_0 ) )
        var_0 scripts\engine\utility::_id_12913( var_2, "crate_dropped" );
    else
        wait( var_2 );
...
set_track_operational_status()
{
    ...
    if ( istrue( level.useobjectives ) )
        level thread set_train_stopped();
}
...
set_train_stopped()
{
    ...
    for (;;)
    {
        if ( computerrebootused( "axis" ) == level.objectives.size )
            level thread set_up_minigun( "axis" );
        else if ( computerrebootused( "allies" ) == level.objectives.size )
            level thread set_up_minigun( "allies" );

        waitframe();
    }
}

set_up_minigun( var_0 )
{
    ...
    setomnvar( "ui_nuke_timer_time", level.respawntagsfreed );
    setomnvar( "ui_nuke_countdown_active", 1 );
    var_1 = 0;
    var_2 = gettime();
    var_3 = level.respawntagsfreed * 1000 + var_2;
    setomnvar( "ui_nuke_end_milliseconds", level.respawntagsfreed * 1000 + var_2 );

    while ( computerrebootused( var_0 ) == level.objectives.size )
    {
        waitframe();

        if ( gettime() > var_3 )
        {
            set_up_rear_spotlight( var_0 );
...
set_up_rear_spotlight( var_0 )
{
    ...
    foreach ( var_5 in level.players )
    {
        ...
        {
            if ( isdefined( var_5.silo_door_left ) )
            {
                var_5 clearsoundsubmix( "iw8_mp_spawn_camera" );
                var_5.silo_door_left stoploopsound( var_5.silo_door_right );
                var_5.silo_door_left delete();
                var_5.silo_door_left = undefined;
                var_5.silo_door_right = undefined;
            }
        }
    }
    ...
    if ( isdefined( level.teamdata[var_0]["alivePlayers"][0] ) )
    {
        ...
        var_7 scripts\mp\trials\mp_trl_boneyard_gw_race::tryusenuke();
    }
    else
...
{
    thread scripts\mp\spawnselection::usbserver( var_0.team, var_0.squadindex );
}
...
parachute( var_0, var_1 )
{
    ...
    self playershow();
    self.plotarmor = 0;
    scripts\mp\utility\game::_id_11AB7( self, 0 );
...
init_groundwarvehicles()
{
    ...
    foreach ( var_1 in level.vehicleinfo )
    {
        ...
        foreach ( var_9 in var_1.vehiclespawns )
        {
            if ( isdefined( scripts\cp_mp\utility\game_utility::getlocaleid() ) && isdefined( var_9.script_noteworthy ) && var_9.script_noteworthy == level.localeid )
            {
                ...
                if ( istrue( level.getactiveteamcount ) && var_1.refname == "jeep" || istrue( level.getactiveforteam ) && var_1.refname == "cargo_truck" )
                    continue;
                else
                {
    ...
    var_28 = level.setherodropscriptable;
...
_id_126E7( var_0, var_1 )
{
    ...
    var_0._id_11E52 = var_2;
    thread _id_127BD( var_0 );
    level._id_122D3[var_1][level._id_122D3[var_1].size] = var_0;
}

_id_127BD( var_0 )
{
    var_1 = var_0._id_11E52.objidnum;
    var_2 = var_0.team;
    var_0 waittill( "death" );
    scripts\mp\objidpoolmanager::returnobjectiveid( var_1 );
    level._id_122D3[var_2] = scripts\engine\utility::array_remove( level._id_122D3[var_2], var_0 );
}
...
_id_127A7()
{
    ...
    foreach ( var_1 in level._id_122D3[self.team] )
        scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( var_1._id_11E52.objidnum, self );

    while ( self.inspawnselection )
        waitframe();

    foreach ( var_1 in level._id_122D3["axis"] )
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( var_1._id_11E52.objidnum, self );

    foreach ( var_1 in level._id_122D3["allies"] )
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( var_1._id_11E52.objidnum, self );
}
...
_id_126E6( var_0, var_1 )
{
    ...
    var_0._id_11E52 = var_2;
    thread _id_127BC( var_0 );
    level._id_122D2["untouched"][level._id_122D2["untouched"].size] = var_0;
}
...
_id_127BC( var_0 )
{
    ...
    var_0 waittill( "death" );
    scripts\mp\objidpoolmanager::returnobjectiveid( var_1 );
    var_2 = var_0.prepdoorsforunload;

    if ( !isdefined( var_0.prepdoorsforunload ) )
        var_2 = "untouched";

    level._id_122D2[var_2] = scripts\engine\utility::array_remove( level._id_122D2[var_2], var_0 );
}
...
_id_127A8()
{
    ...
    foreach ( var_1 in level._id_122D2[self.team] )
        scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( var_1._id_11E52.objidnum, self );

    foreach ( var_1 in level._id_122D2["untouched"] )
        scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( var_1._id_11E52.objidnum, self );

    while ( self.inspawnselection )
        waitframe();

    foreach ( var_1 in level._id_122D2["axis"] )
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( var_1._id_11E52.objidnum, self );

    foreach ( var_1 in level._id_122D2["allies"] )
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( var_1._id_11E52.objidnum, self );
...
_id_1279A( var_0 )
{
    if ( isdefined( self.prepdoorsforunload ) )
        level._id_122D2[self.prepdoorsforunload] = scripts\engine\utility::array_remove( level._id_122D2[self.prepdoorsforunload], self );
    else
        level._id_122D2["untouched"] = scripts\engine\utility::array_remove( level._id_122D2["untouched"], self );

    self.prepdoorsforunload = var_0;
    level._id_122D2[var_0][level._id_122D2[var_0].size] = self;
    scripts\mp\objidpoolmanager::update_objective_icon( self._id_11E52.objidnum, "icon_minimap_littlebird_spawn_selection" );

    foreach ( var_2 in level.players )
    {
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( self._id_11E52.objidnum, var_2 );

        if ( isdefined( var_2 ) && isdefined( var_2.team ) && var_2.team == var_0 && istrue( var_2.inspawnselection ) )
        {
            scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( self._id_11E52.objidnum, var_2 );
            continue;
        }
...
init_rallyvehicles()
{
    ...
    if ( !istrue( level.disablelittlebirdrally ) )
    {
        foreach ( var_2 in var_0 )
        {
            ...
            if ( level.giveachievementpilotkill )
                var_6 = _calloutmarkerping_predicted_timeout::race_is_player_driving_vehicle( var_4, var_5 );
            else
    ...
    {
        if ( level.localeid == "locale_6" && level.mapname == "mp_downtown_gw" && istrue( level.relic_punchbullets_fire_fists ) )
        {
            var_10 = [];
        ...
        foreach ( var_2 in var_0 )
        {
            ...
            level thread _id_126E6( var_6, "neutral" );
        }
    }
...
arm_initoutofbounds()
{
    ...
    foreach ( var_2 in var_0 )
    {
        if ( isdefined( scripts\cp_mp\utility\game_utility::getlocaleid() ) && isdefined( var_2.script_noteworthy ) && var_2.script_noteworthy == level.localeid && scripts\mp\utility\game_utility_mp::run_common_functions_solider_stealth( var_2 ) )
        {
