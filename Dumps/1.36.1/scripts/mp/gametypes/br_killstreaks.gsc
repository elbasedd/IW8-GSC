...
extraction_balloon_total_plunder( var_0, var_1, var_2, var_3 )
{
    ...
    foreach ( var_6 in var_4 )
    {
        if ( isdefined( var_6 ) && scripts\mp\utility\player::isreallyalive( var_6 ) && var_6.team != self.team )
            extraction_balloon_num_completed( var_6, var_2, var_3 );
    }
}

extraction_balloon_num_completed( var_0, var_1, var_2, var_3 )
{
    if ( isdefined( level.extracting ) && isdefined( level.extractingplayers ) )
    {
        if ( isdefined( level.extractingplayers[var_1] ) && isdefined( level.extractingplayers[var_1][var_0.guid] ) && gettime() < level.extractingplayers[var_1][var_0.guid] )
            var_2 = 0;
        else
            level.extractingplayers[var_1][var_0.guid] = gettime() + level.extracting * 1000;
    }

    if ( istrue( var_2 ) && scripts\mp\utility\killstreak::getkillstreakenemyusedialogue( var_1 ) )
    {
        var_4 = scripts\mp\gametypes\br_public.gsc::checked_missiles( self );
    ...
    if ( var_1 > 0 )
    {
        ...
        extraction_helicoptor_num_completed( var_0, var_3 );
    }
}

extraction_helicoptor_num_completed( var_0, var_1 )
{
    var_2 = var_0 getxuid();
    ...
    level thread _id_129D6( var_0, var_2, "death_or_disconnect" );
    level thread _id_129D6( var_0, var_2, undefined, var_1 );
}
