...
createspawnweaponatpos( var_0, var_1, var_2 )
{
    ...
    var_5 = scripts\mp\gametypes\br_pickups.gsc::killstreak_additional_targets( var_0, var_1 );
...
createspawnweaponatposfromname( var_0, var_1 )
{
    ...
    var_4 = scripts\mp\gametypes\br_pickups.gsc::killstreak_additional_targets( var_0, ( 0, 0, 90 ) );
...
playerdropweaponfrominventory( var_0 )
{
    ...
    if ( isdefined( var_8 ) )
    {
        ...
        var_14 = scripts\mp\gametypes\br_pickups.gsc::onsquadeliminatedplacement();
...
weaponspawn( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    level.showing_ui_record scripts\mp\trials\mp_trl_create_a_script_race_euphrates::controlledcallbacks( var_6, var_1, var_0 );
    return var_6;
}
...
takeweaponpickup( var_0 )
{
    ...
    if ( !isdefined( var_0.weapon ) )
    {
        ...
        {
            var_3 = scripts\mp\gametypes\br_pickups.gsc::keypad_input_clear( var_0 );
        ...
        if ( !isdefined( var_4 ) )
        {
            scripts\mp\utility\script::fusefx( "takeWeaponPickup error - scriptable not setup for pickup: " + var_0.scriptablename );
            return;
        }
    ...
    {
        if ( isnullweapon( var_11 ) )
        {
            var_1--;
            continue;
        }

        if ( issameweapon( var_11, var_3 ) )
        {
            var_9 = 1;
            var_7 = 1;
            var_8 = var_11;
    ...
    if ( var_7 )
    {
        ...
        if ( var_8.basename != "none" )
        {
            ...
            if ( var_8.hasalternate )
            {
                ...
                if ( !can_trex_apply_to_player( var_8, var_16 ) )
            ...
            var_21 = !scripts\mp\gametypes\br_extract_quest.gsc::heli_fire_at_players( var_8 );

            if ( var_21 )
            {
                var_22 = scripts\mp\gametypes\br_pickups.gsc::killstreak_additional_targets( var_0.origin, ( 0, var_20, 0 ) );
                var_23 = vehicle_getarrayinradius( var_22.origin, 500, 500 );
                ...
                if ( var_28["fraction"] < 1.0 )
                    var_22.loadoutprimaryaddblueprintattachments = var_28["entity"];

                var_29 = weaponspawn( var_8, self, var_22, 0 );

                if ( isdefined( var_29 ) )
                {
                    scripts\mp\gametypes\br_pickups.gsc::regendelayspeedfunc( var_29, var_13, var_14, var_15 );
                    var_30 = 1;
    ...
    {
        var_33 = var_0.count;
        var_34 = var_0.drop_new_carepackage;
        var_32 = weaponclipsize( var_3 );
        var_35 = 0;
        ...
        {
            var_36 = var_0.drop_jugg_suit_on_death;
            var_37 = var_3 getaltweapon();

            if ( !can_trex_apply_to_player( var_3, var_37 ) )
            {
    ...
    var_3.vo_jug_wait_to_be_seen = undefined;
    br_ammo_update_weapons( self );
    ...
    {
        if ( isweapon( var_41 ) && var_41.inventorytype == "primary" )
        {
            var_4 = getcompleteweaponname( var_41 );
    ...
    if ( isdefined( var_0.parachute_set_spawn_values ) )
        level.showintelinstancetoplayer scripts\mp\trials\mp_trl_create_a_script_race_euphrates::controlledcallbacks( var_0.parachute_set_spawn_values, self, var_3 );
}
...
br_forcegiveweapon( var_0, var_1, var_2 )
{
    ...
    var_3.count = 0;
    var_3.drop_new_carepackage = 0;
    var_3.drop_jugg_suit_on_death = 0;
    var_1 takeweaponpickup( var_3 );
}

care_pkg( var_0, var_1 )
{
    if ( !isdefined( var_0 ) )
        var_0 = "ar";
...
br_forcegivecustomweapon( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    ...
    var_7.drop_new_carepackage = 0;
    var_7.drop_jugg_suit_on_death = 0;
    ...
    if ( scripts\mp\utility\weapon::platform_cleanup_on_death( var_1 ) )
        var_7.drop_new_carepackage = var_7.count;

    if ( var_1.hasalternate )
    {
        var_10 = var_1 getaltweapon();

        if ( !can_trex_apply_to_player( var_1, var_10 ) )
        {
            var_11 = weaponclipsize( var_10 );
            var_7.drop_jugg_suit_on_death = var_11;

            if ( var_6 )
                var_7.drop_jugg_suit_on_death = 999;
        }
    }
    ...
    for (;;)
    {
        ...
        if ( !scripts\mp\flags::gameflag( "prematch_done" ) )
        {
            ...
            foreach ( var_2 in var_0 )
            {
                ...
                if ( isdefined( var_3 ) )
                {
                    if ( scripts\mp\utility\weapon::platform_cleanup_on_death( var_2 ) )
                    {
...
carepackage_drop_from_heli()
{
    if ( scripts\mp\gametypes\br_gametypes.gsc::play_station_closed_vo( "weapons" ) )
        return;
...
br_ammo_player_hud_monitor()
{
    ...
    for (;;)
    {
        scripts\engine\utility::_id_12902( "ammo_update", "pickedupweapon", "weapon_switch_done", "weapon_change", "weapon_change_complete" );
        br_ammo_player_hud_update_ammotype( "brloot_ammo_919" );
        br_ammo_player_hud_update_ammotype( "brloot_ammo_12g" );
        br_ammo_player_hud_update_ammotype( "brloot_ammo_762" );
        br_ammo_player_hud_update_ammotype( "brloot_ammo_50cal" );
...
can_trex_apply_to_player( var_0, var_1 )
{
...
br_ammo_player_reload_watch()
{
    ...
    for (;;)
    {
        ...
        if ( !getdvarint( "scr_prematch_infinite_ammo", istrue( level.trap_room_dogtag_revive ) ) || scripts\mp\flags::gameflag( "prematch_done" ) )
            self.br_ammo[var_1] = self getweaponammostock( var_0 );

        cancelgivequest( self, var_1 );
    }
}
...
carepackage_get_dropped_entities( var_0, var_1 )
{
    var_2 = self;
...
{
    if ( istrue( level.can_spawn_extras ) && !scripts\mp\gametypes\br_public.gsc::play_player_approach() )
        return 1;
}
...
{
    foreach ( var_2 in var_0 )
        random_barrel_explosion( var_2, istrue( var_2.maphint_cheesescriptableused ) );
}
