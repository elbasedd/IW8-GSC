...
main()
{
    ...
    if ( _calloutmarkerping_predicted_isanypingactive::_id_123AE() )
    ...
    level.audio_fire_loop_start = var_0;
    ...
    switch ( level.trial["missionScript"] )
    {
        case "gun_nonlinear":
        case "gun":
            thread [[ level._id_123B3["gun"] ]]();
            break;
        case "clear":
            thread [[ level._id_123B3["clear"] ]]();
            break;
        case "sniper":
            thread [[ level._id_123B3["sniper"] ]]();
            break;
        case "jugg":
            thread [[ level._id_123B3["jugg"] ]]();
            break;
        case "lava":
            thread [[ level._id_123B3["lava"] ]]();
            break;
        case "pitcher":
            thread [[ level._id_123B3["pitcher"] ]]();
            break;
        case "gunslinger":
            thread [[ level._id_123B3["gunslinger"] ]]();
            break;
        case "race":
            thread [[ level._id_123B3["race"] ]]();
            break;
        case "arm_course":
            thread [[ level._id_123B3["arm_course"] ]]();
            break;
        default:
            break;
    }

    _calloutmarkerping_iskiosk::init();
...
_id_123FA( var_0, var_1 )
{
    level notify( "exitLevel_called" );
    processlobbydata();

    if ( isdefined( level._id_12394 ) )
        [[ level._id_12394 ]]();
...
trial_mission_data_init()
{
    var_0 = _calloutmarkerping_predicted_isanypingactive::_id_123A4();
...
getspawnpoint()
{
    ...
    if ( isdefined( level._id_123CB ) )
    {
        var_3 = scripts\cp_mp\vehicles\vehicle_occupancy::vehicle_occupancy_getavailablevehicleseats( level._id_123CB, 1 );
        var_4 = spawnstruct();
        var_4.useonspawn = 1;
        var_4.enterstartwaitmsg = "spawned_player";
        thread scripts\cp_mp\vehicles\vehicle_occupancy::vehicle_occupancy_enter( level._id_123CB, var_3[0], self, var_4 );
...
onplayerconnect( var_0 )
{
    ...
    level.player scripts\mp\gametypes\br.gsc::submap_cull_targetname();
    wait 1;
...
trial_end_score_dialogue()
{
    ...
    _calloutmarkerping_predicted_isanypingactive::trial_ui_waittill_retry();

    if ( !istrue( level._id_123C2 ) )
...
_id_123C1()
{
    for (;;)
    {
        ...
        {
            if ( !isdefined( level.playbattlechattersoundexpensive ) || !level.playbattlechattersoundexpensive )
                _calloutmarkerping_predicted_isanypingactive::_id_123C0();
        }
    }
}
