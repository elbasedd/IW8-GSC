...
setupobjective( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    if ( istrue( level.init_subway_cars ) )
        var_9._id_11E6A = ::_id_11E6B;
    else if ( istrue( level.lootcontentsadjusteconomy_toptier ) && scripts\mp\utility\game::getgametype() != "br" )
        var_9._id_11E6A = ::_id_11E6C;
    ...
    if ( istrue( level.lootcontentsadjusteconomy_toptier ) && scripts\mp\utility\game::getgametype() != "br" )
    {
        var_19 = "decor_halloween_scarecrow";
        var_9.flagmodel.player_test_ending_teleport = 1;
    }
    else
        var_19 = "military_dom_flag_neutral";

    var_9.flagmodel setmodel( var_19 );
    var_9.flagmodel.angles = debug_calculatecashonground( var_9 );
    var_9.outlineent = var_9.flagmodel;

    if ( istrue( level.lootcontentsadjusteconomy_toptier ) && scripts\mp\utility\game::getgametype() != "br" )
    {
        var_9 thread lootcontentsadjusteconomy_bottomtier();
        var_9 thread get_veh_spawn_test_spawners();
    }
...
debug_calculatecashonground( var_0 )
{
...
dompoint_setcaptured( var_0, var_1 )
{
    ...
    if ( isdefined( self.assisttouchlist[var_0] ) )
    {
        ...
        foreach ( var_8 in var_6 )
        {
            ...
            var_9 scripts\cp\agents\gametype_cp_specops::setuptimelimit();
        }
    }
...
dompoint_onuse( var_0 )
{
    ...
    if ( !self.neutralized )
    {
        ...
        if ( isdefined( level.onobjectivecomplete ) )
        {
            ...
            var_5 = "Flag " + laser_sights() + " Captured";

            if ( var_2 == "neutral" || var_2 == "none" )
                scripts\mp\utility\game::rear_spotlight_target_found_dist_sq( var_0, undefined, var_5, var_0.origin, "neutral_flag" );
            else
                scripts\mp\utility\game::rear_spotlight_target_found_dist_sq( var_0, undefined, var_5, var_0.origin );
        }
    }
...
dompoint_onusebegin( var_0 )
{
    ...
    {
        if ( var_1 != var_0.team )
            var_2 = keep_trying_to_kill_off_ai();
        else
            var_2 = 0;
    }
...
{
    self.dontspawncargotruck = gettime();
...
dompoint_onuncontested( var_0 )
{
    ...
    var_2 = ( gettime() - self.dontspawncargotruck ) * 0.001;
    var_3 = "Flag " + laser_sights() + " Contested";
    scripts\mp\utility\game::rear_spotlight_target_found_dist_sq( undefined, undefined, var_3, undefined, var_2 + " seconds" );
    self.dontspawncargotruck = undefined;
...
_id_11E6B()
{
    ...
    if ( var_7["fraction"] != 1 )
    {
        ...
        if ( var_7["fraction"] != 1 )
        {
            playfx( level.loadout_copyclassstruct["gos_fireworks"], var_7["position"] );
            playfx( level.loadout_copyclassstruct["gos_fireworks"], var_7["position"], anglestoforward( self.flagmodel.angles ) + ( -10, -10, 0 ) );
            playfx( level.loadout_copyclassstruct["gos_fireworks"], var_7["position"], anglestoforward( self.flagmodel.angles ) + ( 10, 10, 0 ) );
            level thread scripts\mp\gametypes\common.gsc::_id_128F1( var_7["position"] );
        }
    }
    else
    {
        playfx( level.loadout_copyclassstruct["gos_fireworks"], var_2 );
        playfx( level.loadout_copyclassstruct["gos_fireworks"], var_2, anglestoforward( self.flagmodel.angles ) + ( -10, -10, 0 ) );
        playfx( level.loadout_copyclassstruct["gos_fireworks"], var_2, anglestoforward( self.flagmodel.angles ) + ( 10, 10, 0 ) );
        level thread scripts\mp\gametypes\common.gsc::_id_128F1( var_2 );
    }
}

_id_11E6C()
{
    if ( isdefined( self.mover_update ) )
        return;
    else
    {
        self.mover_update = 1;
        ...
        if ( var_7["fraction"] != 1 )
            playfx( level.movequestlocale["blood_ceil_hw"], var_7["position"] - ( 0, 0, 5 ), ( -180, -180, 0 ), ( -180, -180, -180 ) );

        playfx( level.movequestlocale["blood_floor_hw"], self.flagmodel.origin + ( 0, 0, 2 ) );
        var_0 = 4000;
        var_1 = 20;
        var_2 = self.flagmodel.origin;
        var_3 = var_2 + ( 0, 0, var_0 );
        ...
        if ( var_7["fraction"] != 1 )
            playfx( level.movequestlocale["bats_fly_hw"], var_7["position"] + ( 0, 0, 50 ) );
        else
            playfx( level.movequestlocale["bats_fly_hw"], self.flagmodel.origin + ( 0, 0, 50 ) );
    }
}

lootcontentsadjusteconomy_bottomtier()
{
    ...
    if ( var_7["fraction"] != 1 )
        playfx( level.movequestlocale["bats_hw"], var_7["position"] );
    else
        playfx( level.movequestlocale["bats_hw"], self.flagmodel.origin + ( 0, 0, 200 ) );
}
...
updateflagstate( var_0, var_1, var_2, var_3 )
{
    ...
    if ( istrue( level.lootcontentsadjusteconomy_toptier ) && scripts\mp\utility\game::getgametype() != "br" )
    {
        if ( istrue( var_3 ) )
            ...
            self.flagmodel thread weapon_class();
        else if ( isdefined( var_2 ) )
            playfx( level.movequestlocale["vanish_hw_fr"], self.flagmodel.origin + ( 0, 0, 60 ) );
    }
    else if ( isdefined( var_2 ) )
    {
        if ( var_2 == "allies" )
...
giveflagcapturexp( var_0, var_1, var_2 )
{
    ...
    if ( isplayer( var_3 ) )
    {
        ...
        var_3 thread scripts\common\utility::_id_12456( level.remove_objective_on_flag, "capture", var_3.origin );
    }
...
awardgenericmedals( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9 )
{
    ...
    if ( isdefined( var_0 ) )
    {
        ...
        {
            if ( istrue( var_0.player_test_ending_teleport ) )
                return;
        }
    }
            ...
            if ( var_20 || var_21 )
            {
                ...
                else if ( var_18.claimteam == var_1.team )
                {
                    if ( !var_12 )
                    {
                        ...
                        thread scripts\common\utility::_id_12456( level.remove_munition_on_use, var_9, "defending" );
                        continue;
                    }
                }
            }
...
weapon_class()
{
    ...
    wait 0.55;
    playfx( level.movequestlocale["vanish_hw_fr"], self.origin + ( 0, 0, 60 ) );
...
get_veh_spawn_test_spawners()
{
    ...
    for (;;)
    {
        ...
        if ( isplayer( var_0 ) )
        {
            if ( !isdefined( self._id_11EE0 ) )
            {
                self._id_11EE0 = var_0;
                thread _id_12A19( self.flagmodel, var_0 );
            }
        }
...
_id_12A19( var_0, var_1 )
{
    level endon( "game_ended" );
    self notify( "new_stalker_target" );
    self endon( "new_stalker_target" );

    while ( isdefined( self._id_11EE0 ) )
    {
        if ( !_id_1268D( var_0, var_1 ) )
        {
            self._id_11EE0 = undefined;
            self._id_11EDF = 0;
            self.helihint_wait = undefined;
            self.battle_tracks_onentervehicle = undefined;
            self.usb_tape_animation_test = undefined;
            self.flagmodel rotateto( ( 0, self.flagmodel.angles[1], self.flagmodel.angles[2] ), 0.2, 0.1, 0.1 );
        }
        else if ( isdefined( self._id_11EE0 ) && scripts\mp\utility\player::isreallyalive( self._id_11EE0 ) )
        {
...
_id_12A12()
{
    ...
    for (;;)
    {
        ...
        if ( isdefined( var_1 ) && isdefined( self._id_11EE0 ) && self._id_11EE0 == var_1 )
            self._id_11EDF = self._id_11EDF + var_0;

        if ( isdefined( self._id_11EDF ) )
        {
            if ( self._id_11EDF > 100 && !isdefined( self.helihint_wait ) )
                self.helihint_wait = 1;

            if ( self._id_11EDF > 200 && !isdefined( self.battle_tracks_onentervehicle ) )
            {
                self.battle_tracks_onentervehicle = 1;
                playfx( level.movequestlocale["vanish_hw_en"], self.flagmodel.origin + ( 0, 0, 80 ) );
            }

            if ( self._id_11EDF > 300 && !isdefined( self.usb_tape_animation_test ) )
            {
                self.usb_tape_animation_test = 1;
                thread vo_paratroopers( var_1 );
            }
        }
...
lb_mg_dmg_factor_tail_stabilizer()
{
...
_id_1268D( var_0, var_1 )
{
    var_2 = 0.05;
    var_3 = var_1 lb_mg_dmg_factor_tail_rotor();
    var_4 = var_0 lasttuttxt();
