...
main()
{
    ...
    foreach ( var_2 in var_0 )
        level.flash_group[level.flash_group.size] = var_2.origin;

    level.flash_crate_use = ( 0, 0, 0 );
    ...
    foreach ( var_6 in var_4 )
    {
        ...
        {
            level.flash_crate_use = var_6.origin;
            break;
        }
    }
    ...
    level.flashentercode = 1;
    level.flashbang_ai = [];
    level.scorefrozenuntil = 0;
    level.flashenemies = 0;
...
updategametypedvars()
{
    ...
    level.traintracefails = scripts\mp\utility\dvars::dvarintvalue( "ppkTeamNoFlag", 50, 0, 250 );
    level.traintracerelpos = scripts\mp\utility\dvars::dvarintvalue( "ppkTeamWithFlag", 100, 0, 250 );
    level.trainmove = scripts\mp\utility\dvars::dvarintvalue( "ppkFlagCarrier", 250, 0, 250 );
    ...
    level.hvi_patrol_intro = scripts\mp\utility\dvars::dvarfloatvalue( "flagActivationDelay", 10, 0, 30 );
...
initspawns()
{
    ...
    foreach ( var_3 in level.spawnpoints )
        concussionimmune( var_3 );
}

concussionimmune( var_0 )
{
    var_0.scriptdata.get_offset_from_stance = undefined;
    var_1 = getpathdist( var_0.origin, level.flash_group[0], 1000 );

    if ( var_1 < 0 )
        var_1 = scripts\engine\utility::distance_2d_squared( var_0.origin, level.flash_group[0] );
    else
        var_1 = var_1 * var_1;

    var_0.scriptdata.get_offset_from_stance = var_1;
}
...
getspawnpoint()
{
    ...
    {
        var_3 = [];
        var_3["defenderFlagPosition"] = level.flash_crate_update_hint_logic_alt.visuals[0].origin;

        if ( isdefined( level.flash_crate_update_hint_logic_alt.carrier ) )
            var_3["activeCarrierPosition"] = level.flash_crate_update_hint_logic_alt.carrier.origin;
        else
...
tdef()
{
    ...
    level.round_at_max = "waypoint_mlg_empty_flag";
    level.round_enemies_fallback_logic = "waypoint_mlg_full_flag";
...
end_origin_final()
{
    level.flash_group = scripts\engine\utility::array_randomize( level.flash_group );
    var_0 = level.flash_group[0] + ( 0, 0, 64 );
    var_1 = level.flash_group[0] + ( 0, 0, -64 );
    var_2 = scripts\engine\trace::ray_trace( var_0, var_1, undefined, scripts\engine\trace::create_default_contents( 1 ) );
    level.flash_group[0] = var_2["position"];
    level.flash_crate_update_hint_logic_alt = enemies_validate_life( "allies" );
    level thread hvt_can_lose_health();
}
...
hvt_can_lose_health()
{
    ...
    {
        level.flash_icon_extraction.setup_enemy_sentries scripts\mp\gameobjects::setvisibleteam( "none" );
        level scripts\engine\utility::_id_128FF( "prematch_done", "start_mode_setup" );
        level.flash_icon_extraction.setup_enemy_sentries scripts\mp\gameobjects::setvisibleteam( "any" );
    }

    if ( level.hvi_patrol_intro )
    {
        scripts\mp\flags::gameflagwait( "prematch_done" );
        level.flash_icon_extraction.setup_enemy_sentries thread scripts\mp\gametypes\obj_zonecapture.gsc::rear_minigun_warning_time( level.hvi_patrol_intro, level.flash_crate_update_hint_logic_alt.curorigin + level.flash_crate_update_hint_logic_alt.offset3d );
        wait( level.hvi_patrol_intro );
        level.flash_icon_extraction.setup_enemy_sentries scripts\mp\gameobjects::_id_11AA7( level.iconcaptureflag, level.iconcaptureflag, level.round_enemies_fallback_logic );
        level.flash_crate_update_hint_logic_alt.trigger scripts\engine\utility::trigger_on();
...
enemies_validate_life( var_0 )
{
    ...
    level.returntime = 0;
    var_1 = 32;
    var_2 = spawn( "trigger_radius", level.flash_group[0], 0, var_1, 128 );
    var_3 = [];
    var_3[0] = spawn( "script_model", level.flash_group[0] );
    ...
    var_5 scripts\mp\gameobjects::_id_11AA7( level.iconescort, level.iconkill, level.round_enemies_fallback_logic );
    ...
    var_5 scripts\mp\gameobjects::usingobject( ::hvt_drop_from_truck_to_ground );
    ...
    if ( isdefined( level.showenemycarrier ) )
    {
        ...
        var_5 scripts\mp\gameobjects::_id_11AA7( level.iconescort, level.iconkill, level.round_enemies_fallback_logic, var_6 );
    }
    else
    {
        var_5.objidpingfriendly = 1;
    ...
    level.flash_icon_extraction = enemy_aggro_at_closest_player( var_0, var_5 );
    return var_5;
}

hvt_drop_from_truck_to_ground( var_0 )
{
    return !var_0 scripts\cp_mp\utility\player_utility::isinvehicle();
}

enemy_aggro_at_closest_player( var_0, var_1 )
{
    ...
    var_3.setup_enemy_sentries = scripts\mp\gameobjects::createobjidobject( var_2, "neutral", ( 0, 0, 85 ), undefined, "any", 0 );
    var_3.setup_enemy_sentries scripts\mp\gameobjects::setvisibleteam( "any" );

    if ( level.hvi_patrol_intro )
    {
        var_1.trigger scripts\engine\utility::trigger_off();
        var_3.setup_enemy_sentries scripts\mp\gameobjects::_id_11AA7( level.icontarget, level.icontarget, level.round_enemies_fallback_logic );
    }
    else
        var_3.setup_enemy_sentries scripts\mp\gameobjects::_id_11AA7( level.iconcaptureflag, level.iconcaptureflag, level.round_enemies_fallback_logic );
...
onpickup( var_0, var_1, var_2 )
{
    ...
    level.flash_icon_extraction.setup_enemy_sentries scripts\mp\gameobjects::setvisibleteam( "none" );
    level.flash_crate_update_hint_logic_alt.entmantlingendtime = var_0;
    var_0 thread botloadoutfavoritecamoprimary();
    var_0 thread hvi_vehicle_rider_special_setup();
    ...
    if ( isdefined( level.showenemycarrier ) )
    {
        ...
        else
        {
            ...
            scripts\mp\objidpoolmanager::setup_backup_respawn_points_in_verdansk( self.pingobjidnum, 1 );
    ...
    scripts\mp\gameobjects::_id_11AA7( level.iconescort, level.iconkill, level.round_enemies_fallback_logic );
    ...
    var_0 thread scripts\common\utility::_id_12456( level.remove_objective_on_flag, "pickup", var_0.origin );
...
ondrop( var_0 )
{
    ...
    level.flash_crate_update_hint_logic_alt.entmantlingendtime = undefined;
    ...
    if ( level.returntime >= 0 )
        scripts\mp\gameobjects::_id_11AA7( level.iconreturnflag, level.iconreturnflag, level.round_enemies_fallback_logic );
    else
    {
        scripts\mp\gameobjects::_id_11AA7( level.iconreturnflag, level.iconreturnflag, level.round_enemies_fallback_logic );
        scripts\mp\objidpoolmanager::setup_backup_respawn_points_in_verdansk( self.objidnum, 1 );
    }
...
onreset()
{
    ...
    if ( isdefined( level.flash_crate_update_hint_logic_alt.train_attach_player_hurts ) )
    {
        level.flash_crate_update_hint_logic_alt.train_attach_player_hurts clearportableradar();
        level.flash_crate_update_hint_logic_alt.train_attach_player_hurts delete();
    }
...
botloadoutfavoritecamoprimary( var_0 )
{
    ...
    level.flash_crate_update_hint_logic_alt endon( "dropped" );
    level.flash_crate_update_hint_logic_alt endon( "reset" );
    ...
    while ( !level.gameended )
    {
        ...
        if ( !level.gameended )
        {
            level.flash_crate_update_hint_logic_alt.carrier scripts\mp\utility\stats::incpersstat( "objTime", 1 );
            level.flash_crate_update_hint_logic_alt.carrier scripts\mp\persistence::statsetchild( "round", "objTime", level.flash_crate_update_hint_logic_alt.carrier.pers["objTime"] );
            level.flash_crate_update_hint_logic_alt.carrier scripts\mp\utility\stats::setextrascore0( level.flash_crate_update_hint_logic_alt.carrier.pers["objTime"] );
            level.flash_crate_update_hint_logic_alt.carrier scripts\mp\gamescore::giveplayerscore( "tdef_hold_obj", 10 );
        }
    }
}

hvi_vehicle_rider_special_setup( var_0 )
{
    level endon( "game_ended" );
    level.flash_crate_update_hint_logic_alt endon( "dropped" );
    level.flash_crate_update_hint_logic_alt endon( "reset" );
    ...
    var_1 = keypad_increase_failnum( var_0 );
    var_2 = spawn( "script_model", level.flash_crate_update_hint_logic_alt.visuals[0].origin );
    var_2.team = scripts\mp\utility\game::getotherteam( var_0 )[0];
    var_2.owner = var_1;
    var_2 makeportableradar( var_1 );
    level.flash_crate_update_hint_logic_alt.train_attach_player_hurts = var_2;
    level.flash_crate_update_hint_logic_alt thread hvt_key_dropped();
    level.flash_crate_update_hint_logic_alt thread hvt_key_picked_up();
}
...
hvt_key_dropped()
{
    level endon( "game_ended" );
    self endon( "dropped" );
    self.train_attach_player_hurts endon( "death" );
    level endon( "portableRadarRunning" );

    for (;;)
    {
        self.train_attach_player_hurts moveto( self.entmantlingendtime.origin, 0.05 );
        wait 0.05;
    }
}
...
hvt_key_picked_up()
{
    level endon( "game_ended" );
    self endon( "dropped" );
    var_0 = self.train_attach_player_hurts.team;
    var_0 = scripts\mp\utility\game::getotherteam( var_0 )[0];
    self.train_attach_player_hurts.owner scripts\engine\utility::_id_12900( "disconnect", "joined_team", "joined_spectators" );
    self.train_attach_player_hurts clearportableradar();
    self.train_attach_player_hurts = undefined;
    hvi_vehicle_rider_special_setup( var_0 );
}

getrespawndelay()
{
    var_0 = level.flash_crate_update_hint_logic_alt scripts\mp\gameobjects::getownerteam();
...
onplayerkilled( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9 )
{
    ...
    var_10 = level.traintracefails;

    if ( isdefined( level.flash_crate_update_hint_logic_alt ) && level.flash_crate_update_hint_logic_alt scripts\mp\gameobjects::getownerteam() == var_1.pers["team"] )
    {
        if ( isdefined( level.flash_crate_update_hint_logic_alt.carrier ) && var_1 != level.flash_crate_update_hint_logic_alt.carrier )
        {
            level.flash_crate_update_hint_logic_alt.carrier thread scripts\mp\rank::scoreeventpopup( "carrier_bonus" );
            var_11 = scripts\mp\rank::getscoreinfovalue( "carrier_bonus" );
            scripts\mp\gamescore::giveplayerscore( "carrier_bonus", var_11, self );
            level.flash_crate_update_hint_logic_alt.carrier thread scripts\mp\rank::giverankxp( "carrier_bonus", var_11 );
        ...
        var_10 = level.traintracerelpos;
    }
    else if ( isdefined( self.carryflag ) )
        var_10 = level.trainmove;
    ...
    if ( isdefined( var_1 ) && isplayer( var_1 ) && var_1.pers["team"] != self.pers["team"] )
    {
        ...
        if ( isdefined( self.carryflag ) )
        {
            ...
            thread scripts\common\utility::_id_12456( level.remove_prohibited_weapons, var_9, "carrying" );
        ...
        if ( !var_12 )
        {
            var_15 = 0;
            var_16 = 0;
            var_17 = distsquaredcheck( var_13, self.origin, level.flash_crate_update_hint_logic_alt.curorigin );

            if ( var_17 )
            {
                if ( level.flash_crate_update_hint_logic_alt.ownerteam == self.team )
                    var_15 = 1;
                else
                    var_16 = 1;
            }
            ...
            if ( var_15 )
            {
                ...
                thread scripts\common\utility::_id_12456( level.remove_prohibited_weapons, var_9, "defending" );
            ...
            else if ( var_16 )
            {
                ...
                var_1 scripts\mp\utility\stats::incpersstat( "defends", 1 );
                var_1 scripts\mp\persistence::statsetchild( "round", "defends", var_1.pers["defends"] );
                thread scripts\common\utility::_id_12456( level.remove_prohibited_weapons, var_9, "assaulting" );
            }
        }
    }
