...
player_latespawn_safehouse( var_0, var_1, var_2 )
{
...
qm_intro_dialogue( var_0, var_1, var_2, var_3 )
{
...
jugg_get_priority_player( var_0, var_1, var_2 )
{
    var_3 = qm_intro_dialogue( var_0, var_1, var_2.origin, var_2.radius );

    if ( !isdefined( var_3 ) )
        return;
    else if ( !isarray( var_3 ) )
    {
        if ( player_latespawn_safehouse( var_3, var_0, var_1 ) )
            return var_3;
    }
    else
    {
        var_4 = player_latespawn_safehouse( var_3[0], var_0, var_1 );
        var_5 = player_latespawn_safehouse( var_3[1], var_0, var_1 );
...
unset_relic_rocket_kill_ammo( var_0, var_1, var_2 )
{
    var_3 = [];

    foreach ( var_5 in var_2 )
    {
        var_6 = jugg_get_priority_player( var_0, var_1, var_5 );
...
_id_11CA8( var_0, var_1 )
{
    if ( !isdefined( level.outofboundstriggers ) || level.outofboundstriggers.size == 0 )
        return snappointtomapbounds2d( var_1 );

    if ( !isdefined( level.skipdeath ) )
        level.skipdeath = level.outofboundstriggers;

    var_2 = physics_createcontents( [ "physicscontents_playertrigger" ] );
    var_3 = scripts\engine\trace::ray_trace_ents( var_0, var_1, level.skipdeath, var_2 );

    if ( var_3["fraction"] < 1.0 )
        var_1 = var_3["position"];

    if ( isdefined( level.skipdeathicon ) && level.skipdeathicon.size > 0 && getdvarint( "scr_br_spawnOOBKillswitch", 0 ) == 0 )
    {
        var_4 = unset_relic_rocket_kill_ammo( var_0, var_1, level.skipdeathicon );
...
playerhandlesandboxmenu( var_0 )
{
    ...
    var_2 = last_unresolved_collision_time();
...
spawnc130pathstructnew( var_0, var_1 )
{
    ...
    var_3 = last_unresolved_collision_time( var_1 );
    var_4 = randomfloat( 360 );

    if ( scripts\mp\gametypes\br_public.gsc::play_death_audio() )
    {
        if ( isdefined( level.nodes_set_children ) )
            var_4 = level.nodes_set_children;
        else
            var_4 = 75;
    }
    ...
    var_5 = var_5 + ( 0, 0, jugg_removefromactivejugglist() );

    if ( !scripts\mp\gametypes\br_gametypes.gsc::play_station_closed_vo( "randomizeCircleCenter" ) )
        var_2 = var_2 + var_5;
    ...
    if ( scripts\mp\gametypes\br_gametypes.gsc::play_stealthy_disguise_vo( "planeConstrainedFromMapCenter" ) )
        var_2 = domoralesnags( var_2 );
    ...
    if ( scripts\mp\gametypes\br_gametypes.gsc::play_stealthy_disguise_vo( "planeUseCircleRadius" ) )
        var_12 = var_11;

    var_13 = _id_11DEC( var_2, var_6, var_12 );
...
domoralesnags( var_0 )
{
    ...
    var_2 = scripts\engine\trace::ray_trace_ents( level.br_level.br_mapcenter, var_0, level.skipdeath, var_1 );
...
_id_11DEC( var_0, var_1, var_2 )
{
    ...
    if ( !scripts\mp\gametypes\br_gametypes.gsc::play_station_closed_vo( "planeSnapToOOB" ) )
    {
        var_4 = _id_11CA8( var_0, var_4 );
        var_5 = _id_11CA8( var_0, var_5 );
    }
    ...
    if ( scripts\mp\gametypes\br_gametypes.gsc::play_station_closed_vo( "planeSnapToOOB" ) )
    {
        if ( !_purchasemenuclosedbyclient( var_4 ) )
            var_6 = _id_11CA8( var_0, var_4 );

        if ( !_purchasemenuclosedbyclient( var_5 ) )
            var_7 = _id_11CA8( var_0, var_5 );
    }

    var_3 = vectornormalize( var_5 - var_4 );
    var_8 = 10;

    if ( scripts\mp\utility\game::lb_mg_impulse_dmg_threshold_top() == "mini" || scripts\mp\utility\game::lb_mg_impulse_dmg_threshold_top() == "zxp" || getdvarint( "scr_br_alt_mode_escape_skip_initial_circle", 0 ) || scripts\mp\utility\game::lb_mg_impulse_dmg_threshold_top() == "reveal" || isdefined( level.check_for_early_impact ) && level.check_for_early_impact._id_1280A == 3 || getdvarint( "scr_br_alt_mode_rebirth_skip_initial_circle", 0 ) != 0 )
    {
    ...
    var_9.startpt = var_4;
    var_9.endpt = var_5;
    var_9._id_11FCE = var_6;
    var_9.grenade_obj = var_7;
    var_9.angle = var_1;
    var_9.pathdir = var_3;
...
updatec130pathomnvars( var_0 )
{
    ...
    if ( scripts\mp\gametypes\br_gametypes.gsc::play_station_closed_vo( "c130PlaneLine" ) )
        return;

    setomnvar( "ui_br_c130_path_start_x", int( var_0._id_11FCE[0] ) );
    setomnvar( "ui_br_c130_path_start_y", int( var_0._id_11FCE[1] ) );
    setomnvar( "ui_br_c130_path_end_x", int( var_0.grenade_obj[0] ) );
    setomnvar( "ui_br_c130_path_end_y", int( var_0.grenade_obj[1] ) );
}
...
createtestc130path( var_0, var_1 )
{
    ...
    if ( isdefined( level.br_level.br_mapcenter ) && isdefined( level.br_level.br_mapsize ) )
    {
        ...
        level thread scripts\mp\gametypes\br_analytics.gsc::cause( var_2.centerpt, var_2.angle[1], var_2._id_11FCE, var_2.grenade_obj );
    }
    else
...
spawnc130( var_0, var_1, var_2 )
{
    ...
    level.br_ac130.spawn_boss_wave_section = var_0;
    return var_4;
}
...
spawnplayertoc130()
{
    ...
    scripts\mp\gametypes\br_infils.gsc::grenade_menu_kill();
    thread playerputinc130( level.br_ac130 );
    self.chopper_kill = 1;

    if ( istrue( self.play_ending ) && getdvar( "scr_br_gametype", "" ) != "dmz" && getdvar( "scr_br_gametype", "" ) != "risk" )
        level scripts\mp\gametypes\br_public.gsc::chooseanim_vehicleturretdeath( "deploy_squad_leader", self, 1, 0 );
}
...
orbitcam( var_0 )
{
    ...
    if ( isdefined( level._id_12839 ) )
        scripts\mp\utility\player::_visionsetnaked( level._id_12839, 0 );
    else
        scripts\mp\utility\player::_visionsetnaked( "", 0 );

    self setplayerangles( var_0.angles );
    var_1 = scripts\mp\gametypes\br_infils.gsc::killed_by_chopper( self );
...
gunship_spawn( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    ...
    if ( isdefined( level.canparachutebecut ) )
        var_7 = level.canparachutebecut;
...
gunship_spawnvfx()
{
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" )
    {
        ...
        if ( !isdefined( level.canplaycircleclosedialog ) )
            level.canplaycircleclosedialog = "br_fx";

        self setscriptablepartstate( level.canplaycircleclosedialog, "clouds" );
    }
    else
    {
...
ispointinbounds( var_0, var_1, var_2 )
{
    ...
    var_3 = scripts\mp\utility\game_utility_mp::packsplashparambits( var_0, var_2 );

    if ( !var_3 )
        return 0;

    var_4 = scripts\mp\gametypes\br_public.gsc::play_patrol_sequence_based_on_first_vehicle() || scripts\mp\gametypes\br_public.gsc::play_death_audio();

    if ( var_4 && isdefined( level.br_ac130 ) )
        return var_3;

    if ( playerhandlesandboxmenu( var_0 ) )
        return 1;
    ...
    if ( istrue( var_1 ) )
    {
        if ( scripts\mp\outofbounds::playercinematiccompletecallback( var_0 ) )
            return 0;
    }
...
spawnwalltriggers()
{
    ...
    for (;;)
    {
        var_1 = vectornormalize( self.spawn_boss_wave_section._id_11FCE - self.origin );
...
kickwhenoutofbounds( var_0 )
{
    ...
    waitframe();
    spawnwalltriggers();
    var_1 = jugg_removefromactivejugglist();

    while ( !_purchasemenuclosedbyclient( ( self.origin[0], self.origin[1], var_1 ) ) )
        waitframe();

    if ( !scripts\cp_mp\utility\game_utility::isrealismenabled() )
        setomnvarforallclients( "ui_hide_minimap", 0 );

    level.cargo_truck_initdamage = 1;
    ...
    for (;;)
    {
        ...
        var_3 = vectornormalize( self.spawn_boss_wave_section.centerpt - self.origin );
    ...
    for (;;)
    {
        ...
        if ( !_purchasemenuclosedbyclient( var_6 ) )
        {
            level.canplaycircleopendialog = 1;
...
killaftertime( var_0, var_1 )
{
    ...
    self stoploopsound();
    scripts\mp\gametypes\br_infils.gsc::node_fields_after_goal_skit( self );
    wait 0.1;
    ...
    if ( isdefined( self.animstruct ) )
    {
        ...
        scripts\mp\gametypes\br_infils.gsc::deployingplayer( self.animstruct );
    }
    else
    {
        if ( isdefined( self.innards ) )
...
listenjump( var_0, var_1 )
{
    ...
    for (;;)
    {
        ...
        var_5 = scripts\mp\gametypes\br_public.gsc::lb_mg_dmg_factor_fuselage( self.team, self.squadindex );

        if ( !var_3 )
        {
            var_2 = var_0 calctrailpoint();

            if ( _purchasemenuclosedbyclient( var_2 ) )
            {
                self.jumptype = "solo";
        ...
        var_6 = scripts\mp\gametypes\br_public.gsc::player_give_intel_2_ks() || isdefined( self.pers["squadMemberIndex"] ) && self.pers["squadMemberIndex"] == 0;
...
leaveplane( var_0, var_1, var_2, var_3 )
{
    ...
    thread scripts\mp\gametypes\br_gametypes.gsc::watch_for_player_going_belowmap_or_oob( "onLeaveAC130" );
    ...
    if ( scripts\mp\utility\game::lb_mg_impulse_dmg_threshold_top() == "truckwar" )
    {
        ...
        self setorigin( self.updateteamplunderhud.origin, 1 );
        self setplayerangles( self.updateteamplunderhud.angles );
    }
}
...
{
    var_3 = scripts\engine\utility::ter_op( var_0 scripts\mp\gametypes\br_public.gsc::player_give_intel_2_ks(), 2, var_2 );
...
pushplayertoplayeroffset( var_0, var_1 )
{
    ...
    var_2 thread tac_cover_get_stuck_to_ent();
    var_3 = length( var_1 );
...
parachute( var_0, var_1, var_2, var_3 )
{
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" )
    {
        self.carepackage_waittill_settle = undefined;
...
infilfreefallparachuteaudio( var_0 )
{
    ...
    if ( scripts\mp\utility\game::lb_mg_impulse_dmg_threshold_top() != "truckwar" && scripts\mp\utility\game::lb_mg_impulse_dmg_threshold_top() != "reveal" && scripts\mp\utility\game::lb_mg_impulse_dmg_threshold_top() != "brdov" )
    {
        ...
        var_1.playersetignoreattractions = 1;
    }

    wait 1.0;
    var_1 clearclienttriggeraudiozone( 3.0 );

    if ( scripts\mp\utility\game::lb_mg_impulse_dmg_threshold_top() == "reveal" || scripts\mp\utility\game::lb_mg_impulse_dmg_threshold_top() == "brdov" )
    {
        wait 3.0;
...
waittoplayinfildialog()
{
    ...
    level.br_ac130 spawnwalltriggers();

    for (;;)
    {
        var_0 = level.br_ac130 calctrailpoint();

        if ( _purchasemenuclosedbyclient( var_0 ) )
        {
