...
main()
{
    ...
    level.run_cleanup_funcs_for_unused_objectives = ::createcallbacks;
...
initrules()
{
    ...
    updategametypedvars();
    level.ontimelimitgraceperiod = killstreak_wave_spawns();
...
killstreak_cooldown_start()
{
    return getdvarfloat( "scr_rugby_juggCaptureTime" );
}

killstreak_wave_spawns()
{
...
initrugbyents()
{
    level.pavelow_boss_explodes = scripts\engine\utility::array_combine( level.outofboundstriggers, level.minetriggers, level.hurttriggers, level.radtriggers );
    initendzoneents();
...
{
    if ( isdefined( level.wam_timer_between_each_node ) )
        var_0 setclientomnvar( "ui_rugby_jugg_friendly", scripts\engine\utility::ter_op( var_0.team == level.wam_timer_between_each_node.team, 0, 1 ) );
}
...
onplayerspawned( var_0 )
{
    ...
    if ( isdefined( level.wam_timer_between_each_node ) && isdefined( level.wam_timer_between_each_node.team ) && isdefined( var_0.team ) )
        var_0 setclientomnvar( "ui_rugby_jugg_friendly", scripts\engine\utility::ter_op( var_0.team == level.wam_timer_between_each_node.team, 0, 1 ) );
}
...
awardgenericmedals( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9 )
{
    ...
    if ( isdefined( level.wam_timer_between_each_node ) )
    {
        ...
        {
            if ( var_1 == level.wam_timer_between_each_node )
                var_1 thread scripts\mp\utility\points::giveunifiedpoints( "kill_as_juggernaut" );
            else if ( var_12 == level.wam_timer_between_each_node )
                var_1 thread scripts\mp\utility\points::giveunifiedpoints( "kill_juggernaut" );
            else if ( var_1.team == level.wam_timer_between_each_node.team && var_1 != level.wam_timer_between_each_node )
            {
                var_16 = distancesquared( level.wam_timer_between_each_node.origin, var_14 );
            ...
            else if ( var_1.team != level.wam_timer_between_each_node.team && var_12.team == level.wam_timer_between_each_node.team )
            {
                var_16 = distancesquared( level.wam_timer_between_each_node.origin, var_13 );

                if ( var_16 < 105625 )
                {
                    var_1 thread scripts\mp\rank::scoreeventpopup( "assault" );
                    var_1 thread scripts\mp\awards::givemidmatchaward( "mode_x_assault" );
                    thread scripts\common\utility::_id_12456( level.remove_munition_on_use, var_9, "assaulting" );
                }
            }
        }
    }
...
ontimelimitot()
{
    if ( level.rugby.activejuggernauts.size > 0 )
    {
        ...
        {
            setomnvar( "ui_overtime_time", gettime() + killstreak_wave_spawns() * 1000 );
            level.rugby.inot = 1;
            level.canprocessot = 1;
            _id_11A94();
            level thread playerpreloadintrocinematic();
        }
    }
    else
    {
...
getjuggorcratepos()
{
    ...
    if ( isdefined( level.wam_timer_between_each_node ) )
        return level.wam_timer_between_each_node.origin;
...
_id_11B2A( var_0 )
{
    ...
    var_0 thread bomber_radiusdamage( var_1, var_0.trigger.targetname );
    var_0 thread updatechevrons( "idle" );
}

bomber_radiusdamage( var_0, var_1 )
{
    wait 1.0;
    var_2 = getentitylessscriptablearray( var_0, "targetname" );
    var_2 = train_mover_test( var_2, var_1 );
    var_3 = [];
...
enemy_claymore_watchfortrigger( var_0, var_1, var_2 )
{
    var_0.origin = var_1;
    var_0.angles = var_2;
    return var_0;
...
activatenewjuggernaut( var_0 )
{
    ...
    level.wam_timer_between_each_node = var_0;
    ...
    level thread start_reach_exhaust_waste( var_0.team, var_6 );
...
start_reach_exhaust_waste( var_0, var_1 )
{
    wait 3;
...
deactivatejuggernaut( var_0 )
{
    ...
    level.wam_timer_between_each_node = undefined;
    ...
    level.rugby.getblueprintforpickupweapon = undefined;
    level.rugby.trial_start_time = undefined;
    level.rugby.playersetcontrols = undefined;
    level.rugby.playersetattractiontype = undefined;
...
watchjuggprogress()
{
    ...
    for (;;)
    {
        ...
        if ( var_9 > 0.8 )
            level thread debugforcesre2( var_9, var_0, var_2 );
        ...
        if ( isdefined( level.wam_timer_between_each_node ) )
        {
            ...
            {
                var_7 = var_9;
                level.wam_timer_between_each_node thread scripts\mp\utility\points::giveunifiedpoints( "rugby_obj_push" );
            }
        }
...
debugforcesre2( var_0, var_1, var_2 )
{
    if ( !isdefined( level.rugby.getblueprintforpickupweapon ) )
    {
        level.rugby.getblueprintforpickupweapon = 1;
        level.rugby.trial_start_time = gettime();
        level.rugby.playersetcontrols = var_0;
        level.rugby.playersetattractiontype = var_0;
    }
    else if ( isdefined( level.rugby.trial_start_time ) && level.rugby.trial_start_time + 30000 < gettime() )
    {
        level.rugby.getblueprintforpickupweapon = 1;
        level.rugby.trial_start_time = gettime();
        level.rugby.playersetcontrols = var_0;
    ...
    if ( istrue( level.rugby.getblueprintforpickupweapon ) )
    {
    ...
    level.rugby.getblueprintforpickupweapon = 0;
}
...
watchjuggtimeout()
{
    ...
    self endon( "rugby_jugg_end" );
    _id_11A95();
    level thread watch_minigun_ammo_depleted( self );
    wait( getjuggtimeout() );
    setomnvar( "ui_rugby_jugg_timer", 0 );
    setomnvar( "ui_rugby_jugg_radial", 0 );
    self suicide();
...
watch_minigun_ammo_depleted( var_0 )
{
    level endon( "stop_rugby_timeout" );
...
handlejuggjumpspam()
{
    ...
    for (;;)
    {
        ...
        for (;;)
        {
            var_2 = scripts\engine\utility::_id_12913( 1, "jugg_jumped" );
...
{
    var_0 = 0;
    level.playersetgulagdataomnvar = spawnstruct();
    level.playersetgulagdataomnvar.objidnum = scripts\mp\objidpoolmanager::requestreservedid( var_0 );
}
...
activatejuggcrate( var_0, var_1, var_2 )
{
    ...
    if ( !playerfriendlyto( var_0 ) )
    {
        var_0 = level.dropped_weapon_cleanup;
        var_1 = level.dropped_weapon;
    }
...
playerfriendlyto( var_0 )
{
    ...
    foreach ( var_3 in level.pavelow_boss_explodes )
    {
...
createjuggcrate( var_0, var_1, var_2 )
{
    ...
    if ( istrue( var_2 ) )
    {
        level.dropped_weapon_cleanup = var_0;
        level.dropped_weapon = var_1;
...
juggcratemanageuse( var_0 )
{
    ...
    var_1.playerexitlaststand = 1;
    self._id_12646 = scripts\mp\gameobjects::createholduseobject( "neutral", var_1, self.visuals, ( 0, 0, 64 ) );

    foreach ( var_3 in level.teamnamelist )
        self._id_12646.teamprogress = undefined;

    self._id_12646.trigger._id_1265A = 16900;
    self._id_12646.trigger setuserange( 130 );
    self._id_12646.trigger sethintstring( &"MP_MODE_RUGBY/CRATE_USE" );
    self._id_12646.trigger makeusable();
    self._id_12646.trigger setcursorhint( "HINT_BUTTON" );
    self._id_12646.trigger setuseholdduration( "duration_none" );
    self._id_12646.trigger setusehideprogressbar( 1 );
    self._id_12646.trigger setusepriority( -3 );
    self._id_12646.trigger sethintonobstruction( "hide" );
    self._id_12646 scripts\mp\gameobjects::setusetime( killstreak_cooldown_start() );
    self._id_12646.interactteam = "any";
    self._id_12646.curprogress = 0;
    self._id_12646.defaultusetime = self._id_12646._id_1266A;
    self._id_12646.userate = 1;
    self._id_12646.id = "rugby_jugg";
    self._id_12646.exclusiveuse = 0;
    self._id_12646.exclusiveclaim = 0;
    self._id_12646.skiptouching = 1;
    self._id_12646.onbeginuse = ::wam_number_of_failures;
    self._id_12646.onuse = ::wam_interaction_structs;
    self._id_12646.onenduse = ::wam_pointer;
    self._id_12646.oncantuse = ::wam_sequence;
    self._id_12646.inuse = 0;

    foreach ( var_6 in level.teamnamelist )
    {
        self._id_12646.numtouching[var_6] = 0;
        self._id_12646.touchlist[var_6] = [];
    }

    self._id_12646.cancontestclaim = 0;
    self._id_12646.stalemate = 0;
    self._id_12646.wasstalemate = 0;
    self._id_12646.cancontestclaim = 0;
    self._id_12646.majoritycapprogress = 1;
    self._id_12646.wasmajoritycapprogress = 0;
    self._id_12646.resetprogress = 1;

    if ( !scripts\mp\flags::gameflag( "prematch_done" ) )
        level scripts\engine\utility::_id_128FF( "prematch_done", "start_mode_setup" );

    self._id_12646.type = "useObject";
    self._id_12646.offset3d = ( 0, 0, 32 );
    self._id_12646 scripts\mp\gameobjects::requestid( 1, 1 );
    self._id_12646 scripts\mp\gameobjects::setobjectivestatusicons( "icon_waypoint_jugg_crate" );
    self._id_12646 scripts\mp\gameobjects::setvisibleteam( "any" );
    scripts\mp\objidpoolmanager::objective_set_play_intro( self._id_12646.objidnum, 0 );
    scripts\mp\objidpoolmanager::objective_set_play_outro( self._id_12646.objidnum, 0 );
}

wam_number_of_failures( var_0 )
{
...
wam_interaction_structs( var_0 )
{
    ...
    if ( isdefined( self._id_12646 ) )
        self._id_12646 delete();
...
createcallbacks( var_0 )
{
    ...
    var_2 = var_0 scripts\mp\juggernaut::playerplunderdepositcallback( var_1 );
...
updateoverheadcamerapos( var_0 )
{
    ...
    var_10 = scripts\cp_mp\utility\killstreak_utility::kingslayer();
