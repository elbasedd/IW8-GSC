...
onstartgametype()
{
    ...
    if ( !level.zonerandomlocationorder )
        vehicle_damage_shouldskipburndown();
...
updategametypedvars()
{
    ...
    level.spawn_convoy_and_move = scripts\mp\utility\dvars::dvarintvalue( "pauseTime", 1, 0, 1 );
    ...
    if ( scripts\mp\utility\game::matchmakinggame() && !scripts\mp\utility\game::isanymlgmatch() && getdvarint( "scr_koth_playlistZoneActivationDelay", 15 ) != 0 )
        level.zoneactivationdelay = ammobox_canweaponuserandomattachments();
}

ammobox_canweaponuserandomattachments()
{
...
hardpointmainloop()
{
    ...
    scripts\mp\utility\game::rear_spotlight_target_found_dist_sq( undefined, undefined, "New Hardpoint", level.zone.trigger.origin );
    level.zone thread scripts\common\utility::_id_12456( level.remove_objective_on_flag, "hill_moved", level.zone.trigger.origin );
    scripts\mp\flags::gameflagwait( "prematch_done" );

    if ( level.spawn_convoy_and_move )
    ...
    if ( level.firstzoneactivationdelay )
    {
        ...
        level.zone thread scripts\mp\gametypes\obj_zonecapture.gsc::rear_minigun_warning_time( level.firstzoneactivationdelay, level.zone.curorigin + level.zone.offset3d );
    ...
    for (;;)
    {
        if ( !isdefined( level.relic_squadlink_watch_for_visionset_end ) )
...
vehicle_damage_shouldskipburndown()
{
    switch ( level.mapname )
    {
        ...
        case "mp_hackney_yard":
            level.vehicle_damage_updatestate_br = [ "1", "4", "3", "9", "8" ];
            break;
        case "mp_spear":
        case "mp_spear_pm":
            level.vehicle_damage_updatestate_br = [ "5", "4", "3", "2", "6" ];
            break;
        case "mp_cave":
        case "mp_cave_am":
            level.vehicle_damage_updatestate_br = [ "1", "5", "2", "3", "4" ];
            break;
        case "mp_petrograd":
            level.vehicle_damage_updatestate_br = [ "1", "9", "4", "5", "2" ];
            break;
        case "mp_deadzone":
            level.vehicle_damage_updatestate_br = [ "12", "11", "3", "1", "2" ];
            break;
        case "mp_raid":
            level.vehicle_damage_updatestate_br = [ "20", "2", "8", "9", "5" ];
            break;
        case "mp_piccadilly":
            level.vehicle_damage_updatestate_br = [ "1", "5", "7", "3", "8" ];
            break;
        case "mp_crash2_pm":
        case "mp_crash2":
            level.vehicle_damage_updatestate_br = [ "1", "20", "5", "4", "19" ];
            break;
        case "mp_emporium":
            level.vehicle_damage_updatestate_br = [ "1", "2", "3", "4", "5" ];
            break;
        case "mp_broadcast2":
            level.vehicle_damage_updatestate_br = [ "1", "2", "3", "4", "5" ];
            break;
...
getfirstzone()
{
    if ( isdefined( level.vehicle_damage_updatestate_br ) )
    {
        var_0 = level.objectives[level.vehicle_damage_updatestate_br[0]];
        level.prevzoneindex = 0;
        level.init_relic_noks = level.objectives[level.vehicle_damage_updatestate_br[1]];
    }
    else
    {
        var_0 = level.objectives["1"];
        level.prevzoneindex = 1;
        level.init_relic_noks = level.objectives["2"];
    }

    var_0 thread scripts\mp\gametypes\obj_zonecapture.gsc::_id_12A01();
    level.init_relic_noks thread scripts\mp\gametypes\obj_zonecapture.gsc::_id_12A01();
    return var_0;
}
...
getnextzone()
{
    ...
    else
    {
        if ( isdefined( level.vehicle_damage_updatestate_br ) )
        {
            ...
            if ( level.prevzoneindex > level.vehicle_damage_updatestate_br.size - 1 )
                level.prevzoneindex = 0;

            var_23 = level.objectives[level.vehicle_damage_updatestate_br[level.prevzoneindex]];
            var_30 = level.prevzoneindex + 1;

            if ( var_30 > level.vehicle_damage_updatestate_br.size - 1 )
                var_30 = 1;

            level.init_relic_noks = level.objectives[level.vehicle_damage_updatestate_br[var_30]];
        }
        else
        {
            level.prevzoneindex++;
            ...
            level.init_relic_noks = level.objectives[scripts\engine\utility::string( var_30 )];
        }
...
hqactivatenextzone( var_0, var_1 )
{
    ...
    else if ( level.zoneactivationdelay )
    {
        ...
        level._id_12BF4 = 1;
        level thread scripts\mp\hud_message::updatematchstatushintforallplayers( undefined, 36, 36 );
        level.zone thread scripts\mp\gametypes\obj_zonecapture.gsc::rear_minigun_warning_time( level.zoneactivationdelay, level.zone.curorigin + level.zone.offset3d );
...
hpcaptureloop()
{
    ...
    for (;;)
    {
        ...
        var_0 = level scripts\engine\utility::_id_12907( "zone_captured", "zone_destroyed" );
...
initspawns()
{
    ...
    if ( !isdefined( level.spawnpoints ) || istrue( level.ammobox_bufferedattachmentweapon ) )
    {
    ...
    if ( istrue( level.ammobox_bufferedattachmentweapon ) )
        return;
...
give_capture_credit( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    level endon( "game_ended" );
    level.lastcaptime = gettime();
    scripts\mp\utility\game::rear_spotlight_target_found_dist_sq( var_4, undefined, "Hardpoint Captured", var_4.origin );
    var_6 = var_4;
    ...
    if ( isplayer( var_6 ) )
    {
        var_6 thread scripts\common\utility::_id_12456( level.remove_objective_on_flag, "capture", var_6.origin );

        if ( play_sound_on_pa_systems( var_6 ) && !scripts\mp\utility\game::isanymlgmatch() )
        {
    ...
    while ( !level.gameended )
    {
        ...
        if ( !level.zone.stalemate && !level.gameended )
        {
            ...
            foreach ( var_5 in level.zone.touchlist[var_3] )
            {
                ...
                if ( isdefined( var_5.player.monitorhotfoot ) && var_5.player.monitorhotfoot > 3 )
                {
                    var_5.player.monitorhotfoot = 0;
                ...
                if ( !isdefined( var_5.player.monitorhotfoot ) )
                    var_5.player.monitorhotfoot = 0;
                else
                    var_5.player.monitorhotfoot++;
...
play_sound_on_pa_systems( var_0 )
{
