...
init()
{
    ...
    scripts\mp\gametypes\br_publicevents.gsc::van_blocker_moves( 3, ::_id_12689, var_0, ::_id_128DF, ::airdrop_givecrateuseweapon );
...
_id_12689()
{
    var_0 = !scripts\mp\gametypes\br_gametypes.gsc::play_station_closed_vo( "oneLife" );
...
airdrop_givecrateuseweapon()
{
    ...
    spawn_collision( 1 );
    _id_128CF();
    scripts\mp\gametypes\br_publicevents.gsc::_id_11C20( "br_pe_jailbreak_incoming" );
    times_in_b( "incoming" );
    ...
    scripts\mp\gametypes\br_publicevents.gsc::_id_11C20( "br_pe_jailbreak_active" );
    times_in_b( "now" );
...
spawn_collision( var_0 )
{
    scripts\mp\gametypes\br_gulag.gsc::spawn_compound_final_push( var_0 );
}
...
_id_128CF()
{
    ...
    for (;;)
    {
        if ( !scripts\mp\gametypes\br_gulag.gsc::binoculars_onstateunmarkedenter() )
            break;
...
clients_hacked( var_0 )
{
    ...
    foreach ( var_4 in level.teamnamelist )
    {
        ...
        if ( var_5 )
        {
            ...
            {
                var_8 = var_2 && var_7 scripts\mp\gametypes\br_public.gsc::player_gun_game_next_weapon_think();
...
vo_areas_remaining()
{
    var_0 = isalive( self ) && isdefined( level.gulag ) && !istrue( level.gulag.shutdown ) && scripts\mp\gametypes\br_public.gsc::player_gun_game_next_weapon_think();

    if ( var_0 )
        thread scripts\mp\gametypes\br_gulag.gsc::playerinlaststandhold();
    else
...
vo_boss_heli()
{
    ...
    foreach ( var_3 in var_1 )
    {
        ...
        if ( var_0 )
            var_3 verify_chopper_pilot();

        var_3 thread vo_areas_remaining();
        waitframe();
    }
}
...
{
    self.spawnposition = undefined;
    self.chopperexfil_set_brcircle = undefined;
    self.trial_restart = undefined;
    self.change_fronttruck_label = undefined;
...
times_in_b( var_0 )
{
    ...
    foreach ( var_7 in level.teamnamelist )
    {
        ...
        if ( var_8 )
        {
            ...
            {
                var_11 = var_4 && var_10 scripts\mp\gametypes\br_public.gsc::player_gun_game_next_weapon_think();
