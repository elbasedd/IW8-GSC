...
main()
{
    ...
    if ( var_0 )
        level.play_vo_or_timeout = getdvarint( "scr_infect_groundwarInfect", 0 );

    var_1[0] = scripts\mp\utility\game::getgametype();
    var_1[var_1.size] = "grnd";

    if ( istrue( level.play_vo_or_timeout ) )
    ...
    updategametypedvars();
    level.helihint_activate_vo = 0;
    level.helihint_deposit = getdvarint( "scr_infect_time_increase_cap", 20 );
    ...
    level.allowed_gametypes = ::nextcombatareaid;
    level.init_standard_range = ::init_standard_range;
    ...
    level.nextareanags = getdvarint( "scr_infect_nvgMode", 0 );
    level.next_threshold = getdvarint( "scr_infect_disable_nvg", 0 );
    level.should_save_debug_info = getdvarint( "scr_infect_onlineprimaryoverride", 0 );

    if ( level.play_vo_or_timeout )
    {
        ...
        level.nextdest = var_3;
        level.avoids_recently_cleared_area = getdvarint( "scr_infect_alwaysTacInsertSkySpawn", 0 );
        level._id_11C88 = getdvarint( "scr_infect_skyDiveOnTacInsertPlacement", 1 );
        level.audio_player_stop_mud_loop = getdvarint( "scr_infect_allowSkyDiveCutParachute", 0 );
        level.spawn_obit_struct = ::spawn_obit_struct;
        level._id_1210F = ::_id_1210F;
    }
...
onstartgametype()
{
    ...
    level.timed_laser_trap_trigger_array = 0;
    ...
    scripts\cp_mp\utility\script_utility::registersharedfunc( "vehicle_compass", "shouldBeVisibleToPlayer", ::_id_126D0 );

    if ( istrue( level.play_vo_or_timeout ) )
    {
        scripts\mp\gametypes\arm.gsc::gettruegroundposition();
        thread num_rocket_per_attack();
        thread redbutton_init();
    }
    ...
    if ( scripts\mp\utility\game::matchmakinggame() )
    {
        ...
        if ( level.droptime > 0 )
            level thread unset_relic_dogtags();
    }
}
...
updategametypedvars()
{
    ...
    level._id_120CD = getdvar( "scr_infect_superSurvivorTwo", "none" );
    ...
    level.nextbombplanttime = getdvar( "scr_infect_superInfectTwo", scripts\engine\utility::ter_op( level.play_vo_or_timeout, "super_tac_insert", "none" ) );
    ...
    level.new_rider_combat_logic = getdvarint( "scr_infect_infectBonusScore", 150 );
    level.newhitlocs = getdvarint( "scr_infect_infectBonusSuperOnSpawn", 0 );
    level.next_drone_cd = getdvarint( "scr_infect_infectBonusSuperOnTacInsert", 0 );
    level.holdoutafterspawnfunc = getdvarint( "scr_infect_finalSuvivorCount", 1 );
    level.nextfixupcheckms = getdvarint( "scr_infect_setRadarOnNumSurvivors", 8 );
    level._id_120CC = [];
    var_0 = getdvar( "scr_infect_survivorStreakOverride", "" );

    if ( var_0 != "" )
        level._id_120CC = strtok( var_0, "," );

    var_1 = scripts\cp_mp\utility\game_utility::getmapname();

    if ( issubstr( var_1, "mp_m_" ) && var_1 != "mp_m_speed" )
    {
        level.survivorsuper = hudzombie( 0, 0 );
        level._id_120CD = hudzombie( 0, 1 );
        level.infectedsuper = hudzombie( 1, 0 );
        level.nextbombplanttime = hudzombie( 1, 1 );
    }

    if ( level.survivorsuper == level._id_120CD )
        level._id_120CD = "none";
    else if ( level.survivorsuper == "none" && level._id_120CD != "none" )
    {
        level.survivorsuper = level._id_120CD;
        level._id_120CD = "none";
    }

    if ( level.infectedsuper == level.nextbombplanttime )
        level.nextbombplanttime = "none";
    else if ( level.infectedsuper == "none" && level.nextbombplanttime != "none" )
    {
        level.infectedsuper = level.nextbombplanttime;
        level.nextbombplanttime = "none";
    }
...
buildandloadweapons()
{
    ...
    if ( level.should_save_debug_info )
    {
    ...
    foreach ( var_1 in level.allweapons )
    {
        if ( var_1 != "none" )
        {
            ...
            if ( level.nextareanags )
                game["arenaRandomLoadout"][0]["loadoutPrimaryAttachment"] = leader_charge_dialogue();
...
hudzombie( var_0, var_1 )
{
    if ( istrue( var_1 ) )
        var_2 = scripts\engine\utility::ter_op( var_0, level.nextbombplanttime, level._id_120CD );
    else
...
onspawnfinished()
{
    ...
    if ( self.pers["team"] == "allies" )
    {
        ...
        if ( level.nextareanags && !level.next_threshold && !isbot( self ) )
    ...
    else if ( self.pers["team"] == "axis" )
    {
        if ( istrue( level.lootcontentsadjusteconomy_toptier ) )
        {
            ...
            self.playapache_dialogue = 1;
        }

        if ( istrue( level.audio_player_stop_mud_loop ) )
    ...
    {
        if ( !level.play_vo_or_timeout || level.play_vo_or_timeout && level.mapname == "mp_aniyah" )
    ...
    else
    {
        ...
        if ( var_2 != "iw8_knife" )
        {
            var_3 = makeweapon( "iw8_knifestab_mp" );
    ...
    if ( istrue( self.player_fov_default_1 ) )
    {
        self.player_fov_default_1 = undefined;
        thread scripts\cp_mp\parachute::node_targetname( 1 );
    }

    thread scripts\mp\supers::givesuperpoints( level.newhitlocs, undefined, 1 );
    thread _id_128F3();
}
...
_id_128F3()
{
    ...
    if ( istrue( level.lootcontentsadjusteconomy_toptier ) && self.team == "axis" )
...
choosefirstinfected()
{
    ...
    level.hud_timer_reward_tiers = 0;
...
setfirstinfected( var_0 )
{
    ...
    while ( !scripts\mp\utility\player::isreallyalive( self ) || scripts\mp\utility\player::isusingremote() || isdefined( self._id_127E0 ) )
        waitframe();
    ...
    if ( isdefined( var_1 ) )
    {
        ...
        var_2.headlessloadoutindexsecondary = "INVOLUNTARY";
    ...
    if ( var_0 )
    {
        ...
        thread scripts\mp\music_and_dialog::next_mortar_vo();
    }

    self.isinitialinfected = 1;
    scripts\mp\utility\stats::incpersstat( "firstInfected", 1 );
    self.pers["gamemodeLoadout"] = level.infect_loadouts["axis_initial"];
    scripts\mp\equipment\tac_insert::_id_11E3C( self.origin, self.angles );
    ...
    if ( isdefined( scripts\mp\supers::getcurrentsuper() ) && scripts\mp\supers::getcurrentsuperref() == "super_deadsilence" )
        thread _id_11FAD();
    else if ( level.play_vo_or_timeout )
        thread scripts\mp\supers::givesuperpoints( 350, undefined, 1 );

    if ( var_0 )
        level.infect_players[self.name] = 1;

    level.hud_timer_reward_tiers = 1;

    if ( !level.hud_timer_reward_tiers )
...
onplayerkilled( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9 )
{
    ...
    {
        if ( isdefined( level.setarenaomnvar ) && self == level.setarenaomnvar )
        {
            var_10 = 0;
            var_11 = 0;
        }
    ...
    if ( var_10 )
    {
        ...
        if ( scripts\mp\utility\dvars::getwatcheddvar( "timelimit" ) != 0 )
        {
            ...
            if ( scripts\mp\utility\game::matchmakinggame() )
            {
                level.helihint_activate_vo++;
                var_17 = level.helihint_activate_vo <= level.helihint_deposit;
            }
...
{
    self.search_turret_no_target_stop_delay = 1;

    if ( level.play_vo_or_timeout && level.mapname != "mp_aniyah" )
        _id_11A84();
    ...
    if ( level.infect_teamscores["allies"] > 1 )
    {
        if ( level.play_vo_or_timeout )
        {
            level.timed_laser_trap_trigger_array++;

            if ( level.timed_laser_trap_trigger_array > 3 )
                level.timed_laser_trap_trigger_array = 1;

            if ( level.infect_teamscores["allies"] == level.nextfixupcheckms )
                level thread _id_11FC3();
        }

        if ( level.timed_laser_trap_trigger_array == 1 )
        {
        ...
        if ( !var_1 )
        {
            ...
            foreach ( var_5 in level.players )
            {
                ...
                if ( var_5.team == "axis" && var_5 != var_0 && var_5 != self )
                    var_5 thread scripts\mp\utility\points::giveunifiedpoints( "assist", undefined, level.new_rider_combat_logic );
            }
        }
    }
    else if ( level.infect_teamscores["allies"] == level.holdoutafterspawnfunc )
        onfinalsurvivor();
...
redbutton_init()
{
    ...
    while ( level.infect_teamscores["allies"] > level.nextfixupcheckms )
    {
        foreach ( var_1 in level.players )
        {
            if ( var_1.team == "axis" )
...
enduavonlatejoiner( var_0 )
{
    ...
    for (;;)
    {
        if ( level.infect_teamscores["allies"] > level.holdoutafterspawnfunc )
        {
...
monitordisconnect()
{
    ...
    if ( isdefined( self.infect_isbeingchosen ) || level.infect_chosefirstinfected )
    {
        if ( level.infect_teamscores["axis"] && level.infect_teamscores["allies"] )
        {
            if ( level.play_vo_or_timeout )
            {
                if ( var_0 == "allies" && level.infect_teamscores["allies"] == level.nextfixupcheckms )
                    level thread _id_11FC3();
            }

            if ( var_0 == "allies" && level.infect_teamscores["allies"] == level.holdoutafterspawnfunc )
                onfinalsurvivor();
...
setspecialloadouts()
{
    ...
    var_0 = 0;
    var_1 = scripts\mp\gametypes\gun.gsc::kicknullmusicondeath( level.survivorprimaryweapon );
    var_0 = scripts\mp\class::_id_120F3( level.survivorprimaryweapon, var_1 );
    var_2 = 0;
    var_3 = scripts\mp\gametypes\gun.gsc::kicknullmusicondeath( level.survivorsecondaryweapon );
    var_2 = scripts\mp\class::_id_120F3( level.survivorsecondaryweapon, var_3 );
    ...
    else if ( level.should_save_debug_info )
    {
        ...
        if ( level._id_120CC.size > 0 )
        {
            level.infect_loadouts["allies"]["loadoutKillstreak1"] = level._id_120CC[0];
            level.infect_loadouts["allies"]["loadoutKillstreak2"] = level._id_120CC[1];
            level.infect_loadouts["allies"]["loadoutKillstreak3"] = level._id_120CC[2];
        ...
        level.infect_loadouts["allies"]["loadoutFieldUpgrade2"] = level._id_120CD;
    }
    else
    {
        level.infect_loadouts["allies"]["loadoutPrimary"] = level.survivorprimaryweapon;
    ...
    {
        var_6 = scripts\mp\gametypes\gun.gsc::kicknullmusicondeath( level.initialprimaryweapon );
        var_4 = scripts\mp\class::_id_120F3( level.initialprimaryweapon, var_6 );
    }
    ...
    {
        var_7 = scripts\mp\gametypes\gun.gsc::kicknullmusicondeath( level.initialsecondaryweapon );
        var_5 = scripts\mp\class::_id_120F3( level.initialsecondaryweapon, var_7 );
    }
    ...
    else if ( level.should_save_debug_info )
    {
        ...
        level.infect_loadouts["axis_initial"]["loadoutFieldUpgrade2"] = level.nextbombplanttime;
    }
    else
    {
        level.infect_loadouts["axis_initial"]["loadoutPrimary"] = level.initialprimaryweapon;
    ...
    {
        var_9 = 0;
        var_10 = scripts\mp\gametypes\gun.gsc::kicknullmusicondeath( level.infectedprimaryweapon );
        var_9 = scripts\mp\class::_id_120F3( level.infectedprimaryweapon, var_10 );
    }
    ...
    else
    {
        ...
        level.infect_loadouts["axis"]["loadoutFieldUpgrade2"] = level.nextbombplanttime;
    ...
    for (;;)
    {
        if ( self isnightvisionon() )
        {
            ...
            if ( var_0 >= 10 )
            {
                ...
                scripts\common\utility::atv_trail( 0 );
                updateplayereliminatedomnvar();
                scripts\common\utility::atv_trail( 1 );
                var_1 = 0;
...
dialoguelines()
{
    ...
    self.loadoutfieldupgrade2 = level.nextbombplanttime;
    wait 0.1;
    ...
    if ( isdefined( self ) && scripts\mp\utility\player::isreallyalive( self ) )
    {
        ...
        var_0 = "gamemode";
        var_1 = scripts\mp\class::raid_vindia_unload_func();
        var_1 = scripts\mp\class::randomize_stealth_broken_music_array( var_1, var_0 );
        ...
        scripts\mp\equipment::giveequipment( level.infectedtactical, "secondary" );
        thread scripts\mp\supers::givesuperpoints( level.next_drone_cd, undefined, 1 );
    }

    if ( istrue( level._id_11C88 ) )
        thread _id_12097();
    else
...
num_rocket_per_attack()
{
    ...
    var_18 = level.setherodropscriptable;
...
{
    level endon( "game_ended" );
    level.next_subway_track_hurt_time = [];
    level thread _id_11B43();
    scripts\mp\flags::gameflagwait( "prematch_done" );
    wait( level.droptime );
...
kill_trig()
{
    var_0 = level.next_subway_track_hurt_time[0];
    level.next_subway_track_hurt_time[level.next_subway_track_hurt_time.size] = level.next_subway_track_hurt_time[0];
    level.next_subway_track_hurt_time[0] = undefined;
    level.next_subway_track_hurt_time = scripts\engine\utility::array_removeundefined( level.next_subway_track_hurt_time );
    return var_0;
}
...
init_standard_range( var_0, var_1, var_2, var_3, var_4 )
{
    var_5 = 0;
