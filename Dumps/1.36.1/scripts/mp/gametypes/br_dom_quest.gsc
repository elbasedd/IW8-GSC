...
init()
{
    ...
    scripts\mp\gametypes\br_quest_util.gsc::vandalize_attack_min_cooldown( "domination", ::get_vehicle_node_to_wait_for_lbravo_based_on_group );
    ...
    scripts\mp\gametypes\br_quest_util.gsc::validchallengetimer( "dom_locale", ::get_vehicle_ai_spawner );
    scripts\mp\gametypes\br_quest_util.gsc::valuehud( "dom_locale", ::get_vehicle_driver_hint_string_func );
    scripts\mp\gametypes\br_quest_util.gsc::getquestdata( "dom_locale" ).nextid = 0;
    scripts\mp\gametypes\br_quest_util.gsc::tut_loadout( "domination", 1 );
    scripts\mp\gametypes\br_quest_util.gsc::valve_steam_off( "domination", ::get_track_star_time_from_col );
    _id_11B2F();
...
domquest_removequestinstance()
{
    ...
    if ( isdefined( self.tablet.parachute_skydive ) )
        [[ self.tablet.parachute_skydive ]]( self );

    self._id_12071 thread get_valid_seats();
}
...
get_vehicle_node_to_wait_for_lbravo_based_on_group()
{
    var_0 = lastweaponfiretimestart( self );
    var_1 = scripts\mp\gametypes\br_quest_util.gsc::hostoverride( "domination", var_0 );
...
domlocale_createquestlocale( var_0 )
{
    ...
    var_3 = scripts\mp\gametypes\br_quest_util.gsc::turretobjweapon( var_0 );
    var_4 = spawn( "trigger_radius", var_2, 0, int( var_3 ), int( level.capturetrigger ) );
    ...
    var_5.onbeginuse = ::get_tv_station_infil_rider_start_targetname;
    var_5.onuseupdate = ::domflag_onuseupdate;
    var_5.onenduse = ::domflag_onenduse;
    var_5.usecondition = ::get_unique_id;
    var_5.lockupdatingicons = 1;
    var_5.decoy_canseeplayer = 1;

    if ( istrue( self.tablet.setupprops ) && isdefined( var_0.traincar ) )
    {
        if ( istrue( level._id_12AE6.get_track_location_index ) )
            var_5.flagmodel setmodel( "lm_domination_point_01_mover_nocol" );
        else
            var_5.flagmodel setmodel( "lm_domination_point_01_mover" );

        var_6 = var_0.traincar.get_track_setting;
        var_7 = var_0.traincar.angles;

        if ( isdefined( var_0.traincar.get_track_end_struct ) )
            var_7 = var_0.traincar.get_track_end_struct;

        var_5.flagmodel linkto( var_0.traincar.quarry_hacks_count, "tag_origin", var_6, var_7 );
        var_4 enablelinkto();
        ...
        var_5.scriptable linkto( var_0.traincar.quarry_hacks_count );
...
domlocale_circletick( var_0, var_1 )
{
    ...
    if ( var_3 > var_1 )
    {
        foreach ( var_5 in self.subscribedinstances )
        {
            ...
            if ( isdefined( var_5.tablet.parachute_skydive ) )
                [[ var_5.tablet.parachute_skydive ]]( var_5 );
        }
...
get_vehicle_ai_spawner( var_0 )
{
    get_turret_target_pos( var_0 );
}

get_vehicle_driver_hint_string_func( var_0 )
{
    if ( var_0.team == self.subscribedinstances[0].team )
        get_type_to_drop( var_0 );
}
...
takequestitem( var_0 )
{
    ...
    var_1 scripts\mp\gametypes\br_quest_util.gsc::tut_loot( var_2, 4 );
    var_3 = lastweaponfiretimestart( var_0 );
    ...
    if ( !var_4.enabled )
    {
        ...
        if ( isdefined( var_1.tablet.parachute_skydive ) )
            [[ var_1.tablet.parachute_skydive ]]( var_1 );
    ...
    else
    {
        ...
        scripts\mp\gametypes\br_quest_util.gsc::_id_11FD4( "domination", self, self.team );
        var_5 = spawnstruct();
        var_5.excludedplayers = [];
        var_5.excludedplayers[0] = self;
        var_5.trackriotshield_tryback = scripts\mp\gametypes\br_quest_util.gsc::lastdirtyscore( "domination", scripts\mp\gametypes\br_quest_util.gsc::lastplunderbankindex( self.team ) );
        ...
        scripts\mp\gametypes\br_quest_util.gsc::get_nearby_enemy_in_tvstation( var_1.team, self, 6, scripts\mp\gametypes\br_quest_util.gsc::getquestindex( "domination" ) );
        level thread scripts\mp\gametypes\br_public.gsc::choosebestpropforkillcam( "mission_dom_accept", var_1.team, 1 );
    }
}
...
domflagupdateicons()
{
    ...
    foreach ( var_1 in self.subscribedinstances )
    {
        foreach ( var_3 in scripts\mp\gametypes\br_public.gsc::lb_mg_dmg_factor_fuselage( var_1.team, self.squadindex ) )
        {
...
_id_11B2F()
{
    if ( isdefined( level.capturetrigger ) )
        return;

    level.disableinitplayergameobjects = 0;
    level.capturetrigger = 120;
    ...
    level.mp_hackney_yard_patch = "icon_waypoint_ot";
...
get_tv_station_infil_rider_start_targetname( var_0 )
{
    if ( !isdefined( self.setplayersquadindex ) || !self.setplayersquadindex )
    {
        ...
        {
            if ( isdefined( level._id_12AE6 ) && istrue( level._id_12AE6.get_track_timer_struct ) )
            {
                var_1 = self.traincar.get_track_setting + ( 200, 0, 0 );
                var_2 = self.traincar.origin + rotatevector( var_1, self.traincar.angles );
                level thread scripts\mp\gametypes\br_quest_util.gsc::_id_12677( var_2, "dom", 3 );
            }
        }
        else
            level thread scripts\mp\gametypes\br_quest_util.gsc::_id_12677( self.curorigin, "dom" );

        var_3 = scripts\mp\gametypes\br_public.gsc::lb_mg_dmg_factor_fuselage( var_0.team, self.squadindex );
        ...
        var_8 = scripts\mp\gametypes\br_public.gsc::lb_mg_dmg_factor_fuselage( var_0.team, self.squadindex );
...
domflag_onuse( var_0 )
{
    foreach ( var_2 in self.locale.subscribedinstances )
    {
        ...
        {
            var_3 = spawnstruct();
            var_4 = scripts\mp\gametypes\br_quest_util.gsc::lastplunderbankindex( var_0.team );
            var_5 = scripts\mp\gametypes\br_quest_util.gsc::getquestindex( "domination" );
            var_6 = scripts\mp\gametypes\br_quest_util.gsc::lastgunkilltime( scripts\mp\gametypes\br_quest_util.gsc::lastgoodjobplayer( "domination" ) );
            var_3.so_endgame = scripts\mp\gametypes\br_quest_util.gsc::so_getstarinfo( var_5, var_4, var_6 );
            ...
            scripts\mp\gametypes\br_quest_util.gsc::get_nearby_enemy_in_tvstation( var_2.team, var_0, 8, var_5 );
            level thread scripts\mp\gametypes\br_public.gsc::choosebestpropforkillcam( "mission_dom_success", var_2.team, 1, 1 );
            var_2.wait_until_truck_arrives_at_station = self.flagmodel.origin;
            var_2.wait_until_impact = self.flagmodel.angles;
            var_2.result = "success";

            if ( isdefined( var_2.tablet.parachute_skydive ) )
                [[ var_2.tablet.parachute_skydive ]]( var_2 );
            ...
            if ( isdefined( self.assisttouchlist[var_2.team] ) )
            {
                ...
                foreach ( var_9 in var_7 )
                {
                    ...
                    var_10 scripts\cp\agents\gametype_cp_specops::vehicle_tracker( "t9_ch_global_complete_recon_objective_for_operator_mission", 1 );
                    var_10 scripts\cp\agents\gametype_cp_specops::vehicle_tracker( "t9_ch_global_complete_recon_objective_for_operator_mission_op2", 1 );
                    var_2 scripts\mp\gametypes\br_quest_util.gsc::utilflare_shootflare( var_10 );
                }
            }
        ...
        level thread scripts\mp\gametypes\br_public.gsc::choosebestpropforkillcam( "mission_gen_fail", var_2.team, 1 );
        var_2.result = "fail";
...
{
    while ( self._id_12071.domflag.numtouching[self.id] )
        waitframe();
...
lastweaponfiretimestart( var_0 )
{
    var_1 = spawnstruct();
    var_1.x1opsunitrespawnselection = "questPointsArray";
    var_1.x1opsruntoicon = ( var_0.origin[0], var_0.origin[1], 0 );
    var_1.x1questmanager = 12000;
    var_1.x1spawnlocationoverride = 0;
    var_1.x1playerspawnoverride = 8000;
    var_1.x1playertransition = 6000;
    var_1.tutdmzonplayerkilled = 7;
    ...
    if ( init_track() )
    {
        if ( var_1.x1questmanager < level.turret_fob_watcher )
            var_1.x1questmanager = level.turret_fob_watcher;

        var_1.x1playerspawnoverride = level.turret_fob_watcher;
        var_1.x1playertransition = level.turret_guncourse_explode_on_end;
    }

    if ( istrue( var_0.setupprops ) )
    {
        var_1.x1opsunitrespawnselection = "questPointsArrayWZTrain";
        var_1._id_1264F = 1;
    }
    ...
    if ( var_2 >= 0 )
        var_1.x1playertransition = var_2;

    if ( var_3 >= 0 )
        var_1.x1playerspawnoverride = var_3;
...
{
    var_0 = 0;
    var_1 = scripts\mp\gametypes\br_gametypes.gsc::watch_for_player_going_belowmap_or_oob( "overrideQuestSearchParams", "domination" );
