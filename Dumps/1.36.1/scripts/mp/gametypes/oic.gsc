...
main()
{
    ...
    else
    {
        ...
        level.remove_steam_damage = 1;
        level.remove_specific_structs = 1;
        level.remove_tank_class = 1;
    }
...
updategametypedvars()
{
    ...
    level.remove_specific_structs = scripts\mp\utility\dvars::dvarintvalue( "initialAmmoCount", 1, 0, 15 );
    level.remove_tank_class = scripts\mp\utility\dvars::dvarintvalue( "killRewardAmmoCount", 1, 0, 15 );
    level.remove_steam_damage = scripts\mp\utility\dvars::dvarintvalue( "oneShotKill", 1, 0, 1 );
    level.remove_stay_at_spawn_spawn_flag = spawnstruct();
    var_0 = getdvar( "scr_oic_weapon" );
    level.remove_stay_at_spawn_spawn_flag.select_woods_one_spawners = var_0;
    level.remove_stay_at_spawn_spawn_flag.weapon = scripts\mp\utility\weapon::getweaponrootname( var_0 );
    level.remove_stay_at_spawn_spawn_flag.setupcirclepeek = 0;
}
...
onplayerconnect()
{
    for (;;)
    {
        ...
        var_0.pers["gamemodeLoadout"] = level.setupbrsquadleader["axis"];
        var_0 loadweaponsforplayer( [ level.remove_stay_at_spawn_spawn_flag.select_woods_one_spawners ], 1 );
        var_0.setupblueprintpickupweapons = 1;
        var_0.setupbobbingboatmultiple = 0;
    }
}

getspawnpoint()
{
    if ( self.setupblueprintpickupweapons )
        thread searchcirclesize();
...
searchcirclesize()
{
    ...
    if ( !self.setupbobbingboatmultiple )
        scripts\mp\menus::addtoteam( "spectator", 1 );
}

onspawnplayer()
{
    if ( isdefined( self.setupcellspawn ) && self.setupcellspawn )
        link_active();
    else
        self.setupcellspawn = 0;

    thread waitloadoutdone();
    level notify( "spawned_player" );

    if ( scripts\mp\utility\game::getgametypenumlives() != 0 )
        thread tabletreplacefrequency();
}
...
waitloadoutdone()
{
    ...
    if ( level.remove_specific_structs > var_0 )
    {
        self setweaponammoclip( self.primaryweapon, var_0 );
        self setweaponammostock( self.primaryweapon, level.remove_specific_structs - var_0 );

        if ( self.primaryweapons[0] hasattachment( "akimbo", 1 ) )
            self setweaponammoclip( self.primaryweapon, level.remove_specific_structs - var_0, "left" );
    }
    else
    {
        self setweaponammoclip( self.primaryweapon, level.remove_specific_structs );
        self setweaponammostock( self.primaryweapon, 0 );

        if ( self.primaryweapons[0] hasattachment( "akimbo", 1 ) )
            self setweaponammoclip( self.primaryweapon, level.remove_specific_structs, "left" );
    }
    ...
    self.setupbobbingboatmultiple = 1;
...
onplayerkilled( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9 )
{
    if ( isdefined( var_1 ) && isplayer( var_1 ) && self != var_1 )
    {
        if ( isdefined( var_3 ) && var_3 == "MOD_EXECUTION" )
            var_1.setupcellspawn = var_1.setupcellspawn + ( level.remove_tank_class + 2 );
        else
            var_1.setupcellspawn = var_1.setupcellspawn + level.remove_tank_class;

        if ( var_1 attackbuttonpressed() )
            var_1 thread _id_128E0();
        else
            var_1 link_active();
...
link_active()
{
    ...
    if ( var_2 + self.setupcellspawn > var_3 )
    {
        self setweaponammoclip( var_0, var_3 );
        self setweaponammostock( var_0, var_1 + var_2 + self.setupcellspawn - var_3 );

        if ( isdefined( self.primaryweapons[0] ) && self.primaryweapons[0] hasattachment( "akimbo", 1 ) )
        {
            var_4 = self getweaponammoclip( var_0, "left" );
            self setweaponammoclip( var_0, var_1 + var_4 + self.setupcellspawn - var_3, "left" );
        }
    }
    else
    {
        self setweaponammoclip( var_0, var_2 + self.setupcellspawn );

        if ( isdefined( self.primaryweapons[0] ) && self.primaryweapons[0] hasattachment( "akimbo", 1 ) )
        {
            var_4 = self getweaponammoclip( var_0, "left" );
            self setweaponammoclip( var_0, var_4 + self.setupcellspawn, "left" );
        }
    }
...
gettingupfromlaststand( var_0 )
{
    ...
    {
        if ( var_3.pers["deaths"] < scripts\mp\utility\game::getgametypenumlives() && var_3.setupbobbingboatmultiple )
        {
...
shipment_spawnpatchtriggers()
{
    ...
    for (;;)
    {
        if ( gettime() > level._id_12250 + var_2 )
        {
...
_id_11A8A()
{
    if ( scripts\mp\utility\game::matchmakinggame() && getdvarint( "scr_oic_randomWeapon", 2 ) > 0 )
    {
        ...
        level.remove_stay_at_spawn_spawn_flag.weapon = var_0["weapon"];
        level.remove_stay_at_spawn_spawn_flag.setupcirclepeek = var_0["variantID"];
    }

    if ( !isdefined( level.remove_stay_at_spawn_spawn_flag.weapon ) || level.remove_stay_at_spawn_spawn_flag.weapon == "none" )
        level.remove_stay_at_spawn_spawn_flag.weapon = "iw8_pi_golf21";

    var_1 = play_extract_3mins();

    if ( var_1 > 1 )
    {
        level.remove_specific_structs = level.remove_specific_structs * var_1;
        level.remove_tank_class = level.remove_tank_class * var_1;
    }
}

play_extract_3mins()
{
    switch ( level.remove_stay_at_spawn_spawn_flag.weapon )
    {
        case "iw8_ar_falpha":
            return 3;
        case "iw8_pi_mike9":
            if ( level.remove_stay_at_spawn_spawn_flag.setupcirclepeek == 1 )
                return 3;
        case "iw8_ar_falima":
            if ( level.remove_stay_at_spawn_spawn_flag.setupcirclepeek == 6 || level.remove_stay_at_spawn_spawn_flag.setupcirclepeek == 9 )
                return 3;
        case "iw8_sm_smgolf45":
            if ( level.remove_stay_at_spawn_spawn_flag.setupcirclepeek == 2 )
                return 2;
        default:
            return 1;
    }
...
{
    level.setupboardroomcode = spawnstruct();
    level.setupboardroomcode._id_12A90 = [];
    level.setupboardroomcode._id_12A90["rand_pistol"] = 80;
    level.setupboardroomcode._id_12A90["rand_smg"] = 40;
    level.setupboardroomcode._id_12A90["rand_assault"] = 40;
    level.setupboardroomcode._id_12A90["rand_lmg"] = 25;
    level.setupboardroomcode._id_12A90["rand_sniper"] = 40;
    buildrandomweapontable();

    if ( var_0 == 2 )
    {
        level.setupboardroomcode._id_12A90["rand_pistol"] = 100;
        level.setupboardroomcode._id_12A90["rand_smg"] = 0;
        level.setupboardroomcode._id_12A90["rand_assault"] = 0;
        level.setupboardroomcode._id_12A90["rand_lmg"] = 0;
        level.setupboardroomcode._id_12A90["rand_sniper"] = 0;
    }
    else if ( var_0 == 3 )
    {
        level.setupboardroomcode._id_12A90["rand_pistol"] = 0;
        level.setupboardroomcode._id_12A90["rand_smg"] = 0;
        level.setupboardroomcode._id_12A90["rand_assault"] = 0;
        level.setupboardroomcode._id_12A90["rand_lmg"] = 0;
        level.setupboardroomcode._id_12A90["rand_sniper"] = 100;
    }
    else if ( var_0 == 4 )
    {
        level.setupboardroomcode._id_12A90["rand_pistol"] = 0;
        level.setupboardroomcode._id_12A90["rand_smg"] = 100;
        level.setupboardroomcode._id_12A90["rand_assault"] = 0;
        level.setupboardroomcode._id_12A90["rand_lmg"] = 0;
        level.setupboardroomcode._id_12A90["rand_sniper"] = 0;
    }
    else if ( var_0 == 5 )
    {
        level.setupboardroomcode._id_12A90["rand_pistol"] = 0;
        level.setupboardroomcode._id_12A90["rand_smg"] = 0;
        level.setupboardroomcode._id_12A90["rand_assault"] = 100;
        level.setupboardroomcode._id_12A90["rand_lmg"] = 0;
        level.setupboardroomcode._id_12A90["rand_sniper"] = 0;
    }
    else if ( var_0 == 6 )
    {
        ...
        level.setupboardroomcode._id_12A90["rand_smg"] = 0;
        level.setupboardroomcode._id_12A90["rand_assault"] = 0;
        level.setupboardroomcode._id_12A90["rand_lmg"] = 100;
        level.setupboardroomcode._id_12A90["rand_sniper"] = 0;
    }
    else if ( var_0 == 7 )
    {
        ...
        level.setupboardroomcode._id_12A90["rand_assault"] = 0;
        level.setupboardroomcode._id_12A90["rand_lmg"] = 100;
        level.setupboardroomcode._id_12A90["rand_sniper"] = 50;
    }

    var_1 = laststandgiveloadoutonrevive( level.setupboardroomcode._id_12A90 );
...
buildrandomweapontable()
{
    ...
    for (;;)
    {
        ...
        var_3 = scripts\mp\gametypes\gun.gsc::kicknullmusicondeath( var_2["weapon"] );
        var_2["variantID"] = scripts\mp\class::_id_120F3( var_2["weapon"], var_3 );
...
setspecialloadouts()
{
    level.setupbrsquadleader["allies"]["loadoutPrimary"] = level.remove_stay_at_spawn_spawn_flag.weapon;
    level.setupbrsquadleader["allies"]["loadoutPrimaryAttachment"] = "none";
    level.setupbrsquadleader["allies"]["loadoutPrimaryAttachment2"] = "none";
    level.setupbrsquadleader["allies"]["loadoutPrimaryCamo"] = "none";
    level.setupbrsquadleader["allies"]["loadoutPrimaryReticle"] = "none";
    level.setupbrsquadleader["allies"]["loadoutPrimaryAddBlueprintAttachments"] = scripts\engine\utility::ter_op( level.remove_stay_at_spawn_spawn_flag.setupcirclepeek != 0, 1, 0 );
    level.setupbrsquadleader["allies"]["loadoutPrimaryVariantID"] = level.remove_stay_at_spawn_spawn_flag.setupcirclepeek;
    level.setupbrsquadleader["allies"]["loadoutSecondary"] = "none";
    level.setupbrsquadleader["allies"]["loadoutSecondaryAttachment"] = "none";
    level.setupbrsquadleader["allies"]["loadoutSecondaryAttachment2"] = "none";
    level.setupbrsquadleader["allies"]["loadoutSecondaryCamo"] = "none";
    level.setupbrsquadleader["allies"]["loadoutSecondaryReticle"] = "none";
    level.setupbrsquadleader["allies"]["loadoutSecondaryVariantID"] = 0;
    level.setupbrsquadleader["allies"]["loadoutEquipmentPrimary"] = "none";
    level.setupbrsquadleader["allies"]["loadoutEquipmentSecondary"] = "none";
    level.setupbrsquadleader["allies"]["loadoutStreakType"] = "assault";
    level.setupbrsquadleader["allies"]["loadoutKillstreak1"] = "none";
    level.setupbrsquadleader["allies"]["loadoutKillstreak2"] = "none";
    level.setupbrsquadleader["allies"]["loadoutKillstreak3"] = "none";
    level.aon_loadouts["allies"]["loadoutPerks"] = [ "specialty_hustle" ];
    level.setupbrsquadleader["allies"]["loadoutGesture"] = "playerData";
    level.setupbrsquadleader["allies"]["loadoutFieldUpgrade1"] = "super_deadsilence";
    level.setupbrsquadleader["allies"]["loadoutFieldUpgrade2"] = "none";
    level.setupbrsquadleader["axis"] = level.setupbrsquadleader["allies"];
}
