...
main()
{
    ...
    if ( getdvarint( "scr_hotfoot", 0 ) != 0 )
        level.onplayerkilled = ::should_do_damage_check_func_relics;
...
onstartgametype()
{
    ...
    if ( isdefined( level.overrideviewkickscaledmr ) )
        [[ level.overrideviewkickscaledmr ]]();
    else
        initspawns();
    ...
    if ( getdvarint( "scr_hotfoot", 0 ) != 0 )
        thread sat_wait_for_transmission_start();
}
...
sat_wait_for_transmission_start()
{
    ...
    for (;;)
    {
        ...
        {
            if ( !isdefined( var_2.monitor_traversal_timer ) )
            {
                var_2.monitor_traversal_cooldown = 0;
                var_2.monitor_traversal_timer = var_2.origin;
                var_2.agent_pickup_hostage_scene = var_2.origin;
            }

            var_2.monitor_traversal_cooldown = var_2.monitor_traversal_cooldown + distancesquared( var_2.monitor_traversal_timer, var_2.origin );
            var_2.monitor_traversal_timer = var_2.origin;
        }
        ...
        if ( var_0 > 3.0 )
        {
            if ( level.monitor_target_death >= 2 )
            {
                level.monitor_target_death = 0;

                foreach ( var_2 in level.players )
                {
                    if ( !isdefined( var_2.agent_pickup_hostage_scene ) )
                        var_2.agent_pickup_hostage_scene = var_2.origin;

                    var_2.init_ai_spawns = distancesquared( var_2.agent_pickup_hostage_scene, var_2.origin ) < 4225;
                    var_2.agent_pickup_hostage_scene = var_2.origin;
                }
            }
            ...
            foreach ( var_2 in level.players )
            {
                ...
                if ( isalive( var_2 ) && ( istrue( var_2.init_ai_spawns ) || var_2.monitor_traversal_cooldown < 4225 && !istrue( var_2.monitor_traversal_watcher ) ) )
                {
                    var_2.init_ai_spawns = 0;
                ...
                {
                    if ( isalive( var_2 ) )
                        var_2.monitor_traversal_watcher = 0;
        ...
        level.monitor_target_death++;
        waitframe();
    }
}

should_do_damage_check_func_relics( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9 )
{
    ...
    self.monitor_traversal_watcher = 1;
}
