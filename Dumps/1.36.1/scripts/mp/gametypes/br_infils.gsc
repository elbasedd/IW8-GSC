...
main()
{
    ...
    level.nolandingdamage = _id_1196B();
    ...
    if ( istrue( getdvarint( "br_spawnSelectionInfil", 0 ) ) )
    {
        ...
        level.create_vehicle_omnvars_data = getdvarint( "scr_br_canSoloJump", 1 );
    }
...
plant_remove_zplanes_override()
{
    return level.nolandingdamage == 0;
}

play_go_to_safehouse()
{
    return level.nolandingdamage == 1;
}
...
freefallstartfunc()
{
    ...
    thread scripts\cp_mp\parachute::teamreviveperkdiscount();
}
...
_id_128C0( var_0, var_1, var_2 )
{
    ...
    foreach ( var_5 in var_3 )
    {
        ...
        if ( istrue( var_5.carepackage_spawn ) )
            scripts\mp\gametypes\br.gsc::_id_12531( var_5 );
    ...
    if ( !scripts\mp\gametypes\br_gametypes.gsc::play_station_closed_vo( "squadLeader" ) )
    {
        if ( level.teambased )
        {
            ...
            {
                var_9 = scripts\mp\gametypes\br_public.gsc::lb_impulse_dmg_threshold_top( var_8 );

                for ( var_10 = 0; var_10 < var_9.size; var_10++ )
                {
                    var_11 = var_9[var_10];
                    var_12 = scripts\mp\gametypes\br_public.gsc::lb_mg_dmg_factor_fuselage( var_8, var_11 );
                    var_13 = _id_11B25( var_12 );

                    if ( isdefined( var_13 ) )
                    {
                        usingfallback( var_13 );
                        var_13 thread _id_129D2( var_8, var_11 );
                    }

                    scripts\mp\gametypes\br.gsc::_id_125FB( var_8, var_11 );
                }
            }
        }
        else
...
grenade_menu_kill()
{
    ...
    if ( isdefined( var_0.his_completemission ) )
        var_0.his_completemission = 5;

    var_0 scripts\cp_mp\parachute::soldier_agent_lwfn8();
}

usingfallback( var_0 )
{
    if ( !istrue( var_0.playersetisbecomingzombie ) )
    {
        var_0.playersetisbecomingzombie = 1;
...
{
    scripts\engine\utility::_id_128FF( "death", "disconnect" );
    var_2 = scripts\mp\gametypes\br_public.gsc::lb_mg_dmg_factor_fuselage( var_0, var_1 );
    var_3 = _id_11B25( var_2 );

    if ( isdefined( var_3 ) )
    {
        usingfallback( var_3 );
        var_3 thread _id_129D2( var_0, var_1 );
    }
}
...
latespawnplayer()
{
    self endon( "disconnect" );
    var_0 = self;
    var_1 = scripts\mp\gametypes\br_public.gsc::playerexecutionsenable() || scripts\mp\gametypes\br_public.gsc::play_player_approach();
    ...
    else if ( !isalive( var_0 ) )
    {
        ...
        {
            var_2 = isbot( var_0 ) && scripts\mp\gametypes\br_public.gsc::play_death_audio();

            while ( !istrue( level.cargo_truck_initdamage ) && !var_2 )
            {
                if ( !var_2 )
                {
                    var_0 scripts\mp\gametypes\br.gsc::clean_up_eliminate_drone();
            ...
            var_0 thread prevbrbonusxp();
        }
        else
        {
            var_0.carepackage_unlink_from_heli = 0;
            ...
            var_0 thread roof_lander_spawn_func();
        }
    }
...
prevbrbonusxp()
{
    ...
    if ( !isalive( var_0 ) )
    {
        var_0.prevcallback = 1;
        var_0 scripts\mp\playerlogic::spawnplayer( 0 );
    }

    var_0 playerhide();
    var_0.prevcallback = undefined;
    ...
    var_0.no_previous_interaction_point = var_1;

    if ( var_0 scripts\mp\gametypes\br_public.gsc::player_give_intel_2_ks() )
        var_0.infilanimindex = 1;

    if ( !isdefined( var_0.infilanimindex ) && level.teambased )
    {
        var_2 = scripts\mp\gametypes\br_public.gsc::lb_mg_dmg_factor_fuselage( var_0.team, var_0.squadindex );
        var_3 = clients( var_2.size );
        ...
        for ( var_4 = 0; var_4 < var_3.size; var_4++ )
        {
            ...
            {
                if ( var_7 != var_0 && var_7 scripts\mp\gametypes\br_public.gsc::player_give_intel_2_ks() )
                    continue;
    ...
    var_0 playerlinktopositionent( level.prev_weapon_taccover );
    scripts\mp\utility\game::_id_11AB7( var_0, 1 );
    var_9 = killcam_timescalefactor( level.prev_weapon_taccover );
    var_0 setorigin( var_9.origin );
    var_0 playerplayinfilloopanim( level.prev_weapon_taccover );
    waitframe();
    var_10 = 0;
...
setuphudelemninfilcover()
{
    ...
    if ( !scripts\mp\gametypes\br_gametypes.gsc::play_station_closed_vo( "squadLeader" ) )
    {
        if ( level.teambased )
        {
            var_1 = scripts\mp\gametypes\br_public.gsc::lb_mg_dmg_factor_fuselage( var_0.team, var_0.squadindex );

            if ( !isdefined( var_1 ) || var_1.size == 0 )
                var_1 = [ self ];

            var_2 = _id_11B25( var_1 );
...
_id_11B25( var_0 )
{
    ...
    {
        if ( isdefined( var_3 ) && var_3 scripts\mp\gametypes\br_public.gsc::player_give_intel_2_ks() )
            return var_3;
    }
    ...
    var_1 scripts\mp\gametypes\br.gsc::_id_11AB1( 1 );

    foreach ( var_3 in var_0 )
    {
        if ( var_3 != var_1 )
            var_3 scripts\mp\gametypes\br.gsc::_id_11AB1( 0 );

        var_16 = scripts\engine\utility::ter_op( var_3 scripts\mp\gametypes\br_public.gsc::player_give_intel_2_ks(), 2, 1 );
...
clearhint( var_0, var_1 )
{
    if ( play_go_to_safehouse() )
        return clearcurrenthintforced( var_0, var_1 );
    else
...
buildac130infilanimstruct( var_0, var_1 )
{
    ...
    if ( isdefined( var_0 ) )
    {
        ...
        var_2.create_badplace_extraction = spawn( "script_model", var_2.movingc130.origin );
        var_2.create_badplace_extraction setmodel( "generic_prop_x5" );
        var_2.create_badplace_extraction linkto( var_2.movingc130, "tag_origin", ( 0, 0, 0 ), ( 0, 0, 0 ) );
        var_2.create_badplace_extraction forcenetfieldhighlod( 1 );
    }
    ...
    var_2.disableplayerkillrewards = spawnfx( level._effect["vfx_br_infil_cloud_anim"], var_2.staticc130.origin );
    return var_2;
}
...
clearcurrenthintforced( var_0, var_1 )
{
    ...
    {
        var_0.animstruct = var_2;
        var_2.select_bridge_one_spawners = var_0;
    }
    ...
    var_2.delete_dropped_weapon = spawn( "script_model", var_2.origin );
    var_2.delete_dropped_weapon setmodel( "veh8_mil_air_mindia8_interior_infil_netting" );
    var_2.delete_dropped_weapon linkto( var_2.chopper, "tag_origin", ( 0, 0, 0 ), ( 0, 0, 0 ) );
    var_2.delay_thirdpersoncamera = spawn( "script_model", var_2.origin );
    var_2.delay_thirdpersoncamera setmodel( "veh8_mil_air_mindia8_interior_infil_cabin_door" );
    var_2.delay_thirdpersoncamera linkto( var_2.chopper, "tag_origin", ( 0, 0, 0 ), ( 0, 0, 0 ) );
    var_2.create_bomb_wire_info = spawn( "script_model", var_2.origin );
    var_2.create_bomb_wire_info setmodel( "generic_prop_x3" );
    var_2.create_bomb_wire_info linkto( var_2.chopper, "tag_origin", ( 0, 0, 0 ), ( 0, 0, 0 ) );
    var_2.create_chopper_boss = spawn( "script_model", var_2.origin );
    var_2.create_chopper_boss setmodel( "generic_prop_x3" );
    var_2.create_chopper_boss linkto( var_2.chopper, "tag_origin", ( 0, 0, 0 ), ( 0, 0, 0 ) );

    if ( isdefined( var_0 ) )
    {
        var_0 forcenetfieldhighlod( 1 );
        var_2.create_cam = spawn( "script_model", var_0.origin );
        var_2.create_cam setmodel( "generic_prop_x3" );
        var_2.create_cam linkto( var_0, "tag_origin", ( 0, 0, 0 ), ( 0, 0, 0 ) );
        var_2.create_cam forcenetfieldhighlod( 1 );
        var_2.create_circular_path_around = spawn( "script_model", var_0.origin );
        var_2.create_circular_path_around setmodel( "generic_prop_x3" );
        var_2.create_circular_path_around linkto( var_0, "tag_origin", ( 0, 0, 0 ), ( 0, 0, 0 ) );
        var_2.create_circular_path_around forcenetfieldhighlod( 1 );
    }
    ...
    if ( var_1 == "script_model" )
    {
        var_2.targets_killed_by_hunters = spawn( "script_model", var_2.origin );
        var_2.targets_killed_by_hunters setmodel( "fullbody_usmc_ar_br_infil" );
        var_2.targets_killed_by_hunters linkto( var_2.playerpositionents["parent_solo"], "j_prop_1", ( 0, 0, 0 ), ( 0, 0, 0 ) );
        var_2.targets_killed_stat_row = spawn( "script_model", var_2.origin );
        var_2.targets_killed_stat_row setmodel( "fullbody_usmc_ar_br_infil" );
        var_2.targets_killed_stat_row linkto( var_2.playerpositionents["parent_squad"], "j_prop_1", ( 0, 0, 0 ), ( 0, 0, 0 ) );
        var_2.targetsite = spawn( "script_model", var_2.origin );
        var_2.targetsite setmodel( "fullbody_usmc_ar_br_infil" );
        var_2.targetsite linkto( var_2.playerpositionents["parent_squad"], "j_prop_2", ( 0, 0, 0 ), ( 0, 0, 0 ) );
        var_2.targetsshot = spawn( "script_model", var_2.origin );
        var_2.targetsshot setmodel( "fullbody_usmc_ar_br_infil" );
        var_2.targetsshot linkto( var_2.playerpositionents["parent_squad"], "j_prop_3", ( 0, 0, 0 ), ( 0, 0, 0 ) );
        var_2.targetteamremaining = spawn( "script_model", var_2.origin );
        var_2.targetteamremaining setmodel( "fullbody_usmc_ar_br_infil" );
        var_2.targetteamremaining linkto( var_2.playerpositionents["parent_squad"], "j_prop_4", ( 0, 0, 0 ), ( 0, 0, 0 ) );
        var_2.infil_anim_type = "script_model";
    }
    else
    {
    ...
    var_2.armsrace_c4_plant_spot_reset_on_failed_plant = spawn( "script_model", var_2.origin );
    var_2.armsrace_c4_plant_spot_reset_on_failed_plant setmodel( "fullbody_mp_eastern_bale_3_1" );
    var_2.armsrace_c4_plant_spot_reset_on_failed_plant linkto( var_2.playerpositionents["parent_solo"], "j_prop_2", ( 0, 0, 0 ), ( 0, 0, 0 ) );
    var_2.arenaknivesout = spawn( "script_model", var_2.origin );
    var_2.arenaknivesout setmodel( "fullbody_usmc_ar_br_infil" );
    var_2.arenaknivesout linkto( var_2.playerpositionents["parent_solo"], "j_prop_3", ( 0, 0, 0 ), ( 0, 0, 0 ) );
    var_2.binocularsinited = spawn( "script_model", var_2.origin );
    var_2.binocularsinited setmodel( "generic_prop_x10" );
    var_2.binocularsinited linkto( var_2.chopper, "tag_origin", ( 0, 0, 0 ), ( 0, 0, 0 ) );
    var_2.binocularsinited forcenetfieldhighlod( 1 );
    var_4 = 9;
    var_2.binoculars_watchracetake = [];

    for ( var_5 = 0; var_5 < var_4; var_5++ )
    {
        var_6 = spawn( "script_model", var_2.origin );
        var_6 setmodel( "veh8_mil_air_mindia8_infil_flight" );
        var_6 linkto( var_2.binocularsinited, "j_prop_" + ( var_5 + 2 ), ( 0, 0, 0 ), ( 0, 0, 0 ) );
        var_6 forcenetfieldhighlod( 1 );
        var_6 playloopsound( "br_heli_infil_fleet_lp" );
        var_2.binoculars_watchracetake[var_2.binoculars_watchracetake.size] = var_6;
    }
...
deployed( var_0 )
{
    if ( isdefined( var_0.binoculars_watchracetake ) )
    {
        foreach ( var_2 in var_0.binoculars_watchracetake )
            var_2 delete();
    }

    if ( isdefined( var_0.binocularsinited ) )
        var_0.binocularsinited delete();

    if ( isdefined( var_0.arenaknivesout ) )
        var_0.arenaknivesout delete();

    if ( isdefined( var_0.armsrace_c4_plant_spot_reset_on_failed_plant ) )
        var_0.armsrace_c4_plant_spot_reset_on_failed_plant delete();
    ...
    if ( isdefined( var_0.targets_killed_by_hunters ) )
        var_0.targets_killed_by_hunters delete();

    if ( isdefined( var_0.targets_killed_stat_row ) )
        var_0.targets_killed_stat_row delete();

    if ( isdefined( var_0.targetsite ) )
        var_0.targetsite delete();

    if ( isdefined( var_0.targetsshot ) )
        var_0.targetsshot delete();

    if ( isdefined( var_0.targetteamremaining ) )
        var_0.targetteamremaining delete();

    if ( isdefined( var_0.create_bomb_wire_info ) )
        var_0.create_bomb_wire_info delete();

    if ( isdefined( var_0.create_chopper_boss ) )
        var_0.create_chopper_boss delete();

    if ( isdefined( var_0.create_cam ) )
        var_0.create_cam delete();

    if ( isdefined( var_0.create_circular_path_around ) )
        var_0.create_circular_path_around delete();

    if ( isdefined( var_0.delay_thirdpersoncamera ) )
        var_0.delay_thirdpersoncamera delete();

    if ( isdefined( var_0.delete_dropped_weapon ) )
        var_0.delete_dropped_weapon delete();

    if ( isdefined( var_0.chopper ) && istrue( var_0.chopper.cleanme ) )
        var_0.chopper delete();

    if ( isdefined( var_0.select_bridge_one_spawners ) && istrue( var_0.select_bridge_one_spawners.cleanme ) )
    {
        if ( isdefined( var_0.select_bridge_one_spawners.innards ) && istrue( var_0.select_bridge_one_spawners.innards.cleanme ) )
            var_0.select_bridge_one_spawners.innards delete();

        var_0.select_bridge_one_spawners delete();
    }
}
...
{
    if ( play_go_to_safehouse() )
        return killstreaktoscorestreak_killtoscore( var_0 );
    else
        return killstreaks( var_0 );
}
...
killstreaktoscorestreak_killtoscore( var_0 )
{
    ...
    var_3 = anglestoforward( var_0.select_bridge_one_spawners.angles );
    var_4 = anglestoright( var_0.select_bridge_one_spawners.angles );
...
watchinfiljumpanim( var_0 )
{
    ...
    if ( var_6 )
    {
        ...
        {
            var_8 = 0;
            [var_10, var_11, var_12] = killstreaktoscorestreak_scoretokill( var_0 );
            var_13 = killcam_timescalefactor( var_0 );
        ...
        foreach ( var_4 in var_2 )
        {
            ...
            var_4 scripts\mp\gametypes\br.gsc::subscribedlocale();
        }
...
playinfilplayeranims( var_0, var_1 )
{
    ...
    foreach ( var_5 in var_3 )
    {
        ...
        var_5 scripts\mp\gametypes\br.gsc::submap_cull_targetname();
        scripts\mp\utility\game::_id_11AB7( var_5, 1 );
        var_5.plotarmor = 1;
        var_5 unlink();
        var_5 playerlinktopositionent( var_0 );
        var_6 = var_5 successfunction( var_0 );
...
successfunction( var_0 )
{
    if ( play_go_to_safehouse() )
    {
        ...
        if ( self.no_previous_interaction_point )
        {
...
terminal_pusher_approach_array( var_0, var_1 )
{
    var_0 endon( "stopScene" );
    ...
    foreach ( var_4 in var_2 )
    {
        ...
        var_4 scripts\mp\utility\player::_id_11B67( "100", 1 );
    }

    var_0.aidoorchief scripts\mp\utility\player::_id_11B67( "100" );
}
...
directimpactkill( var_0, var_1 )
{
    ...
    var_0.aidoorchief scripts\mp\utility\player::_id_11B67( "0" );
}

terminal_pusher_approach_array_counter( var_0, var_1 )
{
    var_0 endon( "stopScene" );
    var_2 = scripts\engine\utility::ter_op( isdefined( var_1 ), var_1, level.players );
    ...
    foreach ( var_4 in var_2 )
    {
        ...
        var_4 scripts\mp\utility\player::_id_11B67( "80", 1 );
    }
}

disable_all_turrets_permanently( var_0, var_1 )
{
    var_2 = scripts\engine\utility::ter_op( isdefined( var_1 ), var_1, level.players );
    ...
    foreach ( var_3 in var_1 )
    {
        ...
        if ( isdefined( var_4 ) && var_4.size )
        {
            ...
            if ( var_5 > 0 )
            {
                ...
                scripts\mp\utility\script::fusefx( var_6 );
            }
        }
    }
}

timeoflastexecute( var_0, var_1 )
{
    vehicle_stop_moving( var_1 );

    if ( play_go_to_safehouse() )
        start_safehouse_return( var_0, var_1 );
    else
...
start_safehouse_return( var_0, var_1 )
{
    thread _id_11ACE( var_1 );
    start_safehouse_restart( var_0, var_1 );
    _id_12356( var_0, var_1 );
    globalstruct( var_0, var_1 );
}

playac130infilanim( var_0, var_1 )
{
    thread _id_11ACE( var_1 );
    start_link_logic_on_players( var_0, var_1 );
    _id_12355( var_0, var_1 );
    global_relic_team_prox_func( var_0, var_1 );
}
...
disablepersupdates( var_0 )
{
    ...
    triggerfx( var_0.disableplayerkillrewards );
}
...
helis_assault3_fob_check_size( var_0, var_1 )
{
    level endon( "game_ended" );
    var_2 = killandvision( var_0 );
    ...
    if ( var_4 )
    {
        level.node_get_children = newhudelem();
        _id_11B40( level.node_get_children );
    }
    else
    {
        foreach ( var_6 in var_3 )
        {
            ...
            var_6.node_get_children = newclienthudelem( var_6 );
            _id_11B40( var_6.node_get_children );
        }
    }

    [var_9, var_10] = killable_targets();
    var_2 waittillmatch( "camera", "fadeinstart" );

    if ( var_4 )
    {
        level.node_get_children fadeovertime( var_9 );
        level.node_get_children.alpha = 1;
    }
    else
    {
        foreach ( var_6 in var_3 )
        {
            ...
            var_6.node_get_children fadeovertime( var_9 );
            var_6.node_get_children.alpha = 1;
        }
    }
    ...
    if ( var_4 )
    {
        level.node_get_children fadeovertime( var_10 );
        level.node_get_children.alpha = 0;
    }
    else
    {
        foreach ( var_6 in var_3 )
        {
            ...
            var_6.node_get_children fadeovertime( var_10 );
            var_6.node_get_children.alpha = 0;
        }
    }

    _id_128C2( var_10 + 0.1 );

    if ( var_4 )
    {
        level.node_get_children destroy();
        level.node_get_children = undefined;
    }
    else
    {
        foreach ( var_6 in var_3 )
        {
            ...
            var_6.node_get_children destroy();
        }
    }
}
...
node_set_children()
{
    if ( istrue( level.carepackage_set_collision_model ) )
        return;

    if ( getdvarint( "scr_br_c130_intro_s2", 0 ) == 1 )
        return;

    level.carepackage_set_collision_model = 1;
    ...
    wait 24.0;
    level.carepackage_set_collision_model = undefined;
}
...
{
    if ( isdefined( var_0.delay_thirdpersoncamera ) )
        var_0.delay_thirdpersoncamera hide();

    var_0.create_bomb_wire_info scripts\engine\utility::waittill_match_or_timeout( "camera", "showdoor", 30.0 );

    if ( isdefined( var_0.delay_thirdpersoncamera ) )
        var_0.delay_thirdpersoncamera show();
}

bomber_death_watcher( var_0, var_1 )
{
    var_2 = undefined;
    var_3 = 0;
    ...
    if ( var_0.infil_anim_type == "player" )
    {
        if ( !isdefined( var_1 ) )
        {
            ...
            {
                var_6 = scripts\mp\gametypes\br_public.gsc::lb_impulse_dmg_threshold_top( var_5 );

                for ( var_7 = 0; var_7 < var_6.size; var_7++ )
                {
                    var_8 = var_6[var_7];
                    var_9 = scripts\mp\gametypes\br_public.gsc::lb_mg_dmg_factor_fuselage( var_5, var_8 );
                    var_10 = var_9.size == 1;
                    var_11 = clients( var_9.size );

                    foreach ( var_18, var_13 in var_9 )
                    {
                        var_13 playerhide();

                        if ( var_13 scripts\mp\gametypes\br_public.gsc::player_give_intel_2_ks() )
                            var_13.infilanimindex = 1;
                        else
                        {
                            var_14 = scripts\engine\utility::random( var_11 );
                            var_13.infilanimindex = var_14;
                            var_11 = scripts\engine\utility::array_remove( var_11, var_14 );
                        }

                        var_13.no_previous_interaction_point = var_10;

                        if ( !var_3 )
                        {
                            foreach ( var_16 in var_9 )
                                var_13 showtoplayer( var_16 );
                        }
                    }
                }
            }
        ...
        foreach ( var_13 in var_1 )
        {
            ...
            var_13 playerhide();
            var_13 showtoplayer( var_13 );
            var_13.infilanimindex = 1;
            var_13.no_previous_interaction_point = 1;
        }
...
start_link_logic_on_players( var_0, var_1 )
{
    ...
    if ( isdefined( var_0.create_badplace_extraction ) )
        var_0.create_badplace_extraction scriptmodelplayanim( "sdr_mp_infil_ac130_redux_players_cam", "camera" );
    ...
    foreach ( var_10 in var_8 )
    {
        ...
        if ( !var_7 )
        {
            ...
            if ( !istrue( level.playerhastrock ) && !isdefined( level.wait_for_sequence_start ) )
                var_10 setsoundsubmix( "mp_br_infil_ac130", 0.0 );

            var_10 setclienttriggeraudiozone( "mp_donetsk_infil_int", 1.0 );
        }

        var_0.create_apc_vehicle_interaction = "j_prop_" + var_10.infilanimindex;
        var_10 cameraunlink();
        var_10 cameralinkto( var_0.cameraent, var_0.create_apc_vehicle_interaction, 1, 1 );
    ...
    thread disable_collect_leads( var_1 );
    ...
    level thread setup_train_entarray( var_0 );
    level thread updateteamscoreplacements( var_0 );
    level thread loadout_giveweaponobj( var_0 );
    level thread terminal_pusher_approach_array( var_0, var_1 );
    level thread disablepersupdates( var_0 );
    level thread helis_assault3_fob_check_size( var_0, var_1 );
    level thread no_csm( var_0 );
    level thread forcegivekillstreak( var_0, var_1 );
    ...
    level directimpactkill( var_0, var_1 );
}

start_safehouse_restart( var_0, var_1, var_2, var_3 )
{
    var_0 endon( "stopScene" );
    bomber_death_watcher( var_0, var_1 );
    var_4 = 48;
    ...
    var_7 = 2.0;
    var_8 = 4;
    var_0.create_bomb_wire_info scriptmodelplayanim( "wz_infil_mindia8_solo_cam", "camera" );

    if ( isdefined( var_0.create_cam ) )
        var_0.create_cam scriptmodelplayanim( "wz_infil_mindia8_solo_cam", "camera" );

    var_0.create_chopper_boss scriptmodelplayanim( "wz_infil_mindia8_squad_cam", "camera" );

    if ( isdefined( var_0.create_circular_path_around ) )
        var_0.create_circular_path_around scriptmodelplayanim( "wz_infil_mindia8_squad_cam", "camera" );

    if ( isdefined( var_0.select_bridge_one_spawners ) )
        var_0.select_bridge_one_spawners stoploopsound();

    var_0.armsrace_c4_plant_spot_reset_on_failed_plant hide();
    var_0.arenaknivesout hide();
    ...
    foreach ( var_11 in var_9 )
    {
        ...
        var_12 = scripts\engine\utility::ter_op( var_11.no_previous_interaction_point, var_0.create_bomb_wire_info, var_0.create_chopper_boss );
        var_13 = "j_prop_1";
        var_11 cameraunlink();
        ...
        var_0.armsrace_c4_plant_spot_reset_on_failed_plant showtoplayer( var_11 );
        var_0.arenaknivesout showtoplayer( var_11 );
    }

    thread disable_collect_leads( var_1 );

    if ( isdefined( var_0.select_bridge_one_spawners ) )
        var_0.select_bridge_one_spawners setscriptablepartstate( "infil_fx_hero", "on" );
    ...
    if ( var_0.infil_anim_type == "script_model" )
    {
        if ( istrue( var_2 ) )
        {
            var_0.targets_killed_by_hunters show();
            var_0.targets_killed_stat_row hide();
            var_0.targetsite hide();
            var_0.targetsshot hide();
            var_0.targetteamremaining hide();
        }
        else
        {
            var_0.targets_killed_by_hunters hide();
            var_0.targets_killed_stat_row show();
            var_0.targetsite show();
            var_0.targetsshot show();
            var_0.targetteamremaining show();
        }
        ...
        var_0.targets_killed_by_hunters scriptmodelplayanim( "wz_infil_mindia8_solo_player" + var_15, "p1" );
        var_0.targets_killed_stat_row scriptmodelplayanim( "wz_infil_mindia8_squad_player1" + var_15, "p1" );
        var_0.targetsite scriptmodelplayanim( "wz_infil_mindia8_squad_player2" + var_15, "p2" );
        var_0.targetsshot scriptmodelplayanim( "wz_infil_mindia8_squad_player3" + var_15, "p3" );
        var_0.targetteamremaining scriptmodelplayanim( "wz_infil_mindia8_squad_player4" + var_15, "p4" );
    }
    else if ( var_0.infil_anim_type == "player" )
        thread playinfilplayeranims( var_0, var_1 );

    var_0.armsrace_c4_plant_spot_reset_on_failed_plant scriptmodelplayanim( "wz_infil_mindia8_solo_pilot", "aiPilot" );
    var_0.arenaknivesout scriptmodelplayanim( "wz_infil_mindia8_solo_copilot", "aiCopilot" );
    var_0.binocularsinited scriptmodelplayanim( "wz_infil_mindia8_armada", "armadaRig" );

    foreach ( var_17 in var_0.binoculars_watchracetake )
    {
    ...
    level thread terminal_pusher_approach_array_counter( var_0, var_1 );
    level thread delay_show_balloon( var_0 );
    level thread helis_assault3_fob_check_size( var_0, var_1 );
    level thread no_enemy_weapon_drops( var_0 );
    level thread forcegivekillstreak( var_0, var_1 );
    var_0.create_bomb_wire_info scripts\engine\utility::waittill_match_or_timeout( "camera", "transition", 30.0 );
    var_0 notify( "infil_reset_light_dvars" );
    level disable_all_turrets_permanently( var_0, var_1 );
}

forcegivekillstreak( var_0, var_1 )
{
    var_2 = killcam_timescalefactor( var_0 );
    ...
    if ( isdefined( var_2 ) && getdvarint( "scr_br_streamToMovingPlane", 1 ) == 1 )
    {
        ...
        foreach ( var_9 in var_7 )
        {
            ...
            var_9 scripts\mp\gametypes\br_public.gsc::teamhasuav_ignorecuav( var_4 );
        }
    }
}
...
_id_1203E( var_0, var_1 )
{
    ...
    if ( isdefined( var_2 ) && getdvarint( "scr_br_streamToMovingPlane", 1 ) == 1 )
    {
        ...
        foreach ( var_6 in var_4 )
        {
            ...
            var_6 thread scripts\mp\gametypes\br_public.gsc::stream_hit_location_for_players_inside();
        }
    }
}

_id_12519( var_0, var_1 )
{
    wait( var_0 );
    var_2 = scripts\engine\utility::ter_op( isdefined( var_1 ), var_1, level.players );
...
_id_12355( var_0, var_1 )
{
    ...
    if ( isdefined( var_0.create_badplace_extraction ) )
    {
        ...
        foreach ( var_9 in var_7 )
        {
            ...
            var_0.create_apc_vehicle_interaction = "j_prop_" + var_9.infilanimindex;
            var_9 cameraunlink();
            var_9 cameralinkto( var_0.create_badplace_extraction, var_0.create_apc_vehicle_interaction, 1, 1 );
            var_9 setclientdvar( "QTSPTNLOL", var_2 );
            var_9 setphysicaldepthoffield( var_3, var_4, var_5, var_6 );
        }

        thread disable_bomb_detonator_interactivity( var_1 );
    }
    else
    {
        var_7 = scripts\engine\utility::ter_op( isdefined( var_1 ), var_1, level.players );
    ...
    if ( isdefined( var_0.disableplayerkillrewards ) )
    {
        if ( isdefined( var_0.movingc130 ) )
            var_0.disableplayerkillrewards.origin = var_0.movingc130.origin;
    }
    ...
    thread disable_cinematic_skip( var_1 );
    ...
    if ( isdefined( var_0.create_badplace_extraction ) )
        var_0.create_badplace_extraction waittillmatch( "camera", "end" );

    thread _id_12519( 1.2, var_1 );
}
...
_id_12356( var_0, var_1 )
{
    ...
    if ( isdefined( var_0.create_cam ) && isdefined( var_0.create_circular_path_around ) )
    {
        var_7 = scripts\engine\utility::ter_op( isdefined( var_1 ), var_1, level.players );
        ...
        foreach ( var_9 in var_7 )
        {
            ...
            var_10 = scripts\engine\utility::ter_op( var_9.no_previous_interaction_point, var_0.create_cam, var_0.create_circular_path_around );
            var_11 = "j_prop_1";
            var_9 cameraunlink();
    ...
    if ( isdefined( var_0.select_bridge_one_spawners ) )
    {
        var_0.playerpositionents["parent_solo"] linkto( var_0.select_bridge_one_spawners, "", ( 0, 0, 0 ), ( 0, 0, 0 ) );
        var_0.playerpositionents["parent_squad"] linkto( var_0.select_bridge_one_spawners, "", ( 0, 0, 0 ), ( 0, 0, 0 ) );
        var_0.select_bridge_one_spawners notify( "start_moving" );
    }

    var_0.binocularsinited unlink();

    if ( isdefined( var_0.select_bridge_one_spawners ) )
        var_0.binocularsinited linkto( var_0.select_bridge_one_spawners, "", ( 0, 0, 0 ), ( 0, 0, 0 ) );
    ...
    if ( var_0.infil_anim_type == "script_model" )
    {
        var_0.targets_killed_by_hunters scriptmodelplayanim( "wz_infil_mindia8_loop_pl01", "p1" );
        var_0.targets_killed_stat_row scriptmodelplayanim( "wz_infil_mindia8_loop_pl01", "p1" );
        var_0.targetsite scriptmodelplayanim( "wz_infil_mindia8_loop_pl02", "p2" );
        var_0.targetsshot scriptmodelplayanim( "wz_infil_mindia8_loop_pl03", "p3" );
        var_0.targetteamremaining scriptmodelplayanim( "wz_infil_mindia8_loop_pl04", "p4" );
    }

    var_0.armsrace_c4_plant_spot_reset_on_failed_plant hide();
    var_0.arenaknivesout hide();
    level thread strafe( var_0 );

    if ( isdefined( var_0.create_cam ) )
        var_0.create_cam waittillmatch( "camera", "end" );
...
global_relic_team_prox_func( var_0, var_1 )
{
    ...
    level thread _id_1203E( var_0, var_1 );
}
...
globalstruct( var_0, var_1 )
{
    ...
    if ( isdefined( var_0.select_bridge_one_spawners ) )
        var_0.select_bridge_one_spawners playloopsound( "br_heli_infil_hero_lp" );
...
node_fields_after_goal_skit( var_0 )
{
    if ( play_go_to_safehouse() )
        node_cansee_child( var_0 );
    else
        nocrouch( var_0 );
}
...
strafe( var_0 )
{
    if ( isdefined( var_0.select_bridge_one_spawners ) )
    {
        var_0.select_bridge_one_spawners scriptmodelplayanim( "wz_infil_mindia8_loop_veh", "plane" );
        var_0.select_bridge_one_spawners.innards scriptmodelplayanim( "wz_infil_mindia8_loop_veh", "planeInnards" );
    }
...
nonbunkerdoors( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    else if ( isdefined( level.checkbluemassacre ) && isdefined( level.checkbluemassacre.name ) && level.checkbluemassacre.name == "zxp" )
    ...
    if ( var_6 )
    {
        if ( istrue( level.playerhastrock ) || var_2 )
            getscriptcachecontents( var_1 );
        else
...
non_detectable_killstreaks( var_0, var_1 )
{
    if ( var_0 == "bink_complete" )
    {
...
{
    if ( getdvarint( "scr_br_c130_intro_s2", 0 ) == 1 )
        scripts\mp\utility\lui_game_event_aggregator::registeronluieventcallback( ::nodetype );
    else
        scripts\mp\utility\lui_game_event_aggregator::registeronluieventcallback( ::non_detectable_killstreaks );
...
killcam_timescalefactor( var_0 )
{
    if ( play_go_to_safehouse() )
        return var_0.select_bridge_one_spawners;
    else
...
{
    if ( play_go_to_safehouse() )
        return var_0.create_cam;
    else
        return var_0.create_badplace_extraction;
}

killed_by_chopper( var_0 )
{
    var_1 = 1;
    ...
    if ( play_go_to_safehouse() )
    {
        if ( istrue( var_0.no_previous_interaction_point ) )
        {
...
branalytics_plunder_placed_into_extraction( var_0, var_1, var_2 )
{
    level endon( "game_ended" );
    ...
    level.infilstruct.transporttime = scripts\mp\gametypes\br_c130.gsc::spawnc130( level.infilstruct.c130pathstruct, ::losqueuehigh, ::loschecktime );
    level.infilstruct.firstc130endtime = level.infilstruct.transporttime + gettime();
    var_7 = clearhint( level.br_ac130, var_1 );
    level.prev_weapon_taccover = var_7;

    if ( isdefined( level.trap_consoles ) )
        [[ level.trap_consoles ]]( var_7 );

    level.nocrash = 1;
    level.allowprematchdamage = 0;
    level.br_ac130 thread gunship_updateplayercount();
    var_8 = killedenemy( var_7 );

    if ( getdvarint( "scr_br_c130_intro_s2", 0 ) == 1 )
    {
        _id_1235D( var_3, "player", var_8, 1 );
        nonnvgnightmap( var_3, "mp_donetsk_c130_intro_s2" );
        nonbunkerdoors( var_3, "mp_donetsk_c130_intro_s2", 0, 0, "s2" );
        noprone( var_3 );
    ...
    else if ( scripts\mp\utility\game::lb_mg_impulse_dmg_threshold_top() == "reveal" )
    {
        _id_1235D( var_3, var_1, var_8, 0 );
        var_13 = undefined;
        var_14 = 0;
        ...
        nonnvgnightmap( var_3, var_13 );
        nonbunkerdoors( var_3, var_13, 0, var_14, "infil_dov_part1" );
        noprone( var_3 );
    }
    else
    {
        ...
        nonnvgnightmap( var_3, var_13 );
        nonbunkerdoors( var_3, var_13, 0, var_14, var_15 );
        noprone( var_3 );
    }

    if ( scripts\mp\gametypes\br_public.gsc::play_death_audio() || scripts\mp\gametypes\br_public.gsc::play_patrol_sequence_based_on_first_vehicle() )
    {
        var_3 = scripts\engine\utility::ter_op( isdefined( var_2 ), var_2, level.players );
    ...
    if ( getdvarint( "scr_br_c130_intro_s2", 0 ) == 1 )
        damage_stage_final_watcher( var_3, 0 );

    [var_19, var_20] = kill_vip();
    level thread infilallfadetoblack( 0.0, var_19, var_20, undefined, var_2 );
    timeoflastexecute( var_7, var_2 );
    var_3 = scripts\engine\utility::ter_op( isdefined( var_2 ), var_2, level.players );
    _id_1235B( var_3, var_1 );
    level.nocrash = 0;
    level.infilstruct.infil_anim_type = var_1;

    if ( isdefined( level.handleimpact ) )
        level thread scripts\mp\gametypes\br_alt_mode_escape.gsc::handlepersistentlandlocked( var_2 );
    else
    {
        thread scripts\mp\gametypes\br_gametypes.gsc::watch_for_player_going_belowmap_or_oob( "onInfilSequenceEnd", var_2 );

        if ( !scripts\mp\gametypes\br_gametypes.gsc::play_station_closed_vo( "match_start_VO" ) )
            scripts\mp\gametypes\br_public.gsc::brleaderdialog( "match_start", 0, var_2 );
    }

    var_21 = scripts\mp\gametypes\br_public.gsc::playerexecutionsenable() || scripts\mp\gametypes\br_public.gsc::play_player_approach();
    ...
    if ( !var_21 )
    {
        ...
        foreach ( var_5 in var_3 )
        {
            ...
            if ( !istrue( var_5.carepackage_unlink_from_heli ) && var_22 )
                continue;

            var_5 thread playerjoininfil();

            if ( var_5 isplayerheadless() )
                validate_track( var_5 );
        }

        if ( scripts\mp\gametypes\br_gametypes.gsc::play_disguise_vo( "addToC130Infil" ) )
            level.br_ac130 scripts\mp\gametypes\br_gametypes.gsc::watch_for_player_going_belowmap_or_oob( "addToC130Infil" );
    }

    if ( isdefined( level.matchdata_logweaponstat ) )
        level thread matchdata_onmatchstart();
}
...
infilallfadetoblack( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    ...
    if ( var_7 && !isdefined( level.infilblackoverlay ) )
    {
        ...
        _id_11B3F( level.infilblackoverlay );
    }
    else if ( !var_7 )
    {
        foreach ( var_9 in var_6 )
        {
            ...
            _id_11B3F( var_9.infilblackoverlay );
        }
    }
...
_id_1235D( var_0, var_1, var_2, var_3 )
{
    foreach ( var_5 in var_0 )
    {
        ...
        var_5 scripts\mp\class::brloot_ammo();
    }
    ...
    _id_128C0( var_0, var_2.origin, var_2 );
    ...
    foreach ( var_5 in var_0 )
    {
        ...
        var_5 grenade_menu_kill();
        var_5.carepackage_unlink_from_heli = 1;
    }
...
_id_1235B( var_0, var_1 )
{
    ...
    foreach ( var_4 in var_0 )
    {
        ...
        var_4 scripts\mp\class::_id_12516();
    ...
    setomnvar( "ui_in_infil", -1 );
    level.nogroundfoundtime = undefined;
}
...
playerjoininfil()
{
    ...
    var_0 scripts\mp\gametypes\br_public.gsc::tank_check_player_firing();
    var_0 thread strafe_pass_target_dist();
}
...
handleexclusionils( var_0, var_1 )
{
    ...
    if ( !istrue( level.create_vehicle_omnvars_data ) && !istrue( var_2.issquadleader ) )
        return 0;
    ...
    if ( istrue( level.create_vehicle_omnvars_data ) && !istrue( var_2.issquadleader ) )
        return var_2 make_outline_ents( var_0, var_1, var_6 );

    foreach ( var_9 in level.br_spawns )
    {
        var_10 = var_2 lb_mg_dmg_factor_main_rotor();

        if ( isdefined( var_10 ) && var_9 == var_10 )
        {
            if ( level.create_vehicle_omnvars_data )
            {
                if ( var_2 debugtype( var_9, var_6, var_0, 1 ) )
                    return 1;
            }

            continue;
        }
        else if ( level.create_vehicle_omnvars_data )
        {
            if ( !var_2 debugtype( var_9, var_6, var_0, 0 ) )
                return 0;
        }
...
make_outline_ents( var_0, var_1, var_2 )
{
    var_3 = self;

    foreach ( var_5 in level.br_spawns )
    {
        var_6 = lb_mg_dmg_factor_main_rotor();
        var_7 = 0;
        ...
        if ( pow( var_2 + var_5.spawnexclusion2ddist, 2 ) > distance2dsquared( var_5.locationorigin, var_0 ) )
        {
            if ( var_7 )
            {
                var_8 = var_3 launcher_second_techo();
                fulton_open( var_8 );
        ...
        if ( var_7 && var_3 debugtype( var_5, var_2, var_0, var_7 ) )
            return 1;
    }
...
debugtype( var_0, var_1, var_2, var_3 )
{
    var_4 = self;

    foreach ( var_6 in var_0._id_11CD9 )
    {
        if ( !isdefined( var_6 ) )
            continue;

        var_7 = var_4 launcher_second_techo();
...
registerspawnlocation( var_0, var_1, var_2 )
{
    ...
    {
        var_5 = var_3 lb_mg_dmg_factor_main_rotor();

        if ( !isdefined( var_5 ) )
            var_5 = var_3 overtime_music();
    }
    else if ( istrue( level.create_vehicle_omnvars_data ) )
    {
        var_5 = var_3 launcher_second_techo();

        if ( !isdefined( var_5 ) )
            var_5 = var_3 overridefieldupgrade2();
    }
    else
        return 0;
...
_id_121D2()
{
    ...
    var_0 playershow( 1 );
    var_0 unlink( 1 );
    var_1 = var_0 lb_mg_dmg_factor_main_rotor();

    if ( isdefined( var_1.locationorigin ) )
    {
        var_2 = var_1.locationorigin;
        var_3 = var_0 launcher_second_techo();
...
{
    var_0 = self;
    var_1 = var_0 launcher_second_techo();
    var_2 = var_0 lb_mg_dmg_factor_main_rotor();
...
spawnselectionend()
{
    ...
    foreach ( var_2 in level.players )
    {
        ...
        var_3 = var_2 lb_mg_dmg_factor_main_rotor();
    ...
    thread vehicle_remove_invulnerability_onenter();
    wait 4;
...
launcher_second_techo()
{
    ...
    if ( !istrue( level.create_vehicle_omnvars_data ) || istrue( var_0.issquadleader ) )
        return undefined;
    ...
    {
        if ( isdefined( var_1._id_11CD9[var_0.pers["squadMemberIndex"]] ) && isdefined( var_1._id_11CD9[var_0.pers["squadMemberIndex"]].locationorigin ) )
            return var_1._id_11CD9[var_0.pers["squadMemberIndex"]];
    }

    return undefined;
}

lb_mg_dmg_factor_main_rotor()
{
    var_0 = self;
...
overridefieldupgrade2()
{
    ...
    {
        var_1._id_11CD9[var_0.pers["squadMemberIndex"]] = spawnstruct();
        var_1._id_11CD9[var_0.pers["squadMemberIndex"]].circleent = undefined;
        return var_1._id_11CD9[var_0.pers["squadMemberIndex"]];
    }
...
overtime_music()
{
    ...
    if ( isdefined( level.br_spawns ) && isdefined( var_0.team ) )
    {
        ...
        level.br_spawns[var_0.team]._id_11CD9 = [];
        return level.br_spawns[var_0.team];
    }
...
vehicle_remove_invulnerability_onenter()
{
    ...
    if ( isdefined( level.br_spawns ) )
    {
        ...
        {
            if ( isdefined( var_1._id_11CD9 ) )
            {
                foreach ( var_3 in var_1._id_11CD9 )
                    fulton_open( var_3 );
            }

            fulton_planted( var_1 );
        }
    }
}
...
select_back_one_spawners( var_0, var_1, var_2, var_3 )
{
    self moveto( var_0, var_1, var_2, var_3 );
}

waittill_player_completes_scavenger_contract( var_0, var_1, var_2, var_3 )
{
    self rotateto( var_0, var_1, var_2, var_3 );
}

matchdata_onmatchstart()
{
    var_0 = distance( level.br_ac130.spawn_boss_wave_section.startpt, level.br_ac130.spawn_boss_wave_section.endpt );
    var_1 = var_0 / scripts\mp\gametypes\br_c130.gsc::getc130speed() / level.matchdata_logweaponstat.size;

    for ( var_2 = 0; var_2 < level.matchdata_logweaponstat.size; var_2++ )
    {
        wait( var_1 );
        level.matchdata_logweaponstat[var_2] notify( "halo_jump_c130" );
    }

    level.matchdata_logweaponstat = undefined;
}

validate_track( var_0 )
{
    if ( !isdefined( level.matchdata_logweaponstat ) )
        level.matchdata_logweaponstat = [];

    level.matchdata_logweaponstat[level.matchdata_logweaponstat.size] = var_0;
}
