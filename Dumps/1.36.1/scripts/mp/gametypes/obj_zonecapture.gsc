...
setupobjective( var_0, var_1, var_2, var_3 )
{
    ...
    if ( istrue( level.lootcontentsadjusteconomy_toptier ) )
    {
        if ( scripts\mp\utility\game::getgametype() == "hq" )
            var_4 thread _id_128F0( var_4 );
        else if ( scripts\mp\utility\game::getgametype() == "grnd" )
            var_4 thread _id_128EF( var_4 );
    }
...
_id_128F0( var_0 )
{
    ...
    playfx( level.movequestlocale["ghostcat_hw"], var_3 + var_2 );
}

_id_128EF( var_0 )
{
    scripts\mp\flags::gameflagwait( "prematch_done" );
    playfx( level.movequestlocale["blood_floor_hw"], getgroundposition( var_0.trigger.origin, 4 ) + ( 0, 0, 2 ) );
}
...
assignchevrons( var_0, var_1 )
{
    ...
    var_2 = vehicle_occupancy_isfriendlytoteam( var_2, var_1 );
    var_2 = train_play_anim( var_2, var_1 );
    var_3 = [];
...
activatezone()
{
    ...
    if ( isdefined( level.kill_rate_too_slow ) )
        var_0 = [[ level.kill_rate_too_slow ]]();

    var_1 = 1024;

    if ( isdefined( level.kill_rate_watcher ) )
        var_1 = [[ level.kill_rate_watcher ]]();
    ...
    self.extraction_helicoptor_total_plunder = var_2;
...
deactivatezone()
{
    ...
    foreach ( var_1 in self.extraction_helicoptor_total_plunder )
    ...
    level.weapon_xp_iw8_sn_sbeta = 0;
}
...
cleanupgametypevips()
{
    level scripts\engine\utility::_id_128FF( "game_ended", "zone_moved" );
...
zone_onuse( var_0 )
{
    ...
    if ( level.usehqrules && !level.usehprules && self.ownerteam != "neutral" )
    {
        ...
        if ( isdefined( var_3 ) )
        {
            ...
            foreach ( var_11 in var_9 )
            {
                ...
                var_7 scripts\cp\agents\gametype_cp_specops::setuptimelimit();
            }
        }
    ...
    else
    {
        ...
        if ( scripts\mp\utility\game::getgametype() == "hq" && !level.usehprules )
        {
            ...
            if ( var_18 > var_19 )
                level.weapon_xp_iw8_sn_sbeta = 1;

            ammo_restock( var_1 );
        }
        else if ( level.spawn_convoy_and_move )
    ...
    if ( istrue( level.lootcontentsadjusteconomy_toptier ) )
        playfx( level.movequestlocale["vanish_hw_fr"], level.zone.trigger.origin + ( 0, 0, 40 ) );
}
...
zone_onuseupdate( var_0, var_1, var_2, var_3 )
{
    ...
    if ( var_4 == "neutral" && self.claimteam != var_4 )
        ammo_restock( self.claimteam );
    else if ( var_4 != "neutral" && self.claimteam != var_4 )
        ammo_restock( var_4, 1 );
    else if ( level.spawn_convoy_and_move )
        level scripts\mp\gamelogic::pausetimer();
...
zone_oncontested()
{
    ...
    level.zone thread scripts\common\utility::_id_12456( level.remove_objective_on_flag, "hill_contested", level.zone.trigger.origin );
    self.didstatusnotify = 1;
}
...
zone_onuncontested( var_0 )
{
    ...
    if ( var_0 == "none" || var_1 == "neutral" )
    {
        ...
        level.zone thread scripts\common\utility::_id_12456( level.remove_objective_on_flag, "hill_empty", level.zone.trigger.origin );
    }
    else
    {
        if ( scripts\mp\utility\game::getgametype() == "koth" )
        ...
        level.zone thread scripts\common\utility::_id_12456( level.remove_objective_on_flag, "hill_uncontested", level.zone.trigger.origin );
    }

    var_7 = ( gettime() - self.dontspawncargotruck ) * 0.001;
    scripts\mp\utility\game::rear_spotlight_target_found_dist_sq( undefined, undefined, "Zone Contested", level.zone.trigger.origin, var_7 + " seconds" );
...
_id_12A01()
{
    ...
    foreach ( var_2 in level.teamnamelist )
    {
        self.init_relic_mythic[var_2] = 0;
        self.init_relic_no_ammo_mun[var_2] = [];
    }

    self.init_relic_mythic["neutral"] = 0;
    self.init_relic_no_ammo_mun["neutral"] = [];
    self.init_relic_no_ammo_mun["none"] = [];
    self.deal_damage_after_time = 1;
    ...
    for (;;)
    {
        ...
        if ( scripts\mp\utility\player::isreallyalive( var_4 ) && !isdefined( var_4.touchtriggers[var_0] ) )
        {
            ...
            self.init_relic_mythic[var_2]++;
            var_6 = var_4.guid;
            var_7 = spawnstruct();
            var_7.player = var_4;
            var_7.starttime = gettime();
            self.init_relic_no_ammo_mun[var_2][var_6] = var_7;
            var_4.monitorhotfoot = 0;
        }

        if ( scripts\mp\utility\player::isreallyalive( var_4 ) && !isdefined( var_4.touchtriggers[var_0] ) )
            var_4 thread init_relic_nobulletdamage( self );
    }
}

init_relic_nobulletdamage( var_0 )
{
    level endon( "stop_watching_trigger" );
    ...
    if ( isdefined( self ) )
    {
        var_0.init_relic_no_ammo_mun[var_1][self.guid] = undefined;
        self.monitorhotfoot = 0;
    }
    else
    {
        var_2 = [];

        foreach ( var_5, var_4 in var_0.init_relic_no_ammo_mun[var_1] )
        {
        ...
        foreach ( var_5 in var_2 )
            var_0.init_relic_no_ammo_mun[var_1][var_5] = undefined;
    }

    var_0.init_relic_mythic[var_1]--;
}
