...
setupcommoncallbacks()
{
    ...
    level.longgulagstream = getdvarint( "scr_game_gunGameWeapons", 0 );

    if ( level.longgulagstream > 0 )
    {
        _id_11AD0();
    ...
    level.getburnfxstatepriority = 0;
    var_2 = getdvarint( "LTSNLQNRKO" ) && !getdvarint( "LSTLQTSSRM" );

    if ( var_2 )
    {
        level.getburnfxstatepriority = getdvarint( "scr_game_soccerevent", 0 );

        if ( getdvar( "NSQLTTMRMP" ) == "mp_m_stadium" && getdvarint( "scr_stadium_soccerball", 1 ) )
            level.getburnfxstatepriority = 1;
    }
    else if ( getdvar( "NSQLTTMRMP" ) == "mp_m_stadium" && !isintournament() && !isgamebattlematch() )
        level.getburnfxstatepriority = 1;

    if ( level.getburnfxstatepriority || istrue( var_0 ) )
    {
        reader();
        level._id_11B55 = ::_id_11B55;
        level thread [[ level._id_11B55 ]]();
        ...
        level.getburnfxstatepriority = undefined;

    if ( getdvarint( "scr_game_specialEventObjs", 0 ) == 1 )
    {
        level.loadout_copyclassstruct["gos_fireworks"] = loadfx( "vfx/iw8_mp/gamemode/vfx_gos_firework.vfx" );
        level.init_subway_cars = 1;
    }

    var_3 = scripts\mp\utility\game::matchmakinggame() && scripts\mp\utility\game::playercanbuyrespawn();

    if ( var_3 )
    {
        level.lootcontentsadjusteconomy_toptier = 1;
        level.movequestlocale["vanish_hw_fr"] = loadfx( "vfx/iw8_mp/gamemode/vfx_halloween_kc_capture_friendly.vfx" );
        level.movequestlocale["vanish_hw_en"] = loadfx( "vfx/iw8_mp/gamemode/vfx_halloween_kc_capture_enemy.vfx" );
        level.movequestlocale["ghostcat_hw"] = loadfx( "vfx/iw8_br/gameplay/hween/vfx_halloween_ghost_cat.vfx" );
        level.movequestlocale["bats_hw"] = loadfx( "vfx/iw8_br/gameplay/hween/vfx_hween_bats.vfx" );
        level.movequestlocale["bats_fly_hw"] = loadfx( "vfx/iw8_br/gameplay/hween/vfx_hween_bats_fly.vfx" );
        level.movequestlocale["blood_ceil_hw"] = loadfx( "vfx/iw8_br/gameplay/hween/vfx_halloween_blood_ceiling_01.vfx" );
        level.movequestlocale["blood_floor_hw"] = loadfx( "vfx/iw8_br/gameplay/hween/vfx_halloween_blood_floor_01.vfx" );

        if ( getdvarint( "scr_player_event_models", 1 ) > 0 )
        {
            var_4 = strtok( getdvar( "scr_player_hwhead_killcount", "3 10" ), " " );
            level.movequestobjicon = int( var_4[0] );
            level.mover_init = int( var_4[1] );
            level.shouldgamelobbyremainintact = ::shouldgamelobbyremainintact;
        }
    }

    level._id_11E44 = undefined;
...
updatecommongametypedvars()
{
    ...
    level.mpingamelootdrop = scripts\mp\utility\game::getgametype() == "hvt" || scripts\mp\utility\dvars::last_good_drop_pos( "scr_" + scripts\mp\utility\game::getgametype() + "_inGameLoot", "scr_game_inGameLoot" );
...
oncommonnormaldeath( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    ...
    if ( isbot( self ) && isdefined( self.play_counter_beep_sfx_on_bomb_vests ) )
    ...
    var_7 = scripts\mp\utility\game::islaststandenabled() && istrue( var_0.superonset );
...
onspawnplayercommon()
{
    ...
    if ( istrue( level.longgulagstream ) )
        thread _id_128E9();

    if ( istrue( level.lootcontentsadjusteconomy_toptier ) )
    {
        self.movequestcircle = 0;
...
onplayerkilledcommon( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9, var_10 )
{
    ...
    if ( isdefined( level.shot_to_miss_for_dialog ) )
        [[ level.shot_to_miss_for_dialog ]]( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9 );

    if ( isdefined( level.shouldgamelobbyremainintact ) )
        [[ level.shouldgamelobbyremainintact ]]( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9 );
}
...
shouldgamelobbyremainintact( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9 )
{
    ...
    {
        if ( !isdefined( var_1.movequestcircle ) )
            var_1.movequestcircle = 1;
        else
        {
            var_1.movequestcircle++;

            if ( var_1.movequestcircle == level.movequestobjicon )
            {
            ...
            else if ( var_1.movequestcircle == level.mover_init )
...
_id_128F6()
{
    ...
    level.ladderindex = level.longgulagstream;
    scripts\mp\gametypes\gun.gsc::setgunladder();
    scripts\mp\gametypes\gun.gsc::setgunsfinal();

    if ( play_3p_anim() )
        level.shot_to_miss_for_dialog = ::should_ai_take_damage;
    else
        level.shot_to_miss_for_dialog = ::shot_to_miss_for_dialog;

    scripts\mp\flags::gameflagset( "gungame_set" );
    level.gun_loadouts["axis"]["loadoutPrimary"] = getnextgun( 0 );
}

shot_to_miss_for_dialog( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9 )
{
    ...
    if ( var_3 == "MOD_FALLING" || isdefined( var_1 ) && isplayer( var_1 ) )
    {
        ...
        var_11 = scripts\mp\utility\weapon::isknifeonly( var_4.basename ) || scripts\mp\utility\weapon::platform_model( var_4.basename ) || scripts\mp\utility\weapon::isaxeweapon( var_4.basename ) || scripts\mp\utility\weapon::player_choppergunnericon( var_4 );
        ...
        if ( var_1 != self && var_3 == "MOD_PISTOL_BULLET" || var_3 == "MOD_RIFLE_BULLET" || var_3 == "MOD_HEAD_SHOT" || var_3 == "MOD_PROJECTILE" || var_3 == "MOD_PROJECTILE_SPLASH" || var_3 == "MOD_IMPACT" || var_3 == "MOD_GRENADE" || var_3 == "MOD_GRENADE_SPLASH" || var_3 == "MOD_EXPLOSIVE" || var_3 == "MOD_FIRE" || var_3 == "MOD_MELEE" && var_1 player_equipment_use_stop() && ( var_11 || var_10 || var_12 ) || var_3 == "MOD_MELEE" && !var_11 || var_12 )
        {
            var_14 = getweaponbasename( var_1.primaryweapon );

            if ( !var_1 create_thrust_fire( var_4, var_14, var_12, var_10, var_11 ) )
                return;
...
create_thrust_fire( var_0, var_1, var_2, var_3, var_4 )
{
...
should_ai_take_damage( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9 )
{
    if ( isdefined( var_1 ) && isplayer( var_1 ) )
    {
        if ( isdefined( self.pers["classCache"] ) && self.pers["classCache"].size > 0 )
...
play_get_in_sequence()
{
    return level.longgulagstream == 1;
}

play_gas_sequence()
{
    return level.longgulagstream == 2;
}

play_fulton_evac_on_player()
{
    return level.longgulagstream == 3;
}

play_3p_anim()
{
    return level.longgulagstream == 7 || level.longgulagstream == 8 || level.longgulagstream == 9 || level.longgulagstream == 10;
}
...
_id_11B55()
{
    ...
    level.send_notify_after_player_tac_vis = var_7;
    var_12 = undefined;
    ...
    level thread end_gates( var_1, 1 );
}

end_gates( var_0, var_1 )
{
    if ( isdefined( var_1 ) )
        wait( var_1 );
    ...
    var_5.vfx_height = var_0;
    var_5.playerattractiontriggerexit = 1;
    var_5.onreset = ::show_deposit_hint;
    var_5.should_break_stealth_immediately = ::should_break_stealth_immediately;
    var_5._id_11CC8 = ::_id_11EFF;
    var_6 = ( 0, 30, 0 );
    ...
    var_5 thread skipequipmentdropondeath();
    var_5 thread skipequippedstreakcheck();

    if ( getdvarint( "scr_game_soccergoal", 1 ) == 1 )
        var_5 thread _id_11CC6();

    if ( isdefined( var_5._id_11CC8 ) )
        var_5 [[ var_5._id_11CC8 ]]();

    level._id_11CC3[level._id_11CC3.size] = var_5;
}
...
show_deposit_hint( var_0 )
{
    self.mp_t_reflex_patch = 1;

    if ( !istrue( var_0 ) )
    {
        wait 0.5;
        self.visuals playsound( "soc_ball_vanish" );
        playfx( level._id_11CC2["vanish"], self.visuals.origin );
    }
    else
    {
        wait 0.1;
    ...
    self.visuals hide();
    wait 1;
    self.visuals.origin = self.vfx_height;
    var_1 = ( 0, 30, 0 );
    ...
    self.mp_t_reflex_patch = 0;

    if ( isdefined( self._id_11CC8 ) )
        self [[ self._id_11CC8 ]]();
}

should_break_stealth_immediately( var_0 )
{
    if ( istrue( self.mp_t_reflex_patch ) )
    {
        self.mp_t_reflex_patch = 0;
        return;
    }
    ...
    var_0.little_bird_mg_playercontrolmg++;
    level _id_125F4( var_0 );
    var_0 thread little_bird_mg_onenterheavydamagestate();
        ...
        foreach ( var_5 in level.players )
        {
            if ( var_5.team == var_1 )
            {
                if ( var_5.team == "allies" )
                    var_5 thread timed_death( "dx_mpa_rutl_summer_game_goal" + scripts\engine\utility::random( level.little_bird_mg_mp_waitandspawn ), "summer_game_goal", 2, "soccer_goal_them" );
                else
                    var_5 thread timed_death( "dx_mpa_uktl_summer_game_goal" + scripts\engine\utility::random( level.little_bird_mg_mp_waitandspawn ), "summer_game_goal", 2, "soccer_goal_them" );
            ...
            if ( var_5.team == "allies" )
            {
                var_5 thread timed_death( "dx_mpa_uktl_summer_game_goal" + scripts\engine\utility::random( level.little_bird_mg_mp_waitandspawn ), "summer_game_goal", 2, "soccer_goal_us" );
                continue;
            }

            var_5 thread timed_death( "dx_mpa_rutl_summer_game_goal" + scripts\engine\utility::random( level.little_bird_mg_mp_waitandspawn ), "summer_game_goal", 2, "soccer_goal_us" );
        }
    }
    else
    {
    ...
    self.visuals thread timed_laser_trap_trigger();
    ...
    if ( var_0.little_bird_mg_playercontrolmg >= 10 && isdefined( game["multiBall"] ) && !game["multiBall"] || getdvarint( "scr_game_forcemultiball", 0 ) == 1 && isdefined( game["multiBall"] ) && !game["multiBall"] )
    {
        game["multiBall"] = 1;
        ...
        if ( level.send_notify_after_player_tac_vis.size > 0 )
        {
            var_15 = getdvarint( "scr_game_soccermultiball", 3 );

            for ( var_16 = 0; var_16 < var_15; var_16++ )
                end_gates( level.send_notify_after_player_tac_vis[var_16] );
        }
    }
}
...
{
    for ( var_2 = 0; var_2 < min( self.little_bird_mg_playercontrolmg, 10 ); var_2++ )
    {
        playfx( level.loadout_copyclassstruct["gos_fireworks"], self.trigger.origin + ( randomintrange( -100, 100 ), randomintrange( -100, 100 ), randomintrange( -25, 60 ) ), anglestoforward( self.trigger.angles + ( randomintrange( int( max( clamp( 0 - var_2 * 5, -19, 0 ), -20 ) ), int( min( 20, 1 + var_2 * 5 ) ) ), randomintrange( int( max( clamp( 0 - var_2 * 5, -19, 0 ), -20 ) ), int( min( 20, 1 + var_2 * 5 ) ) ), 0 ) ), anglestoup( self.trigger.angles + ( randomintrange( int( max( clamp( 0 - var_2 * 5, -19, 0 ), -20 ) ), int( min( 20, 1 + var_2 * 5 ) ) ), randomintrange( int( max( clamp( 0 - var_2 * 5, -19, 0 ), -20 ) ), int( min( 20, 1 + var_2 * 5 ) ) ), 0 ) ) );

        if ( istrue( var_0 ) )
            level thread _id_128F1( self.trigger.origin );
...
_id_125F4( var_0 )
{
    foreach ( var_2 in level.autoassignlowteamconsistent )
    {
        if ( var_2.weaponfixup == var_0.team )
        {
            if ( var_0.little_bird_mg_playercontrolmg >= 10 )
            {
                var_2.scriptable setscriptablepartstate( "score", "10" );
                continue;
            }

            var_2.scriptable setscriptablepartstate( "score", "0" + scripts\engine\utility::string( var_0.little_bird_mg_playercontrolmg ) );
        }
    }
}
...
skipequippedstreakcheck()
{
    ...
    for (;;)
    {
        ...
        if ( distancesquared( self.visuals.origin, self.vfx_height ) > 9000000 )
        {
            if ( isdefined( self.onreset ) )
                self thread [[ self.onreset ]]();
        }
...
_id_11CC7()
{
    ...
    {
        level.attackerregenammo = spawnstruct();
        level.attackerregenammo.team = "allies";
        level.attackerregenammo.trigger = var_0[0];
        level.attackerregenammo.entsinside = [];
        level.bots_with_player_enemy = spawnstruct();
        level.bots_with_player_enemy.team = "axis";
        level.bots_with_player_enemy.trigger = var_1[0];
        level.bots_with_player_enemy.entsinside = [];
        level.bots_with_player_enemy.little_bird_mg_playercontrolmg = 0;

        if ( !isdefined( game["roundsPlayed"] ) || isdefined( game["roundsPlayed"] ) && game["roundsPlayed"] == 0 )
        {
            game["alliesGoals"] = 0;
            level.attackerregenammo.little_bird_mg_playercontrolmg = 0;
            game["axisGoals"] = 0;
            level.bots_with_player_enemy.little_bird_mg_playercontrolmg = 0;
        }
        else if ( game["switchedsides"] )
        {
            level.bots_with_player_enemy.little_bird_mg_playercontrolmg = game["alliesGoals"];
            level.attackerregenammo.little_bird_mg_playercontrolmg = game["axisGoals"];
        }
        else
        {
            level.attackerregenammo.little_bird_mg_playercontrolmg = game["alliesGoals"];
            level.bots_with_player_enemy.little_bird_mg_playercontrolmg = game["axisGoals"];
        }

        level._id_11CC4 = [ level.bots_with_player_enemy, level.attackerregenammo ];
        level thread _id_128F4();
    }
    else
...
{
    wait 1;
    level.attackerswaittime = scripts\engine\utility::getstructarray( "home", "targetname" );
    level.botstopmovingonlaststand = scripts\engine\utility::getstructarray( "away", "targetname" );
    level.autoassignlowteamconsistent = scripts\engine\utility::array_combine( level.attackerswaittime, level.botstopmovingonlaststand );

    foreach ( var_1 in level.attackerswaittime )
    {
        ...
        var_1.weaponfixup = "allies";
    }

    foreach ( var_1 in level.botstopmovingonlaststand )
    {
        var_1.scriptable = spawn( "script_model", var_1.origin );
        var_1.scriptable.angles = var_1.angles;
        var_1.scriptable setmodel( "vfx_stadium_scoreboard_scriptable" );
        var_1.weaponfixup = "axis";
    }

    level _id_125F4( level.attackerregenammo );
    level _id_125F4( level.bots_with_player_enemy );
}
...
_id_11CC6( var_0 )
{
    ...
    while ( !isdefined( level._id_11CC4 ) )
        waitframe();
    ...
    for (;;)
    {
        ...
        for ( var_1 = 0; var_1 < level._id_11CC4.size; var_1++ )
        {
            if ( !playerentersafearea( level._id_11CC4[var_1], self.visuals ) )
                continue;

            if ( !_id_11BB3( level._id_11CC4[var_1], self.visuals ) )
                continue;

            var_2 = self.visuals getentitynumber();
            level._id_11CC4[var_1].entsinside[var_2] = self.visuals;
            self.visuals._id_11CC5 = level._id_11CC4[var_1];

            if ( isdefined( self.should_break_stealth_immediately ) )
                [[ self.should_break_stealth_immediately ]]( level._id_11CC4[var_1] );
...
playerentersafearea( var_0, var_1 )
{
    if ( !var_1 istouching( var_0.trigger ) )
    {
        ...
        if ( isdefined( var_1._id_11CC5 ) && var_1._id_11CC5 == var_0 )
            var_1._id_11CC5 = undefined;
...
{
    level.loadout_copyclassstruct["gos_fireworks"] = loadfx( "vfx/iw8_mp/gamemode/vfx_gos_firework.vfx" );
    level._id_11CC2["vanish"] = loadfx( "vfx/iw8_mp/gamemode/vfx_soccer_ball_burst.vfx" );
    level._id_11CC2["score_00"] = loadfx( "vfx/iw8_mp/level/stadium/vfx_stad_score_00.vfx" );
    level._id_11CC2["score_01"] = loadfx( "vfx/iw8_mp/level/stadium/vfx_stad_score_01.vfx" );
    level._id_11CC2["score_02"] = loadfx( "vfx/iw8_mp/level/stadium/vfx_stad_score_02.vfx" );
    level._id_11CC2["score_03"] = loadfx( "vfx/iw8_mp/level/stadium/vfx_stad_score_03.vfx" );
    level._id_11CC2["score_04"] = loadfx( "vfx/iw8_mp/level/stadium/vfx_stad_score_04.vfx" );
    level._id_11CC2["score_05"] = loadfx( "vfx/iw8_mp/level/stadium/vfx_stad_score_05.vfx" );
    level._id_11CC2["score_06"] = loadfx( "vfx/iw8_mp/level/stadium/vfx_stad_score_06.vfx" );
    level._id_11CC2["score_07"] = loadfx( "vfx/iw8_mp/level/stadium/vfx_stad_score_07.vfx" );
    level._id_11CC2["score_08"] = loadfx( "vfx/iw8_mp/level/stadium/vfx_stad_score_08.vfx" );
    level._id_11CC2["score_09"] = loadfx( "vfx/iw8_mp/level/stadium/vfx_stad_score_09.vfx" );
    level._id_11CC2["score_10"] = loadfx( "vfx/iw8_mp/level/stadium/vfx_stad_score_10.vfx" );
}
...
_id_11B2D()
{
    ...
    level.little_bird_mg_mp_waitandspawn = [ "_1", "_2", "_3" ];
}

timed_death( var_0, var_1, var_2, var_3 )
{
