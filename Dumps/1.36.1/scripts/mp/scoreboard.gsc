...
setplayerscoreboardinfo()
{
    ...
    if ( var_0 < 200 )
    {
        ...
        if ( scripts\mp\utility\game::getgametype() == "br" )
        {
            var_12 = scripts\mp\gametypes\br.gsc::computer_interface_think_internal( self );
            ...
            {
                var_15 = scripts\mp\gamescore::lbravo_spawner_jammer3b();
                var_16 = var_15[self.team];
                setclientmatchdata( "players", self.clientmatchdataid, "placement", var_16 );
                var_17 = scripts\mp\gametypes\br_gametype_dmz.gsc::so_displaygameend();
                setclientmatchdata( "players", self.clientmatchdataid, "extrascore4", var_17 );
                var_18 = 0;

                if ( isdefined( self.register_bank_cut_modules ) )
                    var_18 = self.register_bank_cut_modules;

                setclientmatchdata( "players", self.clientmatchdataid, "extrascore5", var_18 );
            }
            else if ( scripts\mp\utility\game::lb_mg_impulse_dmg_threshold_top() == "kingslayer" )
            {
                var_15 = scripts\mp\gamescore::lbravo_spawner_jammer3b();
                var_16 = var_15[self.team];
                setclientmatchdata( "players", self.clientmatchdataid, "placement", var_16 );
                var_17 = scripts\mp\gametypes\br_gametype_kingslayer.gsc::so_displaygameend();
                setclientmatchdata( "players", self.clientmatchdataid, "extrascore4", var_17 );
                var_18 = 0;
            ...
            else if ( scripts\mp\utility\game::lb_mg_impulse_dmg_threshold_top() == "rebirth" )
            {
                if ( isdefined( self._id_121A5 ) )
                    setclientmatchdata( "players", self.clientmatchdataid, "placement", self._id_121A5 );

                var_17 = scripts\mp\gametypes\br_gametype_rebirth.gsc::circleorigin();
                setclientmatchdata( "players", self.clientmatchdataid, "extrascore4", var_17 );
            }
            else if ( isdefined( self._id_121A5 ) )
                setclientmatchdata( "players", self.clientmatchdataid, "placement", self._id_121A5 );
        }
        ...
        foreach ( var_3 in level.teamnamelist )
        {
            if ( scripts\mp\menus::shouldmodesetsquads() )
            {
                foreach ( var_5 in level.squaddata[var_3] )
                {
                    ...
                    {
                        scripts\mp\gamelogic::bomb_vest_timer_total_num_of_frame( var_8 );
