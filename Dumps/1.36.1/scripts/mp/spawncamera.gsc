...
init()
{
    if ( !isdefined( level.get_force_push_direction ) )
        level.get_force_push_direction = getdvarint( "scr_game_disablespawncamera", 0 ) == 1;
...
startspawncamera( var_0, var_1, var_2 )
{
    ...
    scripts\mp\utility\player::trial_headicon( "spawnCamera::startSpawnCamera() START" );
    scripts\mp\utility\player::hideminimap( 1 );
    endptui();

    if ( !isdefined( self.spawncameraent ) )
    {
        scripts\mp\utility\player::trial_headicon( "spawnCamera::startSpawnCamera() UNDEFINED SPAWNCAMERAENT!!!" );
    ...
    scripts\engine\utility::_id_12919( 0.1, "force_spawn" );
    thread removecameraondisconnect( self.spawncameraent );
    self cameralinkto( self.spawncameraent, "tag_origin", 1, 1 );
    scripts\mp\utility\player::trial_headicon( "spawnCamera::startSpawnCamera() CameraLinkTo()" );
...
lb_dmg_factor_driverless_collision()
{
    return 1.0;
}
...
getspawncamera()
{
    ...
    else if ( istrue( level.evaluatespawnforcameraselection ) && isdefined( level.ppkflagcarrier ) && isdefined( level.ppkflagcarrier[self.team] ) )
    {
        var_14 = level.ppkflagcarrier[self.team];
        var_15 = undefined;
        var_16 = undefined;
...
snaptospawncamera()
{
    ...
    while ( !istrue( self.create_vehicle_occupancy_data ) )
        waitframe();
    ...
    scripts\mp\utility\player::trial_headicon( "spawnCamera::snapToSpawnCamera()" );
...
endspawncamera()
{
    if ( isdefined( self.silo_door_left ) )
    {
        self clearsoundsubmix( "iw8_mp_spawn_camera" );
        self.silo_door_left stoploopsound( self.silo_door_right );
        self.silo_door_left delete();
        self.silo_door_left = undefined;
        self.silo_door_right = undefined;
    }
...
{
    self endon( "disconnect" );
    scripts\mp\utility\player::trial_headicon( "spawnCamera::runSlamZoomOnSpawn() START" );
    self waittill( "spawncamera_start" );
    scripts\mp\utility\player::trial_headicon( "spawnCamera::runSlamZoomOnSpawn() spawncamera_start PASSED" );
    ...
    scripts\mp\utility\player::trial_headicon( "spawnCamera::runSlamZoomOnSpawn() CameraLinkTo()" );
    handlemovetoblended();
    deletespawncamera();
    ...
    scripts\mp\utility\player::trial_headicon( "spawnCamera::runSlamZoomOnSpawn() COMPLETE" );
}
...
handlemovetoblended( var_0 )
{
    ...
    scripts\mp\utility\player::trial_headicon( "spawnCamera::handleMoveTo() START" );
    ...
    if ( !var_3 || !var_4 )
    {
        ...
        scripts\mp\utility\player::trial_headicon( "spawnCamera::handleMoveTo() FULL Blend Set" );
    ...
    else
    {
        ...
        scripts\mp\utility\player::trial_headicon( "spawnCamera::handleMoveTo() FIRST HALF Blend Set" );
        ...
        scripts\mp\utility\player::trial_headicon( "spawnCamera::handleMoveTo() SECOND HALF Blend Set" );
        wait 0.3;
        wait 0.2;
    }

    scripts\mp\utility\player::trial_headicon( "spawnCamera::handleMoveTo() COMPLETE" );
...
fadeblackforgeo( var_0 )
{
    ...
    if ( istrue( self.spawningintovehicle ) || scripts\cp_mp\utility\game_utility::isnightmap() )
    {
        ...
        if ( isdefined( self._id_127E6 ) && self._id_127E6 == "light_tank" )
        {
...
{
    self endon( "disconnect" );
    self.create_vehicle_occupancy_data = 0;
    ...
    self.create_vehicle_occupancy_data = 1;
...
startoperatorsound()
{
    ...
    self.silo_door_left = spawn( "script_origin", ( 0, 0, 0 ) );
    self.silo_door_left showonlytoplayer( self );
    ...
    self.silo_door_right = var_1;
    self.silo_door_left playloopsound( var_1 );
    thread grenade_exploded_during_stealth_listener();
}

grenade_exploded_during_stealth_listener()
{
    self endon( "spawned_player" );
    var_0 = self.silo_door_left;
    var_1 = self.silo_door_right;
    self waittill( "disconnect" );
