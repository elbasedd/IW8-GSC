...
timeuntilspawn( var_0 )
{
    ...
    if ( !isdefined( self.open_trap_room_door ) )
        self.open_trap_room_door = var_1;
...
mayspawn()
{
    ...
    if ( isdefined( level.get_freight_lift_spawnpoints ) && [[ level.get_freight_lift_spawnpoints ]]( self ) )
        return 0;
...
spawnclient( var_0 )
{
    ...
    if ( isdefined( level.rpgafterspawnfunc ) )
    {
        self [[ level.rpgafterspawnfunc ]]( var_0 );
        return;
    }
...
waitandspawnclient( var_0 )
{
    ...
    if ( isdefined( level.trial_dogtags ) )
        [[ level.trial_dogtags ]]( var_0 );
    ...
    if ( isdefined( self.setspawnpoint ) && !tivalidationcheck() )
        scripts\mp\equipment\tac_insert::_id_11E39( 1 );

    var_3 = timeuntilspawn( 0 );
    var_4 = 0.0;

    if ( !istrue( level.get_force_push_direction ) )
    ...
    {
        if ( isdefined( self.open_trap_room_door ) )
            var_3 = max( self.open_trap_room_door - self.killcamwatchtime, 0 );
        else
    ...
    if ( !istrue( level.get_force_push_direction ) && var_9 && !istrue( self.skipspawncamera ) && !istrue( level.usespawnselection ) && var_3 > 0 )
    ...
    else if ( var_3 > 0 )
    {
        ...
        var_1 = 1;
        scripts\engine\utility::_id_12919( var_3, "force_spawn" );
    ...
    if ( !istrue( level.get_force_push_direction ) || scripts\mp\utility\game::getgametype() == "arm" )
...
tivalidationcheck()
{
    ...
    if ( isdefined( level.create_weapon_pick_up ) && ![[ level.create_weapon_pick_up ]]( self ) )
        return 0;
    ...
    foreach ( var_2 in var_0 )
    {
        ...
        scripts\mp\equipment\tac_insert::_id_11E39();
        return 0;
    }
...
revivespawnvalidationcheck()
{
    ...
    var_0 = spawnstruct();
    var_0._id_11E41 = self.forcespawnorigin;
    var_0.playergetlaststandpistol = 1;
    var_1 = [];
    var_1[0] = self;
    ...
    {
        var_0.playergetlaststandpistol = 0;
        return var_0;
    }
...
getspawnpoint()
{
    ...
    else if ( isdefined( self.forcespawnorigin ) )
    {
        ...
        if ( !var_4.playergetlaststandpistol && isdefined( self.prevrevivepos ) && !isdefined( self.rallypoint ) )
        ...
        {
            if ( positionwouldtelefrag( var_1 ) || !var_4.playergetlaststandpistol )
            {
    ...
    else if ( isdefined( self.setspawnpoint ) )
    {
        ...
        {
            if ( istrue( level.play_vo_or_timeout ) && level.mapname != "mp_aniyah" )
            {
                self.search_turret_no_target_stop_delay = undefined;
                var_5 = scripts\engine\utility::ter_op( istrue( level.avoids_recently_cleared_area ), 1, istrue( self.setspawnpoint.issuper ) );

                if ( var_5 )
                {
                    var_6 = min( level.nextdest - self.setspawnpoint.playerspawnpos[2], level.nextdest );

                    if ( var_6 < level.nextdest - 950 )
                        var_6 = var_6 + 950;

                    self.setspawnpoint.playerspawnpos = self.setspawnpoint.playerspawnpos + ( 0, 0, var_6 );
                    self.player_fov_default_1 = 1;
                }
            }
...
spawnplayer( var_0, var_1 )
{
    ...
    if ( scripts\mp\utility\game::teamhasinfil( self.team ) && !scripts\mp\flags::gameflag( "infil_started" ) && !isdefined( level.bypassclasschoicefunc ) )
    {
        if ( getdvarint( "scr_force_cac_sre_callstack", 0 ) == 1 && scripts\mp\utility\game::getgametype() == "br" && scripts\mp\utility\game::lb_mg_impulse_dmg_threshold_top() != "dmz" && scripts\mp\utility\game::lb_mg_impulse_dmg_threshold_top() != "risk" && scripts\mp\utility\game::lb_mg_impulse_dmg_threshold_top() != "sandbox" )
        {
            ...
            scripts\mp\utility\script::fusefx( "spawnPlayer()" + self.name + " ui_options_menu = 2, hasInfil = " + var_2 + " infil_started = " + var_3 + "bypassClassChoiceFunc = " + var_4 );
        }
    ...
    else
    {
        ...
        scripts\mp\class::raid_umike_unload_func( self.pers["copiedClass"], var_7 );

        if ( isdefined( self.pers["lastKiller"] ) )
        {
            recondrone_watchsuperinternal( self.pers["lastKiller"] );
    ...
    if ( !var_0 )
    {
        ...
        if ( istrue( self.init_intro_heli_anims ) )
        {
            self.init_intro_heli_anims = undefined;
    ...
    if ( !var_0 )
    {
        ...
        if ( !istrue( self.gulag ) && !istrue( self.respawningfromtoken ) && !istrue( self.visible2state ) )
            addtoalivecount();
    ...
    thread scripts\cp\agents\gametype_cp_specops::onspawn();
    ...
    scripts\mp\utility\player::trial_headicon( "playerlogic::spawnPlayer() !!!CODE SPAWN!!! @" + self.spawndata.spawnorigin );

    if ( !isdefined( level.ppkflagcarrier ) )
        level.ppkflagcarrier = [];

    level.ppkflagcarrier[self.team] = self.spawndata.spawnorigin;
    ...
    scripts\mp\utility\game::create_silencer_pick_up( self );
    [[ level.onspawnplayer ]]();
    ...
    if ( !var_0 )
    {
        self.matchdatalifeindex = scripts\common\utility::_id_12456( level.remove_player_from_focus_fire_attacker_list );
    ...
    if ( isdefined( self.selectedspawnarea ) )
        scripts\mp\analyticslog::recharge_equipment_init( self, self.selectedspawnarea );
    ...
    if ( !istrue( level.get_force_push_direction ) && isdefined( self.spawncameraent ) || istrue( level.get_force_push_direction ) && isdefined( self.spawncameraent ) && scripts\mp\utility\game::getgametype() == "arm" )
    {
...
logstartingloadout()
{
    ...
    if ( isdefined( self.curclass ) && self.curclass != "juggernaut" )
        scripts\mp\class::loadout_lognewlygivenloadout( self.little_bird_mg_initdamage, self.classstruct, self.curclass );
}
...
spawnspectatormapcam( var_0 )
{
    ...
    scripts\mp\utility\player::trial_headicon( "playerlogic::spawnSpectatorMapCam() !!!CODE SPAWN!!! @" + var_6.origin );
    ...
    if ( isdefined( var_0 ) && var_0 == 99 )
        ...
        var_15 = scripts\engine\utility::_id_12914( 2, "versus_menu_done", "prematch_over" );
...
onspawnspectator( var_0, var_1, var_2 )
{
    if ( isdefined( var_0 ) && isdefined( var_1 ) )
    {
        ...
        scripts\mp\utility\player::trial_headicon( "playerlogic::onSpawnSpectator() !!!CODE SPAWN!!! @" + var_0 );
        return;
    }
    ...
    if ( scripts\mp\utility\game::play_vo_internal() && !self.hasspawned )
        self predictstreampos( var_3.origin );

    self spawn( var_3.origin, var_3.angles );
    scripts\mp\utility\player::trial_headicon( "playerlogic::onSpawnSpectator() !!!CODE SPAWN!!! @" + var_3.origin );
}
...
getspectatepoint()
{
    ...
    if ( scripts\mp\utility\game::getgametype() == "brtdm" )
        return level.cleanup_lights._id_11E75;
...
spawnintermission( var_0, var_1, var_2 )
{
    ...
    if ( !isdefined( level.custom_ending ) )
    {
        ...
        scripts\mp\utility\player::trial_headicon( "playerlogic::spawnIntermission() !!!CODE SPAWN!!! @" + var_0.origin );
        scripts\mp\utility\player::setdof_spectator();
    }
...
spawnendofgame()
{
    if ( isdefined( level.rpggetclosetoapc ) && self [[ level.rpggetclosetoapc ]]() )
        return;
...
callback_playerdisconnect( var_0 )
{
    ...
    self.get_gunshotteammate_alias = 1;
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" )
    {
        ...
        if ( scripts\mp\utility\game::lb_mg_impulse_dmg_threshold_top() == "br" )
        {
            var_3 = [];
            ...
            var_7 = var_3.size + 1;
            scripts\cp_mp\utility\game_utility::_id_11A96( var_7 );
        }
    }
    ...
    scripts\cp_mp\utility\player_utility::vehicle_occupancy_linktooriginandangles();
    ...
    if ( analyticsstreamerislogfilestreamingenabled() )
        analyticsstreamerlogfilewritetobuffer( "Q;" + var_13 + ";" + var_12 + ";" + self.name + "\n" );
    ...
    else if ( self.sessionstate != "playing" && scripts\mp\utility\game::getgametype() == "br" && ( scripts\mp\gametypes\br_public.gsc::iswaitingtoentergulag( self ) || scripts\mp\gametypes\br_public.gsc::player_ammo_think( self ) || scripts\mp\gametypes\br_public.gsc::player_set_weapon( self ) ) )
        removefromalivecount( 1 );
    ...
    if ( isdefined( self.team ) )
        scripts\mp\utility\teams::_id_12684( "disconnect", self.team, self );
    ...
    scripts\common\utility::_id_12456( level.remove_outline, var_0 );
    ...
    if ( level.players.size == 0 )
    {
        if ( scripts\mp\utility\game::getgametype() == "br" && getdvarint( "scr_data_force_send_matchdata_for_no_players_left", 1 ) == 1 && istrue( level.br_prematchstarted ) )
            scripts\mp\gamelogic::_id_11979();
...
recordplayersegmentdata()
{
    ...
    for (;;)
    {
        ...
        {
            if ( istrue( var_1.create_trial_weapon_spawn ) )
...
createplayersegmentstats( var_0 )
{
    ...
    var_0.create_trial_weapon_spawn = 1;
}
...
callback_playerconnect()
{
    ...
    if ( isdefined( level.shouldblockdamage ) )
        var_0 = self thread [[ level.shouldblockdamage ]]();
    ...
    if ( level.uniqueplayersconnected <= ks_circledelaytime() )
    {
    ...
    self.markplayeraseliminated = undefined;
    self.markingendtime = undefined;
    self.lifeid = 0;
    ...
    scripts\cp_mp\utility\player_utility::alternate_breach_anim_func();
    ...
    thread _computerrebootsequence_start::br_onvehicledeath( self );
    ...
    if ( !isdefined( self.pers["team"] ) )
    {
        ...
        else if ( ( scripts\mp\utility\game::matchmakinggame() || scripts\mp\utility\game::lobbyteamselectenabled() || isgamebattlematch() ) && self.sessionteam != "none" )
        {
            ...
            if ( ( getdvar( "scr_br_gametype", "" ) == "dmz" || getdvar( "scr_br_gametype", "" ) == "risk" ) && getdvarint( "scr_br_manual_team_assignment", 0 ) == 1 )
                thread scripts\mp\teams::spawn_overwatch_final_tank();
            else
            ...
            if ( scripts\mp\utility\game::allowclasschoice() || scripts\mp\utility\game::showfakeloadout() && !isai( self ) )
            {
                if ( getdvarint( "scr_force_cac_sre_callstack", 0 ) == 1 && scripts\mp\utility\game::getgametype() == "br" && scripts\mp\utility\game::lb_mg_impulse_dmg_threshold_top() != "dmz" && scripts\mp\utility\game::lb_mg_impulse_dmg_threshold_top() != "risk" && scripts\mp\utility\game::lb_mg_impulse_dmg_threshold_top() != "sandbox" )
                {
                    ...
                    scripts\mp\utility\script::fusefx( "Callback_PlayerConnect() elseif MMG " + self.name + " ui_options_menu = 2, allowClassChoiceFunc = " + var_14 );
                }
        ...
        else
        {
            ...
            if ( scripts\mp\utility\game::allowclasschoice() || scripts\mp\utility\game::showfakeloadout() && !isai( self ) )
            {
                ...
                {
                    var_14 = isdefined( level.allowclasschoicefunc ) && istrue( self [[ level.allowclasschoicefunc ]]() );
                    scripts\mp\utility\script::fusefx( "Callback_PlayerConnect() else " + self.name + " ui_options_menu = 2, allowClassChoiceFunc = " + var_14 );
                }
...
kickifdontspawn()
{
    ...
    if ( istrue( self._id_11E06 ) && !mayspawn() )
        return;
...
totaldisttracking( var_0 )
{
    ...
    for (;;)
    {
        var_4 = scripts\engine\utility::_id_12917( 5.0, "death", "vehicle_enter", "vehicle_change_seat", "vehicle_exit", "afk_tracking_resume" );

        if ( var_4 == "vehicle_exit" && !isdefined( self.lastvehicleseatchangetime ) )
            continue;

        if ( istrue( self.mp_raid_patch ) )
            continue;
        ...
        {
            var_8 = scripts\mp\utility\game::lb_mg_impulse_dmg_threshold_top() != "br";
...
initleanplayerstats()
{
    ...
    scripts\mp\utility\stats::opennukecrate( "deaths" );
    ...
    scripts\mp\utility\stats::opennukecrate( "score" );
    ...
    scripts\mp\utility\stats::opennukecrate( "kills" );
}
...
initplayerstats()
{
    ...
    if ( !isdefined( self.propaddtocircle ) )
        self.propaddtocircle = [];
    ...
    scripts\mp\utility\stats::opennukecrate( "suicides" );

    if ( !isdefined( self.pers["headshots"] ) )
        scripts\mp\utility\stats::initpersstat( "headshots" );

    scripts\mp\utility\stats::opennukecrate( "headshots" );
    ...
    scripts\mp\utility\stats::opennukecrate( "assists" );
    ...
    scripts\mp\utility\stats::opennukecrate( "captures" );
    ...
    scripts\mp\utility\stats::opennukecrate( "returns" );
    ...
    scripts\mp\utility\stats::opennukecrate( "defends" );
    ...
    scripts\mp\utility\stats::opennukecrate( "plants" );
    ...
    scripts\mp\utility\stats::opennukecrate( "defuses" );
    ...
    scripts\mp\utility\stats::opennukecrate( "destructions" );
    ...
    scripts\mp\utility\stats::opennukecrate( "confirmed" );
    ...
    scripts\mp\utility\stats::opennukecrate( "denied" );
    ...
    scripts\mp\utility\stats::opennukecrate( "rescues" );
    ...
    scripts\mp\utility\stats::opennukecrate( "touchdowns" );
    ...
    scripts\mp\utility\stats::opennukecrate( "fieldgoals" );
    ...
    scripts\mp\utility\stats::opennukecrate( "killChains" );
    ...
    scripts\mp\utility\stats::opennukecrate( "killsAsSurvivor" );
    ...
    scripts\mp\utility\stats::opennukecrate( "stabs" );
    ...
    scripts\mp\utility\stats::opennukecrate( "setbacks" );
    ...
    scripts\mp\utility\stats::opennukecrate( "objTime" );
    ...
    scripts\mp\utility\stats::opennukecrate( "gamemodeScore" );
    ...
    scripts\mp\utility\stats::opennukecrate( "damage" );

    if ( !isdefined( self.pers["supersEarned"] ) )
        scripts\mp\utility\stats::initpersstat( "supersEarned" );

    scripts\mp\utility\stats::opennukecrate( "supersEarned" );
    ...
    scripts\mp\utility\stats::opennukecrate( "downs" );
    ...
    scripts\mp\utility\stats::opennukecrate( "killStreak" );
    scripts\mp\utility\stats::opennukecrate( "loss" );
    scripts\mp\utility\stats::opennukecrate( "win" );
}
...
removefromalivecount( var_0 )
{
    ...
    var_2 = scripts\mp\utility\game::getgametypenumlives() != 0 && var_1 == 0 && !istrue( level._id_11C75 );
...
updateplayerwindmaterial()
{
    ...
    for (;;)
    {
        foreach ( var_6 in level.players )
        {
            ...
            if ( !isdefined( var_6._id_12AC0 ) || var_6._id_12AC0 != var_8 )
            {
                switch ( var_8 )
                {
                    case 0:
...
{
    return istrue( self.beingrevived ) || istrue( self.player_unresolved_collision_suspend );
}
