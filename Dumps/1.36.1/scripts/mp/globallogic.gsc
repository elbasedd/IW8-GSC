...
init()
{
    ...
    level.relic_squadlink_turn_team_headobjectives = getdvarint( "scr_map_use10v10_objectives", 0 );
    level.teambased = 0;
...
registerdvars()
{
    ...
    validate_demeanor();
    vandalize_attack_nodes();
}

validate_demeanor()
{
    if ( ( scripts\mp\utility\game::getgametype() == "arm" || scripts\mp\utility\game::play_vo_internal() || scripts\mp\utility\game::playergetspectatingplayer() ) && getdvarint( "scr_game_forceDefaultFallHeight", 0 ) != 1 )
    {
...
setupcallbacks()
{
    ...
    scripts\cp\utility\auto_ascender_ai::register_ai_damage_callbacks();
}

setdefaultcallbacks()
{
    level.callbackstartgametype = scripts\mp\gamelogic::callback_startgametype;
    level.controls_unlink_safe = scripts\mp\playerlogic::control_station;
