...
getclasschoice( var_0 )
{
    ...
    if ( isdefined( level.longgulagstream ) && level.longgulagstream > 0 )
    {
        if ( scripts\mp\utility\game::matchmakinggame() )
...
executeclasschange( var_0, var_1 )
{
    ...
    else
    {
        ...
        var_3 = 1;
        var_4 = scripts\mp\utility\game::lb_mg_impulse_dmg_threshold_top() == "dmz" || scripts\mp\utility\game::lb_mg_impulse_dmg_threshold_top() == "risk" || scripts\mp\utility\game::lb_mg_impulse_dmg_threshold_top() == "kingslayer";
...
handleclassedit( var_0 )
{
    ...
    var_3 = scripts\mp\class::raid_seq3_objectives_func();
    var_4 = var_2 || var_3;
...
autoassign()
{
    ...
    else
    {
        ...
        if ( var_5.size > 0 )
        {
            ...
            if ( istrue( level.bot_get_flag_carrier ) )
                var_9 = 0;
...
setteam( var_0 )
{
    ...
    if ( scripts\mp\utility\game::allowclasschoice() || scripts\mp\utility\game::showfakeloadout() && !isai( self ) )
    {
        if ( getdvarint( "scr_force_cac_sre_callstack", 0 ) == 1 && scripts\mp\utility\game::getgametype() == "br" && scripts\mp\utility\game::lb_mg_impulse_dmg_threshold_top() != "dmz" && scripts\mp\utility\game::lb_mg_impulse_dmg_threshold_top() != "risk" && scripts\mp\utility\game::lb_mg_impulse_dmg_threshold_top() != "sandbox" )
        {
            ...
            scripts\mp\utility\script::fusefx( "setTeam() " + self.name + " ui_options_menu = 2, allowClassChoiceFunc = " + var_1 );
        }
    ...
    if ( isdefined( level.shouldblockdamage ) )
        self thread [[ level.shouldblockdamage ]]();

    waitforclassselect();
    endrespawnnotify();

    if ( ( self.sessionstate == "spectator" || switcharray() ) && !istrue( self.suicideswitched ) )
    {
...
switcharray()
{
    return isdefined( level.switcharray ) && self [[ level.switcharray ]]();
}
...
leavesquad( var_0, var_1 )
{
    ...
    if ( isdefined( var_0 ) && isdefined( var_1 ) )
        thread scripts\mp\spawnselection::usbserver( var_0, var_1 );
...
waitforclassselect()
{
    ...
    for (;;)
    {
        var_0 = scripts\mp\utility\game::getgametype() == "br" && scripts\mp\utility\game::allowclasschoice() && ( !scripts\mp\flags::gameflag( "prematch_done" ) || istrue( level._id_11C76 ) );
        ...
        if ( var_0 || var_1 )
        {
            ...
            if ( !isai( self ) )
            {
                ...
                if ( var_3 && !scripts\mp\utility\game::play_hack_reminder_goto1() )
...
beginclasschoice( var_0 )
{
    ...
    if ( scripts\mp\utility\game::allowclasschoice() || scripts\mp\utility\game::showfakeloadout() && !isai( self ) )
    {
        if ( getdvarint( "scr_force_cac_sre_callstack", 0 ) == 1 && scripts\mp\utility\game::getgametype() == "br" && scripts\mp\utility\game::lb_mg_impulse_dmg_threshold_top() != "dmz" && scripts\mp\utility\game::lb_mg_impulse_dmg_threshold_top() != "risk" && scripts\mp\utility\game::lb_mg_impulse_dmg_threshold_top() != "sandbox" )
        {
            ...
            scripts\mp\utility\script::fusefx( "beginClassChoice() " + self.name + " ui_options_menu = 2, allowClassChoiceFunc = " + var_2 );
        }
...
bypassclasschoice()
{
    ...
    if ( player_maxhealth() && level.enforceantiboosting && !isbot( self ) )
    {
...
addtoteam( var_0, var_1, var_2 )
{
    ...
    if ( var_0 == "spectator" || var_0 == "follower" )
    {
        ...
        scripts\cp_mp\pet_watch::should_drop_scavenger_bag();
    }
    else
