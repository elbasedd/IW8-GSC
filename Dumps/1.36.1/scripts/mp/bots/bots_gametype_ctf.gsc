...
setup_callbacks()
{
    ...
    level.bot_funcs["get_watch_node_chance"] = ::cache1_defender_after_spawn;
}

setup_bot_ctf()
{
    level.call_exfil = ::enemy_sentry_debug;
    level.call_for_reinforcements = ::enemy_signal_flare;
    level.calculateobjectivesheld = ::get_allied_attackers_for_team;
    level.calculatepurchasexp = ::get_allied_defenders_for_team;
    level.calculateweaponmatchbonus = ::c4vehiclecooperator;
    level.callback_frontendplayeractive = ::cacheentity;
...
crate_can_use( var_0 )
{
    ...
    if ( caclulate_track_distance() )
        return 0;

    return cac_getaccessorylogic( self.team );
}
...
bot_ctf_think()
{
    ...
    for (;;)
    {
        ...
        if ( !isdefined( self.role ) )
            scripts\mp\bots\bots_gametype_common.gsc::callback_unsubscribe();

        var_2 = scripts\mp\utility\game::getotherteam( self.team )[0];

        if ( cac_getaccessorylogic( self.team ) )
        {
            if ( isdefined( level.calloutmarkerpingvo_canplaywithspamavoidance ) && isdefined( level.calloutmarkerpingvo_canplaywithspamavoidance[var_2] ) )
                level.calloutmarkerpingvo_canplaywithspamavoidance[var_2] = undefined;
        }
        ...
        if ( caclulate_track_distance() )
        {
            ...
            else if ( scripts\mp\bots\bots_gametype_common.gsc::callback( self.team ) == 1 )
...
calchelicoptertrailpoint()
{
    ...
    else if ( self botcanseeentity( var_2 ) )
    {
        ...
        if ( self botgetdifficultysetting( "strategyLevel" ) > 0 && !cac_getaccessorylogic( self.team ) )
        {
            if ( !isdefined( level.calloutmarkerpingvo_canplaywithspamavoidance ) )
                level.calloutmarkerpingvo_canplaywithspamavoidance = [];

            if ( !isdefined( level.calloutmarkerpingvo_canplaywithspamavoidance[var_2.team] ) )
                level.calloutmarkerpingvo_canplaywithspamavoidance[var_2.team] = [];

            level.calloutmarkerpingvo_canplaywithspamavoidance[var_2.team]["origin"] = var_0;
            level.calloutmarkerpingvo_canplaywithspamavoidance[var_2.team]["time"] = gettime();
        }
    }
    ...
    else
    {
        ...
        {
            if ( isdefined( level.calloutmarkerpingvo_canplaywithspamavoidance ) && isdefined( level.calloutmarkerpingvo_canplaywithspamavoidance[var_2.team] ) )
            {
                var_4 = gettime() - level.calloutmarkerpingvo_canplaywithspamavoidance[var_2.team]["time"];

                if ( var_4 < 10000 )
                    var_3 = level.calloutmarkerpingvo_canplaywithspamavoidance[var_2.team]["origin"];
            }
        }
...
enemy_sentry_debug( var_0 )
{
    var_1 = scripts\mp\bots\bots_gametype_common.gsc::callback( var_0 );

    if ( var_1 == 1 )
        return 1;

    if ( cac_getaccessorylogic( var_0 ) )
        var_2 = var_1 * 0.67;
    else
    {
        var_3 = scripts\mp\utility\game::getotherteam( var_0 )[0];

        if ( cac_getaccessorylogic( var_3 ) )
            var_2 = var_1 * 0.5;
        else
            var_2 = var_1 * 0.4;
    }
...
enemy_signal_flare( var_0 )
{
    var_1 = scripts\mp\bots\bots_gametype_common.gsc::callback( var_0 );
    return var_1 - enemy_sentry_debug( var_0 );
}

get_allied_attackers_for_team( var_0 )
{
    return scripts\mp\bots\bots_gametype_common.gsc::call_func_on_death( var_0, level.capzones[var_0].curorigin, get_flag_protect_radius() );
}

get_allied_defenders_for_team( var_0 )
{
    return scripts\mp\bots\bots_gametype_common.gsc::call_in_carepackage( var_0, level.capzones[var_0].curorigin, get_flag_protect_radius() );
}
...
c4_placing_bc()
{
    ...
    level endon( "game_ended" );
    thread scripts\mp\bots\bots_gametype_common.gsc::calculatewinningteam();
}
...
cachedomnars()
{
    return cac_getweaponattachmentid( self.team );
}

c4vehiclemultkill()
{
    var_0 = scripts\mp\utility\game::getotherteam( self.team )[0];
    return cac_getweaponattachmentid( var_0 );
}

caclulate_track_distance()
{
