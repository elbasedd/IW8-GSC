...
call_func_on_death( var_0, var_1, var_2 )
{
    var_3 = callback_create( "attacker", var_0 );

    foreach ( var_5 in level.players )
    {
        if ( !isai( var_5 ) && isdefined( var_5.team ) && var_5.team == var_0 )
        {
            if ( var_5 callback_playeractive() || distancesquared( var_1, var_5.origin ) > squared( var_2 ) )
...
call_in_carepackage( var_0, var_1, var_2 )
{
    var_3 = callback_create( "defender", var_0 );

    foreach ( var_5 in level.players )
    {
        if ( !isai( var_5 ) && isdefined( var_5.team ) && var_5.team == var_0 )
        {
            if ( var_5 callback_subscribe() || distancesquared( var_1, var_5.origin ) <= squared( var_2 ) )
                var_3 = scripts\engine\utility::array_add( var_3, var_5 );
        }
    }
...
{
    if ( isdefined( level.callback_frontendplayeractive ) )
        return self [[ level.callback_frontendplayeractive ]]();
...
callback_subscribe()
{
    if ( isdefined( level.callback_trigger ) )
        return self [[ level.callback_trigger ]]();
...
{
    var_0 = [[ level.calculateobjectivesheld ]]( self.team );
    var_1 = [[ level.calculatepurchasexp ]]( self.team );
    var_2 = [[ level.call_exfil ]]( self.team );
    var_3 = [[ level.call_for_reinforcements ]]( self.team );
    ...
    if ( var_4 == "active" )
    {
        if ( var_0.size >= var_2 )
        {
            ...
            {
                if ( isai( var_7 ) && level.bot_personality_type[var_7.personality] == "stationary" && var_7 c4_charge_detonate_think() )
                {
            ...
            if ( var_5 )
            {
                calldropbag( "attacker" );
                return;
            }

            calldropbag( "defender" );
            return;
        }
        else
            calldropbag( "attacker" );
    ...
    else if ( var_4 == "stationary" )
    {
        if ( var_1.size >= var_3 )
        {
            ...
            {
                if ( isai( var_10 ) && level.bot_personality_type[var_10.personality] == "active" && var_10 c130successfulairdrops() )
                {
...
calculatewinningteam()
{
    ...
    for (;;)
    {
        if ( gettime() > var_1 )
        {
            ...
            {
                var_4 = [[ level.calculateobjectivesheld ]]( var_3 );
                var_5 = [[ level.calculatepurchasexp ]]( var_3 );
                var_6 = [[ level.call_exfil ]]( var_3 );
                var_7 = [[ level.call_for_reinforcements ]]( var_3 );
                ...
                if ( var_4.size > var_6 )
                {
                    ...
                    {
                        if ( isai( var_11 ) && var_11 c4_charge_detonate_think() )
                        {
                            if ( level.bot_personality_type[var_11.personality] == "stationary" )
                            {
                                var_11 calldropbag( "defender" );
                                var_9 = 1;
                                break;
                            }
                            else
                    ...
                    if ( !var_9 && var_8.size > 0 )
                        scripts\engine\utility::random( var_8 ) calldropbag( "defender" );
                }
                ...
                if ( var_5.size > var_7 )
                {
                    ...
                    {
                        if ( isai( var_16 ) && var_16 c130successfulairdrops() )
                        {
                            if ( level.bot_personality_type[var_16.personality] == "active" )
                            {
                                var_16 calldropbag( "attacker" );
                                var_14 = 1;
                                break;
                            }
                            else
                    ...
                    if ( !var_14 && var_13.size > 0 )
                        scripts\engine\utility::random( var_13 ) calldropbag( "attacker" );
                }
            }
        }
...
c130successfulairdrops()
{
    if ( isdefined( level.calculatespawndisttodefenderflagstart ) )
        return self [[ level.calculatespawndisttodefenderflagstart ]]();

    return 1;
}

c4_charge_detonate_think()
{
    if ( isdefined( level.calculateweaponmatchbonus ) )
        return self [[ level.calculateweaponmatchbonus ]]();
...
c130deliverydirection( var_0 )
{
    ...
    foreach ( var_5 in var_0 )
    {
        ...
        var_5._id_12BE2 = compound_objective( var_5 );
        var_5.customturret = _id_12BE6( var_5, 0, 0 );
        var_7 = [ ( 0, 0, 0 ), ( 1, 1, 0 ), ( 1, -1, 0 ), ( -1, 1, 0 ), ( -1, -1, 0 ) ];

        foreach ( var_9 in var_7 )
        {
            var_10 = _id_12BE6( var_5, var_9[0], var_9[1] );
...
{
    var_1 = spawnstruct();
    var_1.revive_friendly = ( 999999, 999999, 999999 );
    var_1.removegasmaskbr = ( -999999, -999999, -999999 );

    foreach ( var_3 in var_0.nodes )
    {
        var_1.revive_friendly = ( min( var_3.origin[0], var_1.revive_friendly[0] ), min( var_3.origin[1], var_1.revive_friendly[1] ), min( var_3.origin[2], var_1.revive_friendly[2] ) );
        var_1.removegasmaskbr = ( max( var_3.origin[0], var_1.removegasmaskbr[0] ), max( var_3.origin[1], var_1.removegasmaskbr[1] ), max( var_3.origin[2], var_1.removegasmaskbr[2] ) );
    }

    var_1.center = ( ( var_1.revive_friendly[0] + var_1.removegasmaskbr[0] ) / 2, ( var_1.revive_friendly[1] + var_1.removegasmaskbr[1] ) / 2, ( var_1.revive_friendly[2] + var_1.removegasmaskbr[2] ) / 2 );
    var_1.lootchopper_postmodifydamage = ( var_1.removegasmaskbr[0] - var_1.center[0], var_1.removegasmaskbr[1] - var_1.center[1], var_1.removegasmaskbr[2] - var_1.center[2] );
    var_1.radius = max( var_1.lootchopper_postmodifydamage[0], var_1.lootchopper_postmodifydamage[1] );
    return var_1;
}

_id_12BE6( var_0, var_1, var_2 )
{
    var_3 = ( var_0._id_12BE2.center[0] + var_1 * var_0._id_12BE2.lootchopper_postmodifydamage[0], var_0._id_12BE2.center[1] + var_2 * var_0._id_12BE2.lootchopper_postmodifydamage[1], 0 );
    var_4 = undefined;
    var_5 = 9999999;
...
burningstate( var_0, var_1, var_2, var_3 )
{
    var_4 = 0;
    ...
    {
        childthread set_relic_punchbullets();
        childthread set_relic_oneclip();
    }
