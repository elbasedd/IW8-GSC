...
setup_callbacks()
{
    level.bot_funcs["gametype_think"] = ::calloutmarkerpingvohandlerpool;
    level.bot_funcs["crate_can_use"] = ::crate_can_use;
    level.bot_funcs["know_enemies_on_start"] = undefined;
    level.bot_funcs["jugg_picked_up_cancel"] = ::c130airdrop_createpathstruct;
}

_id_11AE6()
{
    camera_character_gamebattles();
    scripts\mp\bots\bots_util::bot_waittill_bots_enabled();
    var_0 = camera_loadout_showcase_preview_sticker( [ "_allies", "_axis" ] );
...
calloutmarkerpingvohandlerpool()
{
    ...
    for (;;)
    {
        ...
        if ( self == level.wam_timer_between_each_node )
        {
            ...
            var_4 = is_enemy_dangerous( var_3.trigger );
            self botsetscriptgoal( var_4.origin, 0, "critical", var_4.angles[1] );
            var_5 = scripts\mp\bots\bots_util::bot_waittill_goal_or_fail();
            continue;
        }

        if ( level.wam_timer_between_each_node.team == self.team )
        {
            if ( !scripts\mp\bots\bots_util::bot_is_defending() )
                scripts\mp\bots\bots_strategy::bot_guard_player( level.wam_timer_between_each_node, 400 );

            continue;
        }

        if ( gettime() > self.next_time_hunt_carrier || sighttracepassed( self.origin + ( 0, 0, 77 ), level.wam_timer_between_each_node.origin + ( 0, 0, 77 ), 0, self ) )
        {
            self botsetscriptgoal( level.wam_timer_between_each_node.origin, 16, "hunt" );
...
wam_interaction_init( var_0 )
{
    var_1 = playerprestreamrespawnorigin( scripts\mp\gametypes\rugby.gsc::killstreak_cooldown_start() + 2, "jugg_captured", randomint( 100 ) > 50 );
...
{
    wait 1.0;
    cam( level.rugby.endzones["allies"][0].trigger );
    cam( level.rugby.endzones["axis"][0].trigger );
