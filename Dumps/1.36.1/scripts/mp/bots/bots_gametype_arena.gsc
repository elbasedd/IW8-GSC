...
setup_callbacks()
{
    ...
    {
        level.bot_funcs["dropped_weapon_think"] = ::binoculars_processlosqueuehigh;
        level.bot_funcs["dropped_weapon_cancel"] = ::binoculars_processtargetlos;
    }

    level.bot_funcs["gametype_think"] = ::c130airdrop_isnearotherdrop;
}
...
c130airdrop_isnearotherdrop()
{
    ...
    if ( !scripts\mp\gametypes\arena.gsc::isnormalloadouts() )
    {
        if ( self botgetdifficultysetting( "advancedPersonality" ) && self botgetdifficultysetting( "strategyLevel" ) > 0 )
            scripts\mp\bots\bots_gametype_gun.gsc::calloutmarkerping_requestinventoryslotindex( self getcurrentweapon().basename, self.pers["bot_original_personality"] );
    }

    self.loadout_finalizeweapons = 0;
    ...
    for (;;)
    {
        if ( isdefined( level.arenaflag ) && level.arenaflag.visibleteam == "any" )
        {
            ...
            if ( !scripts\mp\bots\bots_util::bot_is_capturing() || !self.loadout_finalizeweapons )
            ...
            self.loadout_finalizeweapons = 1;
        }
        else
        {
...
binoculars_processlosqueuehigh()
{
    ...
    for (;;)
    {
        ...
        if ( binoculars_onstateupdatefunc() || scripts\mp\bots\bots_util::calloutmarkerping_gettimeoutforpoolid( 0.33 ) )
        {
            if ( self [[ level.bot_funcs["should_pickup_weapons"] ]]() && !scripts\mp\bots\bots_util::bot_is_remote_or_linked() )
            {
                ...
                {
                    var_4 = var_3[0];
                    level.getpropnextstartlocation = var_4;
                    binoculars_processlosqueue( var_4 );
                }
            }
        }
            ...
            if ( var_6 )
            {
                ...
                foreach ( var_10 in var_8 )
                {
                    ...
                    if ( isdefined( var_10.closest_point_on_grid ) )
                    {
                        ...
                        binoculars_processlosqueue( var_10 );
                    }
                }
            }
            else if ( var_5 )
...
binoculars_processlosqueue( var_0 )
{
    if ( scripts\mp\bots\bots_strategy::bot_has_tactical_goal( "seek_dropped_weapon", var_0 ) == 0 )
    {
        ...
        if ( isdefined( var_0.calloutmarkerpingvo_playpredictivepingcleared ) )
            var_13 = var_0.calloutmarkerpingvo_playpredictivepingcleared;

        scripts\mp\bots\bots_strategy::bot_new_tactical_goal( "seek_dropped_weapon", var_13, 100, var_11 );
    }
}

binoculars_ontake( var_0 )
{
...
{
    if ( binoculars_onstateunmarkedupdate() > 0 )
    {
