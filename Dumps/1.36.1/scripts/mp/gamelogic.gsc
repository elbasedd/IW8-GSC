...
roundend_checkscorelimit( var_0, var_1 )
{
    ...
    else if ( level.roundscorelimit > 0 )
    {
        ...
        {
            if ( level scripts\mp\utility\game::playerhumanconcusspushcleanup( var_8 ) )
                continue;
...
forceend( var_0 )
{
    ...
    if ( level.teambased )
    {
        ...
        {
            if ( level scripts\mp\utility\game::playerhumanconcusspushcleanup( var_2 ) )
                continue;
...
onscorelimit( var_0 )
{
    ...
    if ( level.teambased )
    {
        var_2 = scripts\mp\gamescore::connect_circlar_path();
    ...
    else
    {
        ...
        {
            level.rpg_shoot_at_trig_watch = var_2;
...
updategameevents()
{
    ...
    if ( level.teambased )
    {
        ...
        if ( !var_17 && !var_20 )
        {
            ...
            if ( _id_11BCF() && game["roundsPlayed"] == 0 )
        ...
        foreach ( var_7 in level.teamnamelist )
        {
            ...
            if ( var_27 == 1 || var_27 == 2 )
            {
                ...
                if ( var_28 == 0 )
                {
                    ...
                    else if ( getdvar( "scr_br_gametype", "" ) != "dmz" && getdvar( "scr_br_gametype", "" ) != "risk" && getdvar( "scr_br_gametype", "" ) != "kingslayer" && var_27 == 2 && var_30.size > 2 )
                    {
...
waitforplayers( var_0 )
{
    ...
    for (;;)
    {
        ...
        var_7 = lb_mg_impulse_dmg_threshold_mid();
...
prematchperiod()
{
    ...
    if ( isdefined( level.traps_disabled ) && [[ level.traps_disabled ]]() )
        return;
    ...
    if ( getdvarint( "scr_live_lobby", 0 ) == 1 && !istrue( level._id_11C76 ) )
    {
        ...
        if ( scripts\mp\utility\game::getgametype() == "br" )
        {
            ...
            level thread scripts\mp\gametypes\br_public.gsc::capsule_contents();
    ...
    else if ( !istrue( level._id_11C76 ) )
    {
        if ( scripts\mp\utility\game::getgametype() == "br" )
        {
            ...
            if ( var_2 != 0 )
            {
                level thread _id_129EB();
                level thread cleanuplinkent();
                level waittill( "start_prematch" );
            }
...
givematchbonus( var_0, var_1 )
{
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" && scripts\mp\utility\game::lb_mg_impulse_dmg_threshold_top() != "dmz" && scripts\mp\utility\game::lb_mg_impulse_dmg_threshold_top() != "risk" && scripts\mp\utility\game::lb_mg_impulse_dmg_threshold_top() != "kingslayer" )
        return;
...
confirmed_pilot( var_0 )
{
    if ( istrue( self.pers["ignoreWeaponMatchBonus"] ) || !isdefined( self.pers["killsPerWeapon"] ) )
        return;

    var_1 = scripts\mp\weaponrank::keypad_playerinteractwithkeypadloop() / 60;
    var_2 = var_0 / 60;
...
setxenonranks( var_0 )
{
    ...
    {
        var_3 = var_1[var_2];
        add_to_fulton_actor_players( var_3 );
    }
}

add_to_fulton_actor_players( var_0, var_1 )
{
    if ( !isdefined( var_0 ) )
        return;
    ...
    else if ( scripts\mp\utility\game::playerenemypool() )
    {
        var_3 = 1000 - var_1;
...
checktimelimit( var_0 )
{
    ...
    if ( var_1 > 0 && !isdefined( level.init_civs ) )
    {
        if ( var_2 )
        {
            if ( istrue( level._id_11A62 ) )
    ...
    if ( var_2 )
    {
        ...
        if ( level.currenttimelimitdelay < level.ontimelimitgraceperiod )
        {
            if ( istrue( level._id_11A62 ) )
            {
                if ( !isdefined( level.init_chopper_boss ) )
...
checkteamscorelimitsoon( var_0 )
{
    ...
    if ( !level.playedmatchendingsoon && scripts\mp\utility\game::canplayhalfwayvo() )
    {
        if ( getteamscore( var_0 ) >= int( level.scorelimit * level.currentround - level.scorelimit / 2 ) )
        {
            ...
            thread scripts\mp\music_and_dialog::restock_dialoguesectionalogic( var_0 );
...
checkscorelimit( var_0 )
{
    ...
    if ( level.teambased )
    {
        ...
        {
            if ( level scripts\mp\utility\game::playerhumanconcusspushcleanup( level.teamnamelist[var_2] ) )
                continue;
    ...
    else
    {
        ...
        if ( self.score < level.roundscorelimit && !istrue( level.weaponclassweights ) )
            return 0;
    }
...
teamstarttimer( var_0, var_1 )
{
    ...
    if ( !level.gameended )
    {
        if ( scripts\mp\utility\game::getgametype() == "br" )
        {
            thread scripts\mp\gametypes\br_public.gsc::trap_trigger_logic( var_0 );
            thread scripts\mp\gametypes\br_public.gsc::trap_door_nvg_reset( var_0 );
            thread scripts\mp\gametypes\br_public.gsc::binoculars_hascoldblooded();
        }
...
checkroundswitch( var_0 )
{
    ...
    if ( get_random_search_node() )
    {
        if ( waittillhuntersdrop() )
            return 1;
    }
...
{
    if ( isdefined( level.waittill_wave_died_to_or_timeout ) )
    {
        level [[ level.waittill_wave_died_to_or_timeout ]]( var_0, var_1 );
        return;
    }

    if ( !level.doeomcombat )
    {
        wait( level.train_init_lootcrates_on_train + 0.1 );
    ...
    if ( isdefined( var_0 ) )
    {
        foreach ( var_2 in level.players )
        {
            ...
            if ( var_3 == var_0 )
            {
                ...
                if ( isdefined( var_2.start_marquee ) )
                    var_2.start_marquee = !var_2.start_marquee;
                else
                    var_2.start_marquee = 1;
                ...
                if ( istrue( scripts\mp\utility\game::matchmakinggame() ) )
                {
                    if ( isdefined( var_2 ) && var_2 isfireteamleader() )
                    {
                        ...
                        foreach ( var_10 in var_8 )
                        {
                            ...
                            if ( var_11 != var_3 )
                            {
                                if ( isdefined( var_10.start_marquee ) )
                                    var_10.start_marquee = !var_10.start_marquee;
                                else
                                    var_10.start_marquee = 1;
...
make_fly_struct()
{
    if ( scripts\mp\utility\game::getgametype() == "br" )
    {
        if ( !scripts\mp\utility\game::privatematch() && !scripts\mp\gametypes\br_public.gsc::play_death_audio() )
        {
            var_0 = [];
            ...
            foreach ( var_2 in level.players )
            {
                ...
                if ( isdefined( var_2.start_marquee ) && var_2.start_marquee )
                {
                    var_3 = var_2.team;
                    var_4 = 0;
                    var_5 = var_0.size;
                    ...
                    {
                        var_0[var_5] = var_3;
                        var_7 = scripts\mp\utility\teams::getteamdata( var_3, "players" );
                        var_8 = [];
...
callback_startgametype()
{
    ...
    level.custom_shouldtakedamage = getdvarint( "scr_game_cdltuning", 0 );
    level.get_fake_digit_from_pool = scripts\mp\utility\game::isanymlgmatch();
    ...
    level._id_12AE3 = getdvarint( "scr_game_wrapKillstreaks" );
    ...
    level.matchrules_vampirism = scripts\mp\utility\dvars::last_good_drop_pos( "scr_" + scripts\mp\utility\game::getgametype() + "_vampirism", "scr_game_vampirism" );

    if ( level.matchrules_vampirism )
        level.modifyplayerdamage = scripts\mp\damage::gamemodemodifyplayerdamage;

    level.finalkillcamtype = scripts\mp\utility\dvars::last_good_drop_pos( "scr_" + scripts\mp\utility\game::getgametype() + "_killcamType", "scr_game_killcamType" );

    if ( level.finalkillcamtype == 2 )
        level.skipfinalkillcam = 1;

    level.allowkillstreaks = scripts\mp\utility\dvars::last_good_drop_pos( "scr_" + scripts\mp\utility\game::getgametype() + "_allowKillstreaks", "scr_game_allowKillstreaks" );
    level.roundretainstreaks = scripts\mp\utility\dvars::last_good_drop_pos( "scr_" + scripts\mp\utility\game::getgametype() + "_roundRetainStreaks", "scr_game_roundRetainStreaks" );
    level.roundretainstreakprog = scripts\mp\utility\dvars::last_good_drop_pos( "scr_" + scripts\mp\utility\game::getgametype() + "_roundRetainStreakProg", "scr_game_roundRetainStreakProg" );
    level.deathretainstreaks = scripts\mp\utility\dvars::last_good_drop_pos( "scr_" + scripts\mp\utility\game::getgametype() + "_deathRetainStreaks", "scr_game_deathRetainStreaks" );
    level.allowperks = scripts\mp\utility\dvars::last_good_drop_pos( "scr_" + scripts\mp\utility\game::getgametype() + "_allowPerks", "scr_game_allowPerks" );
    level.allowsupers = scripts\mp\utility\dvars::last_good_drop_pos( "scr_" + scripts\mp\utility\game::getgametype() + "_allowSupers", "scr_game_allowSupers" );
    level.superfastchargerate = scripts\mp\utility\dvars::last_good_drop_pos( "scr_" + scripts\mp\utility\game::getgametype() + "_superFastChargeRate", "scr_game_superFastChargeRate" );
    level.superpointsmod = scripts\mp\utility\dvars::last_exfil_nag( "scr_" + scripts\mp\utility\game::getgametype() + "_superPointsMod", "scr_game_superPointsMod" );
    level.spawnprotectiontimer = scripts\mp\utility\dvars::last_exfil_nag( "scr_" + scripts\mp\utility\game::getgametype() + "_spawnProtectionTimer", "scr_game_spawnProtectionTimer" );
    level.lethaldelay = scripts\mp\utility\dvars::last_exfil_nag( "scr_" + scripts\mp\utility\game::getgametype() + "_lethalDelay", "scr_game_lethalDelay" );
    level.equipmentmatchstartshieldms = scripts\mp\utility\dvars::last_exfil_nag( "scr_" + scripts\mp\utility\game::getgametype() + "_equipmentMSProtect", "scr_game_equipmentMSProtect" ) * 1000;
    level.magcount = scripts\mp\utility\dvars::last_good_drop_pos( "scr_" + scripts\mp\utility\game::getgametype() + "_magcount", "scr_game_magcount" );

    if ( !isdefined( level.practiceround ) )
        level.practiceround = scripts\mp\utility\dvars::last_good_drop_pos( "scr_" + scripts\mp\utility\game::getgametype() + "_practiceRound", "scr_game_practiceRound" );

    level.postgameexfil = scripts\mp\utility\dvars::last_good_drop_pos( "scr_" + scripts\mp\utility\game::getgametype() + "_postGameExfil", "scr_game_postGameExfil" );
    level.exfilactivetimer = scripts\mp\utility\dvars::last_exfil_nag( "scr_" + scripts\mp\utility\game::getgametype() + "_exfilActiveTimer", "scr_game_exfilActiveTimer" );
    level.exfilextracttimer = scripts\mp\utility\dvars::last_exfil_nag( "scr_" + scripts\mp\utility\game::getgametype() + "_exfilExtractTimer", "scr_game_exfilExtractTimer" );
    level.useammorestocklocs = scripts\mp\utility\dvars::last_exfil_nag( "scr_" + scripts\mp\utility\game::getgametype() + "_useAmmoRestockLocs", "scr_game_useAmmoRestockLocs" );
    level._id_11E71 = scripts\mp\utility\dvars::last_good_drop_pos( "scr_" + scripts\mp\utility\game::getgametype() + "_allow3rdspectate", "scr_game_allow3rdspectate" );
    setdvar( "LKNNQKNTS", level._id_11E71 );
    ...
    if ( scripts\mp\utility\game::playercleargulagomnvars() )
        deafen_ai();
    ...
    level.radarhideshots = scripts\mp\utility\dvars::last_good_drop_pos( "scr_" + var_0 + "_radarHidePings", "scr_game_radarHidePings" );
    setomnvar( "ui_compass_hide_weapon_pings_minimap", level.radarhideshots );
    level.navbarhideshots = scripts\mp\utility\dvars::last_good_drop_pos( "scr_" + var_0 + "_navBarHidePings", "scr_game_navBarHidePings" );
    setomnvar( "ui_compass_hide_weapon_pings_navbar", level.navbarhideshots );
    level.navbarhideenemies = scripts\mp\utility\dvars::last_good_drop_pos( "scr_" + var_0 + "_navBarHideEnemy", "scr_game_navBarHideEnemy" );
    ...
    level.get_evade_start_structs_in_front = getdvarint( "scr_player_disableMount" );
    level.disablebattlechatter = getdvarint( "scr_game_disableBattleChatter" );
    level.get_current_armor_ammo = getdvarint( "scr_game_disableAnnouncer" );
    level.scoremod = [];
    ...
    {
        if ( scripts\mp\utility\game_utility_mp::rpg_guys_construction_spawners() )
        {
    ...
    scripts\cp_mp\utility\game_utility::onplayereliminated_func();
    level.maxallowedteamkills = scripts\mp\utility\dvars::last_good_drop_pos( "scr_" + scripts\mp\utility\game::getgametype() + "_ffPunishLimit", "scr_game_ffPunishLimit" );
    thread scripts\mp\init_cp_mp::init();
    thread scripts\mp\teams::init();
    scripts\mp\trials\mp_trl_gunslinger_crash_create_script::_id_1218E();
    scripts\cp_mp\utility\player_utility::onscavengerbagpickup();
    ...
    thread scripts\cp\agents\gametype_cp_specops::init();
    thread scripts\cp\stealth\manager::init();
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" )
    {
        thread scripts\cp\helicopter\utility::init();
    ...
    scripts\mp\utility\player_frame_update_aggregator::registerplayerframeupdatecallback( scripts\mp\weapons::loadoutdefaultfiresalediscount );

    if ( scripts\mp\utility\game::matchmakinggame() && !scripts\mp\utility\game::privatematch() )
        level scripts\common\ui::lui_registercallback( "play_again_squad_request", ::start_loc );
    ...
    if ( !istrue( game["isLaunchChunk"] ) )
    {
        ...
        thread scripts\asm\soldier\cp::init();
    ...
    thread scripts\cp\equipment\cp_incendiarylauncher::init();
    ...
    foreach ( var_5 in level.teamnamelist )
    {
        ...
        if ( !istrue( level.vehicle_watchmarkedtankdeath ) )
    ...
    level.train_init_lootcrates_on_train = 0.25;
    ...
    if ( getdvarint( "scr_allow_custom_loadouts", 0 ) == 0 || scripts\mp\utility\game::play_hack_reminder_goto1() )
    ...
    level thread vehicle_occupancy_updateriotshield();
}
...
timelimitthread()
{
    ...
    while ( game["state"] == "playing" )
    {
        ...
        if ( isdefined( level.starttime ) )
        {
            if ( gettimeremaining() < 3000 || istrue( level.init_civs ) )
            {
                waitframe();
                continue;
            }
...
timelimitclock()
{
    ...
    if ( isdefined( level._id_1223C ) )
    {
        thread [[ level._id_1223C ]]();
        return;
    }
    ...
    while ( game["state"] == "playing" )
    {
        if ( !level.timerstopped && scripts\mp\utility\game::gettimelimit() && !istrue( level.bombsplanted ) )
        {
            ...
            if ( var_6 <= 10 || var_6 <= 30 && var_6 % var_4 == var_7 )
            {
                ...
                if ( var_8 )
                {
                    var_9 = juggernaut_kill_assists_included( var_5 );
...
gametimer()
{
    ...
    if ( !isdefined( game["startTimeFromMatchStart"] ) )
    {
        ...
        scripts\common\utility::_id_12456( level.remove_reduce_recoil );
    }
...
startgame()
{
    ...
    if ( getdvarint( "scr_game_updated_prematch_allows", 1 ) == 1 )
    {
        foreach ( var_1 in level.players )
        {
            ...
            if ( scripts\mp\utility\game::getgametype() == "br" )
                var_1 scripts\mp\persistence::overwatch_player();
        }
    }
    ...
    thread graceperiod();
    thread scripts\cp\agents\gametype_cp_specops::roundbegin();
    ...
    if ( scripts\mp\codcasterclientmatchdata::shouldlogcodcasterclientmatchdata() )
        thread rear_spotlight_turret_info();
}
...
cleanpatchablecollision()
{
    ...
    foreach ( var_3, var_2 in var_0 )
    {
        if ( isdefined( var_2 ) )
        {
            if ( isdefined( level.spawn_aitype ) && istrue( level.spawn_aitype[var_3] ) )
            {
                level.spawn_aitype[var_3] = undefined;
                continue;
            }
    ...
    level.spawn_ai_solo = 1;

    if ( isdefined( level.spawn_aitype ) )
    {
        foreach ( var_3, var_5 in level.spawn_aitype )
        {

        }
    }

    level.spawn_aitype = undefined;
}

vehiclecanopendoor( var_0 )
{
    if ( istrue( level.spawn_ai_solo ) )
        return;

    if ( !isdefined( level.spawn_aitype ) )
        level.spawn_aitype = [];

    level.spawn_aitype[var_0] = 1;
}
...
wavetimerwatcher()
{
    ...
    for (;;)
    {
        scripts\engine\utility::_id_128FF( "wave_respawn_allies", "wave_respawn_axis" );
...
displayroundend( var_0, var_1, var_2 )
{
    ...
    if ( scripts\mp\utility\game::waslastround() && scripts\mp\utility\game::iswinbytworulegametype() && scripts\mp\utility\game::_id_11BEB() )
    {
        var_0 = "tie";
...
freezeallplayers( var_0, var_1, var_2, var_3 )
{
    ...
    if ( isdefined( level.agentarray ) )
    {
        foreach ( var_8 in level.agentarray )
            var_8 scripts\mp\utility\player::_freezecontrols( 1, undefined, "freezeAllPlayers" );
    }
}
...
setendofroundsoundtimescalefactor()
{
    if ( !isdefined( level.grenade_structs ) )
    {
        soundsettimescalefactorfromtable( "end_of_round" );
        level.grenade_structs = 1;
        level.vehoccupancy_lastseatbc = undefined;
    }
}

resetsoundtimescalefactor()
{
    if ( !isdefined( level.vehoccupancy_lastseatbc ) )
    {
        soundsettimescalefactorfromtable( "reset" );
        level.grenade_structs = undefined;
        level.set_up_rear_minigun = undefined;
        level.vehoccupancy_lastseatbc = 1;
    }
}
...
restart()
{
    if ( isdefined( level.should_spawn_drones ) )
        [[ level.should_spawn_drones ]]();
...
endgame( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    if ( isdefined( level.rpg_shoot_at_trig_watch ) )
        var_0 = level.rpg_shoot_at_trig_watch;
...
endgame_regularmp( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    scripts\common\utility::_id_12456( level.remove_roof_nodes );
    var_5 = 0;
    ...
    level.enemy_spawners = istrue( var_3 ) && scripts\mp\utility\game::getgametype() == "ctf";

    if ( isdefined( var_0 ) && level.enemy_spawners )
    {
...
endgame_showkillcam()
{
    ...
    if ( istrue( level.check_carrier_status ) )
        return;
...
postroundfadenokillcam()
{
    foreach ( var_1 in level.players )
    {
        ...
        var_1 thread transitionplayerstoac130cinematic();
    }
...
goal_ar( var_0 )
{
    if ( isdefined( var_0 ) )
    {
        if ( var_0 == 1 || var_0 == 2 )
        {
            level.addwatch2v2topscore = 0;
            level.activestate = var_0;
...
endgame_endround( var_0, var_1, var_2, var_3 )
{
    ...
    if ( scripts\mp\utility\game::waslastround() )
    {
        if ( scripts\mp\utility\game::getgametype() == "arena" )
        {
            ...
            foreach ( var_7 in level.players )
            {
                ...
                {
                    if ( var_7 == var_4[0] )
                        var_7 scripts\cp_mp\pet_watch::ammo_boxes();
                }
            }
        }
    ...
    else
    {
        if ( level.teambased )
        {
            ...
            {
                if ( level scripts\mp\utility\game::playerhumanconcusspushcleanup( var_25 ) )
                    continue;
            ...
            if ( !scripts\mp\utility\game::waslastround() )
            {
                ...
                foreach ( var_7 in level.players )
                {
                    ...
                    if ( isalive( var_7 ) && isdefined( var_7.matchdatalifeindex ) )
                        var_7 scripts\common\utility::_id_12456( level.remove_padding_damage, var_7.matchdatalifeindex, undefined, undefined, "MOD_ROUND_ENDED", "none", undefined, undefined );
                }
...
endgame_endgame( var_0, var_1, var_2, var_3 )
{
    ...
    if ( scripts\mp\gametypes\br_public.gsc::play_death_audio() && !level.forcedend )
    {
        level.addwatch2v2topscore = 1;
        level scripts\common\ui::lui_registercallback( "exit_squad_eliminated", ::goal_ar );
    }

    if ( scripts\mp\utility\game::matchmakinggame() && isdefined( level._id_11BC6 ) && [[ level._id_11BC6 ]]() )
        requestgamelobbyremainintact();

    if ( !istrue( level.processedwinloss ) && ( istrue( level.forfeitinprogress ) || level.forcedend ) )
        updatewinlossstats( var_0 );

    scripts\cp\agents\gametype_cp_specops::roundend( var_0 );
    checkforpersonalbests();
    updatespmstats();
    scripts\mp\persistence::writebestscores();
    level notify( "stop_leaderboard_stats" );
    updateleaderboardstats();
    level scripts\mp\accolades::handleteamvisibility();
    level.doingbroshot = scripts\mp\broshot::initbroshot( var_0 );
    var_4 = scripts\mp\gametypes\br_gametypes.gsc::play_stealthy_disguise_vo( "endGameIntermission" );

    if ( !level.doingbroshot && istrue( var_4 ) )
    {
        level notify( "spawning_intermission" );
        var_5 = killleaderaftertimeout();

        foreach ( var_7 in level.players )
        {
            var_7 thread scripts\mp\utility\game::setuipostgamefade( 0.0 );

            if ( !isdefined( level.rpggetclosetoapc ) || !var_7 [[ level.rpggetclosetoapc ]]() )
    ...
    if ( !level.doingbroshot )
    {
        if ( scripts\mp\utility\game::getgametype() == "br" )
        {
            scripts\mp\gametypes\br_circle.gsc::modeignorevehicleexplosivedamage();
            ...
            if ( getdvarint( "post_game_play_again_enabled", 1 ) == 1 )
            {
                ...
                {
                    if ( istrue( level.addwatch2v2topscore ) )
    ...
    if ( scripts\mp\utility\game::getgametype() != "br" || scripts\mp\utility\game::lb_mg_impulse_dmg_threshold_top() == "dmz" || scripts\mp\utility\game::lb_mg_impulse_dmg_threshold_top() == "risk" || scripts\mp\utility\game::lb_mg_impulse_dmg_threshold_top() == "kingslayer" || scripts\mp\utility\game::player_near_obit() )
        processlobbydata();
    ...
    foreach ( var_7 in level.players )
    {
        ...
        if ( isalive( var_7 ) && isdefined( var_7.matchdatalifeindex ) )
            var_7 scripts\common\utility::_id_12456( level.remove_padding_damage, var_7.matchdatalifeindex, undefined, undefined, "MOD_GAME_ENDED", "none", undefined, undefined );
        ...
        var_7 scripts\common\utility::_id_12456( level.remove_outline );
    }

    scripts\common\utility::_id_12456( level.remove_soldier_armor );
    ...
    if ( getdvarint( "TLRPKRKMS" ) != 0 )
    {
        if ( isgamebattlematch() )
        {
            foreach ( var_7 in level.players )
            {
                ...
                setgamebattleplayerstats( var_16, var_17, var_18 );
            }
            ...
            setgamebattlematchstats( var_20, var_21, var_22, var_23, var_24, var_25, var_26 );
        }
    }
    ...
    if ( scripts\mp\gametypes\br_public.gsc::play_death_audio() && isdefined( level.activestate ) && level.activestate == 2 )
    {
    ...
    if ( !isdefined( level.grenade_effect ) )
    {
        ...
        if ( getdvarint( "TLRPKRKMS" ) != 0 )
        {
            ...
            if ( scripts\mp\utility\game::matchmakinggame() )
            {
                ...
                setmatchdata( "playlist_name", _func_41E() );
...
{
    wait( level.train_init_lootcrates_on_train );
...
killleaderaftertimeout()
{
    ...
    {
        var_0 = spawnstruct();
        var_0.origin = level.cleanup_lights._id_11E75.origin;
        var_0.angles = level.cleanup_lights._id_11E75.angles;
    }
...
bomb_vest_timer_total_num_of_frame( var_0 )
{
    ...
    {
        var_0.clientmatchdataid = level.ents_to_clean_up;
        level.ents_to_clean_up++;
    }
}

trial_restart( var_0 )
{
    if ( istrue( var_0.trial_restart ) )
        return;

    var_0.trial_restart = 1;
    bomb_vest_timer_total_num_of_frame( var_0 );
    var_1 = var_0.name;
    ...
    scripts\mp\matchdata::_id_11A89( var_0 );
}
...
trackattackerleaderboarddeathstats( var_0, var_1, var_2, var_3 )
{
    if ( isdefined( var_0 ) && isplayer( var_0 ) )
    {
        if ( var_3 != "MOD_FALLING" )
        {
            ...
            {
                var_0 scripts\common\utility::_id_12456( level.remove_medic_class, "tactical", "kills", 1, var_2 );
                var_0 scripts\common\utility::_id_12456( level.remove_medic_class, "tactical", "hits", 1, var_2 );
            ...
            if ( var_3 == "MOD_MELEE" && !scripts\mp\riotshield::isriotshield( var_2 ) && !scripts\mp\utility\weapon::isknifeonly( var_2 ) && !scripts\mp\utility\weapon::platform_model( var_2 ) && !scripts\mp\utility\weapon::isaxeweapon( var_2 ) )
            {
                var_0 scripts\common\utility::_id_12456( level.remove_medic_class, "none", "kills", 1, var_2 );
                var_0 scripts\common\utility::_id_12456( level.remove_medic_class, "none", "hits", 1, var_2 );
...
setweaponstat( var_0, var_1, var_2 )
{
    ...
    if ( isdefined( var_3.sortbylastzombietime ) )
        var_3 = var_3.sortbylastzombietime;

    if ( !isdefined( level.create_temp_infil_structs ) )
        level.create_temp_infil_structs = scripts\mp\utility\game::getgametype() == "br" && getdvarint( "scr_track_picked_up_weapon_stats", 1 ) == 1;

    if ( scripts\mp\utility\weapon::ispickedupweapon( var_3 ) && !istrue( level.create_temp_infil_structs ) )
        return;
    ...
    if ( var_5 == "weapon_grenade" || var_5 == "weapon_explosive" || var_4 == "trophy_mp" || var_4 == "forcepush_mp" )
    {
        ...
        scripts\common\utility::_id_12456( level.remove_punchcard, var_4, var_2, var_1, var_6, var_3 );
        return;
    }
    ...
    if ( var_2 == "deaths" )
    {
        ...
        scripts\common\utility::_id_12456( level.remove_punchcard, var_9, "deaths", var_1, var_6, var_3 );

        foreach ( var_12 in var_10 )
        {
            scripts\mp\persistence::incrementattachmentstat( var_12, var_2, var_1, var_3 );
            scripts\common\utility::_id_12456( level.remove_medic_class, var_12, var_2, var_1, var_3 );
        }
    }
}
...
_id_125E9()
{
    ...
    if ( isdefined( level.checkbluemassacre ) && isdefined( level.checkbluemassacre.name ) )
    {
        switch ( level.checkbluemassacre.name )
        {
            ...
            case "kingslayer":
            case "dmz":
                var_0 = var_0 + ( "_" + level.checkbluemassacre.name );
                break;
            case "":
                break;
            default:
    ...
    var_1 = scripts\engine\utility::ter_op( level.teambased, self.score, self.propaddtocircle["gamemodeScore"] );
    ...
    incrementleaderboardstat( "kills", var_0, self.propaddtocircle["kills"] );
    incrementleaderboardstat( "deaths", var_0, self.propaddtocircle["deaths"] );

    switch ( scripts\mp\utility\game::getgametype() )
    {
        case "war":
            incrementleaderboardstat( "assists", var_0, self.propaddtocircle["assists"] );
            break;
        case "arena":
            incrementleaderboardstat( "damage", var_0, self.propaddtocircle["damage"] );
            incrementleaderboardstat( "assists", var_0, self.propaddtocircle["assists"] );
            break;
        case "arm":
            incrementleaderboardstat( "captures", var_0, self.propaddtocircle["captures"] );
            incrementleaderboardstat( "defends", var_0, self.propaddtocircle["defends"] );
            break;
        case "cyber":
            incrementleaderboardstat( "revives", var_0, self.propaddtocircle["rescues"] );
            incrementleaderboardstat( "plants", var_0, self.propaddtocircle["plants"] );
            break;
        case "dm":
            updateleaderboardstatmaximum( "streak", var_0, self.propaddtocircle["killChains"] );
            break;
        case "dom":
            incrementleaderboardstat( "captures", var_0, self.propaddtocircle["captures"] );
            incrementleaderboardstat( "defends", var_0, self.propaddtocircle["defends"] );
        ...
        case "sd":
            incrementleaderboardstat( "plants", var_0, self.propaddtocircle["plants"] );
            incrementleaderboardstat( "defuses", var_0, self.propaddtocircle["defuses"] );
            break;
        case "conf":
            incrementleaderboardstat( "confirms", var_0, self.propaddtocircle["confirmed"] );
            incrementleaderboardstat( "denies", var_0, self.propaddtocircle["denied"] );
            break;
        case "hq":
            incrementleaderboardstat( "captures", var_0, self.propaddtocircle["captures"] );
            incrementleaderboardstat( "defends", var_0, self.propaddtocircle["defends"] );
            break;
        case "koth":
            incrementleaderboardstat( "objTime", var_0, self.propaddtocircle["objTime"] );
            incrementleaderboardstat( "defends", var_0, self.propaddtocircle["defends"] );
            break;
        case "ctf":
            incrementleaderboardstat( "captures", var_0, self.propaddtocircle["captures"] );
            incrementleaderboardstat( "returns", var_0, self.propaddtocircle["returns"] );
            break;
        case "sr":
            incrementleaderboardstat( "plants", var_0, self.propaddtocircle["plants"] );
            incrementleaderboardstat( "rescues", var_0, self.propaddtocircle["rescues"] );
            break;
        case "siege":
            incrementleaderboardstat( "captures", var_0, self.propaddtocircle["captures"] );
            incrementleaderboardstat( "revives", var_0, self.propaddtocircle["rescues"] );
            break;
        case "grind":
            incrementleaderboardstat( "banks", var_0, self.propaddtocircle["confirmed"] );
            incrementleaderboardstat( "denies", var_0, self.propaddtocircle["denied"] );
            break;
        case "infect":
            incrementleaderboardstat( "time", var_0, scripts\mp\utility\stats::getpersstat( "extrascore0" ) );
            incrementleaderboardstat( "infected", var_0, self.propaddtocircle["killsAsInfected"] );
            break;
        case "gun":
            incrementleaderboardstat( "stabs", var_0, self.propaddtocircle["stabs"] );
            incrementleaderboardstat( "setBacks", var_0, self.propaddtocircle["setbacks"] );
            break;
        case "grnd":
            incrementleaderboardstat( "objTime", var_0, self.propaddtocircle["objTime"] );
            incrementleaderboardstat( "defends", var_0, self.propaddtocircle["defends"] );
        ...
        case "pill":
            ...
            break;
        case "br":
            incrementleaderboardstat( "downs", var_0, self.propaddtocircle["downs"] );
            incrementleaderboardstat( "contracts", var_0, self.propaddtocircle["contracts"] );
            incrementleaderboardstat( "wins", var_0, self.propaddtocircle["wins"] );
            incrementleaderboardstat( "topFive", var_0, self.propaddtocircle["topFive"] );
            incrementleaderboardstat( "topTen", var_0, self.propaddtocircle["topTen"] );
            incrementleaderboardstat( "topTwentyFive", var_0, self.propaddtocircle["topTwentyFive"] );
            incrementleaderboardstat( "gamesPlayed", var_0, self.propaddtocircle["gamesPlayed"] );
            incrementleaderboardstat( "revives", var_0, self.propaddtocircle["rescues"] );
            incrementleaderboardstat( "cash", var_0, self.propaddtocircle["cash"] );
            break;
    }
}
...
checkforbestweapon()
{
    var_0 = scripts\common\utility::_id_12456( level.remove_marker_when_player_disconnects );
    var_1 = "";
    var_2 = -1;
...
_id_129EB( var_0 )
{
    if ( !istrue( level.registeronrespawn ) )
        return;
    ...
    if ( var_6 <= var_8 )
    {
        level.registerontimerexpired = 1;
        var_9 = gettime() - var_3;
        ...
        if ( isdefined( level.should_skip_default_intro_scene ) )
            [[ level.should_skip_default_intro_scene ]]();
    }
...
vehicle_occupancy_updateriotshield()
{
    wait 10;
    var_0 = [];
    ...
    foreach ( var_2 in var_0 )
        scripts\cp_mp\utility\game_utility::vehicle_occupancy_mp_updateriotshield( var_2, "targetname" );
}
...
deafen_ai()
{
    level.allowsupers = getdvarint( "scr_" + scripts\mp\utility\game::lb_mg_impulse_dmg_threshold_top() + "_allowSupers", level.allowsupers );
}

rear_spotlight_turret_info()
{
    foreach ( var_1 in level.players )
    {
        if ( isplayer( var_1 ) )
...
reason_stealth_broken( var_0 )
{
    ...
    if ( isdefined( var_0.extracthelipadusecallback ) )
        var_6 = var_0.extracthelipadusecallback;
