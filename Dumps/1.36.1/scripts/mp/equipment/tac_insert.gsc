...
{
    level._id_12119 = [];
    scripts\mp\utility\join_team_aggregator::registeronplayerjointeamcallback( ::_id_12113 );

    if ( scripts\mp\utility\game::getgametype() == "br" )
        level thread _id_1210B();
}

_id_12113( var_0 )
{
    foreach ( var_2 in level._id_12119 )
    {
        var_2 _id_12118( var_0 );
        var_2 _id_12117( var_0 );
    }
}
...
_id_12112()
{
    ...
    for (;;)
    {
        ...
        scripts\engine\utility::_id_12913( var_2, "offhand_pullback" );
    }
}
...
tacinsert_used( var_0 )
{
    ...
    if ( !_id_1210C( var_0, var_3, var_4 ) )
    {
        scripts\mp\hud_message::showerrormessage( "MP/TAC_INSERT_CANNOT_PLACE" );
        thread _id_12115();
        return 0;
    ...
    var_17 thread _id_1210D();
    var_17 setotherent( self );
    var_17.weapon_name = "tac_insert_trigger";
    var_18 = var_7["entity"];

    if ( isdefined( var_18 ) && ( scripts\mp\trials\mp_trl_gunslinger_knife_create_script::pity_timer_end_time( var_18 ) || scripts\mp\gametypes\br_gondola.gsc::phone_group_spawned( var_18 ) ) )
    {
        var_17 linkto( var_18 );
        var_19 = getdvarint( "scr_br_tac_insert_moving_platform_valid", 1 );

        if ( var_19 )
            var_17 thread _id_12111( self );
    }
    ...
    _id_11E3C( var_1, var_16, var_17, 1 );
    var_21 = var_17 getentitynumber();
    level._id_12119[var_21] = var_17;

    if ( var_17.issuper && isdefined( level._id_1210F ) )
        self [[ level._id_1210F ]]();
...
tacinsert_setupandwaitfordeath( var_0 )
{
    ...
    thread _id_1210E( var_0 );

    if ( !self.issuper )
        thread _id_12114( var_0 );
}
...
_id_12114( var_0 )
{
    ...
    level endon( "game_ended" );
    var_0 endon( "disconnect" );
    self.spawndistancemin = spawn( "script_origin", self.origin );
    self.spawndistancemin makeusable();
    self.spawndistancemin setcursorhint( "HINT_NOICON" );
    self.spawndistancemin sethintstring( &"MP_PATCH/PICKUP_TI" );
    self.spawndistancemin linkto( self );
    ...
    for (;;)
    {
        self.spawndistancemin waittill( "trigger", var_2 );
...
_id_12118( var_0 )
{
    ...
    if ( !isdefined( self ) || !isdefined( self.spawndistancemin ) )
        return;
    ...
    if ( var_1 )
        self.spawndistancemin enableplayeruse( var_0 );
    else
        self.spawndistancemin disableplayeruse( var_0 );
}
...
tacinsert_destroy( var_0, var_1, var_2, var_3 )
{
    ...
    if ( isdefined( self.owner ) )
    {
        if ( istrue( var_2 ) )
        {
            ...
            self.owner scripts\mp\supers::dmg_trig( "super_tac_insert" );
            self.owner scripts\cp\agents\gametype_cp_specops::short_term_goal( "super_tac_insert", 1, var_0, var_1 );
            scripts\mp\analyticslog::logevent_fieldupgradeexpired( self.owner, self.superid, 1, istrue( var_1 ) );
        }
        else
        {
            self.owner scripts\cp\agents\gametype_cp_specops::short_term_goal( "super_tac_insert", 0, var_0, var_1 );
            scripts\mp\analyticslog::logevent_fieldupgradeexpired( self.owner, self.superid, 0, istrue( var_1 ) );
        }

        if ( !istrue( var_2 ) && !istrue( var_3 ) )
            self.owner thread _id_12116();
...
_id_1210C( var_0, var_1, var_2 )
{
    ...
    if ( isdefined( var_6 ) )
    {
        if ( isdefined( var_6["hittype"] ) && var_6["hittype"] != "hittype_none" )
        {
            ...
            if ( isdefined( var_8 ) )
            {
                if ( var_8 scripts\cp_mp\vehicles\vehicle::isvehicle() && !scripts\mp\trials\mp_trl_gunslinger_knife_create_script::pity_timer_end_time( var_8 ) )
                    var_3 = 0;

                var_9 = getdvarint( "scr_br_tac_insert_moving_platform_valid", 1 );

                if ( !var_9 && ( scripts\mp\trials\mp_trl_gunslinger_knife_create_script::pity_timer_end_time( var_8 ) || scripts\mp\gametypes\br_gondola.gsc::phone_group_spawned( var_8 ) ) )
                    var_3 = 0;
...
_id_12115()
{
    ...
    if ( istrue( level.play_vo_or_timeout ) )
    {
        ...
        var_1 = scripts\engine\utility::_id_12913( 5, "super_use_finished_lb" );
