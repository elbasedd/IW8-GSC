...
updatekillstreakuislots()
{
    ...
    for ( var_1 = 0; var_1 <= kiosk_spent_total(); var_1++ )
...
updatekillstreakuislot( var_0 )
{
    ...
    if ( isdefined( level.run_current_spawn_group ) && ![[ level.run_current_spawn_group ]]( var_0 ) )
        return;
    ...
    if ( isdefined( var_2 ) && isdefined( var_2.streakname ) )
    {
        ...
        add_to_bomb_detonator_waiting_for_pick_up_array( var_0, var_3 );
        add_struct( var_0, var_2.available );
    }
    else
    {
        self setclientomnvar( var_1.slotomnvars[var_0], 0 );
        self setclientomnvar( var_1.availableomnvars[var_0], 0 );
        add_to_bomb_detonator_waiting_for_pick_up_array( var_0, 0 );
        add_struct( var_0, 0 );
    }
}
...
triggerkillstreak( var_0, var_1 )
{
    ...
    if ( isdefined( level.round_spawn_bombers ) && !self [[ level.round_spawn_bombers ]]( var_0, var_1 ) )
    {
        scripts\mp\hud_message::showerrormessage( "KILLSTREAKS/CANNOT_BE_USED" );
        return 0;
    }
...
onsuccessfulstreakactivation( var_0, var_1 )
{
    ...
    if ( istrue( level._id_12AE3 ) && self.streaktype != "specialist" )
        var_0.available = -1;
    else
        var_0.available = -1;

    add_struct( var_1, var_0.available );
    ...
    thread scripts\cp\agents\gametype_cp_specops::usedkillstreak( var_2 );
    ...
    var_0.airdrop_crushchicken = var_6;
    scripts\mp\analyticslog::logevent_killstreakactivated( self, var_0._id_12052, var_0.streakname, var_0.isgimme, var_6, self.origin );
    ...
    if ( isdefined( self.petwatch ) )
    {
        ...
        if ( var_2 == "nuke" )
            ...
            scripts\cp_mp\pet_watch::allowassassinationdamage();
    }
}
...
createstreakitemstruct( var_0 )
{
    ...
    var_1.isspecialist = scripts\mp\perks\perks::playercanseedangercircleworld( var_0 );
    var_1._id_11E6E = scripts\mp\perks\perks::getspecialistperkforstreak( var_0 );
    return var_1;
}
...
makekillstreakavailable( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    if ( isdefined( var_5._id_11E6E ) )
        var_6 = var_5._id_11E6E;
    ...
    var_5.available = 1;
    add_struct( var_0, var_5.available );
    ...
    var_5._id_12052 = self.matchdatalifeindex;
    var_5.owner = self;
    var_5.snowballfighthint = self getxuid();

    if ( isdefined( var_2 ) )
        var_5.streaklifeid = var_2;

    if ( isdefined( var_3 ) )
        var_5._id_12052 = var_3;

    if ( isdefined( var_4 ) )
    {
        var_5.owner = var_4;
        var_5.snowballfighthint = var_4 getxuid();
    }

    scripts\cp\agents\gametype_cp_specops::should_enter_combat_after_checking_smoke_grenade( var_5.streakname, var_1 );
    scripts\mp\analyticslog::logevent_killstreakavailable( self, var_5._id_12052, var_6, var_5.isgimme, var_5.madeavailabletime, self.origin );

    if ( isdefined( self.petwatch ) && var_0 == kiosk_spent_total() )
    {
        var_8 = 0;
        var_9 = 0;
        ...
        if ( var_8 && var_9 )
            scripts\cp_mp\pet_watch::allow_earthquake();
        else
            scripts\cp_mp\pet_watch::allow_deleteme_path();
    }
}
...
killstreakselectionwatcher()
{
    ...
    for (;;)
    {
        var_0 = scripts\engine\utility::_id_12907( "ks_select_up", "ks_select_down" );
...
selectfirstavailablekillstreak()
{
    ...
    {
        for ( var_1 = kiosk_spent_total(); var_1 >= 0; var_1-- )
        {
...
{
    var_1 = var_0;
    var_2 = scripts\engine\utility::ter_op( var_0 >= kiosk_spent_total(), 0, var_0 + 1 );
    var_3 = var_0;
    var_4 = var_2;
...
{
    var_1 = var_0;
    var_2 = scripts\engine\utility::ter_op( var_0 <= 0, kiosk_spent_total(), var_0 - 1 );
    var_3 = var_0;
    var_4 = var_2;
...
selectmostexpensivekillstreak()
{
    ...
    for ( var_2 = kiosk_spent_total(); var_2 >= 0; var_2-- )
    {
...
selectnextavailablekillstreak()
{
    ...
    for ( var_2 = 0; var_2 <= kiosk_spent_total(); var_2++ )
    {
        var_3 = self.streakdata.streaks[var_2];
...
registerkillstreak( var_0, var_1, var_2, var_3 )
{
    ...
    var_4.get_best_goal_position = var_3;
}
...
checkstreakreward( var_0, var_1 )
{
    for ( var_2 = 1; var_2 <= 4; var_2++ )
    {
        ...
        if ( isdefined( var_3.lifeid ) && var_3.lifeid == self.lifeid && ( !istrue( level._id_12AE3 ) || istrue( level._id_12AE3 ) && self.streaktype == "specialist" ) || istrue( level._id_12AE3 ) && istrue( level.casualscorestreaks ) && self.streaktype != "specialist" && var_3.available == -1 )
            continue;
...
givestreakpoints( var_0, var_1, var_2 )
{
    ...
    if ( var_6 && !istrue( level.get_fake_digit_from_pool ) && ( istrue( level.allowkillstreaks ) || isdefined( level.spawn_overwatch_soldiers_01 ) ) )
    {
        var_7 = self.nukepoints + var_1;
        var_8 = calcstreakcost( "nuke" );

        if ( isdefined( level.spawn_overwatch_soldiers_01 ) )
            var_8 = level.spawn_overwatch_soldiers_01;

        if ( var_7 >= var_8 )
        {
            self.nukepoints = var_8;

            if ( !istrue( self.getrandompointincirclewithindistance ) )
            {
                thread scripts\mp\hud_message::showkillstreaksplash( "nuke", var_8 );
                awardkillstreak( "nuke", "earned" );
                self.getrandompointincirclewithindistance = 1;
            }
        }
        else
        {
            ...
            if ( isdefined( self.petwatch ) )
                scripts\cp_mp\pet_watch::_id_125E1( self.nukepoints / var_8 );
        }
    }
...
resetstreakavailability( var_0 )
{
    ...
    for ( var_1 = 0; var_1 <= 4; var_1++ )
    {
        ...
        if ( isdefined( var_2 ) && isdefined( var_2.streakname ) )
        {
            ...
            add_struct( var_1, var_2.available );
        }
    }
}
        ...
        foreach ( var_1 in level.players )
        {
            ...
            if ( isdefined( var_1.streakpoints ) )
            {
                if ( var_1 scripts\mp\utility\perk::_hasperk( "specialty_killstreak_to_scorestreak" ) )
                    var_2 = scripts\mp\perks\perkfunctions::playerzombiestreamwaittillcomplete( var_1.streakpoints );
                else
...
_id_125F5( var_0, var_1 )
{
    if ( !isdefined( var_0 ) || !isdefined( self ) )
        return;

    if ( !isdefined( self.weaponusagecheck ) )
        self.weaponusagecheck = [];

    var_2 = var_0 getxuid();
    var_0.mark_as_bomb_vest_controller_holder = 1;

    if ( !isdefined( self.weaponusagecheck[var_2] ) )
    {
        var_3 = spawnstruct();
        var_3.damage = 0;
        var_3.player = var_0;
        self.weaponusagecheck[var_2] = var_3;
    }

    self.weaponusagecheck[var_2].damage = self.weaponusagecheck[var_2].damage + var_1;
}
...
lastunruly( var_0 )
{
    ...
    foreach ( var_3 in self.weaponusagecheck )
    {
        var_4 = var_3.damage;
        var_5 = var_3.player;
...
initridekillstreak_internal( var_0 )
{
    if ( isdefined( var_0 ) && islaptoptimeoutkillstreak( var_0 ) )
        ...
        var_1 = scripts\engine\utility::_id_12915( 1.0, "death", "disconnect", "weapon_switch_started" );
    ...
    if ( !isdefined( var_0 ) || !issubstr( var_0, "pointSelect" ) )
    {
        if ( var_0 == "drone_hive" )
        {
            ...
            var_2 = scripts\engine\utility::_id_12913( 0, "death_or_disconnect" );
        }
        else
        {
            ...
            var_2 = scripts\engine\utility::_id_12913( 0.8, "death_or_disconnect" );
        }
    }
    else
        var_2 = scripts\engine\utility::_id_12913( 1.0, "death_or_disconnect" );
...
checkcasualstreaksreset()
{
    ...
    if ( isdefined( self.streaktype ) && self.streaktype == "specialist" || isdefined( self.loadoutstreaktype ) && self.loadoutstreaktype == "specialist" || !istrue( level._id_12AE3 ) )
    {
