...
init()
{
    ...
    level.laststandhealth = scripts\mp\utility\dvars::last_good_drop_pos( "scr_" + scripts\mp\utility\game::getgametype() + "_lastStandHealth", "scr_player_lastStandHealth" );
    level.laststandrevivehealth = scripts\mp\utility\dvars::last_good_drop_pos( "scr_" + scripts\mp\utility\game::getgametype() + "_lastStandReviveHealth", "scr_player_lastStandReviveHealth" );
    ...
    level.laststandinvulntime = scripts\mp\utility\dvars::last_exfil_nag( "scr_" + scripts\mp\utility\game::getgametype() + "_lastStandInvulnTime", "scr_player_lastStandInvulnTime" );
    level.prematchplayedwelcomevo = scripts\mp\utility\dvars::last_exfil_nag( "scr_" + scripts\mp\utility\game::getgametype() + "_lastStandReviveDecayScale", "scr_player_lastStandReviveDecayScale" );
    level.laststandrevivetimer = scripts\mp\utility\dvars::last_exfil_nag( "scr_" + scripts\mp\utility\game::getgametype() + "_lastStandReviveTimer", "scr_player_lastStandReviveTimer" );
    level.laststandsuicidetimer = scripts\mp\utility\dvars::last_exfil_nag( "scr_" + scripts\mp\utility\game::getgametype() + "_lastStandSuicideTimer", "scr_player_lastStandSuicideTimer" );
    level.laststandtimer = scripts\mp\utility\dvars::last_exfil_nag( "scr_" + scripts\mp\utility\game::getgametype() + "_lastStandTimer", "scr_player_lastStandTimer" );
    level.prematchspawnoriginteamcount = scripts\mp\utility\dvars::last_bag_drop_time( "scr_" + scripts\mp\utility\game::getgametype() + "_lastStandWeapon", "scr_player_lastStandWeapon" );
    level.premount_goalradius = scripts\mp\utility\dvars::last_exfil_nag( "scr_" + scripts\mp\utility\game::getgametype() + "_lastStandWeaponDelay", "scr_player_lastStandWeaponDelay" );
...
laststandthink()
{
    ...
    onenter();
    var_0 = scripts\engine\utility::_id_12911( "last_stand_heal_success", "last_stand_revived", "last_stand_bleedout", "death_or_disconnect", "last_stand_self_revive" );
...
onenter()
{
    ...
    self.inlaststand = 1;
    self.superonset = undefined;
    self.hasshownlaststandicon = 0;

    if ( level.gametype == "br" && !istrue( self.mark_danger_timeout ) )
    {
        if ( isdefined( level.deadmans_cash_pickup ) )
            [[ level.deadmans_cash_pickup ]]();
    }
    ...
    var_4 = level.prematchspawnoriginteamcount;
    ...
    else
    {
        ...
        if ( isdefined( level.propwaitminigamehudsetpoint ) )
            self thread [[ level.propwaitminigamehudsetpoint ]]();
        ...
        thread make_all_oscilloscopes_usable( var_4 );

        if ( isdefined( level.allowed_objectives ) )
            self [[ level.allowed_objectives ]]();
        else
            addoverheadicon();
        ...
        if ( isdefined( self.precomputeddropbagpos ) && self.precomputeddropbagpos scripts\mp\utility\perk::_hasperk( "specialty_reduce_regen_delay_on_kill" ) )
            self.precomputeddropbagpos scripts\mp\perks\perkfunctions::regendelayreduce_onkill();
        ...
        {
            var_6 = "outline_nodepth_brplayer" + self.pers["squadMemberIndex"];
            self.prematchloadouts = scripts\mp\utility\outline::outlineenableforteam( self, self.team, var_6, "laststand" );
        }
...
plundertotal()
{
    if ( !istrue( self.f14_3pastcodes ) )
    {
        self.f14_3pastcodes = 1;
...
make_all_oscilloscopes_usable( var_0 )
{
    ...
    {
        var_2 = level.premount_goalradius;
...
disableweaponsovertime( var_0 )
{
    ...
    scripts\engine\utility::_id_12913( var_0, "death_or_disconnect" );
...
dodamagewhiledown()
{
    ...
    if ( !istrue( level.prematchrandomloadout ) )
        thread suicidesetup();
    ...
    while ( self.health > 0 )
    {
        ...
        if ( !istrue( scripts\mp\playerlogic::jugg_enter_combat_callback() ) )
        {
            var_5 = int( var_4 + var_2 ) - int( var_4 );
            var_4 = var_4 + var_2;
            self.player_abilities_disable = 1;
            self dodamage( var_5, self.origin, self, undefined, "MOD_TRIGGER_HURT", var_3, "none" );
            self.player_abilities_disable = undefined;
        }
...
_id_11970()
{
    ...
    for (;;)
    {
        if ( _id_1196E( var_1 ) && !istrue( var_1.player_unresolved_collision_suspend ) && var_1 isonground() )
        {
            var_0 notify( "self_revive_start" );
            var_1 setlaststandselfreviving( 1 );
            var_0 thread _id_1196F();
        }
...
_id_1196F()
{
    ...
    var_1._id_128E7 = 0;
    var_1 thread _id_1196D();

    while ( scripts\mp\utility\player::isreallyalive( var_0 ) && _id_1196E( var_0 ) && var_1.curprogress < var_1.usetime )
    {
        while ( !var_0 isonground() && _id_1196E( var_0 ) && !istrue( var_0.player_unresolved_collision_suspend ) )
            waitframe();
        ...
        if ( !istrue( var_0.player_unresolved_collision_suspend ) )
        {
            var_0 thread scripts\mp\gametypes\br_public.gsc::tac_rover( "iw8_ges_plyr_self_revive", 10 );

            if ( !istrue( var_0._id_12000 ) )
            {
                var_0 attach( "offhand_wm_stim", "tag_accessory_left" );
                var_0._id_12000 = 1;
            }

            var_0.player_unresolved_collision_suspend = 1;
        ...
        if ( var_1.curprogress >= var_1.usetime )
        {
            ...
            if ( scripts\mp\utility\game::getgametype() == "br" )
                scripts\mp\gametypes\br_analytics.gsc::center_struct( var_0 );

            var_0 hostvictimdamagefactormedium( "self_revive_success", var_0 );

            if ( isdefined( level.vehicle_process_node_when_at_goal ) )
                var_0 [[ level.vehicle_process_node_when_at_goal ]]();
    ...
    if ( !istrue( var_0.beingrevived ) )
    {
        scripts\mp\gametypes\br.gsc::_id_125FC( var_0, var_0, var_1.curprogress, 1 );
...
_id_1196D()
{
    ...
    var_0 scripts\engine\utility::_id_1290F( "last_stand_finished", "stopped_self_revive", "death_or_disconnect" );
    ...
    var_0.player_unresolved_collision_suspend = 0;
}
...
onexitcommon( var_0 )
{
    ...
    var_1 = level.prematchspawnoriginteamcount;
    ...
    if ( istrue( scripts\mp\playerlogic::jugg_enter_combat_callback() ) )
        scripts\mp\playerlogic::_id_11A67( 0 );

    if ( level.gametype == "br" )
    {
        scripts\mp\gametypes\br.gsc::_id_125FC( self, self, 0, 1 );
    ...
    if ( isdefined( level.routers_picked_up ) )
        self [[ level.routers_picked_up ]]( var_0 );
...
onrevive( var_0, var_1 )
{
    ...
    scripts\common\utility::attackpressed( 1 );

    if ( getdvar( "scr_br_gametype", "" ) == "dmz" || getdvar( "scr_br_gametype", "" ) == "risk" )
        scripts\mp\gametypes\br_plunder.gsc::toma_strike_trace_offset( self, 1, 1 );
    ...
    var_3 = level.prematchspawnoriginteamcount;
    ...
    self.pregeneratespawnpoints = undefined;
...
ondeath( var_0 )
{
    ...
    if ( isdefined( var_0 ) && isdefined( self.precomputeddropbagpos ) && istrue( self.precomputeddropbagpos.inlaststand ) )
        self.precomputeddropbagpos thread should_hide_buried_mother_corpse( var_0, self.prematchinitx1blueprintloadouts, self.prepare_mission_failed_text );

    if ( istrue( self.player_unresolved_collision_suspend ) )
...
revivesetup( var_0 )
{
    ...
    if ( var_0 scripts\mp\gametypes\br_public.gsc::mark_danger_timeout() )
        var_0.laststandreviveent _id_11970();
}
...
xmike109()
{
    ...
    var_11 = var_2.precomputeddropbagpos;
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" )
        scripts\mp\gametypes\br_analytics.gsc::center_node( var_0, var_11, var_2, var_5, var_6, var_7, var_8, var_12, var_13, var_14, var_16 );

    var_0 hostvictimdamagefactormedium( "self_revive_on_kill_success", var_0 );
...
should_hide_buried_mother_corpse( var_0, var_1, var_2 )
{
    var_3 = var_0.victim;
...
{
    self endon( "death" );
    self.owner scripts\engine\utility::_id_12900( "death_or_disconnect", "last_stand_finished", "last_stand_heal_active" );
    self delete();
}
...
revivetriggerthink( var_0 )
{
    ...
    for (;;)
    {
        ...
        if ( istrue( var_1.para_groups ) )
        {
            ...
            while ( istrue( var_1.para_groups ) && var_1 usebuttonpressed() )
                waitframe();
        }
        ...
        self.owner scripts\mp\playerlogic::_id_11A67( 1 );
        var_2 = 0;
        ...
        var_1 thread _id_120E3( self.owner );

        if ( level.gametype == "br" )
            self.owner scripts\mp\gametypes\br_public.gsc::_id_11AB0( 1 );

        var_3 = scripts\engine\utility::_id_1290F( "use_hold_revive_success", "use_hold_revive_fail", "death_or_disconnect" );

        if ( var_3 == "use_hold_revive_success" )
            var_2 = 1;

        var_1 notify( "finish_buddy_reviving" );
        self.owner scripts\mp\playerlogic::_id_11A67( 0 );
        var_2 = self.owner hostvictimdamagefactormedium( var_3, var_1 );
...
_id_120E3( var_0 )
{
    ...
    if ( istrue( var_1._id_128E8 ) )
        return;

    var_2 = makeweapon( "teammate_revive_stim_mp" );
    var_3 = var_1 scripts\cp_mp\utility\killstreak_utility::createstreakinfo( "", var_1 );
    var_3.wait_until_done = var_2;

    for ( var_4 = 0; !var_4 || istrue( var_1.brmini_initdialog ); var_1._id_128E8 = 1 )
    {
        var_4 = var_1 scripts\cp_mp\killstreaks\killstreakdeploy::switchtodeployweapon( var_2, var_3, ::_id_121A1, undefined, ::showdangercircle, undefined, ::showdebugresult, 0 );
        waitframe();
    }

    var_1.brmini_initdialog = 1;
    var_1._id_128E8 = 0;
    var_1 thread _id_129E1();
    var_1 thread _id_129FC( var_0 );
    var_1 thread _id_129FD( var_0 );
}

showdangercircle( var_0, var_1 )
{
    var_2 = self;
    var_2 disableweaponswitch();
}

showdebugresult( var_0, var_1 )
{
    var_2 = self;
...
_id_121A1( var_0 )
{
    ...
    if ( !istrue( var_1.wait_until_emp_drone_done ) )
        return;

    var_1 scripts\engine\utility::_id_1290F( "death_or_disconnect", "finish_buddy_reviving", "last_stand_start" );
}
...
hostvictimdamagefactormedium( var_0, var_1 )
{
    var_2 = self;
    var_3 = 0;
    ...
    var_2.player_unresolved_collision_suspend = 0;

    if ( var_0 == "self_revive_success" )
        var_2 selfrevivingdoneanimevent();

    if ( level.gametype == "br" )
        var_2 scripts\mp\gametypes\br_public.gsc::_id_11AB0( 0 );
    ...
    if ( var_3 )
    {
        ...
        if ( level.gametype == "br" )
        {
            if ( !istrue( level.gameended ) )
            {
                var_2 scripts\mp\gametypes\br_public.gsc::_id_11AB2( 0 );
                scripts\mp\events::revivedplayer( var_1, var_2 );
                scripts\cp\agents\gametype_cp_specops::should_play_player_infil( var_1, var_2 );
                var_2 scripts\cp\agents\gametype_cp_specops::_id_11FAA( "alive_not_downed" );

                if ( var_1 != var_2 )
                    var_1 scripts\mp\utility\stats::incpersstat( "rescues", 1 );

                var_2.f14_3pastcodes = 0;
    ...
    var_2 _id_12139();
    var_2 thread destructiblevehiclesetup();
    return var_3;
}
...
decayreviveprogress()
{
    ...
    level endon( "game_ended" );
    var_0 = level.prematchplayedwelcomevo;
...
useholdthinkcleanup( var_0, var_1 )
{
    ...
    var_3 = self.owner;
    var_4 = var_3 scripts\engine\utility::_id_12910( "death_or_disconnect", "use_hold_think_success", "use_hold_think_fail", "last_stand_finished" );
    self.inuse = 0;
    var_1 delete();
    var_0 buddyrevivingdoneanimevent();
    var_0 setlaststandreviving( 0 );
    var_0.wait_until_emp_drone_done = 0;
...
suicidesetup()
{
    ...
    for (;;)
    {
        ...
        if ( self stancebuttonpressed() && self isinexecutionvictim() == 0 && !scripts\mp\playerlogic::jugg_enter_combat_callback() )
        {
...
_id_12099()
{
    ...
    while ( !var_4 && !var_1 )
    {
        ...
        var_5 = scripts\engine\utility::_id_12917( 0.5, "stance_pressed_down", "stance_pressed_up", "last_stand_finished", "last_stand_self_revive", "last_stand_bleedout" );
...
handlerevivemessage()
{
    ...
    for (;;)
    {
        ...
        for (;;)
        {
            var_2 = scripts\engine\utility::_id_1290E( "super_ready", "handle_revive_message" );
...
laststandmonitor()
{
    ...
    if ( istrue( level.prematchinitloadouts ) )
        return;
