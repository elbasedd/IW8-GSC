...
{
    var_2 = playerplunderdepositcallback( var_0 );
    ...
    if ( isdefined( level.didgasmaskpipschange ) )
        self [[ level.didgasmaskpipschange ]]();
    ...
    if ( istrue( self.spawnboardroom_gasmask ) )
    {
        scripts\mp\playeractions::allowactionset( "fakeJugg", 1 );
        self.spawnboardroom_gasmask = undefined;
    }
    ...
    else
    {
        ...
        foreach ( var_11 in var_9 )
        {
            ...
            if ( playerparachutedetachresetomnvars() )
            {
                self givemaxammo( var_11 );
                thread playerplunderkioskpurchase( var_11 );
            }
        }
    }
    ...
    thread playerpowersaddhudelem();
    thread jugg_watchmusictoggle();
    thread jugg_watchfordeath();
    thread jugg_watchforgameend();
    thread jugg_watchfordisconnect();
    thread playerpowerssetupkeybindings();
    thread playerpowersupdateongamepadchange();

    if ( playerpackdataintoomnvar() )
    {
        self skydive_setbasejumpingstatus( 1 );
        self skydive_setdeploymentstatus( 1 );
    }

    if ( isdefined( level.allowattackfromexposednonode ) )
        self [[ level.allowattackfromexposednonode ]]();
...
jugg_createconfig( var_0, var_1 )
{
    ...
    var_2.required_tank_capacity = 5;

    if ( level.gametype == "br" )
        var_2.required_tank_capacity = 3;
...
playerpowersaddhudelem()
{
    if ( playerpackdataintogulagomnvar() )
    {
        ...
        if ( var_0 )
        {
            ...
            if ( playerplundereventomnvars( var_0 ) )
                var_1 = "flames";
...
jugg_watchmusictoggle()
{
    ...
    if ( isdefined( level.checkbluemassacre ) && isdefined( level.checkbluemassacre.name ) && getdvarint( "scr_br_alt_mode_zxp", 0 ) )
        return;
...
jugg_getjuggmodels()
{
    ...
    if ( playerpackdataintogulagomnvar() )
    {
        ...
        if ( var_1 )
        {
            if ( playerplundereventomnvars( var_1 ) )
            {
...
playerplunderdepositcallback( var_0 )
{
    ...
    if ( var_2 != "stand" )
    {
        ...
        else if ( var_2 == "crouch" )
        {
            if ( !var_3 )
            {
                var_6 = scripts\engine\trace::ray_trace_passed( self.origin, scripts\mp\utility\player::lb_mg_impulse_dmg_factor_low( "stand" ), self, var_5 );
        ...
        else if ( var_2 == "prone" )
        {
            if ( !var_4 )
            {
                if ( var_3 )
                {
                    var_7 = scripts\engine\trace::ray_trace_passed( self.origin, scripts\mp\utility\player::lb_mg_impulse_dmg_factor_low( "crouch" ), self, var_5 );
...
jugg_watchoverlaydamagestates( var_0 )
{
    ...
    for (;;)
    {
        scripts\engine\utility::_id_128FF( "damage", "jugg_health_regen" );
...
{
    return istrue( self.juggcontext.juggconfig._id_12661 );
}

playerplaydepositanim()
{
    return istrue( self.juggcontext.juggconfig._id_12671 );
}
...
playerplunderkioskpurchase( var_0 )
{
    ...
    thread playerpowerswaittillinputreturn( var_0, var_1 );
    thread playerpowerscleanuppowers( var_0, var_1 );
...
playerpowerswaittillinputreturn( var_0, var_1 )
{
    self endon( "death_or_disconnect" );
    self endon( "juggernaut_end" );
    level endon( "game_ended" );
...
playerpowerscleanuppowers( var_0, var_1 )
{
    self endon( "death_or_disconnect" );
    self endon( "juggernaut_end" );
    level endon( "game_ended" );
...
playernumlivesvo()
{
    ...
    thread playerpowerstartcooldown();
}
...
playerplunderlosedepositcallback( var_0, var_1 )
{
    ...
    {
        var_4 = 7;
        var_5 = var_3.required_tank_capacity;
    }
...
playerplundereventomnvars( var_0 )
{
    return istrue( level.lootcontentsadjusteconomy_toptier ) && var_0 == 1;
}
