...
{
    if ( var_0 == level_respawn_func() && isdefined( level._id_12B58 ) )
        return 1;
...
nextroundisfinalround()
{
    ...
    {
        if ( level playerhumanconcusspushcleanup( var_2 ) )
            continue;
...
waslastround()
{
    ...
    if ( istrue( level.enemy_spawners ) )
        return 1;
...
{
    return level.systemlink || privatematch() && getdvarint( "NQORMNOQQM", 0 ) || _func_421();
}
...
isanymlgmatch()
{
    ...
    return getdvarint( "LOMTKQTRTM" ) || istrue( level.custom_shouldtakedamage );
}

ismlgsystemlink()
{
    if ( level.systemlink && ( getdvarint( "LOMTKQTRTM" ) || istrue( level.custom_shouldtakedamage ) ) )
        return 1;
...
ismlgprivatematch()
{
    if ( privatematch() && ( getdvarint( "LOMTKQTRTM" ) || istrue( level.custom_shouldtakedamage ) ) )
        return 1;
...
{
    var_0 = getgametype();
    var_1 = lb_mg_impulse_dmg_threshold_top();
    return var_0 != var_1;
}

playerenemypool()
{
    if ( lb_mg_impulse_dmg_threshold_top() == "br" )
        return 1;
    else if ( lb_mg_impulse_dmg_threshold_top() == "jugg" )
        return 1;
...
ks_circleminimapradius()
{
    level.skipburndownlow = 5;
    return level.skipburndownlow;
}

ks_circlemoving()
{
    if ( !isdefined( level.skipcorpse ) )
        level.skipcorpse = getdvarint( "scr_br_payload_retreat_time", 30 );

    return level.skipcorpse;
}

ks_circlemovedist()
{
    if ( !isdefined( level.skipburndownmedium ) )
        level.skipburndownmedium = getdvarint( "scr_br_payload_redraw_time", 10 );

    return level.skipburndownmedium;
}
...
rear_spotlight_target_found_dist_sq( var_0, var_1, var_2, var_3, var_4 )
{
...
player_near_obit()
{
    return istrue( level.player_name_who_broke_stealth );
}

play_vo_internal()
{
    return istrue( level.play_vo_or_timeout ) || istrue( level.playac130animinternal ) || istrue( level.play_vo_on_intel_pickup );
}
...
create_silencer_pick_up( var_0 )
{
    ...
    if ( ( getgametype() == "arm" || play_vo_internal() ) && playergetspectatingplayer() || getdvarint( "scr_game_forceEnableParachuteCut", 0 ) == 1 )
    {
        if ( var_3 != "mp_aniyah" )
        {
            ...
            if ( istrue( level.soldier_agent_lwfn7 ) && !istrue( var_0.get_defuse_time ) )
                var_2 = 1;
        }
    }
...
deletescriptableinstanceaftertime()
{
    ...
    else if ( isdefined( level.longgulagstream ) && level.longgulagstream > 0 )
        return 1;
