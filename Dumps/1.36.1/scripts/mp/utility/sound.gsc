...
{
    if ( istrue( level._id_11C57 ) || var_0 == "MOD_EXECUTION" )
        return;
    ...
    else if ( istrue( self.iszombie ) || istrue( level.lootcontentsadjusteconomy_toptier ) && level.gametype == "infect" && self.team == "axis" )
...
addspecialistbonus()
{
    if ( !isarray( level.add_to_spotlight_array ) )
        level.add_to_spotlight_array = [];

    if ( !isent( level.add_to_spawnflags ) )
        level.add_to_spawnflags = spawn( "sound_transient_soundbanks", ( 0, 0, 0 ) );
}

ambush_lmg_guy( var_0 )
{
    addspecialistbonus();

    if ( isdefined( level.add_to_spotlight_array[var_0] ) )
        return;

    level.add_to_spawnflags settransientsoundbank( var_0 + ".all", 1 );
    level.add_to_spotlight_array[var_0] = level.add_to_spotlight_array.size;
    return;
}

vehicle_spawn_abandonedtimeout( var_0 )
{
    addspecialistbonus();

    if ( isdefined( level.add_to_spotlight_array[var_0] ) )
    {
        level.add_to_spawnflags settransientsoundbank( var_0 + ".all", 0 );
        level.add_to_spotlight_array[var_0] = undefined;
    }
    else
    {
