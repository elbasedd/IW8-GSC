...
init()
{
    var_0 = scripts\mp\utility\dvars::last_exfil_nag( "scr_" + scripts\mp\utility\game::getgametype() + "_healthregentime", "scr_player_healthregentime" );
...
healhregenthink( var_0 )
{
    ...
    if ( var_2 < 0.55 && !scripts\mp\utility\player::isusingremote() && !istrue( self.mp_rust_patch ) )
        onenterdeathsdoor();
    ...
    for (;;)
    {
        ...
        if ( isdefined( level.use_alt_computer_controls ) )
            var_7 = [[ level.use_alt_computer_controls ]]( var_7 );
    ...
    if ( var_6 )
    {
        ...
        scripts\cp\agents\gametype_cp_specops::vehicle_tracker( "t9_ch_global_stim_shot_health_recovery_for_operator_mission", var_10 );
        scripts\cp\agents\gametype_cp_specops::vehicle_tracker( "t9_ch_global_stim_shot_health_recovery_for_operator_mission_s3", var_10 );
        scripts\cp\agents\gametype_cp_specops::show_on_minimap( var_10 );
        scripts\mp\analyticslog::logevent_playerhealed( self, var_10, self );
    }
    else if ( !isai( self ) )
        scripts\mp\analyticslog::recentghostridekillcount( self, var_10 );
...
waitforhealthregendelay()
{
    ...
    while ( self.currentregendelay - self.regendelayreduction > 0 )
    {
        ...
        if ( isdefined( level.use_aitype ) )
            var_5 = [[ level.use_aitype ]]( var_5 );
...
applyshrapnelfxinternal()
{
    ...
    level endon( "game_ended" );
    var_0 = scripts\engine\utility::_id_12908( "shrapnel_ended_early", "shrapnel_ended", "force_regeneration" );
