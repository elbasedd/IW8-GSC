...
init()
{
    ...
    if ( !isdefined( level.delete_headicon_on_death ) )
        level.delete_headicon_on_death = ::choppersupport_movetolocation;
    ...
    level.heli_structs_entrances = scripts\cp_mp\utility\game_utility::kiosksearchradiusidealmin( "ks_heli_entrance" );
    level.heli_structs_goals = scripts\cp_mp\utility\game_utility::kiosksearchradiusidealmin( "ks_heli_goal" );
    level.heli_structs_paths = scripts\cp_mp\utility\game_utility::kiosksearchradiusidealmin( "ks_heli_path" );
...
spawnchopper( var_0, var_1 )
{
    ...
    var_22.dropcount = var_2[2] - 750;
    var_22._id_11EDC = 50;
    var_22._id_11EDD = 25;
    var_22._id_11EDE = undefined;
    var_22.rodwatcher = 7;
    var_22.role_edit = 12;
...
choppersupport_monitorowner()
{
    ...
    self.owner scripts\engine\utility::_id_128FF( "joined_team", "disconnect" );
    thread choppersupport_leave();
}
...
choppersupport_engageturrettarget( var_0 )
{
    ...
    for (;;)
    {
        if ( !istrue( var_0.turreton ) || istrue( var_0._id_124BC ) )
        {
            waitframe();
            continue;
        }
...
choppersupport_acquireturrettarget( var_0, var_1 )
{
    ...
    if ( isdefined( var_4 ) )
    {
        ...
        if ( istrue( var_5 ) && self.currenttarget == var_4 )
            self thread [[ level.delete_headicon_on_death ]]( var_4, 1 );
...
choppersupport_clearcurrenttarget( var_0 )
{
    ...
    if ( istrue( var_0._id_12173 ) )
        var_0._id_12173 = undefined;
...
choppersupport_fireonturrettarget( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    thread delete_furthest_respawn_enemy( var_0 );
    thread choppersupport_watchtargetlos( var_0, var_2 );
    thread choppersupport_watchtargettimeout( var_0 );
    thread delete_exfil_ai_structs( var_0 );
    var_5 = weaponfiretime( "chopper_support_turret_mp" );
    var_6 = 0;
    var_7 = 100;

    if ( isdefined( self._id_11EDC ) )
        var_7 = self._id_11EDC;

    var_8 = 20;

    if ( isdefined( self.rodwatcher ) )
        var_8 = self.rodwatcher;

    var_9 = 40;

    if ( isdefined( self.role_edit ) )
        var_9 = self.role_edit;

    if ( istrue( var_4 ) )
    {
        var_10 = 3750;

        while ( istrue( delete_enemies_if_reaching_max_ai( var_0, var_1 ) ) )
        {
    ...
    while ( istrue( delete_enemies_if_reaching_max_ai( var_0, var_1 ) ) )
    {
        if ( istrue( var_0._id_12173 ) )
        {
            if ( isdefined( self.currenttarget ) && var_1 == self.currenttarget )
                self [[ level.delete_headicon_on_death ]]( self.currenttarget, 1 );
            else
        ...
        else if ( var_0 choppersupport_turretlookingattarget() )
        {
            ...
            if ( var_6 == var_8 )
            {
                ...
                if ( isdefined( self._id_11EDD ) )
                    var_7 = self._id_11EDD;
            }
            else if ( var_6 == var_9 )
            {
                var_7 = undefined;

                if ( isdefined( self._id_11EDE ) )
                    var_7 = self._id_11EDE;
            }
...
delete_furthest_respawn_enemy( var_0 )
{
    self endon( "leaving" );
    self endon( "explode" );
    self endon( "death" );
    ...
    if ( !scripts\cp_mp\utility\game_utility::playedonesplash() )
        return;
    ...
    for (;;)
    {
        if ( isdefined( var_0.attackingtarget ) )
        {
            ...
            {
                if ( !istrue( self.molotov_delete_pool_by_id ) )
                    self.molotov_delete_pool_by_id = 1;
...
choppersupport_watchtargetlos( var_0, var_1 )
{
    ...
    for (;;)
    {
        if ( !istrue( var_0._id_12173 ) && isdefined( var_0.attackingtarget ) )
        {
...
choppersupport_patrolfield( var_0 )
{
    ...
    if ( isdefined( self.spawn_carriables_from_prefabs_all ) )
        self [[ self.spawn_carriables_from_prefabs_all ]]( var_0 );
    else
    {
        if ( self.currentaction != "patrol" )
        ...
        for (;;)
        {
            ...
            if ( isdefined( var_3 ) )
                self [[ level.delete_headicon_on_death ]]( var_3, 1 );
            else
            {
                var_5 = [];
                var_6 = ( 0, 0, 0 );
                ...
                if ( isdefined( var_6 ) && var_5.size > 0 )
                {
                    ...
                    self [[ level.delete_headicon_on_death ]]( var_11 );
                }
            }
...
choppersupport_movetolocation( var_0, var_1 )
{
    ...
    for (;;)
    {
        ...
        scripts\engine\utility::_id_128FF( "near_goal", "begin_evasive_maneuvers" );
...
canqueuearmorbydefault( var_0, var_1 )
{
    self endon( "death" );
    self endon( "leaving" );
    self endon( "crashing" );
...
choppersupport_cleanup( var_0 )
{
    ...
    if ( isdefined( self.destroypropspecatehud ) )
        self [[ self.destroypropspecatehud ]]();

    if ( isdefined( self.streakinfo ) )
        self.streakinfo.healdamage = istrue( var_0 );
    ...
    for (;;)
    {
        if ( istrue( self.molotov_delete_pool_by_id ) )
        {
            ...
            for (;;)
            {
                ...
                {
                    self.molotov_delete_pool_by_id = undefined;
...
choppersupport_gettargets( var_0, var_1, var_2, var_3 )
{
    ...
    for ( var_7 = 0; var_7 < var_5.size; var_7++ )
    {
        ...
        if ( istrue( var_10 ) )
        {
            var_13 = spawnstruct();
            var_13.player = var_8;
            var_13.x1opstransition = var_11;
            var_13._id_12187 = var_12;
...
choppersupport_istarget( var_0, var_1, var_2, var_3 )
{
    ...
    if ( isdefined( self.playerconnectwatcher ) && ![[ self.playerconnectwatcher ]]( var_1 ) )
        return [ 0, 0, 0 ];
...
choppersupport_getbesttarget( var_0, var_1 )
{
    ...
    {
        if ( !delete_ent( var_7.player ) )
            continue;

        if ( choppersupport_isactivetarget( var_7.player ) && !istrue( var_7._id_12187 ) )
            continue;
        ...
        if ( istrue( var_7.x1opstransition ) )
        {
            var_8 = 1;
            var_10 = var_10 + 40;
        }

        if ( istrue( var_7._id_12187 ) )
        {
            var_9 = 1;
            var_10 = var_10 + 20;
        }
...
beginevasivemaneuvers()
{
    ...
    var_0 = scripts\engine\utility::_id_12913( 3.0, "death" );
