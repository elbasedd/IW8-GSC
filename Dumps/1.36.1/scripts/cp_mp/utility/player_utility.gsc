...
isinvehicle( var_0 )
{
    ...
    if ( istrue( var_0 ) )
    {
        if ( isdefined( self._id_127E0 ) && isdefined( self._id_127E0.vehiclename ) )
            return 1;
    }
...
watchthermalinputchange()
{
    ...
    for (;;)
    {
        ...
        var_1 = scripts\engine\utility::_id_1290E( "input_type_changed", "thermal_handling_ended" );
...
alternate_breach_anim_func()
{
    if ( !self isdismembermentenabledforplayer() )
        level.teamplacement[self getxuid()] = self;
}

vehicle_occupancy_linktooriginandangles()
{
    level.teamplacement[self getxuid()] = undefined;
}

keep_firing_minigun()
{
    return level.teamplacement;
}

add_object_to_trap_room_ents()
{
    if ( !isdefined( self.support_box_make_entity_usable ) )
        self.support_box_make_entity_usable = 0;

    if ( self.support_box_make_entity_usable == 0 )
        self playerhide();

    self.support_box_make_entity_usable++;
}

add_outline()
{
    if ( self.support_box_make_entity_usable == 1 )
        self playershow();

    self.support_box_make_entity_usable--;

    if ( self.support_box_make_entity_usable == 0 )
        self.support_box_make_entity_usable = undefined;
}

support_box_use_logic()
{
    self.support_box_make_entity_usable = undefined;

    if ( !isdefined( level.targeting ) )
        level.targeting = getdvarint( "scr_playerShowOnReset", 0 );
