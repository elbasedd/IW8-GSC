...
init()
{
    ...
    for (;;)
    {
        ...
        if ( isdefined( var_3 ) && isdefined( var_4 ) && var_4 != "" )
            level.initlocs_radio[var_3] = var_4;
...
kickafkplayer( var_0 )
{
    return level.initlocs_radio[var_0];
}
...
_id_11FB7()
{
    ...
    thread support_update_hint_logic();
}
...
support_update_hint_logic()
{
    ...
    for (;;)
    {
        scripts\engine\utility::_id_12900( "skydive_deployparachute", "skydive_end", "skydive_beginfreefall" );
        thread _id_12464();
    }
}
...
_id_11FA8()
{
    if ( istrue( self.broadcast_health ) )
        return;

    self setscriptablepartstate( "watchVFXPlayer", "bluntWatchOn" );
    self setclientomnvar( "ui_pet_watch_state", 0 );
    self setwristwatchtime( 4, 20, 1 );
    self.broadcast_health = 1;
    thread bronattackerdamagenottracked();
}

bronattackerdamagenottracked()
{
    self endon( "disconnect" );
    self endon( "game_ended" );
    wait 4.2;
    self.broadcast_health = undefined;
...
_id_12464()
{
    ...
    {
        wait 0.2;
        var_2 = self hasweapon( "iw8_acc_weapon_watch_2+iw8_acc_attach_face_digital_ag" ) || self hasweapon( "iw8_acc_weapon_watch_2+iw8_acc_attach_face_digital_ag_big" ) || self hasweapon( "iw8_acc_weapon_watch_2+iw8_acc_attach_face_digital_ag_female" );

        if ( var_2 && ( !isdefined( self._id_11C93 ) || !istrue( self._id_11C93 ) ) )
        {
            if ( var_2 )
            {
                _id_11FB7();
    ...
    {
        wait 0.2;
        var_3 = self hasweapon( "iw8_acc_weapon_watch_2+iw8_acc_attach_face_digital_ag_b" ) || self hasweapon( "iw8_acc_weapon_watch_2+iw8_acc_attach_face_digital_ag_b_big" ) || self hasweapon( "iw8_acc_weapon_watch_2+iw8_acc_attach_face_digital_ag_b_female" );

        if ( var_3 && ( !isdefined( self._id_11C93 ) || !istrue( self._id_11C93 ) ) )
        {
            if ( var_3 )
                _id_11FB8();
        }
...
watchradialgestureactivation( var_0, var_1 )
{
    ...
    if ( var_0 == "radial_menu_selection_gesture" )
    {
        ...
        if ( isdefined( var_2 ) )
        {
            ...
            {
                if ( !isdefined( self._id_11C93 ) )
                    self._id_11C93 = 0;

                self._id_11C93 = !self._id_11C93;
                self setclientomnvar( "ui_smart_watch_interact", self._id_11C93 );
                ...
                if ( isdefined( self.accessorylogic ) )
                {
                    ...
                    else if ( self.accessorylogic == "holo2" )
                    {
                        ...
                        scripts\cp\equipment\cp_adrenaline::_id_12A07();
                }
            }
            else if ( var_2 == "iw8_ges_plyr_gesture023" )
            {
                if ( !isdefined( self._id_129D3 ) )
                    self._id_129D3 = 0;

                self._id_129D3 = !self._id_129D3;
                self setclientomnvar( "ui_smart_watch_check", self._id_129D3 );
...
_id_12A18( var_0 )
{
    ...
    while ( gettime() < var_1 )
    {
        ...
        if ( self isgestureplaying( "iw8_ges_plyr_spray" ) )
        {
            ...
            scripts\cp\agents\gametype_cp_specops::show_label();
            return;
        }
    }
}
