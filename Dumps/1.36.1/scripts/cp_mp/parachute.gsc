...
initparachutedvars()
{
    ...
    level.soldier_agent_lwfn6 = getdvarint( "scr_parachute_autodeploy_cut", 1 );
    level.soldier_agent_lwfn7 = getdvarint( "scr_parachute_cut", 1 );
    level.parachuteinitfinished = 1;
    level.airlock_show_doors = [];
    ...
    if ( !isdefined( level.solutions ) )
        level.solutions = ::solved;

    level._id_11C89 = spawnstruct();
    level._id_11C89._id_12056 = getdvarint( "scr_parachute_hint_enabled", 1 );
    level._id_11C89._id_12B4F = getdvarfloat( "scr_parachute_hint_xyvelscale_high", 0.0 );
    level._id_11C89._id_12B50 = getdvarfloat( "scr_parachute_hint_xyvelscale_low", 0.0 );
    level._id_11C89._id_12B51 = getdvarfloat( "scr_parachute_hint_xyvelscale_maxheight", 0.0 );
    level._id_11C89._id_12B4E = getdvarfloat( "scr_parachute_hint_xylimit", 0.0 );
    level._id_11C89._id_12B60 = getdvarfloat( "scr_parachute_hint_zdrop", 0.0 );
    level._id_11C89.zoffset = getdvarfloat( "scr_parachute_hint_zoffset", 0.0 );
    level._id_11C89._id_12BF7 = getdvarfloat( "scr_parachute_hint_zvelscale", 0.0 );
    level._id_11C89._id_12B61 = getdvarfloat( "scr_parachute_hint_zlimit", 0.0 );
    level._id_11C89.hidequestcirclefromplayer = getdvarfloat( "scr_parachute_hint_falling_xyratio", 0.0 );
}
...
startfreefall( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    ...
    {
        thread init_exit_doors( var_0, var_1, var_2, var_3, var_4, var_5 );
        return;
    }
    ...
    if ( level.gametype == "br" && ( !istrue( var_4 ) || ( getdvar( "scr_br_gametype", "" ) == "dmz" || getdvar( "scr_br_gametype", "" ) == "risk" ) ) && getdvarint( "scr_infil_parachute_vfx", 1 ) )
        thread node_targetname( getdvarint( "scr_bmo_parachuteTouchdownVFX", 1 ) == 1 && ( getdvar( "scr_br_gametype", "" ) == "dmz" || getdvar( "scr_br_gametype", "" ) == "risk" ) );
...
node_targetname( var_0 )
{
    ...
    if ( isdefined( self.operatorcustomization ) && isdefined( self.operatorcustomization.checkformatchend ) )
        var_2 = var_2 + self.operatorcustomization.checkformatchend;
...
jugg_pursue_target()
{
    if ( isdefined( level.colmaps ) )
        return level.colmaps;
...
{
    return isdefined( self.his_completemission ) && ( self.his_completemission == 5 || self.his_completemission == 6 );
}
...
enablemanualpullchute( var_0 )
{
    ...
    if ( isdefined( self ) )
    {
        if ( player_waittilljumpedfromc130() )
            return;
...
startparachute()
{
    ...
    if ( isparachutegametype() )
    {
        if ( isdefined( level.shot_start_offset ) )
            self [[ level.shot_start_offset ]]( self );
    }
}
...
solved()
{
    ...
    {
        if ( isdefined( var_0.his_completemission ) && ( var_0.his_completemission == 1 || var_0.his_completemission == 2 ) )
        {
            var_0 hint_seq_button();
            var_0.his_completemission = 3;
        }
    }
    else if ( level.gametype == "cp_survival" )
    {
        var_0 setclientomnvar( "ui_br_altimeter_state", 0 );
        var_0 hint_seq_button( 1 );
    }
}
...
_id_125E5()
{
    ...
    var_2 = clamp( var_0.origin[2], 0, level._id_11C89._id_12B51 ) / level._id_11C89._id_12B51;
    var_3 = level._id_11C89._id_12B50 + ( level._id_11C89._id_12B4F - level._id_11C89._id_12B50 ) * var_2;
    var_4 = var_1 * ( 1, 1, 0 );
    var_5 = length( var_4 );
    var_6 = var_4 * var_3;
    var_7 = var_5 * var_3;

    if ( var_7 > level._id_11C89._id_12B4E )
        var_6 = var_6 * ( level._id_11C89._id_12B4E / var_7 );

    var_8 = 0;
    var_8 = var_8 - var_0.origin[2] * level._id_11C89._id_12B60;
    var_8 = var_8 + level._id_11C89.zoffset;
    var_8 = var_8 + var_1[2] * level._id_11C89._id_12BF7;
    var_8 = clamp( var_8, -1 * level._id_11C89._id_12B61, 0 );
    var_9 = 1;

    if ( var_5 < level._id_11C89.hidequestcirclefromplayer * var_1[2] )
        var_9 = 2;

    var_0 setadditionalstreampos( var_0.origin + ( var_6[0], var_6[1], var_8 ), 0, var_9 );
}

init_exit_doors( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    self endon( "disconnect" );
    level endon( "game_ended" );
    ...
    var_6 = self;
    var_6.his_completemission = 1;
    var_6.hint_outline_target_think = istrue( var_4 );
    var_6.hint_target_think = 0;
    var_6 hint_obj_name( var_0, var_1, var_2, var_3, var_5 );
    var_6 hinttext();
    var_7 = gettime();

    for (;;)
    {
        if ( var_6 isskydiving() || var_7 + 2000 < gettime() || var_6 player_waittilljumpedfromc130() )
            break;
    ...
    for (;;)
    {
        if ( level._id_11C89._id_12056 && var_6.hint_target_think < gettime() )
        {
            var_6 _id_125E5();
            var_6.hint_target_think = gettime() + 500;
        }

        if ( var_6 isinfreefall() && var_6.his_completemission != 1 )
        {
            var_6 hinttext();
            var_6.his_completemission = 1;
        }

        if ( var_6 isparachuting() && var_6.his_completemission != 2 )
        {
            var_6 hints_vo_audio();
            var_6.his_completemission = 2;
        }

        if ( !var_6 isskydiving() && var_6.his_completemission != 3 && var_6.his_completemission != 4 && var_6.his_completemission != 6 || var_6.his_completemission == 5 )
        {
            var_6 hint_seq_button();

            if ( var_6.his_completemission != 5 )
                var_6.his_completemission = 3;
            else
                var_6.his_completemission = 6;
        }

        var_8 = istrue( var_6.inlaststand );
        var_9 = var_6 isonground() && ( var_6.his_completemission == 3 || var_6 player_waittilljumpedfromc130() );
        ...
        {
            var_6 hintmdl();
            var_6.his_completemission = undefined;
            var_6.hint_outline_target_think = undefined;
            var_6.hint_target_think = undefined;
...
hint_obj_name( var_0, var_1, var_2, var_3, var_4 )
{
    self skydive_interrupt();
    ...
    if ( level.gametype == "br" && ( !istrue( self.hint_outline_target_think ) || getdvar( "scr_br_gametype", "" ) == "dmz" ) || getdvar( "scr_br_gametype", "" ) == "risk" )
        thread node_targetname( getdvarint( "scr_bmo_parachuteTouchdownVFX", 1 ) == 1 && ( getdvar( "scr_br_gametype", "" ) == "dmz" || getdvar( "scr_br_gametype", "" ) == "risk" ) );
...
hinttext()
{
    ...
    {
        self setclientomnvar( "ui_br_altimeter_state", 1 );
        level.airlock_show_doors[self getxuid()] = self;
    }

    if ( !self.hint_outline_target_think )
    {
        var_0 = "enabled";

        if ( isdefined( self.operatorcustomization ) && isdefined( self.operatorcustomization.checkformatchend ) )
            var_0 = var_0 + self.operatorcustomization.checkformatchend;
    }
}
...
hints_vo_audio()
{
    ...
    if ( isdefined( self.playersetignoreattractions ) && self.playersetignoreattractions == 1 && var_0 != "truckwar" && var_0 != "reveal" && var_0 != "brdov" )
    {
        ...
        self.playersetignoreattractions = 0;
    }
    ...
    if ( isdefined( level.should_take_damage_from_trigger_hurt ) )
        self [[ level.should_take_damage_from_trigger_hurt ]]();
}

hint_seq_button( var_0 )
{
    self.ignorefalldamagetime = gettime() + 5000;
    self.plunder_repositoryatcapacity = gettime();
    ...
    level.airlock_show_doors[self getxuid()] = undefined;
    self notify( "parachute_complete" );
    self animscriptsetinputparamreplicationstatus( 0 );
}
