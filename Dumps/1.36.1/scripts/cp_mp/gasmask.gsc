...
init( var_0, var_1 )
{
    ...
    if ( var_2 )
        var_3 = ks_circlecount( var_1 );
    else
        var_3 = 180;

    if ( !isdefined( level.initbunkerdoor ) )
    {
        if ( var_2 )
            level.initbunkerdoor = getdvarfloat( "scr_br_gasMask_resist", 0 );
        else
            level.initbunkerdoor = 0.2;
    }

    self.gasmaskhealth = var_3;
    self.initcallbacks = var_3;
    self.initcircuitbreakers = var_1;
    var_4 = 2;

    if ( play_random_sound_event( var_1 ) )
        var_4 = 3;
    ...
    if ( var_2 )
        level.initbunkervfx = self.gasmaskhealth / 6;

    level.initcallbacks = var_3;
...
{
    if ( play_random_sound_event( var_0 ) )
...
processdamage( var_0 )
{
    ...
    var_2 = self.gasmaskhealth / self.initcallbacks;
    self.gasmaskhealth = self.gasmaskhealth - var_0;
    self.gasmaskhealth = max( 0, self.gasmaskhealth );

    if ( var_1 )
        var_3 = ( self.gasmaskhealth + 1 ) / self.initcallbacks;
    else
        var_3 = self.gasmaskhealth / self.initcallbacks;
    ...
    self.gasdamagebuffer = self.gasdamagebuffer + var_0 * level.initbunkerdoor;
...
get_any_player_spectating( var_0, var_1 )
{
    ...
    if ( !isdefined( self.initcallbacks ) )
        self.initcallbacks = ks_circlecount( self.initcircuitbreakers );

    var_3 = var_0 / self.initcallbacks;
    var_4 = var_1 / self.initcallbacks;
    var_5 = floor( var_3 * 6 + 0.5 );
...
equipgasmask()
{
    ...
    if ( istrue( self.operatorcustomization.model_parts ) )
...
helis_assault3_hangar_check_size( var_0 )
{
    self.gasmaskoverlay.alpha = 1.0;
    self.gasmaskoverlay fadeovertime( var_0 );
    self.gasmaskoverlay.alpha = 0.0;
