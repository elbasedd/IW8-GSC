...
init()
{
    level.bomb_carrier = scripts\engine\utility::getstructarray( "ascend_solo_begin", "script_noteworthy" );
    level.gamemodeoverridemeleeviewkickscale = scripts\engine\utility::getstructarray( "descend_solo_begin", "script_noteworthy" );

    foreach ( var_1 in level.bomb_carrier )
        scripts\cp_mp\auto_ascender::markupascenderstruct( var_1, 1 );

    foreach ( var_4 in level.gamemodeoverridemeleeviewkickscale )
    ...
    foreach ( var_1 in level.bomb_carrier )
    {
        foreach ( var_4 in level.gamemodeoverridemeleeviewkickscale )
        {
            if ( !isdefined( var_4._id_11CD8 ) && distance2dsquared( var_1.origin, var_4.origin ) < 10 )
            {
                var_1._id_11CD8 = var_4;
                var_4._id_11CD8 = var_1;
                break;
            }
        }
    }
    ...
    initanimtree();
    scripts\engine\scriptable::winindex( "ascender_solo", ::boltunlink );
}
...
boltunlink( var_0, var_1, var_2, var_3, var_4 )
{
    if ( var_2 == "on" )
    {
...
startascenderanim( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    if ( var_4 > 0 )
        thread _id_11E81( self.angles + ( 0, var_4 * -1, 0 ) );
...
ascenderuse( var_0, var_1 )
{
    ...
    if ( !scripts\cp_mp\auto_ascender::create_solution_digit_mark( var_0, var_1 ) )
        return;
    ...
    var_1.boltdeleteonnote = var_12;
    var_1.boltdeletethread = var_13;
    ...
    if ( !var_2.dir )
    {
        if ( scripts\common\utility::iscp() )
            var_15 = getdvarfloat( "scr_descender_speed_cp", scripts\cp_mp\auto_ascender::jeep_initdamage() );
        else
            var_15 = getdvarfloat( "scr_descender_speed", scripts\cp_mp\auto_ascender::jeep_initcollision() );
    }
    else
        var_15 = getdvarfloat( "scr_ascender_speed", scripts\cp_mp\auto_ascender::jeep() );

    var_16 = var_14 / var_15;
    var_17 = scripts\cp_mp\auto_ascender::javelin_interact_monitor() * var_16;
    var_18 = scripts\cp_mp\auto_ascender::jeep_horn() * var_16;
...
player_isusingtacmap( var_0 )
{
    ...
    {
        if ( distance2dsquared( var_2.origin, var_0 ) < scripts\cp_mp\auto_ascender::joininprogresstimeout() )
        {
            if ( isdefined( var_2._id_11CD8 ) )
            {
                if ( abs( var_0[2] - var_2.origin[2] ) < scripts\cp_mp\auto_ascender::jeep_initomnvars() || abs( var_0[2] - var_2._id_11CD8.origin[2] ) < scripts\cp_mp\auto_ascender::jeep_initomnvars() )
                    return 1;
            }
        }
    }
