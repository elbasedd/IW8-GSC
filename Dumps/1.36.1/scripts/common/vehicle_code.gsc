...
vehicle_killriders()
{
    ...
    foreach ( var_1 in self.riders )
    {
        ...
        if ( isdefined( var_1._blackboard ) && isdefined( var_1._blackboard.chosenvehicleanimpos ) && istrue( var_1._blackboard.chosenvehicleanimpos.eyeoffnotehandler ) )
        {
            var_2 = 100.0;
        ...
        if ( scripts\common\utility::iscp() && isdefined( self.expected_card ) )
        {
            if ( isdefined( self.expected_card.attacker ) && isplayer( self.expected_card.attacker ) )
                var_1 dodamage( var_1.maxhealth, self.expected_card.attacker.origin, self.expected_card.attacker, self.expected_card.attacker, self.expected_card.meansofdeath, self.expected_card.objweapon );
            else
                var_1 kill();
    ...
    if ( isdefined( var_5 ) )
    {
        ...
        foreach ( var_11 in self.riders )
        {
            if ( isalive( var_11 ) && isdefined( var_6[var_11.vehicle_position] ) )
            {
                ...
                if ( isdefined( var_8 ) && istrue( var_8.get_random_impact_point ) )
                    continue;
...
death_firesound( var_0 )
{
    ...
    scripts\engine\utility::_id_128FF( "fire_extinguish", "stop_crash_loop_sound" );
    ...
    while ( self.health > 0 )
    {
        ...
        if ( isdefined( var_3 ) )
        {
            ...
            if ( scripts\engine\utility::func_ref_exist( "vehicle_damage_modifier" ) )
            {
                ...
                if ( isdefined( level.impulsefx ) )
                    var_15 = [[ level.impulsefx ]]( var_3, self, var_2, var_11, var_6, undefined, var_5, var_4, var_7, var_9, var_8, var_10 );

                if ( isdefined( var_15 ) )
                    self.expected_card = var_15;
                else
                    self.expected_card = undefined;
...
helicopter_crash_move( var_0, var_1 )
{
    ...
    if ( var_5 )
    {
        ...
        scripts\engine\utility::_id_128FF( "goal", "near_goal" );
    ...
    else
    {
        ...
        while ( var_8 != "death" )
        {
            var_8 = scripts\engine\utility::_id_12900( "goal", "near_goal", "death" );
