// IW8 GSC SOURCE
// Generated by https://github.com/xensik/gsc-tool

ownerxuid( var_0 )
{
    if ( !isdefined( level._id_12B49 ) )
        level._id_12B49 = [];
}

_id_12617( var_0 )
{
    self endon( "death_or_disconnect" );
    self endon( "faux_spawn" );
    self endon( "end_launcher" );
    var_1 = level_logic( var_0 );

    if ( var_1 == "mag_xmike109" )
        return;

    var_2 = level_offhand_refill_spawn( var_1 );
    thread munitions_override_waittill( var_0, var_1, var_2 );
}

destroy_vehicle_on_pilot_death()
{
    self waittill( "end_launcher" );
    wait 6;
    self notify( "cleanupXMike109ImpactWatcher" );
}

munitions_override_waittill( var_0, var_1, var_2 )
{
    self notify( "cleanupXMike109ImpactWatcher" );
    self endon( "disconnect" );
    self endon( "cleanupXMike109ImpactWatcher" );
    childthread destroy_vehicle_on_pilot_death();

    if ( var_1 == "default" )
        return;

    for (;;)
    {
        self waittill( "bullet_first_impact", var_3, var_4, var_5, var_6, var_7, var_8, var_9, var_10 );

        if ( var_0 != var_7 )
            continue;

        make_all_doors_solid( var_3, var_4, var_10, var_5, var_6, var_7, var_8, var_9, var_2, var_1 );
    }
}

make_all_doors_solid( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9 )
{
    var_10 = _id_11BE0( var_4, var_9, var_0, var_1, var_3, var_6 );
    var_11 = _id_11E42( var_7, var_6, var_9, var_5, var_10, var_3 );

    if ( _id_11BBB( var_0 ) )
    {
        var_11 delete();
        return;
    }
    else if ( _id_11BCE( var_0, var_10 ) )
        linktoent( var_11, var_0, var_1 );

    if ( isdefined( var_8 ) )
        [[ var_8 ]]( var_11, var_0, var_1, var_2, var_4, var_10 );
}

multiplepubliceventsenabled( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    var_6 = 0.1;
    var_0.grenade = magicgrenademanual( "semtex_xmike109_mp", var_0.origin, ( 0, 0, 0 ), var_6 );
    var_0.grenade.angles = var_0.angles;
    var_0.grenade linkto( var_0, "tag_origin" );
    thread trial_target_think_func( var_0, var_6, var_3 );
}

munition_remover( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    thread trial_target_thread_func( var_0, var_1, var_2, var_3, var_5 );
}

munition_slot_gunship_emptied_message( var_0, var_1, var_2, var_3, var_4, var_5 )
{

}

trial_target_think_func( var_0, var_1, var_2 )
{
    self endon( "disconnect" );
    var_0 endon( "entitydeleted" );
    var_0.grenade scripts\engine\utility::_id_12919( var_1, "explode" );
    var_0 setscriptablepartstate( "effects", "explode" );
    var_3 = makeweapon( "semtex_xmike109_mp" );
    var_4 = makeweapon( "semtex_xmike109_splash_mp" );
    var_3.sortbylastzombietime = var_0.weapon;
    var_4.sortbylastzombietime = var_0.weapon;
    glassradiusdamage( var_0.origin, 150, 50, 1 );

    if ( isdefined( var_0.stuckenemyentity ) && isalive( var_0.stuckenemyentity ) )
    {
        var_0.stuckenemyentity scripts\cp_mp\utility\damage_utility::init_cp_execution();
        var_0.stuckenemyentity dodamage( 140, var_0.origin, self, self, "MOD_EXPLOSIVE", var_3, var_2 );
        var_0.stuckenemyentity scripts\cp_mp\utility\damage_utility::init_cp_vehicle();
    }

    getaltbunkerindexforname( var_0.origin, 9, 35, 15, 25, 60, 14, self, "MOD_EXPLOSIVE", var_4 );
    wait 0.4;

    if ( !var_0 _id_1268C() )
        return;

    var_0 delete();
}

trial_target_thread_func( var_0, var_1, var_2, var_3, var_4 )
{
    if ( istrue( var_0.play_usb_pickup_gesture ) )
    {
        var_0 setscriptablepartstate( "effects", "reflectThermite" );
        _id_128DD( var_0 );
    }

    var_0 setscriptablepartstate( "effects", "burn" );
    thread _id_12204( var_0 );
    thread _id_12201( var_0 );
    thread _id_12200( var_0 );
}

_id_128DD( var_0 )
{
    var_0 endon( "stuckWaitTimeout" );
    var_0 thread trial_thermite_watcher();
    var_0 waittill( "missile_stuck", var_1, var_2 );

    if ( isdefined( var_1 ) )
    {
        linktoent( var_0, var_1, var_2 );
        var_0.can_be_seen_by_any_player = 1;
    }
}

trial_thermite_watcher()
{
    wait 3;

    if ( isdefined( self ) )
        self notify( "stuckWaitTimeout" );
}

_id_12204( var_0 )
{
    self endon( "disconnect" );
    var_0 endon( "entitydeleted" );
    var_1 = makeweapon( "thermite_xmike109_mp" );
    var_1.sortbylastzombietime = var_0.weapon;

    if ( isdefined( var_0.stuckenemyentity ) && isalive( var_0.stuckenemyentity ) )
    {
        if ( var_0.stuckenemyentity scripts\cp_mp\vehicles\vehicle::isvehicle() || isdefined( var_0.stuckenemyentity.classname ) && var_0.stuckenemyentity.classname == "misc_turret" )
            var_2 = 0.95;
        else
            var_2 = 0.25;

        if ( istrue( var_0.can_be_seen_by_any_player ) )
            var_0.stuckenemyentity dodamage( 80, var_0.origin, self, var_0, "MOD_FIRE", var_1 );

        var_3 = int( 3.0 / var_2 );

        while ( isdefined( var_0 ) && isdefined( var_0.stuckenemyentity ) && isalive( var_0.stuckenemyentity ) && var_3 >= 0 )
        {
            var_0.stuckenemyentity scripts\cp_mp\utility\damage_utility::init_cp_execution();
            var_0.stuckenemyentity dodamage( 3, var_0.origin, self, var_0, "MOD_FIRE", var_1 );
            var_0.stuckenemyentity scripts\cp_mp\utility\damage_utility::init_cp_vehicle();
            var_3--;
            wait( var_2 );
        }
    }
}

_id_12201( var_0 )
{
    self endon( "disconnect" );
    var_0 endon( "entitydeleted" );
    var_1 = int( 12.0 );
    var_2 = makeweapon( "thermite_xmike109_radius_mp" );
    var_2.sortbylastzombietime = var_0.weapon;
    var_0._id_12202 = var_2.basename;

    while ( var_1 > 0 )
    {
        if ( isdefined( var_0.stuckenemyentity ) && isalive( var_0.stuckenemyentity ) )
            var_0.stuckenemyentity scripts\cp_mp\utility\damage_utility::adddamagemodifier( "thermite25mmStuck", 0, 0, ::_id_121F7 );

        var_0 radiusdamage( var_0.origin, 50, 2, 2, self, "MOD_FIRE", var_2 );

        if ( isdefined( var_0.stuckenemyentity ) && isalive( var_0.stuckenemyentity ) )
            var_0.stuckenemyentity scripts\cp_mp\utility\damage_utility::removedamagemodifier( "thermite25mmStuck", 0 );

        var_1--;
        wait 0.25;
    }
}

_id_121F7( var_0, var_1, var_2, var_3, var_4, var_5, var_6 )
{
    if ( !isdefined( var_0 ) )
        return 1;

    if ( !isdefined( var_0._id_12202 ) || var_0._id_12202 != "thermite_xmike109_radius_mp" )
        return 1;

    if ( !isdefined( var_0.stuckenemyentity ) || var_0.stuckenemyentity != var_2 )
        return 1;

    return 0;
}

_id_12200( var_0 )
{
    var_0 endon( "entitydeleted" );
    wait 3.0;

    if ( !var_0 _id_1268C() )
        return;

    var_0 setscriptablepartstate( "effects", "burnout" );
    var_0 setscriptablepartstate( "visibility", "hide" );
    wait( randomfloatrange( 0.3, 2.0 ) );

    if ( !var_0 _id_1268C() )
        return;

    wait( randomfloatrange( 2, 3 ) );
    var_0 delete();
}

_id_11E42( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    if ( var_4 && var_2 == "thermal" )
    {
        var_6 = usbmodel( var_5, var_1, var_0, 1 );
        var_7 = var_0 + var_5 * 10;
        var_8 = magicgrenademanual( "xmike109_grenade", var_7, var_6, 10 );
        var_8.play_usb_pickup_gesture = 1;
        playfx( scripts\engine\utility::getfx( "xmike109ThermiteBounce" ), var_0, var_5 );
    }
    else
    {
        var_8 = spawn( "script_model", var_0 );
        var_8 setmodel( "weapon_wm_sn_xmike109_projectile" );
        var_8.origin = var_0;

        if ( var_4 )
        {
            var_6 = usbmodel( var_5, var_1, var_0, 0 );
            var_8 physicslaunchserver( var_0, var_6 );
        }
    }

    var_8.angles = vectortoangles( var_1 );
    var_8 _id_11A7A( var_2 );
    var_8.owner = self;
    var_8.battle_tracks_createexternallisteningzone = var_2;
    var_8.weapon = var_3;
    var_8.playerhumanconcusspush = 1;
    var_8 thread trial_target_think();
    relic_mythic_should_ai_play_pain( var_8 );
    return var_8;
}

_id_11A7A( var_0 )
{
    switch ( var_0 )
    {
        case "thermal":
            thread _id_11A7B( 3.5 );
            self.fullweaponname = 0;
            break;
        case "explosive":
            self.fullweaponname = 0;
            break;
        default:
            self.fullweaponname = 1;
    }
}

_id_11A7B( var_0 )
{
    self endon( "entitydeleted" );
    wait( var_0 );
    self.fullweaponname = 1;
    relic_mythic_should_ai_play_pain();
}

relic_mythic_should_ai_play_pain( var_0 )
{
    if ( isdefined( var_0 ) )
        var_1 = [ var_0 ];
    else
        var_1 = [];

    foreach ( var_3 in level._id_12B49 )
    {
        if ( !isdefined( var_3 ) )
            continue;

        if ( isdefined( var_3 ) )
        {
            if ( var_1.size >= 24 && var_3.fullweaponname )
            {
                var_3 delete();
                continue;
            }

            var_1[var_1.size] = var_3;
        }
    }

    level._id_12B49 = var_1;
}

level_logic( var_0 )
{
    var_1 = "";

    foreach ( var_3 in var_0.attachments )
    {
        if ( issubstr( var_3, "calcust1" ) )
        {
            var_1 = "calcust1_xmike109";
            break;
        }

        if ( issubstr( var_3, "calcust2" ) )
        {
            var_1 = "calcust2_xmike109";
            break;
        }
    }

    switch ( var_1 )
    {
        case "calcust2_xmike109":
            return "thermal";
        case "calcust1_xmike109":
            return "explosive";
        default:
            return "default";
    }
}

level_offhand_refill_spawn( var_0 )
{
    switch ( var_0 )
    {
        case "thermal":
            return ::munition_remover;
        case "explosive":
            return ::multiplepubliceventsenabled;
        default:
            return ::munition_slot_gunship_emptied_message;
    }
}

_id_11BE0( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    if ( !isdefined( var_0 ) )
        var_0 = "";

    if ( var_0 == "riotshield" )
        return 1;

    if ( var_1 != "thermal" )
        return 0;

    if ( !isdefined( var_2 ) && isdefined( var_3 ) )
        return 1;

    if ( player_standing_on_train( var_2 ) || player_standing_in_doorway( var_2 ) )
        return 1;

    if ( play_smoke_fx( var_2 ) || play_skit_and_watch_for_endons( var_2 ) )
        return 0;

    if ( isdefined( var_2 ) )
    {
        if ( var_2 scripts\cp_mp\vehicles\vehicle::isvehicle() )
            return 0;

        if ( isdefined( var_2.equipmentref ) && var_2.equipmentref == "equip_tac_cover" )
            return 0;
    }

    switch ( var_0 )
    {
        case "glass_solid":
        case "glass_pane":
        case "riotshield":
            return 1;
        default:
            var_6 = abs( vectordot( var_5, var_4 ) );

            if ( var_6 < 0.2 )
            {
                return 1;
                return;
            }

            return 0;
            return;
    }
}

usbmodel( var_0, var_1, var_2, var_3 )
{
    if ( var_3 )
    {
        var_4 = 1500;
        var_5 = 500;
        var_6 = 150;
    }
    else
    {
        var_4 = 500;
        var_5 = 500;
        var_6 = 500;
    }

    var_7 = scripts\engine\math::vector_reflect( var_1, var_0 );
    var_7 = vectorlerp( var_7, var_0, 0.2 );
    var_8 = abs( vectordot( var_1, var_0 ) );

    if ( var_8 < 0.2 )
    {
        var_8 = scripts\engine\math::normalize_value( 0, 0.2, var_8 );
        var_9 = scripts\engine\math::factor_value( var_4, var_5, var_8 );
    }
    else
    {
        var_8 = scripts\engine\math::normalize_value( 0.2, 1, var_8 );
        var_9 = scripts\engine\math::factor_value( var_5, var_6, var_8 );
    }

    var_7 = var_7 * var_9;
    return var_7;
}

_id_11BBB( var_0 )
{
    if ( !isdefined( var_0 ) )
        return 0;

    if ( isagent( var_0 ) && var_0 pitch_max_angles() && !isalive( var_0 ) && !isdefined( var_0 getcorpseentity() ) )
        return 1;

    return 0;
}

_id_11BCE( var_0, var_1 )
{
    if ( var_1 || !isdefined( var_0 ) )
        return 0;

    if ( isdefined( var_0.classname ) && var_0.classname == "weapon_scavenger_bag_mp" )
        return 0;

    return 1;
}

pitch_max_angles()
{
    return istrue( isdefined( self.unittype ) && self.unittype == "suicidebomber" );
}

linktoent( var_0, var_1, var_2 )
{
    if ( ( isplayer( var_1 ) || isagent( var_1 ) ) && !isalive( var_1 ) )
    {
        var_3 = var_1 getcorpseentity();

        if ( isdefined( var_3 ) )
            var_1 = var_3;
    }

    if ( isplayer( var_1 ) )
        var_0 hidefromplayer( var_1 );

    if ( isdefined( var_2 ) )
        var_0 linkto( var_1, var_2 );
    else
        var_0 linkto( var_1 );

    if ( createallhistorydestinations( var_1 ) )
    {
        var_0.stuckenemyentity = var_1;
        var_0 thread vehicle_showvalidlittlebirds( var_1 );
    }

    if ( !istrue( var_0.play_usb_pickup_gesture ) )
        var_0 notsolid();

    var_0 thread trial_targs_combo( var_1 );
    var_0 thread trial_target_requisites( var_1, "vehicle_deleted" );
    var_0 thread trial_target_requisites( var_1, "detonated" );
    var_0 thread trial_target_requisites( var_1, "beginC130" );
}

_id_1268C()
{
    if ( isdefined( self ) && istrue( self.playerhumanconcusspush ) )
        return 1;
    else
    {

    }
}

vehicle_showvalidlittlebirds( var_0 )
{
    self endon( "entitydeleted" );
    var_0 scripts\engine\utility::_id_12900( "entitydeleted", "death", "disconnect" );

    if ( !_id_1268C() )
        return;

    self.stuckenemyentity = undefined;

    if ( isdefined( var_0 ) && isdefined( var_0.nocorpse ) )
        self delete();
}

trial_targs_combo( var_0 )
{
    self endon( "entitydeleted" );

    if ( isagent( var_0 ) )
        var_0 waittill( "entitydeleted" );
    else
        var_0 scripts\engine\utility::_id_128FF( "entitydeleted", "disconnect" );

    if ( !_id_1268C() )
        return;

    trial_targs();
}

trial_targs( var_0 )
{
    if ( !isdefined( var_0 ) )
        var_0 = ( 0, 0, 100 );

    if ( self islinked() )
        self unlink();

    if ( !istrue( self.play_usb_pickup_gesture ) )
        self solid();

    self physicslaunchserver( self.origin, var_0 );
}

trial_target_requisites( var_0, var_1 )
{
    self endon( "entitydeleted" );
    var_0 waittill( var_1 );

    if ( !_id_1268C() )
        return;

    self delete();
}

trial_target_think()
{
    self waittill( "entitydeleted" );

    if ( isdefined( self.grenade ) )
        self.grenade delete();
}

player_standing_on_train( var_0 )
{
    if ( !isplayer( var_0 ) )
        return 0;

    if ( scripts\mp\utility\player::isenemy( var_0 ) )
        return 0;
    else
        return 1;
}

play_smoke_fx( var_0 )
{
    if ( !isplayer( var_0 ) )
        return 0;

    if ( scripts\mp\utility\player::isenemy( var_0 ) )
        return 1;
    else
        return 0;
}

player_standing_in_doorway( var_0 )
{
    if ( !isagent( var_0 ) )
        return 0;

    if ( isdefined( var_0.agentteam ) && self.team == var_0.agentteam )
        return 1;

    return 0;
}

play_skit_and_watch_for_endons( var_0 )
{
    if ( !isagent( var_0 ) )
        return 0;

    if ( isdefined( var_0.agentteam ) && self.team == var_0.agentteam )
        return 0;

    return 1;
}

createallhistorydestinations( var_0 )
{
    var_1 = 0;

    if ( isplayer( var_0 ) || isagent( var_0 ) )
        var_1 = 1;

    if ( var_0 scripts\cp_mp\vehicles\vehicle::isvehicle() )
        var_1 = 1;

    if ( isdefined( var_0.classname ) )
    {
        if ( var_0.classname == "misc_turret" )
            var_1 = 1;

        if ( var_0.classname == "script_model" )
        {
            if ( isdefined( var_0.streakinfo ) && ( var_0.streakinfo.streakname == "uav" || var_0.streakinfo.streakname == "gunship" ) )
                var_1 = 1;
        }
    }

    if ( isdefined( var_0.equipmentref ) )
    {
        if ( var_0.equipmentref == "equip_tac_cover" )
            var_1 = 1;
    }

    return var_1;
}
