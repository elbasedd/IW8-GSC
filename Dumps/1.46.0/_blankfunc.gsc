...
init()
{
    ...
    level.nuke_expl_struct = scripts\cp_mp\utility\game_utility::playerstartbesttimeupdate( "nuke_expl_pos" );
}
...
tryusenukefromstruct( var_0 )
{
    ...
    if ( !isdefined( level.nukeincoming ) )
    {
        ...
        level.weapon_xp_iw8_ar_kilo433 = self;
    }
    else
    {
    ...
    else if ( !istrue( level.validate_station ) )
    ...
    if ( !istrue( var_4 ) || level.gameended )
    {
        ...
        level.weapon_xp_iw8_ar_kilo433 = undefined;
        var_0 notify( "killstreak_finished_with_deploy_weapon" );
        return 0;
    }
    ...
    if ( var_1 != 1 && !istrue( level.validate_station ) )
        scripts\common\utility::_id_13592( level.update_objective_ownerclient, "nuke", self.origin );
...
nuke_start( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7 )
{
    ...
    if ( istrue( level.validate_station ) )
        var_12 = 0.7;
...
{
    if ( !isdefined( level.wavesv_finite_ending ) )
    {
        createheadiconatorigin( "nuke" );
        level.wavesv_finite_ending = 1;
    }

    level endon( "game_ended" );
    var_0 waittill( "disconnect" );
    watchweapondeathordisconnect();
}
...
nuke_starttimer( var_0 )
{
    ...
    if ( istrue( level.validate_station ) )
        _branalytics_addeventallowed::_id_129C4( "ui_nuke_data", 9, 2, 2 );
    else
    ...
    if ( isdefined( level.nukeinfo.player ) )
    {
        ...
        if ( !istrue( level.validate_station ) )
        {
            if ( isdefined( level.nukeinfo.player ) && isdefined( var_1 ) )
                level.nukeinfo.player scripts\cp_mp\utility\killstreak_utility::_id_12356( var_1 );
        }
    }
...
{
    level endon( "game_ended" );
    level thread wave_goto_on_spawn( 1 );
