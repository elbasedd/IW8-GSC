...
init()
{
    ...
    if ( !isdefined( level.get_power_charges_in_slot ) )
        level.get_power_charges_in_slot = ::choppersupport_movetolocation;
    ...
    level.heli_structs_entrances = scripts\cp_mp\utility\game_utility::playerstartjailsetcontrols( "ks_heli_entrance" );
    level.heli_structs_goals = scripts\cp_mp\utility\game_utility::playerstartjailsetcontrols( "ks_heli_goal" );
    level.heli_structs_paths = scripts\cp_mp\utility\game_utility::playerstartjailsetcontrols( "ks_heli_path" );
...
spawnchopper( var_0, var_1 )
{
    ...
    var_22.gunnerdamagemodifier = var_2[2] - 750;
    var_22._id_12F41 = 50;
    var_22._id_12F42 = 25;
    var_22._id_12F43 = undefined;
    var_22.vehicle_damage_getheavystatehealthadd = 7;
    var_22.vehicle_damage_getheavystatemaxhealth = 12;
...
choppersupport_monitorowner()
{
    ...
    self.owner scripts\engine\utility::_id_13AD9( "joined_team", "disconnect" );
    thread choppersupport_leave();
}
...
choppersupport_neargoalsettings()
{
    ...
    thread get_players_inside_the_plane();
...
choppersupport_engageturrettarget( var_0 )
{
    ...
    for (;;)
    {
        if ( !istrue( var_0.turreton ) || istrue( var_0._id_13609 ) )
        {
            waitframe();
            continue;
        }
        ...
        if ( isdefined( var_1 ) && var_1.size > 0 )
        {
            ...
            {
                if ( istrue( self.canseesafecircleui ) && isdefined( self.hint_outline_target_think ) )
                    self thread [[ self.hint_outline_target_think ]]();
                else if ( isdefined( self._id_11B22 ) )
                    self thread [[ self._id_11B22 ]]();
                else
...
choppersupport_acquireturrettarget( var_0, var_1 )
{
    ...
    if ( isdefined( var_4 ) )
    {
        ...
        if ( istrue( var_5 ) && self.currenttarget == var_4 && !istrue( self.canseesafecircleui ) )
            self thread [[ level.get_power_charges_in_slot ]]( var_4, 1 );
...
choppersupport_clearcurrenttarget( var_0 )
{
    ...
    if ( istrue( var_0._id_1322F ) )
        var_0._id_1322F = undefined;
...
choppersupport_fireonturrettarget( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    thread get_players_not_targeted_by_other_battle_station( var_0 );
    thread choppersupport_watchtargetlos( var_0, var_2 );
    thread choppersupport_watchtargettimeout( var_0 );
    thread get_players_in_mortar_range( var_0 );
    var_5 = weaponfiretime( "chopper_support_turret_mp" );
    var_6 = 0;
    var_7 = 100;

    if ( isdefined( self._id_12F41 ) )
        var_7 = self._id_12F41;

    var_8 = 20;

    if ( isdefined( self.vehicle_damage_getheavystatehealthadd ) )
        var_8 = self.vehicle_damage_getheavystatehealthadd;

    var_9 = 40;

    if ( isdefined( self.vehicle_damage_getheavystatemaxhealth ) )
        var_9 = self.vehicle_damage_getheavystatemaxhealth;

    if ( istrue( var_4 ) )
    {
        var_10 = 3750;

        while ( istrue( get_player_who_most_likely_broke_stealth( var_0, var_1 ) ) )
        {
    ...
    thread get_positions_around_vector( var_0 );

    while ( istrue( get_player_who_most_likely_broke_stealth( var_0, var_1 ) ) )
    {
        if ( istrue( var_0._id_1322F ) )
        {
            if ( isdefined( self.currenttarget ) && var_1 == self.currenttarget && !istrue( self.canseesafecircleui ) )
                self [[ level.get_power_charges_in_slot ]]( self.currenttarget, 1 );
            else
        ...
        else if ( var_0 choppersupport_turretlookingattarget() )
        {
            ...
            if ( var_6 == var_8 )
            {
                ...
                if ( isdefined( self._id_12F42 ) )
                    var_7 = self._id_12F42;
            }
            else if ( var_6 == var_9 )
            {
                var_7 = undefined;

                if ( isdefined( self._id_12F43 ) )
                    var_7 = self._id_12F43;
            }
...
get_player_who_most_likely_broke_stealth( var_0, var_1 )
{
...
get_players_not_targeted_by_other_battle_station( var_0 )
{
    ...
    for (;;)
    {
        if ( isdefined( var_0.attackingtarget ) )
        {
            ...
            {
                if ( !istrue( self.rpg_attack_apc ) )
                    self.rpg_attack_apc = 1;
...
get_positions_around_vector( var_0 )
{
    self endon( "leaving" );
    self endon( "explode" );
    self endon( "death" );
...
choppersupport_watchtargetlos( var_0, var_1 )
{
    ...
    for (;;)
    {
        if ( !istrue( var_0._id_1322F ) && isdefined( var_0.attackingtarget ) )
        {
...
get_players_in_mortar_range( var_0 )
{
    self endon( "leaving" );
    self endon( "explode" );
    self endon( "death" );
...
choppersupport_patrolfield( var_0 )
{
    ...
    if ( isdefined( self._id_11B22 ) )
        self [[ self._id_11B22 ]]( var_0 );
    else
    {
        if ( self.currentaction != "patrol" )
        ...
        if ( get_player_who_most_recently_threw_grenade() )
            var_1 = 50;
        ...
        for (;;)
        {
            ...
            if ( isdefined( var_3 ) && !istrue( self.canseesafecircleui ) )
                self [[ level.get_power_charges_in_slot ]]( var_3, 1 );
            else
            {
                var_5 = [];
                var_6 = ( 0, 0, 0 );
                ...
                if ( isdefined( var_6 ) && var_5.size > 0 && !istrue( self.canseesafecircleui ) )
                {
                    var_11 = var_6 / var_5.size;
                    self [[ level.get_power_charges_in_slot ]]( var_11 );
                }
            }
...
create_saw_interaction( var_0, var_1 )
{
    ...
    for (;;)
    {
        ...
        if ( istrue( self.canseesafecircleui ) && isdefined( self.hint_obj_name ) )
            break;
        else
        {
            var_7 = 20;
...
choppersupport_cleanup( var_0 )
{
    ...
    if ( isdefined( self.getbestintersectionpt ) )
        self [[ self.getbestintersectionpt ]]();

    if ( isdefined( self.streakinfo ) )
        self.streakinfo.maxxpcap = istrue( var_0 );
...
choppersupport_gettargets( var_0, var_1, var_2, var_3 )
{
    ...
    for ( var_7 = 0; var_7 < var_5.size; var_7++ )
    {
        ...
        if ( istrue( var_10 ) )
        {
            var_13 = spawnstruct();
            var_13.player = var_8;
            var_13._id_127E4 = var_11;
            var_13._id_1324C = var_12;
...
choppersupport_istarget( var_0, var_1, var_2, var_3 )
{
    ...
    if ( isdefined( self.spotlight_origin_offset ) && ![[ self.spotlight_origin_offset ]]( var_1 ) )
        return [ 0, 0, 0 ];

    if ( !get_player_who_most_recently_fired_weapon( var_1 ) )
        return [ 0, 0, 0 ];
...
get_player_who_most_recently_fired_weapon( var_0 )
{
...
choppersupport_getbesttarget( var_0, var_1 )
{
    ...
    {
        if ( !get_player_who_most_recently_fired_weapon( var_7.player ) )
            continue;

        if ( choppersupport_isactivetarget( var_7.player ) && !istrue( var_7._id_1324C ) )
            continue;
        ...
        if ( istrue( var_7._id_127E4 ) )
        {
            var_8 = 1;
            var_10 = var_10 + 40;
        }

        if ( istrue( var_7._id_1324C ) )
        {
            var_9 = 1;
            var_10 = var_10 + 20;
        }
...
choppersupport_explode()
{
    ...
    if ( isdefined( self.highlighttoteamonce ) )
        [[ self.highlighttoteamonce ]]();
...
choppersupport_crash( var_0 )
{
    ...
    if ( istrue( self.canseesafecircleui ) && isdefined( self.hidetimedrunhudfromplayer ) )
    {
        self setneargoalnotifydist( 300 );
        var_1 = 1;
        var_2 = [[ self.hidetimedrunhudfromplayer ]]();
    }
    else
...
{
    var_3 = self.origin;
    var_4 = self.gunnerdamagemodifier;
    var_5 = undefined;
...
beginevasivemaneuvers()
{
    ...
    var_0 = scripts\engine\utility::_id_13AED( 3.0, "death" );
...
choppersupport_handledeathdamage( var_0 )
{
    ...
    if ( istrue( self.spectatepoint ) && isdefined( level._id_12C5E ) )
    {
        if ( isdefined( var_0.objweapon ) )
            [[ level._id_12C5E._id_12C71 ]]( var_0.objweapon.basename );
        else
            [[ level._id_12C5E._id_12C71 ]]( "none" );
    }
