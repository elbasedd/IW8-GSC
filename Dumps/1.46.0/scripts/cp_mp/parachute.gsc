...
initparachutedvars()
{
    ...
    level._id_11AE1 = getdvarint( "scr_parachute_autodeploy_cut", 1 );
    level._id_11AE2 = getdvarint( "scr_parachute_cut", 1 );
    ...
    if ( !isdefined( level._id_11AEA ) )
        level._id_11AEA = ::_id_11AEB;

    level._id_12C0F = spawnstruct();
    level._id_12C0F._id_130E2 = getdvarint( "scr_parachute_hint_enabled", 1 );
    level._id_12C0F._id_13D90 = getdvarfloat( "scr_parachute_hint_xyvelscale_high", 0.0 );
    level._id_12C0F._id_13D91 = getdvarfloat( "scr_parachute_hint_xyvelscale_low", 0.0 );
    level._id_12C0F._id_13D92 = getdvarfloat( "scr_parachute_hint_xyvelscale_maxheight", 0.0 );
    level._id_12C0F._id_13D8F = getdvarfloat( "scr_parachute_hint_xylimit", 0.0 );
    level._id_12C0F._id_13DA4 = getdvarfloat( "scr_parachute_hint_zdrop", 0.0 );
    level._id_12C0F.zoffset = getdvarfloat( "scr_parachute_hint_zoffset", 0.0 );
    level._id_12C0F._id_13E40 = getdvarfloat( "scr_parachute_hint_zvelscale", 0.0 );
    level._id_12C0F._id_13DA5 = getdvarfloat( "scr_parachute_hint_zlimit", 0.0 );
    level._id_12C0F.modular_spawning_vehicles = getdvarfloat( "scr_parachute_hint_falling_xyratio", 0.0 );
}
...
startfreefall( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    ...
    {
        thread objective_locations( var_0, var_1, var_2, var_3, var_4, var_5 );
        return;
    }
    ...
    if ( level.gametype == "br" && ( !istrue( var_4 ) || ( getdvar( "scr_br_gametype", "" ) == "dmz" || getdvar( "scr_br_gametype", "" ) == "risk" ) ) && getdvarint( "scr_infil_parachute_vfx", 1 ) )
        thread select_stadium_three_spawners( getdvarint( "scr_bmo_parachuteTouchdownVFX", 1 ) == 1 && ( getdvar( "scr_br_gametype", "" ) == "dmz" || getdvar( "scr_br_gametype", "" ) == "risk" ) );
...
select_stadium_three_spawners( var_0 )
{
    var_1 = self;
    ...
    if ( isdefined( self.operatorcustomization ) && isdefined( self.operatorcustomization.deathnoise ) )
        var_2 = var_2 + self.operatorcustomization.deathnoise;
...
playerfriendlyto()
{
    if ( isdefined( level.ending_zplanes ) )
        return level.ending_zplanes;
...
spectatableprops()
{
    return isdefined( self.mortar_smoke_occlusions ) && ( self.mortar_smoke_occlusions == 5 || self.mortar_smoke_occlusions == 6 );
}
...
_id_11AEB()
{
    ...
    {
        if ( isdefined( var_0.mortar_smoke_occlusions ) && ( var_0.mortar_smoke_occlusions == 1 || var_0.mortar_smoke_occlusions == 2 ) )
        {
            var_0 mortar_cooldown();
            var_0.mortar_smoke_occlusions = 3;
        }
    }
    else if ( level.gametype == "cp_survival" )
    {
        var_0 setclientomnvar( "ui_br_altimeter_state", 0 );
        var_0 mortar_cooldown( 1 );
    }
}
...
_id_13774()
{
    ...
    var_2 = clamp( var_0.origin[2], 0, level._id_12C0F._id_13D92 ) / level._id_12C0F._id_13D92;
    var_3 = level._id_12C0F._id_13D91 + ( level._id_12C0F._id_13D90 - level._id_12C0F._id_13D91 ) * var_2;
    var_4 = var_1 * ( 1, 1, 0 );
    var_5 = length( var_4 );
    var_6 = var_4 * var_3;
    var_7 = var_5 * var_3;

    if ( var_7 > level._id_12C0F._id_13D8F )
        var_6 = var_6 * ( level._id_12C0F._id_13D8F / var_7 );

    var_8 = 0;
    var_8 = var_8 - var_0.origin[2] * level._id_12C0F._id_13DA4;
    var_8 = var_8 + level._id_12C0F.zoffset;
    var_8 = var_8 + var_1[2] * level._id_12C0F._id_13E40;
    var_8 = clamp( var_8, -1 * level._id_12C0F._id_13DA5, 0 );
    var_9 = 1;

    if ( var_5 < level._id_12C0F.modular_spawning_vehicles * var_1[2] )
        var_9 = 2;

    var_0 setadditionalstreampos( var_0.origin + ( var_6[0], var_6[1], var_8 ), 0, var_9 );
}

objective_locations( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    self endon( "disconnect" );
    level endon( "game_ended" );
    ...
    var_6 = self;
    var_6.mortar_smoke_occlusions = 1;
    var_6.mortar_add_fov_user_scale = istrue( var_4 );
    var_6.mortar_getimpactspot = 0;
    var_6 morsenumber( var_0, var_1, var_2, var_3, var_5 );
    var_6 mortar_remove_fov_user_scale();
    var_7 = gettime();

    for (;;)
    {
        if ( var_6 isskydiving() || var_7 + 2000 < gettime() || var_6 spectatableprops() )
            break;
    ...
    for (;;)
    {
        if ( level._id_12C0F._id_130E2 && var_6.mortar_getimpactspot < gettime() )
        {
            var_6 _id_13774();
            var_6.mortar_getimpactspot = gettime() + 500;
        }

        if ( var_6 isinfreefall() && var_6.mortar_smoke_occlusions != 1 )
        {
            var_6 mortar_remove_fov_user_scale();
            var_6.mortar_smoke_occlusions = 1;
        }

        if ( var_6 isparachuting() && var_6.mortar_smoke_occlusions != 2 )
        {
            var_6 mortar_initmortars();
            var_6.mortar_smoke_occlusions = 2;
        }

        if ( !var_6 isskydiving() && var_6.mortar_smoke_occlusions != 3 && var_6.mortar_smoke_occlusions != 4 && var_6.mortar_smoke_occlusions != 6 || var_6.mortar_smoke_occlusions == 5 )
        {
            var_6 mortar_cooldown();

            if ( var_6.mortar_smoke_occlusions != 5 )
                var_6.mortar_smoke_occlusions = 3;
            else
                var_6.mortar_smoke_occlusions = 6;
        }

        var_8 = istrue( var_6.inlaststand );
        var_9 = var_6 isonground() && ( var_6.mortar_smoke_occlusions == 3 || var_6 spectatableprops() );
        ...
        {
            var_6 mortar_init();
            var_6.mortar_smoke_occlusions = undefined;
            var_6.mortar_add_fov_user_scale = undefined;
            var_6.mortar_getimpactspot = undefined;
...
morsenumber( var_0, var_1, var_2, var_3, var_4 )
{
    self skydive_interrupt();
    ...
    if ( level.gametype == "br" && ( !istrue( self.mortar_add_fov_user_scale ) || getdvar( "scr_br_gametype", "" ) == "dmz" ) || getdvar( "scr_br_gametype", "" ) == "risk" )
        thread select_stadium_three_spawners( getdvarint( "scr_bmo_parachuteTouchdownVFX", 1 ) == 1 && ( getdvar( "scr_br_gametype", "" ) == "dmz" || getdvar( "scr_br_gametype", "" ) == "risk" ) );
...
mortar_remove_fov_user_scale()
{
    ...
    {
        self setclientomnvar( "ui_br_altimeter_state", 1 );
        level.apcturret[self getxuid()] = self;
    }

    if ( !self.mortar_add_fov_user_scale )
    {
        var_0 = "enabled";

        if ( isdefined( self.operatorcustomization ) && isdefined( self.operatorcustomization.deathnoise ) )
            var_0 = var_0 + self.operatorcustomization.deathnoise;
    }
}
...
{
    self notify( "freefall_complete" );
    var_0 = scripts\cp_mp\utility\script_utility::_id_13838( "game", "isGameTypeBR", 0 );
    var_1 = scripts\cp_mp\utility\script_utility::_id_13838( "game", "getSubGameType", "" );
    var_2 = var_1 != "truckwar" && var_1 != "reveal" && var_1 != "brdov";

    if ( var_0 && var_2 && istrue( self.select_mountain_three_spawners ) )
    {
        var_3 = scripts\cp_mp\utility\script_utility::_id_13838( "music", "getRandomMusicSet", "", [ "br_infil_jump_parachute" ] );
        self setplayermusicstate( var_3 );
        self.select_mountain_three_spawners = undefined;
    }
    ...
    if ( isdefined( level._id_1199D ) )
        self [[ level._id_1199D ]]();
}

mortar_cooldown( var_0 )
{
    self.ignorefalldamagetime = gettime() + 5000;
    self.swapped_car = gettime();
    ...
    level.apcturret[self getxuid()] = undefined;
    self notify( "parachute_complete" );
    self animscriptsetinputparamreplicationstatus( 0 );
}
...
_id_12025()
{
    ...
    if ( isalive( self ) && self.sessionstate == "playing" )
    {
        ...
        {
            wait 5.0;
            var_1 = scripts\cp_mp\utility\script_utility::_id_13838( "music", "isBRSuspenseMusicEnabled", 0, [ self ] );
