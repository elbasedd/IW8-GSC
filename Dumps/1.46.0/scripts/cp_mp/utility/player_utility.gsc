...
isinvehicle( var_0 )
{
    ...
    if ( istrue( var_0 ) )
    {
        if ( isdefined( self._id_139A5 ) && isdefined( self._id_139A5.vehiclename ) )
            return 1;
    }
...
watchthermalinputchange()
{
    ...
    for (;;)
    {
        ...
        var_1 = scripts\engine\utility::_id_13AE8( "input_type_changed", "thermal_handling_ended" );
...
assign_lowest_full_slot_to_active()
{
    if ( !self isdismembermentenabledforplayer() )
        level._id_11F9C[self getxuid()] = self;
}

_id_124A2()
{
    level._id_11F9C[self getxuid()] = undefined;
}

playerpackdataintogulagomnvar()
{
    return level._id_11F9C;
}

ai_hold_debug()
{
    if ( !isdefined( self._id_11EAF ) )
        self._id_11EAF = 0;
...
_id_11EB2()
{
    ...
    if ( !isdefined( level._id_11F6F ) )
        level._id_11F6F = getdvarint( "scr_playerShowOnReset", 0 );
