...
init( var_0, var_1 )
{
    ...
    if ( var_2 )
        var_3 = playerteleportprop( var_1 );
    else
        var_3 = 180;

    if ( !isdefined( level.oscope_ampl ) )
    {
        if ( var_2 )
            level.oscope_ampl = getdvarfloat( "scr_br_gasMask_resist", 0 );
        else
            level.oscope_ampl = 0.2;
    }

    self.gasmaskhealth = var_3;
    self.oscope_freq_think = var_3;
    self.outline_enemy_ai_for_duration = var_1;
    var_4 = 2;

    if ( spawn_first_leads_early( var_1 ) )
        var_4 = 3;
...
{
    if ( spawn_first_leads_early( var_0 ) )
...
processdamage( var_0 )
{
    ...
    var_2 = self.gasmaskhealth / self.oscope_freq_think;
    self.gasmaskhealth = self.gasmaskhealth - var_0;
    self.gasmaskhealth = max( 0, self.gasmaskhealth );

    if ( var_1 )
        var_3 = ( self.gasmaskhealth + 1 ) / self.oscope_freq_think;
    else
        var_3 = self.gasmaskhealth / self.oscope_freq_think;
    ...
    self.gasdamagebuffer = self.gasdamagebuffer + var_0 * level.oscope_ampl;
...
isballisticspecial( var_0, var_1 )
{
    var_2 = scripts\cp_mp\utility\script_utility::issharedfuncdefined( "game", "getGameType" ) && [[ scripts\cp_mp\utility\script_utility::getsharedfunc( "game", "getGameType" ) ]]() == "br";
    ...
    if ( !isdefined( self.oscope_freq_think ) )
        self.oscope_freq_think = playerteleportprop( self.outline_enemy_ai_for_duration );

    var_3 = var_0 / self.oscope_freq_think;
    var_4 = var_1 / self.oscope_freq_think;
    var_5 = floor( var_3 * 6 + 0.5 );
...
equipgasmask()
{
    ...
    if ( istrue( self.operatorcustomization.risktokencountondeath ) )
        self attach( var_2 );
