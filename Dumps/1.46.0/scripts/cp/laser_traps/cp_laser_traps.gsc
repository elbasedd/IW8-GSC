...
init()
{
    level.brvictiminlaststand = scripts\engine\utility::getstructarray( "ascend_solo_begin", "script_noteworthy" );
    level.intro_heli_add_player = scripts\engine\utility::getstructarray( "descend_solo_begin", "script_noteworthy" );
    ...
    foreach ( var_1 in level.brvictiminlaststand )
        scripts\cp_mp\auto_ascender::markupascenderstruct( var_1, 1 );

    foreach ( var_4 in level.intro_heli_add_player )
    ...
    foreach ( var_1 in level.brvictiminlaststand )
    {
        foreach ( var_4 in level.intro_heli_add_player )
        {
            if ( !isdefined( var_4._id_12CC1 ) && distance2dsquared( var_1.origin, var_4.origin ) < 10 )
            {
                var_1._id_12CC1 = var_4;
                var_4._id_12CC1 = var_1;
                level._id_13394--;
                break;
            }
        }
    ...
    initanimtree();
    scripts\engine\scriptable::_id_1279F( "ascender_solo", ::brtruck_ontimelimit );
}

seq3_wave_delay( var_0, var_1 )
{
...
brtruck_ontimelimit( var_0, var_1, var_2, var_3, var_4 )
{
    level._id_127BA = var_0;
...
startascenderanim( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    if ( var_4 > 0 )
        thread _id_12ECC( self.angles + ( 0, var_4 * -1, 0 ) );
...
ascenderuse( var_0, var_1 )
{
    ...
    if ( !scripts\cp_mp\auto_ascender::f14_3pastcodes( var_0, var_1 ) )
        return;
    ...
    var_12 setmodel( "misc_vm_ascender_ch3" );
    var_12 hide();
    var_13 = spawn( "script_model", var_2.origin );
    var_13 setmodel( "misc_wm_ascender_ch3" );
    var_13 hide();
    var_1.brtruck_playerwelcomesplashes = var_2;
    var_1.brtruck_initdialog = var_12;
    var_1.brtruck_initexternalfeatures = var_13;
    ...
    if ( !var_2.dir )
    {
        if ( scripts\common\utility::iscp() )
            var_15 = getdvarfloat( "scr_descender_speed_cp", scripts\cp_mp\auto_ascender::playerattractiontriggerenter() );
        else
            var_15 = getdvarfloat( "scr_descender_speed", scripts\cp_mp\auto_ascender::playerapplyjumpvelocity() );
    }
    else
        var_15 = getdvarfloat( "scr_ascender_speed", scripts\cp_mp\auto_ascender::player_weapon_fired_monitor() );

    var_16 = var_14 / var_15;
    var_17 = scripts\cp_mp\auto_ascender::player_waittilljumpedfromc130() * var_16;
    var_18 = scripts\cp_mp\auto_ascender::player_xp() * var_16;
    ...
    if ( getdvarint( "scr_ascender_allowDisconnect", 1 ) > 0 )
        var_19 = var_1 _id_13BF2( var_2, var_16, var_12, var_13 );
    else
        wait( var_16 );
...
_id_13BF2( var_0, var_1, var_2, var_3 )
{
    var_4 = self;
    var_5 = gettime();
...
spawnerindex( var_0 )
{
    foreach ( var_2 in level.brvictiminlaststand )
    {
        if ( distance2dsquared( var_2.origin, var_0 ) < scripts\cp_mp\auto_ascender::playerbloodrestricted() )
        {
            if ( isdefined( var_2._id_12CC1 ) )
            {
                if ( abs( var_0[2] - var_2.origin[2] ) < scripts\cp_mp\auto_ascender::playerattractiontriggerexit() || abs( var_0[2] - var_2._id_12CC1.origin[2] ) < scripts\cp_mp\auto_ascender::playerattractiontriggerexit() )
                    return 1;
            }
        }
    }
