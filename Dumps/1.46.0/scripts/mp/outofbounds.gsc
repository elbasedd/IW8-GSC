...
spawn_module_lmg_5( var_0 )
{
    if ( scripts\mp\utility\game::spawn_maint_wave_1() )
        return scripts\mp\gametypes\br_c130.gsc::ispointinbounds( var_0, 0, 1 );
    else
...
onenteroob( var_0 )
{
    ...
    if ( isplayer( var_0 ) )
    {
        if ( isdefined( level.vehicle_dismount_watcher ) )
            var_1 = level.vehicle_dismount_watcher;
        else
...
onexitoob( var_0, var_1, var_2 )
{
    ...
    if ( isplayer( var_0 ) )
    {
        if ( isdefined( level.vehicle_dlog_enterevent ) )
            var_3 = level.vehicle_dlog_enterevent;
        else
...
watchoobtriggerenter( var_0 )
{
    ...
    for (;;)
    {
        ...
        if ( isplayer( var_1 ) && scripts\mp\utility\game::spawnboardroom_miniguns( var_1 ) )
            continue;
...
{
    var_2 = var_1._id_12559;

    if ( istrue( var_2 ) )
        return scripts\mp\utility\game::playertimestart();

    if ( isdefined( var_2 ) && !var_2 )
        return scripts\mp\utility\game::playertimestamp();

    if ( var_1 scripts\cp_mp\vehicles\vehicle::isvehicle() )
    {
        var_3 = var_1 scripts\mp\utility\game::pretrograd_interaction_effects();
    ...
    switch ( var_0 )
    {
        ...
        case "br":
            return scripts\mp\utility\game::playertimedinvunerable();
        case "default":
