...
create_name_fx_base()
{
    ...
    create_mp_version_of_vehicle( self.br_armorhealth );
    scripts\mp\equipment::decrementequipmentslotammo( "health", 1 );
    scripts\cp\utility\cp_controlled_callbacks::_id_119E5( "armor_plate" );
    scripts\mp\gametypes\br_gametypes.gsc::_id_1268B( "onArmorPlate" );
...
create_mp_version_of_vehicle( var_0 )
{
...
currentbestpos( var_0, var_1 )
{
    self endon( "disconnect" );

    if ( var_1 == 0 )
        return;
    else if ( istrue( self.shouldpickup ) )
        return;
    else if ( self isswitchingweapon() )
        return;

    if ( !createhistorydestination() )
        return;

    var_2 = getcompleteweaponname( "armor_plate_deploy_mp" );
    var_3 = scripts\cp_mp\utility\killstreak_utility::createstreakinfo( "", self );
    var_3.brmini_cleanupents = var_2;
    current_volume_num( 1 );
    thread currentclientmatchdataid();
    var_4 = scripts\cp_mp\killstreaks\killstreakdeploy::switchtodeployweapon( var_2, var_3, ::createheliextractobjectiveicons, undefined, undefined, undefined, undefined, 0 );
}
...
currentclientmatchdataid()
{
    ...
    thread currentdebugweapon();
    self._id_130A2 = 0;
    scripts\engine\utility::_id_13ADD( "death", "mantle_start", "last_stand_start", "special_weapon_fired", "br_try_armor_cancel", "br_armor_plate_done" );
    self._id_130A2 = 1;
    thread create_nav_obstacle_for_wheelson();
}
...
current_volume_num( var_0 )
{
    ...
    scripts\common\utility::bot_pick_new_loadout_next_spawn( !var_0 );
    self.shouldpickup = var_0;
}
...
createheliextractobjectiveicons( var_0 )
{
    ...
    if ( !createhistorydestination() || istrue( self._id_130A2 ) )
        return;
    ...
    {
        if ( !isdefined( var_0.brmini_cleanupents ) || var_0.brmini_cleanupents != self getcurrentweapon() )
            return;
    ...
    while ( current_respawn_point_override() )
    {
...
createhistorydestination()
{
    ...
    var_2 = istrue( self._id_125A9 ) || istrue( self.isjuggernaut ) || scripts\mp\supers::issuperinuse() && self.super.staticdata.ref != "super_deadsilence";
    ...
    {
        scripts\mp\hud_message::showerrormessage( level.br_pickups.cumulative_damage_expire_time );
        return 0;
    }
...
current_respawn_point_override()
{
    ...
    var_1 = isdefined( self.brmayconsiderplayerdead ) && self.brmayconsiderplayerdead > 0;
    return var_0 || var_1;
}
...
create_nav_obstacle_for_wheelson()
{
    ...
    if ( istrue( self.brmayconsiderplayerdead ) )
        self.brmayconsiderplayerdead = 0;
