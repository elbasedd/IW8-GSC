...
supportbox_init()
{
    level.br_armor_plate_amount_equipped_set = [];
    scripts\common\interactive::interactive_addusedcallback( ::supportbox_usedcallback, "equip_supportBox" );
    scripts\common\interactive::interactive_addusedcallback( ::brloot_ammo, "equip_armorBox" );
    scripts\mp\utility\join_team_aggregator::registeronplayerjointeamcallback( ::_id_13175 );
...
_id_13175( var_0 )
{
    foreach ( var_2 in level.br_armor_plate_amount_equipped_set )
    {
        if ( isdefined( var_2 ) )
            var_2 _id_13173( var_0 );
    }
}

create_cypher_piece( var_0, var_1, var_2, var_3 )
{
    var_0 endon( "death" );
    var_0 thread supportbox_watchdisownedtimeout();
    var_0 thread supportbox_hideandshowaftertime();
    var_0 thread _id_13176( self );

    if ( !scripts\mp\flags::gameflag( "prematch_fade_done" ) )
        var_0 thread _id_13177();
    ...
    level.br_armor_plate_amount_equipped_set[var_0 getentitynumber()] = var_0;
...
{
    var_0 endon( "death" );
    var_0._id_13337 = "vfx/iw8_mp/equipment/vfx_offhand_wm_supportbox_timeout.vfx";
    create_cypher_piece( var_0, "equip_supportBox", "super_ammo_drop", "hud_icon_equipment_support_box" );
...
{
    var_0 endon( "death" );
    var_0._id_13337 = "vfx/iw8_mp/equipment/vfx_offhand_wm_armorbox_timeout.vfx";
    create_cypher_piece( var_0, "equip_armorBox", "super_armor_drop", "ui_mp_br_loot_icon_health_armor_box" );
...
supportbox_destroy( var_0 )
{
    ...
    if ( isdefined( self._id_13337 ) )
        var_1 = self._id_13337;
...
supportbox_delete( var_0, var_1, var_2 )
{
    ...
    if ( isdefined( self.owner ) )
    {
        if ( isdefined( self.equipmentref ) && self.equipmentref == "equip_armorBox" )
            self.owner scripts\cp\utility\cp_controlled_callbacks::_id_11972( "super_armor_drop", self.usedcount, var_0, var_2 );
        else
            self.owner scripts\cp\utility\cp_controlled_callbacks::_id_11972( "super_ammo_drop", self.usedcount, var_0, var_2 );
    }
...
supportbox_playercanuse( var_0 )
{
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" && scripts\mp\utility\game::postupdategameevents() != "x2" )
    {
        if ( isdefined( self.owner ) && !scripts\engine\utility::array_contains( scripts\mp\gametypes\br_public.gsc::postplunder( self.owner.team, self.owner.squadindex ), var_0 ) )
            return 0;
...
{
    self.remove_usability_crutch_on_death = scripts\cp_mp\entityheadicons::setheadicon_singleimage( [], var_0, 20, 1, 1000, 100, undefined, 1 );
...
supportbox_givexpforuse( var_0 )
{
    if ( isdefined( self.owner ) && !scripts\cp_mp\utility\player_utility::playersareenemies( self.owner, var_0 ) )
    {
        if ( self.owner != var_0 )
        {
            ...
            self.owner scripts\cp\utility\cp_controlled_callbacks::_id_119D5( self );
        }

        self.owner scripts\mp\utility\stats::incpersstat( "munitionsBoxUsed", 1 );
        self.owner scripts\mp\supers::getsquadspawnstruct( "super_ammo_drop" );
        self.usedcount++;
    }
}
...
supportbox_handlemovingplatforms( var_0 )
{
    ...
    if ( isdefined( var_0 ) && scripts\mp\equipment\claymore::so_setplayerdata( var_0 ) )
    {
        var_1._id_11CA8 = 1;
        self _meth_87B9( 1 );
    }
...
_id_13173( var_0 )
{
    ...
    var_1 = self.remove_usability_crutch_on_death;

    if ( !isdefined( var_1 ) )
        return;

    var_2 = self.remove_veh_spawners_from_passive_wave_spawning;

    if ( !isdefined( var_2 ) )
        return;

    if ( scripts\mp\utility\game::getgametype() == "br" && scripts\mp\menus::_id_12F0E() && isdefined( self.owner ) )
        var_3 = scripts\engine\utility::array_contains( scripts\mp\gametypes\br_public.gsc::postplunder( self.owner.team, self.owner.squadindex ), var_0 );
    else
    {
        if ( isdefined( self.owner ) )
    ...
    if ( var_3 && !var_5 )
    {
        scripts\cp_mp\entityheadicons::_id_12994( var_1, var_0 );
        scripts\cp_mp\entityheadicons::_id_12995( var_2, var_0 );
    }
    else if ( var_3 )
    {
        scripts\cp_mp\entityheadicons::_id_12995( var_1, var_0 );
        scripts\cp_mp\entityheadicons::_id_12994( var_2, var_0 );
    }
    else
    {
        scripts\cp_mp\entityheadicons::_id_12995( var_1, var_0 );
...
supportbox_usedcallback( var_0, var_1 )
{
    ...
    {
        if ( isdefined( level._id_119D6 ) )
            var_2 = var_0 [[ level._id_119D6 ]]( var_1 );
        else
        ...
        if ( var_2 )
        {
            ...
            var_0 _id_13173( var_1 );
            var_0 supportbox_givexpforuse( var_1 );
        }
    }
}

brloot_ammo( var_0, var_1 )
{
    ...
    if ( var_0 supportbox_playercanuse( var_1 ) )
    {
        if ( isdefined( level.zombiespawnabovedeath ) )
            var_2 = var_0 [[ level.zombiespawnabovedeath ]]( var_1 );
        else
            var_2 = 0;

        if ( var_2 && !istrue( var_0._id_1368F ) )
        {
            var_0.playersused[var_1 getentitynumber()] = var_1;
            var_0 _id_13173( var_1 );
            var_0 supportbox_givexpforuse( var_1 );
