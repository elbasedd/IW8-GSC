...
molotov_init()
{
    var_0 = spawnstruct();
    level.molotov = var_0;
    var_0.updaterespawnstatus = getdvarint( "scr_molotovMaxPools", 100 );
    var_0.updateinstantclassswapallowedinternal = getdvarint( "scr_molotovMaxCastsPerFrame", 8 );
    var_0.scriptedspawnpointsonmigration = getdvarint( "scr_molotovInstantCleanup", 1 ) > 0;
    var_0._id_1368C = 0;
    var_0._id_120B8 = [];
    var_0.scriptables = [];
    var_0.triggers = [];
...
molotov_used( var_0 )
{
    ...
    if ( isdefined( level._id_12ACA ) && [[ level._id_12ACA.jugg_setherodropscriptable ]]( var_0 ) )
    {
        if ( isdefined( level._id_12ACA ) && isdefined( level._id_12ACA.handlemolotov ) )
        {
            [[ level._id_12ACA.handlemolotov ]]( var_0 );
            return;
        }
    }

    if ( isdefined( level.onsquadeliminatedplacement ) )
        [[ level.onsquadeliminatedplacement ]]( var_0, self );
...
molotov_stuck_player( var_0, var_1, var_2, var_3 )
{
    ...
    if ( !isdefined( level._id_12ACA ) || ![[ level._id_12ACA.gas_linger_vfx ]]( var_1 ) )
    ...
    if ( !isdefined( level._id_12ACA ) || ![[ level._id_12ACA.gas_payloads ]]( var_1 ) )
    {
        var_3 = var_3 * ( 0, 0, 1 );
        var_4 = var_0.origin;
        var_5 = ( 0, 0, -1 );
...
{
    if ( !scripts\mp\bots\bots_util::bot_bots_enabled_or_added() && !scripts\mp\utility\game::currentmultikill() )
        return;
...
molotov_simulate_impact( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    ...
    if ( !istrue( level.i_am_seeing_this_player ) )
    {
    ...
    var_22 = molotov_create_branch( var_18, var_20, var_21, undefined, var_13, var_34, var_3, 0, var_26, var_27 );
    var_18.branches[var_18.branches.size] = var_22;
    vehicle_preventplayercollisiondamagefortimeafterexitinternal( var_18 );
...
vehicle_preventplayercollisiondamagefortimeafterexitinternal( var_0 )
{
    ...
    var_1 = vehicle_outline_watcher();
    var_1.caststhisframe++;
}

vehicle_process_node_when_at_goal( var_0 )
{
    var_0._id_127B0++;
    var_1 = vehicle_outline_watcher();
    var_1.scriptables[self.id] = self;

    if ( var_1.scriptables.size > var_1.updaterespawnstatus )
...
vehicle_register_on_level( var_0 )
{
    ...
    if ( var_1.triggers.size > var_1.updaterespawnstatus )
...
vehicle_occupancy_takeriotshield()
{
    ...
    if ( !isdefined( var_1 ) )
        var_0._id_120B8 = scripts\engine\utility::array_remove( var_0._id_120B8, self.id );
...
vehicle_occupancy_mp_updateriotshield( var_0 )
{
    var_1 = vehicle_outline_watcher();
    var_2 = undefined;
    var_3 = undefined;

    foreach ( var_3 in var_1._id_120B8 )
    {
    ...
    var_0 = istrue( var_0 ) || var_1.scriptedspawnpointsonmigration;

    if ( !isdefined( var_2 ) )
        vehicle_occupancy_showcashbag( var_3, var_0 );
}
...
vehicle_occupancy_showcashbag( var_0, var_1 )
{
    ...
    if ( isdefined( var_3 ) )
    {
        if ( !istrue( var_1 ) )
        {
            ...
            var_2._id_120B8 = scripts\engine\utility::array_remove( var_2._id_120B8, var_0 );
            var_3 thread molotov_pool_end();
            return;
        }

        var_3 thread vehicle_occupancy_takeriotshield();
    }

    var_4 = var_2.triggers[var_0];

    if ( isdefined( var_4 ) )
        var_4 thread vehicle_occupancy_updateriotshield();
}

vehicle_occupancy_monitorcontrols( var_0 )
{
    ...
    if ( var_1.caststhisframe >= var_1.updateinstantclassswapallowedinternal )
        return 0;
...
vehicle_registerturret()
{
    ...
    if ( !isdefined( var_0.xyvelscale_maxheight ) )
        var_0.xyvelscale_maxheight = [];
    ...
    foreach ( var_2 in self.branches )
    {
        ...
        {
            if ( isdefined( var_4 ) )
                var_0.xyvelscale_maxheight[var_0.xyvelscale_maxheight.size] = var_4;
        }
    }
}
...
molotov_start_branch()
{
    ...
    else
    {
        ...
        for (;;)
        {
            ...
            if ( !vehicle_occupancy_monitorcontrols( self.shareddata ) )
            {
                waitframe();
                continue;
            }
...
{
    vehicle_preventplayercollisiondamagefortimeafterexitinternal( self.shareddata );
    self.casts++;
...
molotov_create_pool( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9 )
{
    ...
    var_11 thread vehicle_remove_invulnerability_onenter();
    return var_11;
}
...
molotov_branch_create_pool( var_0, var_1, var_2 )
{
    ...
    var_7.id = vehicle_playerkilledfx();
    var_7 vehicle_process_node_when_at_goal( self.shareddata );
    return var_7;
}
...
molotov_pool_start()
{
    ...
    if ( isdefined( self ) )
        ...
        level thread vehicle_preventplayercollisiondamagefortimeafterexit( var_1, var_0 );
}
...
molotov_watch_pool()
{
    ...
    var_1 = scripts\mp\utility\game::spawn_maint_wave_1();
...
molotov_pool_end( var_0 )
{
    ...
    if ( isdefined( self.trigger ) )
        self.trigger thread vehicle_occupancy_updateriotshield();

    if ( !istrue( var_0 ) )
        wait 3.5;

    thread vehicle_occupancy_takeriotshield();
}

vehicle_preventplayercollisiondamagefortimeafterexit( var_0, var_1 )
{
    if ( isdefined( var_0 ) )
        var_0 thread vehicle_occupancy_updateriotshield();
    ...
    if ( !isdefined( var_0 ) )
        var_2._id_120B8 = scripts\engine\utility::array_remove( var_2._id_120B8, var_1 );
}
...
molotov_create_pool_trigger( var_0, var_1, var_2 )
{
    ...
    var_4 vehicle_register_on_level();
    return var_4;
}
...
molotov_watch_pool_trigger_enter()
{
    ...
    for (;;)
    {
        ...
        if ( !isplayer( var_0 ) && !istrue( var_0.bots_seek_player ) )
            continue;
...
{
    var_0 endon( "death" );
    scripts\engine\utility::_id_13AEE( 7, "disconnect", "joined_team" );
    var_0 delete();
}
...
{
    var_0 = vehicle_outline_watcher();
    var_1 = var_0._id_1368C;
    var_0._id_1368C++;
    var_0._id_120B8 = scripts\engine\utility::array_add( var_0._id_120B8, var_1 );
    return var_1;
}

molotov_get_cast_data( var_0 )
{
    var_1 = vehicle_on_last_pathing_array();
    var_2 = spawnstruct();
...
{
    var_1 = vehicle_playerenteredtrackedlittlebird();
    var_2 = spawnstruct();
    var_2.typeid = var_0;
...
molotov_start_burning( var_0, var_1, var_2, var_3 )
{
    ...
    if ( isdefined( level._id_12ACA ) && [[ level._id_12ACA.gas_linger_vfx ]]( self ) )
        return;
...
vehicle_occupancy_monitormovementcontrols( var_0, var_1 )
{
    if ( isdefined( var_0.shareddata ) )
    {
        if ( isdefined( var_0.shareddata.branches ) )
        {
            foreach ( var_3 in var_0.shareddata.branches )
                vehicle_occupancy_monitorgameended( var_3, var_1 );
        }

        if ( isdefined( var_0.shareddata.xyvelscale_maxheight ) )
        {
            foreach ( var_6 in var_0.shareddata.xyvelscale_maxheight )
            {
                if ( isdefined( var_6 ) )
