...
{
    level._id_131D2 = [];
    scripts\mp\utility\join_team_aggregator::registeronplayerjointeamcallback( ::_id_131CC );

    if ( scripts\mp\utility\game::getgametype() == "br" )
        level thread _id_131C4();
}

_id_131CC( var_0 )
{
    foreach ( var_2 in level._id_131D2 )
    {
        var_2 _id_131D1( var_0 );
        var_2 _id_131D0( var_0 );
    }
}
...
_id_131CB()
{
    ...
    for (;;)
    {
        ...
        scripts\engine\utility::_id_13AED( var_2, "offhand_pullback" );
    }
}
...
tacinsert_used( var_0 )
{
    ...
    if ( !_id_131C5( var_0, var_3, var_4 ) )
    {
        scripts\mp\hud_message::showerrormessage( "MP/TAC_INSERT_CANNOT_PLACE" );
        thread _id_131CE();
        return 0;
    ...
    var_17 thread _id_131C6();
    var_17 setotherent( self );
    var_17.weapon_name = "tac_insert_trigger";
    var_18 = var_7["entity"];

    if ( isdefined( var_18 ) && ( _branalytics_headerplayer::so_setplayerdata( var_18 ) || scripts\mp\gametypes\br_gondola.gsc::skipdeath( var_18 ) ) )
    {
        var_17 linkto( var_18 );
        var_19 = getdvarint( "scr_br_tac_insert_moving_platform_valid", 1 );

        if ( var_19 )
            var_17 thread _id_131CA( self );
    }
    ...
    _id_12E66( var_1, var_16, var_17, 1 );
    var_21 = var_17 getentitynumber();
    level._id_131D2[var_21] = var_17;

    if ( var_17.issuper && isdefined( level._id_131C8 ) )
        self [[ level._id_131C8 ]]();
...
tacinsert_setupandwaitfordeath( var_0 )
{
    ...
    thread _id_131C7( var_0 );

    if ( !self.issuper )
        thread _id_131CD( var_0 );
}
...
_id_131CD( var_0 )
{
    ...
    level endon( "game_ended" );
    var_0 endon( "disconnect" );
    self._id_11C4D = spawn( "script_origin", self.origin );
    self._id_11C4D makeusable();
    self._id_11C4D setcursorhint( "HINT_NOICON" );
    self._id_11C4D sethintstring( &"MP_PATCH/PICKUP_TI" );
    self._id_11C4D linkto( self );
    ...
    for (;;)
    {
        self._id_11C4D waittill( "trigger", var_2 );
...
_id_131D1( var_0 )
{
    ...
    if ( !isdefined( self ) || !isdefined( self._id_11C4D ) )
        return;
    ...
    if ( var_1 )
        self._id_11C4D enableplayeruse( var_0 );
    else
        self._id_11C4D disableplayeruse( var_0 );
}
...
tacinsert_destroy( var_0, var_1, var_2, var_3 )
{
    ...
    if ( isdefined( self.owner ) )
    {
        if ( istrue( var_2 ) )
        {
            ...
            self.owner scripts\mp\supers::getsquadspawnstruct( "super_tac_insert" );
            self.owner scripts\cp\utility\cp_controlled_callbacks::_id_11972( "super_tac_insert", 1, var_0, var_1 );
            scripts\mp\analyticslog::logevent_fieldupgradeexpired( self.owner, self.superid, 1, istrue( var_1 ) );
        }
        else
        {
            self.owner scripts\cp\utility\cp_controlled_callbacks::_id_11972( "super_tac_insert", 0, var_0, var_1 );
            scripts\mp\analyticslog::logevent_fieldupgradeexpired( self.owner, self.superid, 0, istrue( var_1 ) );
        }

        if ( !istrue( var_2 ) && !istrue( var_3 ) )
            self.owner thread _id_131CF();
...
_id_131C5( var_0, var_1, var_2 )
{
    ...
    if ( isdefined( var_6 ) )
    {
        if ( isdefined( var_6["hittype"] ) && var_6["hittype"] != "hittype_none" )
        {
            ...
            if ( isdefined( var_8 ) )
            {
                if ( var_8 scripts\cp_mp\vehicles\vehicle::isvehicle() && !_branalytics_headerplayer::so_setplayerdata( var_8 ) )
                    var_3 = 0;

                var_9 = getdvarint( "scr_br_tac_insert_moving_platform_valid", 1 );

                if ( !var_9 && ( _branalytics_headerplayer::so_setplayerdata( var_8 ) || scripts\mp\gametypes\br_gondola.gsc::skipdeath( var_8 ) ) )
                    var_3 = 0;
...
_id_131CE()
{
    ...
    if ( istrue( level.spawn_module_building_chopper1 ) )
    {
        ...
        var_1 = scripts\engine\utility::_id_13AED( 5, "super_use_finished_lb" );
