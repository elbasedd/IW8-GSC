...
main()
{
    ...
    level.little_bird_mg_exitendinternal = getdvarint( "scr_arm_enableLittleBirdMG", 0 );
    ...
    level._id_13217 = getdvarint( "scr_arm_tankRespawnTime", 120 );
    level.breventflagclear = getdvarint( "scr_arm_apcRespawnTime", 120 );
    level.longdialoguecooldown = getdvarint( "scr_arm_longDialogueCooldown", 1 );
    level.validate_and_activate_stations = getdvarint( "scr_arm_mercyEndingNuke", 1 );
    level.validate_demeanor = getdvarint( "scr_arm_mercyEndingTime", 30 );
    level.weapon_xp_iw8_sn_crossbow = getdvarint( "scr_arm_numNonRallyVehicles", 25 );
    level.isparachutespawning = getdvarint( "scr_arm_disablePersonalNuke", 0 );
    level._id_11BFD = getdvarint( "scr_personalNukeCostOverride", 30 );
    level._id_12B94 = getdvarint( "scr_arm_showTeamTanks", 1 );
    level._id_12B92 = getdvarint( "scr_arm_showTeamLittleBirds", 1 );
    level._id_12E7E = getdvarint( "scr_arm_spawnSelectionAFKTime", 120 );
    level.unset_heavy_hitter = getdvarint( "scr_arm_manualAdjustLittleBirdLocs", 1 );
    level.kill_vip = getdvarint( "scr_arm_dontSpawnJeep", 0 );
    level.kill_trig = getdvarint( "scr_arm_dontSpawnCargoTruck", 0 );
    level._id_137F5 = getdvarint( "scr_arm_usePingSystem", 1 );

    if ( istrue( level._id_12B94 ) )
    {
        level._id_133EB = [];
        level._id_133EB["axis"] = [];
        level._id_133EB["allies"] = [];
    }

    if ( istrue( level._id_12B92 ) )
    {
        level._id_133EA = [];
        level._id_133EA["axis"] = [];
        level._id_133EA["allies"] = [];
        level._id_133EA["untouched"] = [];
    }
...
onstartgametype()
{
    ...
    if ( istrue( level.validate_and_activate_stations ) )
        level thread wave_default_internal();

    light_tank_watchmissileinputchange();

    if ( istrue( level._id_137F5 ) )
...
light_tank_watchmissileinputchange()
{
    ...
    scripts\cp_mp\utility\game_utility::_id_124AF( "vehicle_volume", "script_noteworthy" );
    scripts\cp_mp\utility\game_utility::_id_124AF( "vehicle_volume_simplified", "script_noteworthy" );
    scripts\cp_mp\utility\game_utility::_id_124AF( "super", "script_noteworthy" );
    scripts\cp_mp\utility\game_utility::_id_124AF( "militarybase", "script_noteworthy" );
    scripts\cp_mp\utility\game_utility::_id_124AF( "location_volume", "targetname" );
    scripts\cp_mp\utility\game_utility::_id_124AF( "locale_area_trigger", "targetname" );
    scripts\cp_mp\utility\game_utility::_id_124AF( "shadow_blocker", "targetname" );
    scripts\cp_mp\utility\game_utility::_id_124B0( "door_prison_cell_metal_mp", 1 );
    scripts\cp_mp\utility\game_utility::_id_124B0( "veh8_mil_air_acharlie130", 1 );
    ...
    scripts\cp_mp\utility\game_utility::_id_124AE( "light" );
    scripts\cp_mp\utility\game_utility::_id_124AE( "trigger_use_touch" );

    if ( isdefined( level.localeid ) && level.localeid == "locale_6" )
        scripts\cp_mp\utility\game_utility::_id_124AF( "locale_8", "script_noteworthy" );
...
updatedomscores()
{
    ...
    level thread scripts\mp\spawnselection::_id_13750();
...
updatescores()
{
    ...
    var_5 = scripts\mp\gamescore::enter_combat_maze_ai();
    ...
    var_8 = scripts\mp\gamescore::enter_combat_maze_ai();

    if ( var_5 != var_8 )
        scripts\mp\gamescore::_id_12039( var_8, 1, var_5 );
}
...
runobjectives( var_0 )
{
    ...
    if ( istrue( level.useobjectives ) )
    {
        ...
        foreach ( var_2 in level.allfobs )
        {
            ...
            if ( istrue( level.onteamproximitybecameinvalidplayer ) )
                var_2._id_12EAB = scripts\mp\gametypes\obj_dom.gsc::_id_12EAC;
    ...
    hackfixcameras();
    level thread botstopmovingonlaststand();
}

botstopmovingonlaststand()
{
...
objective_manageobjectivesintrovisibility()
{
    ...
    scripts\mp\objidpoolmanager::objective_playermask_hidefromall( level.gw_objstruct.axishqloc.lootchopper_droploot.objidnum );
    scripts\mp\objidpoolmanager::objective_playermask_hidefromall( level.gw_objstruct.allieshqloc.lootchopper_droploot.objidnum );
...
sethqmarkerobjective()
{
    ...
    var_2.lockupdatingicons = 1;
    level.gw_objstruct.axishqloc.lootchopper_droploot = var_2;
    level.uncapturableobjectives[level.uncapturableobjectives.size] = var_2;
    ...
    scripts\mp\objidpoolmanager::update_objective_setbackground( var_2.objidnum, 7 );
    var_2.lockupdatingicons = 1;
    level.gw_objstruct.allieshqloc.lootchopper_droploot = var_2;
    level.uncapturableobjectives[level.uncapturableobjectives.size] = var_2;
...
spawnselection_showenemyhq()
{
    ...
    if ( self.team == "allies" )
    {
        ...
        scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( level.gw_objstruct.axishqloc.lootchopper_droploot.objidnum, self );
    }
    else
    {
        scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( level.gw_objstruct.allieshqloc.lootchopper_droploot.objidnum, self );
    ...
    if ( self.team == "axis" )
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( level.gw_objstruct.allieshqloc.lootchopper_droploot.objidnum, self );
    else
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( level.gw_objstruct.axishqloc.lootchopper_droploot.objidnum, self );
}
...
docratedropsmoke( var_0, var_1, var_2 )
{
    ...
    if ( isdefined( var_0 ) )
        var_0 scripts\engine\utility::_id_13AED( var_2, "crate_dropped" );
    else
        wait( var_2 );
...
wave_default_internal()
{
    ...
    if ( istrue( level.useobjectives ) )
        level thread wave_enemies_remaining();
}

wave_enemies_remaining()
{
    level endon( "game_ended" );
    ...
    for (;;)
    {
        if ( enemymarker( "axis" ) == level.objectives.size )
            level thread wave_spawner( "axis" );
        else if ( enemymarker( "allies" ) == level.objectives.size )
            level thread wave_spawner( "allies" );
...
wave_spawner( var_0 )
{
    ...
    _branalytics_addeventallowed::_id_129C4( "ui_nuke_data", 0, 9, level.validate_demeanor );
    _branalytics_addeventallowed::_id_129C4( "ui_nuke_data", 11, 1, 1 );
    var_1 = 0;
    var_2 = gettime();
    var_3 = level.validate_demeanor * 1000 + var_2;
    setomnvar( "ui_nuke_end_milliseconds", level.validate_demeanor * 1000 + var_2 );

    while ( enemymarker( var_0 ) == level.objectives.size )
    {
        waitframe();

        if ( gettime() > var_3 )
        {
            wavesv_finite_max( var_0 );
...
wavesv_finite_max( var_0 )
{
    ...
    foreach ( var_5 in level.players )
    {
        ...
        {
            if ( isdefined( var_5._id_11A56 ) )
            {
                var_5 clearsoundsubmix( "iw8_mp_spawn_camera" );
                var_5._id_11A56 stoploopsound( var_5._id_11A57 );
                var_5._id_11A56 delete();
                var_5._id_11A56 = undefined;
                var_5._id_11A57 = undefined;
            }
        }
    }
...
{
    thread scripts\mp\spawnselection::_id_12377( var_0.team, var_0.squadindex );
}
...
parachute( var_0, var_1 )
{
    ...
    self playershow();
    self.plotarmor = 0;
    scripts\mp\utility\game::_id_129D6( self, 0 );
...
init_groundwarvehicles()
{
    ...
    foreach ( var_1 in level.vehicleinfo )
    {
        ...
        foreach ( var_9 in var_1.vehiclespawns )
        {
            if ( isdefined( scripts\cp_mp\utility\game_utility::getlocaleid() ) && isdefined( var_9.script_noteworthy ) && var_9.script_noteworthy == level.localeid )
            {
                ...
                if ( istrue( level.kill_vip ) && var_1.refname == "jeep" || istrue( level.kill_trig ) && var_1.refname == "cargo_truck" )
                    continue;
                else
                {
    ...
    var_28 = level.weapon_xp_iw8_sn_crossbow;
...
vehiclespawn_tank( var_0, var_1 )
{
    ...
    if ( istrue( level._id_12B94 ) )
        _id_1388E( var_3, var_3.team );

    return var_3;
}

_id_1388E( var_0, var_1 )
{
    wait 1;
    ...
    var_0._id_12E7F = var_2;
    thread _id_13975( var_0 );
    level._id_133EB[var_1][level._id_133EB[var_1].size] = var_0;
}

_id_13975( var_0 )
{
    var_1 = var_0._id_12E7F.objidnum;
    var_2 = var_0.team;
    var_0 waittill( "death" );
    scripts\mp\objidpoolmanager::returnobjectiveid( var_1 );
    level._id_133EB[var_2] = scripts\engine\utility::array_remove( level._id_133EB[var_2], var_0 );
}
...
_id_1395C()
{
    ...
    foreach ( var_1 in level._id_133EB[self.team] )
        scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( var_1._id_12E7F.objidnum, self );

    while ( self.inspawnselection )
        waitframe();

    foreach ( var_1 in level._id_133EB["axis"] )
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( var_1._id_12E7F.objidnum, self );

    foreach ( var_1 in level._id_133EB["allies"] )
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( var_1._id_12E7F.objidnum, self );
}
...
_id_1388D( var_0, var_1 )
{
    ...
    var_0._id_12E7F = var_2;
    thread _id_13974( var_0 );
    level._id_133EA["untouched"][level._id_133EA["untouched"].size] = var_0;
}
...
_id_13974( var_0 )
{
    ...
    var_0 waittill( "death" );
    scripts\mp\objidpoolmanager::returnobjectiveid( var_1 );
    var_2 = var_0.targets_killed_by_hunters;

    if ( !isdefined( var_0.targets_killed_by_hunters ) )
        var_2 = "untouched";

    level._id_133EA[var_2] = scripts\engine\utility::array_remove( level._id_133EA[var_2], var_0 );
}
...
_id_1395D()
{
    ...
    foreach ( var_1 in level._id_133EA[self.team] )
        scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( var_1._id_12E7F.objidnum, self );

    foreach ( var_1 in level._id_133EA["untouched"] )
        scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( var_1._id_12E7F.objidnum, self );

    while ( self.inspawnselection )
        waitframe();

    foreach ( var_1 in level._id_133EA["axis"] )
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( var_1._id_12E7F.objidnum, self );

    foreach ( var_1 in level._id_133EA["allies"] )
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( var_1._id_12E7F.objidnum, self );
...
_id_1394D( var_0 )
{
    if ( isdefined( self.targets_killed_by_hunters ) )
        level._id_133EA[self.targets_killed_by_hunters] = scripts\engine\utility::array_remove( level._id_133EA[self.targets_killed_by_hunters], self );
    else
        level._id_133EA["untouched"] = scripts\engine\utility::array_remove( level._id_133EA["untouched"], self );

    self.targets_killed_by_hunters = var_0;
    level._id_133EA[var_0][level._id_133EA[var_0].size] = self;
    scripts\mp\objidpoolmanager::update_objective_icon( self._id_12E7F.objidnum, "icon_minimap_littlebird_spawn_selection" );

    foreach ( var_2 in level.players )
    {
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( self._id_12E7F.objidnum, var_2 );

        if ( isdefined( var_2 ) && isdefined( var_2.team ) && var_2.team == var_0 && istrue( var_2.inspawnselection ) )
        {
            scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( self._id_12E7F.objidnum, var_2 );
            continue;
        }
...
init_rallyvehicles()
{
    ...
    if ( !istrue( level.disablelittlebirdrally ) )
    {
        foreach ( var_2 in var_0 )
        {
            ...
            if ( level.little_bird_mg_exitendinternal )
                var_6 = _purchasemenuclosedbyclient::train_stopper( var_4, var_5 );
            else
    ...
    {
        if ( level.localeid == "locale_6" && level.mapname == "mp_downtown_gw" && istrue( level.unset_heavy_hitter ) )
        {
            var_10 = [];
        ...
        foreach ( var_2 in var_0 )
        {
            ...
            level thread _id_1388D( var_6, "neutral" );
        }
    }
...
arm_initoutofbounds()
{
    ...
    foreach ( var_2 in var_0 )
    {
        if ( isdefined( scripts\cp_mp\utility\game_utility::getlocaleid() ) && isdefined( var_2.script_noteworthy ) && var_2.script_noteworthy == level.localeid && scripts\mp\utility\game_utility_mp::vehicle_isenemytoplayer( var_2 ) )
        {
