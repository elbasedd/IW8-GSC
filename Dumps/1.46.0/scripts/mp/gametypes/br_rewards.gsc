...
initdropbagsystem()
{
    ...
    if ( isdefined( level.dropbagstruct ) )
        return;
...
_id_1211B( var_0 )
{
    ...
    foreach ( var_7 in var_5 )
    {
        if ( level.teamdata[var_7]["teamCount"] > 0 )
        {
            ...
            {
                if ( !isdefined( var_9 ) && !istrue( var_11.gulag ) && !istrue( var_11.spawn_maint_wave_2 ) )
                    var_9 = var_11;

                if ( istrue( var_11.issquadleader ) && !istrue( var_11.gulag ) && !istrue( var_11.spawn_maint_wave_2 ) )
                {
                    var_8 = 1;
    ...
    if ( isdefined( var_21 ) )
    {
        for ( var_22 = 0; var_22 < var_21.size; var_22++ )
        {
            ...
            if ( isdefined( var_21[var_22].node ) )
            {
                ...
                var_11._id_12119 = var_24.droplocations[var_25].origin;
                continue;
            }

            var_11._id_12119 = var_21[var_22].origin;
        }

        level._id_1211A = 1;
        level thread _id_12E23( var_1 );
    }

    level thread unload_all_vehicles_in_module();
}

unload_all_vehicles_in_module()
{
...
_id_12E22( var_0, var_1, var_2 )
{
    getarenaomnvarbitpackinginfo( self.team, 1 );
    var_3 = 4096;
    var_4 = scripts\engine\utility::getstruct( "soa_tower_elevator_floor_3", "targetname" );

    if ( istrue( self.spawn_ending_individual_guys_death ) || isdefined( var_4 ) && distance2d( var_0, var_4.origin ) < 5000 )
        var_3 = 10000;
    ...
    get_grenade_settings_from_group( var_5 );
    ...
    if ( !scripts\mp\gametypes\br_public.gsc::spreadshotdamagemod() )
        scripts\mp\gametypes\br_public.gsc::decrement_num_of_frame_frozen( "dropbag_incoming", self.team, 1 );

    if ( scripts\mp\utility\game::getgametype() == "br" )
    {
        scripts\mp\gametypes\br_analytics.gsc::currenttime_bonus( self, var_0, var_1, var_5 );
        thread regroup_trigger( var_5 );
    }
}

regroup_trigger( var_0 )
{
    var_0 endon( "death" );
    wait 1.0;
    ...
    while ( isdefined( var_0 ) )
    {
        ...
        if ( abs( var_2[2] ) < 0.01 )
        {
            ...
            if ( var_5["fraction"] == 1 )
                var_0 scripts\cp_mp\killstreaks\airdrop::hack_console_activation_func( var_0.origin, ( 0, 0, -10 ) );
            else
                break;
        }
...
_id_12E23( var_0 )
{
    foreach ( var_2 in var_0 )
    {
        ...
        if ( isdefined( level.br_circle ) && isdefined( level.br_circle.safecircleent ) )
        {
            ...
            else if ( istrue( level._id_137F3 ) && isscriptabledefined() )
                var_3 = getclosestpointonnavmesh( var_3 );

            if ( istrue( level._id_137DA ) )
                var_3 = plunder_awarded_by_missions_total( var_3 );

            if ( scripts\mp\outofbounds::ispointinoutofbounds( var_3 ) )
            {
                var_4 = scripts\mp\gametypes\br_circle.gsc::getsafecircleorigin();
                var_7 = scripts\mp\gametypes\br_c130.gsc::_id_12C37( var_4, var_3 );
        ...
        if ( isdefined( level.publiceventdropbagsplashoverride ) )
            var_8 = level.publiceventdropbagsplashoverride;

        var_9 = playerplunderbankcallback( 1, 0, 0, 0, 0 );
        var_2 _id_12E22( var_3, var_9, var_8 );
        waitframe();
    }
}

plunder_awarded_by_missions_total( var_0 )
{
    if ( isdefined( level.lastseentime ) )
    {
        if ( !spawner_poison_structs( var_0, level.lastseentime ) )
        {
            var_1 = playerhumanprestream( var_0, level.lastseentime );
...
spawner_poison_structs( var_0, var_1 )
{
...
playerhumanprestream( var_0, var_1 )
{
    var_2 = undefined;
    var_3 = undefined;
...
spawndropbagonlanding()
{
    ...
    if ( isdefined( var_0 ) )
    {
        var_3 = playerplunderbankcallback( 0, 0, 0, 1, 0 );
        _id_12E22( var_0.origin, var_3 );
        level thread unload_all_vehicles_in_module();
    }
}
...
_id_133E8( var_0 )
{
    level thread _id_133E9( var_0, self );
}

_id_133E9( var_0, var_1 )
{
    level endon( "game_ended" );
    var_0 endon( "death" );
    var_2 = var_1;
...
get_grenade_settings_from_group( var_0 )
{
...
initarmsraceanims( var_0 )
{
    ...
    foreach ( var_3 in var_0 )
    {
        ...
        if ( level.createpropspecatehud )
            scripts\mp\gametypes\br_armory_kiosk.gsc::_id_129A0( "supply_drop", 0 );
    }
}

bottompercentagetoadjusteconomy()
{
    if ( scripts\mp\gametypes\br_gametypes.gsc::spawn_juggernauts_fob( "dropbag" ) )
        return;
...
playerplunderbankcallback( var_0, var_1, var_2, var_3, var_4 )
{
    if ( istrue( var_0 ) )
        return 0;
