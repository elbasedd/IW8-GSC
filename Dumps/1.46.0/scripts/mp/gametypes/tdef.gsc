...
main()
{
    ...
    level.init_relic_noluck = [];
    var_0 = getentarray( "cyber_emp_pickup_trig", "targetname" );

    foreach ( var_2 in var_0 )
        level.init_relic_noluck[level.init_relic_noluck.size] = var_2.origin;

    level.init_relic_noks = ( 0, 0, 0 );
    ...
    foreach ( var_6 in var_4 )
    {
        ...
        {
            level.init_relic_noks = var_6.origin;
            break;
        }
    }
    ...
    level.init_relic_punchbullets = 1;
    level.init_relic_nuketimer = [];
    level.scorefrozenuntil = 0;
    level.init_relic_oneclip = 0;
...
updategametypedvars()
{
    ...
    level._id_1210A = scripts\mp\utility\dvars::dvarintvalue( "ppkTeamNoFlag", 50, 0, 250 );
    level._id_1210B = scripts\mp\utility\dvars::dvarintvalue( "ppkTeamWithFlag", 100, 0, 250 );
    level._id_12109 = scripts\mp\utility\dvars::dvarintvalue( "ppkFlagCarrier", 250, 0, 250 );
    ...
    level.nextdest = scripts\mp\utility\dvars::dvarfloatvalue( "flagActivationDelay", 10, 0, 30 );
...
initspawns()
{
    ...
    foreach ( var_3 in level.spawnpoints )
        engagementtime( var_3 );
}

engagementtime( var_0 )
{
    var_0.scriptdata.isvalidinflictorc4vehicle = undefined;
    var_1 = getpathdist( var_0.origin, level.init_relic_noluck[0], 1000 );

    if ( var_1 < 0 )
        var_1 = scripts\engine\utility::distance_2d_squared( var_0.origin, level.init_relic_noluck[0] );
    else
        var_1 = var_1 * var_1;

    var_0.scriptdata.isvalidinflictorc4vehicle = var_1;
}
...
getspawnpoint()
{
    ...
    {
        var_3 = [];
        var_3["defenderFlagPosition"] = level.init_relic_nobulletdamage.visuals[0].origin;

        if ( isdefined( level.init_relic_nobulletdamage.carrier ) )
            var_3["activeCarrierPosition"] = level.init_relic_nobulletdamage.carrier.origin;
        else
...
tdef()
{
    ...
    level.vehicle_damage_onexitstateheavy = "waypoint_mlg_empty_flag";
    level.vehicle_damage_onexitstatelight = "waypoint_mlg_full_flag";
    level.icontarget = "waypoint_target";
    have_target_hud_turned_on();
}
...
have_target_hud_turned_on()
{
    level.init_relic_noluck = scripts\engine\utility::array_randomize( level.init_relic_noluck );
    var_0 = level.init_relic_noluck[0] + ( 0, 0, 64 );
    var_1 = level.init_relic_noluck[0] + ( 0, 0, -64 );
    var_2 = scripts\engine\trace::ray_trace( var_0, var_1, undefined, scripts\engine\trace::create_default_contents( 1 ) );
    level.init_relic_noluck[0] = var_2["position"];
    level.init_relic_nobulletdamage = heli_convertforplayerfunc( "allies" );
    level thread nextplayertospectate();
}
...
nextplayertospectate()
{
    ...
    {
        level.init_relic_noregen.x1circletime scripts\mp\gameobjects::setvisibleteam( "none" );
        level scripts\engine\utility::_id_13AD9( "prematch_done", "start_mode_setup" );
        level.init_relic_noregen.x1circletime scripts\mp\gameobjects::setvisibleteam( "any" );
    }

    if ( level.nextdest )
    {
        scripts\mp\flags::gameflagwait( "prematch_done" );
        level.init_relic_noregen.x1circletime thread scripts\mp\gametypes\obj_zonecapture.gsc::trial_start_time( level.nextdest, level.init_relic_nobulletdamage.curorigin + level.init_relic_nobulletdamage.offset3d );
        wait( level.nextdest );
        level.init_relic_noregen.x1circletime scripts\mp\gameobjects::_id_129B5( level.iconcaptureflag, level.iconcaptureflag, level.vehicle_damage_onexitstatelight );
        level.init_relic_nobulletdamage.trigger scripts\engine\utility::trigger_on();
...
heli_convertforplayerfunc( var_0 )
{
    ...
    level.returntime = 0;
    var_1 = 32;
    var_2 = spawn( "trigger_radius", level.init_relic_noluck[0], 0, var_1, 128 );
    var_3 = [];
    var_3[0] = spawn( "script_model", level.init_relic_noluck[0] );
    ...
    var_5 scripts\mp\gameobjects::_id_129B5( level.iconescort, level.iconkill, level.vehicle_damage_onexitstatelight );
    ...
    var_5 scripts\mp\gameobjects::_id_123B7( ::nextswitch );
    ...
    if ( isdefined( level.showenemycarrier ) )
    {
        ...
        var_5 scripts\mp\gameobjects::_id_129B5( level.iconescort, level.iconkill, level.vehicle_damage_onexitstatelight, var_6 );
    }
    else
    {
        var_5.objidpingfriendly = 1;
    ...
    level.init_relic_noregen = heli_counter( var_0, var_5 );
    return var_5;
}
...
heli_counter( var_0, var_1 )
{
    ...
    var_3.x1circletime = scripts\mp\gameobjects::createobjidobject( var_2, "neutral", ( 0, 0, 85 ), undefined, "any", 0 );
    var_3.x1circletime scripts\mp\gameobjects::setvisibleteam( "any" );

    if ( level.nextdest )
    {
        var_1.trigger scripts\engine\utility::trigger_off();
        var_3.x1circletime scripts\mp\gameobjects::_id_129B5( level.icontarget, level.icontarget, level.vehicle_damage_onexitstatelight );
    }
    else
        var_3.x1circletime scripts\mp\gameobjects::_id_129B5( level.iconcaptureflag, level.iconcaptureflag, level.vehicle_damage_onexitstatelight );
...
onpickup( var_0, var_1, var_2 )
{
    ...
    level.init_relic_noregen.x1circletime scripts\mp\gameobjects::setvisibleteam( "none" );
    level.init_relic_nobulletdamage.helperdrone_isbeingpingedbydrone = var_0;
    var_0 thread cancelupcomingpublicevents();
    var_0 thread nextfixupcheckms();
    ...
    if ( isdefined( level.showenemycarrier ) )
    {
        ...
        else
        {
            ...
            scripts\mp\objidpoolmanager::wp_loop( self.pingobjidnum, 1 );
    ...
    scripts\mp\gameobjects::_id_129B5( level.iconescort, level.iconkill, level.vehicle_damage_onexitstatelight );
    ...
    var_0 thread scripts\common\utility::_id_13592( level.update_objective_mlgicon_reset, "pickup", var_0.origin );
...
ondrop( var_0 )
{
    ...
    level.init_relic_nobulletdamage.helperdrone_isbeingpingedbydrone = undefined;
    ...
    if ( level.returntime >= 0 )
        scripts\mp\gameobjects::_id_129B5( level.iconreturnflag, level.iconreturnflag, level.vehicle_damage_onexitstatelight );
    else
    {
        scripts\mp\gameobjects::_id_129B5( level.iconreturnflag, level.iconreturnflag, level.vehicle_damage_onexitstatelight );
        scripts\mp\objidpoolmanager::wp_loop( self.objidnum, 1 );
    }
...
onreset()
{
    ...
    if ( isdefined( level.init_relic_nobulletdamage._id_120BF ) )
    {
        level.init_relic_nobulletdamage._id_120BF clearportableradar();
        level.init_relic_nobulletdamage._id_120BF delete();
    }
...
cancelupcomingpublicevents( var_0 )
{
    ...
    level.init_relic_nobulletdamage endon( "dropped" );
    level.init_relic_nobulletdamage endon( "reset" );
    ...
    while ( !level.gameended )
    {
        ...
        if ( !level.gameended )
        {
            level.init_relic_nobulletdamage.carrier scripts\mp\utility\stats::incpersstat( "objTime", 1 );
            level.init_relic_nobulletdamage.carrier scripts\mp\persistence::statsetchild( "round", "objTime", level.init_relic_nobulletdamage.carrier.pers["objTime"] );
            level.init_relic_nobulletdamage.carrier scripts\mp\utility\stats::setextrascore0( level.init_relic_nobulletdamage.carrier.pers["objTime"] );
            level.init_relic_nobulletdamage.carrier scripts\mp\gamescore::giveplayerscore( "tdef_hold_obj", 10 );
        }
    }
}

nextfixupcheckms( var_0 )
{
    level endon( "game_ended" );
    level.init_relic_nobulletdamage endon( "dropped" );
    level.init_relic_nobulletdamage endon( "reset" );
    ...
    var_1 = playerpowerresetpowers( var_0 );
    var_2 = spawn( "script_model", level.init_relic_nobulletdamage.visuals[0].origin );
    var_2.team = scripts\mp\utility\game::getotherteam( var_0 )[0];
    var_2.owner = var_1;
    var_2 makeportableradar( var_1 );
    level.init_relic_nobulletdamage._id_120BF = var_2;
    level.init_relic_nobulletdamage thread nexttrackindex();
    level.init_relic_nobulletdamage thread ninetypercent_music();
}
...
nexttrackindex()
{
    level endon( "game_ended" );
    self endon( "dropped" );
    self._id_120BF endon( "death" );
    level endon( "portableRadarRunning" );

    for (;;)
    {
        self._id_120BF moveto( self.helperdrone_isbeingpingedbydrone.origin, 0.05 );
        wait 0.05;
    }
}
...
ninetypercent_music()
{
    level endon( "game_ended" );
    self endon( "dropped" );
    var_0 = self._id_120BF.team;
    var_0 = scripts\mp\utility\game::getotherteam( var_0 )[0];
    self._id_120BF.owner scripts\engine\utility::_id_13ADA( "disconnect", "joined_team", "joined_spectators" );
    self._id_120BF clearportableradar();
    self._id_120BF = undefined;
    nextfixupcheckms( var_0 );
}

getrespawndelay()
{
    var_0 = level.init_relic_nobulletdamage scripts\mp\gameobjects::getownerteam();
...
onplayerkilled( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9 )
{
    ...
    var_10 = level._id_1210A;

    if ( isdefined( level.init_relic_nobulletdamage ) && level.init_relic_nobulletdamage scripts\mp\gameobjects::getownerteam() == var_1.pers["team"] )
    {
        if ( isdefined( level.init_relic_nobulletdamage.carrier ) && var_1 != level.init_relic_nobulletdamage.carrier )
        {
            level.init_relic_nobulletdamage.carrier thread scripts\mp\rank::scoreeventpopup( "carrier_bonus" );
            var_11 = scripts\mp\rank::getscoreinfovalue( "carrier_bonus" );
            scripts\mp\gamescore::giveplayerscore( "carrier_bonus", var_11, self );
            level.init_relic_nobulletdamage.carrier thread scripts\mp\rank::giverankxp( "carrier_bonus", var_11 );
        ...
        var_10 = level._id_1210B;
    }
    else if ( isdefined( self.carryflag ) )
        var_10 = level._id_12109;
    ...
    if ( isdefined( var_1 ) && isplayer( var_1 ) && var_1.pers["team"] != self.pers["team"] )
    {
        ...
        if ( isdefined( self.carryflag ) )
        {
            ...
            thread scripts\common\utility::_id_13592( level.update_readings, var_9, "carrying" );
        ...
        if ( !var_12 )
        {
            var_15 = 0;
            var_16 = 0;
            var_17 = distsquaredcheck( var_13, self.origin, level.init_relic_nobulletdamage.curorigin );

            if ( var_17 )
            {
                if ( level.init_relic_nobulletdamage.ownerteam == self.team )
                    var_15 = 1;
                else
                    var_16 = 1;
            }
            ...
            if ( var_15 )
            {
                ...
                thread scripts\common\utility::_id_13592( level.update_readings, var_9, "defending" );
            ...
            else if ( var_16 )
            {
                ...
                var_1 scripts\mp\utility\stats::incpersstat( "defends", 1 );
                var_1 scripts\mp\persistence::statsetchild( "round", "defends", var_1.pers["defends"] );
                thread scripts\common\utility::_id_13592( level.update_readings, var_9, "assaulting" );
            }
        }
    }
