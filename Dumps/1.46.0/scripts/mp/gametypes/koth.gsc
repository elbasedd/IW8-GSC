...
updategametypedvars()
{
    ...
    level._id_11B30 = scripts\mp\utility\dvars::dvarintvalue( "pauseTime", 1, 0, 1 );
    ...
    if ( scripts\mp\utility\game::matchmakinggame() && !scripts\mp\utility\game::isanymlgmatch() && getdvarint( "scr_koth_playlistZoneActivationDelay", 15 ) != 0 )
        level.zoneactivationdelay = attachedvelocity();
}
...
hardpointmainloop()
{
    ...
    scripts\mp\utility\game::trial_targs_combo( undefined, undefined, "New Hardpoint", level.zone.trigger.origin );
    level.zone thread scripts\common\utility::_id_13592( level.update_objective_mlgicon_reset, "hill_moved", level.zone.trigger.origin );
    ...
    if ( level.firstzoneactivationdelay )
    {
        ...
        level.zone thread scripts\mp\gametypes\obj_zonecapture.gsc::trial_start_time( level.firstzoneactivationdelay, level.zone.curorigin + level.zone.offset3d );
...
_id_12460()
{
    switch ( level.mapname )
    {
        ...
        case "mp_hackney_yard":
            level._id_12462 = [ "1", "4", "3", "9", "8" ];
            break;
        case "mp_spear":
        case "mp_spear_pm":
            level._id_12462 = [ "5", "4", "3", "2", "6" ];
            break;
        case "mp_cave":
        case "mp_cave_am":
            level._id_12462 = [ "1", "5", "2", "3", "4" ];
            break;
        case "mp_petrograd":
            level._id_12462 = [ "1", "9", "4", "5", "2" ];
            break;
        case "mp_deadzone":
            level._id_12462 = [ "12", "11", "3", "1", "2" ];
            break;
        case "mp_raid":
            level._id_12462 = [ "20", "2", "8", "9", "5" ];
            break;
        case "mp_piccadilly":
            level._id_12462 = [ "1", "5", "7", "3", "8" ];
            break;
        case "mp_crash2_pm":
        case "mp_crash2":
            level._id_12462 = [ "1", "20", "5", "4", "19" ];
            break;
        case "mp_emporium":
            level._id_12462 = [ "1", "2", "3", "4", "5" ];
            break;
        case "mp_broadcast2":
            level._id_12462 = [ "1", "2", "3", "4", "5" ];
            break;
...
getfirstzone()
{
    if ( isdefined( level._id_12462 ) )
    {
        var_0 = level.objectives[level._id_12462[0]];
        level.prevzoneindex = 0;
        level.onping = level.objectives[level._id_12462[1]];
    }
    else
    {
        var_0 = level.objectives["1"];
        level.prevzoneindex = 1;
        level.onping = level.objectives["2"];
    }

    var_0 thread scripts\mp\gametypes\obj_zonecapture.gsc::_id_13BFC();
    level.onping thread scripts\mp\gametypes\obj_zonecapture.gsc::_id_13BFC();
    return var_0;
}
...
getnextzone()
{
    ...
    else
    {
        if ( isdefined( level._id_12462 ) )
        {
            ...
            if ( level.prevzoneindex > level._id_12462.size - 1 )
                level.prevzoneindex = 0;

            var_23 = level.objectives[level._id_12462[level.prevzoneindex]];
            var_30 = level.prevzoneindex + 1;

            if ( var_30 > level._id_12462.size - 1 )
                var_30 = 1;

            level.onping = level.objectives[level._id_12462[var_30]];
        }
        else
        {
            level.prevzoneindex++;
            ...
            level.onping = level.objectives[scripts\engine\utility::string( var_30 )];
        }
...
hqactivatenextzone( var_0, var_1 )
{
    ...
    else if ( level.zoneactivationdelay )
    {
        ...
        level._id_13E3B = 1;
        level thread scripts\mp\hud_message::updatematchstatushintforallplayers( undefined, 36, 36 );
        level.zone thread scripts\mp\gametypes\obj_zonecapture.gsc::trial_start_time( level.zoneactivationdelay, level.zone.curorigin + level.zone.offset3d );
...
hpcaptureloop()
{
    ...
    for (;;)
    {
        ...
        var_0 = level scripts\engine\utility::_id_13AE1( "zone_captured", "zone_destroyed" );
...
initspawns()
{
    ...
    if ( !isdefined( level.spawnpoints ) || istrue( level.attached_bag ) )
    {
...
give_capture_credit( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    level endon( "game_ended" );
    level.lastcaptime = gettime();
    scripts\mp\utility\game::trial_targs_combo( var_4, undefined, "Hardpoint Captured", var_4.origin );
    var_6 = var_4;
    ...
    if ( isplayer( var_6 ) )
    {
        var_6 thread scripts\common\utility::_id_13592( level.update_objective_mlgicon_reset, "capture", var_6.origin );

        if ( spawn_individual_guys_at_pos( var_6 ) && !scripts\mp\utility\game::isanymlgmatch() )
        {
    ...
    while ( !level.gameended )
    {
        ...
        if ( !level.zone.stalemate && !level.gameended )
        {
            ...
            foreach ( var_5 in level.zone.touchlist[var_3] )
            {
                ...
                if ( isdefined( var_5.player.rungwperif_plumes ) && var_5.player.rungwperif_plumes > 3 )
                {
                    var_5.player.rungwperif_plumes = 0;
                ...
                if ( !isdefined( var_5.player.rungwperif_plumes ) )
                    var_5.player.rungwperif_plumes = 0;
                else
                    var_5.player.rungwperif_plumes++;
