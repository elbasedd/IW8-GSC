...
main()
{
    ...
    if ( var_0 )
        level.spawn_module_building_chopper1 = getdvarint( "scr_infect_groundwarInfect", 0 );

    var_1[0] = scripts\mp\utility\game::getgametype();
    var_1[var_1.size] = "grnd";

    if ( istrue( level.spawn_module_building_chopper1 ) )
    ...
    updategametypedvars();
    level.minsteps = 0;
    level.mintokensdropondeath = getdvarint( "scr_infect_time_increase_cap", 20 );
    ...
    level.array_sort_by_script_index = ::select_hostage_room_two_spawners;
    ...
    level.select_hostage_room_one_spawners = getdvarint( "scr_infect_nvgMode", 0 );
    level.select_equipment_spawners = getdvarint( "scr_infect_disable_nvg", 0 );
    level._id_11991 = getdvarint( "scr_infect_onlineprimaryoverride", 0 );

    if ( level.spawn_module_building_chopper1 )
    {
        ...
        level.select_kitchen_spawners = var_3;
        level.br_domheight = getdvarint( "scr_infect_alwaysTacInsertSkySpawn", 0 );
        level._id_12C0E = getdvarint( "scr_infect_skyDiveOnTacInsertPlacement", 1 );
        level.bounceshot = getdvarint( "scr_infect_allowSkyDiveCutParachute", 0 );
        level._id_11BF7 = ::_id_11BF7;
        level._id_131C8 = ::_id_131C8;
    }
...
onstartgametype()
{
    ...
    scripts\cp_mp\utility\script_utility::registersharedfunc( "vehicle_compass", "shouldBeVisibleToPlayer", ::_id_13877 );

    if ( istrue( level.spawn_module_building_chopper1 ) )
    {
        scripts\mp\gametypes\arm.gsc::light_tank_watchmissileinputchange();
        thread set_flag_after_vo();
        thread triggereliminatedoverlay();
    }
...
updategametypedvars()
{
    ...
    level._id_13181 = getdvar( "scr_infect_superSurvivorTwo", "none" );
    ...
    level.select_hostage_room_three_spawners = getdvar( "scr_infect_superInfectTwo", scripts\engine\utility::ter_op( level.spawn_module_building_chopper1, "super_tac_insert", "none" ) );
    ...
    level.select_bunker_server_one_spawners = getdvarint( "scr_infect_infectBonusScore", 150 );
    level.select_bunker_server_two_spawners = getdvarint( "scr_infect_infectBonusSuperOnSpawn", 0 );
    level.select_chopper_boss_target_player = getdvarint( "scr_infect_infectBonusSuperOnTacInsert", 0 );
    level.movedtoinfected = getdvarint( "scr_infect_finalSuvivorCount", 1 );
    level.select_lobby_door_one_spawners = getdvarint( "scr_infect_setRadarOnNumSurvivors", 8 );
    level._id_13180 = [];
    var_0 = getdvar( "scr_infect_survivorStreakOverride", "" );

    if ( var_0 != "" )
        level._id_13180 = strtok( var_0, "," );

    var_1 = scripts\cp_mp\utility\game_utility::getmapname();

    if ( issubstr( var_1, "mp_m_" ) && var_1 != "mp_m_speed" )
    {
        level.survivorsuper = new_col_map( 0, 0 );
        level._id_13181 = new_col_map( 0, 1 );
        level.infectedsuper = new_col_map( 1, 0 );
        level.select_hostage_room_three_spawners = new_col_map( 1, 1 );
    }

    if ( level.survivorsuper == level._id_13181 )
        level._id_13181 = "none";
    else if ( level.survivorsuper == "none" && level._id_13181 != "none" )
    {
        level.survivorsuper = level._id_13181;
        level._id_13181 = "none";
    }

    if ( level.infectedsuper == level.select_hostage_room_three_spawners )
        level.select_hostage_room_three_spawners = "none";
    else if ( level.infectedsuper == "none" && level.select_hostage_room_three_spawners != "none" )
    {
        level.infectedsuper = level.select_hostage_room_three_spawners;
        level.select_hostage_room_three_spawners = "none";
    }
...
buildandloadweapons()
{
    ...
    foreach ( var_1 in level.allweapons )
    {
        if ( var_1 != "none" )
        {
            ...
            if ( level.select_hostage_room_one_spawners )
                game["arenaRandomLoadout"][0]["loadoutPrimaryAttachment"] = prematchinitx1blueprintloadouts();
...
new_col_map( var_0, var_1 )
{
    if ( istrue( var_1 ) )
        var_2 = scripts\engine\utility::ter_op( var_0, level.select_hostage_room_three_spawners, level._id_13181 );
    else
...
onspawnplayer()
{
    ...
    if ( self.pers["team"] == "axis" )
    {
        ...
        if ( istrue( level.spawn_module_building_chopper1 ) )
            self.iskillstreakdeployweapon = 1;
    }
...
onspawnfinished()
{
    ...
    if ( self.pers["team"] == "allies" )
    {
        ...
        if ( level.spawn_module_building_chopper1 )
            thread quarry_fight_across();

        spawnwithplayersecondary();

        if ( level.select_hostage_room_one_spawners && !level.select_equipment_spawners && !isbot( self ) )
            thread weapongetflinchtype();
    }
    else if ( self.pers["team"] == "axis" )
    {
        if ( istrue( level.registeraccesscardlocs ) )
        {
            ...
            self.spawn_module_p3_form_c = 1;
        }

        if ( istrue( level.bounceshot ) )
    ...
    if ( istrue( self.wait_to_close_in ) )
    {
        if ( !level.spawn_module_building_chopper1 || level.spawn_module_building_chopper1 && level.mapname == "mp_aniyah" )
    ...
    if ( istrue( self.spawn_vo_lines ) )
    {
        self.spawn_vo_lines = undefined;
        thread scripts\cp_mp\parachute::select_stadium_three_spawners( 1 );
    }

    thread scripts\mp\supers::givesuperpoints( level.select_bunker_server_two_spawners, undefined, 1 );
    thread _id_13ACD();
}
...
_id_13ACD()
{
    ...
    if ( istrue( level.registeraccesscardlocs ) && self.team == "axis" )
...
setfirstinfected( var_0 )
{
    ...
    while ( !scripts\mp\utility\player::isreallyalive( self ) || scripts\mp\utility\player::isusingremote() || isdefined( self._id_139A5 ) )
        waitframe();
    ...
    if ( isdefined( var_1 ) )
    {
        ...
        var_2.maxplunderextractions = "INVOLUNTARY";
    ...
    if ( var_0 )
    {
        ...
        thread scripts\mp\music_and_dialog::select_cliff_one_spawners();
    }

    self.isinitialinfected = 1;
    scripts\mp\utility\stats::incpersstat( "firstInfected", 1 );
    self.pers["gamemodeLoadout"] = level.infect_loadouts["axis_initial"];
    scripts\mp\equipment\tac_insert::_id_12E66( self.origin, self.angles );
    ...
    if ( isdefined( scripts\mp\supers::getcurrentsuper() ) && scripts\mp\supers::getcurrentsuperref() == "super_deadsilence" )
        thread _id_13015();
    else if ( level.spawn_module_building_chopper1 )
        thread scripts\mp\supers::givesuperpoints( 350, undefined, 1 );

    if ( var_0 )
        level.infect_players[self.name] = 1;

    level.nag_get_in_heli = 1;

    if ( !level.nag_get_in_heli )
...
onplayerkilled( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9 )
{
    ...
    {
        if ( isdefined( level.weapon_xp_iw8_ar_kilo433 ) && self == level.weapon_xp_iw8_ar_kilo433 )
        {
            var_10 = 0;
            var_11 = 0;
        }
    ...
    if ( var_10 )
    {
        ...
        if ( scripts\mp\utility\dvars::getwatcheddvar( "timelimit" ) != 0 )
        {
            ...
            {
                level.minsteps++;
                var_17 = level.minsteps <= level.mintokensdropondeath;
            }
...
{
    self.wait_to_close_in = 1;

    if ( level.spawn_module_building_chopper1 && level.mapname != "mp_aniyah" )
        _id_12984();
    ...
    if ( level.infect_teamscores["allies"] > 1 )
    {
        if ( level.spawn_module_building_chopper1 )
        {
            ...
            if ( level._id_12010 > 3 )
                level._id_12010 = 1;

            if ( level.infect_teamscores["allies"] == level.select_lobby_door_one_spawners )
                level thread _id_13034();
        }

        if ( level._id_12010 == 1 )
        {
        ...
        if ( !var_1 )
        {
            ...
            foreach ( var_5 in level.players )
            {
                ...
                if ( var_5.team == "axis" && var_5 != var_0 && var_5 != self )
                    var_5 thread scripts\mp\utility\points::giveunifiedpoints( "assist", undefined, level.select_bunker_server_one_spawners );
            }
        }
    }
    else if ( level.infect_teamscores["allies"] == level.movedtoinfected )
        onfinalsurvivor();
...
triggereliminatedoverlay()
{
    ...
    while ( level.infect_teamscores["allies"] > level.select_lobby_door_one_spawners )
    {
        foreach ( var_1 in level.players )
        {
            if ( var_1.team == "axis" )
...
enduavonlatejoiner( var_0 )
{
    ...
    for (;;)
    {
        if ( level.infect_teamscores["allies"] > level.movedtoinfected )
        {
...
monitordisconnect()
{
    ...
    if ( isdefined( self.infect_isbeingchosen ) || level.infect_chosefirstinfected )
    {
        if ( level.infect_teamscores["axis"] && level.infect_teamscores["allies"] )
        {
            if ( level.spawn_module_building_chopper1 )
            {
                if ( var_0 == "allies" && level.infect_teamscores["allies"] == level.select_lobby_door_one_spawners )
                    level thread _id_13034();
            }

            if ( var_0 == "allies" && level.infect_teamscores["allies"] == level.movedtoinfected )
                onfinalsurvivor();
...
setspecialloadouts()
{
    ...
    var_0 = 0;
    var_1 = scripts\mp\gametypes\gun.gsc::playerprespawngulagjail( level.survivorprimaryweapon );
    var_0 = scripts\mp\class::_id_131AA( level.survivorprimaryweapon, var_1 );
    var_2 = 0;
    var_3 = scripts\mp\gametypes\gun.gsc::playerprespawngulagjail( level.survivorsecondaryweapon );
    var_2 = scripts\mp\class::_id_131AA( level.survivorsecondaryweapon, var_3 );
    ...
    else if ( level._id_11991 )
    {
        ...
        if ( level._id_13180.size > 0 )
        {
            level.infect_loadouts["allies"]["loadoutKillstreak1"] = level._id_13180[0];
            level.infect_loadouts["allies"]["loadoutKillstreak2"] = level._id_13180[1];
            level.infect_loadouts["allies"]["loadoutKillstreak3"] = level._id_13180[2];
        ...
        level.infect_loadouts["allies"]["loadoutFieldUpgrade2"] = level._id_13181;
    }
    else
    {
        level.infect_loadouts["allies"]["loadoutPrimary"] = level.survivorprimaryweapon;
    ...
    {
        var_6 = scripts\mp\gametypes\gun.gsc::playerprespawngulagjail( level.initialprimaryweapon );
        var_4 = scripts\mp\class::_id_131AA( level.initialprimaryweapon, var_6 );
    }
    ...
    {
        var_7 = scripts\mp\gametypes\gun.gsc::playerprespawngulagjail( level.initialsecondaryweapon );
        var_5 = scripts\mp\class::_id_131AA( level.initialsecondaryweapon, var_7 );
    }
    ...
    else if ( level._id_11991 )
    {
        ...
        level.infect_loadouts["axis_initial"]["loadoutFieldUpgrade2"] = level.select_hostage_room_three_spawners;
    }
    else
    {
        level.infect_loadouts["axis_initial"]["loadoutPrimary"] = level.initialprimaryweapon;
    ...
    {
        var_9 = 0;
        var_10 = scripts\mp\gametypes\gun.gsc::playerprespawngulagjail( level.infectedprimaryweapon );
        var_9 = scripts\mp\class::_id_131AA( level.infectedprimaryweapon, var_10 );
    }
    ...
    else
    {
        ...
        level.infect_loadouts["axis"]["loadoutFieldUpgrade2"] = level.select_hostage_room_three_spawners;
    ...
    for (;;)
    {
        if ( self isnightvisionon() )
        {
            ...
            if ( var_0 >= 10 )
            {
                ...
                scripts\common\utility::bot_pick_dd_zone_with_fewer_defenders( 0 );
                _id_12343();
                scripts\common\utility::bot_pick_dd_zone_with_fewer_defenders( 1 );
                var_1 = 0;
...
_id_131C8()
{
    thread getgametypekillsperhouravg();
}
...
getgametypekillsperhouravg()
{
    ...
    self.loadoutfieldupgrade2 = level.select_hostage_room_three_spawners;
    wait 0.1;
    ...
    if ( isdefined( self ) && scripts\mp\utility\player::isreallyalive( self ) )
    {
        ...
        var_0 = "gamemode";
        var_1 = scripts\mp\class::trap_room_ents();
        var_1 = scripts\mp\class::tread_sfx( var_1, var_0 );
        ...
        scripts\mp\equipment::giveequipment( level.infectedtactical, "secondary" );
        thread scripts\mp\supers::givesuperpoints( level.select_chopper_boss_target_player, undefined, 1 );
    }

    if ( istrue( level._id_12C0E ) )
        thread _id_1312F();
    else
...
set_flag_after_vo()
{
    ...
    var_18 = level.weapon_xp_iw8_sn_crossbow;
...
{
    level endon( "game_ended" );
    level.select_cliff_two_spawners = [];
    level thread _id_12A7C();
    scripts\mp\flags::gameflagwait( "prematch_done" );
    wait( level.droptime );
    ...
    for (;;)
    {
        ...
        if ( isdefined( var_0 ) && scripts\mp\utility\killstreak::currentactivevehiclecount() < scripts\mp\utility\killstreak::maxvehiclesallowed() && level.fauxvehiclecount + var_1 < scripts\mp\utility\killstreak::maxvehiclesallowed() && scripts\cp_mp\killstreaks\airdrop::getnumdroppedcrates() < 8 )
        {
            ...
            var_3 = playersetattractionbesttime();
...
{
    var_0 = level.select_cliff_two_spawners[0];
    level.select_cliff_two_spawners[level.select_cliff_two_spawners.size] = level.select_cliff_two_spawners[0];
    level.select_cliff_two_spawners[0] = undefined;
    level.select_cliff_two_spawners = scripts\engine\utility::array_removeundefined( level.select_cliff_two_spawners );
    return var_0;
}
...
select_hostage_room_two_spawners()
{
...
onsupportboxusedbyplayer( var_0, var_1, var_2, var_3, var_4 )
{
    var_5 = 0;
