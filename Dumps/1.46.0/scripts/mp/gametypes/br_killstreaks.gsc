...
hvt_waittill_pickup_players_gobackup( var_0, var_1, var_2, var_3 )
{
    ...
    foreach ( var_6 in var_4 )
    {
        if ( isdefined( var_6 ) && scripts\mp\utility\player::isreallyalive( var_6 ) && var_6.team != self.team )
            hvt_visual_leaving_callout( var_6, var_2, var_3 );
    }
}

hvt_visual_leaving_callout( var_0, var_1, var_2, var_3 )
{
    if ( isdefined( level.hvt_key_picked_up ) && isdefined( level.hvt_visual_callout ) )
    {
        if ( isdefined( level.hvt_visual_callout[var_1] ) && isdefined( level.hvt_visual_callout[var_1][var_0.guid] ) && gettime() < level.hvt_visual_callout[var_1][var_0.guid] )
            var_2 = 0;
        else
            level.hvt_visual_callout[var_1][var_0.guid] = gettime() + level.hvt_key_picked_up * 1000;
    }

    if ( istrue( var_2 ) && scripts\mp\utility\killstreak::getkillstreakenemyusedialogue( var_1 ) )
    {
        var_4 = scripts\mp\gametypes\br_public.gsc::deadyellow( self );
        var_5 = var_4 + "_enemy_" + var_1 + "_inbound";
        var_0 scripts\mp\utility\dialog::leaderdialogonplayer( var_5, "killstreak_used" );
    }

    var_6 = spawnstruct();
    var_6._id_11D5A = istrue( var_2 );
    var_6.watchparachutersoverhead = var_1;
    ...
    if ( var_1 > 0 )
    {
        ...
        hvtboardingside( var_0, var_3 );
    }

    var_0 scripts\mp\gametypes\br_gametypes.gsc::_id_1268B( "onKillstreakDanger", var_6 );
}

hvtboardingside( var_0, var_1 )
{
    var_2 = var_0 getxuid();
    ...
    level thread _id_13BC4( var_0, var_2, "death_or_disconnect" );
    level thread _id_13BC4( var_0, var_2, undefined, var_1 );
}

_id_13BC4( var_0, var_1, var_2, var_3 )
{
