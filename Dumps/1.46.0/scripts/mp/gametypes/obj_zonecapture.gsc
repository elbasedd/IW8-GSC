...
setupobjective( var_0, var_1, var_2, var_3 )
{
    ...
    if ( istrue( level.registeraccesscardlocs ) )
    {
        if ( scripts\mp\utility\game::getgametype() == "hq" )
            var_4 thread _id_13ACA( var_4 );
        else if ( scripts\mp\utility\game::getgametype() == "grnd" )
            var_4 thread _id_13AC9( var_4 );
    }
...
_id_13ACA( var_0 )
{
    ...
    playfx( level.scalertoscaleinfo["ghostcat_hw"], var_3 + var_2 );
}

_id_13AC9( var_0 )
{
    scripts\mp\flags::gameflagwait( "prematch_done" );
    playfx( level.scalertoscaleinfo["blood_floor_hw"], getgroundposition( var_0.trigger.origin, 4 ) + ( 0, 0, 2 ) );
}
...
assignchevrons( var_0, var_1 )
{
    ...
    var_2 = _id_1249F( var_2, var_1 );
    var_2 = _id_120D9( var_2, var_1 );
    var_3 = [];
...
activatezone()
{
    ...
    if ( isdefined( level.players_weapon_fired_monitor ) )
        var_0 = [[ level.players_weapon_fired_monitor ]]();

    var_1 = 1024;

    if ( isdefined( level.playerscoreeventref ) )
        var_1 = [[ level.playerscoreeventref ]]();
...
cleanupgametypevips()
{
    level scripts\engine\utility::_id_13AD9( "game_ended", "zone_moved" );
...
zone_onuse( var_0 )
{
    ...
    if ( level.usehqrules && !level.usehprules && self.ownerteam != "neutral" )
    {
        ...
        if ( isdefined( var_3 ) )
        {
            ...
            foreach ( var_11 in var_9 )
            {
                ...
                var_7 scripts\cp\utility\cp_controlled_callbacks::zone_get_node_nearest_2d_bounds();
            }
        }
    ...
    if ( istrue( level.registeraccesscardlocs ) )
        playfx( level.scalertoscaleinfo["vanish_hw_fr"], level.zone.trigger.origin + ( 0, 0, 40 ) );
}
...
zone_onuseupdate( var_0, var_1, var_2, var_3 )
{
    ...
    if ( var_4 == "neutral" && self.claimteam != var_4 )
        attach_turret( self.claimteam );
    else if ( var_4 != "neutral" && self.claimteam != var_4 )
        attach_turret( var_4, 1 );
    else if ( level._id_11B30 )
        level scripts\mp\gamelogic::pausetimer();
...
zone_oncontested()
{
    ...
    level.zone thread scripts\common\utility::_id_13592( level.update_objective_mlgicon_reset, "hill_contested", level.zone.trigger.origin );
    self.didstatusnotify = 1;
}
...
zone_onuncontested( var_0 )
{
    ...
    if ( var_0 == "none" || var_1 == "neutral" )
    {
        ...
        level.zone thread scripts\common\utility::_id_13592( level.update_objective_mlgicon_reset, "hill_empty", level.zone.trigger.origin );
    }
    else
    {
        if ( scripts\mp\utility\game::getgametype() == "koth" )
        ...
        level.zone thread scripts\common\utility::_id_13592( level.update_objective_mlgicon_reset, "hill_uncontested", level.zone.trigger.origin );
    }

    var_7 = ( gettime() - self.given_achievement ) * 0.001;
    scripts\mp\utility\game::trial_targs_combo( undefined, undefined, "Zone Contested", level.zone.trigger.origin, var_7 + " seconds" );
...
_id_13BFC()
{
    ...
    foreach ( var_2 in level.teamnamelist )
    {
        self.onmatchstartbr[var_2] = 0;
        self.onnewequipmentpickup[var_2] = [];
    }

    self.onmatchstartbr["neutral"] = 0;
    self.onnewequipmentpickup["neutral"] = [];
    self.onnewequipmentpickup["none"] = [];
    self.gasfxair = 1;
    ...
    for (;;)
    {
        ...
        if ( scripts\mp\utility\player::isreallyalive( var_4 ) && !isdefined( var_4.touchtriggers[var_0] ) )
        {
            ...
            self.onmatchstartbr[var_2]++;
            var_6 = var_4.guid;
            var_7 = spawnstruct();
            var_7.player = var_4;
            var_7.starttime = gettime();
            self.onnewequipmentpickup[var_2][var_6] = var_7;
            var_4.rungwperif_plumes = 0;
        }

        if ( scripts\mp\utility\player::isreallyalive( var_4 ) && !isdefined( var_4.touchtriggers[var_0] ) )
            var_4 thread onpickupitem( self );
    }
}
...
onpickupitem( var_0 )
{
    ...
    if ( isdefined( self ) )
    {
        var_0.onnewequipmentpickup[var_1][self.guid] = undefined;
        self.rungwperif_plumes = 0;
    }
    else
    {
        var_2 = [];

        foreach ( var_5, var_4 in var_0.onnewequipmentpickup[var_1] )
        {
        ...
        foreach ( var_5 in var_2 )
            var_0.onnewequipmentpickup[var_1][var_5] = undefined;
    }

    var_0.onmatchstartbr[var_1]--;
}
