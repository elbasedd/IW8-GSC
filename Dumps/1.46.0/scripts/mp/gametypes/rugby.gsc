...
main()
{
    ...
    level.vehicle_is_ambient = ::fake_hit_target_monitor;
...
initrules()
{
    ...
    updategametypedvars();
    level.ontimelimitgraceperiod = playersetwasingulag();
...
initrugbyents()
{
    level.siege_bot_team_had_advantage = scripts\engine\utility::array_combine( level.outofboundstriggers, level.minetriggers, level.hurttriggers, level.radtriggers );
    initendzoneents();
...
initjuggcratesetupents()
{
    ...
    foreach ( var_3 in var_1 )
    {
        var_3 = never_unloaded_from_vehicle( var_3 );
...
never_unloaded_from_vehicle( var_0 )
{
...
modeonteamchangedeath( var_0 )
{
    if ( isdefined( level._id_1265A ) )
        var_0 setclientomnvar( "ui_rugby_jugg_friendly", scripts\engine\utility::ter_op( var_0.team == level._id_1265A.team, 0, 1 ) );
}
...
onplayerspawned( var_0 )
{
    ...
    if ( isdefined( level._id_1265A ) && isdefined( level._id_1265A.team ) && isdefined( var_0.team ) )
        var_0 setclientomnvar( "ui_rugby_jugg_friendly", scripts\engine\utility::ter_op( var_0.team == level._id_1265A.team, 0, 1 ) );
}
...
awardgenericmedals( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9 )
{
    ...
    if ( isdefined( level._id_1265A ) )
    {
        ...
        {
            if ( var_1 == level._id_1265A )
                var_1 thread scripts\mp\utility\points::giveunifiedpoints( "kill_as_juggernaut" );
            else if ( var_12 == level._id_1265A )
                var_1 thread scripts\mp\utility\points::giveunifiedpoints( "kill_juggernaut" );
            else if ( var_1.team == level._id_1265A.team && var_1 != level._id_1265A )
            {
                var_16 = distancesquared( level._id_1265A.origin, var_14 );
            ...
            else if ( var_1.team != level._id_1265A.team && var_12.team == level._id_1265A.team )
            {
                var_16 = distancesquared( level._id_1265A.origin, var_13 );

                if ( var_16 < 105625 )
                {
                    var_1 thread scripts\mp\rank::scoreeventpopup( "assault" );
                    var_1 thread scripts\mp\awards::givemidmatchaward( "mode_x_assault" );
                    thread scripts\common\utility::_id_13592( level.update_keypad_currentdisplay_models, var_9, "assaulting" );
                }
            }
        }
    }
...
ontimelimitot()
{
    if ( level.rugby.activejuggernauts.size > 0 )
    {
        ...
        {
            setomnvar( "ui_overtime_time", gettime() + playersetwasingulag() * 1000 );
            level.rugby.inot = 1;
            level.canprocessot = 1;
            _id_1299D();
            level thread start_vault_assault_retrieve_saw();
        }
    }
    else
    {
...
_id_12A5C( var_0 )
{
    ...
    var_0 thread burst_fire_turret( var_1, var_0.trigger.targetname );
...
heli_extraction( var_0, var_1, var_2 )
{
    var_0.origin = var_1;
    var_0.angles = var_2;
    return var_0;
...
activatenewjuggernaut( var_0 )
{
    ...
    level thread _id_11D45( var_0.team, var_6 );
...
deactivatejuggernaut( var_0 )
{
    ...
    level.rugby.kiosksearchradiusidealmin = undefined;
    level.rugby._id_12186 = undefined;
    level.rugby.startjuggdelivery = undefined;
    level.rugby.startingteamcount = undefined;
...
watchjuggprogress()
{
    ...
    for (;;)
    {
        ...
        if ( var_9 > 0.8 )
            level thread get_chopper_minigun_start_node( var_9, var_0, var_2 );
        ...
        if ( isdefined( level._id_1265A ) )
        {
            ...
            {
                var_7 = var_9;
                level._id_1265A thread scripts\mp\utility\points::giveunifiedpoints( "rugby_obj_push" );
            }
        }
...
get_chopper_minigun_start_node( var_0, var_1, var_2 )
{
    if ( !isdefined( level.rugby.kiosksearchradiusidealmin ) )
    {
        level.rugby.kiosksearchradiusidealmin = 1;
        level.rugby._id_12186 = gettime();
        level.rugby.startjuggdelivery = var_0;
        level.rugby.startingteamcount = var_0;
    }
    else if ( isdefined( level.rugby._id_12186 ) && level.rugby._id_12186 + 30000 < gettime() )
    {
        level.rugby.kiosksearchradiusidealmin = 1;
        level.rugby._id_12186 = gettime();
        level.rugby.startjuggdelivery = var_0;
    ...
    if ( istrue( level.rugby.kiosksearchradiusidealmin ) )
    {
    ...
    level.rugby.kiosksearchradiusidealmin = 0;
}
...
handlejuggjumpspam()
{
    ...
    for (;;)
    {
        ...
        for (;;)
        {
            var_2 = scripts\engine\utility::_id_13AED( 1, "jugg_jumped" );
...
{
    var_0 = 0;
    level.startlastsurvivorsuavsweep = spawnstruct();
    level.startlastsurvivorsuavsweep.objidnum = scripts\mp\objidpoolmanager::requestreservedid( var_0 );
}
...
activatejuggcrate( var_0, var_1, var_2 )
{
    ...
    if ( !squadlink_outline_id( var_0 ) )
    {
        var_0 = level.hack_started;
        var_1 = level.hack_laser_trap_control;
    }
...
squadlink_outline_id( var_0 )
{
    var_1 = 1;

    foreach ( var_3 in level.siege_bot_team_had_advantage )
    {
...
createjuggcrate( var_0, var_1, var_2 )
{
    ...
    if ( istrue( var_2 ) )
    {
        level.hack_started = var_0;
        level.hack_laser_trap_control = var_1;
...
juggcratemanageuse( var_0 )
{
    ...
    var_1.sq_ontimerexpired = 1;
    self._id_137DF = scripts\mp\gameobjects::createholduseobject( "neutral", var_1, self.visuals, ( 0, 0, 64 ) );

    foreach ( var_3 in level.teamnamelist )
        self._id_137DF.teamprogress = undefined;

    self._id_137DF.trigger._id_137F4 = 16900;
    self._id_137DF.trigger setuserange( 130 );
    self._id_137DF.trigger sethintstring( &"MP_MODE_RUGBY/CRATE_USE" );
    self._id_137DF.trigger makeusable();
    self._id_137DF.trigger setcursorhint( "HINT_BUTTON" );
    self._id_137DF.trigger setuseholdduration( "duration_none" );
    self._id_137DF.trigger setusehideprogressbar( 1 );
    self._id_137DF.trigger setusepriority( -3 );
    self._id_137DF.trigger sethintonobstruction( "hide" );
    self._id_137DF scripts\mp\gameobjects::setusetime( playersetomnvarattraction() );
    self._id_137DF.interactteam = "any";
    self._id_137DF.curprogress = 0;
    self._id_137DF.defaultusetime = self._id_137DF._id_13805;
    self._id_137DF.userate = 1;
    self._id_137DF.id = "rugby_jugg";
    self._id_137DF.exclusiveuse = 0;
    self._id_137DF.exclusiveclaim = 0;
    self._id_137DF.skiptouching = 1;
    self._id_137DF.onbeginuse = ::_id_12657;
    self._id_137DF.onuse = ::_id_12656;
    self._id_137DF.onenduse = ::_id_12658;
    self._id_137DF.oncantuse = ::_id_12659;
    self._id_137DF.inuse = 0;

    foreach ( var_6 in level.teamnamelist )
    {
        self._id_137DF.numtouching[var_6] = 0;
        self._id_137DF.touchlist[var_6] = [];
    }

    self._id_137DF.cancontestclaim = 0;
    self._id_137DF.stalemate = 0;
    self._id_137DF.wasstalemate = 0;
    self._id_137DF.cancontestclaim = 0;
    self._id_137DF.majoritycapprogress = 1;
    self._id_137DF.wasmajoritycapprogress = 0;
    self._id_137DF.resetprogress = 1;

    if ( !scripts\mp\flags::gameflag( "prematch_done" ) )
        level scripts\engine\utility::_id_13AD9( "prematch_done", "start_mode_setup" );

    self._id_137DF.type = "useObject";
    self._id_137DF.offset3d = ( 0, 0, 32 );
    self._id_137DF scripts\mp\gameobjects::requestid( 1, 1 );
    self._id_137DF scripts\mp\gameobjects::setobjectivestatusicons( "icon_waypoint_jugg_crate" );
    self._id_137DF scripts\mp\gameobjects::setvisibleteam( "any" );
    scripts\mp\objidpoolmanager::objective_set_play_intro( self._id_137DF.objidnum, 0 );
    scripts\mp\objidpoolmanager::objective_set_play_outro( self._id_137DF.objidnum, 0 );
}
...
_id_12656( var_0 )
{
    ...
    if ( isdefined( self._id_137DF ) )
        self._id_137DF delete();
...
fake_hit_target_monitor( var_0 )
{
    ...
    var_2 = var_0 scripts\mp\juggernaut::start_safehouse_quarry( var_1 );
...
updateoverheadcamerapos( var_0 )
{
    ...
    var_10 = scripts\cp_mp\utility\killstreak_utility::playerspawn();
