...
spawnexfilchopper( var_0, var_1, var_2 )
{
...
tr_vehiclesearchparams( var_0, var_1, var_2, var_3 )
{
...
playerent( var_0, var_1, var_2 )
{
    var_3 = tr_vehiclesearchparams( var_0, var_1, var_2.origin, var_2.radius );

    if ( !isdefined( var_3 ) )
        return;
    else if ( !isarray( var_3 ) )
    {
        if ( spawnexfilchopper( var_3, var_0, var_1 ) )
            return var_3;
    }
    else
    {
        var_4 = spawnexfilchopper( var_3[0], var_0, var_1 );
        var_5 = spawnexfilchopper( var_3[1], var_0, var_1 );
...
_id_122CF( var_0, var_1, var_2 )
{
    var_3 = [];

    foreach ( var_5 in var_2 )
    {
        var_6 = playerent( var_0, var_1, var_5 );
...
_id_12C37( var_0, var_1, var_2 )
{
    if ( !isdefined( level.outofboundstriggers ) || level.outofboundstriggers.size == 0 )
        return snappointtomapbounds2d( var_1 );

    if ( !isdefined( level._id_11A82 ) )
        level._id_11A82 = level.outofboundstriggers;

    var_3 = physics_createcontents( [ "physicscontents_playertrigger" ] );
    var_4 = scripts\engine\trace::ray_trace_ents( var_0, var_1, level._id_11A82, var_3 );
    var_5 = 0;
    ...
    if ( isdefined( level._id_11A83 ) && level._id_11A83.size > 0 && getdvarint( "scr_br_spawnOOBKillswitch", 0 ) == 0 )
    {
        var_6 = _id_122CF( var_0, var_1, level._id_11A83 );
...
stack_patch_waittill_context_patch( var_0 )
{
    if ( !isdefined( level.br_level ) )
        return 1;

    var_1 = level.br_level.br_mapcenter;
    var_2 = playjumpsoundtosquad();
...
spawnc130pathstructnew( var_0, var_1 )
{
    ...
    var_3 = playjumpsoundtosquad( var_1 );
    var_4 = randomfloat( 360 );

    if ( scripts\mp\gametypes\br_public.gsc::spawn_apache_chopper() )
    {
        if ( isdefined( level.select_stadium_two_spawners ) )
            var_4 = level.select_stadium_two_spawners;
        else
            var_4 = 75;
    }
    ...
    var_5 = var_5 + ( 0, 0, playergetbestdropbagorigin() );

    if ( !scripts\mp\gametypes\br_gametypes.gsc::spawn_juggernauts_fob( "randomizeCircleCenter" ) )
        var_2 = var_2 + var_5;
    ...
    if ( isdefined( level.createquestcircle ) )
        var_2 = level.createquestcircle;

    if ( scripts\mp\gametypes\br_gametypes.gsc::spawn_juggernauts_hangar( "planeConstrainedFromMapCenter" ) )
        var_2 = give_xp_to_all_players_hack( var_2 );
    ...
    if ( scripts\mp\gametypes\br_gametypes.gsc::spawn_juggernauts_hangar( "planeUseCircleRadius" ) )
        var_12 = var_11;

    var_13 = _id_12E03( var_2, var_6, var_12 );
...
give_xp_to_all_players_hack( var_0 )
{
    ...
    var_2 = scripts\engine\trace::ray_trace_ents( level.br_level.br_mapcenter, var_0, level._id_11A82, var_1 );
...
_id_12E03( var_0, var_1, var_2 )
{
    ...
    if ( !scripts\mp\gametypes\br_gametypes.gsc::spawn_juggernauts_fob( "planeSnapToOOB" ) )
    {
        var_4 = _id_12C37( var_0, var_4 );
        var_5 = _id_12C37( var_0, var_5 );
    }
    ...
    if ( scripts\mp\gametypes\br_gametypes.gsc::spawn_juggernauts_fob( "planeSnapToOOB" ) )
    {
        if ( !activate_seq_button( var_4 ) )
            var_6 = _id_12C37( var_0, var_4 );

        if ( !activate_seq_button( var_5 ) )
            var_7 = _id_12C37( var_0, var_5 );
    }

    var_3 = vectornormalize( var_5 - var_4 );
    var_8 = 10;

    if ( scripts\mp\utility\game::postupdategameevents() == "mini" || scripts\mp\utility\game::postupdategameevents() == "zxp" || getdvarint( "scr_br_alt_mode_escape_skip_initial_circle", 0 ) || scripts\mp\utility\game::postupdategameevents() == "reveal" || isdefined( level.dangerzoneskipequipment ) && level.dangerzoneskipequipment._id_139D5 == 3 )
    {
    ...
    var_9.startpt = var_4;
    var_9.endpt = var_5;
    var_9._id_13040 = var_6;
    var_9.locationtriggersetpaused = var_7;
    var_9.angle = var_1;
    var_9.pathdir = var_3;
...
updatec130pathomnvars( var_0 )
{
    ...
    if ( scripts\mp\gametypes\br_gametypes.gsc::spawn_juggernauts_fob( "c130PlaneLine" ) )
        return;

    setomnvar( "ui_br_c130_path_start_x", int( var_0._id_13040[0] ) );
    setomnvar( "ui_br_c130_path_start_y", int( var_0._id_13040[1] ) );
    setomnvar( "ui_br_c130_path_end_x", int( var_0.locationtriggersetpaused[0] ) );
    setomnvar( "ui_br_c130_path_end_y", int( var_0.locationtriggersetpaused[1] ) );
}
...
createtestc130path( var_0, var_1 )
{
    ...
    if ( isdefined( level.br_level.br_mapcenter ) && isdefined( level.br_level.br_mapsize ) )
    {
        ...
        level thread scripts\mp\gametypes\br_analytics.gsc::customusefunc( var_2.centerpt, var_2.angle[1], var_2._id_13040, var_2.locationtriggersetpaused );
    }
    else
...
spawnc130( var_0, var_1, var_2 )
{
    ...
    level.br_ac130._id_11B1B = var_0;
    return var_4;
}
...
spawnplayertoc130()
{
    ...
    scripts\mp\gametypes\br_infils.gsc::locationsnames();
    thread playerputinc130( level.br_ac130 );
    self.destroyawardlaunchonly = 1;

    if ( istrue( self.spawn_bomb ) && getdvar( "scr_br_gametype", "" ) != "dmz" && getdvar( "scr_br_gametype", "" ) != "risk" && getdvar( "scr_br_gametype", "" ) != "bodycount" )
        level scripts\mp\gametypes\br_public.gsc::decoyassists( "deploy_squad_leader", self, 1, 0 );
}
...
orbitcam( var_0 )
{
    ...
    if ( isdefined( level._id_13A0B ) )
        scripts\mp\utility\player::_visionsetnaked( level._id_13A0B, 0 );
    else
        scripts\mp\utility\player::_visionsetnaked( "", 0 );

    self setplayerangles( var_0.angles );
    var_1 = scripts\mp\gametypes\br_infils.gsc::playersetattractiontime( self );
...
gunship_spawn( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    ...
    if ( isdefined( level.create_passengers_unload_groups ) )
        var_7 = level.create_passengers_unload_groups;
...
gunship_spawnvfx()
{
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" )
    {
        ...
        if ( !isdefined( level.create_player_rig_laser_panel ) )
            level.create_player_rig_laser_panel = "br_fx";

        self setscriptablepartstate( level.create_player_rig_laser_panel, "clouds" );
    }
    else
    {
...
ispointinbounds( var_0, var_1, var_2 )
{
    ...
    var_3 = scripts\mp\utility\game_utility_mp::shouldgamelobbyremainintact( var_0, var_2 );

    if ( !var_3 )
        return 0;

    var_4 = scripts\mp\gametypes\br_public.gsc::spawn_exfil_chopper() || scripts\mp\gametypes\br_public.gsc::spawn_apache_chopper();

    if ( var_4 && isdefined( level.br_ac130 ) )
        return var_3;

    if ( stack_patch_waittill_context_patch( var_0 ) )
        return 1;
    ...
    if ( istrue( var_1 ) )
    {
        if ( scripts\mp\outofbounds::spinpropkey( var_0 ) )
            return 0;
    }
...
_id_11CA3()
{
    ...
    for (;;)
    {
        var_1 = vectornormalize( self._id_11B1B._id_13040 - self.origin );
...
kickwhenoutofbounds( var_0 )
{
    ...
    waitframe();
    _id_11CA3();
    var_1 = playergetbestdropbagorigin();

    while ( !activate_seq_button( ( self.origin[0], self.origin[1], var_1 ) ) )
        waitframe();
    ...
    for (;;)
    {
        ...
        var_7 = vectornormalize( self._id_11B1B.centerpt - self.origin );
    ...
    for (;;)
    {
        ...
        if ( !activate_seq_button( var_10 ) )
        {
            level.create_primary_weapon_obj_from_custom_loadout = 1;
...
killaftertime( var_0, var_1 )
{
    ...
    self stoploopsound();
    scripts\mp\gametypes\br_infils.gsc::select_players_in_killzone_first( self );
    wait 0.1;
    ...
    if ( isdefined( self.animstruct ) )
    {
        ...
        scripts\mp\gametypes\br_infils.gsc::get_wave_cur_count( self.animstruct );
    }
    else
    {
        if ( isdefined( self.innards ) )
...
listenkick( var_0, var_1 )
{
    ...
    self.debug_run_helicopter_boss = 1;
...
listenjump( var_0, var_1 )
{
    ...
    for (;;)
    {
        ...
        var_6 = scripts\mp\gametypes\br_public.gsc::spawnanglemin() || isdefined( self.pers["squadMemberIndex"] ) && self.pers["squadMemberIndex"] == 0;
...
leaveplane( var_0, var_1, var_2, var_3 )
{
    ...
    if ( isdefined( self.br_infil_type ) )
        thread select_reception_spawners( var_0 );

    self cameradefault();
    self.br_infil_type = undefined;
    thread parachute( var_0, var_1, var_2, var_3 );
    thread scripts\mp\gametypes\br_gametypes.gsc::_id_1268B( "onLeaveAC130" );

    if ( scripts\mp\utility\game::postupdategameevents() == "truckwar" )
    {
        scripts\mp\gametypes\br_gametype_truckwar.gsc::getspawnpoint( 1 );
        self setorigin( self._id_1235F.origin, 1 );
        self setplayerangles( self._id_1235F.angles );
    }
}
...
{
    var_3 = scripts\engine\utility::ter_op( var_0 scripts\mp\gametypes\br_public.gsc::spawnanglemin(), 2, var_2 );
...
select_reception_spawners( var_0 )
{
    self endon( "disconnect" );
    var_1 = self;
    ...
    if ( scripts\mp\utility\game::postupdategameevents() != "truckwar" && scripts\mp\utility\game::postupdategameevents() != "reveal" && scripts\mp\utility\game::postupdategameevents() != "brdov" )
    {
        var_1 setsoundsubmix( "mp_br_infil_music", 0.0 );
        var_2 = scripts\mp\music_and_dialog::plunderextractalertnearby( "br_infil_jump" );
        var_1 setplayermusicstate( var_2 );
        var_1.select_mountain_three_spawners = 1;
    }

    wait 1.0;
    var_1 clearclienttriggeraudiozone( 3.0 );

    if ( scripts\mp\utility\game::postupdategameevents() == "reveal" || scripts\mp\utility\game::postupdategameevents() == "brdov" )
    {
        wait 3.0;
...
waittoplayinfildialog()
{
    ...
    level.br_ac130 _id_11CA3();

    for (;;)
    {
        var_0 = level.br_ac130 calctrailpoint();

        if ( activate_seq_button( var_0 ) )
        {
