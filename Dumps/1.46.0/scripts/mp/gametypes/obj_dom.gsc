...
setupobjective( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    ...
    if ( istrue( level.onteamproximitybecameinvalidplayer ) )
        var_10._id_12EAB = ::_id_12EAC;
    else if ( istrue( level.registeraccesscardlocs ) && scripts\mp\utility\game::getgametype() != "br" )
        var_10._id_12EAB = ::_id_12EAD;
    ...
    if ( istrue( level.registeraccesscardlocs ) && scripts\mp\utility\game::getgametype() != "br" )
    {
        var_20 = "decor_halloween_scarecrow";
        var_10.flagmodel.spawntimestamp = 1;
    }
    else
        var_20 = "military_dom_flag_neutral";

    var_10.flagmodel setmodel( var_20 );
    var_10.flagmodel.angles = gasvfxstructs( var_10 );
    var_10.outlineent = var_10.flagmodel;

    if ( istrue( level.registeraccesscardlocs ) && scripts\mp\utility\game::getgametype() != "br" )
    {
        var_10 thread register_wave_spawner();
        var_10 thread keypadscriptableused();
    }
...
dompoint_setcaptured( var_0, var_1 )
{
    ...
    if ( isdefined( self.assisttouchlist[var_0] ) )
    {
        ...
        foreach ( var_8 in var_6 )
        {
            ...
            var_9 scripts\cp\utility\cp_controlled_callbacks::zone_get_node_nearest_2d_bounds();
        }
    }
...
dompoint_onuse( var_0 )
{
    ...
    if ( !self.neutralized )
    {
        ...
        if ( isdefined( level.onobjectivecomplete ) )
        {
            ...
            var_5 = "Flag " + playerzombiegetrespawnbyteam() + " Captured";

            if ( var_2 == "neutral" || var_2 == "none" )
                scripts\mp\utility\game::trial_targs_combo( var_0, undefined, var_5, var_0.origin, "neutral_flag" );
            else
                scripts\mp\utility\game::trial_targs_combo( var_0, undefined, var_5, var_0.origin );
        }
    }
...
dompoint_onusebegin( var_0 )
{
    ...
    {
        if ( var_1 != var_0.team )
            var_2 = playerparachutedetachresetomnvars();
        else
            var_2 = 0;
    }
...
{
    self.given_achievement = gettime();
...
dompoint_onuncontested( var_0 )
{
    ...
    var_2 = ( gettime() - self.given_achievement ) * 0.001;
    var_3 = "Flag " + playerzombiegetrespawnbyteam() + " Contested";
    scripts\mp\utility\game::trial_targs_combo( undefined, undefined, var_3, undefined, var_2 + " seconds" );
    self.given_achievement = undefined;
...
_id_12EAC()
{
    ...
    if ( var_7["fraction"] != 1 )
    {
        ...
        if ( var_7["fraction"] != 1 )
        {
            playfx( level.race_flow["gos_fireworks"], var_7["position"] );
            playfx( level.race_flow["gos_fireworks"], var_7["position"], anglestoforward( self.flagmodel.angles ) + ( -10, -10, 0 ) );
            playfx( level.race_flow["gos_fireworks"], var_7["position"], anglestoforward( self.flagmodel.angles ) + ( 10, 10, 0 ) );
            level thread scripts\mp\gametypes\common.gsc::_id_13ACB( var_7["position"] );
        }
    }
    else
    {
        playfx( level.race_flow["gos_fireworks"], var_2 );
        playfx( level.race_flow["gos_fireworks"], var_2, anglestoforward( self.flagmodel.angles ) + ( -10, -10, 0 ) );
        playfx( level.race_flow["gos_fireworks"], var_2, anglestoforward( self.flagmodel.angles ) + ( 10, 10, 0 ) );
        level thread scripts\mp\gametypes\common.gsc::_id_13ACB( var_2 );
    }
}

_id_12EAD()
{
    if ( isdefined( self.scavenger_vo_when_close ) )
        return;
    else
    {
        self.scavenger_vo_when_close = 1;
        var_0 = 20;
        var_1 = 600;
        ...
        if ( var_7["fraction"] != 1 )
            playfx( level.scalertoscaleinfo["blood_ceil_hw"], var_7["position"] - ( 0, 0, 5 ), ( -180, -180, 0 ), ( -180, -180, -180 ) );

        playfx( level.scalertoscaleinfo["blood_floor_hw"], self.flagmodel.origin + ( 0, 0, 2 ) );
        var_0 = 4000;
        var_1 = 20;
        var_2 = self.flagmodel.origin;
        var_3 = var_2 + ( 0, 0, var_0 );
        ...
        if ( var_7["fraction"] != 1 )
            playfx( level.scalertoscaleinfo["bats_fly_hw"], var_7["position"] + ( 0, 0, 50 ) );
        else
            playfx( level.scalertoscaleinfo["bats_fly_hw"], self.flagmodel.origin + ( 0, 0, 50 ) );
    }
}
...
register_wave_spawner()
{
    ...
    if ( var_7["fraction"] != 1 )
        playfx( level.scalertoscaleinfo["bats_hw"], var_7["position"] );
    else
        playfx( level.scalertoscaleinfo["bats_hw"], self.flagmodel.origin + ( 0, 0, 200 ) );
}
...
updateflagstate( var_0, var_1, var_2, var_3 )
{
    ...
    if ( istrue( level.registeraccesscardlocs ) && scripts\mp\utility\game::getgametype() != "br" )
    {
        if ( istrue( var_3 ) )
            ...
            self.flagmodel thread _id_1273C();
        else if ( isdefined( var_2 ) )
            playfx( level.scalertoscaleinfo["vanish_hw_fr"], self.flagmodel.origin + ( 0, 0, 60 ) );
    }
    else if ( isdefined( var_2 ) )
    {
        if ( var_2 == "allies" )
...
giveflagcapturexp( var_0, var_1, var_2 )
{
    ...
    if ( isplayer( var_3 ) )
    {
        ...
        var_3 thread scripts\common\utility::_id_13592( level.update_objective_mlgicon_reset, "capture", var_3.origin );
    }
...
awardgenericmedals( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9 )
{
    ...
    if ( isdefined( var_0 ) )
    {
        ...
        {
            if ( istrue( var_0.spawntimestamp ) )
                return;
        }
    }
            ...
            if ( var_20 || var_21 )
            {
                ...
                else if ( var_18.claimteam == var_1.team )
                {
                    if ( !var_12 )
                    {
                        ...
                        thread scripts\common\utility::_id_13592( level.update_keypad_currentdisplay_models, var_9, "defending" );
                        continue;
                    }
                }
            }
...
_id_1273C()
{
    ...
    wait 0.55;
    playfx( level.scalertoscaleinfo["vanish_hw_fr"], self.origin + ( 0, 0, 60 ) );
...
keypadscriptableused()
{
    ...
    for (;;)
    {
        ...
        if ( isplayer( var_0 ) )
        {
            if ( !isdefined( self._id_12F45 ) )
            {
                self._id_12F45 = var_0;
                thread _id_13C18( self.flagmodel, var_0 );
            }
        }
...
_id_13C18( var_0, var_1 )
{
    level endon( "game_ended" );
    self notify( "new_stalker_target" );
    self endon( "new_stalker_target" );

    while ( isdefined( self._id_12F45 ) )
    {
        if ( !_id_13833( var_0, var_1 ) )
        {
            self._id_12F45 = undefined;
            self._id_12F44 = 0;
            self.missionbonustimer = undefined;
            self.br_watch_armor_weapon = undefined;
            self._id_12373 = undefined;
            self.flagmodel rotateto( ( 0, self.flagmodel.angles[1], self.flagmodel.angles[2] ), 0.2, 0.1, 0.1 );
        }
        else if ( isdefined( self._id_12F45 ) && scripts\mp\utility\player::isreallyalive( self._id_12F45 ) )
        {
...
_id_13C10()
{
    ...
    for (;;)
    {
        ...
        if ( isdefined( var_1 ) && isdefined( self._id_12F45 ) && self._id_12F45 == var_1 )
            self._id_12F44 = self._id_12F44 + var_0;

        if ( isdefined( self._id_12F44 ) )
        {
            if ( self._id_12F44 > 100 && !isdefined( self.missionbonustimer ) )
                self.missionbonustimer = 1;

            if ( self._id_12F44 > 200 && !isdefined( self.br_watch_armor_weapon ) )
            {
                self.br_watch_armor_weapon = 1;
                playfx( level.scalertoscaleinfo["vanish_hw_en"], self.flagmodel.origin + ( 0, 0, 80 ) );
            }

            if ( self._id_12F44 > 300 && !isdefined( self._id_12373 ) )
            {
                self._id_12373 = 1;
                thread _id_12558( var_1 );
            }
        }
...
_id_13833( var_0, var_1 )
{
    var_2 = 0.05;
    var_3 = var_1 postspawn_initial_allies();
    var_4 = var_0 plunderstructreplenish();
