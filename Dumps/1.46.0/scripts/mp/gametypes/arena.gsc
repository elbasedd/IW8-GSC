...
main()
{
    ...
    scripts\mp\utility\join_team_aggregator::registeronplayerjointeamcallback( ::_id_11982 );
    ...
    level.brking_oncrateuse = playerspawnendofgame();

    if ( level.brking_oncrateuse )
    {
        level.onstompeenemyprogressupdate = 0.5;
        level.onstun = 0.05;
    }
    ...
    {
        level._id_12C47 = 1;
        level.br_endgamesplashcallback = getdvarint( "scr_always_snowfight", 0 );

        if ( level.br_endgamesplashcallback )
    ...
    {
        level.brking_rankupdate = 1;
        level._id_12EA9 = 1;
    }
    else if ( getdvarint( "scr_arena_stpday" ) == 1 )
    {
        level.brking_ontimelimit = 1;
        level._id_12EA9 = 1;
    }
...
onstartgametype()
{
    ...
    {
        if ( !level.br_endgamesplashcallback && game["roundsPlayed"] == 0 && istrue( game["practiceRound"] ) )
        {
            level.extratime = 30;
            game["didSnowFight"] = 1;
        ...
        else if ( level.br_endgamesplashcallback )
        {
            level.arenaloadouts = 3;
            var_5 = 0;
        }

        level._id_12C4A["vanish"] = loadfx( "vfx/core/impacts/small_snowhit" );
        level._id_12C4A["screen"] = loadfx( "vfx/iw8/weap/_impact/snowball/vfx_imp_snowball_scrn.vfx" );
    ...
    if ( var_5 )
    {
        if ( israndomloadouts() || spawnpoint_setspawnpoint() || spawnoutofboundstrigger() || spawnpoint_clearspawnpoint() )
        ...
        else if ( ispickuploadouts() )
        {
            ...
            if ( !isdefined( game["roundsPlayed"] ) || isdefined( game["roundsPlayed"] ) && game["roundsPlayed"] == 0 )
            {
                ...
                level.startweapon.variantid = playerentersafeareamessage( level.startweapon.weapon );
                level.arenaweapont1.variantid = playerentersafeareamessage( level.arenaweapont1.weapon );
                level.arenaweapont2.variantid = playerentersafeareamessage( level.arenaweapont2.weapon );
                level.arenaweapont3.variantid = playerentersafeareamessage( level.arenaweapont3.weapon );
                level.arenaweapont4.variantid = playerentersafeareamessage( level.arenaweapont4.weapon );
                level.arenaweapont5.variantid = playerentersafeareamessage( level.arenaweapont5.weapon );
                level.arenaweapont6.variantid = playerentersafeareamessage( level.arenaweapont6.weapon );
                level.arenaweapont7.variantid = playerentersafeareamessage( level.arenaweapont7.weapon );
                level.arenaweapont8.variantid = playerentersafeareamessage( level.arenaweapont8.weapon );
            }
...
updategametypedvars()
{
    ...
    level.brking_onplayerkilled = getdvarint( "scr_arena_arenaAttachments", 0 );
    level.brking_playerwelcomesplashes = getdvar( "scr_arena_arenaSuper", "none" );

    if ( level.brking_playerwelcomesplashes == "none" )
        ...
        if ( !isdefined( var_0 ) )
        {
            ...
            if ( !isdefined( var_0 ) )
            {
                ...
                if ( var_4 > 3 )
                {
                    ...
                    foreach ( var_6 in scripts\mp\utility\teams::getteamdata( self.pers["team"], "players" ) )
                    {
                        if ( istrue( var_6.eliminate_drone_internal ) )
                        {
                            var_9++;
                            continue;
                        }
                    ...
                    foreach ( var_6 in scripts\mp\utility\teams::getteamdata( self.pers["team"], "players" ) )
                    {
                        ...
                        if ( isdefined( var_6.pers["arena_spawn_pos"] ) )
                        {
                            ...
                            if ( var_6.pers["arena_spawn_pos"] == "1" && level.usedspawnposone[self.pers["team"]] == 0 )
                            {
                                ...
                                var_6.eliminate_drone_internal = 1;
                                var_8 = var_6;
                                break;
                            }
                    ...
                    if ( isdefined( var_8 ) )
                    {
                        ...
                        if ( isdefined( var_8._id_12E67 ) )
                            var_0.angles = var_8._id_12E67;
                    }
                }
            }
    ...
    if ( !isnormalloadouts() )
    {
        ...
        foreach ( var_2 in level.teamnamelist )
        {
            if ( isdefined( var_0.pers["team"] ) && game["roundWinStreak"][var_2] > 0 && var_0.pers["team"] == var_2 )
            {
                ...
                var_0.pers["gamemodeLoadout"]["roundWinStreakPrimaryCamo"] = _id_129EF();
                var_0.pers["gamemodeLoadout"]["roundWinStreakecondaryCamoTeam"] = var_2;
                var_0.pers["gamemodeLoadout"]["roundWinStreakSecondaryCamo"] = _id_129EF();
            }
        }
    }
...
onjoinedteam()
{
    ...
    for (;;)
    {
        scripts\engine\utility::_id_13AD9( "joined_team", "joined_spectators" );
        _id_129FD();
    }
}
...
onspawnfinished()
{
    ...
    if ( istrue( game["practiceRound"] ) || istrue( level.br_endgamesplashcallback ) )
        thread _id_12C49();
    ...
    if ( !scripts\mp\flags::gameflag( "prematch_done" ) && game["roundsPlayed"] == 0 )
    {
        ...
        level scripts\engine\utility::_id_13AD9( "prematch_done", "removeArenaOutlines" );
    }
...
onplayerdamaged( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9, var_10, var_11, var_12, var_13 )
{
    ...
    {
        if ( istrue( level._id_12C47 ) && ( var_6.basename == "snowball_mp" || var_6.basename == "pball_mp" ) )
            playfxontagforclients( level._id_12C4A["screen"], var_2, "tag_eye", var_2 );
...
arena_endgame( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    scripts\cp_mp\pet_watch::_id_13735();
...
runarenaloadoutrulesonplayer()
{
    if ( israndomloadouts() || israndompreviewloadouts() || spawnoutofboundstrigger() || spawnpoint_clearspawnpoint() )
    {
        if ( israndompreviewloadouts() )
        {
...
buildloadoutsforweaponstreaming()
{
    ...
    if ( ispickuploadouts() )
    {
        ...
        else
        {
            ...
            else if ( istrue( level.registeraccesscardlocs ) )
            {
                var_4 = game["arenaRandomLoadout"][0]["loadoutPrimaryCamo"];
                var_8 = game["arenaRandomLoadout"][0]["loadoutPrimaryCosmeticAttachment"];
                var_9[0] = game["arenaRandomLoadout"][0]["loadoutPrimarySticker"];
            ...
            if ( spawn_wheelsons() )
                var_1 = scripts\mp\class::enable_oob_immunity_on_riders( var_2, var_4, var_5, level.startweapon.variantid, var_7, var_8, var_9 );
            else
...
buildprimaries( var_0, var_1, var_2 )
{
    ...
    if ( istrue( level._id_12EA9 ) && istrue( var_2 ) )
    {
        var_8 = position_ref();
...
buildsecondaries( var_0, var_1, var_2 )
{
    ...
    else if ( istrue( level.registeraccesscardlocs ) && istrue( var_2 ) )
    {
        var_7 = position_ref();
        game["arenaRandomLoadout"][game["arenaRandomLoadoutIndex"]]["loadoutSecondaryCamo"] = var_7;
...
post_blockade_breadcrumb_struct()
{
    var_0 = [];
...
_id_12A99()
{
    level.arenaweapont1 = enable_leaderboard( level.arenaweapont1 );
    level.arenaweapont2 = enable_leaderboard( level.arenaweapont2 );
    level.arenaweapont3 = enable_leaderboard( level.arenaweapont3 );
    level.arenaweapont4 = enable_leaderboard( level.arenaweapont4 );
    level.arenaweapont5 = enable_leaderboard( level.arenaweapont5 );
    level.arenaweapont6 = enable_leaderboard( level.arenaweapont6 );
    level.arenaweapont7 = enable_leaderboard( level.arenaweapont7 );
    level.arenaweapont8 = enable_leaderboard( level.arenaweapont8 );
}

enable_leaderboard( var_0 )
{
    ...
    if ( !issubstr( var_0.weapon, "equip" ) && var_0.weapon != "none" )
    {
        ...
        var_2 = position_ref();
        var_0.extracthelipadwatchforhelileaving = var_2;

        if ( _id_13836( var_0.weapon ) )
        {
            var_6 = positioncheck();
            var_0.group_hold_free = var_6;
        }

        if ( _id_13837( var_0.weapon ) )
        {
            var_7 = post_blockade_breadcrumb_struct();
            var_0._id_13078[0] = var_7[0];
            var_0._id_13078[1] = var_7[1];
            var_0._id_13078[2] = var_7[2];
            var_0._id_13078[3] = var_7[3];
        }
...
_id_12A51()
{
    level.arenaweapont1 = emp_drone_clean_up( level.arenaweapont1 );
    level.arenaweapont2 = emp_drone_clean_up( level.arenaweapont2 );
    level.arenaweapont3 = emp_drone_clean_up( level.arenaweapont3 );
    level.arenaweapont4 = emp_drone_clean_up( level.arenaweapont4 );
    level.arenaweapont5 = emp_drone_clean_up( level.arenaweapont5 );
    level.arenaweapont6 = emp_drone_clean_up( level.arenaweapont6 );
    level.arenaweapont7 = emp_drone_clean_up( level.arenaweapont7 );
    level.arenaweapont8 = emp_drone_clean_up( level.arenaweapont8 );
}
...
emp_drone_clean_up( var_0 )
{
    ...
    if ( !issubstr( var_0.weapon, "equip" ) && var_0.weapon != "none" )
    {
        ...
        var_7[3] = "none";
        var_8 = scripts\mp\utility\weapon::getweaponrootname( var_0.weapon );
        var_0.weaponobj = scripts\mp\class::enable_oob_immunity_on_riders( var_8, undefined, undefined, var_4, undefined, undefined, scripts\cp_mp\utility\game_utility::isnightmap() );
    }
...
_id_12A9B()
{
    level.startweapon = _id_12AA7( level.startweapon );
    level.arenaweapont1 = _id_12AA7( level.arenaweapont1 );
    level.arenaweapont2 = _id_12AA7( level.arenaweapont2 );
    level.arenaweapont3 = _id_12AA7( level.arenaweapont3 );
    level.arenaweapont4 = _id_12AA7( level.arenaweapont4 );
    level.arenaweapont5 = _id_12AA7( level.arenaweapont5 );
    level.arenaweapont6 = _id_12AA7( level.arenaweapont6 );
    level.arenaweapont7 = _id_12AA7( level.arenaweapont7 );
    level.arenaweapont8 = _id_12AA7( level.arenaweapont8 );
}
...
_id_12AA7( var_0 )
{
    ...
    if ( !issubstr( var_0.weapon, "equip" ) && var_0.weapon != "none" )
    {
        ...
        var_1 = player_waitfortacmapused( var_5 + "_mp" );
...
{
    var_1 = scripts\mp\utility\weapon::getweaponrootname( var_0 );
    var_2 = player_waitfortacmapused( var_1 + "_mp" );
    return var_2;
}
...
defineplayerloadout( var_0 )
{
    ...
    if ( isdefined( level.startweapon.weapon ) && level.startweapon.weapon != "none" && !issubstr( level.startweapon.weapon, "equip" ) )
    {
        ...
        if ( istrue( level._id_12EA9 ) )
        {
            ...
            if ( _id_13836( level.startweapon.weapon ) )
            {
                var_6 = positioncheck();
                game["arenaRandomLoadout"][0]["loadoutPrimaryCosmeticAttachment"] = var_6;
            }

            if ( _id_13837( level.startweapon.weapon ) )
            {
                var_7 = post_blockade_breadcrumb_struct();
        ...
        else if ( istrue( level.registeraccesscardlocs ) )
        {
            ...
            var_1 = playercanplaynotcriticalgesture( level.startweapon.weapon );
    }
    else
    {
        var_11 = "none";
        ...
        {
            if ( spotlights( level.startweapon.weapon ) )
            {
                ...
                if ( level.brking_oncrateuse )
                {
                    ...
                    if ( level.brking_oncrateuse == 2 )
                    {
                        var_12 = scripts\engine\utility::ter_op( spawnoutofboundstrigger(), "mp/classtable_arena_blueprints.csv", "mp/classTable_arena.csv" );
                        var_4 = int( scripts\mp\gametypes\gun.gsc::playerprespawngulagjail( "iw8_knife" ) );
    ...
    if ( level.allowsupers )
    {
        ...
        if ( issubstr( var_14, "mp_m_" ) && var_14 != "mp_m_speed" )
            level.brking_playerwelcomesplashes = new_col_map();
    }
    else
    {
        level.brking_playerwelcomesplashes = "none";
        setomnvar( "ui_disable_fieldupgrades", 1 );
    }
    ...
    level.arena_loadouts["default"]["loadoutFieldUpgrade1"] = level.brking_playerwelcomesplashes;
    ...
    if ( !istrue( level._id_12EA9 ) && !istrue( level.registeraccesscardlocs ) )
    {
        foreach ( var_16 in level.teamnamelist )
        {
            level.arena_loadouts[var_16]["loadoutPrimaryCamo"] = _id_129EF();
            level.arena_loadouts[var_16]["loadoutSecondaryCamo"] = _id_129EF();
        }
    }
}
...
cacherandomloadouts()
{
    ...
    var_0 = [];
    var_1 = 0;
    var_2 = scripts\engine\utility::ter_op( spawnoutofboundstrigger(), "mp/classtable_arena_blueprints.csv", "mp/classTable_arena.csv" );
    ...
    if ( israndompreviewloadouts() )
        var_1 = 144;
    else if ( spawnoffsettacinsertmin() )
        var_1 = 0;
    else if ( spawnpointdangertime() )
        var_1 = 57;
    else if ( spawnpointangles() )
        var_1 = 93;
    else if ( spawnpointdanger() )
        var_1 = 123;
    else if ( spawnposition() )
        var_1 = 146;
    else if ( spawnpoint_is_within_sight() )
        var_1 = 188;
    ...
    if ( var_5 != "" && var_6 > 0.0 )
    {
        ...
        foreach ( var_9 in var_7 )
        {
            ...
            if ( var_10.size == 2 )
            {
                var_11 = var_10[0];
                var_12 = int( var_10[1] );
                var_13 = scripts\mp\utility\weapon::_id_13CA6( var_11, var_12 );
...
updateloadoutarray( var_0, var_1 )
{
    ...
    if ( spawnoutofboundstrigger() || spawnpoint_clearspawnpoint() )
    ...
    if ( spawnoutofboundstrigger() || spawnpoint_clearspawnpoint() )
        var_2["loadoutSecondaryVariantID"] = scripts\mp\class::_id_131A9( var_0, var_1, 1, var_2["loadoutSecondary"] );
    else
    ...
    if ( level.allowsupers )
    {
        ...
        var_2["loadoutFieldUpgrade1"] = level.brking_playerwelcomesplashes;
...
arenaloadouts_select( var_0, var_1, var_2, var_3 )
{
    ...
    for ( var_6 = 0; var_6 < var_1 && var_0.size > 0; var_6++ )
    {
        ...
        if ( isdefined( var_2 ) && isdefined( var_3 ) )
        {
            ...
            var_10 = brking_onplayerconnect( var_2, var_9 );
            ...
            if ( var_10.size > 0 )
            {
                if ( randomfloat( 1.0 ) < var_3 )
                {
                    ...
                    var_7["loadoutPrimary"] = var_12._id_1261C;
...
brking_onplayerconnect( var_0, var_1 )
{
    var_2 = [];

    foreach ( var_4 in var_0 )
    {
        if ( arenaloadouts_getweapongroup( var_4._id_1261C ) == var_1 )
...
{
    if ( spawn_wheelsons() && !issubstr( var_0, "equip" ) && var_0 != "none" )
    {
        ...
        if ( var_4 != "" )
        {
            var_5 = scripts\mp\class::_id_131A9( "mp/classtable_arena_blueprints.csv", var_2, 0, var_4 );
            return var_5;
        }
...
player_waitfortacmapused( var_0 )
{
    var_1 = [];

    if ( level.brking_onplayerkilled == 1 )
    {
...
spawnweapon( var_0, var_1, var_2 )
{
    ...
    var_4 = new_agent_def_main( var_0 );
    ...
    if ( var_9 != "" )
    {
        if ( istrue( level._id_12C47 ) && var_3 == "equip_snowball" )
        {
            if ( isplayer( var_0 ) )
                var_8 = "single";
            ...
            if ( !isplayer( var_0 ) && var_0.script_label == "6" && !level._id_11C3B )
            {
            ...
            else if ( !isplayer( var_0 ) && var_0.script_label == "8" && !level._id_11C3B )
            {
        ...
        if ( isdefined( var_0.angles ) )
        {
            ...
            {
                if ( isdefined( var_0.script_noteworthy ) && var_0.script_noteworthy == "wall" || isdefined( var_0._id_12797 ) && var_0._id_12797 == "wall" )
        ...
        var_10._id_121EE = spawn( "trigger_radius", var_10.origin, 0, 32, 32 );
        var_10 thread stage3accradius();

        if ( istrue( level._id_12C47 ) )
        {
            var_10 thread _id_13C13( var_0, var_8 );
    ...
    else
    {
        ...
        if ( istrue( level._id_12EA9 ) || istrue( level.registeraccesscardlocs ) || spawn_wheelsons() )
            var_17 = var_1.weaponobj;
        else
        {
            var_16 = player_waitfortacmapused( var_15 + "_mp" );
        ...
        if ( isdefined( var_0.script_noteworthy ) && var_0.script_noteworthy == "wall" )
        {
            ...
            var_19.cprooftopcratecapturecallback = var_22;
        }
...
stage3accradius()
{
    ...
    {
        wait 0.05;
        self._id_121EE waittill( "trigger", var_1 );
...
spawngameendflagzone( var_0 )
{
    ...
    var_1.onuncontested = ::brking_initdialog;
    var_1.isarena = 1;
...
deleteotpreview()
{
    level scripts\engine\utility::_id_13AD9( "prematch_done", "start_mode_setup" );
    disableotflag();
}
...
giveflagcapturexp( var_0, var_1 )
{
    ...
    if ( isplayer( var_2 ) )
    {
        ...
        var_2 thread scripts\common\utility::_id_13592( level.update_objective_mlgicon_reset, "capture", var_2.origin );
    }
...
_id_12B34()
{
    ...
    if ( istrue( level.arenaflag._id_11FA9 ) )
        return;
...
getoutlineasset( var_0, var_1 )
{
    ...
    if ( istrue( level._id_12C47 ) && isdefined( var_1 ) && var_1 == "equip_pball" )
        var_2 = "outline_depth_yellow";
...
spawnoffsettacinsertmin()
{
    return level.arenaloadouts == 7;
}

spawnpointdangertime()
{
...
_id_12A94()
{
    ...
    if ( !istrue( level.br_endgamesplashcallback ) && !istrue( game["practiceRound"] ) )
    {
    ...
    foreach ( var_2 in var_0 )
    {
        if ( istrue( level.br_endgamesplashcallback ) || istrue( game["practiceRound"] ) || level.arenaloadouts != 3 )
        {
            spawnweapon( var_2, level.arenaweapont1 );
            continue;
        }
...
_id_13C13( var_0, var_1 )
{
    ...
    else
    {
        ...
        playfx( level._id_12C4A["vanish"], self.origin );
        clearweaponoutlines();

        if ( !isplayer( var_0 ) )
            level thread _id_13ACC( var_0 );
...
_id_13ACC( var_0 )
{
    level endon( "game_ended" );
    wait 15;
    playfx( level._id_12C4A["vanish"], var_0.origin );
...
playerspawnendofgame()
{
    ...
    else if ( spotlights( level.arenaweapont1.weapon ) && spotlights( level.arenaweapont2.weapon ) && spotlights( level.arenaweapont3.weapon ) && spotlights( level.arenaweapont4.weapon ) && spotlights( level.arenaweapont5.weapon ) && spotlights( level.arenaweapont6.weapon ) && spotlights( level.arenaweapont7.weapon ) && spotlights( level.arenaweapont8.weapon ) )
    {
        if ( spotlights( level.startweapon.weapon ) )
            return 2;
        else
...
new_col_map()
{
    switch ( level.brking_playerwelcomesplashes )
    {
        case "super_weapon_drop":
        case "super_emp_drone":
        case "super_recon_drone":
            level.brking_playerwelcomesplashes = "super_ammo_drop";
            break;
        default:
            break;
    }

    return level.brking_playerwelcomesplashes;
}
