...
main()
{
    ...
    level.selfrevivebuttonpresscleanup = _id_1284E();
    ...
    if ( istrue( getdvarint( "br_spawnSelectionInfil", 0 ) ) )
    {
        ...
        level.failhistoryquest = getdvarint( "scr_br_canSoloJump", 1 );
    }

    if ( istrue( level.br_infils_disabled ) && specialistperk() )
        level thread delete_props_skilo( undefined, 0 );
...
_id_1284E()
{
    ...
    if ( var_1 == 1 )
    {
        if ( level.mapname != "mp_don4" && level.mapname != "mp_don4_pm" && level.mapname != "mp_br_mechanics" && level.mapname != "mp_escape3" && level.mapname != "mp_escape4" )
            var_1 = 2;
    }
    else if ( var_1 == 2 )
    {
        if ( level.mapname == "mp_don4" || level.mapname == "mp_escape3" || level.mapname == "mp_escape4" )
            var_1 = 1;
...
solospawn()
{
    return level.selfrevivebuttonpresscleanup == 0;
}

spawn_carried_punchcard_if_player_down()
{
    return level.selfrevivebuttonpresscleanup == 1;
}

specialistperk()
{
    return level.selfrevivebuttonpresscleanup == 2;
}
...
playersetattractionextradata()
{
    ...
    if ( spawn_carried_punchcard_if_player_down() )
    {
        var_0 = 0.0;
        var_1 = 0.0;
    }
    else if ( specialistperk() )
    {
        var_0 = 0.45;
        var_1 = 0.6;
    }
...
freefallstartfunc()
{
    ...
    thread scripts\cp_mp\parachute::_id_11FA7();
}
...
_id_13A97( var_0, var_1, var_2 )
{
    ...
    foreach ( var_5 in var_3 )
    {
        ...
        var_5 locationsnames();
        var_5 clearladderstate();

        if ( !istrue( level.br_infils_disabled ) )
            var_5 scripts\mp\gametypes\br.gsc::_id_11E31();
        ...
        if ( !isalive( var_5 ) )
        {
            ...
            var_5 select_random_card_location();
        }
        else
        {
        ...
        if ( istrue( var_5.createhudelem ) )
            scripts\mp\gametypes\br.gsc::_id_136A1( var_5, "waitAndForceSpawnAllPlayers" );
    ...
    if ( !scripts\mp\gametypes\br_gametypes.gsc::spawn_juggernauts_fob( "squadLeader" ) )
    {
        if ( level.teambased )
        {
            ...
            {
                var_9 = scripts\mp\gametypes\br_public.gsc::postkillcamplunderlost( var_8 );

                for ( var_10 = 0; var_10 < var_9.size; var_10++ )
                {
                    var_11 = var_9[var_10];
                    var_12 = scripts\mp\gametypes\br_public.gsc::postplunder( var_8, var_11 );
                    var_13 = _id_12A56( var_12 );

                    if ( isdefined( var_13 ) )
                    {
                        _id_123B6( var_13 );
                        var_13 thread _id_13BBF( var_8, var_11 );
                    }

                    scripts\mp\gametypes\br.gsc::_id_1378D( var_8, var_11 );
                }
            }
        }
        else
...
locationsnames()
{
    ...
    if ( isdefined( var_0.mortar_smoke_occlusions ) )
        var_0.mortar_smoke_occlusions = 5;

    var_0 scripts\cp_mp\parachute::_id_11AE3();
}

_id_123B6( var_0 )
{
    if ( !istrue( var_0.startpayloadpunish ) )
    {
        var_0.startpayloadpunish = 1;
...
{
    scripts\engine\utility::_id_13AD9( "death", "disconnect" );
    var_2 = scripts\mp\gametypes\br_public.gsc::postplunder( var_0, var_1 );
    var_3 = _id_12A56( var_2 );

    if ( isdefined( var_3 ) )
    {
        _id_123B6( var_3 );
        var_3 thread _id_13BBF( var_0, var_1 );
    }
}
...
latespawnplayer()
{
    self endon( "disconnect" );
    var_0 = self;
    var_1 = scripts\mp\gametypes\br_public.gsc::spreadshotdamagemod() || scripts\mp\gametypes\br_public.gsc::spawn_exfil_enemies();
    ...
    else if ( !isalive( var_0 ) )
    {
        ...
        {
            var_2 = isbot( var_0 ) && scripts\mp\gametypes\br_public.gsc::spawn_apache_chopper();

            while ( !istrue( level.createnukecrate ) && !var_2 )
            {
                if ( !var_2 )
                {
                    var_0 scripts\mp\gametypes\br.gsc::disable_usability_for_duration();
            ...
            var_0 thread teamcanrespawn();
        }
        else
        {
            var_0.createhudstring = 0;
            ...
            var_0 thread vehicle_damage_getpristinestatehealthadd();
        }
    }
...
teamcanrespawn()
{
    ...
    if ( !isalive( var_0 ) )
    {
        var_0.teamclearextractplunder = 1;
        var_0 scripts\mp\playerlogic::spawnplayer( 0 );
    }

    var_0 playerhide();
    var_0.teamclearextractplunder = undefined;
    ...
    var_0.select_patrol_seven_spawners = var_1;

    if ( var_0 scripts\mp\gametypes\br_public.gsc::spawnanglemin() )
        var_0.infilanimindex = 1;

    if ( !isdefined( var_0.infilanimindex ) && level.teambased )
    {
        var_2 = scripts\mp\gametypes\br_public.gsc::postplunder( var_0.team, var_0.squadindex );
        var_3 = empty_vo_func( var_2.size );
        ...
        for ( var_4 = 0; var_4 < var_3.size; var_4++ )
        {
            ...
            {
                if ( var_7 != var_0 && var_7 scripts\mp\gametypes\br_public.gsc::spawnanglemin() )
                    continue;
    ...
    var_0 playerlinktopositionent( level.teamanchoredwidgetinstances );
    scripts\mp\utility\game::_id_129D6( var_0, 1 );
    var_9 = playersetattractionstateindex( level.teamanchoredwidgetinstances );
    var_0 setorigin( var_9.origin );
    var_0 playerplayinfilloopanim( level.teamanchoredwidgetinstances );
    waitframe();
    var_10 = 0;
...
yaw_updater()
{
    ...
    if ( !scripts\mp\gametypes\br_gametypes.gsc::spawn_juggernauts_fob( "squadLeader" ) )
    {
        if ( level.teambased )
        {
            var_1 = scripts\mp\gametypes\br_public.gsc::postplunder( var_0.team, var_0.squadindex );

            if ( !isdefined( var_1 ) || var_1.size == 0 )
                var_1 = [ self ];

            var_2 = _id_12A56( var_1 );
...
_id_12A56( var_0 )
{
    ...
    {
        if ( isdefined( var_3 ) && var_3 scripts\mp\gametypes\br_public.gsc::spawnanglemin() )
            return var_3;
    }
    ...
    var_1 scripts\mp\gametypes\br.gsc::_id_129D0( 1 );

    foreach ( var_3 in var_0 )
    {
        if ( var_3 != var_1 )
            var_3 scripts\mp\gametypes\br.gsc::_id_129D0( 0 );

        var_16 = scripts\engine\utility::ter_op( var_3 scripts\mp\gametypes\br_public.gsc::spawnanglemin(), 2, 1 );
...
emp_drone_pick_up_use_think( var_0, var_1 )
{
    if ( spawn_carried_punchcard_if_player_down() )
        return emp_drone_damage_monitor( var_0, var_1 );
    else if ( specialistperk() )
        return enable_lbravo_player_infil( var_0, var_1 );
    else
...
buildac130infilanimstruct( var_0, var_1 )
{
    ...
    if ( isdefined( var_0 ) )
    {
        ...
        var_2.extract_removemissionweapon = spawn( "script_model", var_2.movingc130.origin );
        var_2.extract_removemissionweapon setmodel( "generic_prop_x5" );
        var_2.extract_removemissionweapon linkto( var_2.movingc130, "tag_origin", ( 0, 0, 0 ), ( 0, 0, 0 ) );
        var_2.extract_removemissionweapon forcenetfieldhighlod( 1 );
    }
    ...
    var_2.getplunderextractionsites = spawnfx( level._effect["vfx_br_infil_cloud_anim"], var_2.staticc130.origin );
    return var_2;
}
...
emp_drone_damage_monitor( var_0, var_1 )
{
    ...
    {
        var_0.animstruct = var_2;
        var_2.waitforgulagfightstocomplete = var_0;
    }
    ...
    var_2.get_player_recent_pos_adjusted_with_exposure = spawn( "script_model", var_2.origin );
    var_2.get_player_recent_pos_adjusted_with_exposure setmodel( "veh8_mil_air_mindia8_interior_infil_netting" );
    var_2.get_player_recent_pos_adjusted_with_exposure linkto( var_2.chopper, "tag_origin", ( 0, 0, 0 ), ( 0, 0, 0 ) );
    var_2.get_next_open_stop = spawn( "script_model", var_2.origin );
    var_2.get_next_open_stop setmodel( "veh8_mil_air_mindia8_interior_infil_cabin_door" );
    var_2.get_next_open_stop linkto( var_2.chopper, "tag_origin", ( 0, 0, 0 ), ( 0, 0, 0 ) );
    var_2.extractcountdown = spawn( "script_model", var_2.origin );
    var_2.extractcountdown setmodel( "generic_prop_x3" );
    var_2.extractcountdown linkto( var_2.chopper, "tag_origin", ( 0, 0, 0 ), ( 0, 0, 0 ) );
    var_2.extractgroundpos = spawn( "script_model", var_2.origin );
    var_2.extractgroundpos setmodel( "generic_prop_x3" );
    var_2.extractgroundpos linkto( var_2.chopper, "tag_origin", ( 0, 0, 0 ), ( 0, 0, 0 ) );

    if ( isdefined( var_0 ) )
    {
        var_0 forcenetfieldhighlod( 1 );
        var_2.extractcountdownmsg = spawn( "script_model", var_0.origin );
        var_2.extractcountdownmsg setmodel( "generic_prop_x3" );
        var_2.extractcountdownmsg linkto( var_0, "tag_origin", ( 0, 0, 0 ), ( 0, 0, 0 ) );
        var_2.extractcountdownmsg forcenetfieldhighlod( 1 );
        var_2.extracthelipadusecallback = spawn( "script_model", var_0.origin );
        var_2.extracthelipadusecallback setmodel( "generic_prop_x3" );
        var_2.extracthelipadusecallback linkto( var_0, "tag_origin", ( 0, 0, 0 ), ( 0, 0, 0 ) );
        var_2.extracthelipadusecallback forcenetfieldhighlod( 1 );
    }
    ...
    if ( var_1 == "script_model" )
    {
        var_2._id_11F7A = spawn( "script_model", var_2.origin );
        var_2._id_11F7A setmodel( "fullbody_usmc_ar_br_infil" );
        var_2._id_11F7A linkto( var_2.playerpositionents["parent_solo"], "j_prop_1", ( 0, 0, 0 ), ( 0, 0, 0 ) );
        var_2._id_11F7B = spawn( "script_model", var_2.origin );
        var_2._id_11F7B setmodel( "fullbody_usmc_ar_br_infil" );
        var_2._id_11F7B linkto( var_2.playerpositionents["parent_squad"], "j_prop_1", ( 0, 0, 0 ), ( 0, 0, 0 ) );
        var_2._id_11F7C = spawn( "script_model", var_2.origin );
        var_2._id_11F7C setmodel( "fullbody_usmc_ar_br_infil" );
        var_2._id_11F7C linkto( var_2.playerpositionents["parent_squad"], "j_prop_2", ( 0, 0, 0 ), ( 0, 0, 0 ) );
        var_2._id_11F7D = spawn( "script_model", var_2.origin );
        var_2._id_11F7D setmodel( "fullbody_usmc_ar_br_infil" );
        var_2._id_11F7D linkto( var_2.playerpositionents["parent_squad"], "j_prop_3", ( 0, 0, 0 ), ( 0, 0, 0 ) );
        var_2._id_11F7E = spawn( "script_model", var_2.origin );
        var_2._id_11F7E setmodel( "fullbody_usmc_ar_br_infil" );
        var_2._id_11F7E linkto( var_2.playerpositionents["parent_squad"], "j_prop_4", ( 0, 0, 0 ), ( 0, 0, 0 ) );
        var_2.infil_anim_type = "script_model";
    }
    else
    {
    ...
    var_2.binoculars_processlosqueue = spawn( "script_model", var_2.origin );
    var_2.binoculars_processlosqueue setmodel( "fullbody_mp_eastern_bale_3_1" );
    var_2.binoculars_processlosqueue linkto( var_2.playerpositionents["parent_solo"], "j_prop_2", ( 0, 0, 0 ), ( 0, 0, 0 ) );
    var_2.binoculars_hascoldblooded = spawn( "script_model", var_2.origin );
    var_2.binoculars_hascoldblooded setmodel( "fullbody_usmc_ar_br_infil" );
    var_2.binoculars_hascoldblooded linkto( var_2.playerpositionents["parent_solo"], "j_prop_3", ( 0, 0, 0 ), ( 0, 0, 0 ) );
    var_2.brleaderdialogteam = spawn( "script_model", var_2.origin );
    var_2.brleaderdialogteam setmodel( "generic_prop_x10" );
    var_2.brleaderdialogteam linkto( var_2.chopper, "tag_origin", ( 0, 0, 0 ), ( 0, 0, 0 ) );
    var_2.brleaderdialogteam forcenetfieldhighlod( 1 );
    var_4 = 9;
    var_2.brleaderdialogplayer = [];

    for ( var_5 = 0; var_5 < var_4; var_5++ )
    {
        var_6 = spawn( "script_model", var_2.origin );
        var_6 setmodel( "veh8_mil_air_mindia8_infil_flight" );
        var_6 linkto( var_2.brleaderdialogteam, "j_prop_" + ( var_5 + 2 ), ( 0, 0, 0 ), ( 0, 0, 0 ) );
        var_6 forcenetfieldhighlod( 1 );
        var_6 playloopsound( "br_heli_infil_fleet_lp" );
        var_2.brleaderdialogplayer[var_2.brleaderdialogplayer.size] = var_6;
    }

    return var_2;
}

enable_lbravo_player_infil( var_0, var_1 )
{
    var_2 = spawnstruct();
    ...
    {
        var_0.animstruct = var_2;
        var_2.waitforoneplayernearlz = var_0;
    }

    var_2._id_13057 = getent( "infil_plane_ch3", "script_noteworthy" );

    if ( isdefined( var_2._id_13057 ) )
        var_2._id_13057 show();
    else
    {
        var_2._id_13057 = spawn( "script_model", getdvarvector( "br_infil_anim_pos", ( 0, 0, 0 ) ) );
        var_2._id_13057 setmodel( "veh8_mil_air_acharlie130_magma_animated" );
        var_2._id_13057.cleanme = 1;
    }

    var_2.card_origin = spawn( "script_model", var_2._id_13057.origin );
    var_2.card_origin setmodel( "veh8_mil_air_skilo_interior_infil_int_bags_back_left_up" );
    var_2.card_origin linkto( var_2._id_13057, "tag_bags_back_left_up", ( 0, 0, 0 ), ( 0, 0, 0 ) );
    var_2.care_packages_unusable_think = spawn( "script_model", var_2._id_13057.origin );
    var_2.care_packages_unusable_think setmodel( "veh8_mil_air_skilo_interior_infil_int_bags_back_right_up" );
    var_2.care_packages_unusable_think linkto( var_2._id_13057, "tag_bags_back_right_up", ( 0, 0, 0 ), ( 0, 0, 0 ) );
    var_2.cardstruct = spawn( "script_model", var_2._id_13057.origin );
    var_2.cardstruct setmodel( "veh8_mil_air_skilo_interior_infil_int_bags_back_right_low" );
    var_2.cardstruct linkto( var_2._id_13057, "tag_bags_back_right_low", ( 0, 0, 0 ), ( 0, 0, 0 ) );
    var_2._id_121D9 = [ var_2.card_origin, var_2.care_packages_unusable_think, var_2.cardstruct ];
    var_2._id_1261E = spawn( "script_model", var_2._id_13057.origin );
    var_2._id_1261E setmodel( "veh8_mil_air_skilo_interior_infil_ropes" );
    var_2._id_1261E linkto( var_2._id_13057, "tag_origin", ( 0, 0, 0 ), ( 0, 0, 0 ) );
    var_2._id_13058 = getent( "infil_skilo_door", "script_noteworthy" );
    var_2.extractcountdown = spawn( "script_model", var_2._id_13057.origin );
    var_2.extractcountdown setmodel( "generic_prop_x3" );
    var_2.extractcountdown linkto( var_2._id_13057, "tag_origin", ( 0, 0, 0 ), ( 0, 0, 0 ) );
    var_2.extractgroundpos = spawn( "script_model", var_2._id_13057.origin );
    var_2.extractgroundpos setmodel( "generic_prop_x3" );
    var_2.extractgroundpos linkto( var_2._id_13057, "tag_origin", ( 0, 0, 0 ), ( 0, 0, 0 ) );
    ...
    var_2.playerpositionents["parent_solo"] = spawn( "script_model", var_2._id_13057.origin );
    var_2.playerpositionents["parent_solo"] setmodel( "generic_prop_x3" );
    var_2.playerpositionents["parent_solo"] linkto( var_2._id_13057, "tag_origin", ( 0, 0, 0 ), ( 0, 0, 0 ) );
    var_2.playerpositionents["parent_squad"] = spawn( "script_model", var_2._id_13057.origin );
    var_2.playerpositionents["parent_squad"] setmodel( "generic_prop_x5" );
    var_2.playerpositionents["parent_squad"] linkto( var_2._id_13057, "tag_origin", ( 0, 0, 0 ), ( 0, 0, 0 ) );

    if ( var_1 == "script_model" )
    {
        var_2._id_11F7A = spawn( "script_model", var_2._id_13057.origin );
        var_2._id_11F7A setmodel( "fullbody_usmc_ar_br_infil" );
        var_2._id_11F7A linkto( var_2.playerpositionents["parent_solo"], "j_prop_1", ( 0, 0, 0 ), ( 0, 0, 0 ) );
        var_2._id_11F7B = spawn( "script_model", var_2._id_13057.origin );
        var_2._id_11F7B setmodel( "fullbody_usmc_ar_br_infil" );
        var_2._id_11F7B linkto( var_2.playerpositionents["parent_squad"], "j_prop_1", ( 0, 0, 0 ), ( 0, 0, 0 ) );
        var_2._id_11F7C = spawn( "script_model", var_2._id_13057.origin );
        var_2._id_11F7C setmodel( "fullbody_usmc_ar_br_infil" );
        var_2._id_11F7C linkto( var_2.playerpositionents["parent_squad"], "j_prop_2", ( 0, 0, 0 ), ( 0, 0, 0 ) );
        var_2._id_11F7D = spawn( "script_model", var_2._id_13057.origin );
        var_2._id_11F7D setmodel( "fullbody_usmc_ar_br_infil" );
        var_2._id_11F7D linkto( var_2.playerpositionents["parent_squad"], "j_prop_3", ( 0, 0, 0 ), ( 0, 0, 0 ) );
        var_2._id_11F7E = spawn( "script_model", var_2._id_13057.origin );
        var_2._id_11F7E setmodel( "fullbody_usmc_ar_br_infil" );
        var_2._id_11F7E linkto( var_2.playerpositionents["parent_squad"], "j_prop_4", ( 0, 0, 0 ), ( 0, 0, 0 ) );
        var_2.infil_anim_type = "script_model";
    }
    ...
    if ( isdefined( var_0 ) && isdefined( var_2.waitforoneplayernearlz.innards ) )
        var_2.waitforoneplayernearlz.innards linkto( var_2.waitforoneplayernearlz, "", ( 0, 0, 0 ), ( 0, 0, 0 ) );

    return var_2;
}

get_wait_between_stations_time( var_0 )
{
    if ( isdefined( var_0.brleaderdialogplayer ) )
    {
        foreach ( var_2 in var_0.brleaderdialogplayer )
            var_2 delete();
    }

    if ( isdefined( var_0.brleaderdialogteam ) )
        var_0.brleaderdialogteam delete();

    if ( isdefined( var_0.binoculars_hascoldblooded ) )
        var_0.binoculars_hascoldblooded delete();

    if ( isdefined( var_0.binoculars_processlosqueue ) )
        var_0.binoculars_processlosqueue delete();
    ...
    if ( isdefined( var_0._id_11F7A ) )
        var_0._id_11F7A delete();

    if ( isdefined( var_0._id_11F7B ) )
        var_0._id_11F7B delete();

    if ( isdefined( var_0._id_11F7C ) )
        var_0._id_11F7C delete();

    if ( isdefined( var_0._id_11F7D ) )
        var_0._id_11F7D delete();

    if ( isdefined( var_0._id_11F7E ) )
        var_0._id_11F7E delete();

    if ( isdefined( var_0.extractcountdown ) )
        var_0.extractcountdown delete();

    if ( isdefined( var_0.extractgroundpos ) )
        var_0.extractgroundpos delete();

    if ( isdefined( var_0.extractcountdownmsg ) )
        var_0.extractcountdownmsg delete();

    if ( isdefined( var_0.extracthelipadusecallback ) )
        var_0.extracthelipadusecallback delete();

    if ( isdefined( var_0.get_next_open_stop ) )
        var_0.get_next_open_stop delete();

    if ( isdefined( var_0.get_player_recent_pos_adjusted_with_exposure ) )
        var_0.get_player_recent_pos_adjusted_with_exposure delete();

    if ( isdefined( var_0.chopper ) && istrue( var_0.chopper.cleanme ) )
        var_0.chopper delete();

    if ( isdefined( var_0.waitforgulagfightstocomplete ) && istrue( var_0.waitforgulagfightstocomplete.cleanme ) )
    {
        if ( isdefined( var_0.waitforgulagfightstocomplete.innards ) && istrue( var_0.waitforgulagfightstocomplete.innards.cleanme ) )
            var_0.waitforgulagfightstocomplete.innards delete();

        var_0.waitforgulagfightstocomplete delete();
    }
}

get_wave_data( var_0 )
{
    scripts\mp\utility\sound::_id_124C6( "br_infil_skilo" );
    ...
    if ( isdefined( var_0._id_1261E ) )
        var_0._id_1261E delete();

    foreach ( var_5 in var_0._id_121D9 )
    {
        if ( isdefined( var_5 ) )
            var_5 delete();
    }

    if ( isdefined( var_0._id_13057 ) && istrue( var_0._id_13057.cleanme ) )
        var_0._id_13057 delete();

    if ( isdefined( var_0.waitforoneplayernearlz ) && istrue( var_0.waitforoneplayernearlz.cleanme ) )
    {
        if ( isdefined( var_0.waitforoneplayernearlz.innards ) && istrue( var_0.waitforoneplayernearlz.innards.cleanme ) )
            var_0.waitforoneplayernearlz.innards delete();

        if ( isdefined( var_0.waitforoneplayernearlz.branalytics_gulagstart ) && istrue( var_0.waitforoneplayernearlz.branalytics_gulagstart.cleanme ) )
            var_0.waitforoneplayernearlz.branalytics_gulagstart delete();

        if ( isdefined( var_0.waitforoneplayernearlz.door ) && istrue( var_0.waitforoneplayernearlz.door.cleanme ) )
            var_0.waitforoneplayernearlz.door delete();

        var_0.waitforoneplayernearlz delete();
    }
}
...
delete_props_skilo( var_0, var_1 )
{
    ...
    if ( istrue( var_1 ) )
    {
        if ( isdefined( var_0 ) )
        {
            var_3 = playersetattractionoff( var_0 );
...
playershowarenastartobjectivetext( var_0 )
{
    if ( spawn_carried_punchcard_if_player_down() )
        return playershouldrespawn( var_0 );
    else if ( specialistperk() )
        return playershowskippromptcinematic( var_0 );
    else
        return playershooting( var_0 );
}
...
playershouldrespawn( var_0 )
{
    ...
    var_3 = anglestoforward( var_0.waitforgulagfightstocomplete.angles );
    var_4 = anglestoright( var_0.waitforgulagfightstocomplete.angles );
...
playershowskippromptcinematic( var_0 )
{
    var_1 = self getplayerangles( 1 );
    var_2 = anglestoforward( var_1 );
    var_3 = anglestoforward( var_0.waitforoneplayernearlz.angles );
    var_4 = anglestoright( var_0.waitforoneplayernearlz.angles );
    var_5 = vectordot( var_2, var_3 );
    var_6 = vectordot( var_2, var_4 );
    var_7 = cos( 45.0 );

    if ( var_5 < -1.0 * var_7 )
        return [ "sdr_mp_infil_ac130_jump_genpropx10_90_r", %sdr_mp_infil_ac130_jump_genpropx10_90_r, "wz_infil_skilo_jump01_pl01" ];
...
_id_12022( var_0 )
{
    ...
    var_0 scripts\mp\gametypes\br_public.gsc::_id_12041( var_1, var_0.team, var_0 );
}
...
watchinfiljumpanim( var_0 )
{
    ...
    if ( var_6 )
    {
        ...
        {
            var_8 = 0;
            [var_10, var_11, var_12] = playershowarenastartobjectivetext( var_0 );
            var_13 = playersetattractionstateindex( var_0 );
        ...
        foreach ( var_4 in var_2 )
        {
            ...
            var_4 scripts\mp\gametypes\br.gsc::_id_11E32();
        }
...
playinfilplayeranims( var_0, var_1 )
{
    ...
    foreach ( var_5 in var_3 )
    {
        ...
        var_5 scripts\mp\gametypes\br.gsc::_id_11E31();
        scripts\mp\utility\game::_id_129D6( var_5, 1 );
        var_5.plotarmor = 1;
        var_5 unlink();
        var_5 playerlinktopositionent( var_0 );
        var_6 = var_5 _id_11E52( var_0 );
...
_id_11E52( var_0 )
{
    ...
    if ( spawn_carried_punchcard_if_player_down() )
    {
        if ( self.select_patrol_seven_spawners )
...
_id_11FCC( var_0, var_1 )
{
    ...
    foreach ( var_4 in var_2 )
    {
        ...
        var_4 scripts\mp\utility\player::_id_12AB3( "100", 1 );
    }

    var_0.aidoorchief scripts\mp\utility\player::_id_12AB3( "100" );
}

gethillspawnshutofforigin( var_0, var_1 )
{
    var_2 = scripts\engine\utility::ter_op( isdefined( var_1 ), var_1, level.players );
    ...
    var_0.aidoorchief scripts\mp\utility\player::_id_12AB3( "0" );
}

_id_11FCD( var_0, var_1 )
{
    var_0 endon( "stopScene" );
    var_2 = scripts\engine\utility::ter_op( isdefined( var_1 ), var_1, level.players );
    ...
    foreach ( var_4 in var_2 )
    {
        ...
        var_4 scripts\mp\utility\player::_id_12AB3( "80", 1 );
    }
}

gethillspawnshutoffradius( var_0, var_1 )
{
    var_2 = scripts\engine\utility::ter_op( isdefined( var_1 ), var_1, level.players );
...
_id_11FCE( var_0, var_1 )
{
    var_0 endon( "stopScene" );
    var_0.extractcountdown scripts\engine\utility::waittill_match_or_timeout( "camera", "opendoor", 30.0 );
    var_2 = scripts\engine\utility::ter_op( isdefined( var_1 ), var_1, level.players );
...
getintermissionspawnpointoverride( var_0 )
{
    _id_13A99( 0.65 );
    var_1 = scripts\engine\utility::ter_op( isdefined( var_0 ), var_0, level.players );
...
_id_12014( var_0, var_1 )
{
    _id_124D7( var_1 );

    if ( spawn_carried_punchcard_if_player_down() )
        _id_11D4E( var_0, var_1 );
    else if ( specialistperk() )
...
_id_11D4E( var_0, var_1 )
{
    thread _id_129F1( var_1 );
    _id_11D4D( var_0, var_1 );
    _id_1347C( var_0, var_1 );
    loadout_updateammo( var_0, var_1 );
}

playac130infilanim( var_0, var_1 )
{
    thread _id_129F1( var_1 );
    _id_11D36( var_0, var_1 );
    _id_1347B( var_0, var_1 );
...
_id_1203C( var_0, var_1 )
{
    ...
    _id_13484( var_0, var_1 );
    lockedradialunfill( var_0, var_1 );
}

getplatformrankxpmultiplier( var_0 )
{
    var_0 endon( "stopScene" );
    var_0.staticc130 waittillmatch( "plane", "transitionstart" );
    triggerfx( var_0.getplunderextractionsites );
}
...
modeignorevehicleexplosivedamage( var_0, var_1 )
{
    level endon( "game_ended" );
    var_2 = playersetattractionoff( var_0 );
    ...
    if ( var_4 )
    {
        level.select_players_not_in_killzone_only = newhudelem();
        _id_12A77( level.select_players_not_in_killzone_only );
    }
    else
    {
        foreach ( var_6 in var_3 )
        {
            ...
            var_6.select_players_not_in_killzone_only = newclienthudelem( var_6 );
            _id_12A77( var_6.select_players_not_in_killzone_only );
        }
    }

    [var_9, var_10] = playersetattractionlocationindex();
    var_2 waittillmatch( "camera", "fadeinstart" );

    if ( var_4 )
    {
        level.select_players_not_in_killzone_only fadeovertime( var_9 );
        level.select_players_not_in_killzone_only.alpha = 1;
    }
    else
    {
        foreach ( var_6 in var_3 )
        {
            ...
            var_6.select_players_not_in_killzone_only fadeovertime( var_9 );
            var_6.select_players_not_in_killzone_only.alpha = 1;
        }
    }
    ...
    if ( var_4 )
    {
        level.select_players_not_in_killzone_only fadeovertime( var_10 );
        level.select_players_not_in_killzone_only.alpha = 0;
    }
    else
    {
        foreach ( var_6 in var_3 )
        {
            ...
            var_6.select_players_not_in_killzone_only fadeovertime( var_10 );
            var_6.select_players_not_in_killzone_only.alpha = 0;
        }
    }

    _id_13A99( var_10 + 0.1 );

    if ( var_4 )
    {
        level.select_players_not_in_killzone_only destroy();
        level.select_players_not_in_killzone_only = undefined;
    }
    else
    {
        foreach ( var_6 in var_3 )
        {
            ...
            var_6.select_players_not_in_killzone_only destroy();
        }
    }
}

getplatformweaponrankxpmultiplier( var_0, var_1 )
{
    level endon( "game_ended" );
    var_0 endon( "stopScene" );
    var_2 = playersetattractionoff( var_0 );
    ...
    if ( var_4 )
    {
        level.select_woods_three_spawners = newhudelem();
        _id_12A78( level.select_woods_three_spawners );
    }
    else
    {
        foreach ( var_6 in var_3 )
        {
            ...
            var_6.select_woods_three_spawners = newclienthudelem( var_6 );
            _id_12A78( var_6.select_woods_three_spawners );
        }
    }
    ...
    if ( var_4 )
    {
        level.select_woods_three_spawners fadeovertime( var_8 );
        level.select_woods_three_spawners.alpha = 1;
    }
    else
    {
        foreach ( var_6 in var_3 )
        {
            ...
            var_6.select_woods_three_spawners fadeovertime( var_8 );
            var_6.select_woods_three_spawners.alpha = 1;
        }
    }
    ...
    if ( var_4 )
    {
        level.select_woods_three_spawners fadeovertime( var_9 );
        level.select_woods_three_spawners.alpha = 0;
    }
    else
    {
        foreach ( var_6 in var_3 )
        {
            ...
            var_6.select_woods_three_spawners fadeovertime( var_9 );
            var_6.select_woods_three_spawners.alpha = 0;
        }
    }
    ...
    if ( var_4 )
    {
        level.select_woods_three_spawners destroy();
        level.select_woods_three_spawners = undefined;
    }
    else
    {
        foreach ( var_6 in var_3 )
        {
            ...
            var_6.select_woods_three_spawners destroy();
        }
    }
}

select_stadium_one_spawners()
{
    ...
    wait 24.0;
    level.creategulagloadoutarry = undefined;
}

select_patrol_five_spawners( var_0 )
{
...
select_patrol_four_spawners( var_0 )
{
    var_1 = getdvarfloat( "LKOLRONRNQ" );
    var_2 = getdvarint( "LTQMSPKRKO" );
    var_3 = getdvarint( "MROOOROPKL" );
...
{
    if ( isdefined( var_0.get_next_open_stop ) )
        var_0.get_next_open_stop hide();

    var_0.extractcountdown scripts\engine\utility::waittill_match_or_timeout( "camera", "showdoor", 30.0 );

    if ( isdefined( var_0.get_next_open_stop ) )
        var_0.get_next_open_stop show();
}
...
burningdown( var_0, var_1 )
{
    ...
    if ( var_0.infil_anim_type == "player" )
    {
        if ( !isdefined( var_1 ) )
        {
            ...
            {
                var_5 = scripts\mp\gametypes\br_public.gsc::postkillcamplunderlost( var_4 );

                for ( var_6 = 0; var_6 < var_5.size; var_6++ )
                {
                    var_7 = var_5[var_6];
                    var_8 = scripts\mp\gametypes\br_public.gsc::postplunder( var_4, var_7 );
                    var_9 = var_8.size == 1;
                    var_10 = empty_vo_func( var_8.size );

                    foreach ( var_14, var_12 in var_8 )
                    {
                        if ( var_12 scripts\mp\gametypes\br_public.gsc::spawnanglemin() )
                        ...
                        var_12.select_patrol_seven_spawners = var_9;
                    }
                }
            }
            ...
            {
                if ( isdefined( var_12 ) && !isdefined( var_12.select_patrol_seven_spawners ) )
                {
                    var_12.infilanimindex = 1;
                    var_12.select_patrol_seven_spawners = 1;
                }
            }
...
_id_11D36( var_0, var_1 )
{
    ...
    if ( isdefined( var_0.extract_removemissionweapon ) )
        var_0.extract_removemissionweapon scriptmodelplayanim( "sdr_mp_infil_ac130_redux_players_cam", "camera" );
    ...
    foreach ( var_10 in var_8 )
    {
        ...
        if ( !var_7 )
        {
            ...
            if ( !istrue( level.stack_patch_waittill_root ) && !isdefined( level._id_12590 ) )
                var_10 setsoundsubmix( "mp_br_infil_ac130", 0.0 );

            var_10 setclienttriggeraudiozone( "mp_donetsk_infil_int", 1.0 );
        }

        var_0.extract_ontimerexpired = "j_prop_" + var_10.infilanimindex;
        var_10 cameraunlink();
        var_10 cameralinkto( var_0.cameraent, var_0.extract_ontimerexpired, 1, 1 );
    ...
    thread getintermissionspawnpointoverride( var_1 );
    ...
    level thread x1opsspawnnpc( var_0 );
    level thread _id_12361( var_0 );
    level thread race_requires_vehicle( var_0 );
    level thread _id_11FCC( var_0, var_1 );
    level thread getplatformrankxpmultiplier( var_0 );
    level thread modeignorevehicleexplosivedamage( var_0, var_1 );
    level thread select_patrol_five_spawners( var_0 );
    level thread initcashtracking( var_0, var_1 );
    ...
    level gethillspawnshutofforigin( var_0, var_1 );
}

_id_11D4D( var_0, var_1, var_2, var_3 )
{
    var_0 endon( "stopScene" );
    burningdown( var_0, var_1 );
    var_4 = 48;
    ...
    var_7 = 2.0;
    var_8 = 4;
    var_0.extractcountdown scriptmodelplayanim( "wz_infil_mindia8_solo_cam", "camera" );

    if ( isdefined( var_0.extractcountdownmsg ) )
        var_0.extractcountdownmsg scriptmodelplayanim( "wz_infil_mindia8_solo_cam", "camera" );

    var_0.extractgroundpos scriptmodelplayanim( "wz_infil_mindia8_squad_cam", "camera" );

    if ( isdefined( var_0.extracthelipadusecallback ) )
        var_0.extracthelipadusecallback scriptmodelplayanim( "wz_infil_mindia8_squad_cam", "camera" );

    if ( isdefined( var_0.waitforgulagfightstocomplete ) )
        var_0.waitforgulagfightstocomplete stoploopsound();

    var_0.binoculars_processlosqueue hide();
    var_0.binoculars_hascoldblooded hide();
    ...
    foreach ( var_11 in var_9 )
    {
        ...
        var_12 = scripts\engine\utility::ter_op( var_11.select_patrol_seven_spawners, var_0.extractcountdown, var_0.extractgroundpos );
        var_13 = "j_prop_1";
        var_11 cameraunlink();
        ...
        var_0.binoculars_processlosqueue showtoplayer( var_11 );
        var_0.binoculars_hascoldblooded showtoplayer( var_11 );
    }

    thread getintermissionspawnpointoverride( var_1 );

    if ( isdefined( var_0.waitforgulagfightstocomplete ) )
        var_0.waitforgulagfightstocomplete setscriptablepartstate( "infil_fx_hero", "on" );
    ...
    if ( var_0.infil_anim_type == "script_model" )
    {
        if ( istrue( var_2 ) )
        {
            var_0._id_11F7A show();
            var_0._id_11F7B hide();
            var_0._id_11F7C hide();
            var_0._id_11F7D hide();
            var_0._id_11F7E hide();
        }
        else
        {
            var_0._id_11F7A hide();
            var_0._id_11F7B show();
            var_0._id_11F7C show();
            var_0._id_11F7D show();
            var_0._id_11F7E show();
        }
        ...
        var_0._id_11F7A scriptmodelplayanim( "wz_infil_mindia8_solo_player" + var_15, "p1" );
        var_0._id_11F7B scriptmodelplayanim( "wz_infil_mindia8_squad_player1" + var_15, "p1" );
        var_0._id_11F7C scriptmodelplayanim( "wz_infil_mindia8_squad_player2" + var_15, "p2" );
        var_0._id_11F7D scriptmodelplayanim( "wz_infil_mindia8_squad_player3" + var_15, "p3" );
        var_0._id_11F7E scriptmodelplayanim( "wz_infil_mindia8_squad_player4" + var_15, "p4" );
    }
    else if ( var_0.infil_anim_type == "player" )
        thread playinfilplayeranims( var_0, var_1 );

    var_0.binoculars_processlosqueue scriptmodelplayanim( "wz_infil_mindia8_solo_pilot", "aiPilot" );
    var_0.binoculars_hascoldblooded scriptmodelplayanim( "wz_infil_mindia8_solo_copilot", "aiCopilot" );
    var_0.brleaderdialogteam scriptmodelplayanim( "wz_infil_mindia8_armada", "armadaRig" );

    foreach ( var_17 in var_0.brleaderdialogplayer )
    {
    ...
    level thread _id_11FCD( var_0, var_1 );
    level thread get_model_for_color_wire_cut( var_0 );
    level thread modeignorevehicleexplosivedamage( var_0, var_1 );
    level thread select_patrol_four_spawners( var_0 );
    level thread initcashtracking( var_0, var_1 );
    var_0.extractcountdown scripts\engine\utility::waittill_match_or_timeout( "camera", "transition", 30.0 );
    var_0 notify( "infil_reset_light_dvars" );
    level gethillspawnshutoffradius( var_0, var_1 );
}
...
_id_1203B( var_0, var_1, var_2, var_3 )
{
    ...
    var_7 = 2.0;
    var_8 = 4;
    var_0.extractcountdown scriptmodelplayanim( "wz_infil_skilo_single_cam", "camera" );

    if ( isdefined( var_0.extractcountdownmsg ) )
        var_0.extractcountdownmsg scriptmodelplayanim( "wz_infil_skilo_single_cam", "camera" );

    var_0.extractgroundpos scriptmodelplayanim( "wz_infil_skilo_squad_cam", "camera" );

    if ( isdefined( var_0.extracthelipadusecallback ) )
        var_0.extracthelipadusecallback scriptmodelplayanim( "wz_infil_skilo_squad_cam", "camera" );

    if ( isdefined( var_0.waitforoneplayernearlz ) )
        var_0.waitforoneplayernearlz stoploopsound();

    var_0._id_13057 setscriptablepartstate( "infil_fx_skilo", "static" );
    ...
    thread changeinfiloverlaystateforallplayersforskilo( var_10, 0 );
    ...
    foreach ( var_12 in var_10 )
    {
        ...
        if ( !var_9 )
        {
            if ( var_12.select_patrol_seven_spawners )
            {
                thread select_woods_one_spawners( "br_skilo_solo_infil_part1_lr", 0.8, var_12 );
                thread select_woods_one_spawners( "br_skilo_solo_infil_part2_lr", 6.4, var_12 );
            }
            else
            {
                thread select_woods_one_spawners( "br_skilo_quad_infil_part1_lr", 0.8, var_12 );
                thread select_woods_one_spawners( "br_skilo_quad_infil_part2_lr", 6.4, var_12 );
            }
        ...
        var_13 = scripts\engine\utility::ter_op( var_12.select_patrol_seven_spawners, var_0.extractcountdown, var_0.extractgroundpos );
        var_14 = "j_prop_1";
        var_12 cameraunlink();
        ...
        var_12 setpredictedstreamloaddist( 5000.0 );
        var_12 scripts\mp\gametypes\br_public.gsc::_id_11F95( var_0._id_13057.origin );
    }

    thread getintermissionspawnpointoverride( var_1 );

    if ( isdefined( var_0.waitforoneplayernearlz.branalytics_gulagstart ) )
    {
        var_0.waitforoneplayernearlz.branalytics_gulagstart setscriptablepartstate( "infil_fx_clouds", "on" );
        var_0.waitforoneplayernearlz.branalytics_gulagstart setscriptablepartstate( "infil_fx_skilo", "moving" );
    }
    ...
    if ( var_0.infil_anim_type == "script_model" )
    {
        ...
        var_0._id_11F7A scriptmodelplayanim( "wz_infil_skilo_single_sdr01" + var_16, "p1" );
        var_0._id_11F7B scriptmodelplayanim( "wz_infil_skilo_squad_sdr01" + var_16, "p1" );
        var_0._id_11F7C scriptmodelplayanim( "wz_infil_skilo_squad_sdr02" + var_16, "p2" );
        var_0._id_11F7D scriptmodelplayanim( "wz_infil_skilo_squad_sdr03" + var_16, "p3" );
        var_0._id_11F7E scriptmodelplayanim( "wz_infil_skilo_squad_sdr04" + var_16, "p4" );
    }
    else if ( var_0.infil_anim_type == "player" )
        thread playinfilplayeranims( var_0, var_1 );

    if ( isdefined( var_0._id_13057 ) )
        var_0._id_13057 scriptmodelplayanim( "wz_infil_skilo_single_veh", "plane", 0, 1 );

    if ( isdefined( var_0._id_1261E ) )
        var_0._id_1261E scriptmodelplayanim( "wz_infil_skilo_single_veh_ropes", "plane", 0, 1 );

    foreach ( var_18 in var_0._id_121D9 )
    {
        if ( isdefined( var_18 ) )
            var_18 scriptmodelplayanim( "wz_infil_skilo_single_veh", "plane", 0, 1 );
    }

    if ( isdefined( var_0._id_13058 ) )
        var_0._id_13058 scriptmodelplayanim( "wz_infil_skilo_single_door", "plane", 0, 1 );

    level thread _id_11FCE( var_0, var_1 );
    level thread _id_12BCC( var_0 );
    level thread modeignorevehicleexplosivedamage( var_0, var_1 );
    level thread getplatformweaponrankxpmultiplier( var_0, var_1 );
    level thread hide_gas_circle_skilo( var_0 );
    level thread delete_props_skilo( var_0, 1 );
    level thread select_patrol_five_spawners( var_0 );
    level thread initcashtracking( var_0, var_1 );
    var_0.extractcountdown scripts\engine\utility::waittill_match_or_timeout( "camera", "transition", 30.0 );
    var_0 notify( "infil_reset_light_dvars" );
    level thread gethost( var_0, var_1 );
}

hide_gas_circle_skilo( var_0 )
{
    level endon( "game_ended" );
    ...
    var_1 = playersetattractionoff( var_0 );
    level.br_circle.dangercircleent.hidden++;
    level notify( "update_circle_hide" );
    var_1 scripts\engine\utility::waittill_match_or_timeout( "camera", "end", 30.0 );
    level.br_circle.dangercircleent.hidden--;
    level notify( "update_circle_hide" );
}

select_woods_one_spawners( var_0, var_1, var_2 )
{
    wait( var_1 );
    var_2 playlocalsound( var_0 );
}

initcashtracking( var_0, var_1 )
{
    var_2 = playersetattractionstateindex( var_0 );
    ...
    if ( isdefined( var_2 ) && getdvarint( "scr_br_streamToMovingPlane", 1 ) == 1 )
    {
        ...
        foreach ( var_9 in var_7 )
        {
            ...
            var_9 scripts\mp\gametypes\br_public.gsc::_id_11F95( var_4 );
        }
    }
}

_id_130C1( var_0, var_1 )
{
    var_2 = playersetattractionstateindex( var_0 );
    ...
    if ( isdefined( var_2 ) && getdvarint( "scr_br_streamToMovingPlane", 1 ) == 1 )
    {
        ...
        foreach ( var_6 in var_4 )
        {
            ...
            var_6 thread scripts\mp\gametypes\br_public.gsc::_id_11E11();
        }
    }
}

_id_13686( var_0, var_1 )
{
    wait( var_0 );
    var_2 = scripts\engine\utility::ter_op( isdefined( var_1 ), var_1, level.players );
...
_id_1347B( var_0, var_1 )
{
    ...
    if ( isdefined( var_0.extract_removemissionweapon ) )
    {
        ...
        foreach ( var_9 in var_7 )
        {
            ...
            var_0.extract_ontimerexpired = "j_prop_" + var_9.infilanimindex;
            var_9 cameraunlink();
            var_9 cameralinkto( var_0.extract_removemissionweapon, var_0.extract_ontimerexpired, 1, 1 );
            var_9 setclientdvar( "QTSPTNLOL", var_2 );
            var_9 setphysicaldepthoffield( var_3, var_4, var_5, var_6 );
        }

        thread getinfilplayers( var_1 );
    }
    else
    {
        var_7 = scripts\engine\utility::ter_op( isdefined( var_1 ), var_1, level.players );
    ...
    if ( isdefined( var_0.getplunderextractionsites ) )
    {
        if ( isdefined( var_0.movingc130 ) )
            var_0.getplunderextractionsites.origin = var_0.movingc130.origin;
    }
    ...
    thread getinitialwinningteam( var_1 );
    ...
    if ( isdefined( var_0.extract_removemissionweapon ) )
        var_0.extract_removemissionweapon waittillmatch( "camera", "end" );

    thread _id_13686( 1.2, var_1 );
}
...
_id_1347C( var_0, var_1 )
{
    ...
    if ( isdefined( var_0.extractcountdownmsg ) && isdefined( var_0.extracthelipadusecallback ) )
    {
        var_7 = scripts\engine\utility::ter_op( isdefined( var_1 ), var_1, level.players );
        ...
        foreach ( var_9 in var_7 )
        {
            ...
            if ( !isdefined( var_9.select_patrol_seven_spawners ) )
                var_9.select_patrol_seven_spawners = 1;

            var_10 = scripts\engine\utility::ter_op( var_9.select_patrol_seven_spawners, var_0.extractcountdownmsg, var_0.extracthelipadusecallback );
            var_11 = "j_prop_1";
            var_9 cameraunlink();
    ...
    if ( isdefined( var_0.waitforgulagfightstocomplete ) )
    {
        var_0.playerpositionents["parent_solo"] linkto( var_0.waitforgulagfightstocomplete, "", ( 0, 0, 0 ), ( 0, 0, 0 ) );
        var_0.playerpositionents["parent_squad"] linkto( var_0.waitforgulagfightstocomplete, "", ( 0, 0, 0 ), ( 0, 0, 0 ) );
        var_0.waitforgulagfightstocomplete notify( "start_moving" );
    }

    var_0.brleaderdialogteam unlink();

    if ( isdefined( var_0.waitforgulagfightstocomplete ) )
        var_0.brleaderdialogteam linkto( var_0.waitforgulagfightstocomplete, "", ( 0, 0, 0 ), ( 0, 0, 0 ) );
    ...
    if ( var_0.infil_anim_type == "script_model" )
    {
        var_0._id_11F7A scriptmodelplayanim( "wz_infil_mindia8_loop_pl01", "p1" );
        var_0._id_11F7B scriptmodelplayanim( "wz_infil_mindia8_loop_pl01", "p1" );
        var_0._id_11F7C scriptmodelplayanim( "wz_infil_mindia8_loop_pl02", "p2" );
        var_0._id_11F7D scriptmodelplayanim( "wz_infil_mindia8_loop_pl03", "p3" );
        var_0._id_11F7E scriptmodelplayanim( "wz_infil_mindia8_loop_pl04", "p4" );
    }

    var_0.binoculars_processlosqueue hide();
    var_0.binoculars_hascoldblooded hide();
    level thread _id_11E07( var_0 );

    if ( isdefined( var_0.extractcountdownmsg ) )
        var_0.extractcountdownmsg waittillmatch( "camera", "end" );
...
_id_13484( var_0, var_1 )
{
    ...
    if ( isdefined( var_0.waitforoneplayernearlz ) )
    {
        var_0.playerpositionents["parent_solo"] linkto( var_0.waitforoneplayernearlz, "", ( 0, 0, 0 ), ( 0, 0, 0 ) );
        var_0.playerpositionents["parent_squad"] linkto( var_0.waitforoneplayernearlz, "", ( 0, 0, 0 ), ( 0, 0, 0 ) );
        var_0.waitforoneplayernearlz notify( "start_moving" );
    }
    ...
    level thread _id_1203D( var_0 );

    if ( isdefined( var_0.waitforoneplayernearlz.door ) )
        var_0.waitforoneplayernearlz.door scriptmodelplayanim( "wz_infil_skilo_single_door_loop", "plane", 0, 1 );
...
loadout_fixcopiedclassstruct( var_0, var_1 )
{
    ...
    level thread _id_130C1( var_0, var_1 );
}
...
loadout_updateammo( var_0, var_1 )
{
    ...
    if ( isdefined( var_0.waitforgulagfightstocomplete ) )
        var_0.waitforgulagfightstocomplete playloopsound( "br_heli_infil_hero_lp" );
...
{
    thread tr_ontimerexpired();
    var_0._id_13057 stoploopsound();

    if ( isdefined( var_0.waitforoneplayernearlz ) )
        var_0.waitforoneplayernearlz playloopsound( "br_infil_skilo_plane_lp" );
...
select_players_in_killzone_first( var_0 )
{
    if ( spawn_carried_punchcard_if_player_down() )
        select_patrol_two_spawners( var_0 );
    else if ( specialistperk() )
        select_players_in_killzone_only( var_0 );
    else
        select_patrol_three_spawners( var_0 );
}
...
_id_1203D( var_0 )
{
    if ( isdefined( var_0.waitforoneplayernearlz ) )
    {
        var_0.waitforoneplayernearlz scriptmodelplayanim( "wz_infil_skilo_loop_veh", "plane" );
        var_0.waitforoneplayernearlz.branalytics_gulagstart scriptmodelplayanim( "wz_infil_skilo_loop_veh", "plane" );
        var_0.waitforoneplayernearlz.innards scriptmodelplayanim( "wz_infil_skilo_loop_veh", "planeInnards" );
    }
...
_id_11E07( var_0 )
{
    if ( isdefined( var_0.waitforgulagfightstocomplete ) )
    {
        var_0.waitforgulagfightstocomplete scriptmodelplayanim( "wz_infil_mindia8_loop_veh", "plane" );
        var_0.waitforgulagfightstocomplete.innards scriptmodelplayanim( "wz_infil_mindia8_loop_veh", "planeInnards" );
    }
...
selfrevivemonitorrevivebuttonpressed( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    foreach ( var_8 in var_5 )
    {
        ...
        switch ( var_4 )
        {
            ...
            case "skilo_infil":
                ...
                var_8 clearsoundsubmix( "fade_to_black_all_except_music", 0.5 );
                var_8 setsoundsubmix( "fade_to_black_all_except_music_and_scripted5", 0.5 );
                scripts\mp\utility\sound::assigned_ai( "br_infil_skilo" );
                break;
            default:
                var_8 playlocalsound( "br_infil_part1_lr" );
                var_8 clearsoundsubmix( "mp_br_lobby_fade", 1.5 );
    ...
    else if ( isdefined( level.deadblue ) && isdefined( level.deadblue.name ) && level.deadblue.name == "zxp" )
    ...
    var_2 = var_2 || istrue( level.stack_patch_waittill_root );

    if ( var_6 )
    {
        if ( istrue( level.stack_patch_waittill_root ) || var_2 )
    ...
    else
    {
        foreach ( var_8 in var_5 )
        {
            ...
            if ( istrue( level.stack_patch_waittill_root ) || var_2 )
            {
                var_8 playcinematicforplayer( var_1, 1, 1 );
                continue;
            }
    ...
    if ( getdvarint( "scr_br_c130_intro_s2", 0 ) == 1 )
        var_12 = 9;
    else if ( spawn_carried_punchcard_if_player_down() )
        var_12 = 11;
    else if ( specialistperk() )
        var_12 = 11;
    ...
    foreach ( var_8 in var_5 )
    {
        ...
        if ( !istrue( var_8.teamclearextractplunder ) )
...
changeinfiloverlaystateforallplayersforskilo( var_0, var_1 )
{
    var_2 = scripts\engine\utility::ter_op( isdefined( var_0 ), var_0, level.players );
    wait 0.45;
...
select_stairway_spawners( var_0, var_1 )
{
...
infilskilovideocompletecallback( var_0, var_1 )
{
    if ( var_0 == "bink_complete" )
    {
        wait 0.6;
...
selfrevivebuttonpressed( var_0, var_1 )
{
    if ( var_0 == "bink_complete" )
    {
        self setclientomnvar( "ui_br_bink_overlay_state", 0 );
...
semtex_killstuckplayer( var_0 )
{
    if ( getdvarint( "scr_br_c130_intro_s2", 0 ) == 1 )
        scripts\mp\utility\lui_game_event_aggregator::registeronluieventcallback( ::select_stairway_spawners );
    else if ( specialistperk() )
        scripts\mp\utility\lui_game_event_aggregator::registeronluieventcallback( ::infilskilovideocompletecallback );
    else
        scripts\mp\utility\lui_game_event_aggregator::registeronluieventcallback( ::selfrevivebuttonpressed );
...
regroup_all_alive_players_to_plane()
{
    ...
    else if ( specialistperk() )
    {
        ...
        self.branalytics_gulagstart = spawn( "script_model", self.origin );
        self.branalytics_gulagstart setmodel( "veh8_mil_air_skilo_infil_flight_articulated" );
        self.branalytics_gulagstart linkto( self, "", ( 0, 0, 0 ), ( 0, 0, 0 ) );
        self.branalytics_gulagstart.cleanme = 1;
        self.door = spawn( "script_model", self.origin );
        self.door setmodel( "veh8_mil_air_skilo_interior_infil_cabin_door" );
        self.door linkto( self.branalytics_gulagstart, "TAG_DOOR", ( 0, 0, 0 ), ( 0, 0, 0 ) );
        self.door.cleanme = 1;
    }
    else
    {
...
{
    if ( spawn_carried_punchcard_if_player_down() )
        return var_0.waitforgulagfightstocomplete;
    else if ( specialistperk() )
        return var_0.waitforoneplayernearlz;
    else
...
{
    if ( spawn_carried_punchcard_if_player_down() )
        return var_0.extractcountdownmsg;
    else if ( specialistperk() )
        return var_0.extractcountdownmsg;
    else
        return var_0.extract_removemissionweapon;
}
...
playersetattractiontime( var_0 )
{
    ...
    if ( spawn_carried_punchcard_if_player_down() )
    {
        if ( istrue( var_0.select_patrol_seven_spawners ) )
        {
...
check_digit_models_to_create( var_0, var_1, var_2 )
{
    level endon( "game_ended" );
    ...
    level.infilstruct.transporttime = scripts\mp\gametypes\br_c130.gsc::spawnc130( level.infilstruct.c130pathstruct, ::regroup_at_truck, ::regroup_all_alive_players_to_plane );
    level.infilstruct.firstc130endtime = level.infilstruct.transporttime + gettime();
    var_7 = emp_drone_pick_up_use_think( level.br_ac130, var_1 );
    level.teamanchoredwidgetinstances = var_7;

    if ( isdefined( level._id_1211F ) )
        [[ level._id_1211F ]]( var_7 );

    level.select_patrol_six_spawners = 1;
    level.allowprematchdamage = 0;
    level.br_ac130 thread gunship_updateplayercount();
    var_8 = playersetcontrols( var_7 );
    var_9 = 0;

    if ( getdvarint( "scr_br_c130_intro_s2", 0 ) == 1 )
    {
        _id_13483( var_3, "player", var_8, 1 );
        selfrevivethink( var_3, "mp_donetsk_c130_intro_s2" );
        selfrevivemonitorrevivebuttonpressed( var_3, "mp_donetsk_c130_intro_s2", var_9, 0, "s2" );
        semtex_killstuckplayer( var_3 );
        var_10 = game["music"]["br_infil_intro"].size;
        ...
        foreach ( var_5 in var_3 )
        {
            ...
            var_5 setplayermusicstate( game["music"]["br_infil_intro"][var_11] );
        }
    }
    else if ( scripts\mp\utility\game::postupdategameevents() == "reveal" )
    {
        _id_13483( var_3, var_1, var_8, 0 );
        var_14 = undefined;
        var_15 = 0;

        if ( spawn_carried_punchcard_if_player_down() )
        {
            var_14 = "mp_donetsk_mindia8_intro";
            var_15 = 1;
        }
        else if ( specialistperk() )
            var_14 = "mp_wz_island_plane_intro";
        else
            var_14 = "mp_donetsk_c130_intro";

        selfrevivethink( var_3, var_14 );
        selfrevivemonitorrevivebuttonpressed( var_3, var_14, var_9, var_15, "infil_dov_part1" );
        semtex_killstuckplayer( var_3 );
    }
    else
    {
        ...
        if ( spawn_carried_punchcard_if_player_down() )
        {
            if ( scripts\cp_mp\utility\game_utility::spawnproplist() )
                var_14 = "mp_escape_mindia8_intro";
            else
                var_14 = "mp_donetsk_mindia8_intro";

            var_15 = 1;
            var_16 = "chopper_infil";
        }
        else if ( specialistperk() )
        {
            var_14 = "mp_wz_island_plane_intro";
        ...
        selfrevivethink( var_3, var_14 );
        selfrevivemonitorrevivebuttonpressed( var_3, var_14, var_9, var_15, var_16 );
        semtex_killstuckplayer( var_3 );
    }

    if ( scripts\mp\gametypes\br_public.gsc::spawn_apache_chopper() || scripts\mp\gametypes\br_public.gsc::spawn_exfil_chopper() )
    {
        var_3 = scripts\engine\utility::ter_op( isdefined( var_2 ), var_2, level.players );
    ...
    [var_20, var_21] = playersetattractionextradata();
    level thread infilallfadetoblack( 0.0, var_20, var_21, undefined, var_2 );
    _id_12014( var_7, var_2 );
    var_3 = scripts\engine\utility::ter_op( isdefined( var_2 ), var_2, level.players );
    _id_13481( var_3, var_1 );
    level.select_patrol_six_spawners = 0;
    level.infilstruct.infil_anim_type = var_1;

    if ( isdefined( level.make_emp_config ) )
        level thread scripts\mp\gametypes\br_alt_mode_escape.gsc::make_javelin_interact( var_2 );
    else
    {
        thread scripts\mp\gametypes\br_gametypes.gsc::_id_1268B( "onInfilSequenceEnd", var_2 );

        if ( !scripts\mp\gametypes\br_gametypes.gsc::spawn_juggernauts_fob( "match_start_VO" ) )
            scripts\mp\gametypes\br_public.gsc::brleaderdialog( "match_start", 0, var_2 );
    }

    var_22 = scripts\mp\gametypes\br_public.gsc::spreadshotdamagemod() || scripts\mp\gametypes\br_public.gsc::spawn_exfil_enemies();

    if ( !var_22 )
    {
        var_23 = getdvar( "scr_br_lateSpawnFallback" ) == "";
        var_3 = scripts\engine\utility::ter_op( isdefined( var_2 ), var_2, level.players );
        ...
        foreach ( var_5 in var_3 )
        {
            ...
            if ( !istrue( var_5.createhudstring ) && var_23 )
                continue;
        ...
        if ( scripts\mp\gametypes\br_gametypes.gsc::spawn_atmines( "addToC130Infil" ) )
            level.br_ac130 scripts\mp\gametypes\br_gametypes.gsc::_id_1268B( "addToC130Infil" );
    }

    if ( isdefined( level.removealldeathicons ) )
        level thread removechevronsfromarray();
}
...
infilallfadetoblack( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    ...
    if ( var_7 && !isdefined( level.infilblackoverlay ) )
    {
        ...
        _id_12A76( level.infilblackoverlay );
    }
    else if ( !var_7 )
    {
        foreach ( var_9 in var_6 )
        {
            ...
            _id_12A76( var_9.infilblackoverlay );
        }
    }
...
_id_13483( var_0, var_1, var_2, var_3 )
{
    foreach ( var_5 in var_0 )
    {
        ...
        var_5 scripts\mp\class::chopper_glow_sticks();
    }
    ...
    level.select_woods_two_spawners = 1;
    _id_13A99( 1 );
    waitframe();
    ...
    _id_13A97( var_0, var_2.origin, var_2 );
...
_id_13481( var_0, var_1 )
{
    ...
    foreach ( var_4 in var_0 )
    {
        ...
        var_4 scripts\mp\class::_id_13682();
    ...
    setomnvar( "ui_in_infil", -1 );
    level.select_woods_two_spawners = undefined;
}
...
playerjoininfil()
{
    ...
    var_0 scripts\mp\gametypes\br_public.gsc::_id_11F2D();
    var_0 thread _id_11E0C();
}
...
handleexclusionils( var_0, var_1 )
{
    ...
    if ( !istrue( level.failhistoryquest ) && !istrue( var_2.issquadleader ) )
        return 0;
    ...
    if ( istrue( level.failhistoryquest ) && !istrue( var_2.issquadleader ) )
        return var_2 relic_dogtags( var_0, var_1, var_6 );

    foreach ( var_9 in level.br_spawns )
    {
        var_10 = var_2 postspawn_assault3_bombers();

        if ( isdefined( var_10 ) && var_9 == var_10 )
        {
            if ( level.failhistoryquest )
            {
                if ( var_2 get_closest_munitions( var_9, var_6, var_0, 1 ) )
                    return 1;
            }

            continue;
        }
        else if ( level.failhistoryquest )
        {
            if ( !var_2 get_closest_munitions( var_9, var_6, var_0, 0 ) )
                return 0;
        }
...
relic_dogtags( var_0, var_1, var_2 )
{
    var_3 = self;

    foreach ( var_5 in level.br_spawns )
    {
        var_6 = postspawn_assault3_bombers();
        var_7 = 0;
        ...
        if ( pow( var_2 + var_5.spawnexclusion2ddist, 2 ) > distance2dsquared( var_5.locationorigin, var_0 ) )
        {
            if ( var_7 )
            {
                var_8 = var_3 poke_the_player_after_faux_death();
                inside_bush( var_8 );
        ...
        if ( var_7 && var_3 get_closest_munitions( var_5, var_2, var_0, var_7 ) )
            return 1;
    }
...
get_closest_munitions( var_0, var_1, var_2, var_3 )
{
    var_4 = self;

    foreach ( var_6 in var_0._id_12CC2 )
    {
        if ( !isdefined( var_6 ) )
            continue;

        var_7 = var_4 poke_the_player_after_faux_death();
...
registerspawnlocation( var_0, var_1, var_2 )
{
    ...
    {
        var_5 = var_3 postspawn_assault3_bombers();

        if ( !isdefined( var_5 ) )
            var_5 = var_3 should_enter_combat_after_checking_gas_grenade();
    }
    else if ( istrue( level.failhistoryquest ) )
    {
        var_5 = var_3 poke_the_player_after_faux_death();

        if ( !isdefined( var_5 ) )
            var_5 = var_3 should_do_damage_check_func_relics();
    }
    else
        return 0;
...
_id_1329F()
{
    ...
    var_0 playershow( 1 );
    var_0 unlink( 1 );
    var_1 = var_0 postspawn_assault3_bombers();

    if ( isdefined( var_1.locationorigin ) )
    {
        var_2 = var_1.locationorigin;
        var_3 = var_0 poke_the_player_after_faux_death();
...
{
    var_0 = self;
    var_1 = var_0 poke_the_player_after_faux_death();
    var_2 = var_0 postspawn_assault3_bombers();
...
spawnselectionend()
{
    ...
    foreach ( var_2 in level.players )
    {
        ...
        var_3 = var_2 postspawn_assault3_bombers();
...
poke_the_player_after_faux_death()
{
    ...
    if ( !istrue( level.failhistoryquest ) || istrue( var_0.issquadleader ) )
        return undefined;
    ...
    {
        if ( isdefined( var_1._id_12CC2[var_0.pers["squadMemberIndex"]] ) && isdefined( var_1._id_12CC2[var_0.pers["squadMemberIndex"]].locationorigin ) )
            return var_1._id_12CC2[var_0.pers["squadMemberIndex"]];
    }
...
should_do_damage_check_func_relics()
{
    var_0 = self;
    ...
    {
        var_1._id_12CC2[var_0.pers["squadMemberIndex"]] = spawnstruct();
        var_1._id_12CC2[var_0.pers["squadMemberIndex"]].circleent = undefined;
        return var_1._id_12CC2[var_0.pers["squadMemberIndex"]];
    }
...
should_enter_combat_after_checking_gas_grenade()
{
    ...
    if ( isdefined( level.br_spawns ) && isdefined( var_0.team ) )
    {
        ...
        level.br_spawns[var_0.team]._id_12CC2 = [];
        return level.br_spawns[var_0.team];
    }
...
_id_124C1()
{
    ...
    if ( isdefined( level.br_spawns ) )
    {
        ...
        {
            if ( isdefined( var_1._id_12CC2 ) )
            {
                foreach ( var_3 in var_1._id_12CC2 )
                    inside_bush( var_3 );
            }
...
waitfor_firstgroup_killedoffenough( var_0, var_1, var_2, var_3 )
{
    self moveto( var_0, var_1, var_2, var_3 );
}

_id_12625( var_0, var_1, var_2, var_3 )
{
    self rotateto( var_0, var_1, var_2, var_3 );
}

removechevronsfromarray()
{
    var_0 = distance( level.br_ac130._id_11B1B.startpt, level.br_ac130._id_11B1B.endpt );
    var_1 = var_0 / scripts\mp\gametypes\br_c130.gsc::getc130speed() / level.removealldeathicons.size;

    for ( var_2 = 0; var_2 < level.removealldeathicons.size; var_2++ )
    {
        wait( var_1 );
        level.removealldeathicons[var_2] notify( "halo_jump_c130" );
    }

    level.removealldeathicons = undefined;
}

_id_123C6( var_0 )
{
    if ( !isdefined( level.removealldeathicons ) )
        level.removealldeathicons = [];

    level.removealldeathicons[level.removealldeathicons.size] = var_0;
}
