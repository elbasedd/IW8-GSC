...
main()
{
    ...
    if ( getdvarint( "scr_hotfoot", 0 ) != 0 )
        level.onplayerkilled = ::_id_1197F;
...
onstartgametype()
{
    ...
    if ( isdefined( level.should_drop_grenade_pickup ) )
        [[ level.should_drop_grenade_pickup ]]();
    else
        initspawns();
    ...
    if ( getdvarint( "scr_hotfoot", 0 ) != 0 )
        thread vehomn_updateomnvarsperframe();
}
...
vehomn_updateomnvarsperframe()
{
    ...
    level.run_track_enemy_patrollers = 0;
    var_0 = 0;
    ...
    for (;;)
    {
        ...
        {
            if ( !isdefined( var_2.run_vehicle_group_with_respawn_on_failed_unload ) )
            {
                var_2.run_trap_room_combat = 0;
                var_2.run_vehicle_group_with_respawn_on_failed_unload = var_2.origin;
                var_2.allowfultondropondeath = var_2.origin;
            }

            var_2.run_trap_room_combat = var_2.run_trap_room_combat + distancesquared( var_2.run_vehicle_group_with_respawn_on_failed_unload, var_2.origin );
            var_2.run_vehicle_group_with_respawn_on_failed_unload = var_2.origin;
        }
        ...
        if ( var_0 > 3.0 )
        {
            if ( level.run_track_enemy_patrollers >= 2 )
            {
                level.run_track_enemy_patrollers = 0;

                foreach ( var_2 in level.players )
                {
                    if ( !isdefined( var_2.allowfultondropondeath ) )
                        var_2.allowfultondropondeath = var_2.origin;

                    var_2.obj_a_behavior = distancesquared( var_2.allowfultondropondeath, var_2.origin ) < 4225;
                    var_2.allowfultondropondeath = var_2.origin;
                }
            }
            ...
            foreach ( var_2 in level.players )
            {
                ...
                if ( isalive( var_2 ) && ( istrue( var_2.obj_a_behavior ) || var_2.run_trap_room_combat < 4225 && !istrue( var_2.runautocircle ) ) )
                {
                    var_2.obj_a_behavior = 0;
        ...
        level.run_track_enemy_patrollers++;
        waitframe();
    }
}

_id_1197F( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9 )
{
