...
main()
{
    ...
    while ( !isdefined( level._id_134DF ) )
        waitframe();
    ...
    switch ( level.trial["missionScript"] )
    {
        case "gun_nonlinear":
        case "gun":
            thread [[ level._id_134DF["gun"] ]]();
            break;
        case "clear":
            thread [[ level._id_134DF["clear"] ]]();
            break;
        case "sniper":
            thread [[ level._id_134DF["sniper"] ]]();
            break;
        case "jugg":
            thread [[ level._id_134DF["jugg"] ]]();
            break;
        case "lava":
            thread [[ level._id_134DF["lava"] ]]();
            break;
        case "pitcher":
            thread [[ level._id_134DF["pitcher"] ]]();
            break;
        case "gunslinger":
            thread [[ level._id_134DF["gunslinger"] ]]();
            break;
        case "race":
            thread [[ level._id_134DF["race"] ]]();
            break;
        case "arm_course":
            thread [[ level._id_134DF["arm_course"] ]]();
            break;
        default:
            break;
    }

    _loadout_setcopyloadoutomvnaronspawn::init();
...
_id_13526( var_0, var_1 )
{
    level notify( "exitLevel_called" );
    processlobbydata();

    if ( isdefined( level._id_134C0 ) )
        [[ level._id_134C0 ]]();
...
trial_mission_data_init()
{
    var_0 = _maphint_offerscriptableused::_id_134D0();
...
getspawnpoint()
{
    ...
    if ( isdefined( level._id_134F7 ) )
    {
        var_3 = scripts\cp_mp\vehicles\vehicle_occupancy::vehicle_occupancy_getavailablevehicleseats( level._id_134F7, 1 );
        var_4 = spawnstruct();
        var_4.useonspawn = 1;
        var_4.enterstartwaitmsg = "spawned_player";
        thread scripts\cp_mp\vehicles\vehicle_occupancy::vehicle_occupancy_enter( level._id_134F7, var_3[0], self, var_4 );
...
onplayerconnect( var_0 )
{
    ...
    level.player scripts\mp\gametypes\br.gsc::_id_11E31();
    wait 1;
...
_id_134ED()
{
    for (;;)
    {
        ...
        {
            if ( !isdefined( level.spawn_new_ents ) || !level.spawn_new_ents )
