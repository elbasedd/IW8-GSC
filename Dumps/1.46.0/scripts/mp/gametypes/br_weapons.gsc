...
createspawnweaponatpos( var_0, var_1, var_2 )
{
    ...
    var_5 = scripts\mp\gametypes\br_pickups.gsc::playersetkeypadcodelengthindex( var_0, var_1 );
...
createspawnweaponatposfromname( var_0, var_1 )
{
    ...
    var_4 = scripts\mp\gametypes\br_pickups.gsc::playersetkeypadcodelengthindex( var_0, ( 0, 0, 90 ) );
...
playerdropweaponfrominventory( var_0 )
{
    ...
    if ( isdefined( var_8 ) )
    {
        ...
        var_14 = scripts\mp\gametypes\br_pickups.gsc::setup_functions();
...
weaponspawn( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    level._id_119EA _branalytics_addevent::entityplunderlosealldeposited( var_6, var_1, var_0 );
    return var_6;
}
...
takeweaponpickup( var_0 )
{
    ...
    if ( !isdefined( var_0.weapon ) )
    {
        ...
        {
            var_3 = scripts\mp\gametypes\br_pickups.gsc::playerpowerrestartallcooldowns( var_0 );
        ...
        if ( !isdefined( var_4 ) )
        {
            scripts\mp\utility\script::intel_location_vo( "takeWeaponPickup error - scriptable not setup for pickup: " + var_0.scriptablename );
            return;
        }
    ...
    if ( var_7 )
    {
        ...
        if ( var_8.basename != "none" )
        {
            ...
            if ( var_8.hasalternate )
            {
                ...
                if ( !create_intel_model( var_8, var_16 ) )
            ...
            var_21 = !scripts\mp\gametypes\br_extract_quest.gsc::mine_caves_support_combat( var_8 );

            if ( var_21 )
            {
                var_22 = scripts\mp\gametypes\br_pickups.gsc::playersetkeypadcodelengthindex( var_0.origin, ( 0, var_20, 0 ) );
                ...
                if ( var_28["fraction"] < 1.0 )
                    var_22.raid_mindia_unload_func = var_28["entity"];

                var_29 = weaponspawn( var_8, self, var_22, 0 );

                if ( isdefined( var_29 ) )
                {
                    scripts\mp\gametypes\br_pickups.gsc::truck_think( var_29, var_13, var_14, var_15 );
                    var_30 = 1;
    ...
    {
        var_33 = var_0.count;
        var_34 = var_0.guard_shack_mantle;
        var_32 = weaponclipsize( var_3 );
        var_35 = 0;
        ...
        {
            var_36 = var_0.group_reset_goalradius;
            var_37 = var_3 getaltweapon();

            if ( !create_intel_model( var_3, var_37 ) )
            {
    ...
    if ( isdefined( var_0.shouldreflect ) )
        level._id_119EB _branalytics_addevent::entityplunderlosealldeposited( var_0.shouldreflect, self, var_3 );
}
...
br_forcegiveweapon( var_0, var_1, var_2 )
{
    ...
    var_3.count = 0;
    var_3.guard_shack_mantle = 0;
    var_3.group_reset_goalradius = 0;
...
br_forcegivecustomweapon( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    ...
    var_7.guard_shack_mantle = 0;
    var_7.group_reset_goalradius = 0;
    ...
    if ( scripts\mp\utility\weapon::sort_wave_spawning_ai( var_1 ) )
        var_7.guard_shack_mantle = var_7.count;

    if ( var_1.hasalternate )
    {
        var_10 = var_1 getaltweapon();

        if ( !create_intel_model( var_1, var_10 ) )
        {
            var_11 = weaponclipsize( var_10 );
            var_7.group_reset_goalradius = var_11;

            if ( var_6 )
                var_7.group_reset_goalradius = 999;
        }
    }
    ...
    for (;;)
    {
        ...
        if ( !scripts\mp\flags::gameflag( "prematch_done" ) )
        {
            ...
            foreach ( var_2 in var_0 )
            {
                ...
                if ( isdefined( var_3 ) )
                {
                    if ( scripts\mp\utility\weapon::sort_wave_spawning_ai( var_2 ) )
                    {
...
{
    if ( scripts\mp\gametypes\br_gametypes.gsc::spawn_juggernauts_fob( "weapons" ) )
        return;
...
{
    create_laser_point_to_point( var_0, var_1 );
...
br_ammo_player_hud_monitor()
{
    ...
    for (;;)
    {
        scripts\engine\utility::_id_13ADC( "ammo_update", "pickedupweapon", "weapon_switch_done", "weapon_change", "weapon_change_complete" );
        br_ammo_player_hud_update_ammotype( "brloot_ammo_919" );
        br_ammo_player_hud_update_ammotype( "brloot_ammo_12g" );
        br_ammo_player_hud_update_ammotype( "brloot_ammo_762" );
        br_ammo_player_hud_update_ammotype( "brloot_ammo_50cal" );
...
br_ammo_player_reload_watch()
{
    ...
    for (;;)
    {
        ...
        if ( !getdvarint( "scr_prematch_infinite_ammo", istrue( level._id_12123 ) ) || scripts\mp\flags::gameflag( "prematch_done" ) )
            self.br_ammo[var_1] = self getweaponammostock( var_0 );

        create_laser_trap( self, var_1 );
    }
}
...
createextractionlocation( var_0, var_1 )
{
    var_2 = self;
    ...
    create_laser_trap( var_2, var_3 );
}

createdevguientryforkidnapper( var_0, var_1 )
{
    var_2 = self;
    var_3 = br_ammo_type_for_weapon( var_0 );
...
squadleaderbeacon_isactive( var_0 )
{
    if ( !isdefined( var_0 ) )
        return 0;
...
currentnumber()
{
    if ( istrue( level.create_fake_loot_model_from_struct ) && !scripts\mp\gametypes\br_public.gsc::spawn_exfil_enemies() )
        return 1;
}

deploy_suicide_truck_in_blockade( var_0 )
{
    foreach ( var_2 in var_0 )
        trap_toggle_logic( var_2, istrue( var_2.reload_use_trigger ) );
}
