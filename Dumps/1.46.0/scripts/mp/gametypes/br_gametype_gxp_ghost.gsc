// IW8 GSC SOURCE
// Generated by https://github.com/xensik/gsc-tool

ref_anim()
{
    wait 5.0;
    scripts\mp\utility\sound::assigned_ai( "br_zxp" );
}

update_hack_objective_marker()
{
    return 0;
}

updatetimedrunhud( var_0, var_1 )
{
    var_2 = var_0 _id_11FAB( var_1 );

    if ( scripts\mp\flags::gameflag( "prematch_done" ) && !var_2 )
        scripts\mp\gametypes\br.gsc::update_goal_population( var_0 );

    return !var_2;
}

headequiptoggleloopbr()
{
    var_0 = [];
    var_0["loadoutArchetype"] = "archetype_assault";
    var_0["loadoutPrimary"] = "iw8_fists";
    var_0["loadoutPrimaryAttachment"] = "none";
    var_0["loadoutPrimaryAttachment2"] = "none";
    var_0["loadoutPrimaryCamo"] = "none";
    var_0["loadoutPrimaryReticle"] = "none";
    var_0["loadoutSecondary"] = "none";
    var_0["loadoutSecondaryAttachment"] = "none";
    var_0["loadoutSecondaryAttachment2"] = "none";
    var_0["loadoutSecondaryCamo"] = "none";
    var_0["loadoutSecondaryReticle"] = "none";
    var_0["loadoutMeleeSlot"] = "none";
    var_0["loadoutEquipmentPrimary"] = "none";
    var_0["loadoutEquipmentSecondary"] = "none";
    var_0["loadoutStreakType"] = "assault";
    var_0["loadoutKillstreak1"] = "none";
    var_0["loadoutKillstreak2"] = "none";
    var_0["loadoutKillstreak3"] = "none";
    var_0["loadoutSuper"] = "super_br_extract";
    var_0["loadoutPerks"] = [];
    var_0["loadoutGesture"] = "playerData";
    return var_0;
}

_id_1245E( var_0 )
{
    if ( !isdefined( level.teamdata[var_0]["aliveCountHuman"] ) )
        return level.teamdata[var_0]["aliveCount"];
    else
        return level.teamdata[var_0]["aliveCountHuman"];
}

_id_11E1B()
{
    self endon( "disconnect" );
    self setscriptablepartstate( "ghost", "off" );
    self setscriptablepartstate( "compassicon", "defaulticon" );
    self setscriptablepartstate( "skydiveVfx", "default", 0 );

    if ( level.deadblue.proplocationindex )
    {
        _id_11EBC();

        if ( !level.deadblue.proplockunlock )
            self setscriptablepartstate( "headVFX", "neutral" );

        self visionsetnakedforplayer( "", 0 );
    }

    if ( !getdvarint( "scr_br_gxp_disable_ghost_death_loadout_fix", 0 ) )
    {
        scripts\mp\class::loadout_emptycacheofloadout( "gamemode" );
        self.pers["gamemodeLoadout"] = level.br_respawn_loadout;
        self.pers["class"] = "gamemode";
        self.class = "gamemode";
    }

    waittillframeend;
    _id_11F5A( 0, 1 );
    _id_11F0C( 0 );
}

_id_11F0C( var_0 )
{
    scripts\mp\gametypes\br_public.gsc::_id_11EB1( var_0 );

    if ( var_0 )
    {
        if ( level.deadblue.sat_setup_access_cards )
        {
            self.sat_computer_think_new.alpha = 1;
            self.sat_create_interaction.alpha = 1;
            self.sales_discount_items.alpha = 1;
        }

        self disableweaponpickup();
    }
    else
    {
        _id_11E6D();
        self enableweaponpickup();
    }
}

_id_11E6D()
{
    if ( isdefined( self.sat_computer_think_new ) )
        thread init_wind_tunnels( self.sat_computer_think_new, 1.5 );

    if ( isdefined( self.sat_create_interaction ) )
        thread init_wind_tunnels( self.sat_create_interaction, 1.5 );

    if ( isdefined( self.sales_discount_items ) )
        self.sales_discount_items destroy();

    self.sat_computer_think_new = undefined;
    self.sat_create_interaction = undefined;
    self.sales_discount_items = undefined;
}

init_wind_tunnels( var_0, var_1 )
{
    wait( var_1 );

    if ( isdefined( var_0 ) )
        var_0 destroy();
}

_id_11EBC()
{
    foreach ( var_1 in level.players )
    {
        if ( !var_1 scripts\mp\gametypes\br_public.gsc::_id_11ECE() )
            var_1 hudoutlinedisableforclient( self );
    }
}

_id_11F26()
{
    self notify( "spawnGhost" );
    self _meth_87A8( "ghost" );
    self setclothtype( "cloth" );
    scripts\mp\deathicons::riskspawn_initialset( self );
}

_id_11F77( var_0 )
{
    return scripts\mp\gametypes\br_public.gsc::_id_11ECE();
}

_id_11F78( var_0 )
{
    return scripts\mp\gametypes\br_public.gsc::_id_11ECE();
}

_id_11FAB( var_0 )
{
    if ( !istrue( var_0 ) )
    {
        if ( scripts\mp\gametypes\br_public.gsc::_id_11ECE() )
        {
            thread _id_11E1B();
            return 0;
        }
    }

    if ( !istrue( self.br_infilstarted ) || !scripts\mp\flags::gameflag( "prematch_done" ) || level.gameended || !level.deadblue.propcleanup )
        return 0;

    thread _id_11E8D( 0 );
    return 1;
}

_id_11E8D( var_0 )
{
    level endon( "game_ended" );
    self endon( "disconnect" );
    self endon( "ghost_unset" );

    if ( level.gameended )
        return;

    if ( !isdefined( level.teamdata[self.team]["lastGhostTime"] ) && !isdefined( level._id_12590 ) )
    {

    }

    _id_129FE( self.team );
    _id_11F58( 1 );
    _id_11E92( 1 );
    waittillframeend;

    if ( isdefined( self.body ) )
        self.body delete();

    _id_11F5A( 1 );
    scripts\mp\gametypes\br_gxp_phones.gsc::ispickupblueprintloadouts( self );
    self._id_12538 = 1;

    if ( isdefined( level._id_12590 ) )
        _id_11F26();
    else if ( var_0 )
        _id_11FC6();
    else
        _id_11F26();

    if ( scripts\mp\gametypes\br_gametypes.gsc::spawn_atmines( "playerGetGhostSpawnLocation" ) )
        [var_2, var_3] = scripts\mp\gametypes\br_gametypes.gsc::_id_1268B( "playerGetGhostSpawnLocation" );
    else
        [var_2, var_3] = _id_11E4D();

    [var_2, var_6] = _id_11E8C( var_2, var_3 );
    scripts\mp\gametypes\br_gulag.gsc::gulagfadetoblack();
    wait 1;
    scripts\mp\class::loadout_emptycacheofloadout( "gamemode" );
    self.pers["gamemodeLoadout"] = level.deadblue.projectiledeletethread;
    self.class = "gamemode";
    self.forcespawnangles = var_3;
    self.forcespawnorigin = var_6;
    scripts\mp\playerlogic::spawnplayer( undefined, 0 );
    thread scripts\mp\music_and_dialog::_id_13DB0();
    scripts\mp\gametypes\br_pickups.gsc::initplayer( 1 );
    scripts\mp\gametypes\br_spectate.gsc::_id_11E10();
    self notify( "endSuperJumpFov" );
    var_7 = getcompleteweaponname( "iw8_fists_mp" );
    scripts\cp_mp\utility\inventory_utility::_takeweapon( var_7 );
    var_8 = getcompleteweaponname( "iw8_fists_mp_gxp" );
    scripts\cp_mp\utility\inventory_utility::_giveweapon( var_8, undefined, undefined, 1 );
    self detachall();
    self setmodel( "c_s4_rus_redshirt_02" );
    self attach( "c_s4_head_sc_m_sharipov" );
    self setviewmodel( "c_s4_vm_fra_novak_01" );
    scripts\mp\utility\player::_setsuit( "iw8_gxp_mp" );
    self._id_12BE2 = 1;

    if ( level.deadblue.progression_deadzone )
        scripts\cp_mp\execution::_giveexecution( "execution_mp_universal_ref_variant_145" );
    else
        scripts\cp_mp\execution::_clearexecution();

    self _meth_87B1();
    _id_11F99( var_2, var_3, var_6 );
    self skydive_interrupt();
    self.mortar_smoke_occlusions = 5;
    self skydive_setbasejumpingstatus( 0 );
    self skydive_setdeploymentstatus( 0 );
    self skydive_beginfreefall();
    thread scripts\cp_mp\utility\inventory_utility::_switchtoweaponimmediate( var_8 );
    _id_11F0C( 1 );
    thread _id_11E7D( var_8 );
    thread scripts\mp\gametypes\br.gsc::create_vehicle_interact();
    thread _id_11E93();

    if ( level.deadblue.bot_verify_and_cache_bombzones )
        thread _id_11E8F();

    thread _id_11E98();
    thread _id_11E79();
    self disableexecutionvictim();
    scripts\mp\utility\perk::giveperk( "specialty_quieter" );
    scripts\mp\utility\perk::giveperk( "specialty_coldblooded" );
    scripts\mp\utility\perk::giveperk( "specialty_heartbreaker" );
    scripts\mp\utility\perk::giveperk( "specialty_noscopeoutline" );
    scripts\mp\utility\perk::giveperk( "specialty_tracker" );
    scripts\mp\utility\perk::giveperk( "specialty_tracker_jammer" );
    scripts\mp\utility\perk::giveperk( "specialty_sprintmelee" );
    scripts\mp\utility\perk::giveperk( "specialty_sprintads" );
    scripts\common\utility::allow_crate_use( 0 );
    scripts\common\utility::allow_vehicle_use( 0 );
    self lerpfovbypreset( "zombiedefault" );

    if ( level.deadblue.proplockunlock )
        self setclientomnvar( "ui_deadsilence_overlay", 0 );

    thread _id_11E6C();
    self setscriptablepartstate( "compassicon", "ghost" );

    if ( getdvarint( "scr_br_gxp_force_netfield_high_lod_zombies", 0 ) )
        self forcenetfieldhighlod( 1 );

    if ( isdefined( level._id_12590 ) )
        scripts\mp\hud_message::showsplash( "br_reveal_stop_exfil" );
    else
        scripts\mp\hud_message::showsplash( "br_gametype_gxp_change" );

    if ( !scripts\mp\utility\perk::_hasperk( "specialty_pistoldeath" ) )
        scripts\mp\utility\perk::giveperk( "specialty_pistoldeath" );

    self.maxhealth = level.deadblue.progression_speed;
    self.health = self.maxhealth;
    scripts\mp\gametypes\br_armor.gsc::proxy_trigger( 0 );
    self allowprone( 0 );
    self._id_12538 = undefined;
    wait 3;

    foreach ( var_10 in level.teamdata[self.team]["players"] )
    {
        if ( self == var_10 )
        {
            if ( !isdefined( self.reflectbolt ) )
            {
                scripts\mp\gametypes\br_public.gsc::decoyassists( "player_into_ghost_first", var_10 );
                self.reflectbolt = 1;
            }
            else
                scripts\mp\gametypes\br_public.gsc::decoyassists( "player_into_ghost", var_10 );

            continue;
        }

        scripts\mp\gametypes\br_public.gsc::decoyassists( "teammate_into_ghost", var_10 );
    }
}

_id_11E98()
{
    level endon( "game_ended" );
    self endon( "death_or_disconnect" );
    self endon( "ghost_unset" );
    var_0 = 0.5;

    for (;;)
    {
        var_1 = self calloutmarkerping_getgscobjectiveindex( 7 );
        var_2 = sortbydistance( level.deadblue._id_1253F, self.origin );
        var_3 = 0;

        foreach ( var_5 in var_2 )
        {
            var_6 = undefined;

            if ( var_3 < level.deadblue.updatesmokinggunhud && level.deadblue.updatesixthsensevo > 0 )
                var_6 = distance2dsquared( self.origin, var_5.origin );

            if ( var_3 < level.deadblue.updatesmokinggunhud && ( level.deadblue.updatesixthsensevo == 0 || var_6 < level.deadblue.updatesixthsensevo ) )
            {
                var_3++;
                scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( var_5.objidnum, self );
                continue;
            }

            var_3 = level.deadblue.updatesmokinggunhud;
            scripts\mp\objidpoolmanager::objective_playermask_hidefrom( var_5.objidnum, self );

            if ( var_1 != -1 && var_1 == var_5.objidnum )
                scripts\cp\utility\auto_ascender_ai::calloutmarkerping_removecallout( 7 );
        }

        wait( var_0 );
    }
}

_id_11E6C()
{
    self endon( "death_or_disconnect" );
    self endon( "ghost_unset" );

    if ( level.deadblue.proplocationindex )
    {
        self hudoutlinedisable();

        if ( !level.deadblue.proplockunlock )
            self setscriptablepartstate( "headVFX", "zombieVision" );

        self visionsetnakedforplayer( "mp_don4_wz_ghost", 0 );
        _id_11E7F();
    }

    waitframe();

    if ( getdvarint( "scr_br_gxp_loop_zombie_fx", 0 ) )
        self setscriptablepartstate( "ghost", "on_loop" );
    else
        self setscriptablepartstate( "ghost", "on" );
}

_id_11E7F()
{
    foreach ( var_1 in level.players )
    {
        if ( !var_1 scripts\mp\gametypes\br_public.gsc::_id_11ECE() )
            scripts\mp\utility\outline::outlineenableforplayer( var_1, self, "outline_depth_zombievision", "top" );
    }
}

_id_11E79()
{
    level endon( "game_ended" );
    self endon( "ghost_unset" );
    self endon( "disconnect" );
    self.prohibited_weapon_list = undefined;

    for (;;)
    {
        if ( _id_11E80() )
        {
            if ( !isdefined( self.prohibited_weapon_list ) || !self.prohibited_weapon_list )
            {
                self.prohibited_weapon_list = 1;
                _id_11E71();
            }
        }
        else if ( !isdefined( self.prohibited_weapon_list ) || self.prohibited_weapon_list )
        {
            self.prohibited_weapon_list = 0;
            _id_11E72();
        }

        waitframe();
    }
}

_id_11E80()
{
    if ( !isdefined( level.br_circle ) || !isdefined( level.br_circle.dangercircleent ) )
        return 0;

    var_0 = scripts\mp\gametypes\br_circle.gsc::getdangercircleorigin();
    var_1 = scripts\mp\gametypes\br_circle.gsc::getdangercircleradius();
    return distance2dsquared( var_0, self.origin ) > var_1 * var_1;
}

_id_11E71()
{
    self notify( "ghost_enter_gas" );
    self unsetperk( "specialty_radarblip", 1 );
}

_id_11E72()
{
    self notify( "ghost_exit_gas" );

    if ( level.deadblue.propaddtocircle >= 0 )
    {
        if ( level.deadblue.propaddtocircle == 0 )
            self setperk( "specialty_radarblip", 1 );
        else
            thread _id_11E8A();
    }
}

_id_11E8A()
{
    if ( level.deadblue.propaddtocircle <= 0 )
        return;

    self endon( "ghost_unset" );
    self endon( "ghost_enter_gas" );
    self endon( "disconnect" );
    level endon( "game_ended" );

    for (;;)
    {
        self setperk( "specialty_radarblip", 1 );
        wait( level.deadblue.propaddtolocation );
        self unsetperk( "specialty_radarblip", 1 );
        wait( level.deadblue.propaddtocircle );
    }
}

_id_11E8F()
{
    self endon( "disconnect" );
    self._id_12BFF = 1;
    self.radarmode = "normal_radar";
    self.hasradar = 1;
    self waittill( "ghost_unset" );
    self._id_12BFF = undefined;
    self.hasradar = 0;
}

_id_11E93()
{
    level endon( "game_ended" );
    self endon( "death_or_disconnect" );
    self endon( "ghost_unset" );

    for (;;)
    {
        if ( self issupersprinting() )
            self refreshsprinttime();

        waitframe();
    }
}

_id_11E7D( var_0 )
{
    level endon( "game_ended" );
    self endon( "death_or_disconnect" );
    self endon( "ghost_unset" );
    wait 1;

    while ( !self isonground() )
    {
        _id_11E9C( var_0 );
        waitframe();
    }

    self setclientomnvar( "ui_br_altimeter_state", 0 );
    self skydive_interrupt();
    playfxontag( level._effect["zombie_splat"], self, "j_mainroot" );
    self playsoundtoplayer( "br_gov_ghost_infil_land", self, self );
    self playsound( "br_gov_ghost_infil_land_npc", self, self );
    self freezecontrols( 1 );
    var_1 = gettime() + 2000;

    while ( self getcurrentprimaryweapon().classname == "none" && gettime() < var_1 )
    {
        _id_11E9C( var_0 );
        waitframe();
    }

    var_2 = pilot_setups( self, 0 );

    if ( !isdefined( var_2 ) )
        var_2 = ( 0, 0, 1 );

    var_3 = anglestoforward( self.angles );
    var_4 = vectortoangles( var_2 );
    var_5 = angleclamp180( var_4[0] + 90 );
    var_4 = ( 0, var_4[1], 0 );
    var_6 = anglestoforward( var_4 );
    var_7 = vectordot( var_6, var_3 );
    var_8 = var_7 * var_5;
    var_9 = getdvarint( "scr_br_gxp_zombie_splat_down_clamp", 20 );
    var_10 = getdvarint( "scr_br_gxp_zombie_splat_up_clamp", -70 );

    if ( var_8 > 0 )
        var_8 = min( var_9, var_8 );
    else
        var_8 = max( var_10, var_8 );

    self setplayerangles( ( var_8, self.angles[1], 0 ) );

    if ( self getcurrentprimaryweapon().classname != "none" )
        self forceplaygestureviewmodel( "ges_gxp_splat" );

    self playfx( level.deadblue.propheight, self.origin );
    wait 1.5;
    self freezecontrols( 0 );
    self freezelookcontrols( 1 );
    self allowsprint( 0 );
    self skydive_setbasejumpingstatus( 0 );
    self skydive_setdeploymentstatus( 0 );
    wait 1;
    self freezelookcontrols( 0 );
    wait 1;
    _id_11E9C( var_0 );
    self allowsprint( 1 );
    thread _id_11E8B();
}

_id_11E8B()
{
    if ( !level.deadblue.propcamerazoom )
        return;

    thread _id_11F90( level.deadblue.processlobbydataforclient );
}

_id_11F90( var_0 )
{
    self notifyonplayercommand( "ghostAttack", "+attack" );
    self notifyonplayercommand( "ghostAttack", "+melee_zoom" );
    thread _id_11E7A();
    thread _id_11F18( var_0 );
    thread _id_11F15( var_0 );
    thread _id_11F16( var_0 );

    if ( level.deadblue.sat_setup_access_cards )
        thread _id_11F1B( var_0 );

    thread _id_11F19( var_0 );
    thread _id_11F11( var_0 );

    foreach ( var_3, var_2 in var_0.powers )
    {
        if ( isdefined( var_0.powers[var_3]._id_1305F ) )
            _id_13754( var_0.powers[var_3]._id_1305F, 2 );
    }
}

_id_13754( var_0, var_1 )
{
    var_2 = 0;
    var_3 = 0;

    switch ( var_0 )
    {
        case "jumpStatus":
            var_2 = 0;
            var_3 = 2;
            break;
        case "jumpProgress":
            var_2 = 2;
            var_3 = 7;
            break;
        case "spectralBlastStatus":
            var_2 = 9;
            var_3 = 2;
            break;
        case "spectralBlastProgress":
            var_2 = 11;
            var_3 = 7;
            break;
        case "teleportStatus":
            var_2 = 18;
            var_3 = 2;
            break;
        case "teleportProgress":
            var_2 = 20;
            var_3 = 7;
            break;
        case "numVaccine":
            var_2 = 27;
            var_3 = 2;
            break;
        case "inSafeZone":
            var_2 = 29;
            var_3 = 1;
            break;
        default:
            break;
    }

    if ( !isdefined( level.propane_detonate_fiery_drips ) )
        level.propane_detonate_fiery_drips = [];

    if ( !isdefined( level.propane_detonate_fiery_drips["ui_br_zombie_powers"] ) )
        level.propane_detonate_fiery_drips["ui_br_zombie_powers"] = 0;

    var_4 = int( pow( 2, var_3 ) ) - 1;
    var_5 = ( int( var_1 ) & var_4 ) << var_2;
    var_6 = ~( var_4 << var_2 );
    var_7 = self getclientomnvar( "ui_br_zombie_powers" );
    var_8 = var_7 & var_6;
    var_9 = var_8 + var_5;
    level.propane_detonate_fiery_drips["ui_br_zombie_powers"] = var_9;
    self setclientomnvar( "ui_br_zombie_powers", level.propane_detonate_fiery_drips["ui_br_zombie_powers"] );
}

_id_11F11( var_0 )
{
    level endon( "game_ended" );
    self endon( "disconnect" );
    scripts\engine\utility::_id_13ADA( "death", "ghost_unset", "ghost_set" );
    self notifyonplayercommandremove( "ghostAttack", "+attack" );
    self notifyonplayercommandremove( "ghostAttack", "+melee_zoom" );
    thread _id_11F0E( var_0 );
    thread _id_11F13( var_0 );
    thread _id_11F14( var_0 );
    thread _id_11F12( var_0 );
}

_id_11F12( var_0 )
{
    if ( !isdefined( self._id_120F1 ) )
        return;

    if ( level.deadblue.sat_setup_access_cards )
    {
        foreach ( var_2 in self._id_120F1 )
        {
            if ( isdefined( var_2 ) )
            {
                if ( isdefined( var_2.cargo_truck_mg_cp_init ) )
                    var_2.cargo_truck_mg_cp_init scripts\mp\hud_util::destroyelem();

                var_2 destroy();
            }
        }
    }

    self._id_120F1 = undefined;
}

_id_11F13( var_0 )
{
    if ( isbot( self ) )
        return;

    foreach ( var_6, var_2 in var_0.powers )
    {
        foreach ( var_4 in var_2.checkarmorpickup )
            self notifyonplayercommandremove( var_6, var_4 );
    }
}

_id_11F14( var_0 )
{
    foreach ( var_3, var_2 in var_0.powers )
    {
        if ( isdefined( var_2.getbestintersectionpt ) )
            self thread [[ var_2.getbestintersectionpt ]]( var_0, var_3 );
    }
}

_id_11F0E( var_0 )
{
    if ( !isdefined( var_0 ) || !isdefined( self._id_120F1 ) )
        return;

    self notify( "disableCooldown" );

    foreach ( var_3, var_2 in var_0.powers )
    {
        if ( !isdefined( self._id_120F1[var_3] ) )
            continue;

        self._id_120F1[var_3].incooldown = 0;

        if ( level.deadblue.sat_setup_access_cards )
            self._id_120F1[var_3].cargo_truck_mg_cp_init scripts\mp\hud_util::updatebar( 0, 0 );
        else
            self._id_120F1[var_3].frac = 0;

        thread _id_11F1A( var_0, var_3 );
    }

    self.intel_use_logic = undefined;
    self.startptui = undefined;
}

_id_11F1A( var_0, var_1 )
{
    level endon( "game_ended" );
    self endon( "death_or_disconnect" );
    self endon( "ghost_unset" );
    self endon( "ghost_set" );
    self endon( "disableCooldown" );

    if ( !isdefined( self._id_120F1[var_1] ) || istrue( self._id_120F1[var_1].incooldown ) )
        return;

    var_2 = self._id_120F1[var_1];

    if ( level.deadblue._id_120F0 && var_2.frac > 0 )
    {
        self._id_120F1[var_1].incooldown = 1;
        var_3 = var_0.powers[var_1].grenade_obj;
        var_4 = "scr_br_gxp_power_cooldown_" + var_1;

        if ( getdvarint( var_4, 0 ) != 0 )
            var_3 = getdvarint( var_4, 0 );

        thread _id_11FB6( var_0, var_1, var_3, int( var_2.frac * 100 ) );
        var_5 = var_2.frac;
        var_3 = var_3 * var_5;

        if ( level.deadblue.sat_setup_access_cards )
        {
            var_2.cargo_truck_mg_cp_init.bar.color = ( 1, 0.6, 0 );
            var_2.cargo_truck_mg_cp_init.bar scaleovertime( var_3, 0, var_2.cargo_truck_mg_cp_init.height );
        }

        wait( var_3 );

        if ( var_1 == "spectralBlast" )
            var_6 = "ui_zxp_restock_emp";
        else if ( var_1 == "teleport" )
            var_6 = "ui_zxp_recharge_tport";
        else
            var_6 = "ui_zxp_restock_" + var_1;

        self playlocalsound( var_6 );
        self._id_120F1[var_1].incooldown = 0;
    }
    else
    {
        if ( level.deadblue.sat_setup_access_cards )
            var_2.cargo_truck_mg_cp_init scripts\mp\hud_util::updatebar( 0, 0 );
        else
            var_2.frac = 0;

        thread _id_11FB6( var_0, var_1, 0, 0 );
    }

    if ( level.deadblue.sat_setup_access_cards )
        var_2.cargo_truck_mg_cp_init.bar.color = ( 1, 1, 1 );

    if ( isdefined( var_0.powers[var_1]._id_120CF ) )
        self [[ var_0.powers[var_1]._id_120CF ]]( var_0, var_1 );
}

_id_11FB6( var_0, var_1, var_2, var_3 )
{
    level endon( "game_ended" );
    self endon( "death_or_disconnect" );
    self endon( "ghost_unset" );
    self endon( "ghost_set" );
    self endon( "disableCooldown" );

    if ( !isdefined( var_0.powers[var_1]._id_1305F ) || !isdefined( var_0.powers[var_1]._id_12185 ) )
        return;

    _id_13754( var_0.powers[var_1]._id_1305F, 1 );
    var_4 = var_2 * 1000 * var_3 / 100;
    var_5 = gettime();
    var_6 = var_5 + var_4;

    while ( gettime() < var_6 )
    {
        var_7 = gettime();
        var_8 = ( var_6 - gettime() ) / var_4;
        var_9 = var_8 * var_3;
        _id_13754( var_0.powers[var_1]._id_12185, int( var_9 ) );
        waitframe();
    }

    _id_13754( var_0.powers[var_1]._id_12185, 0 );
    _id_13754( var_0.powers[var_1]._id_1305F, 2 );
}

_id_11F19( var_0 )
{
    foreach ( var_3, var_2 in var_0.powers )
    {
        if ( isdefined( var_2._id_1304D ) )
            self thread [[ var_2._id_1304D ]]( var_0, var_3 );
    }
}

_id_11F1B( var_0 )
{
    level endon( "game_ended" );
    self endon( "ghost_unset" );
    self endon( "ghost_set" );
    self endon( "death_or_disconnect" );

    if ( isbot( self ) )
        return;

    waittillframeend;
    var_1 = scripts\engine\utility::is_player_gamepad_enabled();

    for (;;)
    {
        var_2 = scripts\engine\utility::is_player_gamepad_enabled();

        if ( var_2 != var_1 )
        {
            var_1 = var_2;

            if ( var_2 )
            {
                foreach ( var_5, var_4 in var_0.powers )
                {
                    if ( isdefined( var_4.suspensemusic_reset ) )
                        self._id_120F1[var_5].label = var_4.label;
                }
            }
            else
            {
                foreach ( var_5, var_4 in var_0.powers )
                {
                    if ( isdefined( var_4.suspensemusic_reset ) )
                        self._id_120F1[var_5].label = var_4.suspensemusic_reset;
                }
            }
        }

        waitframe();
    }
}

_id_11F16( var_0 )
{
    foreach ( var_3, var_2 in var_0.powers )
        thread _id_11F17( var_0, var_3 );
}

_id_11F17( var_0, var_1 )
{
    self endon( "death_or_disconnect" );
    self endon( "ghost_unset" );
    self endon( "ghost_set" );
    level endon( "game_ended" );

    for (;;)
    {
        self waittill( var_1 );
        waittillframeend;

        if ( isdefined( self._id_120F1[var_1] ) && self._id_120F1[var_1].incooldown )
        {
            _id_11EF5();
            continue;
        }

        if ( self isinexecutionattack() )
            continue;

        self thread [[ var_0.powers[var_1].func ]]( var_0, var_1 );
    }
}

_id_11EF5()
{
    if ( !isdefined( self.intel_use_logic ) || gettime() > self.intel_use_logic )
    {
        self playlocalsound( "br_pickup_deny" );
        self.intel_use_logic = gettime() + 1000;
    }
}

_id_11F15( var_0 )
{
    var_1 = 200;
    var_2 = 18;
    var_3 = var_1;
    self._id_120F1 = [];

    foreach ( var_6, var_5 in var_0.powers )
    {
        if ( isdefined( var_5.label ) )
        {
            if ( level.deadblue.sat_setup_access_cards )
                self._id_120F1[var_6] = _id_11F10( var_5.label, var_5.suspensemusic_reset, var_3, var_5._id_1289D );
            else
            {
                self._id_120F1[var_6] = spawnstruct();
                self._id_120F1[var_6].frac = 0;
            }

            self._id_120F1[var_6].incooldown = 0;
            var_3 = var_3 + var_2;
        }
    }
}

_id_11F10( var_0, var_1, var_2, var_3 )
{
    var_4 = scripts\mp\hud_util::createfontstring( "default", 1.5 );
    var_4.x = 15;
    var_4.y = var_2;
    var_4.alignx = "left";
    var_4.aligny = "top";
    var_4.horzalign = "left_adjustable";
    var_4.vertalign = "top_adjustable";
    var_4.alpha = var_3;
    var_4.glowalpha = 0;
    var_4.hidewheninmenu = 1;
    var_4.archived = 0;

    if ( isdefined( var_1 ) && !scripts\engine\utility::is_player_gamepad_enabled() )
        var_4.label = var_1;
    else if ( isdefined( var_0 ) )
        var_4.label = var_0;

    var_5 = scripts\mp\hud_util::createbar( ( 1, 1, 1 ), 160, 14 );
    var_5.x = 13;
    var_5.y = var_2;
    var_5.alignx = "left";
    var_5.aligny = "top";
    var_5.horzalign = "left_adjustable";
    var_5.vertalign = "top_adjustable";
    var_5.alpha = var_3;
    var_5 _id_12ACE();
    var_5.archived = 0;
    var_5.hidewheninmenu = 1;
    var_5.bar.archived = 0;
    var_5.bar.hidewheninmenu = 1;
    var_5.bar.alpha = var_3;
    var_4.cargo_truck_mg_cp_init = var_5;
    return var_4;
}

_id_12ACE( var_0, var_1, var_2, var_3 )
{
    self.bar.horzalign = self.horzalign;
    self.bar.vertalign = self.vertalign;
    self.bar.alignx = "left";
    self.bar.aligny = self.aligny;
    self.bar.y = self.y + 2;
    self.bar.x = self.x + 2;
    scripts\mp\hud_util::updatebar( self.bar.frac );
}

_id_11F18( var_0 )
{
    if ( isbot( self ) )
        return;

    foreach ( var_6, var_2 in var_0.powers )
    {
        foreach ( var_4 in var_2.checkarmorpickup )
            self notifyonplayercommand( var_6, var_4 );
    }
}

pilot_setups( var_0, var_1 )
{
    if ( !isdefined( var_0 ) )
        var_2 = self;
    else
        var_2 = var_0;

    if ( !isdefined( var_1 ) )
        var_1 = 0;

    var_3 = [ var_2 ];
    var_4 = [ self.origin ];

    for ( var_5 = -1.0; var_5 <= 1.0; var_5 = var_5 + 2.0 )
    {
        for ( var_6 = -1.0; var_6 <= 1.0; var_6 = var_6 + 2.0 )
        {
            var_7 = var_2 getpointinbounds( var_5, var_6, 0.0 );
            var_7 = ( var_7[0], var_7[1], self.origin[2] );
            var_4[var_4.size] = var_7;
        }
    }

    var_8 = ( 0, 0, 0 );
    var_9 = 0;

    foreach ( var_11 in var_4 )
    {
        var_12 = scripts\engine\trace::_bullet_trace( var_11 + ( 0, 0, 4 ), var_11 + ( 0, 0, -16 ), 0, var_3 );
        var_13 = var_12["fraction"] > 0.0 && var_12["fraction"] < 1;

        if ( var_13 )
        {
            var_8 = var_8 + var_12["normal"];
            var_9++;
        }
    }

    if ( var_9 > 0 )
    {
        var_8 = var_8 / var_9;
        return var_8;
    }
    else
        return undefined;
}

_id_11E9C( var_0 )
{
    if ( !self hasweapon( var_0 ) )
        scripts\cp_mp\utility\inventory_utility::_giveweapon( var_0, undefined, undefined, 1 );

    if ( self getcurrentprimaryweapon().classname == "none" )
        thread scripts\cp_mp\utility\inventory_utility::_switchtoweaponimmediate( var_0 );
}

_id_11F99( var_0, var_1, var_2 )
{
    scripts\mp\gametypes\br_gulag.gsc::_id_11F9F( var_2, var_1 );
    var_3 = spawn( "script_model", var_2 );
    var_3 setmodel( "tag_origin" );
    var_3.angles = var_1;
    var_3 hide();
    var_3 showtoplayer( self );
    self playerlinktoabsolute( var_3, "tag_origin" );
    self playerhide();
    thread scripts\mp\gametypes\br_gulag.gsc::_id_11E0B( var_3 );
    waitframe();
    scripts\mp\gametypes\br_public.gsc::_id_11FC5();
    scripts\mp\gametypes\br_public.gsc::_id_11E11();
    var_3.origin = var_0;
    waitframe();
    self unlink();
    self clearsoundsubmix( "deaths_door_mp" );
    self clearsoundsubmix( "fade_to_black_all_except_music_and_scripted5", 2 );
    self clearclienttriggeraudiozone( 1 );
    self playershow();
    var_4 = 0;

    if ( isdefined( level._id_11AE5 ) )
        var_4 = level._id_11AE5;

    thread scripts\cp_mp\parachute::startfreefall( var_4, 0, undefined, undefined, 1 );
    self setclientomnvar( "ui_br_transition_type", 0 );
    self setclientomnvar( "ui_show_spectateHud", -1 );
    scripts\mp\gametypes\br_gulag.gsc::_id_1250F();
    wait 0.5;
    scripts\mp\gametypes\br_gulag.gsc::gulagfadefromblack();
    waitframe();
    var_3 delete();
    self notify( "can_show_splashes" );
}

_id_11E8C( var_0, var_1 )
{
    var_2 = var_0;

    if ( level.deadblue.prophasclonesleft )
    {
        var_3 = getdvarint( "scr_br_gxp_respawnGhostHeight", 10000 );
        var_4 = ( 0, 0, var_3 );
        var_0 = scripts\mp\gametypes\br.gsc::getoffsetspawnorigin( var_0, var_4 );
        var_5 = spawnstruct();
        var_5.origin = var_0;
        var_5.angles = var_1;
        var_5.height = var_3;
        var_2 = scripts\mp\gametypes\br_gulag.gsc::_id_11F1F( var_5 );
    }
    else
    {
        self setpredictedstreamloaddist( 0.0 );
        scripts\mp\gametypes\br_public.gsc::_id_11F95( var_2 );
    }

    return [ var_0, var_2 ];
}

_id_11E4D()
{
    var_0 = 50;
    var_1 = 10000;
    [var_3, var_4] = _id_11E7B();

    if ( isdefined( var_3 ) )
        return [ var_3, var_4 ];

    if ( !isdefined( level.br_circle ) || !isdefined( level.br_circle.dangercircleent ) || istrue( level.deadblue.prophaschangesleft ) )
        return [ self.origin, self getplayerangles() ];

    var_5 = scripts\mp\gametypes\br_circle.gsc::getdangercircleradius();
    var_6 = scripts\mp\gametypes\br_circle.gsc::getdangercircleorigin();
    var_7 = var_5 + var_0;
    var_8 = ( self.origin[0], self.origin[1], 0 );
    var_9 = vectornormalize( var_8 - var_6 );
    [var_11, var_4] = pressure_timeout( var_6, var_9, var_7 );

    if ( !isdefined( var_11 ) )
    {
        var_9 = var_9 * -1;
        [var_11, var_4] = pressure_timeout( var_6, var_9, var_7 );
    }

    if ( !isdefined( var_11 ) )
    {
        var_9 = ( 1, 0, 0 );
        [var_11, var_4] = pressure_timeout( var_6, var_9, var_7 );
    }

    if ( !isdefined( var_11 ) )
    {
        var_9 = ( -1, 0, 0 );
        [var_11, var_4] = pressure_timeout( var_6, var_9, var_7 );
    }

    if ( !isdefined( var_11 ) )
    {
        var_9 = ( 0, 1, 0 );
        [var_11, var_4] = pressure_timeout( var_6, var_9, var_7 );
    }

    if ( !isdefined( var_11 ) )
    {
        var_9 = ( 0, -1, 0 );
        [var_11, var_4] = pressure_timeout( var_6, var_9, var_7 );
    }

    if ( !isdefined( var_11 ) )
    {
        var_11 = self.origin;
        var_4 = self.angles;
    }

    var_3 = scripts\mp\gametypes\br_public.gsc::lb_mg_wood_surf_dmg_scalar( var_11, var_1 );
    return [ var_3, var_4 ];
}

pressure_timeout( var_0, var_1, var_2 )
{
    var_3 = var_0 + var_1 * var_2;

    if ( scripts\mp\gametypes\br_c130.gsc::ispointinbounds( var_3, 1 ) )
    {
        var_4 = vectortoangles( var_1 * -1 );
        return [ var_3, var_4 ];
    }
    else
        return [ undefined, undefined ];
}

_id_11E7B()
{
    var_0 = undefined;
    var_1 = undefined;
    var_2 = getdvarfloat( "scr_br_gxp_respawnTeamOffset", 10000 );

    if ( var_2 >= 0 )
    {
        var_3 = scripts\mp\gametypes\br_gulag.gsc::_id_11E4C( 0 );

        if ( isdefined( var_3 ) )
        {
            var_0 = plundersilentcountdownendtime( var_3.origin, var_2 );
            var_0 = scripts\mp\gametypes\br_public.gsc::lb_mg_wood_surf_dmg_scalar( var_0 );
            var_1 = scripts\mp\gametypes\br_gulag.gsc::player_using_rifle( var_0, var_3.origin );
        }
    }

    return [ var_0, var_1 ];
}

plundersilentcountdownendtime( var_0, var_1 )
{
    var_2 = 3.14159;
    var_3 = scripts\mp\gametypes\br_circle.gsc::getsafecircleorigin();
    var_4 = vectornormalize( var_0 - var_3 );
    var_5 = vectortoangles( var_4 );
    var_6 = randomfloatrange( getdvarfloat( "scr_br_respawn_rand_ang_min", 10.0 ), getdvarfloat( "scr_br_respawn_rand_ang_max", 60.0 ) );
    var_7 = var_4;
    var_8 = var_0 + var_7 * var_1;

    if ( scripts\mp\gametypes\br_c130.gsc::ispointinbounds( var_8, 0 ) )
        return var_8;

    var_7 = var_7 * -1;
    var_8 = var_0 + var_7 * var_1;

    if ( scripts\mp\gametypes\br_c130.gsc::ispointinbounds( var_8, 0 ) )
        return var_8;

    var_7 = vectornormalize( var_3 - var_0 );
    var_8 = var_0 + var_7 * var_1;

    if ( scripts\mp\gametypes\br_c130.gsc::ispointinbounds( var_8, 0 ) )
        return var_8;

    var_9 = var_1;
    var_10 = distance2d( var_0, var_3 );
    var_11 = var_9 / var_10;

    if ( var_11 > var_2 )
        var_11 = var_2;

    var_12 = var_11 * 180 / var_2;
    var_8 = rotatepointaroundvector( ( 0, 0, 1 ), var_0 - var_3, var_12 ) + var_3;

    if ( scripts\mp\gametypes\br_c130.gsc::ispointinbounds( var_8, 0 ) )
        return var_8;

    var_8 = scripts\mp\gametypes\br_circle.gsc::getrandompointincircle( var_0, var_1 );

    if ( scripts\mp\gametypes\br_c130.gsc::ispointinbounds( var_8, 0 ) )
        return var_8;

    return undefined;
}

_id_11FC6()
{
    if ( scripts\mp\gametypes\br_public.gsc::_id_11ECE() )
        self waittill( "spawnGhost" );
}

_id_11F58( var_0 )
{
    self.spawn_ai_solo = var_0;
}

_id_11F5B( var_0 )
{
    if ( istrue( var_0 ) )
        self.game_extrainfo = self.game_extrainfo | 4096;
    else
        self.game_extrainfo = self.game_extrainfo & ~4096;
}

_id_11FBE( var_0 )
{
    foreach ( var_2 in level.deadblue._id_1253F )
    {
        if ( isdefined( var_2.visuals[0] ) )
            var_2.visuals[0] _id_12541( self );
    }
}

_id_12541( var_0 )
{
    if ( var_0 scripts\mp\gametypes\br_public.gsc::_id_11ECE() )
    {
        self showtoplayer( var_0 );

        if ( !level.deadblue._id_131E6 )
            self enableplayeruse( var_0 );
    }
    else
    {
        self hidefromplayer( var_0 );

        if ( !level.deadblue._id_131E6 )
            self disableplayeruse( var_0 );
    }
}

_id_11EB0()
{
    foreach ( var_1 in level.deadblue._id_1253F )
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( var_1.objidnum, self );
}

_id_11F5A( var_0, var_1 )
{
    self.spawn_maint_wave_2 = var_0;

    if ( !isdefined( level.deadblue.propanchor ) )
        level.deadblue.propanchor = [];

    _id_11F5B( var_0 );

    if ( isdefined( level.deadblue.updatelocationbesttimehud ) )
        _id_11FBE( var_0 );

    if ( var_0 )
    {
        self notify( "ghost_set" );

        if ( level.deadblue.sat_setup_access_cards )
            _id_11E90();

        self.weapon_xp_iw8_sm_mpapa7 = 0;
        self.bcdisabled = 1;
        self.originalsubtype = 1;
        _id_11F58( 0 );
        scripts\cp\utility\auto_ascender_ai::calloutmarkerping_removecallout( 7 );
        scripts\cp\utility\auto_ascender_ai::calloutmarkerping_removecallout( 9 );
        scripts\cp\utility\auto_ascender_ai::calloutmarkerping_removecallout( 10 );
        scripts\cp\utility\auto_ascender_ai::calloutmarkerping_removecallout( 11 );
        level.deadblue.propanchor[level.deadblue.propanchor.size] = self;
        _id_13754( "numVaccine", self.weapon_xp_iw8_sm_mpapa7 );
    }
    else
    {
        self notify( "ghost_unset" );
        self.weapon_xp_iw8_sm_mpapa7 = undefined;
        self.bcdisabled = undefined;
        self.originalsubtype = undefined;
        scripts\cp\utility\auto_ascender_ai::calloutmarkerping_removecallout( 7 );

        if ( isdefined( level.deadblue.updatelocationbesttimehud ) )
            _id_11EB0();

        level.deadblue.propanchor = scripts\engine\utility::array_remove( level.deadblue.propanchor, self );
    }

    level notify( "players_remaining_changed" );
    self notify( "stop_battlechatter" );

    if ( istrue( var_1 ) )
    {
        self lerpfovbypreset( "default" );

        if ( level.deadblue.proplockunlock )
            thread scripts\mp\supers\super_deadsilence::superdeadsilence_endhudsequence();
    }
}

_id_11E90()
{
    var_0 = -60;
    var_1 = 120;
    var_2 = 180;
    self.sat_computer_think_new = _id_11E16( var_0, var_1, "right", "middle", "center", "middle", &"MP_ZXP/NUM_CONSUMED", 0 );
    self.sat_create_interaction = _id_11E16( var_0, var_1, "left", "middle", "center", "middle", &"MP_ZXP/NUM_TO_CONSUME", level.deadblue.propabilitykeysvisible );
    self.sales_discount_items = _id_11E16( 0, var_2, "center", "middle", "center", "middle", &"MP_ZXP/ZOMBIE" );
}

_id_11E16( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7 )
{
    var_8 = scripts\mp\hud_util::createfontstring( "default", 1.5 );
    var_8.x = var_0;
    var_8.y = var_1;
    var_8.alignx = var_2;
    var_8.aligny = var_3;
    var_8.horzalign = var_4;
    var_8.vertalign = var_5;
    var_8.alpha = 0;
    var_8.glowalpha = 0;
    var_8.hidewheninmenu = 1;
    var_8.archived = 0;

    if ( isdefined( var_6 ) )
        var_8.label = var_6;

    if ( isdefined( var_7 ) )
        var_8 setvalue( var_7 );

    return var_8;
}

_id_11E92( var_0 )
{
    var_1 = self.team;
    scripts\mp\utility\teams::_id_13827( "mode", var_1, self );
    _id_11FB0();

    if ( istrue( var_0 ) )
        [[ level.updategameevents ]]();
}

_id_120E4()
{
    level notify( "post_update_game_events_internal" );
    level endon( "post_update_game_events_internal" );

    if ( istrue( level.br_debugsolotest ) || level.gameended )
        return;

    var_0 = 1000;

    if ( isdefined( level.deal_damage_after_time ) && level.deal_damage_after_time + var_0 > gettime() )
        wait 1;

    var_1 = 0;
    var_2 = [];
    var_3 = [];

    foreach ( var_5 in level.teamnamelist )
    {
        var_6 = level.teamdata[var_5]["teamCount"];

        if ( var_6 > 0 )
        {
            if ( isdefined( level.teamdata[var_5]["aliveCountHuman"] ) && level.teamdata[var_5]["aliveCountHuman"] > 0 )
            {
                if ( ( level.deadblue.starthacktimer || level.deadblue.startfontscale == 0 ) && var_3.size > 0 )
                    return;

                var_3[var_3.size] = var_5;
                var_1 = var_1 + level.teamdata[var_5]["aliveCountHuman"];
                continue;
            }

            if ( level.teamdata[var_5]["aliveCount"] > 0 )
                var_2[var_2.size] = var_5;
        }
    }

    if ( var_3.size > 1 )
        return;

    var_8 = scripts\mp\utility\script::quicksort( var_2, ::_id_12CCB );

    for ( var_9 = 0; var_9 < var_8.size; var_9++ )
    {
        var_5 = var_8[var_9];
        var_10 = var_9 + 2;
        thread scripts\mp\gametypes\br.gsc::_id_119D8( var_5, var_10, 0, 1 );
        var_11 = scripts\mp\utility\teams::getfriendlyplayers( var_5 );

        foreach ( var_13 in var_11 )
        {
            var_13 scripts\cp_mp\utility\game_utility::_id_1299F( var_10 );

            if ( isalive( var_13 ) )
            {
                var_13 setscriptablepartstate( "ghost", "off" );
                var_13 playerhide();
            }
        }
    }

    var_15 = var_3[0];

    foreach ( var_13 in level.players )
    {
        if ( var_13 scripts\mp\gametypes\br_public.gsc::_id_11ECE() )
        {
            if ( var_13.team != var_15 )
            {
                var_13 allowmovement( 0 );
                var_13 allowmelee( 0 );
            }

            if ( var_13 scripts\mp\utility\perk::_hasperk( "specialty_tracker" ) )
                var_13 scripts\mp\utility\perk::removeperk( "specialty_tracker" );
        }

        var_13.ref_angle_doors = 1;
    }

    thread scripts\mp\gamelogic::endgame( var_15, game["end_reason"]["enemies_eliminated"] );
}

_id_120E3()
{
    level thread _id_120E4();
}

_id_129FE( var_0 )
{
    level.teamdata[var_0]["lastGhostTime"] = gettime();
}

_id_12CCB( var_0, var_1 )
{
    var_2 = level.teamdata[var_0]["lastGhostTime"];
    var_3 = level.teamdata[var_1]["lastGhostTime"];
    return var_2 >= var_3;
}

_id_12E42( var_0 )
{
    if ( istrue( var_0.br_infilstarted ) && scripts\mp\flags::gameflag( "prematch_done" ) && var_0 scripts\mp\gametypes\br_public.gsc::_id_11ECE() )
    {
        var_0 _id_11F26();
        return 1;
    }

    return 0;
}

addtoteamlives( var_0, var_1 )
{
    var_0 _id_11FB0();
}

removefromteamlives( var_0, var_1 )
{
    var_0 _id_11FB0();
}

_id_11FB0()
{
    var_0 = self.team;
    level.teamdata[var_0]["aliveCountHuman"] = 0;

    foreach ( var_2 in level.teamdata[var_0]["alivePlayers"] )
    {
        if ( !var_2 scripts\mp\gametypes\br_public.gsc::_id_11ECE() && !var_2 _id_11ECA() )
            level.teamdata[var_0]["aliveCountHuman"]++;
    }
}

_id_11ECA()
{
    return istrue( self.spawn_ai_solo );
}

_id_12A6F()
{
    if ( !level.deadblue.propcamerazoom )
        return;

    level.deadblue.processlobbydataforclient = spawnstruct();
    level.deadblue.processlobbydataforclient.powers = [];
    ascendsolostructs( level.deadblue.processlobbydataforclient, "jump", [ "+speed_throw", "+toggleads_throw", "+ads_akimbo_accessible" ], ::_id_11E82, 0, undefined, ::_id_11E84, undefined, &"MP_GXP/CHARGED_JUMP", undefined, 6, "jumpStatus", "jumpProgress" );
    ascendsolostructs( level.deadblue.processlobbydataforclient, "jumpStop", [ "-speed_throw", "-toggleads_throw", "-ads_akimbo_accessible" ], ::_id_11E87, 0 );
    ascendsolostructs( level.deadblue.processlobbydataforclient, "teleport", "+smoke", ::_id_11E94, 0, undefined, undefined, undefined, &"MP_GXP/TELEPORT", undefined, 15, "teleportStatus", "teleportProgress" );
    ascendsolostructs( level.deadblue.processlobbydataforclient, "spectralBlast", "+frag", ::_id_11E91, 0, undefined, undefined, undefined, &"MP_GXP/SCREAM", undefined, 15, "spectralBlastStatus", "spectralBlastProgress" );

    if ( getdvarint( "scr_br_gxp_bumper_ping_support", 1 ) )
        ascendsolostructs( level.deadblue.processlobbydataforclient, "gas_or_emp", [ "+equip_toggle_throw" ], ::_id_11E95, 0, undefined, ::_id_11E96 );
}

ascendsolostructs( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9, var_10, var_11, var_12 )
{
    var_13 = "scr_br_gxp_power_" + var_1;

    if ( getdvarint( var_13, 1 ) == 0 )
        return;

    if ( isstring( var_2 ) )
        var_2 = [ var_2 ];

    var_0.powers[var_1] = spawnstruct();
    var_0.powers[var_1].checkarmorpickup = var_2;
    var_0.powers[var_1].func = var_3;
    var_0.powers[var_1]._id_1304D = var_5;
    var_0.powers[var_1].getbestintersectionpt = var_6;
    var_0.powers[var_1]._id_120CF = var_7;
    var_0.powers[var_1].label = var_8;
    var_0.powers[var_1].suspensemusic_reset = var_9;
    var_0.powers[var_1].grenade_obj = var_10;
    var_0.powers[var_1]._id_1305F = var_11;
    var_0.powers[var_1]._id_12185 = var_12;
    var_0.powers[var_1]._id_1289D = var_4;
}

_id_11E7A()
{
    level endon( "game_ended" );
    self endon( "death_or_disconnect" );
    self endon( "ghost_unset" );

    for (;;)
    {
        self waittill( "ghostAttack" );

        if ( self isgestureplaying() && !istrue( self.prophidetime ) )
        {
            self stopgestureviewmodel( "ges_gxp_superjump", 0, 1 );
            self stopgestureviewmodel( "ges_gxp_scream" );
            self stopgestureviewmodel( "ges_gxp_teleport" );
        }
    }
}

_id_11E82( var_0, var_1 )
{
    level endon( "game_ended" );
    self endon( "death_or_disconnect" );
    self endon( "ghost_unset" );
    self endon( "playerGhostJumpStop" );
    var_2 = -1;

    if ( _id_11E83() )
    {
        _id_11EF5();
        return;
    }

    var_3 = getdvarfloat( "scr_br_gpx_powers_jump_charge_rate", 1.0 );
    var_4 = getdvarfloat( "scr_br_gxp_powers_jump_min_frac", 0.25 );
    var_5 = getdvarint( "scr_br_gxp_powers_jump_max_hold_time", var_2 );
    var_6 = var_3 * level.framedurationseconds;
    self.startpayloadtanksobjective = 0.0;
    self allowmelee( 0 );
    self disableoffhandweapons();
    self.prophidetime = 1;

    while ( self ismantling() || self isthrowinggrenade() || self ismeleeing() || scripts\mp\utility\weapon::grenadeinpullback() )
        waitframe();

    thread _id_11E7C();
    thread _id_13148();
    var_7 = undefined;
    var_8 = 0;

    if ( !isdefined( self.startptui ) || gettime() > self.startptui )
    {
        self playlocalsound( "ui_zxp_charge_jump_start" );
        self.startptui = gettime() + 500;
    }

    if ( isdefined( var_0.powers[var_1]._id_1305F ) )
        _id_13754( var_0.powers[var_1]._id_1305F, 0 );

    while ( !_id_11E83() )
    {
        if ( level.deadblue.sat_setup_access_cards )
            self._id_120F1[var_1].cargo_truck_mg_cp_init scripts\mp\hud_util::updatebar( self.startpayloadtanksobjective, 0 );
        else
            self._id_120F1[var_1].frac = self.startpayloadtanksobjective;

        var_9 = self.startpayloadtanksobjective;
        self.startpayloadtanksobjective = self.startpayloadtanksobjective + var_6;

        if ( self.startpayloadtanksobjective >= 1 )
        {
            self.startpayloadtanksobjective = 1.0;

            if ( var_5 >= 0 )
            {
                if ( !isdefined( var_7 ) )
                {
                    var_7 = gettime() + var_5 * 1000;

                    if ( level.deadblue.sat_setup_access_cards )
                        thread _id_11E86( var_1, var_5 );
                }

                if ( gettime() >= var_7 )
                    break;
            }
        }

        if ( level.deadblue.sat_setup_access_cards && var_9 < var_4 && self.startpayloadtanksobjective >= var_4 )
            self._id_120F1[var_1].cargo_truck_mg_cp_init.bar.color = ( 0, 1, 0 );

        if ( var_9 < 1 && self.startpayloadtanksobjective >= 1 )
            self playlocalsound( "ui_zxp_charge_jump_full" );

        if ( isdefined( var_0.powers[var_1]._id_12185 ) )
        {
            var_8 = max( int( self.startpayloadtanksobjective * 100 ), 0 );
            _id_13754( var_0.powers[var_1]._id_12185, var_8 );
        }

        waitframe();
    }

    thread _id_11E85( var_0, var_1 );
}

_id_13148()
{
    var_0 = self;
    var_0 endon( "death_or_disconnect" );
    var_0 notify( "applyFOVPresentation" );
    var_0 endon( "applyFOVPresentation" );
    var_0 lerpfovbypreset( "zombiearcade" );
    var_0 waittill( "endSuperJumpFov" );
    var_0 lerpfovbypreset( "zombiedefault" );
}

_id_11E83()
{
    return self getstance() == "prone" || istrue( self.usingascender );
}

_id_11E7C()
{
    level endon( "game_ended" );
    self endon( "death_or_disconnect" );
    self endon( "ghost_unset" );
    self endon( "playerGhostJumpStop" );

    if ( self isgestureplaying( "ges_gxp_superjumpcharge" ) )
        return;

    while ( self ismantling() || self isthrowinggrenade() || self ismeleeing() || scripts\mp\utility\weapon::grenadeinpullback() )
        waitframe();

    self forceplaygestureviewmodel( "ges_gxp_superjumpcharge" );

    while ( self isgestureplaying( "ges_gxp_superjumpcharge" ) )
    {
        if ( self isonladder() )
        {
            self stopgestureviewmodel( "ges_gxp_superjumpcharge" );
            break;
        }

        waitframe();
    }
}

_id_11E86( var_0, var_1 )
{
    level endon( "game_ended" );
    self endon( "death_or_disconnect" );
    self endon( "ghost_unset" );
    self endon( "playerGhostJumpStop" );
    self endon( "playerGhostJumpChargeEnd" );

    if ( var_1 <= 0 )
        return;

    var_2 = scripts\mp\gametypes\br_circle.gsc::brpickupscriptableid( var_1, int( var_1 * 5 ), 1 );
    var_3 = 1;

    for ( var_4 = 0; var_4 < var_2.size; var_4++ )
    {
        if ( var_3 )
            self._id_120F1[var_0].cargo_truck_mg_cp_init.bar.color = ( 1, 0, 0 );
        else
            self._id_120F1[var_0].cargo_truck_mg_cp_init.bar.color = ( 0, 1, 0 );

        wait( var_2[var_4] );
        var_3 = !var_3;
    }
}

_id_11E85( var_0, var_1 )
{
    self stopgestureviewmodel( "ges_gxp_superjumpcharge" );
    self notify( "playerGhostJumpChargeEnd" );
    self notify( "playerGhostJumpStop" );
    var_2 = getdvarfloat( "scr_br_gxp_powers_jump_min_frac", 0.25 );
    var_3 = getdvarint( "scr_br_gxp_powers_jump_min_frac_refund", 1 );

    if ( self.startpayloadtanksobjective >= var_2 && !_id_11E83() && _id_11E88() && !self ismantling() )
    {
        self playsoundtoplayer( "br_gov_ghost_jump_plr", self, self );
        self playsound( "br_gov_ghost_jump_3p", self, self );
        var_4 = getdvarfloat( "scr_br_gxp_powers_jump_velocity", 1300 );
        var_5 = self getplayerangles();
        thread _id_11E7E();
        _id_11DF5( var_5, var_4, self.startpayloadtanksobjective );
        thread _id_11FAF();
        thread _id_11E0F();
        self.intel_use_logic = undefined;
        self.startptui = undefined;
    }
    else if ( var_3 )
    {
        if ( level.deadblue.sat_setup_access_cards )
            self._id_120F1[var_1].cargo_truck_mg_cp_init.bar.frac = 0;
        else
            self._id_120F1[var_1].frac = 0;

        _id_13754( var_0.powers[var_1]._id_12185, 0 );
        self enableoffhandweapons();
        self allowmelee( 1 );
        self notify( "endSuperJumpFov" );
        _id_11EF5();
    }
    else
        _id_11EF5();

    _id_11E84( var_0, var_1, 1 );
}

_id_11DF5( var_0, var_1, var_2, var_3 )
{
    var_4 = 1;
    var_5 = ( 0, 0, 20 );

    if ( !isdefined( var_3 ) )
        var_3 = var_5;

    var_6 = var_0;
    var_7 = ( 0, 0, 1 );
    var_8 = ( 1, 0, 0 );

    if ( getdvarint( "scr_br_gxp_powers_jump_pitch_correction", var_4 ) )
    {
        var_7 = pilot_setups();

        if ( !isdefined( var_7 ) )
            var_7 = ( 0, 0, 1 );

        var_9 = ( 0, var_6[1], 0 );
        var_10 = anglestoright( var_9 );
        var_8 = vectorcross( var_7, var_10 );
        var_11 = vectortoangles( var_8 );
        var_12 = angleclamp180( var_11[0] );
        var_13 = -85;
        var_14 = var_12;
        var_15 = var_6[0];

        if ( var_15 > var_12 )
            var_15 = var_12;

        var_16 = getdvarfloat( "scr_br_gxp_powers_jump_pitch_correction_at_max", -45.0 );
        var_17 = getdvarfloat( "scr_br_gxp_powers_jump_pitch_correction_at_min", 0.0 );
        var_18 = ( var_15 - var_13 ) / ( var_14 - var_13 );
        var_19 = var_17 + var_18 * ( var_16 - var_17 );
        var_6 = ( var_15 + var_19, var_6[1], var_6[2] );
    }

    var_20 = getdvarfloat( "scr_br_gxp_powers_jump_pitch_add", 0.0 );

    if ( var_20 != 0.0 )
        var_6 = ( var_6[0] + var_20, var_6[1], var_6[2] );

    var_21 = anglestoforward( var_6 );
    var_22 = var_21 * var_2 * var_1;
    var_23 = self.origin + var_3;
    self setorigin( var_23 );
    self setvelocity( var_22 );
    glassradiusdamage( self.origin + ( 0, 0, 30 ), 30, 50, 51 );
    var_24 = anglestoforward( self.angles );
    var_25 = self.origin + ( 0, 0, 30 ) + var_24 * 15;
    radiusdamage( var_25, 100, 1, 1 );
}

_id_11E7E()
{
    level endon( "game_ended" );
    self endon( "death_or_disconnect" );
    self setscriptablepartstate( "skydiveVfx", "enabled_ghost", 0 );
    self enableoffhandweapons();
    self giveandfireoffhand( "gxp_superjump_mp" );
    wait 0.4;
    self notify( "endSuperJumpFov" );
    self forceplaygestureviewmodel( "ges_gxp_superjump" );

    while ( !_id_11E81() )
        waitframe();

    self notify( "ghost_jump_complete" );
    self stopgestureviewmodel( "ges_gxp_superjump" );
    self setscriptablepartstate( "skydiveVfx", "default", 0 );
    self playsoundtoplayer( "zxp_splat_plr", self, self );
    self playsound( "zmb_npc_breath_land_hi", self, self );
    self playsound( "zxp_splat_npc", self, self );
    self enableoffhandweapons();
    self allowmelee( 1 );
    self.prophidetime = undefined;
}

_id_11E88()
{
    if ( level.deadblue.startplayer != 0 )
    {
        var_0 = self.origin + ( 0, 0, level.deadblue.startplayer );
        var_1 = playerphysicstrace( self.origin, var_0 );

        if ( var_1 != var_0 )
            return 0;
    }

    if ( level.deadblue.startplunderextractiontimers != 0 )
    {
        var_2 = self geteye();
        var_0 = var_2 + ( 0, 0, level.deadblue.startplunderextractiontimers );
        var_3 = 10;
        var_4 = 20;
        var_5 = scripts\engine\trace::create_contents( 0, 1, 0, 1, 0, 0, 1 );
        var_1 = scripts\engine\trace::capsule_trace( var_2, var_0, var_3, var_4, ( 0, 0, 0 ), self, var_5 );

        if ( var_1["fraction"] != 1 )
            return 0;
    }

    return 1;
}

_id_11FAF()
{
    level endon( "game_ended" );
    self endon( "death_or_disconnect" );
    self endon( "ghost_unset" );
    var_0 = getdvarfloat( "scr_br_gxp_air_control", 400.0 );

    if ( var_0 <= 0.0 )
        return;

    var_1 = getdvarfloat( "scr_br_gxp_air_control_max_speed", 1400.0 );
    var_2 = getdvarfloat( "scr_br_gxp_ghost_fall_speed_scale", 0.85 );
    wait 0.2;

    while ( !_id_11E81() )
    {
        var_3 = self getnormalizedmovement();

        if ( length( var_3 ) > 0 )
        {
            var_4 = rotatevector( ( var_3[0], -1.0 * var_3[1], 0 ), self.angles );
            var_5 = self getvelocity();
            var_6 = length( var_5 );
            var_7 = var_4 * var_0 * level.framedurationseconds;
            var_8 = var_5 + var_7;
            var_9 = length( var_8 );

            if ( var_9 <= var_1 )
            {
                if ( var_8[2] < 0 )
                    var_8 = ( var_8[0], var_8[1], var_8[2] * var_2 );

                self setvelocity( var_8 );
            }
            else if ( var_6 < var_1 )
            {
                var_8 = vectornormalize( var_8 ) * var_1;

                if ( var_8[2] < 0 )
                    var_8 = ( var_8[0], var_8[1], var_8[2] * var_2 );

                self setvelocity( var_8 );
            }
        }

        waitframe();
    }
}

_id_11E0F()
{
    level endon( "game_ended" );
    self endon( "death_or_disconnect" );
    self endon( "ghost_unset" );
    self endon( "ghost_jump_complete" );
    var_0 = getdvarfloat( "scr_br_gxp_clear_moving_platfrom_time", 0.3 );

    if ( var_0 < 0 )
        return;

    if ( var_0 > 0 )
        wait( var_0 );

    self _meth_87AF();
}

_id_11E84( var_0, var_1, var_2 )
{
    if ( istrue( var_2 ) )
        thread _id_11F1A( var_0, var_1 );

    self.startpayloadtanksobjective = undefined;
}

_id_11E81()
{
    return self isonground() || self isonladder() || self ismantling();
}

_id_11E87( var_0, var_1 )
{
    if ( isdefined( self.startpayloadtanksobjective ) )
        _id_11E85( var_0, "jump" );
    else
        self notify( "playerGhostJumpStop" );
}

_id_11E94( var_0, var_1 )
{
    var_2 = self;
    var_3 = getdvarfloat( "scr_br_gxp_ghost_teleport_distance", 400 );
    var_4 = var_2 getplayerangles();
    var_5 = anglestoforward( var_4 );
    var_6 = var_2 geteye();
    var_7 = var_6 + var_5 * var_3;
    var_7 = var_2 call_exfil( var_7, var_6, var_5 );

    if ( !isdefined( var_2 ) || !isdefined( var_2._id_120F1 ) )
        return;

    if ( isdefined( var_7 ) )
        var_2 thread _id_11F0D( var_0, var_1 );
    else
    {
        var_2 _id_11EF5();

        if ( level.deadblue.sat_setup_access_cards )
            var_2._id_120F1["teleport"].cargo_truck_mg_cp_init scripts\mp\hud_util::updatebar( 0.05, 0 );
        else
            var_2._id_120F1["teleport"].frac = 0.05;

        var_2 thread _id_11F1A( var_0, var_1 );
    }
}

call_exfil( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    if ( !isdefined( var_3 ) )
        var_3 = 1;

    var_6 = [ self ];

    if ( isdefined( var_4 ) )
        var_6[var_6.size] = var_4;

    var_7 = var_0 + var_2 * 10;
    var_8 = scripts\engine\trace::create_contents( 0, 1, 0, 1, 0, 1, 1 );
    var_9 = scripts\engine\trace::ray_trace( var_1, var_0, var_6, var_8 );
    var_10 = var_9["position"];

    if ( var_10 == var_0 )
        var_11 = var_0;
    else
        var_11 = var_10 - var_2 * 10 + var_9["normal"] * 10;

    var_12 = var_11 + ( 0, 0, 72 );
    var_13 = scripts\engine\trace::ray_trace( var_11, var_12, var_6, var_8 );
    var_14 = var_13["position"];

    if ( abs( var_14[2] - var_12[2] ) > 1 )
    {
        var_15 = var_14 - ( 0, 0, 72 );
        var_16 = scripts\engine\trace::ray_trace( var_14, var_15, var_6, var_8 );
        var_17 = var_16["position"];

        if ( abs( var_17[2] - var_15[2] ) > 1 )
            var_11 = undefined;
        else
            var_11 = var_17;
    }

    if ( isdefined( var_11 ) && positionwouldtelefrag( var_11, self ) )
        var_11 = undefined;

    if ( isdefined( var_11 ) )
    {
        var_18 = self gettagorigin( "j_spineupper" );
        var_19 = self gettagangles( "j_spineupper" );
        playfx( level.deadblue._id_1329E, var_18, anglestoforward( var_19 ), anglestoup( var_19 ) );
        self enableoffhandweapons();
        self giveandfireoffhand( "gxp_teleport_mp" );

        if ( var_3 )
        {
            var_20 = getdvarfloat( "scr_br_gxp_ghost_teleport_delay", 0.1 );
            wait( var_20 );
        }

        if ( !isdefined( self ) )
            return undefined;

        self setorigin( var_11 );
        playfxontag( level.deadblue._id_1329E, self, "j_spineupper" );
        playfxontagforclients( level.deadblue._id_1329D, self, "tag_eye", self );

        if ( istrue( var_5 ) )
        {
            var_21 = self getvelocity();
            var_22 = length( var_21 );
            var_23 = var_2 * var_22 * 0.5;
            self setvelocity( var_23 );
            self playsoundtoplayer( "br_gov_safespace_repel", self, self );
            self playsound( "br_gov_safespace_repel_3p", self, self );
        }
        else
        {
            self playsoundtoplayer( "br_gov_ghost_teleport_plr", self, self );
            self playsound( "br_gov_ghost_teleport_3p", self, self );
        }
    }

    return var_11;
}

processmatchscoreboardinfo( var_0 )
{
    if ( isdefined( var_0 ) )
        wait( var_0 );

    self enableplayerbreathsystem( 1 );
}

_id_11E91( var_0, var_1 )
{
    var_2 = 64;
    var_3 = var_2 * var_2;
    var_4 = getdvarfloat( "scr_br_gxp_ghost_blast_radius", 768 );
    var_5 = var_4 * var_4;
    var_6 = "zxp_emp_fire_plr";
    var_7 = self;
    var_7 enableplayerbreathsystem( 0 );
    var_7 playsoundonmovingent( var_6 );
    var_7 playsoundtoplayer( "br_gov_ghost_blast_plr", var_7, var_7 );
    var_7 playsound( "br_gov_ghost_blast_3p", var_7, var_7 );
    var_7 thread processmatchscoreboardinfo( 2 );
    playfxontag( level.deadblue._id_12EC1, var_7, "j_spineupper" );
    playfxontagforclients( level.deadblue._id_12EC0, var_7, "tag_eye", var_7 );
    self enableoffhandweapons();
    self giveandfireoffhand( "gxp_blast_mp" );
    var_7 playrumbleonentity( "defaultweapon_fire" );
    var_7 earthquakeforplayer( 0.1, 0.3, var_7.origin, 100 );
    var_8 = getcompleteweaponname( "emp_drone_non_player_mp" );
    var_9 = getcompleteweaponname( "emp_drone_non_player_direct_mp" );
    var_10 = scripts\cp_mp\emp_debuff::get_emp_ents();

    foreach ( var_12 in var_10 )
    {
        var_13 = var_12.owner;

        if ( isdefined( var_13 ) )
        {
            if ( var_13 != var_7 && !scripts\cp_mp\utility\player_utility::playersareenemies( var_7, var_13 ) )
                continue;
        }

        var_14 = distancesquared( var_7.origin, var_12.origin );

        if ( var_14 > var_5 )
            continue;

        var_15 = scripts\engine\utility::ter_op( var_14 > var_3, var_8, var_9 );
        var_12 dodamage( 1, var_7.origin, var_7, var_7, "MOD_EXPLOSIVE", var_15 );
        var_12 playsoundonmovingent( "zxp_emp_impact_ent" );
        var_16 = scripts\cp_mp\utility\damage_utility::packdamagedata( var_7, var_12, 1, var_15, "MOD_EXPLOSIVE", var_7, var_7.origin );
        thread _id_11E6A( var_16 );
    }

    var_18 = getcompleteweaponname( "emp_drone_player_mp" );
    var_19 = undefined;

    if ( scripts\cp_mp\utility\script_utility::issharedfuncdefined( "player", "getPlayersInRadius" ) )
        var_19 = [[ scripts\cp_mp\utility\script_utility::getsharedfunc( "player", "getPlayersInRadius" ) ]]( var_7.origin, var_4 );

    var_20 = var_7 geteye();
    var_21 = scripts\engine\trace::create_contents( 0, 1, 0, 1, 0, 0, 1 );

    foreach ( var_23 in var_19 )
    {
        if ( var_23 == var_7 )
            continue;

        if ( var_23 scripts\mp\gametypes\br_public.gsc::_id_11ECE() )
            continue;

        if ( var_23 != var_7 && !scripts\cp_mp\utility\player_utility::playersareenemies( var_7, var_23 ) )
            continue;

        if ( scripts\mp\gametypes\br_gxp_safe_zones.gsc::smoke_enemy_think( var_23 ) )
            continue;

        var_24 = scripts\engine\trace::ray_trace_passed( var_20, var_23 gettagorigin( "j_head" ), self, var_21 );

        if ( !var_24 )
            continue;

        var_23 dodamage( 1, var_7.origin, var_7, var_7, "MOD_EXPLOSIVE", var_18 );
        var_16 = scripts\cp_mp\utility\damage_utility::packdamagedata( var_7, var_23, 1, var_18, "MOD_EXPLOSIVE", var_7, var_7.origin );
        thread _id_11E6A( var_16 );
    }

    thread _id_11F0D( var_0, var_1 );
}

_id_11F0D( var_0, var_1 )
{
    if ( level.deadblue.sat_setup_access_cards )
        self._id_120F1[var_1].cargo_truck_mg_cp_init scripts\mp\hud_util::updatebar( 1.0, 0 );
    else
        self._id_120F1[var_1].frac = 1.0;

    thread _id_11F1A( var_0, var_1 );
}

_id_11E6A( var_0 )
{
    var_1 = getdvarfloat( "scr_br_gxp_ghost_blast_emp_duration", 5 );
    var_2 = getdvarfloat( "scr_br_gxp_ghost_blast_shellshock_duration", 5 );
    var_3 = var_0.victim;

    if ( isplayer( var_3 ) )
    {
        var_3 playsound( "zxp_emp_impact_plr" );
        var_4 = scripts\engine\utility::ter_op( getdvarint( "scr_br_gxp_ghost_blast_shellshock_nerf", 0 ), "gxp_scream_mp_nerfed", "gxp_scream_mp" );
        var_3 scripts\cp_mp\utility\shellshock_utility::_shellshock( var_4, "gas", var_2, 1 );
        playfxontagforclients( level.deadblue._id_12EC2, var_3, "tag_eye", var_3 );
    }
    else
    {
        scripts\cp_mp\emp_debuff::apply_emp_struct( var_0 );
        linked_model( var_0, var_1 );

        if ( isdefined( var_3 ) )
            var_3 scripts\cp_mp\emp_debuff::remove_emp();
    }
}

linked_model( var_0, var_1 )
{
    var_0.victim endon( "death_or_disconnect" );
    level endon( "game_ended" );
    var_2 = scripts\engine\utility::waittill_notify_or_timeout_return( "emp_cleared", var_1 );

    if ( var_2 != "emp_cleared" )
        var_0.empremoved = 1;
}

_id_11E95( var_0, var_1 )
{
    level endon( "game_ended" );
    self endon( "disconnect" );
    self notify( "playerGhostTeleportOrSpectralBlast" );
    self endon( "playerGhostTeleportOrSpectralBlast" );
    var_2 = gettime() + getdvarint( "scr_br_gxp_teleport_or_spectral_blast_timeout_ms", 500 );

    while ( var_2 > gettime() )
    {
        if ( self secondaryoffhandbuttonpressed() )
        {
            self notify( "teleport" );
            break;
        }
        else if ( self fragbuttonpressed() )
        {
            self notify( "spectralBlast" );
            break;
        }

        waitframe();
    }
}

_id_11E96( var_0, var_1 )
{
    self notify( "playerGhostTeleportOrSpectralBlast" );
}

lootstruct_offsets()
{
    return scripts\common\vehicle::isvehicle() || isdefined( self.classname ) && self.classname == "script_vehicle";
}

modifyplayerdamage( var_0 )
{
    var_1 = 250.0;
    var_2 = 100;
    var_3 = var_0.damage;
    var_4 = isplayer( var_0.attacker ) && var_0.attacker scripts\mp\gametypes\br_public.gsc::_id_11ECE();
    var_5 = isdefined( var_0.attacker ) && var_0.attacker lootstruct_offsets();
    var_6 = isplayer( var_0.victim ) && var_0.victim scripts\mp\gametypes\br_public.gsc::_id_11ECE();
    var_7 = scripts\mp\utility\weapon::getweaponbasenamescript( var_0.objweapon );

    if ( var_4 && var_6 && var_0.meansofdeath == "MOD_MELEE" )
    {
        if ( !level.deadblue.propdeductchange )
            var_3 = 0;
        else
            var_3 = level.deadblue.propdeductclonechange;
    }
    else if ( var_4 && !var_6 && !var_0.attacker isinexecutionattack() && var_0.victim isinexecutionvictim() )
        var_3 = 0;
    else if ( isdefined( var_0.attacker ) && istrue( var_0.attacker.isjuggernaut ) && var_6 && var_0.meansofdeath == "MOD_MELEE" )
        var_3 = var_0.victim.maxhealth / 3;
    else if ( var_6 && var_0.meansofdeath == "MOD_FALLING" )
        var_3 = 0;
    else if ( var_6 && isdefined( var_0.inflictor ) && isdefined( var_0.inflictor.streakinfo ) && ( var_0.inflictor.streakinfo.streakname == "toma_strike" || var_0.inflictor.streakinfo.streakname == "precision_airstrike" || var_0.inflictor.streakinfo.streakname == "manual_turret" ) )
    {
        var_8 = var_0.victim.maxhealth;
        var_9 = var_0.attacker.maxhealth;
        var_3 = var_0.damage * int( floor( var_8 / var_9 ) );
    }
    else if ( level.deadblue.propent && var_6 && isexplosivedamagemod( var_0.meansofdeath ) && isdefined( var_0.inflictor ) && var_0.inflictor scripts\cp_mp\vehicles\vehicle::isvehicle() )
        var_3 = 0;
    else if ( var_6 && ( var_0.meansofdeath == "MOD_GRENADE_SPLASH" || var_0.meansofdeath == "MOD_EXPLOSIVE" || var_0.meansofdeath == "MOD_FIRE" ) && ( var_7 == "molotov_mp" || var_7 == "thermite_av_mp" ) )
        var_3 = var_0.damage * level.deadblue.propdeductflash;
    else if ( var_4 && !var_6 && var_0.meansofdeath == "MOD_MELEE" )
    {
        var_10 = var_0.victim.maxhealth;
        var_11 = level.deadblue.projectileunlinkonnote;

        if ( istrue( var_0.victim.isjuggernaut ) )
            var_11 = level.deadblue.projectilewaittilstucktimeout;

        if ( !level.deadblue.progression_update )
            var_10 = var_10 + var_0.victim.br_maxarmorhealth;

        if ( istrue( var_0.victim.inlaststand ) )
        {
            var_10 = level.laststandhealth;
            var_11 = level.deadblue.prompt;
        }

        var_3 = int( ceil( var_10 / var_11 ) );
    }
    else if ( var_6 && var_5 && istrue( var_0.victim._id_13986 ) )
        var_3 = 0;
    else if ( var_6 && var_5 && level.deadblue.propkilledend )
    {
        var_3 = 0;
        _id_13994( var_0 );
    }
    else if ( var_4 && !var_6 && var_0.meansofdeath == "MOD_IMPACT" && var_7 == "rock_mp" )
    {
        var_12 = spawnstruct();
        var_12.origin = var_0.point;
        var_0.victim thread scripts\mp\equipment\concussion_grenade::applyconcussion( var_12, var_0.attacker );
    }
    else if ( var_6 )
    {
        var_13 = 0.7;
        var_14 = scripts\mp\utility\weapon::getweaponrootname( var_0.objweapon );
        var_15 = weaponclass( var_7 );
        var_16 = scripts\mp\gametypes\br.gsc::spawn_boss_wave_1( var_0.idflags );

        if ( !var_16 )
        {
            switch ( var_15 )
            {
                case "sniper":
                    if ( var_0.shitloc == "head" || var_0.shitloc == "helmet" )
                    {
                        if ( scripts\mp\gametypes\br.gsc::spectatekey( var_14 ) )
                            var_3 = int( ceil( level.deadblue.progression_speed * var_13 ) );
                        else
                            var_3 = level.deadblue.progression_speed;
                    }

                    break;
                default:
                    if ( var_0.shitloc == "head" || var_0.shitloc == "helmet" )
                    {
                        var_17 = var_2;

                        if ( level.deadblue._id_137CD )
                            var_17 = var_1;

                        var_3 = int( ceil( var_3 / var_17 * level.deadblue.progression_speed * level.deadblue.progression_main ) );
                    }

                    break;
            }
        }

        var_18 = "scr_br_gxp_scale_" + var_15;
        var_19 = 0;

        if ( var_15 == "spread" )
            var_19 = 0.7;

        var_20 = getdvarfloat( var_18, var_19 );

        if ( var_20 != 0 )
            var_3 = int( ceil( var_3 * var_20 ) );

        var_21 = "scr_br_gxp_scale_" + var_14;
        var_19 = 0;

        if ( var_14 == "iw8_sh_charlie725" )
            var_19 = 1.43;

        var_22 = getdvarfloat( var_21, var_19 );

        if ( var_22 != 0 )
            var_3 = int( ceil( var_3 * var_22 ) );

        if ( var_0.meansofdeath == "MOD_RIFLE_BULLET" || var_0.meansofdeath == "MOD_PISTOL_BULLET" )
            var_3 = int( ceil( var_3 * level.deadblue.propdeathfx ) );
        else if ( var_0.meansofdeath == "MOD_MELEE" )
            var_3 = int( ceil( var_3 * level.deadblue.proper_damage ) );
    }

    if ( var_3 > 0 && var_4 && !var_6 && var_0.attacker isinexecutionattack() && var_0.victim isinexecutionvictim() )
        level.deal_damage_after_time = gettime();

    return var_3;
}

_id_11EE6()
{
    if ( scripts\mp\gametypes\br_public.gsc::_id_11ECE() )
        return;
    else
        scripts\mp\gametypes\br.gsc::waittill_scout_drone_timeout();
}

droponplayerdeath( var_0 )
{
    if ( scripts\mp\gametypes\br_public.gsc::_id_11ECE() )
        return 1;

    if ( level.deadblue.sat_sound_think )
        _id_11EDE();

    return 0;
}

onplayerkilled( var_0 )
{
    if ( !istrue( level.br_prematchstarted ) )
        return;

    if ( isdefined( self ) )
        thread _id_129D8();

    if ( level.gameended )
        return;

    var_1 = var_0.victim;
    var_2 = var_0.attacker;

    if ( !isdefined( var_2 ) || !isplayer( var_2 ) || !isdefined( var_1 ) )
        return;

    if ( var_1 _id_12B25( var_2, var_0.meansofdeath ) )
        var_1 thread _id_12E47( var_1, var_2 );

    if ( istrue( level.deadblue.proggressionmismatchpopup ) && var_1 _id_12B24( var_2 ) )
        var_1 thread _id_12E3F( var_1, var_2 );

    if ( var_1 _id_12B45( var_2 ) )
        var_1 thread _id_11E6F();

    if ( var_2 _id_12B29( var_0 ) )
        var_2 thread _id_11E6B();

    var_3 = var_0.hitloc;

    if ( isdefined( var_3 ) && var_1 scripts\mp\gametypes\br_public.gsc::_id_11ECE() && ( var_3 == "head" || var_3 == "helmet" ) )
    {
        var_4 = 0;
        var_2 thread scripts\mp\damagefeedback::updatedamagefeedback( "hitzombieheadshot", var_4, 1 );
    }

    if ( var_1 scripts\mp\gametypes\br_public.gsc::_id_11ECE() )
    {
        var_5 = var_1 gettagorigin( "j_spineupper" );
        var_6 = var_1 gettagangles( "j_spineupper" );
        playfx( level.deadblue.processsplashpriorityqueue, var_5, anglestoforward( var_6 ), anglestoup( var_6 ) );
    }

    var_1 setscriptablepartstate( "skydiveVfx", "default", 0 );
}

_id_129D8()
{
    self endon( "disconnect" );

    if ( !scripts\mp\gametypes\br_public.gsc::_id_11ECE() )
        return;

    self waittill( "spawned" );
    scripts\mp\utility\player::_setsuit( "iw8_defaultsuit_mp" );
}

_id_12B25( var_0, var_1 )
{
    if ( !_id_12B44( var_0 ) )
        return 0;

    if ( scripts\mp\gametypes\br_public.gsc::_id_11ECE() )
        return 0;

    if ( var_1 == "MOD_EXECUTION" )
    {
        if ( !level.deadblue.propclonepower && !istrue( self.inlaststand ) )
            return 0;
    }

    if ( scripts\mp\gametypes\br_gxp_safe_zones.gsc::smoke_enemy_think( self ) )
        return 0;

    return 1;
}

_id_12B29( var_0 )
{
    if ( !level.deadblue.propclonecleanup )
        return 0;

    if ( var_0.meansofdeath != "MOD_EXECUTION" )
        return 0;

    if ( var_0.victim scripts\mp\gametypes\br_public.gsc::_id_11ECE() )
        return 0;

    if ( !level.deadblue.propclonepower && istrue( var_0.victim.inlaststand ) )
        return 0;

    if ( !var_0.attacker scripts\mp\gametypes\br_public.gsc::_id_11ECE() )
        return 0;

    return 1;
}

_id_12E47( var_0, var_1 )
{
    var_2 = spawndogtags();
    _id_12A62( var_2, var_0.origin );

    if ( istrue( var_0.isjuggernaut ) )
    {
        var_3 = 2;
        var_4 = getdvarint( "scr_br_gxp_numDropJugg", var_3 );
        var_5 = scripts\mp\gametypes\br_pickups.gsc::setup_functions();

        for ( var_6 = 1; var_6 < var_4; var_6++ )
        {
            var_7 = scripts\mp\gametypes\br_pickups.gsc::getitemdroporiginandangles( var_5, var_0.origin, var_0.angles, var_0, undefined, undefined, 0 );

            if ( !isdefined( var_7 ) || var_7.origin == ( 0, 0, 0 ) )
                var_7.origin = var_0.origin;

            var_2 = spawndogtags();
            _id_12A62( var_2, var_7.origin );
        }
    }
}

_id_12B24( var_0 )
{
    if ( !_id_12B44( var_0 ) )
        return 0;

    if ( scripts\mp\gametypes\br_public.gsc::_id_11ECE() )
    {
        if ( randomintrange( 1, 101 ) <= level.deadblue.progress_obj )
            return 1;
    }

    return 0;
}

_id_12E3F( var_0, var_1 )
{
    var_2 = spawndogtags();
    _id_12A62( var_2, var_0.origin );
}

_id_12B45( var_0 )
{
    if ( !_id_12B44( var_0 ) )
        return 0;

    if ( !scripts\mp\gametypes\br_public.gsc::_id_11ECE() )
        return 0;

    return 1;
}

_id_11E6F()
{
    if ( !isdefined( level.deadblue.project_to_line_seg ) || level.deadblue.project_to_line_seg.size <= 0 )
        return;

    self.itemsdropped = 0;
    var_0 = [];

    for ( var_1 = 0; var_1 < level.deadblue.project_to_line; var_1++ )
    {
        if ( randomintrange( 1, 101 ) > level.deadblue.project )
            continue;

        var_2 = randomintrange( 0, level.deadblue.project_to_line_seg.size );
        var_3 = level.deadblue.project_to_line_seg[var_2];
        var_0[var_0.size] = var_3;
    }

    var_4 = scripts\mp\gametypes\br_lootcache.gsc::turret_fob_self_destruct( var_0, 0 );
}

_id_12B44( var_0 )
{
    if ( isdefined( var_0 ) && var_0 == self )
        return 0;

    if ( level.teambased && isdefined( var_0 ) && isdefined( var_0.team ) && var_0.team == self.team )
        return 0;

    if ( isdefined( var_0 ) && !isdefined( var_0.team ) && ( var_0.classname == "trigger_hurt" || var_0.classname == "worldspawn" ) )
        return 0;

    if ( isagent( self ) || isagent( var_0 ) )
        return 0;

    return 1;
}

spawndogtags()
{
    var_0 = 16;
    var_1 = undefined;
    var_2 = 0;
    var_3 = undefined;

    if ( level.deadblue._id_12540.size > 0 )
    {
        var_4 = level.deadblue._id_12540.size - 1;
        var_1 = level.deadblue._id_12540[var_4];
        level.deadblue._id_12540[var_4] = undefined;
        isplayeronintelchallenge( var_1 );
        var_2 = 1;
        var_3 = var_1.trigger;
        var_5 = var_1.visuals;
    }
    else if ( level.deadblue._id_1253F.size >= level.deadblue.updatelocationbesttimehud )
    {
        var_1 = playerzombiehud();
        isplayeronintelchallenge( var_1 );
        var_2 = 1;
        var_3 = var_1.trigger;
        var_5 = var_1.visuals;
    }
    else
    {
        var_6 = "gxp_soul_pickup";
        var_5[0] = spawn( "script_model", ( 0, 0, 0 ) );
        var_5[0].angles = ( 45, 0, 0 );
        var_5[0] setmodel( var_6 );
        var_5[0] hudoutlineenable( "outline_depth_purple" );
        var_5[0] setasgametypeobjective();
        var_5[0].usetype = 1;
        var_5[0].sq_ontimerexpired = 1;
        var_3 = var_5[0];

        if ( level.deadblue._id_131E6 )
        {
            var_3 = spawn( "trigger_radius", ( 0, 0, 0 ), 0, 32, 32 );
            var_3.sq_ontimerexpired = 1;
            var_5[0].sq_ontimerexpired = undefined;
        }
    }

    var_7 = "any";
    var_8 = 0;
    var_1 = scripts\mp\gameobjects::createuseobject( var_7, var_3, var_5, ( 0, 0, var_0 ), undefined, var_2 );
    var_1._id_12BFB = 1;
    var_1.onuse = ::onuse;
    var_1 scripts\mp\gameobjects::setusetime( var_8 );
    var_1 scripts\mp\gametypes\br_public.gsc::setupkeybindings();
    var_1.inuse = 1;
    var_1.lastusedtime = gettime();
    var_9 = "" + var_1 getentitynumber();
    level.deadblue._id_1253F[var_9] = var_1;
    return var_1;
}

_id_12A62( var_0, var_1 )
{
    var_2 = 36;
    var_3 = ( 0, 0, 36 );
    var_4 = scripts\mp\gametypes\br_public.gsc::lb_mg_wood_surf_dmg_scalar( var_1, 30 );
    var_5 = var_4 + ( 0, 0, var_2 );
    var_0.curorigin = var_5;

    if ( level.deadblue._id_131E6 )
        var_0.trigger.origin = var_5;

    var_0.visuals[0].origin = var_5;
    var_0 scripts\mp\gameobjects::initializetagpathvariables();
    var_0.interactteam = "any";
    var_0.visuals[0] _id_12B95();
    var_0.ownerteam = "neutral";
    var_0.trigger triggerenable();

    if ( isdefined( var_0.objidnum ) )
    {
        if ( var_0.objidnum != -1 )
        {
            var_6 = var_0.objidnum;
            scripts\mp\objidpoolmanager::update_objective_state( var_6, "active" );
            scripts\mp\objidpoolmanager::update_objective_position( var_6, var_4 + var_3 );
            scripts\mp\objidpoolmanager::update_objective_setbackground( var_6, 1 );
            scripts\mp\objidpoolmanager::objective_set_play_intro( var_0.objidnum, 0 );
            scripts\mp\objidpoolmanager::objective_set_play_outro( var_0.objidnum, 0 );
            var_0 scripts\mp\gameobjects::setobjectivestatusicons( "waypoint_dogtags_friendly", "waypoint_dogtags" );
            var_0 scripts\mp\gameobjects::setvisibleteam( "any" );
            objective_icon( var_0.objidnum, "icon_minimap_soul" );
            scripts\mp\objidpoolmanager::objective_playermask_hidefromall( var_0.objidnum );
        }
    }

    playsoundatpos( var_5, "mp_killconfirm_tags_drop" );
    var_0.visuals[0] scriptmodelplayanim( "mp_dogtag_spin" );
}

isplayeronintelchallenge( var_0 )
{
    var_0.visuals[0] dontinterpolate();
    var_0.visuals[0] hide();
    var_0.trigger triggerdisable();
    var_0.trigger notify( "deleted" );
    var_0 scripts\mp\gameobjects::allowuse( "none" );
    var_0.inuse = 0;
    var_0.visuals[0].origin = ( 0, 0, 0 );
    var_0.trigger.origin = ( 0, 0, 0 );
    getggweapontablelootvariants( var_0 );
    scripts\mp\objidpoolmanager::objective_playermask_hidefromall( var_0.objidnum );
}

playerzombiehud()
{
    var_0 = undefined;

    foreach ( var_2 in level.deadblue._id_1253F )
    {
        if ( !isdefined( var_0 ) || var_2.lastusedtime < var_0.lastusedtime )
            var_0 = var_2;
    }

    return var_0;
}

onuse( var_0 )
{
    thread _id_119E4( var_0 );
}

_id_119E4( var_0 )
{
    if ( !isdefined( var_0 ) )
    {
        thread removetags( self );
        return;
    }

    if ( !playercanusetags( var_0 ) )
        return;

    thread removetags( self );
    var_0.weapon_xp_iw8_sm_mpapa7++;

    if ( level.deadblue.sat_setup_access_cards )
        var_0 _id_11EB6();

    var_0 _id_13754( "numVaccine", var_0.weapon_xp_iw8_sm_mpapa7 );

    if ( isdefined( level.deadblue.updatelocationbesttimehud ) && var_0.weapon_xp_iw8_sm_mpapa7 >= level.deadblue.propabilitykeysvisible )
        var_0 thread _id_11E6B();
}

_id_12B95()
{
    self hide();

    if ( !level.deadblue._id_131E6 )
    {
        self makeusable();
        self setcursorhint( "HINT_NOICON" );
        self sethintstring( &"MP_ZXP/PICKUP" );
        self setuseprioritymax();
    }

    foreach ( var_1 in level.players )
        _id_12541( var_1 );
}

getggweapontablelootvariants( var_0 )
{
    foreach ( var_2 in level.players )
    {
        if ( !var_2 scripts\mp\gametypes\br_public.gsc::_id_11ECE() )
            continue;

        var_3 = var_2 calloutmarkerping_getgscobjectiveindex( 7 );

        if ( var_3 != -1 && var_3 == var_0.objidnum )
            var_2 scripts\cp\utility\auto_ascender_ai::calloutmarkerping_removecallout( 7 );
    }
}

removetags( var_0 )
{
    isplayeronintelchallenge( var_0 );
    var_1 = "" + var_0 getentitynumber();
    level.deadblue._id_1253F[var_1] = undefined;
    level.deadblue._id_12540[level.deadblue._id_12540.size] = var_0;
    playfx( level._effect["ghost_soul_pickup"], var_0.curorigin );
    playsoundatpos( var_0.curorigin, "br_gov_soul_pickup" );
}

playercanusetags( var_0 )
{
    return var_0 scripts\mp\gametypes\br_public.gsc::_id_11ECE();
}

_id_11EB6()
{
    var_0 = ( 0, 1, 0 );
    self.sat_computer_think_new setvalue( self.weapon_xp_iw8_sm_mpapa7 );
    self.sat_computer_think_new thread safehouse_spawn( var_0 );
    self.sat_create_interaction thread safehouse_spawn( var_0 );
}

_id_11E6B( var_0 )
{
    if ( !istrue( var_0 ) && !scripts\mp\gametypes\br_public.gsc::_id_11ECE() )
        return;

    var_1 = scripts\mp\gametypes\br_public.gsc::_id_11ECE() || istrue( self._id_13B6A );

    if ( var_1 )
    {
        scripts\mp\gametypes\br_gametype_gxp_challenges.gsc::zombienumhitstruck( self );
        self.propinfo = 1;
    }

    _id_11F5A( 0 );
    _id_11E92( 0 );
    scripts\mp\gametypes\br_gxp_phones.gsc::little_bird_mg_getspawnstructscallback( self );

    if ( isdefined( self.operatorcustomization.gender ) && self.operatorcustomization.gender == "female" )
        self _meth_87A8( "female" );
    else
        self _meth_87A8( "" );

    if ( isdefined( self.operatorcustomization.clothtype ) && self.operatorcustomization.clothtype != "" )
        self setclothtype( self.operatorcustomization.clothtype );
    else
        self setclothtype( "vestlight" );

    self.operatorcustomization = undefined;
    scripts\cp_mp\execution::_clearexecution();
    self._id_12538 = 1;
    var_2 = self.origin;
    var_3 = self.origin;
    var_4 = self getplayerangles();
    var_5 = 0;

    if ( level.deadblue.sat_wait_for_activated_think )
        [var_3, var_4, var_2] = _id_11EBF();
    else
    {
        [var_3, var_4, var_5] = _id_11EC0();
        var_2 = var_3;
    }

    self.plotarmor = 1;
    self setscriptablepartstate( "ghost", "off" );
    self setscriptablepartstate( "compassicon", "defaulticon" );
    self setscriptablepartstate( "skydiveVfx", "default", 0 );
    _id_11E8E();

    if ( !var_5 )
    {
        scripts\mp\gametypes\br_gulag.gsc::gulagfadetoblack();
        wait 1;
    }
    else
        waitframe();

    scripts\mp\class::loadout_emptycacheofloadout( "gamemode" );
    self.pers["gamemodeLoadout"] = level.br_respawn_loadout;
    self.pers["class"] = "gamemode";
    self.class = "gamemode";
    self.forcespawnangles = var_4;
    self.forcespawnorigin = var_2;
    scripts\mp\utility\player::_setsuit( "iw8_defaultsuit_mp" );
    scripts\mp\playerlogic::spawnplayer( undefined, 0 );
    self skydive_deployparachute();
    thread scripts\mp\gametypes\br.gsc::create_vehicle_interact();
    _id_11E97( 1 );
    self enableexecutionvictim();
    self _meth_87B0();
    self lerpfovbypreset( "default_2seconds" );

    if ( level.deadblue.proplockunlock )
        thread scripts\mp\supers\super_deadsilence::superdeadsilence_endhudsequence();

    if ( level.deadblue.proplocationindex )
    {
        _id_11EBC();

        if ( !level.deadblue.proplockunlock )
            self setscriptablepartstate( "headVFX", "neutral" );

        self visionsetnakedforplayer( "", 0 );
    }

    if ( level.deadblue.sat_wait_for_activated_think )
    {
        self.plotarmor = undefined;
        _id_11F99( var_3, var_4, var_2 );
    }
    else
    {
        if ( !var_5 )
        {
            scripts\mp\gametypes\br_public.gsc::_id_11FC5();
            scripts\mp\gametypes\br_public.gsc::_id_11E11();
            playfx( scripts\engine\utility::getfx( "ghost_trans" ), self.origin );
            playfxontagforclients( level.deadblue.propindex, self, "tag_eye", self );
        }

        if ( !var_5 )
            scripts\mp\gametypes\br_gulag.gsc::gulagfadefromblack();

        thread _id_11E12();
    }

    if ( istrue( level.deadblue.sat_sound_think ) && _id_11EDC() )
        _id_11EDD();
    else
    {
        var_8 = scripts\mp\gametypes\br.gsc::deadred();
        scripts\mp\gametypes\br.gsc::quantity( var_8, 0 );
    }

    scripts\mp\gametypes\br_armor.gsc::quarry2_ambient_sound_load();
    thread scripts\mp\gametypes\br.gsc::create_vehicle_interact();

    if ( level.deadblue.proplocationindex )
        little_bird_mg_enterendinternal();

    _id_11F0C( 0 );

    if ( var_1 )
        scripts\mp\hud_message::showsplash( "br_gametype_gxp_change_human" );

    self.plotarmor = undefined;
    thread _id_11EBB();
    self._id_12538 = undefined;

    if ( var_1 )
    {
        foreach ( var_10 in level.teamdata[self.team]["players"] )
        {
            if ( self == var_10 )
            {
                scripts\mp\gametypes\br_public.gsc::decoyassists( "player_into_human", var_10 );
                continue;
            }

            scripts\mp\gametypes\br_public.gsc::decoyassists( "teammate_back_human", var_10 );
        }
    }

    self.propinfo = undefined;
}

little_bird_mg_enterendinternal()
{
    if ( !isdefined( level.deadblue.propanchor ) )
        return;

    level.deadblue.propanchor = scripts\engine\utility::array_removeundefined( level.deadblue.propanchor );

    foreach ( var_1 in level.deadblue.propanchor )
        scripts\mp\utility\outline::outlineenableforplayer( self, var_1, "outline_depth_zombievision", "top" );
}

safehouse_spawn( var_0 )
{
    self endon( "death" );

    if ( istrue( self._id_121FA ) )
        return;

    var_1 = 0.5;
    var_2 = 4;
    self._id_121FA = 1;
    var_3 = self.fontscale;
    var_4 = self.color;

    if ( isdefined( var_0 ) )
        self.color = var_0;

    self changefontscaleovertime( var_1 );
    self.fontscale = var_2;
    wait( var_1 );
    self changefontscaleovertime( var_1 );
    self.fontscale = var_3;
    wait( var_1 );
    self.color = var_4;
    self._id_121FA = undefined;
}

_id_11EBF()
{
    var_0 = getdvarint( "scr_br_gxp_spawnheightoffset", 3000 );
    var_1 = scripts\mp\gametypes\br_public.gsc::playernakeddroploadout() / 1000;
    var_2 = scripts\mp\gametypes\br_gulag.gsc::_id_11EA2( 0, var_1, var_0 );
    var_3 = scripts\mp\gametypes\br_gulag.gsc::_id_11F1F( var_2 );
    return [ var_2.origin, var_2.angles, var_3 ];
}

_id_11EC0()
{
    [var_1, var_2, var_3] = _id_11EBA();

    if ( !var_3 )
        scripts\mp\gametypes\br_public.gsc::_id_11F95( var_1 );

    return [ var_1, var_2, var_3 ];
}

_id_11E8E()
{
    _id_11E97( 0 );
    var_0 = gettime() + 3000;

    while ( self isgestureplaying() && var_0 > gettime() )
    {
        self stopgestureviewmodel();
        waitframe();
    }

    while ( var_0 > gettime() && ( self isinexecutionattack() || self isswitchingweapon() || self isreloading() || self ismantling() || self isthrowinggrenade() || self israisingweapon() || self ismeleeing() || self [[ scripts\cp_mp\utility\script_utility::getsharedfunc( "player", "isPlayerADS" ) ]]() ) )
        waitframe();

    playfx( scripts\engine\utility::getfx( "ghost_trans" ), self.origin );
    playfxontagforclients( level.deadblue.propindex, self, "tag_eye", self );
    self notify( "endSuperJumpFov" );
    self enableoffhandweapons();
    self giveandfireoffhand( "gxp_revive_mp" );
    wait 1.25;
}

_id_11E97( var_0 )
{
    self allowfire( var_0 );
    self allowmovement( var_0 );
    self allowmelee( var_0 );

    if ( var_0 )
    {
        self playershow();
        self enableoffhandweapons();
    }
    else
    {
        self playerhide();
        self disableoffhandweapons();
    }
}

_id_11E12()
{
    if ( !getdvarint( "scr_br_gxp_human_spawn_concuss", 0 ) )
        return;

    var_0 = 650;
    var_1 = getdvarint( "scr_br_gxp_push_radius", var_0 );
    var_2 = incrementpersistentstat( level.players, self.origin, var_1 );

    foreach ( var_4 in var_2 )
    {
        if ( var_4 scripts\mp\gametypes\br_public.gsc::_id_11ED5() && var_4.team != self.team && isalive( var_4 ) )
            _id_11EB9( var_4, var_1 );
    }

    var_6 = anglestoforward( self.angles );
    playfx( level.deadblue.search_target_think, self.origin, var_6 );
    playsoundatpos( self.origin, "sentry_explode_smoke" );
    playrumbleonposition( "grenade_rumble", self.origin );
    earthquake( 0.5, 1.5, self.origin, var_1 );
}

_id_11EDE()
{
    var_0 = spawnstruct();
    var_0._id_12152 = [];
    var_0.br_pickupdenyarmorfull = [];
    var_0.br_pickupdenyjuggernaut = [];
    var_0.br_plunder_site_use_parts = [];
    var_0.offhands = [];
    var_0.make_control_station_interaction = [];
    var_1 = [];
    var_2 = self getweaponslistprimaries();

    foreach ( var_4 in var_2 )
    {
        if ( !scripts\mp\utility\weapon::spawn_sentries_from_targetname( var_4 ) && !issubstr( var_4.basename, "iw8_fists_mp" ) && !scripts\mp\utility\weapon::spawn_module_intro3( var_4.basename ) )
            var_1[var_1.size] = var_4;
    }

    foreach ( var_7 in var_1 )
    {
        var_8 = createheadicon( var_7 );
        var_0.br_pickupdenyarmorfull[var_8] = weaponclipsize( var_7 );
        var_0.br_plunder_site_use_parts[var_8] = self getweaponammostock( var_7 );

        if ( scripts\mp\utility\weapon::sort_wave_spawning_ai( var_7 ) )
            var_0.br_pickupdenyjuggernaut[var_8] = self getweaponammoclip( var_7, "left" );

        if ( getsubstr( var_8, 0, 4 ) == "alt_" )
            continue;

        var_0._id_12152[var_0._id_12152.size] = var_7;
    }

    if ( self.lastcacweaponobj != getcompleteweaponname( "none" ) )
    {
        foreach ( var_4 in var_0._id_12152 )
        {
            if ( self.lastcacweaponobj == var_4 )
            {
                var_0.current = self.lastcacweaponobj;
                break;
            }
        }
    }

    var_12 = self getweaponslistoffhands();

    foreach ( var_14 in var_12 )
    {
        if ( var_14.basename == "bandage_br" )
            continue;

        var_15 = self getweaponammoclip( var_14 );

        if ( var_15 <= 0 )
            continue;

        var_0.offhands[var_0.offhands.size] = var_14;
        var_16 = createheadicon( var_14 );
        var_0.br_pickupdenyarmorfull[var_16] = var_15;
    }

    foreach ( var_20, var_19 in self.equipment )
        var_0.make_control_station_interaction[var_19] = var_20;

    var_0.super = undefined;

    if ( isdefined( self.super ) && !self.super.usepercent )
        var_0.super = self.equipment["super"];

    self.reflectprojectile = var_0;
}

_id_11EDD()
{
    self takeallweapons( 0, 1 );
    scripts\mp\gametypes\br_weapons.gsc::br_ammo_player_clear();
    self.equipment["primary"] = undefined;
    self.equipment["secondary"] = undefined;
    self.equipment["health"] = undefined;
    self.equipment["super"] = undefined;
    var_0 = getcompleteweaponname( "iw8_fists_mp" );

    if ( self.reflectprojectile._id_12152.size < 2 )
        self giveweapon( var_0 );

    var_1 = 0;

    foreach ( var_3 in self.reflectprojectile._id_12152 )
    {
        var_4 = createheadicon( var_3 );
        scripts\cp_mp\utility\inventory_utility::_giveweapon( var_3 );

        if ( !var_1 )
        {
            self assignweaponprimaryslot( var_4 );
            scripts\cp_mp\utility\inventory_utility::_switchtoweapon( var_3 );
            var_1 = 1;
        }

        scripts\mp\weapons::fixupplayerweapons( self, var_4 );
    }

    foreach ( var_7 in self.reflectprojectile.offhands )
    {
        var_8 = scripts\mp\equipment::getequipmentreffromweapon( var_7 );

        if ( !isdefined( var_8 ) )
            continue;

        var_9 = self.reflectprojectile.make_control_station_interaction[var_8];

        if ( !isdefined( var_9 ) )
            continue;

        scripts\mp\equipment::giveequipment( var_8, var_9 );
    }

    foreach ( var_4, var_12 in self.reflectprojectile.br_plunder_site_use_parts )
    {
        self setweaponammostock( var_4, var_12 );
        var_3 = getcompleteweaponname( getweaponbasename( var_4 ) );
        var_13 = scripts\mp\gametypes\br_weapons.gsc::br_ammo_type_for_weapon( var_3 );

        if ( isdefined( var_13 ) )
        {
            self.br_ammo[var_13] = var_12;
            scripts\mp\gametypes\br_weapons.gsc::br_ammo_player_hud_update_ammotype( var_13 );
        }
    }

    foreach ( var_4, var_12 in self.reflectprojectile.br_pickupdenyarmorfull )
        self setweaponammoclip( var_4, var_12 );

    foreach ( var_4, var_12 in self.reflectprojectile.br_pickupdenyjuggernaut )
        self setweaponammoclip( var_4, var_12, "left" );

    waitframe();
    var_16 = var_0;

    if ( isdefined( self.reflectprojectile.current ) )
        var_16 = self.reflectprojectile.current;
    else if ( isdefined( self.reflectprojectile._id_12152[0] ) )
        var_16 = self.reflectprojectile._id_12152[0];

    self switchtoweaponimmediate( var_16 );

    if ( isdefined( self.reflectprojectile.super ) )
    {
        var_17 = level.br_pickups.br_superreference[level.br_pickups.br_equipnametoscriptable[self.reflectprojectile.super]];
        scripts\mp\gametypes\br_pickups.gsc::forcegivesuper( var_17, 0 );
    }

    thread scripts\cp_mp\gestures::_id_135A2();
    self.reflectprojectile = undefined;
}

_id_11EBB()
{
    level endon( "game_ended" );
    self endon( "death_or_disconnect" );
    self endon( "zombie_set" );

    while ( !self isonground() )
        waitframe();

    thread _id_11EBD();
}

_id_11EBA()
{
    var_0 = 500;
    var_1 = 10000;
    var_2 = 5;

    if ( !isdefined( level.br_circle ) || !isdefined( level.br_circle.dangercircleent ) )
        return [ self.origin, self getplayerangles(), 1 ];

    var_3 = scripts\mp\gametypes\br_circle.gsc::getdangercircleradius();
    var_4 = scripts\mp\gametypes\br_circle.gsc::getdangercircleorigin();
    var_5 = distance2dsquared( self.origin, var_4 );

    if ( var_5 <= var_3 * var_3 )
        return [ self.origin, self getplayerangles(), 1 ];

    var_6 = undefined;
    var_7 = undefined;
    var_8 = ( self.origin[0], self.origin[1], 0 );
    var_9 = vectornormalize( var_8 - var_4 );

    for ( var_10 = 1; var_10 <= var_2; var_10++ )
    {
        var_11 = var_3 - var_0 * var_10;

        if ( var_11 < 0 )
            break;

        [var_6, var_7] = playersetarenaomnvarwithloadout( var_4, var_9, var_11 );

        if ( isdefined( var_6 ) )
            break;
    }

    if ( !isdefined( var_6 ) )
    {
        var_6 = var_4;
        var_7 = self getplayerangles();
    }

    var_13 = scripts\mp\gametypes\br_public.gsc::lb_mg_wood_surf_dmg_scalar( var_6, var_1 );
    return [ var_13, var_7, 0 ];
}

_id_11EBD()
{
    if ( !istrue( level.deadblue.sat_wait_for_access_card ) )
        return;

    thread _id_11F90( level.deadblue.human );
}

playersetarenaomnvarwithloadout( var_0, var_1, var_2 )
{
    var_3 = var_0 + var_1 * var_2;
    var_4 = scripts\mp\gametypes\br_public.gsc::playernakeddroploadout() / 1000;

    if ( scripts\mp\gametypes\br_gulag.gsc::ray_trace_trigger_radius_2d( var_3, var_4 ) )
    {
        var_5 = vectortoangles( var_1 * -1 );
        return [ var_3, var_5 ];
    }
    else
        return [ undefined, undefined ];
}

_id_11EDC()
{
    return isdefined( self.reflectprojectile );
}

_id_11EB9( var_0, var_1 )
{
    var_2 = 1800;
    var_3 = spawnstruct();
    var_3.origin = self.origin;
    var_0 thread scripts\mp\equipment\concussion_grenade::applyconcussion( var_3, self );
    var_0 thread _id_11F0F( level.deadblue.processlobbydataforclient );
    var_4 = var_0.origin - self.origin;
    var_5 = vectortoangles( var_4 );
    var_6 = distance( var_0.origin, self.origin );
    var_7 = 1.0 - var_6 / var_1;
    var_0 _id_11DF5( var_5, var_2, var_7 );
}

_id_11F0F( var_0 )
{
    if ( !isdefined( var_0 ) )
        return;

    self notify( "disableCooldown" );

    foreach ( var_3, var_2 in var_0.powers )
    {
        if ( !isdefined( self._id_120F1[var_3] ) )
            continue;

        self._id_120F1[var_3].incooldown = 0;
        thread _id_11F0D( var_0, var_3 );
    }
}

target_found_dist_sq( var_0 )
{
    if ( scripts\mp\gametypes\br_public.gsc::_id_11ECE() )
    {
        var_1 = isdefined( var_0.attacker ) && var_0.attacker lootstruct_offsets();

        if ( !var_1 || !level.deadblue.proplist || level.deadblue.propkilledend )
            return 0;

        thread _id_11F0F( level.deadblue.processlobbydataforclient );
        thread _id_11E89();
        thread _id_11E99( var_0 );
        thread _id_11E6E();
    }

    return 1;
}

_id_11E89()
{
    level endon( "game_ended" );
    self endon( "last_stand_finished" );
    self endon( "death_or_disconnect" );
    self waittill( "last_stand_transition_done" );
    waittillframeend;
    self setlaststandselfreviving( 1 );
    self.spawnvehicles = 1;
    self.laststandreviveent makeunusable();
    var_0 = self.laststandreviveent;
    var_0.usetime = getdvarfloat( "scr_br_gxp_vehicle_getup", 3.0 ) * 1000;

    if ( !isdefined( var_0.curprogress ) )
        var_0.curprogress = 0;

    while ( scripts\mp\utility\player::isreallyalive( self ) && var_0.curprogress < var_0.usetime )
    {
        if ( self isinexecutionvictim() )
        {
            waitframe();
            continue;
        }

        if ( !isdefined( var_0.userate ) )
            var_0.userate = 0;

        var_0.curprogress = var_0.curprogress + level.frameduration * var_0.userate;
        var_0.userate = 1;
        scripts\mp\gameobjects::updateuiprogress( var_0, 1 );

        if ( var_0.curprogress >= var_0.usetime )
            break;

        waitframe();
    }

    var_0.usetime = undefined;
    var_0.curprogress = undefined;
    var_0.userate = undefined;
    scripts\mp\laststand::mp_layover_patch( "self_revive_success", self );
    self playsoundtoplayer( "zmb_breath_land_dropin", self, self );
    self playsound( "zmb_npc_breath_land_dropin" );
    self setlaststandselfreviving( 0 );
}

_id_11E99( var_0 )
{
    var_1 = 500;
    var_2 = 90;
    var_3 = 60;
    var_4 = 30;
    var_5 = var_0.direction_vec;
    var_6 = vectortoyaw( var_5 );
    var_7 = var_2;
    var_8 = var_3;

    if ( scripts\engine\utility::cointoss() )
        var_8 = var_8 * -1;

    var_8 = var_8 + var_6;
    var_9 = ( var_7, var_8, 0 );
    var_10 = vectornormalize( ( var_5[0], var_5[1], 0 ) );
    var_11 = var_10 * var_4 + ( 0, 0, var_4 );
    _id_11DF5( var_9, var_1, 1.0, var_11 );
}

_id_11E6E()
{
    self endon( "disconnect" );
    self._id_13986 = 1;
    var_0 = getdvarfloat( "scr_br_gxp_vehicle_immunity", 1.5 );
    wait( var_0 );
    self._id_13986 = undefined;
}

_id_13994( var_0 )
{
    var_1 = var_0.attacker;
    var_2 = var_0.victim;
    var_3 = var_0.direction_vec;
    var_3 = -1 * vectornormalize( ( var_3[0], var_3[1], min( var_3[2], 0 ) ) );
    var_4 = var_2 geteye();
    var_5 = var_4 + var_3 * 400;
    var_5 = var_2 call_exfil( var_5, var_4, var_3, 0, var_1 );
}

dangercircletick( var_0, var_1 )
{
    if ( level.deadblue.propgetlocation != 1 )
        return;

    if ( !isdefined( level.deadblue._id_1253F ) )
        return;

    var_2 = var_1 + level.deadblue.propgiveteamscore;
    var_3 = var_2 * var_2;

    foreach ( var_5 in level.deadblue._id_1253F )
    {
        if ( isdefined( var_5.visuals ) && distance2dsquared( var_5.origin, var_0 ) >= var_3 )
            thread removetags( var_5 );
    }
}
