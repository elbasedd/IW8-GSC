...
spawn_remote_tank_thermite( var_0 )
{
    return var_0 >= 100;
}
...
getclasschoice( var_0 )
{
    ...
    if ( isdefined( level.rear_minigun_wait_between_shots ) && level.rear_minigun_wait_between_shots > 0 )
    {
        if ( scripts\mp\utility\game::matchmakinggame() )
...
executeclasschange( var_0, var_1 )
{
    ...
    else
    {
        ...
        var_3 = 1;
        var_4 = scripts\mp\gamelogic::extractposition( scripts\mp\utility\game::postupdategameevents() );
        ...
        {
            var_5 = scripts\mp\utility\game::spawn_maint_wave_1() && self getclientomnvar( "ui_open_loadout_bag" );
...
handleclassedit( var_0 )
{
    ...
    var_3 = scripts\mp\class::trap_door_nvg_reset();
    var_4 = var_2 || var_3;
...
autoassign()
{
    if ( scripts\mp\utility\game::getgametype() == "infect" )
        ...
        thread setteam( "allies" );
    else if ( br_ammorestock_playeruse() )
        thread setteam( "allies" );
    else
    {
        ...
        if ( istrue( level.teammaxfill ) )
        {
            ...
            {
                if ( scripts\mp\teams::_id_12B0A() && var_1 == "team_two_hundred" )
                    continue;
        ...
        if ( var_5.size > 0 )
        {
            ...
            if ( istrue( level.calloutmarkerpingvo_createcalloutbattlechatter ) )
                var_9 = 0;
...
setteam( var_0 )
{
    ...
    if ( scripts\mp\utility\game::allowclasschoice() || scripts\mp\utility\game::showfakeloadout() && !isai( self ) )
    {
        if ( getdvarint( "scr_force_cac_sre_callstack", 0 ) == 1 && scripts\mp\utility\game::getgametype() == "br" && scripts\mp\utility\game::postupdategameevents() != "dmz" && scripts\mp\utility\game::postupdategameevents() != "risk" && scripts\mp\utility\game::postupdategameevents() != "sandbox" && scripts\mp\utility\game::postupdategameevents() != "rumble" )
        {
            ...
            scripts\mp\utility\script::intel_location_vo( "setTeam() " + self.name + " ui_options_menu = 2, allowClassChoiceFunc = " + var_1 );
        }
    ...
    if ( ( self.sessionstate == "spectator" || _id_11ED3() ) && !istrue( self.suicideswitched ) )
    {
...
_id_11ED3()
{
    return isdefined( level._id_11ED3 ) && self [[ level._id_11ED3 ]]();
}
...
leavesquad( var_0, var_1 )
{
    ...
    if ( isdefined( var_0 ) && isdefined( var_1 ) )
        thread scripts\mp\spawnselection::_id_12377( var_0, var_1 );
...
waitforclassselect()
{
    ...
    for (;;)
    {
        var_0 = scripts\mp\utility\game::getgametype() == "br" && scripts\mp\utility\game::allowclasschoice() && ( !scripts\mp\flags::gameflag( "prematch_done" ) || istrue( level._id_12BF7 ) || istrue( level.decoy_ignoredbyenemy ) );
        ...
        if ( var_0 || var_1 )
        {
            ...
            if ( !isai( self ) )
            {
                ...
                if ( var_3 && !scripts\mp\utility\game::spawn_chopper_carepackage() )
...
beginclasschoice( var_0 )
{
    ...
    if ( scripts\mp\utility\game::allowclasschoice() || scripts\mp\utility\game::showfakeloadout() && !isai( self ) )
    {
        if ( getdvarint( "scr_force_cac_sre_callstack", 0 ) == 1 && scripts\mp\utility\game::getgametype() == "br" && scripts\mp\utility\game::postupdategameevents() != "dmz" && scripts\mp\utility\game::postupdategameevents() != "risk" && scripts\mp\utility\game::postupdategameevents() != "sandbox" && scripts\mp\utility\game::postupdategameevents() != "rumble" )
        {
            ...
            scripts\mp\utility\script::intel_location_vo( "beginClassChoice() " + self.name + " ui_options_menu = 2, allowClassChoiceFunc = " + var_2 );
        }
...
bypassclasschoice()
{
    ...
    if ( spawnheliactorsfunc() && level.enforceantiboosting && !isbot( self ) )
    {
...
addtoteam( var_0, var_1, var_2 )
{
    ...
    if ( var_0 == "spectator" || var_0 == "follower" )
    {
        ...
        scripts\cp_mp\pet_watch::_id_11983();
    }
    else
