...
{
    if ( istrue( level._id_12BCF ) || var_0 == "MOD_EXECUTION" )
        return;
    ...
    else if ( istrue( self.iszombie ) || istrue( level.registeraccesscardlocs ) && level.gametype == "infect" && self.team == "axis" )
...
{
    if ( !isarray( level.aigroundturretstate ) )
        level.aigroundturretstate = [];

    if ( !isent( level.aigroundturretref ) )
        level.aigroundturretref = spawn( "sound_transient_soundbanks", ( 0, 0, 0 ) );
}
...
assigned_ai( var_0 )
{
    ...
    if ( isdefined( level.aigroundturretstate[var_0] ) )
        return;

    level.aigroundturretref settransientsoundbank( var_0 + ".all", 1 );
    level.aigroundturretstate[var_0] = level.aigroundturretstate.size;
    return;
}
...
_id_124C6( var_0 )
{
    ...
    if ( isdefined( level.aigroundturretstate[var_0] ) )
    {
        level.aigroundturretref settransientsoundbank( var_0 + ".all", 0 );
        level.aigroundturretstate[var_0] = undefined;
    }
    else
    {
