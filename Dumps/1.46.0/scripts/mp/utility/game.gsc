...
processcashpilevalueoverrides( var_0 )
{
    return "team_two_hundred";
}

stadium_two_death_func( var_0 )
{
    if ( var_0 == processcashpilevalueoverrides() && ( isdefined( level._id_13D9C ) || currentmultikill() ) )
        return 1;
...
currentmultikill()
{
    return getdvarint( "scr_br_use_agents", 0 ) > 0 || istrue( level.deactivate_laser_from_struct );
}
...
nextroundisfinalround()
{
    ...
    {
        if ( level stadium_two_death_func( var_2 ) )
            continue;
...
waslastround()
{
    ...
    if ( istrue( level.heliatcapacitycallback ) )
        return 1;
...
isanymlgmatch()
{
    ...
    return getdvarint( "LOMTKQTRTM" ) || istrue( level.friendlyfiredeath );
}

ismlgsystemlink()
{
    if ( level.systemlink && ( getdvarint( "LOMTKQTRTM" ) || istrue( level.friendlyfiredeath ) ) )
        return 1;
...
ismlgprivatematch()
{
    if ( privatematch() && ( getdvarint( "LOMTKQTRTM" ) || istrue( level.friendlyfiredeath ) ) )
        return 1;
...
{
    var_0 = getgametype();
    var_1 = postupdategameevents();
    return var_0 != var_1;
}

spotlight_tag()
{
    if ( postupdategameevents() == "br" )
        return 1;
    else if ( postupdategameevents() == "jugg" )
        return 1;
...
playertimedinvunerable()
{
    level._id_11A7F = 5;
    return level._id_11A7F;
}
...
pretrograd_interaction_effects()
{
    ...
    if ( isdefined( var_0._id_119F1 ) )
        return var_0._id_119F1;
...
playertimestart()
{
    if ( !isdefined( level._id_11A81 ) )
        level._id_11A81 = getdvarint( "scr_br_payload_retreat_time", 30 );

    return level._id_11A81;
}

playertimestamp()
{
    if ( !isdefined( level._id_11A80 ) )
        level._id_11A80 = getdvarint( "scr_br_payload_redraw_time", 10 );

    return level._id_11A80;
}
...
trial_targs_combo( var_0, var_1, var_2, var_3, var_4 )
{
...
spawninsafehouse()
{
    return istrue( level.spawninfo );
}

spawn_ml_p2_player_heli()
{
    return istrue( level.spawn_module_building_chopper1 ) || istrue( level.spawn_module_building_chopper2 ) || istrue( level.spawn_ml_p2_sentries );
}
...
f11lights( var_0 )
{
    ...
    if ( ( getgametype() == "arm" || spawn_ml_p2_player_heli() ) && ss_playerdisconnect() || getdvarint( "scr_game_forceEnableParachuteCut", 0 ) == 1 )
    {
        if ( var_3 != "mp_aniyah" )
        {
            ...
            if ( istrue( level._id_11AE2 ) && !istrue( var_0.iskillstreakdeployweapon ) )
                var_2 = 1;
        }
    }
...
get_to_z_and_fly_off()
{
    if ( spawn_chopper_carepackage() )
    {
    ...
    else if ( isdefined( level.rear_minigun_wait_between_shots ) && level.rear_minigun_wait_between_shots > 0 )
        return 1;
