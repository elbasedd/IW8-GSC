...
init()
{
    var_0 = scripts\mp\utility\dvars::playerzombiethermalcleanup( "scr_" + scripts\mp\utility\game::getgametype() + "_healthregentime", "scr_player_healthregentime" );
    ...
    level.increment_player_participation = getdvarfloat( "scr_br_deathsdoor_exit_pct", 0.8 );
    level.increment_factor = getdvarfloat( "scr_br_deathsdoor_enter_pct", 0.55 );
...
healhregenthink( var_0 )
{
    ...
    if ( var_2 < level.increment_factor && !scripts\mp\utility\player::isusingremote() && !istrue( self.scriptablesstartid ) )
        onenterdeathsdoor();
    ...
    for (;;)
    {
        ...
        if ( isdefined( level._id_1237D ) )
            var_7 = [[ level._id_1237D ]]( var_7 );
        ...
        if ( self.health < self.maxhealth )
        {
            ...
            if ( var_2 > level.increment_player_participation )
                onexitdeathsdoor( 1 );
        }
    ...
    if ( var_6 )
    {
        ...
        scripts\cp\utility\cp_controlled_callbacks::_id_119CF( var_10 );
        scripts\mp\analyticslog::logevent_playerhealed( self, var_10, self );
    }
    else if ( !isai( self ) )
        scripts\mp\analyticslog::trial_ui_set_combo_bar_combo( self, var_10 );
...
waitforhealthregendelay()
{
    ...
    while ( self.currentregendelay - self.regendelayreduction > 0 )
    {
        ...
        if ( isdefined( level._id_1237C ) )
            var_5 = [[ level._id_1237C ]]( var_5 );
...
applyshrapnelfxinternal()
{
    ...
    level endon( "game_ended" );
    var_0 = scripts\engine\utility::_id_13AE2( "shrapnel_ended_early", "shrapnel_ended", "force_regeneration" );
