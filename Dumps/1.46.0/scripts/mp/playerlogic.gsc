...
timeuntilspawn( var_0 )
{
    ...
    if ( !isdefined( self.setupevenlydistributedpads ) )
        self.setupevenlydistributedpads = var_1;
...
mayspawn()
{
    ...
    if ( isdefined( level.isplayerloading ) && [[ level.isplayerloading ]]( self ) )
        return 0;
...
spawnclient( var_0 )
{
    ...
    if ( isdefined( level.vehicle_has_flare ) )
    {
        self [[ level.vehicle_has_flare ]]( var_0 );
        return;
    }
...
waitandspawnclient( var_0 )
{
    ...
    if ( isdefined( level._id_12151 ) )
        [[ level._id_12151 ]]( var_0 );
    ...
    if ( isdefined( self.setspawnpoint ) && !tivalidationcheck() )
        scripts\mp\equipment\tac_insert::_id_12E63( 1 );

    var_3 = timeuntilspawn( 0 );
    var_4 = 0.0;

    if ( !istrue( level.isplayerinsiderectangularzone ) )
    ...
    {
        if ( isdefined( self.setupevenlydistributedpads ) )
            var_3 = max( self.setupevenlydistributedpads - self.killcamwatchtime, 0 );
        else
    ...
    if ( !istrue( level.isplayerinsiderectangularzone ) && var_9 && !istrue( self.skipspawncamera ) && !istrue( level.usespawnselection ) && var_3 > 0 )
    ...
    else if ( var_3 > 0 )
    {
        ...
        var_1 = 1;
        scripts\engine\utility::_id_13AF3( var_3, "force_spawn" );
    ...
    if ( !istrue( level.isplayerinsiderectangularzone ) || scripts\mp\utility\game::getgametype() == "arm" )
...
tivalidationcheck()
{
    ...
    if ( isdefined( level.failsafe_triggered ) && ![[ level.failsafe_triggered ]]( self ) )
        return 0;
    ...
    foreach ( var_2 in var_0 )
    {
        ...
        scripts\mp\equipment\tac_insert::_id_12E63();
        return 0;
    }
...
revivespawnvalidationcheck()
{
    ...
    var_0 = spawnstruct();
    var_0._id_12E6C = self.forcespawnorigin;
    var_0.sr_next_ammo_restock_time = 1;
    var_1 = [];
    var_1[0] = self;
    ...
    {
        var_0.sr_next_ammo_restock_time = 0;
        return var_0;
    }
...
getspawnpoint()
{
    ...
    else if ( isdefined( self.forcespawnorigin ) )
    {
        ...
        if ( !var_4.sr_next_ammo_restock_time && isdefined( self.prevrevivepos ) && !isdefined( self.rallypoint ) )
        ...
        {
            if ( positionwouldtelefrag( var_1 ) || !var_4.sr_next_ammo_restock_time )
            {
    ...
    else if ( isdefined( self.setspawnpoint ) )
    {
        ...
        {
            if ( istrue( level.spawn_module_building_chopper1 ) && level.mapname != "mp_aniyah" )
            {
                self.wait_to_close_in = undefined;
                var_5 = scripts\engine\utility::ter_op( istrue( level.br_domheight ), 1, istrue( self.setspawnpoint.issuper ) );

                if ( var_5 )
                {
                    var_6 = min( level.select_kitchen_spawners - self.setspawnpoint.playerspawnpos[2], level.select_kitchen_spawners );

                    if ( var_6 < level.select_kitchen_spawners - 950 )
                        var_6 = var_6 + 950;

                    self.setspawnpoint.playerspawnpos = self.setspawnpoint.playerspawnpos + ( 0, 0, var_6 );
                    self.spawn_vo_lines = 1;
                }
            }
...
spawnplayer( var_0, var_1 )
{
    ...
    {
        if ( getdvarint( "scr_force_cac_sre_callstack", 0 ) == 1 && scripts\mp\utility\game::getgametype() == "br" && scripts\mp\utility\game::postupdategameevents() != "dmz" && scripts\mp\utility\game::postupdategameevents() != "risk" && scripts\mp\utility\game::postupdategameevents() != "sandbox" && scripts\mp\utility\game::postupdategameevents() != "rumble" )
        {
            ...
            scripts\mp\utility\script::intel_location_vo( "spawnPlayer()" + self.name + " ui_options_menu = 2, hasInfil = " + var_2 + " infil_started = " + var_3 + "bypassClassChoiceFunc = " + var_4 );
        }
    ...
    else
    {
        ...
        scripts\mp\class::trap_room_dogtag_revive( self.pers["copiedClass"], var_7 );

        if ( isdefined( self.pers["lastKiller"] ) )
        {
            trigger_spawn_kill_watcher( self.pers["lastKiller"] );
    ...
    if ( !var_0 )
    {
        ...
        if ( istrue( self.ogangles ) )
        {
            self.ogangles = undefined;
        ...
        self.setupevenlydistributedpads = undefined;
        ...
        if ( isdefined( self.relic_vampire_globalfunc ) )
            self.relic_vampire_globalfunc = 0;
    ...
    if ( !var_0 )
    {
        ...
        if ( !istrue( self.gulag ) && !istrue( self.respawningfromtoken ) && !istrue( self._id_12538 ) )
    ...
    thread scripts\cp\utility\cp_controlled_callbacks::onspawn();
    ...
    scripts\mp\utility\player::_id_12161( "playerlogic::spawnPlayer() !!!CODE SPAWN!!! @" + self.spawndata.spawnorigin );

    if ( !isdefined( level.tanks_help_reminder ) )
        level.tanks_help_reminder = [];

    level.tanks_help_reminder[self.team] = self.spawndata.spawnorigin;
    ...
    scripts\mp\utility\game::f11lights( self );
    [[ level.onspawnplayer ]]();
    ...
    if ( !var_0 )
    {
        self.matchdatalifeindex = scripts\common\utility::_id_13592( level.update_player_about_remaining_enemies );
    ...
    if ( isdefined( self.selectedspawnarea ) )
        scripts\mp\analyticslog::trial_updatehitmarker( self, self.selectedspawnarea );
    ...
    if ( !istrue( level.isplayerinsiderectangularzone ) && isdefined( self.spawncameraent ) || istrue( level.isplayerinsiderectangularzone ) && isdefined( self.spawncameraent ) && scripts\mp\utility\game::getgametype() == "arm" )
    {
...
trigger_spawn_kill_watcher( var_0 )
{
    if ( isdefined( var_0 ) )
...
logstartingloadout()
{
    ...
    if ( isdefined( self.curclass ) && self.curclass != "juggernaut" )
        scripts\mp\class::loadout_lognewlygivenloadout( self.questbasetimer, self.classstruct, self.curclass );
}
...
spawnspectatormapcam( var_0 )
{
    ...
    scripts\mp\utility\player::_id_12161( "playerlogic::spawnSpectatorMapCam() !!!CODE SPAWN!!! @" + var_6.origin );
    ...
    if ( isdefined( var_0 ) && var_0 == 99 )
        ...
        var_15 = scripts\engine\utility::_id_13AEE( 2, "versus_menu_done", "prematch_over" );
...
onspawnspectator( var_0, var_1, var_2 )
{
    if ( isdefined( var_0 ) && isdefined( var_1 ) )
    {
        ...
        scripts\mp\utility\player::_id_12161( "playerlogic::onSpawnSpectator() !!!CODE SPAWN!!! @" + var_0 );
        return;
    }
    ...
    if ( scripts\mp\utility\game::spawn_ml_p2_player_heli() && !self.hasspawned )
        self predictstreampos( var_3.origin );

    self spawn( var_3.origin, var_3.angles );
    scripts\mp\utility\player::_id_12161( "playerlogic::onSpawnSpectator() !!!CODE SPAWN!!! @" + var_3.origin );
}
...
getspectatepoint()
{
    ...
    if ( scripts\mp\utility\game::getgametype() == "brtdm" )
        return level.disabledstate._id_12EBA;
...
spawnintermission( var_0, var_1, var_2 )
{
    ...
    if ( !isdefined( level.custom_ending ) )
    {
        ...
        scripts\mp\utility\player::_id_12161( "playerlogic::spawnIntermission() !!!CODE SPAWN!!! @" + var_0.origin );
        scripts\mp\utility\player::setdof_spectator();
    }
...
spawnendofgame()
{
    if ( isdefined( level.vehicle_incomingcallback ) && self [[ level.vehicle_incomingcallback ]]() )
        return;
...
callback_playerdisconnect( var_0 )
{
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" && self isinexecutionvictim() && isalive( self ) )
    {
        ...
        if ( !isbot( self ) )
        {
            ...
            if ( scripts\mp\damage::_id_12B3E( self ) )
            {
                var_1 scripts\mp\utility\stats::incpersstat( "damage", var_2 );

                if ( !isdefined( var_1.hvt_can_lose_health ) )
                    var_1.hvt_can_lose_health = var_2;
                else
                    var_1.hvt_can_lose_health = var_1.hvt_can_lose_health + var_2;
            }
            else if ( isdefined( level.zombievehiclelaststand ) )
                var_1 [[ level.zombievehiclelaststand ]]( var_2 );
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" )
    {
        ...
        if ( scripts\mp\utility\game::postupdategameevents() == "br" )
        {
            var_5 = [];
            ...
            var_9 = var_5.size + 1;
            scripts\cp_mp\utility\game_utility::_id_1299F( var_9 );
        }
    }
    ...
    scripts\cp_mp\utility\player_utility::_id_124A2();
    ...
    else if ( self.sessionstate != "playing" && scripts\mp\utility\game::getgametype() == "br" && ( scripts\mp\gametypes\br_public.gsc::iswaitingtoentergulag( self ) || scripts\mp\gametypes\br_public.gsc::spawn_reinforcement_group( self ) || scripts\mp\gametypes\br_public.gsc::spawnrisktoken( self ) ) )
    ...
    if ( isdefined( self.team ) )
        scripts\mp\utility\teams::_id_13827( "disconnect", self.team, self );
    ...
    scripts\common\utility::_id_13592( level.update_operator_east_char_loc, var_0 );

    if ( level.players.size == 0 )
        level thread loadout_updatebrammo();
}

loadout_updatebrammo()
{
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" && getdvarint( "scr_data_force_send_matchdata_for_no_players_left", 1 ) == 1 && istrue( level.br_prematchstarted ) )
        scripts\mp\gamelogic::_id_1285E();
...
trigger_waittill_player_enters()
{
...
callback_playerconnect()
{
    ...
    if ( isdefined( level._id_119A4 ) )
        var_0 = self thread [[ level._id_119A4 ]]();
    ...
    if ( level.uniqueplayersconnected <= playerthrowsmokesignal() )
    {
    ...
    self.remove_munitions_globally = undefined;
    self.remove_medic_class = undefined;
    self.lifeid = 0;
    ...
    scripts\cp_mp\utility\player_utility::assign_lowest_full_slot_to_active();
    ...
    thread _start_rooftop_raid_heli::carriable_pickup( self );
    ...
    if ( !isdefined( self.pers["team"] ) )
    {
        ...
        else if ( ( scripts\mp\utility\game::matchmakinggame() || scripts\mp\utility\game::lobbyteamselectenabled() || isgamebattlematch() ) && self.sessionteam != "none" )
        {
            ...
            if ( ( getdvar( "scr_br_gametype", "" ) == "dmz" || getdvar( "scr_br_gametype", "" ) == "risk" ) && getdvarint( "scr_br_manual_team_assignment", 0 ) == 1 )
                thread scripts\mp\teams::_id_11BFC();
            else
            {
            ...
            if ( scripts\mp\utility\game::allowclasschoice() || scripts\mp\utility\game::showfakeloadout() && !isai( self ) )
            {
                if ( getdvarint( "scr_force_cac_sre_callstack", 0 ) == 1 && scripts\mp\utility\game::getgametype() == "br" && scripts\mp\utility\game::postupdategameevents() != "dmz" && scripts\mp\utility\game::postupdategameevents() != "risk" && scripts\mp\utility\game::postupdategameevents() != "rumble" && scripts\mp\utility\game::postupdategameevents() != "sandbox" )
                {
                    ...
                    scripts\mp\utility\script::intel_location_vo( "Callback_PlayerConnect() elseif MMG " + self.name + " ui_options_menu = 2, allowClassChoiceFunc = " + var_14 );
                }
        ...
        else
        {
            ...
            if ( scripts\mp\utility\game::allowclasschoice() || scripts\mp\utility\game::showfakeloadout() && !isai( self ) )
            {
                ...
                {
                    var_14 = isdefined( level.allowclasschoicefunc ) && istrue( self [[ level.allowclasschoicefunc ]]() );
                    scripts\mp\utility\script::intel_location_vo( "Callback_PlayerConnect() else " + self.name + " ui_options_menu = 2, allowClassChoiceFunc = " + var_14 );
                }
...
kickifdontspawn()
{
    ...
    if ( istrue( self._id_12E25 ) && !mayspawn() )
        return;
...
totaldisttracking( var_0 )
{
    ...
    for (;;)
    {
        var_4 = scripts\engine\utility::_id_13AF1( 5.0, "death", "vehicle_enter", "vehicle_change_seat", "vehicle_exit", "afk_tracking_resume" );

        if ( var_4 == "vehicle_exit" && !isdefined( self.lastvehicleseatchangetime ) )
            continue;

        if ( istrue( self.scriptablenousestate ) )
            continue;
...
initleanplayerstats()
{
    ...
    scripts\mp\utility\stats::setuphumanpowers( "deaths" );
    ...
    scripts\mp\utility\stats::setuphumanpowers( "score" );
    ...
    scripts\mp\utility\stats::setuphumanpowers( "kills" );
}
...
initplayerstats()
{
    ...
    if ( !isdefined( self.thermite_damagemodifierignorefunc ) )
        self.thermite_damagemodifierignorefunc = [];
    ...
    scripts\mp\utility\stats::setuphumanpowers( "suicides" );

    if ( !isdefined( self.pers["headshots"] ) )
        scripts\mp\utility\stats::initpersstat( "headshots" );

    scripts\mp\utility\stats::setuphumanpowers( "headshots" );
    ...
    scripts\mp\utility\stats::setuphumanpowers( "assists" );
    ...
    scripts\mp\utility\stats::setuphumanpowers( "captures" );
    ...
    scripts\mp\utility\stats::setuphumanpowers( "returns" );
    ...
    scripts\mp\utility\stats::setuphumanpowers( "defends" );
    ...
    scripts\mp\utility\stats::setuphumanpowers( "plants" );
    ...
    scripts\mp\utility\stats::setuphumanpowers( "defuses" );
    ...
    scripts\mp\utility\stats::setuphumanpowers( "destructions" );
    ...
    scripts\mp\utility\stats::setuphumanpowers( "confirmed" );
    ...
    scripts\mp\utility\stats::setuphumanpowers( "denied" );
    ...
    scripts\mp\utility\stats::setuphumanpowers( "rescues" );
    ...
    scripts\mp\utility\stats::setuphumanpowers( "touchdowns" );
    ...
    scripts\mp\utility\stats::setuphumanpowers( "fieldgoals" );
    ...
    scripts\mp\utility\stats::setuphumanpowers( "killChains" );
    ...
    scripts\mp\utility\stats::setuphumanpowers( "killsAsSurvivor" );
    ...
    scripts\mp\utility\stats::setuphumanpowers( "stabs" );
    ...
    scripts\mp\utility\stats::setuphumanpowers( "setbacks" );
    ...
    scripts\mp\utility\stats::setuphumanpowers( "objTime" );
    ...
    scripts\mp\utility\stats::setuphumanpowers( "gamemodeScore" );
    ...
    scripts\mp\utility\stats::setuphumanpowers( "damage" );

    if ( !isdefined( self.pers["supersEarned"] ) )
        scripts\mp\utility\stats::initpersstat( "supersEarned" );

    scripts\mp\utility\stats::setuphumanpowers( "supersEarned" );
    ...
    scripts\mp\utility\stats::setuphumanpowers( "downs" );
    ...
    scripts\mp\utility\stats::setuphumanpowers( "killStreak" );
    scripts\mp\utility\stats::setuphumanpowers( "loss" );
    scripts\mp\utility\stats::setuphumanpowers( "win" );
}
...
removefromalivecount( var_0, var_1 )
{
    ...
    var_3 = scripts\mp\utility\game::getgametypenumlives() != 0 && var_2 == 0 && !istrue( level._id_12BF6 );
...
updateplayerwindmaterial()
{
    ...
    for (;;)
    {
        foreach ( var_6 in level.players )
        {
            ...
            if ( !isdefined( var_6._id_13CDD ) || var_6._id_13CDD != var_8 )
            {
                switch ( var_8 )
                {
                    case 0:
...
playerdatafield()
{
    return istrue( self.beingrevived ) || istrue( self.spawnvehicles );
}

shouldrecheckafk()
{
    if ( isdefined( level.shouldrecheckafkfunc ) )
        return [[ level.shouldrecheckafkfunc ]]();
