...
init()
{
    ...
    scripts\mp\utility\spawn_event_aggregator::registeronplayerspawncallback( ::spawnboardroom_juggdrop );
}
...
getspawnpoint()
{
    ...
    if ( isdefined( self.selectedspawnarea ) )
    {
        if ( issubstr( self.selectedspawnarea, "squad" ) )
        {
            ...
            if ( isdefined( var_3.vehicle ) )
            {
                ...
                self._id_139AD = var_3.vehicle.vehiclename;
            }
    ...
    if ( istrue( level.usesquadspawn ) && istrue( self.squadspawnconfirmed ) )
    {
        ...
        if ( isdefined( var_14 ) && isdefined( self.squadindex ) && self.team == var_14.team && self.squadindex == var_14.squadindex )
        {
            ...
            var_14 thread scripts\mp\utility\points::quarry_mortar_guys( "squad_spawn" );
            thread scripts\mp\utility\points::quarry_mortar_guys( "squad_spawn_self" );
            self._id_13B6E = 1;
        }
    }
    ...
    for (;;)
    {
        ...
        foreach ( var_3 in level.availablespawnlocations[self.team] )
        {
            ...
            if ( isdefined( var_6.objectivekey ) )
                var_8 = istrue( var_6.select_bunker_interior_three_spawners );
            else if ( issubstr( var_3, "vehicle" ) )
            {
                ...
                {
                    var_8 = istrue( var_11.select_bunker_interior_three_spawners );
                    var_9 = remove_from_spawnflags( var_11, self );
...
{
    return level.objectives[var_0].ownerteam != var_1 || level.objectives[var_0].stalemate || level.objectives[var_0].captureblocked || istrue( level.objectives[var_0]._id_12E68 ) || level.objectives[var_0].claimteam != "none" && level.objectives[var_0].claimteam != var_1;
}

ss_ontimerexpired( var_0 )
{
    var_1 = gettime();
...
{
    var_0 endon( "death" );
    var_0.select_bunker_interior_three_spawners = 0;
    var_1 = getdvarfloat( "scr_gw_apc_ignore_damage_health_pct", 0.15 );

    for (;;)
    {
        var_2 = gettime();
        var_3 = istrue( var_0.end_loop_emp_spark_vfx );
        var_4 = scripts\cp_mp\utility\weapon_utility::islockedonto( var_0 );
        var_5 = scripts\cp_mp\vehicles\vehicle_occupancy::_id_13915( var_0 );
        ...
        var_14 = var_0.select_bunker_interior_three_spawners;

        if ( var_6 || var_8 || var_9 || var_3 || var_4 || !var_5 )
            var_0.select_bunker_interior_three_spawners = 1;
        else
            var_0.select_bunker_interior_three_spawners = 0;
...
spawnselectionthink( var_0 )
{
    ...
    if ( istrue( level._id_12B94 ) )
        thread scripts\mp\gametypes\arm.gsc::_id_1395C();

    if ( istrue( level._id_12B92 ) )
        thread scripts\mp\gametypes\arm.gsc::_id_1395D();
...
watchforafk()
{
    ...
    if ( isdefined( level._id_12E7E ) )
        wait( level._id_12E7E );
    else
        wait 60.0;
    ...
    for (;;)
    {
        ...
        if ( var_0 == "tac_ops_spawn_area_selected" )
        {
            ...
            foreach ( var_4 in level.spawnselectionlocations )
            {
                ...
                {
                    _id_1378C( var_4[self.team].typeref, self, self.team );

                    if ( isdefined( var_4[self.team].objectivekey ) && isobjectiveindanger( var_4[self.team].objectivekey, self.team ) )
                        break;

                    if ( isdefined( var_4[self.team].typeref ) && issubstr( var_4[self.team].typeref, "vehicle" ) && istrue( var_4[self.team].dynamicent.select_bunker_interior_three_spawners ) )
                        break;
            ...
            if ( !isdefined( var_2 ) )
            {
                var_6 = postshipmodifychevrons( self.team, self.squadindex );
                ...
                foreach ( var_4 in var_6 )
                {
                    if ( isdefined( var_4.dynamicent ) && var_4.dynamicent getentitynumber() == var_1 )
                    {
                        ...
                        var_9 = spawnstruct();
                        var_9 = scripts\mp\spawnscoring::ffsm_onground_stateenter( var_9, var_4.dynamicent );

                        if ( !isdefined( var_9._id_12E6C ) )
                        {
                            var_4.dynamicent.watchforhelistatus = gettime();
                            break;
                        }
...
_id_1377C( var_0 )
{
    if ( !isdefined( self.recordedgameendstats ) )
        self.recordedgameendstats = [];

    var_1 = 65536;
    var_2 = 131072;
    var_3 = 262144;
    var_4 = 524288;
    self.recordedgameendstats["rightmouseup"] = istrue( self.recordedgameendstats["rightmouseup"] ) || ( var_0 & var_1 ) > 0;
    self.recordedgameendstats["leftmouseup"] = istrue( self.recordedgameendstats["leftmouseup"] ) || ( var_0 & var_2 ) > 0;
    self.recordedgameendstats["activate"] = istrue( self.recordedgameendstats["activate"] ) || ( var_0 & var_3 ) > 0;
    self.recordedgameendstats["gostand"] = istrue( self.recordedgameendstats["gostand"] ) || ( var_0 & var_4 ) > 0;
...
watchendconditions()
{
    ...
    var_0 = scripts\engine\utility::_id_13AEC( "tac_ops_map_selection_valid", "tac_ops_map_changed", "tac_ops_map_cleared", "spawned_player", "tac_ops_map_game_ended", "tac_ops_map_nuke" );
...
endcondition_focuschanged( var_0 )
{
    ...
    foreach ( var_3 in level.spawnselectionlocations )
    {
        ...
        {
            thread staticcircle( var_3[self.team].anchorentity.origin );
    ...
    if ( !isdefined( var_1 ) )
    {
        var_5 = postshipmodifychevrons( self.team, self.squadindex );

        foreach ( var_3 in var_5 )
        {
            if ( isdefined( var_3.dynamicent ) && var_3.dynamicent getentitynumber() == var_0 )
            {
                thread staticcircle( var_3.dynamicent.origin );
                var_1 = var_3.typeref;
                break;
            }
        }
...
staticcircle( var_0 )
{
    ...
    if ( !isdefined( level._id_13CF6 ) )
        level._id_13CF6 = max( getdvarint( "NOLPSOTORP", 20000 ) * 0.5 / 1000, 1.0 );

    for (;;)
    {
        self predictstreampos( var_0, 1 );
        wait( level._id_13CF6 );
    }
}
...
evaluatesquadspawn()
{
    ...
    for ( var_4 = 0; var_4 < 4; var_4++ )
    {
        ...
        var_5.scriptable_enginedamaged = undefined;
        var_3[var_4] = var_5;
    }
    ...
    for (;;)
    {
        ...
        var_11 = postshipmodifychevrons( self.team, self.squadindex );
        ...
        for ( var_4 = 0; var_4 < 4; var_4++ )
        {
            ...
            {
                var_15 = istrue( var_9.select_bunker_interior_three_spawners );
                var_16 = var_1[var_4];
                ...
                if ( !isdefined( var_3[var_4].scriptable_enginedamaged ) || var_3[var_4].scriptable_enginedamaged != var_16 )
                {
                    var_3[var_4].scriptable_enginedamaged = var_16;
            ...
            if ( isdefined( var_3[var_4].scriptable_enginedamaged ) && var_3[var_4].scriptable_enginedamaged != -1 )
            {
                var_3[var_4].scriptable_enginedamaged = -1;
...
handlesquadspawncycle()
{
    ...
    for (;;)
    {
        var_0 = scripts\engine\utility::_id_13AE1( "cyclePos", "cycleNeg" );
...
spawnboardroom_juggdrop()
{
    ...
    if ( getdvarint( "scr_squad_spawn_debug", 0 ) == 1 )
        thread scripts\mp\spawnscoring::_id_12F22();
}
...
showmaphinttilused()
{
    ...
    for (;;)
    {
        if ( issquadspawnable( self ) )
            self.select_bunker_interior_three_spawners = issquadmateindanger( self );
...
issquadmateindanger( var_0 )
{
    ...
    if ( isdefined( var_0.teamanchoredentomnvars ) && var_0.teamanchoredentomnvars + 3000 > var_1 )
        return 1;
    ...
    {
        if ( istrue( var_0.vehicle.end_loop_emp_spark_vfx ) )
            return 1;
    ...
    if ( isdefined( var_0 ) && isdefined( var_0.watchforhelistatus ) && var_0.watchforhelistatus + 1000 > var_1 )
        return 1;
...
timeuntilspawnmessaging( var_0, var_1 )
{
    ...
    if ( var_0 > 0 )
    {
        ...
        scripts\engine\utility::_id_13AF3( var_0, "force_spawn" );
    }
}
...
getclosestavailablespawnlocation()
{
    ...
    else
    {
        ...
        {
            var_7 = postshipmodifychevrons( self.team, self.squadindex );
        ...
        foreach ( var_1 in level.teamnamelist )
        {
            foreach ( var_3 in level.availablespawnlocations[var_1] )
            {
                ...
                if ( isdefined( var_4.objectivekey ) )
                    var_4.select_bunker_interior_three_spawners = isobjectiveindanger( var_4.objectivekey, var_1 );
            }
        }
        ...
        foreach ( var_1 in level.teamnamelist )
        {
            ...
            foreach ( var_4 in level.availablespawnlocations[var_1] )
            {
                if ( !issubstr( var_4, "HQ" ) && issubstr( var_4, "fob" ) )
                {
                    ...
                    level.objectives[var_17]._id_12E68 = var_14;
                    level.objectives[var_17]._id_12E69 = gettime();
                }
            }
        }
...
_id_1378C( var_0, var_1, var_2 )
{
    var_3 = level.spawnselectionlocations[var_0][var_2].objectivekey;

    if ( !isdefined( var_3 ) || !isdefined( level.objectives[var_3] ) || !isdefined( level.objectives[var_3]._id_12E69 ) || level.objectives[var_3]._id_12E69 == gettime() )
        return;
    ...
    level.objectives[var_3]._id_12E68 = var_13;
    level.objectives[var_3]._id_12E69 = gettime();
}
