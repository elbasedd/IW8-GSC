...
setplayerscoreboardinfo()
{
    ...
    if ( var_0 < 200 )
    {
        ...
        if ( scripts\mp\utility\game::getgametype() == "br" )
        {
            var_12 = scripts\mp\gametypes\br.gsc::enemygunship_spawngunship( self );
            ...
            {
                var_15 = scripts\mp\gamescore::prematchdialog();
                var_16 = var_15[self.team];
                setclientmatchdata( "players", self.clientmatchdataid, "placement", var_16 );
                var_17 = scripts\mp\gametypes\br_gametype_dmz.gsc::_id_11ACD();
                setclientmatchdata( "players", self.clientmatchdataid, "extrascore4", var_17 );
                var_18 = 0;

                if ( isdefined( self.try_play_tv_station_intro_sequence ) )
                    var_18 = self.try_play_tv_station_intro_sequence;

                setclientmatchdata( "players", self.clientmatchdataid, "extrascore5", var_18 );
            }
            else if ( scripts\mp\utility\game::postupdategameevents() == "kingslayer" )
            {
                var_15 = scripts\mp\gamescore::prematchdialog();
                var_16 = var_15[self.team];
                setclientmatchdata( "players", self.clientmatchdataid, "placement", var_16 );
                var_17 = scripts\mp\gametypes\br_gametype_kingslayer.gsc::_id_11ACD();
                setclientmatchdata( "players", self.clientmatchdataid, "extrascore4", var_17 );
                var_18 = 0;
            ...
            else if ( scripts\mp\utility\game::postupdategameevents() == "rebirth" )
            {
                if ( isdefined( self._id_1326D ) )
                    setclientmatchdata( "players", self.clientmatchdataid, "placement", self._id_1326D );

                var_17 = scripts\mp\gametypes\br_gametype_rebirth.gsc::directimpactkill();
                setclientmatchdata( "players", self.clientmatchdataid, "extrascore4", var_17 );
            }
            else if ( scripts\mp\utility\game::postupdategameevents() == "rebirth_reverse" )
            {

            }
            else if ( isdefined( self._id_1326D ) )
                setclientmatchdata( "players", self.clientmatchdataid, "placement", self._id_1326D );
        }
        ...
        foreach ( var_3 in level.teamnamelist )
        {
            if ( scripts\mp\menus::shouldmodesetsquads() )
            {
                foreach ( var_5 in level.squaddata[var_3] )
                {
                    ...
                    {
                        scripts\mp\gamelogic::bunkerinteriorkeypads( var_8 );
