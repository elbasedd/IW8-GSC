...
main( var_0 )
{
    ...
    foreach ( var_3 in var_1 )
    {
        ...
        if ( !scripts\mp\spawnlogic::generatinglosdata() )
        {
            ...
            if ( isdefined( var_4 ) )
            {
                if ( level.unset_relic_doubletap == 1 )
                {
                    if ( var_4 == "10v10" )
                        continue;
                    else if ( var_4 == "6v6" )
...
onjuggernaut()
{
    ...
    {
        self.carryobject thread setdropped();
        self switchtoweapon( scripts\mp\juggernaut::start_safehouse_gunshop() );
    }
}
...
_id_123B7( var_0 )
{
    self._id_11C4C[self._id_11C4C.size] = var_0;
}
...
gas_at_computer( var_0 )
{
    ...
    foreach ( var_3 in self._id_11C4C )
...
carryobjectusethink()
{
    ...
    for (;;)
    {
        ...
        if ( !spawn_little_bird_mg_at_location() && var_0 scripts\mp\utility\weapon::grenadeinpullback() )
        {
...
setpickedup( var_0, var_1, var_2 )
{
    ...
    if ( isdefined( var_0.carryobject ) || isdefined( self.carryweapon ) && !var_0 scripts\common\utility::is_weapon_allowed() || !gas_at_computer( var_0 ) )
    {
...
updatecarryobjectorigin()
{
    ...
    for (;;)
    {
        ...
        if ( isdefined( self.carrier ) )
        {
            ...
            scripts\engine\utility::_id_13AF4( self.objpingdelay, "dropped", "reset" );
            continue;
        }
...
{
    if ( istrue( var_1.sq_ontimerexpired ) )
        var_7 = var_1;
    else
        var_7 = spawnstruct();
...
useobjectusethink()
{
    ...
    for (;;)
    {
        ...
        if ( isdefined( level.vehicle_is_ambient ) )
        {
            if ( ![[ level.vehicle_is_ambient ]]( var_0 ) )
                continue;
        }
...
getearliestclaimplayer()
{
    ...
    if ( isdefined( self.onnewequipmentpickup ) && istrue( self.gasfxair ) )
    {
        if ( self.onnewequipmentpickup[var_0].size > 0 )
        {
            var_2 = undefined;
            var_3 = getarraykeys( self.onnewequipmentpickup[var_0] );

            for ( var_4 = 0; var_4 < var_3.size; var_4++ )
            {
                var_5 = self.onnewequipmentpickup[var_0][var_3[var_4]];
...
applycaptureprogressanduseupdate()
{
    if ( isdefined( self.progressapproval ) )
    {
        var_0 = self [[ self.progressapproval ]]( 1 );
    ...
    if ( isdefined( self.onprogressupdate ) )
        self [[ self.onprogressupdate ]]( 1 );
}

stompenemyteamprogress( var_0 )
{
    if ( isdefined( self._id_13081 ) )
        self [[ self._id_13081 ]]( var_0 );
...
useobjectdecay( var_0 )
{
    ...
    for (;;)
    {
        ...
        if ( self.claimteam == "none" || istrue( self.forcedecaying ) )
        {
            if ( self.usetime )
            {
                if ( !self.stalemate )
                {
                    ...
                    else
                    {
                        ...
                        self.curprogress = self.curprogress - 0.1 * self.decayrate * level.frameduration;

                        if ( isdefined( self.onprogressupdate ) )
                            self [[ self.onprogressupdate ]]( 0 );
                    }
                }
...
proxtriggerthink()
{
    ...
    for (;;)
    {
        ...
        if ( istrue( self.spawn_carriables_from_prefabs_all ) && ( var_1 scripts\mp\utility\player::isusingremote() || isdefined( var_1.spawningafterremotedeath ) ) )
            continue;

        if ( istrue( var_1.inlaststand ) && !istrue( self.trigger.botisonplayerteam ) )
            continue;
...
triggertouchthink( var_0 )
{
    ...
    while ( scripts\mp\utility\player::isreallyalive( self ) && isdefined( var_0.trigger ) && self istouching( var_0.trigger ) && !level.gameended )
    {
        ...
        if ( istrue( self.inlaststand ) && !istrue( var_0.trigger.botisonplayerteam ) )
            break;
        ...
        if ( istrue( var_0.get_combat_action ) && isdefined( var_0.usecondition ) && !var_0 [[ var_0.usecondition ]]( self ) )
            break;
            ...
            if ( hasdomflags() && isdefined( var_0.id ) && ( var_0.id == "domFlag" || var_0.id == "hardpoint" || var_0.id == "bomb_site" || var_0.id == "rugby_jugg" ) )
            {
                ...
                else if ( var_1 && isdefined( self.ui_dom_securing ) && ( isdefined( var_0.stalemate ) && !var_0.stalemate && !istrue( var_0.majoritycapprogress ) ) && var_0.ownerteam != self.team )
                {
                    ...
                    if ( scripts\mp\utility\game::getgametype() == "hq" )
                    {
                        ...
                        self setclientomnvar( "ui_objective_state", var_0 scripts\mp\gametypes\obj_dom.gsc::playerparachutedetachresetomnvars() );
                    else
                        self setclientomnvar( "ui_objective_state", 1 );
                }
                else
                {
                    ...
                    else if ( var_1 && istrue( var_0.majoritycapprogress ) && isdefined( var_0.lastprogressteam ) && var_0.lastprogressteam == var_0.claimteam )
                    {
                        ...
                        {
                            if ( istrue( level.holidayhuntactive ) )
                            {
...
updateuisecuring( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    if ( var_1 )
    {
        ...
        if ( scripts\mp\utility\game::getgametype() == "br" && var_3.id == "laststand_reviver" )
        {
            ...
            scripts\mp\gametypes\br.gsc::_id_1378E( var_6, self, var_0 );
        }
    }
    else
    {
    ...
    else if ( !var_1 )
    {
        if ( isdefined( self.trigger ) && self.trigger isrevivetrigger() || isdefined( self.id ) && self.id == "rugby_jugg" )
        {
            if ( !var_0 scripts\mp\utility\perk::_hasperk( "specialty_revive_use_weapon" ) )
            {
                ...
                {
                    var_0 thread obj_room_fire_04( 1 );
        ...
        if ( !isdefined( var_2 ) && !var_1 )
        {
            if ( isdefined( self.trigger ) && self.trigger isrevivetrigger() || isdefined( self.id ) && self.id == "rugby_jugg" )
            {
                if ( istrue( var_0.weaponsdisabledwhilereviving ) )
                {
                    ...
                    if ( !level.allowreviveweapons )
                    {
                        var_0 thread obj_room_fire_04( 0 );
...
usetest( var_0, var_1, var_2, var_3 )
{
    ...
    if ( isdefined( self.trigger ) && isdefined( self.trigger._id_137F4 ) && distance2dsquared( self.trigger.origin, var_0.origin ) >= self.trigger._id_137F4 )
        return 0;
    ...
    if ( istrue( self.get_combat_action ) && isdefined( self.usecondition ) && !self [[ self.usecondition ]]( var_0 ) )
        return 0;
...
_id_129B5( var_0, var_1, var_2, var_3, var_4 )
{
        ...
        if ( var_1 != -1 )
        {
            if ( level.codcasterenabled )
            {
                if ( isdefined( self.compassicons["codcaster"] ) )
                {
                    ...
                    scripts\mp\objidpoolmanager::_id_1371B( var_1, var_12 );
                }
                else
                    scripts\mp\objidpoolmanager::_id_1371C( var_1 );
            }
            ...
            if ( !var_7 )
            {
                ...
                if ( level.codcasterenabled && isdefined( self.compassicons["codcaster"] ) )
                {
                    ...
                    scripts\mp\objidpoolmanager::_id_1371E( var_1, var_20 );
                }
