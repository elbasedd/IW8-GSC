...
init()
{
    ...
    level.laststandhealth = scripts\mp\utility\dvars::playerzombiethermalupdate( "scr_" + scripts\mp\utility\game::getgametype() + "_lastStandHealth", "scr_player_lastStandHealth" );
    level.laststandrevivehealth = scripts\mp\utility\dvars::playerzombiethermalupdate( "scr_" + scripts\mp\utility\game::getgametype() + "_lastStandReviveHealth", "scr_player_lastStandReviveHealth" );
    ...
    level.laststandinvulntime = scripts\mp\utility\dvars::playerzombiethermalcleanup( "scr_" + scripts\mp\utility\game::getgametype() + "_lastStandInvulnTime", "scr_player_lastStandInvulnTime" );
    level.targetkiosk = scripts\mp\utility\dvars::playerzombiethermalcleanup( "scr_" + scripts\mp\utility\game::getgametype() + "_lastStandReviveDecayScale", "scr_player_lastStandReviveDecayScale" );
    level.laststandrevivetimer = scripts\mp\utility\dvars::playerzombiethermalcleanup( "scr_" + scripts\mp\utility\game::getgametype() + "_lastStandReviveTimer", "scr_player_lastStandReviveTimer" );
    level.laststandsuicidetimer = scripts\mp\utility\dvars::playerzombiethermalcleanup( "scr_" + scripts\mp\utility\game::getgametype() + "_lastStandSuicideTimer", "scr_player_lastStandSuicideTimer" );
    level.laststandtimer = scripts\mp\utility\dvars::playerzombiethermalcleanup( "scr_" + scripts\mp\utility\game::getgametype() + "_lastStandTimer", "scr_player_lastStandTimer" );
    level.targetmarkergroup_watchmarkonspawn = scripts\mp\utility\dvars::playerzombiethermal( "scr_" + scripts\mp\utility\game::getgametype() + "_lastStandWeapon", "scr_player_lastStandWeapon" );
    level.targetoutsidegoal = scripts\mp\utility\dvars::playerzombiethermalcleanup( "scr_" + scripts\mp\utility\game::getgametype() + "_lastStandWeaponDelay", "scr_player_lastStandWeaponDelay" );
...
laststandthink()
{
    ...
    onenter();
    var_0 = scripts\engine\utility::_id_13AEC( "last_stand_heal_success", "last_stand_revived", "last_stand_bleedout", "death", "disconnect", "last_stand_self_revive" );
...
onenter()
{
    ...
    if ( level.gametype == "br" )
        thread tacinsert_getspawnposition();
    ...
    self.inlaststand = 1;
    self._id_11E9D = undefined;
    self.hasshownlaststandicon = 0;

    if ( level.gametype == "br" && !istrue( self.remove_flag_trig ) )
    {
        if ( isdefined( level.gas_trigger_think ) )
            [[ level.gas_trigger_think ]]();
    }
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" )
    {
        ...
        {
            self.target_play_death_anim = gettime();
            self.freight_lift_attacker_internal = undefined;
            scripts\mp\gametypes\br_gametypes.gsc::_id_1268B( "onLastStandEnter" );

            if ( !istrue( level._id_12BF1 ) )
            {
                var_1 = scripts\mp\gametypes\br_public.gsc::post_safeges_weapon( self.team, self.squadindex );
    ...
    var_5 = level.targetmarkergroup_watchmarkonspawn;
    ...
    else
    {
        ...
        if ( isdefined( level.tomastrike_getrandombombingpoint ) )
            self thread [[ level.tomastrike_getrandombombingpoint ]]();
        ...
        thread relic_amped_is_player_valid_to_explode( var_5 );

        if ( isdefined( level.ascender_deathwatcher ) )
            self [[ level.ascender_deathwatcher ]]();
        else
            addoverheadicon();
        ...
        if ( isdefined( self.target_group ) && self.target_group scripts\mp\utility\perk::_hasperk( "specialty_reduce_regen_delay_on_kill" ) )
            self.target_group scripts\mp\perks\perkfunctions::regendelayreduce_onkill();
        ...
        if ( isdefined( self.pers["squadMemberIndex"] ) )
        {
            ...
            self.targetedby = scripts\mp\utility\outline::outlineenableforteam( self, self.team, var_7, "laststand" );
        }
...
{
    if ( !istrue( self.increment_timer ) && !istrue( self.islaststandskippingenabled ) )
    {
...
relic_amped_is_player_valid_to_explode( var_0 )
{
    self endon( "death_or_disconnect" );
    self endon( "last_stand_finished" );
    level endon( "game_ended" );

    while ( isdefined( self.currentweapon ) && isdefined( self.currentweapon.basename ) && ( self.currentweapon.basename == "armor_plate_deploy_mp" || self.currentweapon.basename == getteammatereviveweapon() ) )
        waitframe();
...
disableweaponsovertime( var_0 )
{
    ...
    scripts\engine\utility::_id_13AED( var_0, "death_or_disconnect" );
...
dodamagewhiledown()
{
    ...
    if ( !istrue( level.targetmarkergroup_clearcacheonspawn ) )
        thread suicidesetup();
...
_id_12853()
{
    ...
    for (;;)
    {
        if ( _id_12851( var_1 ) && !istrue( var_1.spawnvehicles ) && var_1 isonground() )
        {
            var_0 notify( "self_revive_start" );
            var_1 setlaststandselfreviving( 1 );
            var_0 thread _id_12852();
        }
...
_id_12852()
{
    ...
    var_1._id_13ABE = 0;
    var_1 thread _id_12850();

    while ( scripts\mp\utility\player::isreallyalive( var_0 ) && _id_12851( var_0 ) && var_1.curprogress < var_1.usetime )
    {
        while ( !var_0 isonground() && _id_12851( var_0 ) && !istrue( var_0.spawnvehicles ) )
            waitframe();
        ...
        if ( !istrue( var_0.spawnvehicles ) )
        {
            var_0 thread scripts\mp\gametypes\br_public.gsc::_id_11EF7( "iw8_ges_plyr_self_revive", 10 );

            if ( !istrue( var_0._id_1307C ) )
            {
                var_0 attach( "wm_equip_gen_stim_motau_ch3", "tag_accessory_left" );
                var_0._id_1307C = 1;
            }

            var_0.spawnvehicles = 1;
        ...
        if ( var_1.curprogress >= var_1.usetime )
        {
            ...
            if ( scripts\mp\utility\game::getgametype() == "br" )
                scripts\mp\gametypes\br_analytics.gsc::damage_from_above( var_0 );

            var_0 mp_layover_patch( "self_revive_success", var_0 );

            if ( isdefined( level._id_124BC ) )
                var_0 [[ level._id_124BC ]]();
    ...
    if ( !istrue( var_0.beingrevived ) )
    {
        scripts\mp\gametypes\br.gsc::_id_1378E( var_0, var_0, var_1.curprogress, 1 );
...
_id_12850()
{
    ...
    var_0 scripts\engine\utility::_id_13AE9( "last_stand_finished", "stopped_self_revive", "death_or_disconnect" );
...
onexitcommon( var_0 )
{
    ...
    var_1 = level.targetmarkergroup_watchmarkonspawn;
    ...
    if ( istrue( scripts\mp\playerlogic::playerdatafield() ) )
        scripts\mp\playerlogic::_id_12963( 0 );

    if ( level.gametype == "br" )
    {
        scripts\mp\gametypes\br.gsc::_id_1378E( self, self, 0, 1 );
    ...
    if ( isdefined( level.vehicle_deregister_on_death ) )
        self [[ level.vehicle_deregister_on_death ]]( var_0 );
...
onrevive( var_0, var_1 )
{
    ...
    scripts\common\utility::bot_loadout_team( 1 );

    if ( getdvar( "scr_br_gametype", "" ) == "dmz" || getdvar( "scr_br_gametype", "" ) == "risk" )
        scripts\mp\gametypes\br_plunder.gsc::_id_12058( self, 1, 1 );
    ...
    var_3 = level.targetmarkergroup_watchmarkonspawn;
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" && !istrue( self.gulag ) )
    {
        ...
        if ( !istrue( level._id_12BF1 ) )
        {
            var_5 = scripts\mp\gametypes\br_public.gsc::post_safeges_weapon( self.team, self.squadindex );
    ...
    self.target_remap_key_value_pairs = undefined;
...
ondeath( var_0 )
{
    ...
    if ( isdefined( var_0 ) && isdefined( self.target_group ) && istrue( self.target_group.inlaststand ) )
        self.target_group thread _id_1198E( var_0, self.targetdata, self.targetoverride );

    if ( istrue( self.spawnvehicles ) )
    ...
    if ( isdefined( level.gas_triggers_init ) )
        [[ level.gas_triggers_init ]]( self, 1 );
...
revivesetup( var_0 )
{
    ...
    if ( var_0 scripts\mp\gametypes\br_public.gsc::remove_flag_trig() )
        var_0.laststandreviveent _id_12853();
}
...
_id_127F6()
{
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" )
        scripts\mp\gametypes\br_analytics.gsc::damage_feedback_watch( var_0, var_11, var_2, var_5, var_6, var_7, var_8, var_12, var_13, var_14, var_16 );

    var_0 mp_layover_patch( "self_revive_on_kill_success", var_0 );
...
{
    self endon( "death" );
    self.owner scripts\engine\utility::_id_13ADA( "death_or_disconnect", "last_stand_finished", "last_stand_heal_active" );
    self delete();
}
...
revivetriggerthink( var_0 )
{
    ...
    for (;;)
    {
        ...
        if ( istrue( var_1.shouldpickup ) )
        {
            ...
            while ( istrue( var_1.shouldpickup ) && var_1 usebuttonpressed() )
                waitframe();
        }
        ...
        self.owner scripts\mp\playerlogic::_id_12963( 1 );
        var_2 = 0;
        ...
        var_1 thread _id_13199( self.owner );

        if ( level.gametype == "br" )
            self.owner scripts\mp\gametypes\br_public.gsc::_id_129CF( 1 );

        var_3 = scripts\engine\utility::_id_13AE9( "use_hold_revive_success", "use_hold_revive_fail", "death_or_disconnect" );

        if ( var_3 == "use_hold_revive_success" )
            var_2 = 1;

        var_1 notify( "finish_buddy_reviving" );
        self.owner scripts\mp\playerlogic::_id_12963( 0 );
        var_2 = self.owner mp_layover_patch( var_3, var_1 );
...
getteammatereviveweapon()
{
    if ( scripts\cp_mp\utility\game_utility::useww2modelswaps() )
...
_id_13199( var_0 )
{
    ...
    if ( istrue( var_1._id_13ABF ) )
        return;

    var_2 = getcompleteweaponname( getteammatereviveweapon() );
    var_3 = var_1 scripts\cp_mp\utility\killstreak_utility::createstreakinfo( "", var_1 );
    var_3._id_125A8 = var_2;

    for ( var_4 = 0; !var_4 || istrue( var_1.chopper_sound_fade_and_delete ); var_1._id_13ABF = 1 )
    {
        var_4 = var_1 scripts\cp_mp\killstreaks\killstreakdeploy::switchtodeployweapon( var_2, var_3, ::_id_13269, undefined, ::_id_119DC, undefined, ::_id_119DD, 0 );
        waitframe();
    }

    var_1.chopper_sound_fade_and_delete = 1;
    var_1._id_13ABF = 0;
    var_1 thread _id_13BDA();
    var_1 thread _id_13BF7( var_0 );
    var_1 thread _id_13BF8( var_0 );
}

_id_119DC( var_0, var_1 )
{
    var_2 = self;
    var_2 disableweaponswitch();
}

_id_119DD( var_0, var_1 )
{
    var_2 = self;
    var_2 enableweaponswitch();
    var_2 notify( "revive_stim_finished" );

    while ( isdefined( self.currentweapon ) && isdefined( self.currentweapon.basename ) && self.currentweapon.basename == getteammatereviveweapon() )
        waitframe();

    waitframe();
    var_2.chopper_sound_fade_and_delete = 0;
}
...
_id_13269( var_0 )
{
    ...
    if ( !istrue( var_1._id_125A9 ) )
        return;

    var_1 scripts\engine\utility::_id_13AE9( "death_or_disconnect", "finish_buddy_reviving", "last_stand_start" );
}
...
mp_layover_patch( var_0, var_1 )
{
    ...
    if ( level.gametype == "br" )
    {
        ...
        var_2 scripts\mp\gametypes\br_public.gsc::_id_129CF( 0 );
        var_2 scripts\mp\gametypes\br_public.gsc::_id_129D9( 0 );
    }
    ...
    if ( var_3 )
    {
        ...
        if ( level.gametype == "br" )
        {
            if ( !istrue( level.gameended ) )
            {
                var_2 scripts\mp\gametypes\br_public.gsc::_id_129D1( 0 );
                scripts\mp\events::revivedplayer( var_1, var_2 );
                scripts\cp\utility\cp_controlled_callbacks::_id_1198F( var_1, var_2 );
                var_2 scripts\cp\utility\cp_controlled_callbacks::_id_13012( "alive_not_downed" );

                if ( var_1 != var_2 )
                    var_1 scripts\mp\utility\stats::incpersstat( "rescues", 1 );

                var_2 scripts\mp\gametypes\br_gametypes.gsc::_id_1268B( "onLastStandRevive", var_1 );
                var_2.increment_timer = 0;
                var_2 stoplocalsound( "deaths_door_in" );

                if ( !istrue( var_2.current_safehouse_spawn_structs ) )
                {
            ...
            if ( !scripts\mp\gametypes\br_public.gsc::spreadshotdamagemod() && !istrue( var_2.current_safehouse_spawn_structs ) )
...
useholdthinkcleanup( var_0, var_1 )
{
    ...
    var_3 = self.owner;
    var_4 = var_3 scripts\engine\utility::_id_13AEA( "death_or_disconnect", "use_hold_think_success", "use_hold_think_fail", "last_stand_finished" );
    self.inuse = 0;
    var_1 delete();
...
suicidesetup()
{
    ...
    for (;;)
    {
        ...
        if ( self stancebuttonpressed() && self isinexecutionvictim() == 0 && !scripts\mp\playerlogic::playerdatafield() )
        {
...
_id_13131()
{
    ...
    while ( !var_4 && !var_1 )
    {
        ...
        var_5 = scripts\engine\utility::_id_13AF1( 0.5, "stance_pressed_down", "stance_pressed_up", "last_stand_finished", "last_stand_self_revive", "last_stand_bleedout" );
...
handlerevivemessage()
{
    ...
    for (;;)
    {
        ...
        for (;;)
        {
            var_2 = scripts\engine\utility::_id_13AE8( "super_ready", "handle_revive_message" );
...
laststandmonitor()
{
    ...
    if ( istrue( level.targetbrokelos ) )
        return;
