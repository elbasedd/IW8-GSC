...
roundend_checkscorelimit( var_0, var_1 )
{
    ...
    else if ( level.roundscorelimit > 0 )
    {
        ...
        {
            if ( level scripts\mp\utility\game::stadium_two_death_func( var_8 ) )
                continue;
...
forceend( var_0 )
{
    ...
    if ( level.teambased )
    {
        ...
        {
            if ( level scripts\mp\utility\game::stadium_two_death_func( var_2 ) )
                continue;
...
onscorelimit( var_0 )
{
    ...
    if ( level.teambased )
    {
        var_2 = scripts\mp\gamescore::enter_combat_maze_ai();
    ...
    else
    {
        ...
        {
            level.vehicle_getturrets = var_2;
...
updategameevents()
{
    ...
    if ( level.teambased )
    {
        ...
        if ( !var_17 && !var_20 )
        {
            ...
            if ( _id_12B2D() && game["roundsPlayed"] == 0 )
...
waitforplayers( var_0 )
{
    ...
    for (;;)
    {
        ...
        foreach ( var_12 in level.teamnamelist )
        {
            if ( !get_closest_attackable_player( var_12 ) )
            {
...
get_closest_attackable_player( var_0 )
{
    var_1 = 0;
...
prematchperiod()
{
    ...
    if ( isdefined( level._id_1212B ) && [[ level._id_1212B ]]() )
        return;
    ...
    if ( getdvarint( "scr_live_lobby", 0 ) == 1 && !istrue( level._id_12BF7 ) )
    {
        ...
        if ( scripts\mp\utility\game::getgametype() == "br" )
        {
            ...
            level thread scripts\mp\gametypes\br_public.gsc::create_vault_assault_loadout_selection();
        ...
        if ( !istrue( level.botdisablekillstreaks ) )
            game["blockJIP"] = 1;
    }
    else if ( !istrue( level._id_12BF7 ) )
    {
        if ( scripts\mp\utility\game::getgametype() == "br" )
        {
            ...
            if ( var_2 != 0 )
            {
                level thread _id_13BE5();
                level thread disallow_player_mantles();
                level waittill( "start_prematch" );
            }
...
updatematchbonusscores( var_0 )
{
    ...
    if ( level.teambased )
    {
        ...
        foreach ( var_3 in level.players )
        {
            ...
            var_3 enter_combat_callback( var_1 );
        }
    }
    else
    {
...
givematchbonus( var_0, var_1 )
{
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" && scripts\mp\utility\game::postupdategameevents() != "dmz" && scripts\mp\utility\game::postupdategameevents() != "risk" && scripts\mp\utility\game::postupdategameevents() != "kingslayer" && scripts\mp\utility\game::postupdategameevents() != "rumble" )
        return;
...
enter_combat_callback( var_0 )
{
    if ( istrue( self.pers["ignoreWeaponMatchBonus"] ) || !isdefined( self.pers["killsPerWeapon"] ) )
        return;

    var_1 = scripts\mp\weaponrank::playerpowerscleanuppowers() / 60;
    var_2 = var_0 / 60;
...
ai_weapons_free( var_0, var_1 )
{
    ...
    else if ( scripts\mp\utility\game::spotlight_tag() )
    {
        var_3 = 1000 - var_1;
...
checktimelimit( var_0 )
{
    ...
    if ( var_1 > 0 && !isdefined( level.obj_ow_atvs_spawned ) )
    {
        if ( var_2 )
        {
            if ( istrue( level._id_1295D ) )
    ...
    if ( var_2 )
    {
        ...
        if ( level.currenttimelimitdelay < level.ontimelimitgraceperiod )
        {
            if ( istrue( level._id_1295D ) )
            {
                if ( !isdefined( level.obj_overwatch_lmgs_remaining ) )
...
runjiprules()
{
    ...
    if ( !level.nojip )
    {
        if ( scripts\mp\utility\game::isroundbased() && !level.nojip )
        {
            switch ( scripts\mp\utility\game::getgametype() )
            {
                ...
                case "dd":
                    ...
                    break;
                case "br":
                    var_0 = gametypeoverridedomsearchparams();
                    break;
                default:
        ...
        else if ( !istrue( level.botdisablekillstreaks ) && scripts\mp\utility\game::gettimepassedpercentage() > level.timepercentagecutoff )
        {
            setnojiptime( 1, 1 );
            level.nojip = 1;
        }
...
{
    return !scripts\mp\gametypes\br_gametypes.gsc::spawn_juggernauts_hangar( "allowLateJoiners" );
}
...
checkteamscorelimitsoon( var_0 )
{
    ...
    if ( !level.playedmatchendingsoon && scripts\mp\utility\game::canplayhalfwayvo() )
    {
        if ( getteamscore( var_0 ) >= int( level.scorelimit * level.currentround - level.scorelimit / 2 ) )
        {
            ...
            thread scripts\mp\music_and_dialog::validateprojectileent( var_0 );
...
checkscorelimit( var_0 )
{
    ...
    if ( level.teambased )
    {
        ...
        {
            if ( level scripts\mp\utility\game::stadium_two_death_func( level.teamnamelist[var_2] ) )
                continue;
    ...
    else
    {
        ...
        if ( self.score < level.roundscorelimit && !istrue( level._id_12777 ) )
            return 0;
    }
...
matchstarttimerperplayer_internal( var_0, var_1 )
{
    ...
    if ( var_1 )
        childthread updateburningtodeath( var_2 );
...
updateburningtodeath( var_0 )
{
    ...
    if ( !isdefined( level.getkeypadstatefromomnvar ) )
    {
        level.getkeypadstatefromomnvar = spawn( "script_origin", ( 0, 0, 0 ) );
        level.getkeypadstatefromomnvar hide();
    }
    ...
    while ( var_0 > 0 && !level.gameended )
    {
        if ( var_0 < 6 )
        {
            ...
            level.getkeypadstatefromomnvar playsound( var_1 );
        }
    ...
    level.getkeypadstatefromomnvar delete();
}
...
teamstarttimer( var_0, var_1, var_2 )
{
    ...
    if ( !level.gameended )
    {
        if ( scripts\mp\utility\game::getgametype() == "br" )
        {
            thread scripts\mp\gametypes\br_public.gsc::_id_1212A( var_0 );
            thread scripts\mp\gametypes\br_public.gsc::_id_12121( var_0 );
            thread scripts\mp\gametypes\br_public.gsc::brinitloadoutoption();
        }
...
{
    if ( isdefined( level._id_1263D ) )
    {
        level [[ level._id_1263D ]]( var_0, var_1 );
        return;
    }

    if ( !level.doeomcombat )
    {
        wait( level._id_120D1 + 0.1 );
...
_id_11D37( var_0 )
{
    if ( isdefined( var_0 ) )
    {
        foreach ( var_2 in level.players )
        {
            ...
            {
                if ( scripts\mp\utility\game::getgametype() == "br" )
                    var_4 = scripts\mp\gametypes\br_public.gsc::postplunder( var_2.team, var_2.squadindex );
                else
                    var_4 = scripts\mp\utility\teams::getteamdata( var_2.team, "players" );

                if ( isdefined( var_2._id_11D38 ) )
                    var_2._id_11D38 = !var_2._id_11D38;
                else
                    var_2._id_11D38 = 1;
                ...
                if ( istrue( scripts\mp\utility\game::matchmakinggame() ) )
                {
                    if ( isdefined( var_2 ) && var_2 isfireteamleader() )
                    {
                        ...
                        foreach ( var_10 in var_8 )
                        {
                            ...
                            if ( var_11 != var_3 )
                            {
                                if ( isdefined( var_10._id_11D38 ) )
                                    var_10._id_11D38 = !var_10._id_11D38;
                                else
                                    var_10._id_11D38 = 1;
...
relic_amped_test_explode_other_player()
{
    if ( scripts\mp\utility\game::getgametype() == "br" )
    {
        if ( !scripts\mp\utility\game::privatematch() && !scripts\mp\gametypes\br_public.gsc::spawn_apache_chopper() )
        {
            var_0 = [];
            ...
            foreach ( var_2 in level.players )
            {
                ...
                if ( isdefined( var_2._id_11D38 ) && var_2._id_11D38 )
                {
                    var_3 = var_2.team;
                    var_4 = 0;
                    var_5 = var_0.size;
                    ...
                    {
                        var_0[var_5] = var_3;
                        var_7 = scripts\mp\gametypes\br_public.gsc::postplunder( var_2.team, var_2.squadindex );
                        var_8 = [];
...
callback_startgametype()
{
    ...
    level.helperdrone_notifyenemyplayersinrange = 0;
    level.gameended = 0;
    level.forcedend = 0;
    ...
    level.friendlyfiredeath = getdvarint( "scr_game_cdltuning", 0 );
    level.isparachutespawning = scripts\mp\utility\game::isanymlgmatch();
    ...
    level._id_13D03 = getdvarint( "scr_game_wrapKillstreaks" );
    ...
    level.matchrules_vampirism = scripts\mp\utility\dvars::playerzombiethermalupdate( "scr_" + scripts\mp\utility\game::getgametype() + "_vampirism", "scr_game_vampirism" );

    if ( level.matchrules_vampirism )
        level.modifyplayerdamage = scripts\mp\damage::gamemodemodifyplayerdamage;

    level.finalkillcamtype = scripts\mp\utility\dvars::playerzombiethermalupdate( "scr_" + scripts\mp\utility\game::getgametype() + "_killcamType", "scr_game_killcamType" );

    if ( level.finalkillcamtype == 2 )
        level.skipfinalkillcam = 1;

    level.allowkillstreaks = scripts\mp\utility\dvars::playerzombiethermalupdate( "scr_" + scripts\mp\utility\game::getgametype() + "_allowKillstreaks", "scr_game_allowKillstreaks" );
    level.roundretainstreaks = scripts\mp\utility\dvars::playerzombiethermalupdate( "scr_" + scripts\mp\utility\game::getgametype() + "_roundRetainStreaks", "scr_game_roundRetainStreaks" );
    level.roundretainstreakprog = scripts\mp\utility\dvars::playerzombiethermalupdate( "scr_" + scripts\mp\utility\game::getgametype() + "_roundRetainStreakProg", "scr_game_roundRetainStreakProg" );
    level.deathretainstreaks = scripts\mp\utility\dvars::playerzombiethermalupdate( "scr_" + scripts\mp\utility\game::getgametype() + "_deathRetainStreaks", "scr_game_deathRetainStreaks" );
    level.allowperks = scripts\mp\utility\dvars::playerzombiethermalupdate( "scr_" + scripts\mp\utility\game::getgametype() + "_allowPerks", "scr_game_allowPerks" );
    level.allowsupers = scripts\mp\utility\dvars::playerzombiethermalupdate( "scr_" + scripts\mp\utility\game::getgametype() + "_allowSupers", "scr_game_allowSupers" );
    level.superfastchargerate = scripts\mp\utility\dvars::playerzombiethermalupdate( "scr_" + scripts\mp\utility\game::getgametype() + "_superFastChargeRate", "scr_game_superFastChargeRate" );
    level.superpointsmod = scripts\mp\utility\dvars::playerzombiethermalcleanup( "scr_" + scripts\mp\utility\game::getgametype() + "_superPointsMod", "scr_game_superPointsMod" );
    level.spawnprotectiontimer = scripts\mp\utility\dvars::playerzombiethermalcleanup( "scr_" + scripts\mp\utility\game::getgametype() + "_spawnProtectionTimer", "scr_game_spawnProtectionTimer" );
    level.lethaldelay = scripts\mp\utility\dvars::playerzombiethermalcleanup( "scr_" + scripts\mp\utility\game::getgametype() + "_lethalDelay", "scr_game_lethalDelay" );
    level.equipmentmatchstartshieldms = scripts\mp\utility\dvars::playerzombiethermalcleanup( "scr_" + scripts\mp\utility\game::getgametype() + "_equipmentMSProtect", "scr_game_equipmentMSProtect" ) * 1000;
    level.magcount = scripts\mp\utility\dvars::playerzombiethermalupdate( "scr_" + scripts\mp\utility\game::getgametype() + "_magcount", "scr_game_magcount" );

    if ( !isdefined( level.practiceround ) )
        level.practiceround = scripts\mp\utility\dvars::playerzombiethermalupdate( "scr_" + scripts\mp\utility\game::getgametype() + "_practiceRound", "scr_game_practiceRound" );

    level.postgameexfil = scripts\mp\utility\dvars::playerzombiethermalupdate( "scr_" + scripts\mp\utility\game::getgametype() + "_postGameExfil", "scr_game_postGameExfil" );
    level.exfilactivetimer = scripts\mp\utility\dvars::playerzombiethermalcleanup( "scr_" + scripts\mp\utility\game::getgametype() + "_exfilActiveTimer", "scr_game_exfilActiveTimer" );
    level.exfilextracttimer = scripts\mp\utility\dvars::playerzombiethermalcleanup( "scr_" + scripts\mp\utility\game::getgametype() + "_exfilExtractTimer", "scr_game_exfilExtractTimer" );
    level.useammorestocklocs = scripts\mp\utility\dvars::playerzombiethermalcleanup( "scr_" + scripts\mp\utility\game::getgametype() + "_useAmmoRestockLocs", "scr_game_useAmmoRestockLocs" );
    level._id_12EB6 = scripts\mp\utility\dvars::playerzombiethermalupdate( "scr_" + scripts\mp\utility\game::getgametype() + "_allow3rdspectate", "scr_game_allow3rdspectate" );
    setdvar( "LKNNQKNTS", level._id_12EB6 );
    ...
    if ( scripts\mp\utility\game::spot_light_nodes() )
        gas_vfx_range_think();
    ...
    level.radarhideshots = scripts\mp\utility\dvars::playerzombiethermalupdate( "scr_" + var_0 + "_radarHidePings", "scr_game_radarHidePings" );
    setomnvar( "ui_compass_hide_weapon_pings_minimap", level.radarhideshots );
    level.navbarhideshots = scripts\mp\utility\dvars::playerzombiethermalupdate( "scr_" + var_0 + "_navBarHidePings", "scr_game_navBarHidePings" );
    setomnvar( "ui_compass_hide_weapon_pings_navbar", level.navbarhideshots );
    level.navbarhideenemies = scripts\mp\utility\dvars::playerzombiethermalupdate( "scr_" + var_0 + "_navBarHideEnemy", "scr_game_navBarHideEnemy" );
    ...
    level.isonturret = getdvarint( "scr_player_disableMount" );
    level.disablebattlechatter = getdvarint( "scr_game_disableBattleChatter" );
    level.isgunlessweapon = getdvarint( "scr_game_disableAnnouncer" );
    level.scoremod = [];
    ...
    {
        if ( scripts\mp\utility\game_utility_mp::vehicle_getteamfriendlyto() )
        {
    ...
    scripts\cp_mp\utility\game_utility::setsoundsubmixfadetoblackamb();
    level.maxallowedteamkills = scripts\mp\utility\dvars::playerzombiethermalupdate( "scr_" + scripts\mp\utility\game::getgametype() + "_ffPunishLimit", "scr_game_ffPunishLimit" );
    thread scripts\mp\init_cp_mp::init();
    thread scripts\mp\teams::init();
    _branalytics_geteventtimestamp::_id_13256();
    scripts\cp_mp\utility\player_utility::setup_comms_obj_a_goals_and_cover();
    ...
    thread scripts\cp\utility\cp_controlled_callbacks::init();
    thread scripts\cp_mp\vehicles\cargo_truck_mg::init();
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" )
    {
        thread scripts\cp_mp\utility\omnvar_utility::init();
    ...
    scripts\mp\utility\player_frame_update_aggregator::registerplayerframeupdatecallback( scripts\mp\weapons::radar_sweeps );

    if ( scripts\mp\utility\game::matchmakinggame() && !scripts\mp\utility\game::privatematch() )
        level scripts\common\ui::lui_registercallback( "play_again_squad_request", ::_id_11D37 );
    ...
    thread scripts\cp\vehicles\vehicle_damage_cp::init();
    ...
    if ( getdvarint( "scr_allow_custom_loadouts", 0 ) == 0 || scripts\mp\utility\game::spawn_chopper_carepackage() )
...
timelimitthread()
{
    ...
    while ( game["state"] == "playing" )
    {
        ...
        if ( isdefined( level.starttime ) )
        {
            if ( gettimeremaining() < 3000 || istrue( level.obj_ow_atvs_spawned ) )
            {
                waitframe();
                continue;
            }
...
timelimitclock()
{
    ...
    if ( isdefined( level._id_1332E ) )
    {
        thread [[ level._id_1332E ]]();
        return;
    }
...
gametimer()
{
    ...
    if ( !isdefined( game["startTimeFromMatchStart"] ) )
    {
        ...
        scripts\common\utility::_id_13592( level.update_spot_limit );
    }
...
startgame()
{
    ...
    if ( getdvarint( "scr_game_updated_prematch_allows", 1 ) == 1 )
    {
        foreach ( var_1 in level.players )
        {
            ...
            if ( scripts\mp\utility\game::getgametype() == "br" )
                var_1 scripts\mp\persistence::should_run_sp_stealth();
        }
    }
    ...
    thread graceperiod();
    thread scripts\cp\utility\cp_controlled_callbacks::roundbegin();
...
cleanpatchablecollision()
{
    ...
    foreach ( var_3, var_2 in var_0 )
    {
        if ( isdefined( var_2 ) )
        {
            if ( isdefined( level._id_11B08 ) && istrue( level._id_11B08[var_3] ) )
            {
                level._id_11B08[var_3] = undefined;
                continue;
            }
    ...
    if ( isdefined( level._id_11B08 ) )
    {
        foreach ( var_3, var_5 in level._id_11B08 )
        {

        }
    }

    level._id_11B08 = undefined;
}

_id_124E4( var_0 )
{
    if ( istrue( level._id_11B07 ) )
        return;

    if ( !isdefined( level._id_11B08 ) )
        level._id_11B08 = [];

    level._id_11B08[var_0] = 1;
}
...
wavetimerwatcher()
{
    ...
    for (;;)
    {
        scripts\engine\utility::_id_13AD9( "wave_respawn_allies", "wave_respawn_axis" );
...
displayroundend( var_0, var_1, var_2 )
{
    ...
    if ( scripts\mp\utility\game::waslastround() && scripts\mp\utility\game::iswinbytworulegametype() && scripts\mp\utility\game::_id_12B4A() )
    {
        var_0 = "tie";
...
displayroundswitch()
{
    ...
    var_0 = playersetignoreattractions();
...
freezeallplayers( var_0, var_1, var_2, var_3 )
{
    ...
    var_4 = scripts\mp\utility\game::spawnheight();
...
setendofroundsoundtimescalefactor()
{
    if ( !isdefined( level.locindex ) )
    {
        createheadiconatorigin( "end_of_round" );
        level.locindex = 1;
        level._id_124F8 = undefined;
    }
}
...
resetsoundtimescalefactor()
{
    if ( !isdefined( level._id_124F8 ) )
    {
        ...
        level.locindex = undefined;
        level.wavesv_finite_ending = undefined;
        level._id_124F8 = 1;
    }
}
...
endgame( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    ...
    if ( isdefined( level.vehicle_getturrets ) )
        var_0 = level.vehicle_getturrets;
...
endgame_regularmp( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    scripts\common\utility::_id_13592( level.update_timer_for_bomb_case_detonator_holder );
    var_5 = 0;
    ...
    level.heliatcapacitycallback = istrue( var_3 ) && scripts\mp\utility\game::getgametype() == "ctf";

    if ( isdefined( var_0 ) && level.heliatcapacitycallback )
    {
    ...
    foreach ( var_14 in level.players )
    {
        if ( !isbot( var_14 ) && isdefined( var_14.team ) && var_14.team != "spectator" && var_14.team != "follower" )
            trial_trigger_activated_func( var_14 );
    }
...
endgame_showkillcam()
{
    ...
    if ( istrue( level.damagethisround ) )
        return;
...
loadoutcustomcost( var_0 )
{
    if ( isdefined( var_0 ) )
    {
        if ( var_0 == 1 || var_0 == 2 )
        {
            level.all_but_one_player_in_vehicle = 0;
            level.ai_dropgren_model = var_0;
...
endgame_endround( var_0, var_1, var_2, var_3 )
{
    ...
    if ( scripts\mp\utility\game::waslastround() )
    {
        if ( scripts\mp\utility\game::getgametype() == "arena" )
        {
            ...
            foreach ( var_7 in level.players )
            {
                ...
                {
                    if ( var_7 == var_4[0] )
                        var_7 scripts\cp_mp\pet_watch::assignintelchallenges();
                }
            }
        }
    ...
    rankedmatchupdates( var_0 );
    setdvar( "SLLNLPRON", scripts\mp\utility\game::spawnheight() );
    ...
    else
    {
        if ( level.teambased )
        {
            ...
            {
                if ( level scripts\mp\utility\game::stadium_two_death_func( var_25 ) )
                    continue;
            ...
            if ( !scripts\mp\utility\game::waslastround() )
            {
                ...
                foreach ( var_7 in level.players )
                {
                    ...
                    if ( isalive( var_7 ) && isdefined( var_7.matchdatalifeindex ) )
                        var_7 scripts\common\utility::_id_13592( level.update_operator_west_char_loc, var_7.matchdatalifeindex, undefined, undefined, "MOD_ROUND_ENDED", "none", undefined, undefined, undefined );
                }
...
endgame_endgame( var_0, var_1, var_2, var_3 )
{
    ...
    if ( scripts\mp\gametypes\br_public.gsc::spawn_apache_chopper() && !level.forcedend )
    {
        level.all_but_one_player_in_vehicle = 1;
        level scripts\common\ui::lui_registercallback( "exit_squad_eliminated", ::loadoutcustomcost );
    }

    if ( scripts\mp\utility\game::matchmakinggame() && isdefined( level._id_12B22 ) && [[ level._id_12B22 ]]() )
        createnavobstaclebyshape();

    if ( !istrue( level.processedwinloss ) && ( istrue( level.forfeitinprogress ) || level.forcedend ) )
        updatewinlossstats( var_0 );

    scripts\cp\utility\cp_controlled_callbacks::roundend( var_0 );
    checkforpersonalbests();
    updatespmstats();
    scripts\mp\persistence::writebestscores();
    level notify( "stop_leaderboard_stats" );
    updateleaderboardstats();
    level scripts\mp\accolades::makeextractionobjective();
    level.doingbroshot = scripts\mp\broshot::initbroshot( var_0 );
    var_4 = scripts\mp\gametypes\br_gametypes.gsc::spawn_juggernauts_hangar( "endGameIntermission" );

    if ( !level.doingbroshot && istrue( var_4 ) )
    {
        level notify( "spawning_intermission" );
        var_5 = playersetignoreattractions();

        foreach ( var_7 in level.players )
        {
            var_7 thread scripts\mp\utility\game::setuipostgamefade( 0.0 );

            if ( !isdefined( level.vehicle_incomingcallback ) || !var_7 [[ level.vehicle_incomingcallback ]]() )
    ...
    if ( !level.doingbroshot )
    {
        if ( scripts\mp\utility\game::getgametype() == "br" )
        {
            ...
            if ( isdefined( level.loadoutcustomperkdiscount ) )
            {
                foreach ( var_7 in level.players )
                {
                    if ( !isdefined( var_7 ) )
            ...
            if ( getdvarint( "post_game_play_again_enabled", 1 ) == 1 )
            {
                ...
                {
                    if ( isdefined( level.loadoutcustomperkdiscount ) )
                        wait( level.loadoutcustomperkdiscount._id_139EA );
                ...
                {
                    if ( istrue( level.all_but_one_player_in_vehicle ) )
    ...
    if ( scripts\mp\utility\game::getgametype() != "br" || scripts\mp\utility\game::postupdategameevents() == "dmz" || scripts\mp\utility\game::postupdategameevents() == "risk" || scripts\mp\utility\game::postupdategameevents() == "kingslayer" || scripts\mp\utility\game::postupdategameevents() == "rumble" || scripts\mp\utility\game::spawninsafehouse() )
        processlobbydata();
    ...
    foreach ( var_7 in level.players )
    {
        ...
        if ( isalive( var_7 ) && isdefined( var_7.matchdatalifeindex ) )
            var_7 scripts\common\utility::_id_13592( level.update_operator_west_char_loc, var_7.matchdatalifeindex, undefined, undefined, "MOD_GAME_ENDED", "none", undefined, undefined, undefined );
        ...
        var_7 scripts\common\utility::_id_13592( level.update_operator_east_char_loc );
    }

    scripts\common\utility::_id_13592( level.update_timer_for_bomb_vest_detonator_holder );
    ...
    if ( scripts\mp\gametypes\br_public.gsc::spawn_apache_chopper() && isdefined( level.ai_dropgren_model ) && level.ai_dropgren_model == 2 )
    {
...
_id_1285E()
{
    if ( !isdefined( level.locationcircles ) )
    {
...
playersetignoreattractions()
{
    ...
    {
        var_0 = spawnstruct();
        var_0.origin = level.disabledstate._id_12EBA.origin;
        var_0.angles = level.disabledstate._id_12EBA.angles;
    }
...
bunkerinteriorkeypads( var_0 )
{
    ...
    {
        var_0.clientmatchdataid = level.helperdrone_notifyenemyplayersinrange;
        level.helperdrone_notifyenemyplayersinrange++;
    }
}

_id_12177( var_0 )
{
    if ( istrue( var_0._id_12177 ) )
        return;
    ...
    if ( getdvarint( "MTKSQRQLKN" ) != 0 )
    {
        ...
        setclientmatchdata( "players", var_0.clientmatchdataid, "nameSuffix", var_0 _meth_87D0() );
    ...
    scripts\mp\matchdata::_id_1298B( var_0 );
}
...
trackattackerleaderboarddeathstats( var_0, var_1, var_2, var_3 )
{
    if ( isdefined( var_0 ) && isplayer( var_0 ) )
    {
        if ( var_3 != "MOD_FALLING" )
        {
            ...
            {
                var_0 scripts\common\utility::_id_13592( level.update_jugg_targets, "tactical", "kills", 1, var_2 );
                var_0 scripts\common\utility::_id_13592( level.update_jugg_targets, "tactical", "hits", 1, var_2 );
            ...
            if ( var_3 == "MOD_MELEE" && !scripts\mp\riotshield::isriotshield( var_2 ) && !scripts\mp\utility\weapon::isknifeonly( var_2 ) && !scripts\mp\utility\weapon::sortbyhvttags( var_2 ) && !scripts\mp\utility\weapon::isaxeweapon( var_2 ) )
            {
                var_0 scripts\common\utility::_id_13592( level.update_jugg_targets, "none", "kills", 1, var_2 );
                var_0 scripts\common\utility::_id_13592( level.update_jugg_targets, "none", "hits", 1, var_2 );
...
setweaponstat( var_0, var_1, var_2 )
{
    ...
    if ( isdefined( var_3._id_11AF1 ) )
        var_3 = var_3._id_11AF1;

    if ( !isdefined( level.facial_clear ) )
        level.facial_clear = scripts\mp\utility\game::getgametype() == "br" && getdvarint( "scr_track_picked_up_weapon_stats", 1 ) == 1;

    if ( scripts\mp\utility\weapon::ispickedupweapon( var_3 ) && !istrue( level.facial_clear ) )
        return;
    ...
    if ( var_5 == "weapon_grenade" || var_5 == "weapon_explosive" || var_4 == "trophy_mp" || var_4 == "forcepush_mp" )
    {
        ...
        scripts\common\utility::_id_13592( level.update_restock_ui, var_4, var_2, var_1, var_6, var_3 );
        return;
    }
    ...
    if ( var_2 == "deaths" )
    {
        ...
        scripts\common\utility::_id_13592( level.update_restock_ui, var_9, "deaths", var_1, var_6, var_3 );

        foreach ( var_12 in var_10 )
        {
            scripts\mp\persistence::incrementattachmentstat( var_12, var_2, var_1, var_3 );
            scripts\common\utility::_id_13592( level.update_jugg_targets, var_12, var_2, var_1, var_3 );
        }
    }
}
...
updateleaderboardstatscontinuous()
{
    ...
    if ( !botloadoutfavoritecamoprimary() )
        return;
...
_id_1377A()
{
    ...
    if ( isdefined( level.deadblue ) && isdefined( level.deadblue.name ) )
    {
        switch ( level.deadblue.name )
        {
            ...
            case "kingslayer":
            case "dmz":
                var_0 = var_0 + ( "_" + level.deadblue.name );
                break;
            case "vov":
            case "dbd":
            case "brz":
    ...
    var_1 = scripts\engine\utility::ter_op( level.teambased, self.score, self.thermite_damagemodifierignorefunc["gamemodeScore"] );
    ...
    incrementleaderboardstat( "kills", var_0, self.thermite_damagemodifierignorefunc["kills"] );
    incrementleaderboardstat( "deaths", var_0, self.thermite_damagemodifierignorefunc["deaths"] );

    switch ( scripts\mp\utility\game::getgametype() )
    {
        case "war":
            incrementleaderboardstat( "assists", var_0, self.thermite_damagemodifierignorefunc["assists"] );
            break;
        case "arena":
            incrementleaderboardstat( "damage", var_0, self.thermite_damagemodifierignorefunc["damage"] );
            incrementleaderboardstat( "assists", var_0, self.thermite_damagemodifierignorefunc["assists"] );
            break;
        case "arm":
            incrementleaderboardstat( "captures", var_0, self.thermite_damagemodifierignorefunc["captures"] );
            incrementleaderboardstat( "defends", var_0, self.thermite_damagemodifierignorefunc["defends"] );
            break;
        case "cyber":
            incrementleaderboardstat( "revives", var_0, self.thermite_damagemodifierignorefunc["rescues"] );
            incrementleaderboardstat( "plants", var_0, self.thermite_damagemodifierignorefunc["plants"] );
            break;
        case "dm":
            updateleaderboardstatmaximum( "streak", var_0, self.thermite_damagemodifierignorefunc["killChains"] );
            break;
        case "dom":
            incrementleaderboardstat( "captures", var_0, self.thermite_damagemodifierignorefunc["captures"] );
            incrementleaderboardstat( "defends", var_0, self.thermite_damagemodifierignorefunc["defends"] );
        ...
        case "sd":
            incrementleaderboardstat( "plants", var_0, self.thermite_damagemodifierignorefunc["plants"] );
            incrementleaderboardstat( "defuses", var_0, self.thermite_damagemodifierignorefunc["defuses"] );
            break;
        case "conf":
            incrementleaderboardstat( "confirms", var_0, self.thermite_damagemodifierignorefunc["confirmed"] );
            incrementleaderboardstat( "denies", var_0, self.thermite_damagemodifierignorefunc["denied"] );
            break;
        case "hq":
            incrementleaderboardstat( "captures", var_0, self.thermite_damagemodifierignorefunc["captures"] );
            incrementleaderboardstat( "defends", var_0, self.thermite_damagemodifierignorefunc["defends"] );
            break;
        case "koth":
            incrementleaderboardstat( "objTime", var_0, self.thermite_damagemodifierignorefunc["objTime"] );
            incrementleaderboardstat( "defends", var_0, self.thermite_damagemodifierignorefunc["defends"] );
            break;
        case "ctf":
            incrementleaderboardstat( "captures", var_0, self.thermite_damagemodifierignorefunc["captures"] );
            incrementleaderboardstat( "returns", var_0, self.thermite_damagemodifierignorefunc["returns"] );
            break;
        case "sr":
            incrementleaderboardstat( "plants", var_0, self.thermite_damagemodifierignorefunc["plants"] );
            incrementleaderboardstat( "rescues", var_0, self.thermite_damagemodifierignorefunc["rescues"] );
            break;
        case "siege":
            incrementleaderboardstat( "captures", var_0, self.thermite_damagemodifierignorefunc["captures"] );
            incrementleaderboardstat( "revives", var_0, self.thermite_damagemodifierignorefunc["rescues"] );
            break;
        case "grind":
            incrementleaderboardstat( "banks", var_0, self.thermite_damagemodifierignorefunc["confirmed"] );
            incrementleaderboardstat( "denies", var_0, self.thermite_damagemodifierignorefunc["denied"] );
            break;
        case "infect":
            incrementleaderboardstat( "time", var_0, scripts\mp\utility\stats::getpersstat( "extrascore0" ) );
            incrementleaderboardstat( "infected", var_0, self.thermite_damagemodifierignorefunc["killsAsInfected"] );
            break;
        case "gun":
            incrementleaderboardstat( "stabs", var_0, self.thermite_damagemodifierignorefunc["stabs"] );
            incrementleaderboardstat( "setBacks", var_0, self.thermite_damagemodifierignorefunc["setbacks"] );
            break;
        case "grnd":
            incrementleaderboardstat( "objTime", var_0, self.thermite_damagemodifierignorefunc["objTime"] );
            incrementleaderboardstat( "defends", var_0, self.thermite_damagemodifierignorefunc["defends"] );
        ...
        case "pill":
            ...
            break;
        case "br":
            incrementleaderboardstat( "downs", var_0, self.thermite_damagemodifierignorefunc["downs"] );
            incrementleaderboardstat( "contracts", var_0, self.thermite_damagemodifierignorefunc["contracts"] );
            incrementleaderboardstat( "wins", var_0, self.thermite_damagemodifierignorefunc["wins"] );
            incrementleaderboardstat( "topFive", var_0, self.thermite_damagemodifierignorefunc["topFive"] );
            incrementleaderboardstat( "topTen", var_0, self.thermite_damagemodifierignorefunc["topTen"] );
            incrementleaderboardstat( "topTwentyFive", var_0, self.thermite_damagemodifierignorefunc["topTwentyFive"] );
            incrementleaderboardstat( "gamesPlayed", var_0, self.thermite_damagemodifierignorefunc["gamesPlayed"] );
            incrementleaderboardstat( "revives", var_0, self.thermite_damagemodifierignorefunc["rescues"] );
            incrementleaderboardstat( "cash", var_0, self.thermite_damagemodifierignorefunc["cash"] );
            incrementleaderboardstat( "objTime", var_0, self.thermite_damagemodifierignorefunc["objTime"] );
            break;
    }
}
...
checkforbestweapon()
{
    var_0 = scripts\common\utility::_id_13592( level.update_hint_logic_offhand );
    var_1 = "";
    var_2 = -1;
...
_id_124B3()
{
    ...
    foreach ( var_2 in var_0 )
        scripts\cp_mp\utility\game_utility::_id_124AF( var_2, "targetname" );
}
...
gas_vfx_range_think()
{
    level.allowsupers = getdvarint( "scr_" + scripts\mp\utility\game::postupdategameevents() + "_allowSupers", level.allowsupers );
}
...
trial_trigger_activated_func( var_0 )
{
    ...
    if ( isdefined( var_0.hvt_can_lose_health ) )
        var_6 = var_0.hvt_can_lose_health;
...
_id_11D59()
{
    ...
    wait 1;
    _id_1203E( level.loadoutcustomperkdiscount._id_139EB, level.loadoutcustomperkdiscount._id_139EA, level.loadoutcustomperkdiscount.spawn_geos );
    self setclientomnvar( "ui_br_transition_type", 0 );
    self setclientomnvar( "post_game_state", 14 );
}

_id_1203E( var_0, var_1, var_2 )
{
    ...
    self playcinematicforplayer( var_0, 1, var_2 );
    var_6 = gettime();
    _id_12BF5( var_6, var_1 * 1000 );
    self stopcinematicforplayer();
    scripts\mp\utility\player::restorebasevisionset( 0 );
    self setclientdvar( "LQKPQMPRQN", scripts\mp\utility\game::spawnheight() );
