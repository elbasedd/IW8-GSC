...
init()
{
    ...
    level.multiteambased = level.teamnamelist.size > 2 || scripts\mp\utility\game::getgametype() == "br" && scripts\mp\menus::_id_12F0E();
    level.teamdata = [];
    ...
    {
        scripts\mp\utility\teams::precomputeddropbags( var_9 );
    ...
    if ( isdefined( level.nuke_vault_jugg_shoot_at_scriptables ) )
        level thread [[ level.nuke_vault_jugg_shoot_at_scriptables ]]();

    if ( getdvarint( "scr_debug_teams", 0 ) == 1 )
        level thread init_hack_console();
}
...
createoperatorcustomization()
{
    ...
    var_10.title = getoperatortitle( var_8 );
    var_10.clothtype = playerzombiejumpstop( var_9 );
    var_10.superfaction = getoperatorsuperfaction( var_8 );
    var_10.execution = getoperatorexecution( var_8 );
    var_10.maphint_cheese2scriptableused = playerzombielaststandrevive( var_8 );
    var_10.maphint_cheesescriptableused = playerzombiemonitorinput( var_8 );
    var_10.suit = var_3;
    var_10.rebuild = 0;
    var_10.risktokencountondeath = spawn_tut_loot( var_1 );

    if ( scripts\mp\utility\game::getgametype() == "br" )
    {
        var_11 = playerzombiejumpmaxholdwarning( var_9 );

        if ( var_11 != "" )
            var_10.deathnoise = var_11;

        relic_healthpacks( var_9 );

        if ( playerzombiepowers( var_8 ) == "s4" )
            self.s4operator = 1;
    }

    var_12 = spawnstruct();
    var_12.apc = prev_weapon_taccover( "apc" );
    var_12.breventflagset = preventleave( "apc" );
    var_12.call_in_carepackage = prev_weapon_taccover( "atv" );
    var_12.findenemytype = prev_weapon_taccover( "cargo_truck" );
    var_12.start_conceal_add = prev_weapon_taccover( "jeep" );
    var_12.trackriotshield_tryarm = prev_weapon_taccover( "little_bird" );
    var_12._id_131BA = prev_weapon_taccover( "tac_rover" );
    var_12._id_13206 = prev_weapon_taccover( "tank_east" );
    var_12._id_13207 = preventleave( "tank_east" );
    var_12._id_13211 = prev_weapon_taccover( "tank_west" );
    var_12._id_13212 = preventleave( "tank_west" );
    var_12.vo_hangar_door = prev_weapon_taccover( "motorcycle" );
    var_12.bomber_death_thread = prev_weapon_taccover( "airplane" );
    var_12.cleanupkeybindingsondeath = prev_weapon_taccover( "bomber" );
    var_12.brdownedbyairstriketime = prevbrbonusxp( "apc", 4 );
    var_12.callback = prevbrbonusxp( "atv", 6 );
    var_12.findeventforchosenweight = prevbrbonusxp( "cargo_truck", 8 );
    var_12.start_coop_defuse_infiltrate = prevbrbonusxp( "jeep", 10 );
    var_12.trackriotshield_tryback = prevbrbonusxp( "little_bird", 12 );
    var_12._id_131BB = prevbrbonusxp( "tac_rover", 14 );
    var_12.vo_while_reviving = prevbrbonusxp( "motorcycle", 16 );
    var_12.callback_trigger = prevcallback( "atv" );
    var_12._id_131BF = prevcallback( "tac_rover" );
    var_12.traintracefails = prevcallback( "little_bird" );
    var_12.wait_and_destroy = prevcallback( "motorcycle" );
    var_12.bomber_disable_movement_for_time = prevcallback( "airplane" );
    var_12.clonedeath = prevcallback( "bomber" );
    ...
    self._id_13984 = var_12;
}

playoverwatch_dialogue_with_endon( var_0 )
{
    return self getplayerdata( level.loadoutsgroup, "customizationSetup", "operators", var_0 );
}

playoverwatch_dialogue( var_0 )
{
    if ( gate_flares_think() )
    {
        ...
        var_3 = playoverwatch_dialogue_with_endon( var_0 );

        if ( spawn_unique_drone( var_3 ) )
        {
            if ( !isdefined( self.pers["restrictedOperatorInfo"] ) )
            ...
            var_2._id_11A58 = int( tablelookup( "operatorskins.csv", 1, var_6, 0 ) );
...
gate_flares_think()
{
    if ( !isdefined( level._id_11A55 ) )
    {
        level._id_11A55 = spawnstruct();
        var_0 = getdvar( "scr_operator_restrict_sources" );
        level._id_11A55.sources = strtok( var_0, "," );
        level._id_11A55.enabled = !!level._id_11A55.sources.size;

        if ( level._id_11A55.enabled )
            level._id_11A55.spawnropeandbag = [];
    }

    return level._id_11A55.enabled;
}

spawn_unique_drone( var_0 )
{
    if ( !gate_flares_think() )
        return 0;

    var_1 = level._id_11A55.spawnropeandbag[var_0];

    if ( !isdefined( var_1 ) )
    {
        var_1 = 0;
        var_2 = playerzombiepowers( var_0 );

        foreach ( var_4 in level._id_11A55.sources )
        {
            if ( var_2 == var_4 )
            {
                var_1 = 1;
        ...
        level._id_11A55.spawnropeandbag[var_0] = var_1;
    }
...
lookupotheroperator( var_0 )
{
    ...
    if ( level.gametype != "br" )
    {
        if ( level.teambased && !isai( self ) )
        {
            ...
            {
                var_4 = spawnstruct();
                var_4.operatorref = playoverwatch_dialogue( var_1 );
...
lookupcurrentoperator( var_0 )
{
    ...
    else if ( !isdefined( level.playercustomizationdata[var_4][var_0] ) )
    {
        ...
        if ( isai( self ) )
            ...
            var_6.operatorref = playoverwatch_dialogue( var_1 );
    ...
    if ( getdvarint( "scr_forceHeadlessCustomization", 1 ) == 1 && !isagent( self ) && self isplayerheadless() )
    {
        ...
        if ( !isdefined( self.removeaccesscard ) )
        {
            foreach ( var_9 in var_7 )
            {
                var_10 = getarraykeys( level.operatorcustomization[var_9] );

                if ( !isdefined( level.removefromlittlebirdmglistondeath ) )
                    level.removefromlittlebirdmglistondeath = [];

                if ( !isdefined( level.removefromlittlebirdmglistondeath[var_9] ) || level.removefromlittlebirdmglistondeath[var_9] > var_10.size )
                    level.removefromlittlebirdmglistondeath[var_9] = 0;

                for ( var_11 = var_10[level.removefromlittlebirdmglistondeath[var_9]]; isdefined( var_11 ) && ( var_11 == "default_western" || var_11 == "default_eastern" ); var_11 = var_10[level.removefromlittlebirdmglistondeath[var_9]] )
                {
                    level.removefromlittlebirdmglistondeath[var_9] = level.removefromlittlebirdmglistondeath[var_9] + 1;

                    if ( level.removefromlittlebirdmglistondeath[var_9] > var_10.size )
                        level.removefromlittlebirdmglistondeath[var_9] = 0;
                }
...
lookupcurrentoperatorskin( var_0 )
{
    ...
    else
    {
        if ( getdvarint( "scr_forceHeadlessCustomization", 1 ) == 1 && !isagent( self ) && self isplayerheadless() )
        {
            ...
            {
                if ( !isdefined( level.removefromdismembermentlist ) )
                    setup_rarity_ui_images();

                var_5 = level.removefromdismembermentlist[var_1]["curIndex"];
                level.playercustomizationdata[var_3][var_0].operatorskinindex = level.removefromdismembermentlist[var_1]["lootIDs"][var_5];
                level.removefromdismembermentlist[var_1]["curIndex"] = level.removefromdismembermentlist[var_1]["curIndex"] + 1;

                if ( level.removefromdismembermentlist[var_1]["curIndex"] >= level.removefromdismembermentlist[var_1]["maxIndex"] )
                    level.removefromdismembermentlist[var_1]["curIndex"] = 0;
            }
        }
        else if ( !isdefined( level.playercustomizationdata[var_3][var_0].operatorskinindex ) )
        {
            if ( isai( self ) )
            {
                if ( !isdefined( self.botskinid ) )
                    create_ai_type_override( var_1 );

                level.playercustomizationdata[var_3][var_0].operatorskinindex = self.botskinid;
            }
            else
                level.playercustomizationdata[var_3][var_0].operatorskinindex = playplundersound( var_1 );
        }
...
getoperatorcustomization()
{
    ...
    if ( isdefined( level.vehicle_damage_setpremoddamagecallback ) )
        [var_0, var_1] = [[ level.vehicle_damage_setpremoddamagecallback ]]( self, var_0, var_1 );
...
checkforafk()
{
    ...
    if ( istrue( self.destroyawardlaunchonly ) )
        return;
    ...
    if ( istrue( self.remove_crusader_class ) )
        return;
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" && istrue( self.debug_run_helicopter_boss ) )
    {
        self.debug_run_helicopter_boss = undefined;
        var_10 = 30;
    }
    else if ( var_11 > 0 )
        var_10 = var_11;
    ...
    if ( scripts\mp\utility\game::isroundbased() && scripts\mp\utility\game::getgametype() != "ctf" )
    {
        ...
        if ( var_0 && !isgamebattlematch() && !istrue( self.attempted_to_use_gunship ) )
        {
            self.attempted_to_use_gunship = 1;
            self.pers["roundsAFK"]++;

            if ( self.pers["roundsAFK"] > 1 )
                level thread stop_counter_beep_sfx_on_bomb_vests( self );
        }
...
getjointeampermissions( var_0 )
{
    ...
    if ( level.maxteamsize == 0 || var_1 < level.maxteamsize )
        ...
        return 1;
    else if ( scripts\mp\menus::br_ammorestock_playeruse() )
        return 1;
...
{
    level endon( "game_ended" );
    scripts\engine\utility::_id_13AD9( "death_or_disconnect", "joined_team" );
...
{
    level endon( "game_ended" );
    scripts\engine\utility::_id_13ADA( "death_or_disconnect", "joined_team", "joined_squad" );
    scripts\mp\utility\outline::outlinedisable( var_0, self );
}

playerzombiepowers( var_0 )
{
    var_1 = tablelookup( "mp/itemsourcetable.csv", 2, var_0, 3 );
...
playerzombielaststandrevive( var_0 )
{
    var_1 = 0;
    ...
    if ( var_1 == 0 )
    {
        ...
        self.trial_callback_ai_killed = tablelookup( "operatorquips.csv", 1, var_2, 6 );
    }
    else
        self.trial_callback_ai_killed = tablelookup( "operatorquips.csv", 0, var_1, 6 );

    return self.trial_callback_ai_killed;
}

playerzombiemonitorinput( var_0 )
{
    self.trial_celebration_flares = tablelookup( "mp_cp/executiontable.csv", 1, self.loadoutexecution, 19 );
    return self.trial_celebration_flares;
}
...
setup_rarity_ui_images()
{
    ...
    level.removefromdismembermentlist = [];
    ...
    for ( var_1 = 0; var_1 < var_0; var_1++ )
    {
        if ( tablelookupbyrow( "operatorskins.csv", var_1, 18 ) != "" )
        {
            ...
            if ( !isdefined( level.removefromdismembermentlist[var_2] ) )
            {
                level.removefromdismembermentlist[var_2]["lootIDs"] = [];
                level.removefromdismembermentlist[var_2]["curIndex"] = 0;
                level.removefromdismembermentlist[var_2]["maxIndex"] = 0;
            }

            level.removefromdismembermentlist[var_2]["lootIDs"][level.removefromdismembermentlist[var_2]["lootIDs"].size] = int( var_3 );
            level.removefromdismembermentlist[var_2]["maxIndex"] = level.removefromdismembermentlist[var_2]["maxIndex"] + 1;
        }
    }
}
...
initoperatorcustomization()
{
    ...
    for (;;)
    {
        ...
        var_6 = playerzombieprestream( var_2 );
        var_7 = var_0 && spawn_unique_drone( var_2 );
    ...
    if ( !var_1 )
    {
        ...
        if ( !isdefined( level.playercustomizationdata[var_2][self.team] ) )
        {
            ...
            if ( isai( self ) )
                ...
                var_4.operatorskinindex = playplundersound( var_0 );
...
_id_11BFC()
{
    ...
    if ( istrue( level.onlinegame ) )
    {
        ...
        if ( isdefined( var_2 ) && var_2.size > 0 )
        {
            foreach ( var_4 in var_2 )
            {
                ...
                {
                    var_0 = var_4.team;
                    self._id_1327D = var_4._id_1327D;
                    break;
                }
            }
        }
    ...
    if ( !isdefined( self._id_1327D ) )
        self._id_1327D = gettime();
...
movequestobjicon( var_0 )
{
    ...
    foreach ( var_3 in level.teamnamelist )
    {
        ...
        if ( var_4 > 0 )
        {
            ...
            if ( isdefined( var_6[0]._id_1327D ) && gettime() > var_6[0]._id_1327D + 300000 )
                continue;
        }
...
_id_12B0A()
{
    return getdvarint( "scr_br_zombie_encounters", 0 ) > 0 || getdvarint( "scr_br_alt_mode_fiend", 0 ) > 0 || scripts\mp\utility\game::currentmultikill();
}
