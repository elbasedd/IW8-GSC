...
init()
{
    ...
    level.updatematchstatushintonhasflag = getdvarint( "scr_ks_jugg_team_max", 0 );
}

jugg_makejuggernaut( var_0, var_1 )
{
    var_2 = start_safehouse_quarry( var_0 );
    ...
    if ( isdefined( level.getgametypeweaponxpmultiplier ) )
        self [[ level.getgametypeweaponxpmultiplier ]]();
    ...
    else
    {
        ...
        foreach ( var_11 in var_9 )
        {
            ...
            if ( start_player_links() )
            {
                self givemaxammo( var_11 );
                thread start_silo_elevator( var_11 );
            }
        }
    }
    ...
    thread start_static_plane_lights();
    thread jugg_watchmusictoggle();
    thread jugg_watchfordeath();
    thread jugg_watchforgameend();
    thread jugg_watchfordisconnect();
    thread start_trap_room_combat();
    thread start_unlock_silo();

    if ( start_pipe_room_menu() )
    {
        self skydive_setbasejumpingstatus( 1 );
        self skydive_setdeploymentstatus( 1 );
    }

    if ( isdefined( level.array_of_light_targetnames ) )
        self [[ level.array_of_light_targetnames ]]();
...
jugg_createconfig( var_0, var_1 )
{
    ...
    var_2.updateteamplunderscore = 5;

    if ( level.gametype == "br" )
        var_2.updateteamplunderscore = 3;
...
start_static_plane_lights()
{
    if ( start_pipe_room() )
    {
        ...
        if ( var_0 )
        {
            ...
            if ( start_safehouse_return( var_0 ) )
                var_1 = "flames";
...
jugg_getjuggmodels()
{
    ...
    if ( start_pipe_room() )
    {
        ...
        if ( var_1 )
        {
            if ( start_safehouse_return( var_1 ) )
            {
...
start_safehouse_quarry( var_0 )
{
    ...
    if ( var_2 != "stand" )
    {
        ...
        else if ( var_2 == "crouch" )
        {
            if ( !var_3 )
            {
                var_6 = scripts\engine\trace::ray_trace_passed( self.origin, scripts\mp\utility\player::postspawn_rescued_allies( "stand" ), self, var_5 );
        ...
        else if ( var_2 == "prone" )
        {
            if ( !var_4 )
            {
                if ( var_3 )
                {
                    var_7 = scripts\engine\trace::ray_trace_passed( self.origin, scripts\mp\utility\player::postspawn_rescued_allies( "crouch" ), self, var_5 );
...
jugg_watchoverlaydamagestates( var_0 )
{
    ...
    for (;;)
    {
        scripts\engine\utility::_id_13AD9( "damage", "jugg_health_regen" );
...
start_player_links()
{
    return istrue( self.juggcontext.juggconfig._id_137FB );
}

start_preserverroom_spawners()
{
    return istrue( self.juggcontext.juggconfig._id_1380C );
}

start_silo_elevator( var_0 )
{
    self endon( "juggernaut_end" );
    level endon( "game_ended" );
    var_1 = self getweaponammostock( var_0 );
    thread start_unstable_rocket_fuel_timer( var_0, var_1 );
    thread start_timer( var_0, var_1 );
...
start_unstable_rocket_fuel_timer( var_0, var_1 )
{
    self endon( "death_or_disconnect" );
    self endon( "juggernaut_end" );
    level endon( "game_ended" );
...
start_unlock_silo()
{
    ...
    var_0.riotshieldswitchawaytimer = 0;

    for (;;)
    {
        self waittill( "perform_hero_drop" );

        if ( !istrue( var_0.riotshieldswitchawaytimer ) )
        {
            var_0.riotshieldswitchawaytimer = 1;
            self radiusdamage( self.origin, 500, 2000, 500, self, "MOD_CRUSH" );
            thread start_swivelroom_obj( var_0 );
        }
    }
}

start_swivelroom_obj( var_0 )
{
    self endon( "death_or_disconnect" );
    self endon( "juggernaut_end" );
    level endon( "game_ended" );
...
start_silo_thrust( var_0, var_1 )
{
    ...
    {
        var_4 = 7;
        var_5 = var_3.updateteamplunderscore;
    }
...
start_safehouse_return( var_0 )
{
    return istrue( level.registeraccesscardlocs ) && var_0 == 1;
}

calculatepurchasexp()
{
    if ( istrue( level.updatematchstatushintonhasflag ) && level.activejuggernauts.size > 0 )
    {
        var_0 = 0;
        ...
        if ( var_0 >= level.updatematchstatushintonhasflag )
            return 1;
    }
