...
setup_callbacks()
{
    ...
    {
        level.bot_funcs["dropped_weapon_think"] = ::brkillchainchance;
        level.bot_funcs["dropped_weapon_cancel"] = ::brking_cleanupents;
    }

    level.bot_funcs["gametype_think"] = ::compass;
}
...
compass()
{
    ...
    if ( !scripts\mp\gametypes\arena.gsc::isnormalloadouts() )
    {
        if ( self botgetdifficultysetting( "advancedPersonality" ) && self botgetdifficultysetting( "strategyLevel" ) > 0 )
            scripts\mp\bots\bots_gametype_gun.gsc::copyplayerplundereventdata( self getcurrentweapon().basename, self.pers["bot_original_personality"] );
    }

    self.race_get_next_checkpoint = 0;
    ...
    for (;;)
    {
        if ( isdefined( level.arenaflag ) && level.arenaflag.visibleteam == "any" )
        {
            ...
            if ( !scripts\mp\bots\bots_util::bot_is_capturing() || !self.race_get_next_checkpoint )
            ...
            self.race_get_next_checkpoint = 1;
        }
        else
        {
...
brkillchainchance()
{
    ...
    for (;;)
    {
        ...
        if ( brjuggernautcratephysicsoncallback() || scripts\mp\bots\bots_util::convoy_start_5b( 0.33 ) )
        {
            if ( self [[ level.bot_funcs["should_pickup_weapons"] ]]() && !scripts\mp\bots\bots_util::bot_is_remote_or_linked() )
            {
                ...
                {
                    var_4 = var_3[0];
                    level.lb_dmg_factor_tail_rotor = var_4;
                    brkickedfromplane( var_4 );
                }
            }
        }
...
brkickedfromplane( var_0 )
{
    if ( scripts\mp\bots\bots_strategy::bot_has_tactical_goal( "seek_dropped_weapon", var_0 ) == 0 )
    {
        ...
        if ( isdefined( var_0.cprooftopcratecapturecallback ) )
            var_13 = var_0.cprooftopcratecapturecallback;
...
{
    if ( brjuggernautcratedestroycallback() > 0 )
    {
...
brjuggernautcratephysicsoncallback()
{
    var_0 = undefined;
