...
setup_callbacks()
{
    level.bot_funcs["gametype_think"] = ::covertransitionrate;
    level.bot_funcs["should_start_cautious_approach"] = ::_id_12B04;
}
...
setup_bot_koth()
{
    ...
    if ( !var_0 )
    {
        ...
        level.convoy4_mortar_guys[var_5.trigger getentitynumber()] = 1;
        level.bot_gametype_precaching_done = 1;
        thread completed_areas( var_5 );
    }
}
...
completed_areas( var_0 )
{
    for ( var_1 = scripts\engine\utility::array_remove( level.objectives, var_0 ); var_1.size > 0; var_1 = scripts\engine\utility::array_remove( var_1, var_2 ) )
    {
        ...
        level.convoy4_mortar_guys[var_2.trigger getentitynumber()] = 1;
    }
}
...
covertransitionrate()
{
    ...
    for (;;)
    {
        ...
        if ( !isdefined( level.zone ) || !isdefined( level.convoy4_mortar_guys[level.zone.trigger getentitynumber()] ) )
        {
        ...
        if ( !istrue( level.zone.trigger.trigger_off ) && isdefined( level.zoneendtime ) && !isdefined( var_0 ) && !level.zonerandomlocationorder && level.correctcodeentered )
        {
            ...
            if ( var_2 > 0 && var_2 < 10000 )
            {
                ...
                if ( !var_3 )
                {
                    var_4 = level.zone._id_13E28.radius * 6;

                    if ( var_2 < 5000 )
                        var_4 = level.zone._id_13E28.radius * 3;

                    var_5 = distance( level.zone._id_13E28.center, self.origin );
                ...
                else
                {
                    ...
                    var_8 = convoy_turret( level.zone );
        ...
        if ( !countdowntimer( var_9 ) )
            completesmokinggunquest( var_9 );
    }
}
...
convoy_turret( var_0 )
{
    return cop_car_initdamage( var_0 ).size;
}

cop_car_initdamage( var_0 )
{
    var_1 = [];
    ...
    foreach ( var_3 in level.participants )
    {
        if ( var_3 != self && scripts\mp\utility\entity::isteamparticipant( var_3 ) && istestclient( self, var_3 ) )
        {
            ...
            {
                if ( !isai( var_3 ) || var_3 countdowntimer( var_0 ) )
...
countdowntimer( var_0 )
{
    ...
    return self.helis_assault3_hangar == var_0;
}

completesmokinggunquest( var_0 )
{
