...
control_station( var_0, var_1, var_2 )
{
    var_3 = controlledcallbacksqueue( "attacker", var_0 );

    foreach ( var_5 in level.players )
    {
        if ( !isai( var_5 ) && isdefined( var_5.team ) && var_5.team == var_0 )
        {
            if ( var_5 controls_unlink_safe() || distancesquared( var_1, var_5.origin ) > squared( var_2 ) )
...
control_station_interact( var_0, var_1, var_2 )
{
    var_3 = controlledcallbacksqueue( "defender", var_0 );

    foreach ( var_5 in level.players )
    {
        if ( !isai( var_5 ) && isdefined( var_5.team ) && var_5.team == var_0 )
        {
            if ( var_5 controlslinked() || distancesquared( var_1, var_5.origin ) <= squared( var_2 ) )
                var_3 = scripts\engine\utility::array_add( var_3, var_5 );
        }
    }
...
{
    if ( isdefined( level.controls_linkto_safe ) )
        return self [[ level.controls_linkto_safe ]]();
...
controlslinked()
{
    if ( isdefined( level.convert_remaining_to_ai ) )
        return self [[ level.convert_remaining_to_ai ]]();

    return 0;
}

convoy4_module_snipers( var_0 )
{
    self.role = var_0;
...
{
    var_0 = [[ level.consecutive_kills ]]( self.team );
    var_1 = [[ level.console_being_hacked ]]( self.team );
    var_2 = [[ level.control_room_button ]]( self.team );
    var_3 = [[ level.control_room_button_green ]]( self.team );
    ...
    if ( var_4 == "active" )
    {
        if ( var_0.size >= var_2 )
        {
            ...
            {
                if ( isai( var_7 ) && level.bot_personality_type[var_7.personality] == "stationary" && var_7 completepayloadexfilobjective() )
                {
            ...
            if ( var_5 )
            {
                convoy4_module_snipers( "attacker" );
                return;
            }

            convoy4_module_snipers( "defender" );
            return;
        }
        else
            convoy4_module_snipers( "attacker" );
    ...
    else if ( var_4 == "stationary" )
    {
        if ( var_1.size >= var_3 )
        {
            ...
            {
                if ( isai( var_10 ) && level.bot_personality_type[var_10.personality] == "active" && var_10 completeobjstr() )
                {
...
contributingplayers()
{
    ...
    for (;;)
    {
        if ( gettime() > var_1 )
        {
            ...
            {
                var_4 = [[ level.consecutive_kills ]]( var_3 );
                var_5 = [[ level.console_being_hacked ]]( var_3 );
                var_6 = [[ level.control_room_button ]]( var_3 );
                var_7 = [[ level.control_room_button_green ]]( var_3 );
                ...
                if ( var_4.size > var_6 )
                {
                    ...
                    {
                        if ( isai( var_11 ) && var_11 completepayloadexfilobjective() )
                        {
                            if ( level.bot_personality_type[var_11.personality] == "stationary" )
                            {
                                var_11 convoy4_module_snipers( "defender" );
                                var_9 = 1;
                                break;
                            }
                            else
                    ...
                    if ( !var_9 && var_8.size > 0 )
                        scripts\engine\utility::random( var_8 ) convoy4_module_snipers( "defender" );
                }
                ...
                if ( var_5.size > var_7 )
                {
                    ...
                    {
                        if ( isai( var_16 ) && var_16 completeobjstr() )
                        {
                            if ( level.bot_personality_type[var_16.personality] == "active" )
                            {
                                var_16 convoy4_module_snipers( "attacker" );
                                var_14 = 1;
                                break;
                            }
                            else
                    ...
                    if ( !var_14 && var_13.size > 0 )
                        scripts\engine\utility::random( var_13 ) convoy4_module_snipers( "attacker" );
                }
            }
        }
...
completeobjstr()
{
    if ( isdefined( level.contestedtime ) )
        return self [[ level.contestedtime ]]();
...
{
    if ( isdefined( level.contract_death_cash_flag ) )
        return self [[ level.contract_death_cash_flag ]]();
...
completedmounttut( var_0 )
{
    ...
    foreach ( var_5 in var_0 )
    {
        ...
        var_5._id_13E28 = enemygunship_firerounds( var_5 );
        var_5.friendlystatusdirty = _id_13E2C( var_5, 0, 0 );
        var_7 = [ ( 0, 0, 0 ), ( 1, 1, 0 ), ( 1, -1, 0 ), ( -1, 1, 0 ), ( -1, -1, 0 ) ];

        foreach ( var_9 in var_7 )
        {
            var_10 = _id_13E2C( var_5, var_9[0], var_9[1] );
...
{
    var_1 = spawnstruct();
    var_1.van_stop_vo = ( 999999, 999999, 999999 );
    var_1.updatecollectionuiforplayer = ( -999999, -999999, -999999 );

    foreach ( var_3 in var_0.nodes )
    {
        var_1.van_stop_vo = ( min( var_3.origin[0], var_1.van_stop_vo[0] ), min( var_3.origin[1], var_1.van_stop_vo[1] ), min( var_3.origin[2], var_1.van_stop_vo[2] ) );
        var_1.updatecollectionuiforplayer = ( max( var_3.origin[0], var_1.updatecollectionuiforplayer[0] ), max( var_3.origin[1], var_1.updatecollectionuiforplayer[1] ), max( var_3.origin[2], var_1.updatecollectionuiforplayer[2] ) );
    }

    var_1.center = ( ( var_1.van_stop_vo[0] + var_1.updatecollectionuiforplayer[0] ) / 2, ( var_1.van_stop_vo[1] + var_1.updatecollectionuiforplayer[1] ) / 2, ( var_1.van_stop_vo[2] + var_1.updatecollectionuiforplayer[2] ) / 2 );
    var_1.register_valid_gametypes_for_create_script = ( var_1.updatecollectionuiforplayer[0] - var_1.center[0], var_1.updatecollectionuiforplayer[1] - var_1.center[1], var_1.updatecollectionuiforplayer[2] - var_1.center[2] );
    var_1.radius = max( var_1.register_valid_gametypes_for_create_script[0], var_1.register_valid_gametypes_for_create_script[1] );
    return var_1;
}

_id_13E2C( var_0, var_1, var_2 )
{
    var_3 = ( var_0._id_13E28.center[0] + var_1 * var_0._id_13E28.register_valid_gametypes_for_create_script[0], var_0._id_13E28.center[1] + var_2 * var_0._id_13E28.register_valid_gametypes_for_create_script[1], 0 );
    var_4 = undefined;
    var_5 = 9999999;
...
col_removelocaleinstance( var_0, var_1, var_2, var_3 )
{
    var_4 = 0;
    ...
    {
        childthread watchherodrop();
        childthread watchheatreduction();
    }
