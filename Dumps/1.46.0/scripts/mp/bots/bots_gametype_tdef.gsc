...
main()
{
    setup_callbacks();
    _id_12A0E();
    thread vehicle_stop_moving();
}

setup_callbacks()
{
    level.bot_funcs["gametype_think"] = ::crate_obj_offset;
}
    ...
    for (;;)
    {
        ...
        if ( self.role == "carrier" )
        {
            ...
            else
            {
                ...
                if ( !isdefined( var_3 ) )
                {
                    var_4 = convoy_handle_stuck_compromise();
        ...
        {
            var_6 = level.init_relic_nobulletdamage.carrier;
            var_7 = var_6.origin;
...
setupblueprintpickupweapons()
{
    ...
    var_2 = new_rider_combat_logic( self.team );
    var_3 = newhitlocs( self.team );
    ...
    if ( var_4 == "active" )
    {
        if ( var_0.size >= var_2 )
        {
            ...
            if ( var_5 )
            {
                next_threshold( "attacker" );
                return;
            }

            next_threshold( "defender" );
            return;
        }
        else
            next_threshold( "attacker" );
...
connectedplayercount()
{
    ...
    for (;;)
    {
        ...
        {
            var_4 = new_rider_combat_logic( var_3 );
            var_5 = newhitlocs( var_3 );
            ...
            if ( var_6.size > var_4 )
            {
                ...
                foreach ( var_11 in var_6 )
                {
                    if ( isai( var_11 ) )
                    {
                        ...
                        {
                            var_11 next_threshold( "defender" );
                            var_9 = 1;
                            break;
                        }
                        else
                ...
                if ( !var_9 && var_8.size > 0 )
                    scripts\engine\utility::random( var_8 ) next_threshold( "defender" );
            }
            ...
            if ( var_7.size > var_5 )
            {
                ...
                foreach ( var_16 in var_7 )
                {
                    if ( isai( var_16 ) )
                    {
                        ...
                        {
                            var_16 next_threshold( "attacker" );
                            var_14 = 1;
                            break;
                        }
                        else
                ...
                if ( !var_14 && var_13.size > 0 )
                    scripts\engine\utility::random( var_13 ) next_threshold( "attacker" );
            }

            var_18 = coop_handle_invuln_ac130();

            if ( isdefined( var_18 ) )
            {
                var_19 = _id_11C2D( var_3, var_18 );
                ...
                if ( isdefined( var_19 ) && isdefined( var_19.role ) && var_19.role != "carrier" )
                {
                    if ( !isdefined( var_19.carryobject ) )
                    {
                        ...
                        if ( isdefined( var_20 ) )
                            var_20 next_threshold( undefined );

                        var_19 next_threshold( "carrier" );
...
vehicle_stop_moving()
{
    ...
    for (;;)
    {
        var_1 = convoy_handle_stuck_compromise();
