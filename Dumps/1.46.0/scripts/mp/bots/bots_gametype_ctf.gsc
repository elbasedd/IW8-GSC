...
setup_callbacks()
{
    ...
    level.bot_funcs["get_watch_node_chance"] = ::computer_objective;
}

setup_bot_ctf()
{
    level.control_room_button = ::heli_watch_for_fly_away;
    level.control_room_button_green = ::heli_yaw;
    level.consecutive_kills = ::get_allied_attackers_for_team;
    level.console_being_hacked = ::get_allied_defenders_for_team;
    level.contract_death_cash_flag = ::computer_force_player_to_exit;
    level.controls_linkto_safe = ::computer_prompt_pressed;
...
crate_can_use( var_0 )
{
    ...
    if ( computermakingnose() )
        return 0;

    return computer_interface_think_internal( self.team );
}
...
bot_ctf_think()
{
    ...
    for (;;)
    {
        ...
        if ( !isdefined( self.role ) )
            scripts\mp\bots\bots_gametype_common.gsc::convoy4_actively_hacking();

        var_2 = scripts\mp\utility\game::getotherteam( self.team )[0];

        if ( computer_interface_think_internal( self.team ) )
        {
            if ( isdefined( level.coverreloadnotetrackhandler ) && isdefined( level.coverreloadnotetrackhandler[var_2] ) )
                level.coverreloadnotetrackhandler[var_2] = undefined;
        }
        ...
        if ( self.role == "attacker" )
        {
            if ( computermakingnose() )
                var_3 = 1;
            else if ( !computer_interaction_start() )
                var_3 = distancesquared( self.origin, level.teamflags[var_2].curorigin ) < squared( get_flag_protect_radius() );
        }
        else if ( !computer_interface_think_internal( self.team ) )
            var_3 = !computer_player_listener();
        ...
        if ( computermakingnose() )
        {
            ...
            else if ( scripts\mp\bots\bots_gametype_common.gsc::controlledcallbacks( self.team ) == 1 )
...
computerrebootsequence_init()
{
    ...
    else if ( self botcanseeentity( var_2 ) )
    {
        ...
        if ( self botgetdifficultysetting( "strategyLevel" ) > 0 && !computer_interface_think_internal( self.team ) )
        {
            if ( !isdefined( level.coverreloadnotetrackhandler ) )
                level.coverreloadnotetrackhandler = [];

            if ( !isdefined( level.coverreloadnotetrackhandler[var_2.team] ) )
                level.coverreloadnotetrackhandler[var_2.team] = [];

            level.coverreloadnotetrackhandler[var_2.team]["origin"] = var_0;
            level.coverreloadnotetrackhandler[var_2.team]["time"] = gettime();
        }
    }
    ...
    else
    {
        ...
        {
            if ( isdefined( level.coverreloadnotetrackhandler ) && isdefined( level.coverreloadnotetrackhandler[var_2.team] ) )
            {
                var_4 = gettime() - level.coverreloadnotetrackhandler[var_2.team]["time"];

                if ( var_4 < 10000 )
                    var_3 = level.coverreloadnotetrackhandler[var_2.team]["origin"];
            }
        }
...
{
    var_1 = scripts\mp\bots\bots_gametype_common.gsc::controlledcallbacks( var_0 );

    if ( var_1 == 1 )
        return 1;

    if ( computer_interface_think_internal( var_0 ) )
        var_2 = var_1 * 0.67;
    else
    {
        var_3 = scripts\mp\utility\game::getotherteam( var_0 )[0];

        if ( computer_interface_think_internal( var_3 ) )
            var_2 = var_1 * 0.5;
        else
            var_2 = var_1 * 0.4;
    }
...
{
    var_1 = scripts\mp\bots\bots_gametype_common.gsc::controlledcallbacks( var_0 );
    return var_1 - heli_watch_for_fly_away( var_0 );
}

get_allied_attackers_for_team( var_0 )
{
    return scripts\mp\bots\bots_gametype_common.gsc::control_station( var_0, level.capzones[var_0].curorigin, get_flag_protect_radius() );
}

get_allied_defenders_for_team( var_0 )
{
    return scripts\mp\bots\bots_gametype_common.gsc::control_station_interact( var_0, level.capzones[var_0].curorigin, get_flag_protect_radius() );
}
...
computer_debugtestloop()
{
    ...
    level endon( "game_ended" );
    thread scripts\mp\bots\bots_gametype_common.gsc::contributingplayers();
}
