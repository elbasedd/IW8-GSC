...
setup_callbacks()
{
    level.bot_funcs["gametype_think"] = ::confirm_good_pickup_location;
...
setup_bot_dd()
{
    ...
    var_0 = scripts\mp\bots\bots_gametype_common.gsc::cratecleanup( [ "_a", "_b" ] );
...
confirm_good_pickup_location()
{
    ...
    for (;;)
    {
        ...
        if ( isdefined( self.current_bombzone ) && !col_localethink_itemspawn( self.current_bombzone ) )
        {
            self.current_bombzone = undefined;
            concussionused();
        }
        ...
        if ( var_0 )
        {
            cpoperationcratecapturecallback( "attack" );
        ...
        cpoperationcratecapturecallback( "defend" );
...
notify_enemy_team_bomb_used( var_0 )
{
    ...
    var_2 = scripts\mp\bots\bots_gametype_common.gsc::parachutecleanup( var_0 );

    foreach ( var_4 in var_2 )
    {
        if ( isdefined( var_4.current_bombzone ) && var_1 == var_4.current_bombzone )
            var_4 concusspushstart( "investigate_someone_using_bomb" );
    }
}
...
plant_bomb()
{
    ...
    var_0 = scripts\mp\bots\bots_gametype_common.gsc::patrollocation( self.current_bombzone, 0 );
    ...
    if ( var_1 == "goal" )
    {
        ...
        var_6 = scripts\mp\bots\bots_gametype_common.gsc::col_removelocaleinstance( level.planttime + 2, "bomb_planted", var_5 );
...
defuse_bomb()
{
    ...
    var_0 = scripts\mp\bots\bots_gametype_common.gsc::patrolfunc( self.current_bombzone ).origin;
    ...
    if ( var_1 == "goal" )
    {
        ...
        var_10 = scripts\mp\bots\bots_gametype_common.gsc::col_removelocaleinstance( level.defusetime + 2, "bomb_defused", var_9 );
...
{
    var_1 = play_nag_players_hvt_callouts( var_0, self.team );
...
col_localethink_itemspawn( var_0 )
{
...
parachute_get_path()
{
    ...
    foreach ( var_2 in level.objectives )
    {
        if ( col_localethink_itemspawn( var_2 ) )
...
{
    var_2[0] = play_nag_players_hvt_callouts( var_0[0], game["defenders"] ).size;
    var_2[1] = play_nag_players_hvt_callouts( var_0[1], game["defenders"] ).size;
...
cpoperationcratecapturecallback( var_0 )
{
    ...
    if ( var_0 == "attack" )
        var_1 = completex1finquest();
    else if ( var_0 == "defend" )
        var_1 = compound_objective();
...
compound_objective()
{
    ...
    else if ( var_0.size == 2 )
    {
        ...
        if ( var_3[0] && var_3[1] || !var_3[0] && !var_3[1] )
        {
            ...
            var_1 = cpcpammoarmorcratecapturecallback( var_0, var_4 );
...
completex1finquest()
{
    ...
    if ( !isdefined( level.current_zone_target ) || !col_localethink_itemspawn( level.current_zone_target ) || gettime() > level.next_target_switch_time )
    {
        level.next_target_switch_time = gettime() + 1000 * randomintrange( 30, 45 );
        level.current_zone_target = scripts\engine\utility::random( parachute_get_path() );
    }
    ...
    var_1 = play_fulton_evac_on_player( var_0 );
    self.current_bombzone = undefined;
...
crate_set_flag_on_use()
{
    var_0 = play_fulton_evac_on_player( self.current_bombzone );
...
completetimedrunquest()
{
    ...
    {
        var_1 = play_lighting_sequence( self.current_bombzone );
...
completex1stashquest()
{
    ...
    if ( is_bomb_planted_on( self.current_bombzone ) )
    {
        var_1 = play_incoming_rpg_vo( self.current_bombzone );
    ...
    for (;;)
    {
        foreach ( var_1 in level.objectives )
        {
            foreach ( var_3 in level.players )
            {
                ...
                {
                    if ( !col_localethink_itemspawn( var_1 ) )
                    {
                        if ( var_3.role == "atk_bomber" || var_3.role == "defuser" )
                            var_3 concussionused();
