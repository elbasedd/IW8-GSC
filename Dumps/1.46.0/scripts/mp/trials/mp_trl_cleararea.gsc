...
init()
{
    ...
    level.attempttier = 0;
    level.updatespecificfobindanger = 59999900;
    level.modeonspawnplayer = ::_id_11DC6;
    ...
    thread _id_1302F();
    level.bot_loadout_choose_from_custom_default_class = getallnodes();
}
...
trial_start_init()
{
    ...
    if ( istrue( level._id_134CF ) )
    {
        level.migratespectators = getentarray( "explosive_barrel", "targetname" );
        level.milestonephasepercent_drops = getentarray( "explosive_car", "targetname" );
        level.br_latejoininfilready = getentarray( "ammo_crate", "targetname" );
        scripts\engine\utility::array_thread( level.milestonephasepercent_drops, ::is_cs_scriptable );
        scripts\engine\utility::array_thread( level.migratespectators, ::cargo_truck_mg_deathcallback );
        scripts\engine\utility::array_thread( level.br_latejoininfilready, ::br_itemrarity );
    }
    ...
    {
        if ( isdefined( var_2 ) )
            var_0.getquestxprewardinstance = var_2;

        if ( isdefined( var_3 ) )
            var_1.getquestxprewardinstance = var_3;
    }
    else
    {
    ...
    if ( isdefined( level._id_134E7 ) )
    {
        foreach ( var_13 in level._id_134E7 )
    ...
    if ( isdefined( var_0.getquestxprewardinstance ) )
        var_0.getquestxprewardinstance delete();

    if ( isdefined( var_1.getquestxprewardinstance ) )
        var_1.getquestxprewardinstance delete();
}
...
_id_11DC6()
{
    ...
    scripts\engine\utility::delaythread( 1, ::_id_11D7F );
    scripts\engine\utility::delaythread( 1, ::_id_11D60 );
}
...
_id_11D7F()
{
    if ( isdefined( level._id_11D7B ) )
        level.player scripts\mp\equipment::giveequipment( level._id_11D7B, "primary" );

    if ( isdefined( level._id_11D7D ) )
        level.player scripts\mp\equipment::giveequipment( level._id_11D7D, "secondary" );

    if ( istrue( level._id_11D7C ) )
        thread _id_1233E();
}
...
races()
{
    ...
    scripts\engine\utility::array_thread( var_0, ::race_wait_for_current_checkpoint );
}
...
enemies_init()
{
    ...
    thread logevent_challengeitemunlocked();
    thread lockpropkey();
}
...
logevent_challengeitemunlocked()
{
...
enemy_move_and_cover( var_0 )
{
    ...
    if ( var_2.size == 0 )
    {
        ...
        if ( var_3.size > 0 )
        {
            ...
            var_5 = scripts\engine\utility::getclosest( var_4.origin, level.bot_loadout_choose_from_custom_default_class, 500 );
            var_2[0] = var_5;
        }
        else
            var_2[0] = scripts\engine\utility::getclosest( var_0.origin, level.bot_loadout_choose_from_custom_default_class, 1000 );
    }
    ...
    while ( isalive( var_0 ) )
    {
        if ( isdefined( var_6 ) )
        {
            ...
            var_0 scripts\engine\utility::_id_13ADB( "goal", "badpath", "grenade danger", "bullet_whizby" );
            var_8 = var_6 sixthsense_inotherplayertargetcone();
        }
        ...
        var_10 = var_0 loop_emp_spark_vfx( var_6 );
        ...
        if ( var_7 && var_10 && !var_9 )
        {
            ...
            for ( var_10 = var_0 loop_emp_spark_vfx( var_6 ); var_11 && var_10; var_11 = var_0 iscovervalidagainstenemy( var_6 ) )
                wait 1;
        }
        ...
        if ( var_14.size > 0 )
        {
            ...
            var_7 = var_6 sixthsense_inotherplayertargetcone();
            continue;
        }
...
sixthsense_inotherplayertargetcone()
{
...
enemy_soldier_think()
{
    ...
    while ( isalive( self ) )
    {
        ...
        level.player thread _maphint_offerscriptableused::_id_134D9( self, var_12, 0, 1 );
...
enemy_monitor_death( var_0 )
{
    ...
    if ( var_12 && var_11 )
    {
        ...
        level.player thread _maphint_offerscriptableused::_id_134D9( self, 1, 0, 1 );
    }
...
hud_init()
{
    _maphint_offerscriptableused::trial_ui_set_reward_tier( game["trial"]["best_reward"] );
...
hud_objectives()
{
    _maphint_offerscriptableused::trial_ui_set_objective_icon_index( 0 );
    _maphint_offerscriptableused::trial_ui_set_objective_progress( level.enemieskilled, level.enemiestotal );
    _maphint_offerscriptableused::trial_ui_set_stat_and_bonus_time( 1, "enemies_killed", level.enemieskilled, 0 );
...
hud_timer()
{
    ...
    _maphint_offerscriptableused::trial_ui_set_main_time( 0 );
    _maphint_offerscriptableused::trial_ui_set_subtime( 0 );
    level scripts\engine\utility::flag_wait( "trial_starting" );
    ...
    while ( !scripts\engine\utility::flag( "trial_completed" ) )
    {
        ...
        _maphint_offerscriptableused::trial_ui_set_main_time( level.totaltimeelapsed );
        _maphint_offerscriptableused::trial_ui_set_subtime( level.totaltimeelapsed );
        wait 0.05;
    }
...
hud_reward_tiers_tracking()
{
    ...
    for ( var_1 = 3; var_1 >= 0; var_1-- )
    {
        ...
        _maphint_offerscriptableused::trial_ui_set_reward_tier_preview( var_1 );
    ...
    while ( level.totaltimeelapsed < level.updatespecificfobindanger )
        wait 0.05;

    level notify( "max_time_limit_reached" );
    level scripts\engine\utility::flag_set( "trial_ready_for_endscreen" );
    _maphint_offerscriptableused::trial_ui_set_main_time( level.updatespecificfobindanger );
    _maphint_offerscriptableused::trial_ui_set_subtime( level.updatespecificfobindanger );
}
...
hud_attempt_over()
{
    ...
    if ( !scripts\engine\utility::flag( "trial_player_death" ) )
    {
        ...
        if ( level.attempttier > var_0 )
        {
            ...
            _maphint_offerscriptableused::trial_ui_set_reward_tier( level.attempttier );
        }
    ...
    {
        _maphint_offerscriptableused::trial_ui_set_reward_tier_preview( 0 );
    ...
    _maphint_offerscriptableused::_id_13517( 1 );
    _maphint_offerscriptableused::trial_ui_set_stat_and_bonus_time( 1, "enemies_killed", level.enemieskilled, 0 );
    _maphint_offerscriptableused::trial_ui_open_results_screen();
    level._id_134EE = 1;
    ...
    if ( var_3 > 0 )
    {
        ...
        game["state"] = "playing";
        _maphint_offerscriptableused::_id_134EC();
    }
    else
    {
...
hud_besttime_update()
{
    ...
    _maphint_offerscriptableused::trial_ui_set_best_time( var_0 );
    _maphint_offerscriptableused::trial_ui_set_reward_tier( var_1 );
}
...
_id_12341( var_0 )
{
    ...
    _id_12340( var_0, "primary" );
    _id_12340( var_0, "secondary" );
    _id_12342( var_0 );
}
...
cargo_truck_mg_deathcallback()
{
    ...
    if ( isdefined( self.script_noteworthy ) && !istrue( level.started ) )
    {
        if ( self.script_noteworthy == "big_explosion" )
        {
            ...
            level thread _blankfunc::nuke_explosion( undefined, var_11 );
            level thread _blankfunc::nuke_earthquake( undefined, var_11 );
        ...
        playfx( scripts\engine\utility::getfx( _id_122AD() ), self.origin );
