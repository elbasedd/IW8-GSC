...
laststandheal_onset()
{
    ...
    for (;;)
    {
        if ( self.laststandheal_hassuperweapon && ( !istrue( self.inlaststand ) || istrue( self.stuckinlaststand ) || istrue( scripts\mp\playerlogic::playerdatafield() ) ) )
        {
        ...
        else if ( !self.laststandheal_hassuperweapon && istrue( self.inlaststand ) && !istrue( self.stuckinlaststand ) && !istrue( scripts\mp\playerlogic::playerdatafield() ) )
        {
            var_0 = scripts\mp\supers::getcurrentsuper().staticdata.weapon;
...
laststandheal_setinactivewhendone()
{
    level endon( "game_ended" );
    self endon( "disconnect" );
    scripts\engine\utility::_id_13AE8( "death", "last_stand_finished" );
