...
logendofround()
{
    var_0 = scripts\mp\gametypes\br_plunder.gsc::_id_11AA6( "equip_mp_fulton", 1 );
    var_0.type = 0;
    var_0.usetime = getdvarfloat( "scr_fultonUseTime", 0.75 );
    var_0._id_1310B = 2;
    var_0._id_13109 = getdvarint( "scr_fultonUseAmount", 250 );
    var_0._id_12BF3 = 1;
    var_0._id_12BCE = 1;
    var_0._id_1310C = "MP/CANNOT_DEPOSIT_CASH_BALLOON_FULL";
    var_0._id_1310D = "MP/CANNOT_DEPOSIT_CASH_BALLOON_LEAVING";
    var_0._id_121BE = "fulton_use_cache";
    var_0._id_121BF = "usable";
    var_0._id_121B8 = "unusable";
    var_0.jugg_go_to_node_callback = getdvarint( "scr_fultonCountdown", 20 );
    var_0.jugg_health_debug = "MP/CASH_BALLOON_LEAVING_IN_N";
    var_0.dmzwincost = getdvarint( "scr_fultonCapacity", 1500 );
    var_0.gas_badplace = ::logevent_munitionused;
    var_0.jugg_get_priority_player = ::logevent_playerregen;
    var_0.brcirclecoughnexttime = ::logevent_kill;
    var_0.jugg_source = "fulton";
    var_0.dropplunder = getdvarint( "scr_fultonDropPlunder", 1 ) > 0;
    var_0.dmzlootleaderupdateonpickup = getdvarint( "scr_fultonCanTakeDamage", 1 ) > 0;
}

locked_proxylod( var_0 )
{
    ...
    self.location_tracker = var_1;
    var_1 thread logevent_givecpweaponxp();
}

locknonbunkerdoors( var_0 )
{
    var_1 = scripts\mp\gametypes\br_plunder.gsc::_id_11AA6( "equip_mp_fulton" );
    thread scripts\mp\gametypes\br.gsc::_id_12BEE( "br_fulton_balloon_shot_down", self.owner, self.team );
    scripts\mp\gametypes\br_plunder.gsc::isbulletpenetration( var_1.dropplunder );
...
lockedradialunfill()
{
    ...
    if ( isdefined( self.owner ) )
        self.owner.location_tracker = undefined;
    ...
    scripts\mp\gametypes\br_plunder.gsc::_id_11AA0( self );
...
logevent_spawnviaautorevive( var_0 )
{
    ...
    if ( isdefined( var_1 ) )
    {
        ...
        if ( getdvarint( "br_sticky_fulton", 1 ) == 1 )
            locationtriggerupdate( var_0, var_1 );
    ...
    if ( !locationtriggersetpaused( var_0 ) )
    {
        self playlocalsound( "br_pickup_deny" );
    ...
    else if ( 1 && isdefined( self.location_tracker ) )
    {
        self playlocalsound( "br_pickup_deny" );
...
locationtriggerupdate( var_0, var_1 )
{
    if ( isdefined( var_1.showtozombies ) )
    {
        if ( isdefined( var_1.showtozombies.targetname ) )
        {
            if ( var_1.showtozombies.targetname == "train_wz" )
...
logevent_givecpweaponxp()
{
    self endon( "death" );
    self endon( "start_extract" );
    playsoundatpos( self.origin, "fulton_bag_drop" );
    thread scripts\mp\gametypes\br.gsc::_id_12BEE( "br_fulton_device_placed", self.owner, self.team );
    var_0 = scripts\mp\utility\teams::getfriendlyplayers( self.team );
    scripts\mp\gametypes\br_plunder.gsc::_id_11AB0( self, "equip_mp_fulton" );
    thread scripts\mp\gametypes\br_plunder.gsc::_id_11ABE( self, var_0 );
    scripts\mp\gametypes\br_plunder.gsc::_id_11AC4( self, var_0 );
    var_1 = spawn( "script_model", self.origin );
    var_1 setmodel( "tag_origin" );
    var_1.angles = self.angles * ( 0, 1, 0 );
    var_1.location_tracker = self;
    ...
    var_2 = scripts\mp\gametypes\br_plunder.gsc::_id_11AA6( "equip_mp_fulton", 1 );

    if ( var_2.dmzlootleaderupdateonpickup )
        thread scripts\mp\damage::monitordamage( 500, "", ::lockscriptabledoors, ::lockpropkey );
}
...
{
    thread locknonbunkerdoors( var_0 );
}

logevent_servermatchend( var_0, var_1, var_2 )
{
    var_3 = scripts\mp\gametypes\br_plunder.gsc::_id_11AA6( "equip_mp_fulton" );
    var_1 thread scripts\mp\gametypes\br_plunder.gsc::_id_11958( var_2, var_3._id_1310B, var_0 );
}

logevent_munitionused( var_0, var_1 )
{
    if ( isdefined( var_0.owner ) )
        var_0.owner.location_tracker = undefined;

    scripts\mp\gametypes\br_plunder.gsc::_id_11AB7( var_0 );
}

logevent_playerregen( var_0 )
{
    var_0 endon( "death" );
    var_1 = scripts\mp\gametypes\br_plunder.gsc::_id_11AA6( "equip_mp_fulton" );
    var_2 = undefined;
    var_0 scripts\mp\damage::monitordamageend();
    var_2 = scripts\mp\gametypes\br_plunder.gsc::isbrsquadleader();
    thread scripts\mp\gametypes\br.gsc::_id_12BEE( "br_fulton_balloon_successfully_away", self.owner, self.team );
...
logevent_kill( var_0 )
{
    thread scripts\mp\gametypes\br.gsc::_id_12BEE( "br_fulton_balloon_full", var_0.owner, var_0.team );
}
...
logevent_kidnapevent()
{
    ...
    scripts\cp\helicopter\chopper_boss::untrack_enemy( self.super, 1 );
    var_0 = scripts\mp\supers::getcurrentsuper();
    scripts\mp\supers::_id_123DF( 0 );
    scripts\mp\supers::_id_123DE( 0 );
    var_0.wasrefunded = 1;
