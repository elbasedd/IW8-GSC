...
updatestreakcost( var_0 )
{
    ...
    if ( isdefined( var_2 ) )
    {
        ...
        if ( var_0 <= nuke_carrier_last_transform() )
        {
...
updatekillstreakuislots()
{
    ...
    for ( var_1 = 0; var_1 <= nuke_carrier_last_transform(); var_1++ )
...
updatekillstreakuislot( var_0 )
{
    ...
    if ( isdefined( level.suppressdamageflash ) && ![[ level.suppressdamageflash ]]( var_0 ) )
        return;
    ...
    if ( isdefined( var_2 ) && isdefined( var_2.streakname ) )
    {
        ...
        adjustactivespawnlogic( var_0, var_3 );
        adjust_player_pos_memory_array_size( var_0, var_2.available );
    }
    else
    {
        self setclientomnvar( var_1.slotomnvars[var_0], 0 );
        self setclientomnvar( var_1.availableomnvars[var_0], 0 );
        adjustactivespawnlogic( var_0, 0 );
        adjust_player_pos_memory_array_size( var_0, 0 );
    }
}
...
triggerkillstreak( var_0, var_1 )
{
    ...
    if ( isdefined( level.super_onspawned ) && !self [[ level.super_onspawned ]]( var_0, var_1 ) )
    {
        scripts\mp\hud_message::showerrormessage( "KILLSTREAKS/CANNOT_BE_USED" );
        return 0;
    }
...
onsuccessfulstreakactivation( var_0, var_1 )
{
    ...
    if ( istrue( level._id_135FA ) && self.streaktype != "specialist" )
        var_0.available = -1;
    else
        var_0.available = -1;

    adjust_player_pos_memory_array_size( var_1, var_0.available );
    ...
    var_0.allow_ascender_use = var_6;
    scripts\mp\analyticslog::logevent_killstreakactivated( self, var_0._id_12A6F, var_0.streakname, var_0.isgimme, var_6, self.origin );
    ...
    if ( isdefined( self.petwatch ) )
    {
        ...
        if ( var_2 == "nuke" )
            ...
            scripts\cp_mp\pet_watch::animscripted_loop_n_times();
    }
}
...
createstreakitemstruct( var_0 )
{
    ...
    var_1.isspecialist = scripts\mp\perks\perks::saw_head_icon( var_0 );
    var_1._id_1284E = scripts\mp\perks\perks::getspecialistperkforstreak( var_0 );
    return var_1;
}
...
makekillstreakavailable( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    if ( isdefined( var_5._id_1284E ) )
        var_6 = var_5._id_1284E;
    ...
    var_5.available = 1;
    adjust_player_pos_memory_array_size( var_0, var_5.available );
    ...
    var_5._id_12A6F = self.matchdatalifeindex;
    var_5.owner = self;
    var_5.useweaponpickups = self getxuid();

    if ( isdefined( var_2 ) )
        var_5.streaklifeid = var_2;

    if ( isdefined( var_3 ) )
        var_5._id_12A6F = var_3;

    if ( isdefined( var_4 ) )
    {
        var_5.owner = var_4;
        var_5.useweaponpickups = var_4 getxuid();
    }

    scripts\cp\helicopter\chopper_boss::unreachable_function( var_5.streakname, var_1 );
    scripts\mp\analyticslog::logevent_killstreakavailable( self, var_5._id_12A6F, var_6, var_5.isgimme, var_5.madeavailabletime, self.origin );

    if ( isdefined( self.petwatch ) && var_0 == nuke_carrier_last_transform() )
    {
        var_8 = 0;
        var_9 = 0;
        ...
        if ( var_8 && var_9 )
            scripts\cp_mp\pet_watch::anim_override();
        else
            scripts\cp_mp\pet_watch::anim_offset();
    }
}
...
killstreakselectionwatcher()
{
    ...
    for (;;)
    {
        var_0 = scripts\engine\utility::_id_133F7( "ks_select_up", "ks_select_down" );
...
selectfirstavailablekillstreak()
{
    ...
    {
        for ( var_1 = nuke_carrier_last_transform(); var_1 >= 0; var_1-- )
        {
...
{
    var_1 = var_0;
    var_2 = scripts\engine\utility::ter_op( var_0 >= nuke_carrier_last_transform(), 0, var_0 + 1 );
    var_3 = var_0;
    var_4 = var_2;
    ...
    while ( var_4 != var_3 )
    {
        ...
        if ( var_4 > nuke_carrier_last_transform() )
            var_4 = 0;
    }
...
{
    var_1 = var_0;
    var_2 = scripts\engine\utility::ter_op( var_0 <= 0, nuke_carrier_last_transform(), var_0 - 1 );
    var_3 = var_0;
    var_4 = var_2;
    ...
    while ( var_4 != var_3 )
    {
        ...
        if ( var_4 < 0 )
            var_4 = nuke_carrier_last_transform();
    }
...
selectmostexpensivekillstreak()
{
    ...
    for ( var_2 = nuke_carrier_last_transform(); var_2 >= 0; var_2-- )
    {
...
selectnextavailablekillstreak()
{
    ...
    for ( var_2 = 0; var_2 <= nuke_carrier_last_transform(); var_2++ )
    {
        var_3 = self.streakdata.streaks[var_2];
...
registerkillstreak( var_0, var_1, var_2, var_3 )
{
    ...
    var_4.heli_carrier_logic = var_3;
}
...
checkstreakreward( var_0, var_1 )
{
    for ( var_2 = 1; var_2 <= 4; var_2++ )
    {
        ...
        if ( isdefined( var_3.lifeid ) && var_3.lifeid == self.lifeid && ( !istrue( level._id_135FA ) || istrue( level._id_135FA ) && self.streaktype == "specialist" ) || istrue( level._id_135FA ) && istrue( level.casualscorestreaks ) && self.streaktype != "specialist" && var_3.available == -1 )
            continue;
...
givestreakpoints( var_0, var_1, var_2 )
{
    ...
    if ( var_6 && !istrue( level.helipadscriptable ) && ( istrue( level.allowkillstreaks ) || isdefined( level.vehiclereserved ) ) )
    {
        var_7 = self.nukepoints + var_1;
        var_8 = calcstreakcost( "nuke" );

        if ( isdefined( level.vehiclereserved ) )
            var_8 = level.vehiclereserved;

        if ( var_7 >= var_8 )
        {
            self.nukepoints = var_8;

            if ( !istrue( self.init_track_settings ) )
            {
                thread scripts\mp\hud_message::showkillstreaksplash( "nuke", var_8 );
                awardkillstreak( "nuke", "earned" );
                self.init_track_settings = 1;
            }
        }
        else
        {
            ...
            if ( isdefined( self.petwatch ) )
                scripts\cp_mp\pet_watch::_id_1309D( self.nukepoints / var_8 );
        }
    }
...
resetstreakavailability( var_0 )
{
    ...
    for ( var_1 = 0; var_1 <= 4; var_1++ )
    {
        ...
        if ( isdefined( var_2 ) && isdefined( var_2.streakname ) )
        {
            ...
            adjust_player_pos_memory_array_size( var_1, var_2.available );
        }
    }
}
        ...
        foreach ( var_1 in level.players )
        {
            ...
            if ( isdefined( var_1.streakpoints ) )
            {
                if ( var_1 scripts\mp\utility\perk::_hasperk( "specialty_killstreak_to_scorestreak" ) )
                    var_2 = scripts\mp\perks\perkfunctions::set_player_munition_currency( var_1.streakpoints );
                else
...
_id_130B3( var_0, var_1 )
{
    if ( !isdefined( var_0 ) || !isdefined( self ) )
        return;

    if ( !isdefined( self._id_12183 ) )
        self._id_12183 = [];

    var_2 = var_0 getxuid();
    var_0.player_latespawn_safehouse = 1;

    if ( !isdefined( self._id_12183[var_2] ) )
    {
        var_3 = spawnstruct();
        var_3.damage = 0;
        var_3.player = var_0;
        self._id_12183[var_2] = var_3;
    }

    self._id_12183[var_2].damage = self._id_12183[var_2].damage + var_1;
}
...
omnvar_bit( var_0 )
{
    ...
    foreach ( var_3 in self._id_12183 )
    {
        var_4 = var_3.damage;
        var_5 = var_3.player;
...
initridekillstreak_internal( var_0 )
{
    if ( isdefined( var_0 ) && islaptoptimeoutkillstreak( var_0 ) )
        ...
        var_1 = scripts\engine\utility::_id_13405( 1.0, "death", "disconnect", "weapon_switch_started" );
    ...
    if ( !isdefined( var_0 ) || !issubstr( var_0, "pointSelect" ) )
    {
        if ( var_0 == "drone_hive" )
        {
            ...
            var_2 = scripts\engine\utility::_id_13403( 0, "death_or_disconnect" );
        }
        else
        {
            ...
            var_2 = scripts\engine\utility::_id_13403( 0.8, "death_or_disconnect" );
        }
    }
    else
        var_2 = scripts\engine\utility::_id_13403( 1.0, "death_or_disconnect" );
...
checkcasualstreaksreset()
{
    ...
    if ( isdefined( self.streaktype ) && self.streaktype == "specialist" || isdefined( self.loadoutstreaktype ) && self.loadoutstreaktype == "specialist" || !istrue( level._id_135FA ) )
    {
...
force_interrupt_current_combat_action( var_0 )
{
    if ( !scripts\mp\utility\stats::canrecordcombatrecordstats() )
        return;
