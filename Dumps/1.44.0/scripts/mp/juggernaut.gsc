...
init()
{
    ...
    level.start_game_end_timer = getdvarint( "scr_ks_jugg_team_max", 0 );
}

jugg_makejuggernaut( var_0, var_1 )
{
    var_2 = search_acceleration( var_0 );
    ...
    if ( isdefined( level.findfirstaliveplayer ) )
        self [[ level.findfirstaliveplayer ]]();
    ...
    if ( istrue( self.vip_removequestinstance ) )
    {
    ...
    else
    {
        ...
        foreach ( var_11 in var_9 )
        {
            ...
            if ( scriptedphysicaldofenabled() )
            {
                self givemaxammo( var_11 );
                thread search_target_think( var_11 );
            }
        }
    }
    ...
    if ( isdefined( level.animscripted_loop_relative ) )
        self [[ level.animscripted_loop_relative ]]();
...
jugg_createconfig( var_0, var_1 )
{
    ...
    var_2.start_safehouse_gunshop = 5;

    if ( level.gametype == "br" )
        var_2.start_safehouse_gunshop = 3;
...
searchradiusmax()
{
    if ( scriptedagentmodifieddamage() )
    {
...
search_acceleration( var_0 )
{
    ...
    if ( var_2 != "stand" )
    {
        ...
        else if ( var_2 == "crouch" )
        {
            if ( !var_3 )
            {
                var_6 = scripts\engine\trace::ray_trace_passed( self.origin, scripts\mp\utility\player::oneoffuavsweeps( "stand" ), self, var_5 );
        ...
        else if ( var_2 == "prone" )
        {
            if ( !var_4 )
            {
                if ( var_3 )
                {
                    var_7 = scripts\engine\trace::ray_trace_passed( self.origin, scripts\mp\utility\player::oneoffuavsweeps( "crouch" ), self, var_5 );
...
jugg_watchoverlaydamagestates( var_0 )
{
    ...
    for (;;)
    {
        scripts\engine\utility::_id_133EF( "damage", "jugg_health_regen" );
...
scriptedphysicaldofenabled()
{
    return istrue( self.juggcontext.juggconfig._id_13124 );
}

scriptedspawnpointarray()
{
    return istrue( self.juggcontext.juggconfig._id_13134 );
}
...
search_target_think( var_0 )
{
    ...
    thread see_equipment_dist( var_0, var_1 );
    thread sec_sys_struct_3( var_0, var_1 );
...
see_equipment_dist( var_0, var_1 )
{
    self endon( "death_or_disconnect" );
    self endon( "juggernaut_end" );
    level endon( "game_ended" );
...
scripted_fov()
{
    ...
    thread secretstashlootcacheused();
}

secretstashlootcacheused()
{
    level endon( "game_ended" );
...
see_air_killstreak_dist()
{
    self endon( "death_or_disconnect" );
    self endon( "juggernaut_end" );
    level endon( "game_ended" );
    var_0 = self.juggcontext.juggconfig;
    var_0.playersetwasingulag = 0;

    for (;;)
    {
        self waittill( "perform_hero_drop" );

        if ( !istrue( var_0.playersetwasingulag ) )
        {
            var_0.playersetwasingulag = 1;
            self radiusdamage( self.origin, 500, 2000, 500, self, "MOD_CRUSH" );
            thread searchradiusmin( var_0 );
        }
    }
}

searchradiusmin( var_0 )
{
    self endon( "death_or_disconnect" );
    self endon( "juggernaut_end" );
    level endon( "game_ended" );
...
search_turret_no_target_stop_delay()
{
    if ( self isskydiving() )
...
searchcircleorigin( var_0, var_1 )
{
    ...
    {
        var_4 = 7;
        var_5 = var_3.start_safehouse_gunshop;
    }
...
search_speed( var_0 )
{
    return istrue( level.play_hud_reminder_vo ) && var_0 == 1;
}

brjugg_initdialog()
{
    if ( istrue( level.start_game_end_timer ) && level.activejuggernauts.size > 0 )
    {
        var_0 = 0;
        ...
        if ( var_0 >= level.start_game_end_timer )
            return 1;
    }
