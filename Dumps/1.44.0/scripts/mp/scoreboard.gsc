...
setplayerscoreboardinfo()
{
    ...
    if ( var_0 < 200 )
    {
        ...
        if ( scripts\mp\utility\game::getgametype() == "br" )
        {
            var_12 = scripts\mp\gametypes\br.gsc::deposit_from_compromised_convoy_delayed( self );
            ...
            {
                var_15 = scripts\mp\gamescore::onlowpopstart();
                var_16 = var_15[self.team];
                setclientmatchdata( "players", self.clientmatchdataid, "placement", var_16 );
                var_17 = scripts\mp\gametypes\br_gametype_dmz.gsc::usingfallback();
                setclientmatchdata( "players", self.clientmatchdataid, "extrascore4", var_17 );
                var_18 = 0;

                if ( isdefined( self.spawn_maint_wave_2 ) )
                    var_18 = self.spawn_maint_wave_2;

                setclientmatchdata( "players", self.clientmatchdataid, "extrascore5", var_18 );
            }
            else if ( scripts\mp\utility\game::onfieldupgradeendbuffer() == "kingslayer" )
            {
                var_15 = scripts\mp\gamescore::onlowpopstart();
                var_16 = var_15[self.team];
                setclientmatchdata( "players", self.clientmatchdataid, "placement", var_16 );
                var_17 = scripts\mp\gametypes\br_gametype_kingslayer.gsc::usingfallback();
                setclientmatchdata( "players", self.clientmatchdataid, "extrascore4", var_17 );
                var_18 = 0;
            ...
            {
                if ( isdefined( self._id_12BE0 ) )
                    setclientmatchdata( "players", self.clientmatchdataid, "placement", self._id_12BE0 );

                var_17 = scripts\mp\gametypes\br_gametype_rebirth.gsc::cs_add_to_struct_array();
                setclientmatchdata( "players", self.clientmatchdataid, "extrascore4", var_17 );
            }
            else if ( isdefined( self._id_12BE0 ) )
                setclientmatchdata( "players", self.clientmatchdataid, "placement", self._id_12BE0 );
        }
        ...
        foreach ( var_3 in level.teamnamelist )
        {
            if ( scripts\mp\menus::shouldmodesetsquads() )
            {
                foreach ( var_5 in level.squaddata[var_3] )
                {
                    ...
                    {
                        scripts\mp\gamelogic::branalytics_missionend( var_8 );
