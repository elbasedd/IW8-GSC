...
main()
{
    ...
    level.initoperatorunlocks = getdvarint( "scr_arm_enableLittleBirdMG", 0 );
    ...
    level._id_12B8D = getdvarint( "scr_arm_tankRespawnTime", 120 );
    level.bot_get_stored_custom_classes = getdvarint( "scr_arm_apcRespawnTime", 120 );
    level.longdialoguecooldown = getdvarint( "scr_arm_longDialogueCooldown", 1 );
    level.startusingbomb = getdvarint( "scr_arm_mercyEndingNuke", 1 );
    level.startvipteamuav = getdvarint( "scr_arm_mercyEndingTime", 30 );
    level.trial_white_phosphorus = getdvarint( "scr_arm_numNonRallyVehicles", 25 );
    level.helipadscriptable = getdvarint( "scr_arm_disablePersonalNuke", 0 );
    level.vehiclereserved = getdvarint( "scr_personalNukeCostOverride", 30 );
    level._id_1255B = getdvarint( "scr_arm_showTeamTanks", 1 );
    level._id_1255A = getdvarint( "scr_arm_showTeamLittleBirds", 1 );
    level._id_1281C = getdvarint( "scr_arm_spawnSelectionAFKTime", 120 );
    level.spawnziptie = getdvarint( "scr_arm_manualAdjustLittleBirdLocs", 1 );
    level.incorrectswitch = getdvarint( "scr_arm_dontSpawnJeep", 0 );
    level.incorrectcodeentered = getdvarint( "scr_arm_dontSpawnCargoTruck", 0 );
    level._id_1311E = getdvarint( "scr_arm_usePingSystem", 1 );

    if ( istrue( level._id_1255B ) )
    {
        level._id_12D3D = [];
        level._id_12D3D["axis"] = [];
        level._id_12D3D["allies"] = [];
    }

    if ( istrue( level._id_1255A ) )
    {
        level._id_12D3C = [];
        level._id_12D3C["axis"] = [];
        level._id_12D3C["allies"] = [];
        level._id_12D3C["untouched"] = [];
    }
...
onstartgametype()
{
    ...
    if ( istrue( level.startusingbomb ) )
        level thread trial_enemy_quota();

    initcpammoarmorcrate();

    if ( istrue( level._id_1311E ) )
...
initcpammoarmorcrate()
{
    ...
    scripts\cp_mp\utility\game_utility::_id_11ED3( "vehicle_volume", "script_noteworthy" );
    scripts\cp_mp\utility\game_utility::_id_11ED3( "vehicle_volume_simplified", "script_noteworthy" );
    scripts\cp_mp\utility\game_utility::_id_11ED3( "super", "script_noteworthy" );
    scripts\cp_mp\utility\game_utility::_id_11ED3( "militarybase", "script_noteworthy" );
    scripts\cp_mp\utility\game_utility::_id_11ED3( "location_volume", "targetname" );
    scripts\cp_mp\utility\game_utility::_id_11ED3( "locale_area_trigger", "targetname" );
    scripts\cp_mp\utility\game_utility::_id_11ED3( "shadow_blocker", "targetname" );
    scripts\cp_mp\utility\game_utility::_id_11ED4( "door_prison_cell_metal_mp", 1 );
    scripts\cp_mp\utility\game_utility::_id_11ED4( "veh8_mil_air_acharlie130", 1 );
    ...
    scripts\cp_mp\utility\game_utility::_id_11ED2( "light" );
    scripts\cp_mp\utility\game_utility::_id_11ED2( "trigger_use_touch" );

    if ( isdefined( level.localeid ) && level.localeid == "locale_6" )
        scripts\cp_mp\utility\game_utility::_id_11ED3( "locale_8", "script_noteworthy" );
...
updatedomscores()
{
    ...
    level thread scripts\mp\spawnselection::_id_1307E();
...
updatescores()
{
    ...
    var_5 = scripts\mp\gamescore::destroy_vehicle_on_pilot_death();
    ...
    var_8 = scripts\mp\gamescore::destroy_vehicle_on_pilot_death();

    if ( var_5 != var_8 )
        scripts\mp\gamescore::_id_11A83( var_8, 1, var_5 );
}
...
runobjectives( var_0 )
{
    ...
    if ( istrue( level.useobjectives ) )
    {
        ...
        foreach ( var_2 in level.allfobs )
        {
            ...
            if ( istrue( level.lootchopper_initspawninfo ) )
                var_2._id_12849 = scripts\mp\gametypes\obj_dom.gsc::_id_1284A;
...
objective_manageobjectivesintrovisibility()
{
    ...
    scripts\mp\objidpoolmanager::objective_playermask_hidefromall( level.gw_objstruct.axishqloc.is_riding_heli.objidnum );
    scripts\mp\objidpoolmanager::objective_playermask_hidefromall( level.gw_objstruct.allieshqloc.is_riding_heli.objidnum );
...
sethqmarkerobjective()
{
    ...
    var_2.lockupdatingicons = 1;
    level.gw_objstruct.axishqloc.is_riding_heli = var_2;
    level.uncapturableobjectives[level.uncapturableobjectives.size] = var_2;
    ...
    scripts\mp\objidpoolmanager::update_objective_setbackground( var_2.objidnum, 7 );
    var_2.lockupdatingicons = 1;
    level.gw_objstruct.allieshqloc.is_riding_heli = var_2;
    level.uncapturableobjectives[level.uncapturableobjectives.size] = var_2;
...
spawnselection_showenemyhq()
{
    ...
    if ( self.team == "allies" )
    {
        ...
        scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( level.gw_objstruct.axishqloc.is_riding_heli.objidnum, self );
    }
    else
    {
        scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( level.gw_objstruct.allieshqloc.is_riding_heli.objidnum, self );
    ...
    if ( self.team == "axis" )
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( level.gw_objstruct.allieshqloc.is_riding_heli.objidnum, self );
    else
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( level.gw_objstruct.axishqloc.is_riding_heli.objidnum, self );
}
...
trial_enemy_quota()
{
    ...
    if ( istrue( level.useobjectives ) )
        level thread trial_explosive_clear();
}
...
trial_explosive_clear()
{
    ...
    for (;;)
    {
        if ( destinations( "axis" ) == level.objectives.size )
            level thread trial_headicon( "axis" );
        else if ( destinations( "allies" ) == level.objectives.size )
            level thread trial_headicon( "allies" );

        waitframe();
    }
}

trial_headicon( var_0 )
{
    ...
    scripts\mp\trials\trial_utility::_id_123B0( "ui_nuke_data", 0, 9, level.startvipteamuav );
    scripts\mp\trials\trial_utility::_id_123B0( "ui_nuke_data", 11, 1, 1 );
    var_1 = 0;
    var_2 = gettime();
    var_3 = level.startvipteamuav * 1000 + var_2;
    setomnvar( "ui_nuke_end_milliseconds", level.startvipteamuav * 1000 + var_2 );

    while ( destinations( var_0 ) == level.objectives.size )
    {
        waitframe();
...
trial_is_event( var_0 )
{
    ...
    foreach ( var_5 in level.players )
    {
        ...
        {
            if ( isdefined( var_5.updateexpiredlootleader ) )
            {
                var_5 clearsoundsubmix( "iw8_mp_spawn_camera" );
                var_5.updateexpiredlootleader stoploopsound( var_5.updatefobindanger );
                var_5.updateexpiredlootleader delete();
                var_5.updateexpiredlootleader = undefined;
                var_5.updatefobindanger = undefined;
            }
        }
    }
...
{
    thread scripts\mp\spawnselection::_id_11DA4( var_0.team, var_0.squadindex );
}
...
init_groundwarvehicles()
{
    ...
    foreach ( var_1 in level.vehicleinfo )
    {
        ...
        foreach ( var_9 in var_1.vehiclespawns )
        {
            if ( isdefined( scripts\cp_mp\utility\game_utility::getlocaleid() ) && isdefined( var_9.script_noteworthy ) && var_9.script_noteworthy == level.localeid )
            {
                ...
                if ( istrue( level.incorrectswitch ) && var_1.refname == "jeep" || istrue( level.incorrectcodeentered ) && var_1.refname == "cargo_truck" )
                    continue;
                else
                {
    ...
    var_28 = level.trial_white_phosphorus;
...
vehiclespawn_tank( var_0, var_1 )
{
    ...
    if ( istrue( level._id_1255B ) )
        _id_131B1( var_3, var_3.team );

    return var_3;
}

_id_131B1( var_0, var_1 )
{
    wait 1;
    ...
    var_0._id_1281D = var_2;
    thread _id_13296( var_0 );
    level._id_12D3D[var_1][level._id_12D3D[var_1].size] = var_0;
}

_id_13296( var_0 )
{
    var_1 = var_0._id_1281D.objidnum;
    var_2 = var_0.team;
    var_0 waittill( "death" );
    scripts\mp\objidpoolmanager::returnobjectiveid( var_1 );
    level._id_12D3D[var_2] = scripts\engine\utility::array_remove( level._id_12D3D[var_2], var_0 );
}
...
_id_1327D()
{
    ...
    foreach ( var_1 in level._id_12D3D[self.team] )
        scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( var_1._id_1281D.objidnum, self );

    while ( self.inspawnselection )
        waitframe();

    foreach ( var_1 in level._id_12D3D["axis"] )
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( var_1._id_1281D.objidnum, self );

    foreach ( var_1 in level._id_12D3D["allies"] )
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( var_1._id_1281D.objidnum, self );
}
...
_id_131B0( var_0, var_1 )
{
    ...
    var_0._id_1281D = var_2;
    thread _id_13295( var_0 );
    level._id_12D3C["untouched"][level._id_12D3C["untouched"].size] = var_0;
}
...
_id_13295( var_0 )
{
    ...
    var_0 waittill( "death" );
    scripts\mp\objidpoolmanager::returnobjectiveid( var_1 );
    var_2 = var_0.setup_comms_obj_a_goals_and_cover;

    if ( !isdefined( var_0.setup_comms_obj_a_goals_and_cover ) )
        var_2 = "untouched";

    level._id_12D3C[var_2] = scripts\engine\utility::array_remove( level._id_12D3C[var_2], var_0 );
}
...
_id_1327E()
{
    ...
    foreach ( var_1 in level._id_12D3C[self.team] )
        scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( var_1._id_1281D.objidnum, self );

    foreach ( var_1 in level._id_12D3C["untouched"] )
        scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( var_1._id_1281D.objidnum, self );

    while ( self.inspawnselection )
        waitframe();

    foreach ( var_1 in level._id_12D3C["axis"] )
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( var_1._id_1281D.objidnum, self );

    foreach ( var_1 in level._id_12D3C["allies"] )
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( var_1._id_1281D.objidnum, self );
...
_id_1326E( var_0 )
{
    if ( isdefined( self.setup_comms_obj_a_goals_and_cover ) )
        level._id_12D3C[self.setup_comms_obj_a_goals_and_cover] = scripts\engine\utility::array_remove( level._id_12D3C[self.setup_comms_obj_a_goals_and_cover], self );
    else
        level._id_12D3C["untouched"] = scripts\engine\utility::array_remove( level._id_12D3C["untouched"], self );

    self.setup_comms_obj_a_goals_and_cover = var_0;
    level._id_12D3C[var_0][level._id_12D3C[var_0].size] = self;
    scripts\mp\objidpoolmanager::update_objective_icon( self._id_1281D.objidnum, "icon_minimap_littlebird_spawn_selection" );

    foreach ( var_2 in level.players )
    {
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( self._id_1281D.objidnum, var_2 );

        if ( isdefined( var_2 ) && isdefined( var_2.team ) && var_2.team == var_0 && istrue( var_2.inspawnselection ) )
        {
            scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( self._id_1281D.objidnum, var_2 );
            continue;
        }
...
init_rallyvehicles()
{
    ...
    if ( !istrue( level.disablelittlebirdrally ) )
    {
        foreach ( var_2 in var_0 )
        {
            ...
            if ( level.initoperatorunlocks )
                var_6 = _getsingleplunderextractlocations::skipfirstraise( var_4, var_5 );
            else
    ...
    {
        if ( level.localeid == "locale_6" && level.mapname == "mp_downtown_gw" && istrue( level.spawnziptie ) )
        {
            var_10 = [];
        ...
        foreach ( var_2 in var_0 )
        {
            ...
            level thread _id_131B0( var_6, "neutral" );
        }
    }
...
arm_initoutofbounds()
{
    ...
    foreach ( var_2 in var_0 )
    {
        if ( isdefined( scripts\cp_mp\utility\game_utility::getlocaleid() ) && isdefined( var_2.script_noteworthy ) && var_2.script_noteworthy == level.localeid && scripts\mp\utility\game_utility_mp::supportboxmaxammo( var_2 ) )
        {
