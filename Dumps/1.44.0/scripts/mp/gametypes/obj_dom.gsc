...
setupobjective( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    ...
    if ( istrue( level.lootchopper_initspawninfo ) )
        var_10._id_12849 = ::_id_1284A;
    else if ( istrue( level.play_hud_reminder_vo ) && scripts\mp\utility\game::getgametype() != "br" )
        var_10._id_12849 = ::_id_1284B;
    ...
    if ( istrue( level.play_hud_reminder_vo ) && scripts\mp\utility\game::getgametype() != "br" )
    {
        var_20 = "decor_halloween_scarecrow";
        var_10.flagmodel.sat_wait_for_activated_think = 1;
    }
    else
        var_20 = "military_dom_flag_neutral";

    var_10.flagmodel setmodel( var_20 );
    var_10.flagmodel.angles = end_pipe_room( var_10 );
    var_10.outlineent = var_10.flagmodel;

    if ( istrue( level.play_hud_reminder_vo ) && scripts\mp\utility\game::getgametype() != "br" )
    {
        var_10 thread play_hit_marker_to_player();
        var_10 thread ignorereset();
    }
...
dompoint_setcaptured( var_0, var_1 )
{
    ...
    if ( isdefined( self.assisttouchlist[var_0] ) )
    {
        ...
        foreach ( var_8 in var_6 )
        {
            ...
            var_9 scripts\cp\helicopter\chopper_boss::tv_station_intro_already_played();
        }
    }
...
dompoint_onuse( var_0 )
{
    ...
    if ( !self.neutralized )
    {
        ...
        if ( isdefined( level.onobjectivecomplete ) )
        {
            ...
            var_5 = "Flag " + numhunters() + " Captured";

            if ( var_2 == "neutral" || var_2 == "none" )
                scripts\mp\utility\game::spawn_chopper_carepackage( var_0, undefined, var_5, var_0.origin, "neutral_flag" );
            else
                scripts\mp\utility\game::spawn_chopper_carepackage( var_0, undefined, var_5, var_0.origin );
        }
    }
...
{
    self.fulton_exfil_model = gettime();
...
dompoint_onuncontested( var_0 )
{
    ...
    var_2 = ( gettime() - self.fulton_exfil_model ) * 0.001;
    var_3 = "Flag " + numhunters() + " Contested";
    scripts\mp\utility\game::spawn_chopper_carepackage( undefined, undefined, var_3, undefined, var_2 + " seconds" );
    self.fulton_exfil_model = undefined;
...
_id_1284A()
{
    ...
    if ( var_7["fraction"] != 1 )
    {
        ...
        if ( var_7["fraction"] != 1 )
        {
            playfx( level.perkpackage_resetoverridefieldupgrades["gos_fireworks"], var_7["position"] );
            playfx( level.perkpackage_resetoverridefieldupgrades["gos_fireworks"], var_7["position"], anglestoforward( self.flagmodel.angles ) + ( -10, -10, 0 ) );
            playfx( level.perkpackage_resetoverridefieldupgrades["gos_fireworks"], var_7["position"], anglestoforward( self.flagmodel.angles ) + ( 10, 10, 0 ) );
            level thread scripts\mp\gametypes\common.gsc::_id_133E1( var_7["position"] );
        }
    }
    else
    {
        playfx( level.perkpackage_resetoverridefieldupgrades["gos_fireworks"], var_2 );
        playfx( level.perkpackage_resetoverridefieldupgrades["gos_fireworks"], var_2, anglestoforward( self.flagmodel.angles ) + ( -10, -10, 0 ) );
        playfx( level.perkpackage_resetoverridefieldupgrades["gos_fireworks"], var_2, anglestoforward( self.flagmodel.angles ) + ( 10, 10, 0 ) );
        level thread scripts\mp\gametypes\common.gsc::_id_133E1( var_2 );
    }
}

_id_1284B()
{
    if ( isdefined( self.postgamehitmarkerwaittime ) )
        return;
    else
    {
        self.postgamehitmarkerwaittime = 1;
        var_0 = 20;
        var_1 = 600;
        ...
        if ( var_7["fraction"] != 1 )
            playfx( level.post_loadout_spawn_func["blood_ceil_hw"], var_7["position"] - ( 0, 0, 5 ), ( -180, -180, 0 ), ( -180, -180, -180 ) );

        playfx( level.post_loadout_spawn_func["blood_floor_hw"], self.flagmodel.origin + ( 0, 0, 2 ) );
        var_0 = 4000;
        var_1 = 20;
        var_2 = self.flagmodel.origin;
        var_3 = var_2 + ( 0, 0, var_0 );
        ...
        if ( var_7["fraction"] != 1 )
            playfx( level.post_loadout_spawn_func["bats_fly_hw"], var_7["position"] + ( 0, 0, 50 ) );
        else
            playfx( level.post_loadout_spawn_func["bats_fly_hw"], self.flagmodel.origin + ( 0, 0, 50 ) );
    }
}
...
play_hit_marker_to_player()
{
    ...
    if ( var_7["fraction"] != 1 )
        playfx( level.post_loadout_spawn_func["bats_hw"], var_7["position"] );
    else
        playfx( level.post_loadout_spawn_func["bats_hw"], self.flagmodel.origin + ( 0, 0, 200 ) );
}
...
updateflagstate( var_0, var_1, var_2, var_3 )
{
    ...
    if ( istrue( level.play_hud_reminder_vo ) && scripts\mp\utility\game::getgametype() != "br" )
    {
        if ( istrue( var_3 ) )
            ...
            self.flagmodel thread _id_1213B();
        else if ( isdefined( var_2 ) )
            playfx( level.post_loadout_spawn_func["vanish_hw_fr"], self.flagmodel.origin + ( 0, 0, 60 ) );
    }
    else if ( isdefined( var_2 ) )
    {
        if ( var_2 == "allies" )
...
giveflagcapturexp( var_0, var_1, var_2 )
{
    ...
    if ( isplayer( var_3 ) )
    {
        ...
        var_3 thread scripts\common\utility::_id_12ED6( level.stack_patch_waittill_context_patch, "capture", var_3.origin );
    }
...
awardgenericmedals( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9 )
{
    ...
    if ( isdefined( var_0 ) )
    {
        ...
        {
            if ( istrue( var_0.sat_wait_for_activated_think ) )
                return;
        }
    }
            ...
            if ( var_20 || var_21 )
            {
                ...
                else if ( var_18.claimteam == var_1.team )
                {
                    if ( !var_12 )
                    {
                        ...
                        thread scripts\common\utility::_id_12ED6( level.stack_patch_waittill_context, var_9, "defending" );
                        continue;
                    }
                }
            }
...
_id_1213B()
{
    ...
    wait 0.55;
    playfx( level.post_loadout_spawn_func["vanish_hw_fr"], self.origin + ( 0, 0, 60 ) );
...
ignorereset()
{
    ...
    for (;;)
    {
        ...
        if ( isplayer( var_0 ) )
        {
            if ( !isdefined( self._id_128E0 ) )
            {
                self._id_128E0 = var_0;
                thread _id_13522( self.flagmodel, var_0 );
            }
        }
...
_id_13522( var_0, var_1 )
{
    level endon( "game_ended" );
    self notify( "new_stalker_target" );
    self endon( "new_stalker_target" );

    while ( isdefined( self._id_128E0 ) )
    {
        if ( !_id_13157( var_0, var_1 ) )
        {
            self._id_128E0 = undefined;
            self._id_128DF = 0;
            self.keep_trying_to_kill_off_ai = undefined;
            self.bot_add_destination_spot = undefined;
            self._id_11DA1 = undefined;
            self.flagmodel rotateto( ( 0, self.flagmodel.angles[1], self.flagmodel.angles[2] ), 0.2, 0.1, 0.1 );
        }
        else if ( isdefined( self._id_128E0 ) && scripts\mp\utility\player::isreallyalive( self._id_128E0 ) )
        {
...
_id_1351A()
{
    ...
    for (;;)
    {
        ...
        if ( isdefined( var_1 ) && isdefined( self._id_128E0 ) && self._id_128E0 == var_1 )
            self._id_128DF = self._id_128DF + var_0;

        if ( isdefined( self._id_128DF ) )
        {
            if ( self._id_128DF > 100 && !isdefined( self.keep_trying_to_kill_off_ai ) )
                self.keep_trying_to_kill_off_ai = 1;

            if ( self._id_128DF > 200 && !isdefined( self.bot_add_destination_spot ) )
            {
                self.bot_add_destination_spot = 1;
                playfx( level.post_loadout_spawn_func["vanish_hw_en"], self.flagmodel.origin + ( 0, 0, 80 ) );
            }

            if ( self._id_128DF > 300 && !isdefined( self._id_11DA1 ) )
            {
                self._id_11DA1 = 1;
                thread _id_11F77( var_1 );
            }
        }
...
{
    var_2 = 0.05;
    var_3 = var_1 onentercallback();
    var_4 = var_0 oldprogress();
