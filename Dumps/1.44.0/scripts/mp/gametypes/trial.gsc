...
main()
{
    ...
    if ( _getdeathstatecode::_id_12E25() )
    ...
    level.blueprint_chancepercontract = var_0;
    ...
    while ( !isdefined( level._id_12E2A ) )
        waitframe();
    ...
    switch ( level.trial["missionScript"] )
    {
        case "gun_nonlinear":
        case "gun":
            thread [[ level._id_12E2A["gun"] ]]();
            break;
        case "clear":
            thread [[ level._id_12E2A["clear"] ]]();
            break;
        case "sniper":
            thread [[ level._id_12E2A["sniper"] ]]();
            break;
        case "jugg":
            thread [[ level._id_12E2A["jugg"] ]]();
            break;
        case "lava":
            thread [[ level._id_12E2A["lava"] ]]();
            break;
        case "pitcher":
            thread [[ level._id_12E2A["pitcher"] ]]();
            break;
        case "gunslinger":
            thread [[ level._id_12E2A["gunslinger"] ]]();
            break;
        case "race":
            thread [[ level._id_12E2A["race"] ]]();
            break;
        case "arm_course":
            thread [[ level._id_12E2A["arm_course"] ]]();
            break;
        default:
            break;
    }
...
_id_12E71( var_0, var_1 )
{
    level notify( "exitLevel_called" );
    processlobbydata();

    if ( isdefined( level._id_12E0B ) )
        [[ level._id_12E0B ]]();
...
trial_mission_data_init()
{
    var_0 = _getdeathstatecode::_id_12E1B();
...
getspawnpoint()
{
    ...
    if ( isdefined( level._id_12E42 ) )
    {
        var_3 = scripts\cp_mp\vehicles\vehicle_occupancy::vehicle_occupancy_getavailablevehicleseats( level._id_12E42, 1 );
        var_4 = spawnstruct();
        var_4.useonspawn = 1;
        var_4.enterstartwaitmsg = "spawned_player";
        thread scripts\cp_mp\vehicles\vehicle_occupancy::vehicle_occupancy_enter( level._id_12E42, var_3[0], self, var_4 );
...
onplayerconnect( var_0 )
{
    ...
    level.player scripts\mp\gametypes\br.gsc::weapon_xp_iw8_sm_augolf();
    wait 1;
...
weapon_think()
{
    ...
    for (;;)
    {
        ...
        if ( !isdefined( self.spawned_weapon ) && !var_14 && !var_15 )
        {
            ...
            if ( istrue( level.watchforscriptabledoorsinradius ) )
...
trial_end_score_dialogue()
{
    ...
    _getdeathstatecode::trial_ui_waittill_retry();

    if ( !istrue( level._id_12E39 ) )
...
_id_12E38()
{
    for (;;)
    {
        ...
        {
            if ( !isdefined( level.round_vehicle_logic ) || !level.round_vehicle_logic )
                _getdeathstatecode::_id_12E37();
        }
    }
}
