...
updategametypedvars()
{
    ...
    level.vehicle_compass_show = scripts\mp\utility\dvars::dvarintvalue( "pauseTime", 1, 0, 1 );
    ...
    if ( scripts\mp\utility\game::matchmakinggame() && !scripts\mp\utility\game::isanymlgmatch() && getdvarint( "scr_koth_playlistZoneActivationDelay", 15 ) != 0 )
        level.zoneactivationdelay = aq_playerremoved();
}
...
hardpointmainloop()
{
    ...
    scripts\mp\utility\game::spawn_chopper_carepackage( undefined, undefined, "New Hardpoint", level.zone.trigger.origin );
    level.zone thread scripts\common\utility::_id_12ED6( level.stack_patch_waittill_context_patch, "hill_moved", level.zone.trigger.origin );
    ...
    if ( level.firstzoneactivationdelay )
    {
        ...
        level.zone thread scripts\mp\gametypes\obj_zonecapture.gsc::spawn_boss_wave_section( level.firstzoneactivationdelay, level.zone.curorigin + level.zone.offset3d );
    ...
    for (;;)
    {
        if ( !isdefined( level.spinafterdelay ) )
...
_id_11E84()
{
    switch ( level.mapname )
    {
        ...
        case "mp_hackney_yard":
            level._id_11E86 = [ "1", "4", "3", "9", "8" ];
            break;
        case "mp_spear":
        case "mp_spear_pm":
            level._id_11E86 = [ "5", "4", "3", "2", "6" ];
            break;
        case "mp_cave":
        case "mp_cave_am":
            level._id_11E86 = [ "1", "5", "2", "3", "4" ];
            break;
        case "mp_petrograd":
            level._id_11E86 = [ "1", "9", "4", "5", "2" ];
            break;
        case "mp_deadzone":
            level._id_11E86 = [ "12", "11", "3", "1", "2" ];
            break;
        case "mp_raid":
            level._id_11E86 = [ "20", "2", "8", "9", "5" ];
            break;
        case "mp_piccadilly":
            level._id_11E86 = [ "1", "5", "7", "3", "8" ];
            break;
        case "mp_crash2_pm":
        case "mp_crash2":
            level._id_11E86 = [ "1", "20", "5", "4", "19" ];
            break;
        case "mp_emporium":
            level._id_11E86 = [ "1", "2", "3", "4", "5" ];
            break;
        case "mp_broadcast2":
            level._id_11E86 = [ "1", "2", "3", "4", "5" ];
            break;
...
getfirstzone()
{
    if ( isdefined( level._id_11E86 ) )
    {
        var_0 = level.objectives[level._id_11E86[0]];
        level.prevzoneindex = 0;
        level.longdeathtracker = level.objectives[level._id_11E86[1]];
    }
    else
    {
        var_0 = level.objectives["1"];
        level.prevzoneindex = 1;
        level.longdeathtracker = level.objectives["2"];
    }

    var_0 thread scripts\mp\gametypes\obj_zonecapture.gsc::_id_13506();
    level.longdeathtracker thread scripts\mp\gametypes\obj_zonecapture.gsc::_id_13506();
    return var_0;
}
...
getnextzone()
{
    ...
    else
    {
        if ( isdefined( level._id_11E86 ) )
        {
            ...
            if ( level.prevzoneindex > level._id_11E86.size - 1 )
                level.prevzoneindex = 0;

            var_23 = level.objectives[level._id_11E86[level.prevzoneindex]];
            var_30 = level.prevzoneindex + 1;

            if ( var_30 > level._id_11E86.size - 1 )
                var_30 = 1;

            level.longdeathtracker = level.objectives[level._id_11E86[var_30]];
        }
        else
        {
            level.prevzoneindex++;
            ...
            level.longdeathtracker = level.objectives[scripts\engine\utility::string( var_30 )];
        }
...
hqactivatenextzone( var_0, var_1 )
{
    ...
    else if ( level.zoneactivationdelay )
    {
        ...
        level._id_13730 = 1;
        level thread scripts\mp\hud_message::updatematchstatushintforallplayers( undefined, 36, 36 );
        level.zone thread scripts\mp\gametypes\obj_zonecapture.gsc::spawn_boss_wave_section( level.zoneactivationdelay, level.zone.curorigin + level.zone.offset3d );
...
hpcaptureloop()
{
    ...
    for (;;)
    {
        ...
        var_0 = level scripts\engine\utility::_id_133F7( "zone_captured", "zone_destroyed" );
...
initspawns()
{
    ...
    if ( !isdefined( level.spawnpoints ) || istrue( level.aq_ontimerupdate ) )
    {
    ...
    if ( istrue( level.aq_ontimerupdate ) )
        return;
...
give_capture_credit( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    level endon( "game_ended" );
    level.lastcaptime = gettime();
    scripts\mp\utility\game::spawn_chopper_carepackage( var_4, undefined, "Hardpoint Captured", var_4.origin );
    var_6 = var_4;
    ...
    if ( isplayer( var_6 ) )
    {
        var_6 thread scripts\common\utility::_id_12ED6( level.stack_patch_waittill_context_patch, "capture", var_6.origin );

        if ( rocket_internal( var_6 ) && !scripts\mp\utility\game::isanymlgmatch() )
        {
    ...
    while ( !level.gameended )
    {
        ...
        if ( !level.zone.stalemate && !level.gameended )
        {
            ...
            foreach ( var_5 in level.zone.touchlist[var_3] )
            {
                ...
                if ( isdefined( var_5.player.plunder_items_dropped ) && var_5.player.plunder_items_dropped > 3 )
                {
                    var_5.player.plunder_items_dropped = 0;
                ...
                if ( !isdefined( var_5.player.plunder_items_dropped ) )
                    var_5.player.plunder_items_dropped = 0;
                else
                    var_5.player.plunder_items_dropped++;
