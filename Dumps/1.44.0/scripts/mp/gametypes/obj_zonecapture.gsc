...
setupobjective( var_0, var_1, var_2, var_3 )
{
    ...
    if ( istrue( level.play_hud_reminder_vo ) )
    {
        if ( scripts\mp\utility\game::getgametype() == "hq" )
            var_4 thread _id_133E0( var_4 );
        else if ( scripts\mp\utility\game::getgametype() == "grnd" )
            var_4 thread _id_133DF( var_4 );
    }
...
_id_133E0( var_0 )
{
    ...
    playfx( level.post_loadout_spawn_func["ghostcat_hw"], var_3 + var_2 );
}

_id_133DF( var_0 )
{
    scripts\mp\flags::gameflagwait( "prematch_done" );
    playfx( level.post_loadout_spawn_func["blood_floor_hw"], getgroundposition( var_0.trigger.origin, 4 ) + ( 0, 0, 2 ) );
}
...
assignchevrons( var_0, var_1 )
{
    ...
    var_2 = _id_11EC3( var_2, var_1 );
    var_2 = _id_11B19( var_2, var_1 );
    var_3 = [];
...
activatezone()
{
    ...
    if ( isdefined( level.nocrouch ) )
        var_0 = [[ level.nocrouch ]]();

    var_1 = 1024;

    if ( isdefined( level.node_cansee_child ) )
        var_1 = [[ level.node_cansee_child ]]();
    ...
    self.getfullweaponobjforpickup = var_2;
...
deactivatezone()
{
    ...
    foreach ( var_1 in self.getfullweaponobjforpickup )
        scripts\mp\spawnlogic::removespawndangerzone( var_1 );

    self.getfullweaponobjforpickup = undefined;
...
spawn_boss_wave_section( var_0, var_1 )
{
    level endon( "game_ended" );
...
cleanupgametypevips()
{
    level scripts\engine\utility::_id_133EF( "game_ended", "zone_moved" );
...
zone_onuse( var_0 )
{
    ...
    if ( level.usehqrules && !level.usehprules && self.ownerteam != "neutral" )
    {
        ...
        if ( isdefined( var_3 ) )
        {
            ...
            foreach ( var_11 in var_9 )
            {
                ...
                var_7 scripts\cp\helicopter\chopper_boss::tv_station_intro_already_played();
            }
        }
    ...
    else
    {
        ...
        if ( scripts\mp\utility\game::getgametype() == "hq" && !level.usehprules )
        {
            ...
            applyxyzoffset( var_1 );
        }
        else if ( level.vehicle_compass_show )
    ...
    if ( istrue( level.play_hud_reminder_vo ) )
        playfx( level.post_loadout_spawn_func["vanish_hw_fr"], level.zone.trigger.origin + ( 0, 0, 40 ) );
}
...
zone_onuseupdate( var_0, var_1, var_2, var_3 )
{
    ...
    if ( var_4 == "neutral" && self.claimteam != var_4 )
        applyxyzoffset( self.claimteam );
    else if ( var_4 != "neutral" && self.claimteam != var_4 )
        applyxyzoffset( var_4, 1 );
    else if ( level.vehicle_compass_show )
        level scripts\mp\gamelogic::pausetimer();
...
zone_onuseend( var_0, var_1, var_2 )
{
    ...
    if ( !var_2 )
    {
        if ( scripts\mp\utility\game::getgametype() == "hq" )
        {
            if ( level.usehprules )
            {
                if ( level.vehicle_compass_show )
                    level scripts\mp\gamelogic::resumetimer();
            }
            else if ( var_3 != "neutral" )
                applyxyzoffset( var_3 );
        }
        else if ( level.vehicle_compass_show )
            level scripts\mp\gamelogic::resumetimer();
    }
...
zone_oncontested()
{
    ...
    level.zone thread scripts\common\utility::_id_12ED6( level.stack_patch_waittill_context_patch, "hill_contested", level.zone.trigger.origin );
    self.didstatusnotify = 1;
}
...
zone_onuncontested( var_0 )
{
    ...
    if ( var_0 == "none" || var_1 == "neutral" )
    {
        ...
        level.zone thread scripts\common\utility::_id_12ED6( level.stack_patch_waittill_context_patch, "hill_empty", level.zone.trigger.origin );
    }
    else
    {
        if ( scripts\mp\utility\game::getgametype() == "koth" )
        ...
        level.zone thread scripts\common\utility::_id_12ED6( level.stack_patch_waittill_context_patch, "hill_uncontested", level.zone.trigger.origin );
    }

    var_7 = ( gettime() - self.fulton_exfil_model ) * 0.001;
    scripts\mp\utility\game::spawn_chopper_carepackage( undefined, undefined, "Zone Contested", level.zone.trigger.origin, var_7 + " seconds" );
...
_id_13506()
{
    ...
    foreach ( var_2 in level.teamnamelist )
    {
        self.logplayermatchstart[var_2] = 0;
        self.logtrophysuccesful[var_2] = [];
    }

    self.logplayermatchstart["neutral"] = 0;
    self.logtrophysuccesful["neutral"] = [];
    self.logtrophysuccesful["none"] = [];
    self.end_health = 1;
    ...
    for (;;)
    {
        ...
        if ( scripts\mp\utility\player::isreallyalive( var_4 ) && !isdefined( var_4.touchtriggers[var_0] ) )
        {
            ...
            self.logplayermatchstart[var_2]++;
            var_6 = var_4.guid;
            var_7 = spawnstruct();
            var_7.player = var_4;
            var_7.starttime = gettime();
            self.logtrophysuccesful[var_2][var_6] = var_7;
            var_4.plunder_items_dropped = 0;
        }

        if ( scripts\mp\utility\player::isreallyalive( var_4 ) && !isdefined( var_4.touchtriggers[var_0] ) )
            var_4 thread long_death_manager( self );
    }
}

long_death_manager( var_0 )
{
    level endon( "stop_watching_trigger" );
    ...
    if ( isdefined( self ) )
    {
        var_0.logtrophysuccesful[var_1][self.guid] = undefined;
        self.plunder_items_dropped = 0;
    }
    else
    {
        var_2 = [];

        foreach ( var_5, var_4 in var_0.logtrophysuccesful[var_1] )
        {
        ...
        foreach ( var_5 in var_2 )
            var_0.logtrophysuccesful[var_1][var_5] = undefined;
    }

    var_0.logplayermatchstart[var_1]--;
}
