...
main()
{
    ...
    if ( var_0 )
        level.rotationids = getdvarint( "scr_infect_groundwarInfect", 0 );

    var_1[0] = scripts\mp\utility\game::getgametype();
    var_1[var_1.size] = "grnd";

    if ( istrue( level.rotationids ) )
    ...
    level.jumped = getdvarint( "scr_infect_time_increase_cap", 20 );
    ...
    level.animscripted_single = ::propchange;
    level.lootchopper_getattackerdata = ::lootchopper_getattackerdata;
    ...
    level.propane_detonate_fiery_drips = getdvarint( "scr_infect_nvgMode", 0 );
    level.propanchor = getdvarint( "scr_infect_disable_nvg", 0 );
    level.unset_forced_aitype = getdvarint( "scr_infect_onlineprimaryoverride", 0 );

    if ( level.rotationids )
    {
        ...
        level.propchangecount = var_3;
        level.bomb_use_player = getdvarint( "scr_infect_alwaysTacInsertSkySpawn", 0 );
        level._id_125CF = getdvarint( "scr_infect_skyDiveOnTacInsertPlacement", 1 );
        level.boardroomdoorcodeentrysuccess = getdvarint( "scr_infect_allowSkyDiveCutParachute", 0 );
        level.vehicleindangertracking = ::vehicleindangertracking;
        level._id_12B3F = ::_id_12B3F;
    }
...
onstartgametype()
{
    ...
    scripts\cp_mp\utility\script_utility::registersharedfunc( "vehicle_compass", "shouldBeVisibleToPlayer", ::_id_1319A );

    if ( istrue( level.rotationids ) )
    {
        scripts\mp\gametypes\arm.gsc::initcpammoarmorcrate();
        thread quarry_mortar_guys();
        thread spawn_fx_on_node();
    }
...
updategametypedvars()
{
    ...
    level._id_12AFA = getdvar( "scr_infect_superSurvivorTwo", "none" );
    ...
    level.propcamerazoom = getdvar( "scr_infect_superInfectTwo", scripts\engine\utility::ter_op( level.rotationids, "super_tac_insert", "none" ) );
    ...
    level.prompt = getdvarint( "scr_infect_infectBonusScore", 150 );
    level.propability = getdvarint( "scr_infect_infectBonusSuperOnSpawn", 0 );
    level.propabilitykeysvisible = getdvarint( "scr_infect_infectBonusSuperOnTacInsert", 0 );
    level.last_rpg_fire = getdvarint( "scr_infect_finalSuvivorCount", 1 );
    level.propchangeto = getdvarint( "scr_infect_setRadarOnNumSurvivors", 8 );
    level._id_12AF9 = [];
    var_0 = getdvar( "scr_infect_survivorStreakOverride", "" );

    if ( var_0 != "" )
        level._id_12AF9 = strtok( var_0, "," );

    var_1 = scripts\cp_mp\utility\game_utility::getmapname();

    if ( issubstr( var_1, "mp_m_" ) && var_1 != "mp_m_speed" )
    {
        level.survivorsuper = lb_impulse_dmg_factor_mid_low( 0, 0 );
        level._id_12AFA = lb_impulse_dmg_factor_mid_low( 0, 1 );
        level.infectedsuper = lb_impulse_dmg_factor_mid_low( 1, 0 );
        level.propcamerazoom = lb_impulse_dmg_factor_mid_low( 1, 1 );
    }

    if ( level.survivorsuper == level._id_12AFA )
        level._id_12AFA = "none";
    else if ( level.survivorsuper == "none" && level._id_12AFA != "none" )
    {
        level.survivorsuper = level._id_12AFA;
        level._id_12AFA = "none";
    }

    if ( level.infectedsuper == level.propcamerazoom )
        level.propcamerazoom = "none";
    else if ( level.infectedsuper == "none" && level.propcamerazoom != "none" )
    {
        level.infectedsuper = level.propcamerazoom;
        level.propcamerazoom = "none";
    }
...
buildandloadweapons()
{
    ...
    foreach ( var_1 in level.allweapons )
    {
        if ( var_1 != "none" )
        {
            ...
            if ( level.propane_detonate_fiery_drips )
                game["arenaRandomLoadout"][0]["loadoutPrimaryAttachment"] = onnewequipmentpickup();
...
{
    if ( istrue( var_1 ) )
        var_2 = scripts\engine\utility::ter_op( var_0, level.propcamerazoom, level._id_12AFA );
    else
...
onspawnfinished()
{
    ...
    if ( self.pers["team"] == "allies" )
    {
        ...
        if ( level.rotationids )
            thread participantxp();

        spawnwithplayersecondary();

        if ( level.propane_detonate_fiery_drips && !level.propanchor && !isbot( self ) )
            thread trigger_spawn_kill();
    }
    else if ( self.pers["team"] == "axis" )
    {
        if ( istrue( level.play_hud_reminder_vo ) )
        {
            ...
            self.round_smoke_molotov_logic = 1;
        }

        if ( istrue( level.boardroomdoorcodeentrysuccess ) )
    ...
    if ( istrue( self.thermiteradiusweaponref ) )
    {
        if ( !level.rotationids || level.rotationids && level.mapname == "mp_aniyah" )
    ...
    if ( istrue( self.run_spawn_module_till_kill_trig ) )
    {
        self.run_spawn_module_till_kill_trig = undefined;
        thread scripts\cp_mp\parachute::propindex( 1 );
    }

    thread scripts\mp\supers::givesuperpoints( level.propability, undefined, 1 );
    thread _id_133E3();
}
...
_id_133E3()
{
    ...
    if ( istrue( level.play_hud_reminder_vo ) && self.team == "axis" )
...
setfirstinfected( var_0 )
{
    ...
    while ( !scripts\mp\utility\player::isreallyalive( self ) || scripts\mp\utility\player::isusingremote() || isdefined( self._id_132C5 ) )
        waitframe();
    ...
    if ( isdefined( var_1 ) )
    {
        ...
        var_2.isvalidattachmentunlock = "INVOLUNTARY";
    ...
    if ( var_0 )
    {
        ...
        thread scripts\mp\music_and_dialog::propaddtocircle();
    }

    self.isinitialinfected = 1;
    scripts\mp\utility\stats::incpersstat( "firstInfected", 1 );
    self.pers["gamemodeLoadout"] = level.infect_loadouts["axis_initial"];
    scripts\mp\equipment\tac_insert::_id_12804( self.origin, self.angles );
    ...
    if ( isdefined( scripts\mp\supers::getcurrentsuper() ) && scripts\mp\supers::getcurrentsuperref() == "super_deadsilence" )
        thread _id_129B0();
    else if ( level.rotationids )
        thread scripts\mp\supers::givesuperpoints( 350, undefined, 1 );

    if ( var_0 )
        level.infect_players[self.name] = 1;

    level.laststandweapondelay = 1;

    if ( !level.laststandweapondelay )
...
onplayerkilled( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9 )
{
    ...
    {
        if ( isdefined( level.trial_spawn_vehicle ) && self == level.trial_spawn_vehicle )
        {
            var_10 = 0;
            var_11 = 0;
        }
    ...
    if ( var_10 )
    {
        ...
        if ( scripts\mp\utility\dvars::getwatcheddvar( "timelimit" ) != 0 )
        {
            ...
            if ( scripts\mp\utility\game::matchmakinggame() )
            {
                level.jumpcomandsregistered++;
                var_17 = level.jumpcomandsregistered <= level.jumped;
            }
...
{
    self.thermiteradiusweaponref = 1;

    if ( level.rotationids && level.mapname != "mp_aniyah" )
        _id_12379();
    ...
    if ( level.infect_teamscores["allies"] > 1 )
    {
        if ( level.rotationids )
        {
            ...
            if ( level._id_11A5C > 3 )
                level._id_11A5C = 1;

            if ( level.infect_teamscores["allies"] == level.propchangeto )
                level thread _id_129CD();
        }

        if ( level._id_11A5C == 1 )
        {
        ...
        if ( !var_1 )
        {
            ...
            foreach ( var_5 in level.players )
            {
                ...
                if ( var_5.team == "axis" && var_5 != var_0 && var_5 != self )
                    var_5 thread scripts\mp\utility\points::giveunifiedpoints( "assist", undefined, level.prompt );
            }
        }
    }
    else if ( level.infect_teamscores["allies"] == level.last_rpg_fire )
        onfinalsurvivor();
...
spawn_fx_on_node()
{
    ...
    while ( level.infect_teamscores["allies"] > level.propchangeto )
    {
        foreach ( var_1 in level.players )
        {
            if ( var_1.team == "axis" )
...
enduavonlatejoiner( var_0 )
{
    ...
    for (;;)
    {
        if ( level.infect_teamscores["allies"] > level.last_rpg_fire )
        {
...
monitordisconnect()
{
    ...
    if ( isdefined( self.infect_isbeingchosen ) || level.infect_chosefirstinfected )
    {
        if ( level.infect_teamscores["axis"] && level.infect_teamscores["allies"] )
        {
            if ( level.rotationids )
            {
                if ( var_0 == "allies" && level.infect_teamscores["allies"] == level.propchangeto )
                    level thread _id_129CD();
            }

            if ( var_0 == "allies" && level.infect_teamscores["allies"] == level.last_rpg_fire )
                onfinalsurvivor();
...
setspecialloadouts()
{
    ...
    var_0 = 0;
    var_1 = scripts\mp\gametypes\gun.gsc::nextdest( level.survivorprimaryweapon );
    var_0 = scripts\mp\class::_id_12B22( level.survivorprimaryweapon, var_1 );
    var_2 = 0;
    var_3 = scripts\mp\gametypes\gun.gsc::nextdest( level.survivorsecondaryweapon );
    var_2 = scripts\mp\class::_id_12B22( level.survivorsecondaryweapon, var_3 );
    ...
    else if ( level.unset_forced_aitype )
    {
        ...
        if ( level._id_12AF9.size > 0 )
        {
            level.infect_loadouts["allies"]["loadoutKillstreak1"] = level._id_12AF9[0];
            level.infect_loadouts["allies"]["loadoutKillstreak2"] = level._id_12AF9[1];
            level.infect_loadouts["allies"]["loadoutKillstreak3"] = level._id_12AF9[2];
        ...
        level.infect_loadouts["allies"]["loadoutFieldUpgrade2"] = level._id_12AFA;
    }
    else
    {
        level.infect_loadouts["allies"]["loadoutPrimary"] = level.survivorprimaryweapon;
    ...
    {
        var_6 = scripts\mp\gametypes\gun.gsc::nextdest( level.initialprimaryweapon );
        var_4 = scripts\mp\class::_id_12B22( level.initialprimaryweapon, var_6 );
    }
    ...
    {
        var_7 = scripts\mp\gametypes\gun.gsc::nextdest( level.initialsecondaryweapon );
        var_5 = scripts\mp\class::_id_12B22( level.initialsecondaryweapon, var_7 );
    }
    ...
    else if ( level.unset_forced_aitype )
    {
        ...
        level.infect_loadouts["axis_initial"]["loadoutFieldUpgrade2"] = level.propcamerazoom;
    }
    else
    {
        level.infect_loadouts["axis_initial"]["loadoutPrimary"] = level.initialprimaryweapon;
    ...
    {
        var_9 = 0;
        var_10 = scripts\mp\gametypes\gun.gsc::nextdest( level.infectedprimaryweapon );
        var_9 = scripts\mp\class::_id_12B22( level.infectedprimaryweapon, var_10 );
    }
    ...
    else
    {
        ...
        level.infect_loadouts["axis"]["loadoutFieldUpgrade2"] = level.propcamerazoom;
    ...
    for (;;)
    {
        if ( self isnightvisionon() )
        {
            ...
            if ( var_0 >= 10 )
            {
                ...
                scripts\common\utility::blockade_get_bomb_icon_on_cell_phone( 0 );
                _id_11D73();
                scripts\common\utility::blockade_get_bomb_icon_on_cell_phone( 1 );
                var_1 = 0;
...
findenemytype()
{
    ...
    self.loadoutfieldupgrade2 = level.propcamerazoom;
    wait 0.1;
    ...
    if ( isdefined( self ) && scripts\mp\utility\player::isreallyalive( self ) )
    {
        ...
        var_0 = "gamemode";
        var_1 = scripts\mp\class::snapplayertotoppos();
        var_1 = scripts\mp\class::sniper_laser_vfx( var_1, var_0 );
        ...
        scripts\mp\equipment::giveequipment( level.infectedtactical, "secondary" );
        thread scripts\mp\supers::givesuperpoints( level.propabilitykeysvisible, undefined, 1 );
    }

    if ( istrue( level._id_125CF ) )
        thread _id_12ABE();
    else
...
quarry_mortar_guys()
{
    ...
    var_18 = level.trial_white_phosphorus;
...
{
    level endon( "game_ended" );
    level.propaddtolocation = [];
    level thread _id_1245A();
    scripts\mp\flags::gameflagwait( "prematch_done" );
    wait( level.droptime );
...
node_is_valid()
{
    var_0 = level.propaddtolocation[0];
    level.propaddtolocation[level.propaddtolocation.size] = level.propaddtolocation[0];
    level.propaddtolocation[0] = undefined;
    level.propaddtolocation = scripts\engine\utility::array_removeundefined( level.propaddtolocation );
    return var_0;
}
