...
main()
{
    ...
    scripts\mp\utility\join_team_aggregator::registeronplayerjointeamcallback( ::unmark_on_death );
    ...
    level.br_ammo_player_max_out = nuke_addteamrankxpmultiplier();

    if ( level.br_ammo_player_max_out )
    {
        level.lootchopper_finddroplocation = 0.5;
        level.lootchopper_findunoccupiedpatrolzone = 0.05;
    }

    if ( scripts\cp_mp\utility\game_utility::getmapname() == "mp_m_overwinter" )
    {
        level._id_12601 = 1;
        level.bomb_use_watcher = getdvarint( "scr_always_snowfight", 0 );

        if ( level.bomb_use_watcher )
    ...
    {
        level.br_armor_plate_used = 1;
        level._id_12847 = 1;
    }
    else if ( getdvarint( "scr_arena_stpday" ) == 1 )
    {
        level.br_armor_plate_amount_equipped_set = 1;
        level._id_12847 = 1;
    }
...
onstartgametype()
{
    ...
    {
        if ( !level.bomb_use_watcher && game["roundsPlayed"] == 0 && istrue( game["practiceRound"] ) )
        {
            level.extratime = 30;
            game["didSnowFight"] = 1;
        ...
        else if ( level.bomb_use_watcher )
        {
            level.arenaloadouts = 3;
            var_5 = 0;
        }

        level._id_12604["vanish"] = loadfx( "vfx/core/impacts/small_snowhit" );
        level._id_12604["screen"] = loadfx( "vfx/iw8/weap/_impact/snowball/vfx_imp_snowball_scrn.vfx" );
    ...
    if ( var_5 )
    {
        if ( israndomloadouts() || sat_get_piece() || sat_choose_missing_piece() || sat_computer_think_new() )
        ...
        else if ( ispickuploadouts() )
        {
            ...
            if ( !isdefined( game["roundsPlayed"] ) || isdefined( game["roundsPlayed"] ) && game["roundsPlayed"] == 0 )
            {
                ...
                level.startweapon.variantid = mover_update( level.startweapon.weapon );
                level.arenaweapont1.variantid = mover_update( level.arenaweapont1.weapon );
                level.arenaweapont2.variantid = mover_update( level.arenaweapont2.weapon );
                level.arenaweapont3.variantid = mover_update( level.arenaweapont3.weapon );
                level.arenaweapont4.variantid = mover_update( level.arenaweapont4.weapon );
                level.arenaweapont5.variantid = mover_update( level.arenaweapont5.weapon );
                level.arenaweapont6.variantid = mover_update( level.arenaweapont6.weapon );
                level.arenaweapont7.variantid = mover_update( level.arenaweapont7.weapon );
                level.arenaweapont8.variantid = mover_update( level.arenaweapont8.weapon );
            }
...
updategametypedvars()
{
    ...
    level.br_ammorestock_playeruse = getdvarint( "scr_arena_arenaAttachments", 0 );
    level.br_armor_plate_broken_remove = getdvar( "scr_arena_arenaSuper", "none" );

    if ( level.br_armor_plate_broken_remove == "none" )
            ...
            if ( !isdefined( var_0 ) )
            {
                ...
                if ( var_4 > 3 )
                {
                    ...
                    if ( isdefined( var_8 ) )
                    {
                        ...
                        if ( isdefined( var_8._id_12805 ) )
                            var_0.angles = var_8._id_12805;
                    }
                }
            }
    ...
    if ( !isnormalloadouts() )
    {
        ...
        foreach ( var_2 in level.teamnamelist )
        {
            if ( isdefined( var_0.pers["team"] ) && game["roundWinStreak"][var_2] > 0 && var_0.pers["team"] == var_2 )
            {
                ...
                var_0.pers["gamemodeLoadout"]["roundWinStreakPrimaryCamo"] = _id_123D5();
                var_0.pers["gamemodeLoadout"]["roundWinStreakecondaryCamoTeam"] = var_2;
                var_0.pers["gamemodeLoadout"]["roundWinStreakSecondaryCamo"] = _id_123D5();
            }
        }
    }
...
onjoinedteam()
{
    ...
    for (;;)
    {
        scripts\engine\utility::_id_133EF( "joined_team", "joined_spectators" );
        _id_123E3();
    }
}
...
onspawnfinished()
{
    ...
    if ( istrue( game["practiceRound"] ) || istrue( level.bomb_use_watcher ) )
        thread _id_12603();
    ...
    if ( !scripts\mp\flags::gameflag( "prematch_done" ) && game["roundsPlayed"] == 0 )
    {
        ...
        level scripts\engine\utility::_id_133EF( "prematch_done", "removeArenaOutlines" );
    }
...
onplayerdamaged( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9, var_10, var_11, var_12, var_13 )
{
    ...
    {
        if ( istrue( level._id_12601 ) && ( var_6.basename == "snowball_mp" || var_6.basename == "pball_mp" ) )
            playfxontagforclients( level._id_12604["screen"], var_2, "tag_eye", var_2 );
...
arena_endgame( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    scripts\cp_mp\pet_watch::_id_13028();
...
runarenaloadoutrulesonplayer()
{
    if ( israndomloadouts() || israndompreviewloadouts() || sat_choose_missing_piece() || sat_computer_think_new() )
    {
        if ( israndompreviewloadouts() )
        {
...
buildloadoutsforweaponstreaming()
{
    ...
    if ( ispickuploadouts() )
    {
        ...
        else
        {
            ...
            else if ( istrue( level.play_hud_reminder_vo ) )
            {
                var_4 = game["arenaRandomLoadout"][0]["loadoutPrimaryCamo"];
                var_8 = game["arenaRandomLoadout"][0]["loadoutPrimaryCosmeticAttachment"];
                var_9[0] = game["arenaRandomLoadout"][0]["loadoutPrimarySticker"];
            ...
            if ( run_track_enemy_patrollers() )
                var_1 = scripts\mp\class::delay_show_balloon( var_2, var_4, var_5, level.startweapon.variantid, var_7, var_8, var_9 );
            else
...
buildprimaries( var_0, var_1, var_2 )
{
    ...
    if ( istrue( level._id_12847 ) && istrue( var_2 ) )
    {
        var_8 = oncrateactivate();
        game["arenaRandomLoadout"][game["arenaRandomLoadoutIndex"]]["loadoutPrimaryCamo"] = var_8;

        if ( _id_1315A( var_0 ) )
        {
            var_12 = oncratedestroy();
...
buildsecondaries( var_0, var_1, var_2 )
{
    ...
    else if ( istrue( level.play_hud_reminder_vo ) && istrue( var_2 ) )
    {
        var_7 = oncrateactivate();
        game["arenaRandomLoadout"][game["arenaRandomLoadoutIndex"]]["loadoutSecondaryCamo"] = var_7;
...
_id_12474()
{
    level.arenaweapont1 = delay_safe_spawn_chopper_boss( level.arenaweapont1 );
    level.arenaweapont2 = delay_safe_spawn_chopper_boss( level.arenaweapont2 );
    level.arenaweapont3 = delay_safe_spawn_chopper_boss( level.arenaweapont3 );
    level.arenaweapont4 = delay_safe_spawn_chopper_boss( level.arenaweapont4 );
    level.arenaweapont5 = delay_safe_spawn_chopper_boss( level.arenaweapont5 );
    level.arenaweapont6 = delay_safe_spawn_chopper_boss( level.arenaweapont6 );
    level.arenaweapont7 = delay_safe_spawn_chopper_boss( level.arenaweapont7 );
    level.arenaweapont8 = delay_safe_spawn_chopper_boss( level.arenaweapont8 );
}
...
delay_safe_spawn_chopper_boss( var_0 )
{
    ...
    if ( !issubstr( var_0.weapon, "equip" ) && var_0.weapon != "none" )
    {
        ...
        var_2 = oncrateactivate();
        var_0.disabledfeatures = var_2;

        if ( _id_1315A( var_0.weapon ) )
        {
            var_6 = oncratedestroy();
            var_0.gas_at_computer = var_6;
        }

        if ( _id_1315B( var_0.weapon ) )
        {
            var_7 = oncrateuse();
            var_0._id_12A0C[0] = var_7[0];
            var_0._id_12A0C[1] = var_7[1];
            var_0._id_12A0C[2] = var_7[2];
            var_0._id_12A0C[3] = var_7[3];
        }
...
_id_12431()
{
    level.arenaweapont1 = delay_delete_tv_station_boss_icon( level.arenaweapont1 );
    level.arenaweapont2 = delay_delete_tv_station_boss_icon( level.arenaweapont2 );
    level.arenaweapont3 = delay_delete_tv_station_boss_icon( level.arenaweapont3 );
    level.arenaweapont4 = delay_delete_tv_station_boss_icon( level.arenaweapont4 );
    level.arenaweapont5 = delay_delete_tv_station_boss_icon( level.arenaweapont5 );
    level.arenaweapont6 = delay_delete_tv_station_boss_icon( level.arenaweapont6 );
    level.arenaweapont7 = delay_delete_tv_station_boss_icon( level.arenaweapont7 );
    level.arenaweapont8 = delay_delete_tv_station_boss_icon( level.arenaweapont8 );
}
...
delay_delete_tv_station_boss_icon( var_0 )
{
    ...
    if ( !issubstr( var_0.weapon, "equip" ) && var_0.weapon != "none" )
    {
        ...
        var_7[3] = "none";
        var_8 = scripts\mp\utility\weapon::getweaponrootname( var_0.weapon );
        var_0.weaponobj = scripts\mp\class::delay_show_balloon( var_8, undefined, undefined, var_4, undefined, undefined, scripts\cp_mp\utility\game_utility::isnightmap() );
    }
...
_id_12476()
{
    level.startweapon = _id_12482( level.startweapon );
    level.arenaweapont1 = _id_12482( level.arenaweapont1 );
    level.arenaweapont2 = _id_12482( level.arenaweapont2 );
    level.arenaweapont3 = _id_12482( level.arenaweapont3 );
    level.arenaweapont4 = _id_12482( level.arenaweapont4 );
    level.arenaweapont5 = _id_12482( level.arenaweapont5 );
    level.arenaweapont6 = _id_12482( level.arenaweapont6 );
    level.arenaweapont7 = _id_12482( level.arenaweapont7 );
    level.arenaweapont8 = _id_12482( level.arenaweapont8 );
}
...
_id_12482( var_0 )
{
    ...
    if ( !issubstr( var_0.weapon, "equip" ) && var_0.weapon != "none" )
    {
        ...
        var_1 = move_door_to_pos( var_5 + "_mp" );
...
{
    var_1 = scripts\mp\utility\weapon::getweaponrootname( var_0 );
    var_2 = move_door_to_pos( var_1 + "_mp" );
    return var_2;
}
...
defineplayerloadout( var_0 )
{
    ...
    if ( isdefined( level.startweapon.weapon ) && level.startweapon.weapon != "none" && !issubstr( level.startweapon.weapon, "equip" ) )
    {
        ...
        if ( istrue( level._id_12847 ) )
        {
            ...
            if ( _id_1315A( level.startweapon.weapon ) )
            {
                var_6 = oncratedestroy();
                game["arenaRandomLoadout"][0]["loadoutPrimaryCosmeticAttachment"] = var_6;
            }

            if ( _id_1315B( level.startweapon.weapon ) )
            {
                var_7 = oncrateuse();
        ...
        else if ( istrue( level.play_hud_reminder_vo ) )
        {
            ...
            var_1 = move_spawnpoints_to_ac130( level.startweapon.weapon );
    }
    else
    {
        var_11 = "none";
        ...
        {
            if ( scn_infil_hackney_heli_npc6( level.startweapon.weapon ) )
            {
                ...
                if ( level.br_ammo_player_max_out )
                {
                    ...
                    if ( level.br_ammo_player_max_out == 2 )
                    {
                        var_12 = scripts\engine\utility::ter_op( sat_choose_missing_piece(), "mp/classtable_arena_blueprints.csv", "mp/classTable_arena.csv" );
                        var_4 = int( scripts\mp\gametypes\gun.gsc::nextdest( "iw8_knife" ) );
    ...
    if ( level.allowsupers )
    {
        ...
        if ( issubstr( var_14, "mp_m_" ) && var_14 != "mp_m_speed" )
            level.br_armor_plate_broken_remove = lb_impulse_dmg_factor_mid_low();
    }
    else
    {
        level.br_armor_plate_broken_remove = "none";
        setomnvar( "ui_disable_fieldupgrades", 1 );
    }
    ...
    level.arena_loadouts["default"]["loadoutFieldUpgrade1"] = level.br_armor_plate_broken_remove;
    ...
    if ( !istrue( level._id_12847 ) && !istrue( level.play_hud_reminder_vo ) )
    {
        foreach ( var_16 in level.teamnamelist )
        {
            level.arena_loadouts[var_16]["loadoutPrimaryCamo"] = _id_123D5();
            level.arena_loadouts[var_16]["loadoutSecondaryCamo"] = _id_123D5();
        }
    }
}
...
cacherandomloadouts()
{
    ...
    var_0 = [];
    var_1 = 0;
    var_2 = scripts\engine\utility::ter_op( sat_choose_missing_piece(), "mp/classtable_arena_blueprints.csv", "mp/classTable_arena.csv" );
    ...
    if ( israndompreviewloadouts() )
        var_1 = 144;
    else if ( sat_adjust_think() )
        var_1 = 0;
    else if ( sat_handle_player_disconnect() )
        var_1 = 57;
    else if ( sat_get_signal_strength() )
        var_1 = 93;
    else if ( sat_hack_paused_monitor() )
        var_1 = 123;
    else if ( sat_loop() )
        var_1 = 146;
    else if ( sat_create_interaction() )
        var_1 = 188;
    ...
    if ( var_5 != "" && var_6 > 0.0 )
    {
        ...
        foreach ( var_9 in var_7 )
        {
            ...
            if ( var_10.size == 2 )
            {
                var_11 = var_10[0];
                var_12 = int( var_10[1] );
                var_13 = scripts\mp\utility\weapon::_id_135A8( var_11, var_12 );
...
updateloadoutarray( var_0, var_1 )
{
    ...
    if ( sat_choose_missing_piece() || sat_computer_think_new() )
    ...
    if ( sat_choose_missing_piece() || sat_computer_think_new() )
        var_2["loadoutSecondaryVariantID"] = scripts\mp\class::_id_12B21( var_0, var_1, 1, var_2["loadoutSecondary"] );
    else
    ...
    if ( level.allowsupers )
    {
        ...
        var_2["loadoutFieldUpgrade1"] = level.br_armor_plate_broken_remove;
...
arenaloadouts_select( var_0, var_1, var_2, var_3 )
{
    ...
    for ( var_6 = 0; var_6 < var_1 && var_0.size > 0; var_6++ )
    {
        ...
        if ( isdefined( var_2 ) && isdefined( var_3 ) )
        {
            ...
            var_10 = br_ammo_update_ammotype_weapons( var_2, var_9 );
            ...
            if ( var_10.size > 0 )
            {
                if ( randomfloat( 1.0 ) < var_3 )
                {
                    ...
                    var_7["loadoutPrimary"] = var_12._id_12036;
...
br_ammo_update_ammotype_weapons( var_0, var_1 )
{
    ...
    foreach ( var_4 in var_0 )
    {
        if ( arenaloadouts_getweapongroup( var_4._id_12036 ) == var_1 )
...
mover_update( var_0 )
{
    if ( run_track_enemy_patrollers() && !issubstr( var_0, "equip" ) && var_0 != "none" )
    {
        ...
        if ( var_4 != "" )
        {
            var_5 = scripts\mp\class::_id_12B21( "mp/classtable_arena_blueprints.csv", var_2, 0, var_4 );
            return var_5;
        }
...
setupweapons()
{
    ...
    if ( istrue( level._id_12847 ) || istrue( level.play_hud_reminder_vo ) )
        _id_12474();
    else if ( run_track_enemy_patrollers() )
        _id_12431();
    else
        _id_12476();
...
move_door_to_pos( var_0 )
{
    var_1 = [];

    if ( level.br_ammorestock_playeruse == 1 )
    {
...
spawnweapon( var_0, var_1, var_2 )
{
    ...
    var_4 = lb_impulse_dmg_factor_low( var_0 );
    ...
    if ( var_9 != "" )
    {
        if ( istrue( level._id_12601 ) && var_3 == "equip_snowball" )
        {
            if ( isplayer( var_0 ) )
                var_8 = "single";
            ...
            if ( !isplayer( var_0 ) && var_0.script_label == "6" && !level.vip_playerremoved )
            {
                if ( distance( var_0.origin, ( 0, 0, 138 ) ) < 10 )
                {
                    level.vip_playerremoved = 1;
                    var_3 = "equip_pball";
                    var_8 = "single";
                }
            }
            else if ( !isplayer( var_0 ) && var_0.script_label == "8" && !level.vip_playerremoved )
            {
        ...
        if ( isdefined( var_0.angles ) )
        {
            ...
            {
                if ( isdefined( var_0.script_noteworthy ) && var_0.script_noteworthy == "wall" || isdefined( var_0._id_12196 ) && var_0._id_12196 == "wall" )
        ...
        var_10._id_11C26 = spawn( "trigger_radius", var_10.origin, 0, 32, 32 );
        var_10 thread scriptable_adddamagedcallback();

        if ( istrue( level._id_12601 ) )
        {
            var_10 thread _id_1351D( var_0, var_8 );
    ...
    else
    {
        ...
        if ( istrue( level._id_12847 ) || istrue( level.play_hud_reminder_vo ) || run_track_enemy_patrollers() )
            var_17 = var_1.weaponobj;
        else
        {
            var_16 = move_door_to_pos( var_15 + "_mp" );
        ...
        if ( isdefined( var_0.script_noteworthy ) && var_0.script_noteworthy == "wall" )
        {
            ...
            var_19.chopper_boss_players_connect_monitor = var_22;
        }
...
scriptable_adddamagedcallback()
{
    ...
    {
        wait 0.05;
        self._id_11C26 waittill( "trigger", var_1 );
...
giveflagcapturexp( var_0, var_1 )
{
    ...
    if ( isplayer( var_2 ) )
    {
        ...
        var_2 thread scripts\common\utility::_id_12ED6( level.stack_patch_waittill_context_patch, "capture", var_2.origin );
    }
...
_id_12504()
{
    ...
    if ( istrue( level.arenaflag._id_119FA ) )
        return;
...
getoutlineasset( var_0, var_1 )
{
    ...
    if ( istrue( level._id_12601 ) && isdefined( var_1 ) && var_1 == "equip_pball" )
        var_2 = "outline_depth_yellow";
...
_id_1246F()
{
    ...
    if ( !istrue( level.bomb_use_watcher ) && !istrue( game["practiceRound"] ) )
    {
    ...
    level.vip_playerremoved = 0;
    var_0 = scripts\engine\utility::getstructarray( "weapon_pickup", "targetname" );

    foreach ( var_2 in var_0 )
    {
        if ( istrue( level.bomb_use_watcher ) || istrue( game["practiceRound"] ) || level.arenaloadouts != 3 )
        {
            spawnweapon( var_2, level.arenaweapont1 );
            continue;
        }
...
_id_1351D( var_0, var_1 )
{
    ...
    else
    {
        ...
        playfx( level._id_12604["vanish"], self.origin );
        clearweaponoutlines();

        if ( !isplayer( var_0 ) )
            level thread _id_133E2( var_0 );
...
_id_133E2( var_0 )
{
    level endon( "game_ended" );
    wait 15;
    playfx( level._id_12604["vanish"], var_0.origin );
...
nuke_addteamrankxpmultiplier()
{
    ...
    else if ( scn_infil_hackney_heli_npc6( level.arenaweapont1.weapon ) && scn_infil_hackney_heli_npc6( level.arenaweapont2.weapon ) && scn_infil_hackney_heli_npc6( level.arenaweapont3.weapon ) && scn_infil_hackney_heli_npc6( level.arenaweapont4.weapon ) && scn_infil_hackney_heli_npc6( level.arenaweapont5.weapon ) && scn_infil_hackney_heli_npc6( level.arenaweapont6.weapon ) && scn_infil_hackney_heli_npc6( level.arenaweapont7.weapon ) && scn_infil_hackney_heli_npc6( level.arenaweapont8.weapon ) )
    {
        if ( scn_infil_hackney_heli_npc6( level.startweapon.weapon ) )
            return 2;
        else
...
scn_infil_hackney_heli_npc6( var_0 )
{
...
lb_impulse_dmg_factor_mid_low()
{
    switch ( level.br_armor_plate_broken_remove )
    {
        case "super_weapon_drop":
        case "super_emp_drone":
        case "super_recon_drone":
            level.br_armor_plate_broken_remove = "super_ammo_drop";
            break;
        default:
            break;
    }

    return level.br_armor_plate_broken_remove;
}
