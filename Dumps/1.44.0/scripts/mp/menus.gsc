...
getclasschoice( var_0 )
{
    ...
    if ( isdefined( level.pitcher_lights ) && level.pitcher_lights > 0 )
    {
        if ( scripts\mp\utility\game::matchmakinggame() )
...
executeclasschange( var_0, var_1 )
{
    ...
    else
    {
        ...
        var_3 = 1;
        var_4 = scripts\mp\gamelogic::disablewinonscore( scripts\mp\utility\game::onfieldupgradeendbuffer() );
        ...
        {
            var_5 = scripts\mp\utility\game::rotate_silo_gyro_lights() && self getclientomnvar( "ui_open_loadout_bag" );
...
handleclassedit( var_0 )
{
    ...
    var_3 = scripts\mp\class::smokinggunprogress();
    var_4 = var_2 || var_3;
...
autoassign()
{
    ...
    else
    {
        ...
        if ( istrue( level.teammaxfill ) )
        {
            ...
            {
                if ( scripts\mp\teams::_id_124DB() && var_1 == "team_two_hundred" )
                    continue;
        ...
        if ( var_5.size > 0 )
        {
            ...
            if ( istrue( level.brking_onplayerconnect ) )
                var_9 = 0;
...
setteam( var_0 )
{
    ...
    if ( scripts\mp\utility\game::allowclasschoice() || scripts\mp\utility\game::showfakeloadout() && !isai( self ) )
    {
        if ( getdvarint( "scr_force_cac_sre_callstack", 0 ) == 1 && scripts\mp\utility\game::getgametype() == "br" && scripts\mp\utility\game::onfieldupgradeendbuffer() != "dmz" && scripts\mp\utility\game::onfieldupgradeendbuffer() != "risk" && scripts\mp\utility\game::onfieldupgradeendbuffer() != "sandbox" && scripts\mp\utility\game::onfieldupgradeendbuffer() != "rumble" )
        {
            ...
            scripts\mp\utility\script::handle_train_veh_collision( "setTeam() " + self.name + " ui_options_menu = 2, allowClassChoiceFunc = " + var_1 );
        }
    ...
    if ( ( self.sessionstate == "spectator" || zombiejumpbar() ) && !istrue( self.suicideswitched ) )
    {
...
zombiejumpbar()
{
    return isdefined( level.zombiejumpbar ) && self [[ level.zombiejumpbar ]]();
}
...
leavesquad( var_0, var_1 )
{
    ...
    if ( isdefined( var_0 ) && isdefined( var_1 ) )
        thread scripts\mp\spawnselection::_id_11DA4( var_0, var_1 );
...
waitforclassselect()
{
    ...
    for (;;)
    {
        var_0 = scripts\mp\utility\game::getgametype() == "br" && scripts\mp\utility\game::allowclasschoice() && ( !scripts\mp\flags::gameflag( "prematch_done" ) || istrue( level._id_125B8 ) || istrue( level.cp_vehicle_damage_monitor ) );
        ...
        if ( var_0 || var_1 )
        {
            ...
            if ( !isai( self ) )
            {
                ...
                if ( var_3 && !scripts\mp\utility\game::ring() )
...
beginclasschoice( var_0 )
{
    ...
    if ( scripts\mp\utility\game::allowclasschoice() || scripts\mp\utility\game::showfakeloadout() && !isai( self ) )
    {
        if ( getdvarint( "scr_force_cac_sre_callstack", 0 ) == 1 && scripts\mp\utility\game::getgametype() == "br" && scripts\mp\utility\game::onfieldupgradeendbuffer() != "dmz" && scripts\mp\utility\game::onfieldupgradeendbuffer() != "risk" && scripts\mp\utility\game::onfieldupgradeendbuffer() != "sandbox" && scripts\mp\utility\game::onfieldupgradeendbuffer() != "rumble" )
        {
            ...
            scripts\mp\utility\script::handle_train_veh_collision( "beginClassChoice() " + self.name + " ui_options_menu = 2, allowClassChoiceFunc = " + var_2 );
        }
...
bypassclasschoice()
{
    ...
    if ( sales_discount() && level.enforceantiboosting && !isbot( self ) )
    {
...
addtoteam( var_0, var_1, var_2 )
{
    ...
    if ( var_0 == "spectator" || var_0 == "follower" )
    {
        ...
        scripts\cp_mp\pet_watch::unmarkplayeraseliminated();
    }
    else
