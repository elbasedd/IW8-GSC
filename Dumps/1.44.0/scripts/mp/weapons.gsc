...
init()
{
    ...
    init_function_refs();
    level.update_health_on_spawn = scripts\mp\trials\trial_target_utility::destructiblevehiclesetup();
    level.update_health_think = scripts\mp\trials\trial_target_utility::destructiblevehiclesetup();
...
dochangeweapon( var_0 )
{
    if ( istrue( self.display_ping_hint_with_radial_check ) )
        spawn_claymore_group( var_0 );

    self.display_ping_hint_with_radial_check = 1;
...
_id_121C9()
{
    ...
    scripts\engine\utility::_id_133EF( "death", "weapon_change" );
...
tokenrespawned( var_0 )
{
    ...
    var_1 = "cancel_" + var_0;
    thread togglelightbutton( var_1 );
    childthread tolerance( var_0, var_1 );
    childthread toma_strike_trace_offset( var_0, var_1 );
    childthread toma_strike_munitionused( var_0, var_1 );
}

toma_strike_trace_offset( var_0, var_1 )
{
    for (;;)
    {
        ...
        if ( self isonladder() )
            toggleusbstickinhand( var_0, var_1 );
    }
}

toma_strike_munitionused( var_0, var_1 )
{
    for (;;)
    {
...
toggleusbstickinhand( var_0, var_1 )
{
    ...
    childthread toma_strike( var_0, var_1 );
}

tolerance( var_0, var_1 )
{
    self notify( var_1 );
    self endon( var_1 );
    self._id_11A67 = 1;
...
tokenrespawnwaittime( var_0 )
{
    self notify( var_0 );

    if ( istrue( self._id_11A67 ) )
    {
        self setscriptablepartstate( "weaponVFXViewmodel", "neutral" );
        self setscriptablepartstate( "weaponVFXWorldModel", "neutral" );
    }

    self._id_11A67 = undefined;
}
...
_id_13077( var_0 )
{
    ...
    {
        if ( scripts\cp\killstreaks\nuke_cp::risktokencount( var_0 ) )
            thread scripts\cp\killstreaks\nuke_cp::register_module_died_poorly_func( var_0 );
    }
}
...
updatelauncherusage()
{
    ...
    switch ( var_1 )
    {
        ...
        case "iw8_sn_crossbow":
            thread scripts\cp\helicopter\utility::register_chopper_boss_combat_actions();
            break;
        case "iw8_sn_xmike109":
            thread scripts\cp\stealth\manager::relic_nuketimer_timer();
            break;
        case "iw8_sh_aalpha12":
            thread scripts\cp\drone\emp_drone::relic_nuketimer_timer();
            break;
        case "iw8_me_t9ballisticknife":
            thread scripts\cp\equipment\cp_decoy_grenade::relic_nuketimer_timer();
            break;
    }
    ...
    switch ( var_1 )
    {
        ...
        case "iw8_sn_t9crossbow":
        case "iw8_sn_crossbow":
            thread scripts\cp\helicopter\utility::get_player_aggro_score( var_0 );
            break;
        case "iw8_sn_xmike109":
            thread scripts\cp\stealth\manager::_id_130D7( var_0 );
            break;
        case "iw8_sh_aalpha12":
            thread scripts\cp\drone\emp_drone::_id_130D7( var_0 );
            break;
        case "iw8_me_t9ballisticknife":
            thread scripts\cp\equipment\cp_decoy_grenade::_id_130D7( var_0 );
            break;
    }
}
...
_id_12391( var_0 )
{
    ...
    if ( isdefined( level.setlethalonunresolvedcollision ) )
        self [[ level.setlethalonunresolvedcollision ]]();
}
...
onplayerspawned()
{
    ...
    _id_12391( scripts\engine\utility::ter_op( isdefined( self.spawnweaponobj ), self.spawnweaponobj, isundefinedweapon() ) );
...
savetogglescopestates()
{
    ...
    foreach ( var_2 in var_0 )
    {
        if ( !getqueuedspleveltransients( self.primaryweaponobj ) && var_2 == self.primaryweaponobj || !getqueuedspleveltransients( self.secondaryweaponobj ) && var_2 == self.secondaryweaponobj )
        {
            if ( isdefined( var_2.scope ) && istogglescope( var_2.scope ) && !_id_12A13( var_2.backpiece ) )
            {
                var_3 = scripts\mp\utility\weapon::getcompleteweaponnamenoalt( var_2 );
...
{
    return restorekillstreakplayerangles( var_0 );
}
...
sniperglint_cleanup()
{
    scripts\engine\utility::_id_133EF( "death_or_disconnect", "weapon_change" );
...
{
    self endon( "death" );
    level.update_health_on_spawn scripts\mp\trials\trial_target_utility::determinetrackingcirclesize( self, var_0, self.objweapon );
    ...
    for (;;)
    {
        ...
        if ( var_6 )
        {
            ...
            if ( var_7.size > 2 )
            {
                ...
                foreach ( var_12 in var_7 )
                {
                    ...
                    if ( scripts\mp\utility\weapon::run_and_stop_group_after( var_12 ) )
                        var_9 = 1;
        ...
        level.update_health_think scripts\mp\trials\trial_target_utility::determinetrackingcirclesize( self, var_2, self.objweapon );
...
watchpickupcomplete( var_0, var_1, var_2 )
{
    ...
    if ( var_4 )
    {
        ...
        if ( isdefined( level.update_hint_logic_juggernaut ) )
            [[ level.update_hint_logic_juggernaut ]]( var_0 );
        else if ( var_2 )
...
getrandomgraverobberattachment( var_0, var_1 )
{
    ...
    if ( isdefined( var_1 ) && var_1.size > 0 )
        ...
        var_4 = scripts\mp\utility\weapon::mountstringtodlogenum( var_2 );
    ...
    foreach ( var_8, var_6 in var_3 )
    {
        ...
        if ( !scripts\mp\utility\weapon::breakerstate( var_0, var_7 ) )
        {
    ...
    var_4 = scripts\engine\utility::br_iseliminated( var_4 );
...
{
    if ( !scripts\mp\utility\weapon::breakerstate( var_0, var_1 ) )
        return 0;
...
{
    if ( isdefined( level.unset_relic_squadlink ) )
        [[ level.unset_relic_squadlink ]]( var_0 );
    else
    {
...
handlescavengerbagpickup( var_0 )
{
    ...
    unset_relic_squadlink( var_1 );

    if ( !isdefined( var_1.pers["scavengerPickedUp"] ) )
        var_1.pers["scavengerPickedUp"] = 0;

    var_1 scripts\cp\helicopter\chopper_boss::unset_relic_damage_from_above( "scavengerAmmo", 0 );
...
scavengerbagcleanupthink()
{
    self endon( "death" );
    level endon( "game_ended" );
    self.owner scripts\engine\utility::_id_133F0( "death_or_disconnect", "joined_team", "bag_timeout" );
...
checkhit( var_0, var_1 )
{
    ...
    else
    {
        ...
        scripts\cp\helicopter\chopper_boss::tv_station_reinforcement_spawn_logic( var_0, self.consecutivehitsperweapon[var_3] );
    }
...
watchgrenadethrows()
{
    ...
    switch ( var_0.weapon_name )
    {
        ...
        case "support_box_mp":
            ...
            break;
        case "armor_box_mp":
            thread scripts\mp\equipment\support_box::br_circle_init_func( var_0 );
            break;
        ...
        case "geiger_counter_mp":
            thread _branalytics_addevent::make_silencer_pick_up_interact( var_0 );
            break;
        case "offhand_spotter_scope_mp":
            thread _attachmentblocks::can_use_super_func( var_0 );
            break;
        case "tac_cover_mp":
        ...
        case "advanced_vehicle_drop_marker_mp":
            thread scripts\mp\equipment\advanced_supply_drop::area_structs( var_0 );
            break;
        case "advanced_loot_drop_marker_mp":
            thread scripts\mp\equipment\advanced_supply_drop::are_relics_active( var_0 );
            break;
        case "deploy_weapondrop_mp":
            thread scripts\mp\equipment\weapon_drop::weapondrop_used( var_0 );
            break;
        case "concussion_grenade_mp":
            var_0 thread friendlyfire_allowed();
            break;
        default:
            if ( isdefined( level.unlockableindex ) )
                [[ level.unlockableindex ]]( var_0.weapon_name, var_0 );

            break;
    }

    spawn_convoy_and_move( var_0.weapon_name );
}
...
friendlyfire_allowed()
{
    ...
    thread scripts\mp\equipment\concussion_grenade::uniquepoolid( var_0, var_1 );
}
...
smokegrenadeused( var_0 )
{
    ...
    if ( istrue( var_0 ) )
    {
        ...
        thread _id_125E9( var_5 );
        thread scripts\mp\bots\bots::get_control_station_side_array( var_5 );
    }
    else
...
monitorsmokeactive()
{
    ...
    var_0 = scripts\engine\utility::_id_13403( 9.25, "death" );
...
lockonlaunchers_gettargetarray( var_0 )
{
    ...
    if ( level.teambased )
    {
        ...
        foreach ( var_14 in var_3 )
        {
            ...
            {
                if ( isdefined( var_17 ) && ( !scripts\cp_mp\vehicles\vehicle::_id_13229( var_17, self ) || var_2 ) )
...
lockonlaunchers_gettargetvehiclerefs()
{
    ...
    if ( isdefined( level.lootchopper_droploot ) )
        var_0 = level.lootchopper_droploot;
...
updatemissilefire( var_0 )
{
    ...
    switch ( var_0.weapon_name )
    {
        ...
        case "iw8_la_t9standard_mp":
            ...
            level thread scripts\mp\battlechatter_mp::setup_target_anims( var_0 );
...
monitordisownedequipment( var_0, var_1, var_2 )
{
    ...
    if ( istrue( var_2 ) )
        var_0 scripts\engine\utility::_id_133EF( "joined_team", "disconnect" );
    else
        var_0 scripts\engine\utility::_id_133F0( "joined_team", "joined_spectators", "disconnect" );
...
updateplantedarray( var_0 )
{
    ...
    if ( !isdefined( var_1 ) )
    {
        ...
        scripts\mp\utility\script::handle_train_veh_collision( var_4 );
    }
...
makeexplosiveusabletag( var_0, var_1 )
{
    ...
    for (;;)
    {
        ...
        if ( isdefined( var_4 ) )
        {
            ...
            else if ( isdefined( level.unset_relic_ammo_drain ) )
                [[ level.unset_relic_ammo_drain ]]( self.owner, var_4 );
        }
...
explosivehandlemovers( var_0, var_1 )
{
    ...
    if ( scripts\mp\equipment\claymore::respawn_after_death( var_0 ) )
    {
        var_2.wait_to_reset_wave_loadout = 1;
        self _meth_87B9( 1 );
    }
...
updateviewkickscale( var_0 )
{
    ...
    else if ( isdefined( self.overrideviewkickscale ) )
    {
        ...
        var_1 = scripts\mp\utility\weapon::_id_135A2( self getcurrentweapon() );

        if ( var_1 == 1 )
            var_0 = self.overrideviewkickscalepistol;
        else if ( var_1 == 4 )
            var_0 = self.use_nvg_think;
        else if ( var_1 == 2 )
            var_0 = self.use_respawn_rules;
        else if ( var_1 == 3 )
            var_0 = self.use_struct;
        else if ( var_1 == 5 )
...
getplayerspeedbyweapon( var_0 )
{
    ...
    else
    {
        ...
        if ( !isdefined( var_2 ) )
            ...
            var_1 = 0.85;
        else if ( scripts\mp\utility\weapon::round_at_max( var_2.basename ) )
            var_1 = 0.85;
        else
        {
            if ( var_2.inventorytype != "primary" && var_2.inventorytype != "altmode" || scripts\mp\utility\weapon::run_and_stop_group_after( var_2 ) )
            {
...
stancerecoiladjuster()
{
    ...
    for (;;)
    {
        scripts\engine\utility::_id_133F0( "adjustedStance", "sprint_begin", "weapon_change" );
        wait 0.5;
...
buildattachmentmaps()
{
    ...
    for ( var_3 = scripts\mp\utility\game::rotate_silo_gyro_lights(); var_2 != ""; var_2 = tablelookupbyrow( "mp/attachmentmap.csv", var_1, 0 ) )
    {
        if ( var_3 || scripts\cp_mp\utility\weapon_utility::isweaponnameiw8( var_2 ) || level.mapname == "mp_firingrange" )
    ...
    foreach ( var_21, var_14 in level.weaponmapdata )
    {
        var_15 = var_21;

        if ( getsubstr( var_21, 0, 4 ) == "iw8_" )
            var_15 = getsubstr( var_21, 4 );

        var_16 = "mp/gunsmith/" + var_15 + "_progression.csv";

        if ( !tableexists( var_16 ) )
            continue;

        level.weaponattachments[var_21] = [];
        var_1 = 1;
        var_17 = tablelookupbyrow( var_16, var_1, 0 );
        var_18 = tablelookupbyrow( var_16, var_1, 1 );

        for ( var_19 = "loot/" + var_21 + "_attachment_ids.csv"; var_17 != ""; var_18 = tablelookupbyrow( var_16, var_1, 1 ) )
        {
            if ( var_18 != "" )
            {
                var_20 = tablelookup( var_19, 0, var_18, 1 );

                if ( var_20 != "" )
                    level.weaponattachments[var_21][var_20] = var_20;
            }
    ...
    level.brenableagents = [];
    level.brevent1 = [];
    level.brevent2 = [];
    var_22 = getattachmentlistuniquenames();

    foreach ( var_24 in var_22 )
    {
        var_25 = tablelookup( "mp/attachmenttable.csv", 4, var_24, 2 );
        var_26 = scripts\mp\utility\weapon::attachmentmap_tobase( var_24 );

        if ( var_25 != "" && isdefined( var_26 ) )
        {
            var_27 = level.brevent1[var_26];

            if ( !isdefined( var_27 ) )
                level.brevent1[var_26] = var_25;
            else if ( var_25 != var_27 )
                level.brevent2[var_24] = var_25;
        }

        var_28 = tablelookup( "mp/attachmenttable.csv", 4, var_24, 12 );

        if ( var_28 != "" )
            level.attachmentmap_attachtoperk[var_24] = var_28;

        var_29 = tablelookup( "mp/attachmenttable.csv", 4, var_24, 13 );

        if ( var_29 != "" )
            level.attachmentmap_uniquetoextra[var_24] = var_29;

        var_30 = tablelookup( "mp/attachmenttable.csv", 4, var_24, 9 );

        if ( var_30 != "" )
            level.brenableagents[var_24] = var_30;
    }

    level.attachmentmap_conflicts = [];
    var_32 = 1;

    for ( var_33 = tablelookupbyrow( "mp/attachmentcombos.csv", var_32, 0 ); var_33 != ""; var_33 = tablelookupbyrow( "mp/attachmentcombos.csv", var_32, 0 ) )
    {
        var_35 = 1;

        for ( var_34 = tablelookupbyrow( "mp/attachmentcombos.csv", 0, var_35 ); var_34 != ""; var_34 = tablelookupbyrow( "mp/attachmentcombos.csv", 0, var_35 ) )
        {
            if ( var_32 != var_35 )
            {
                var_36 = tablelookupbyrow( "mp/attachmentcombos.csv", var_32, var_35 );

                if ( !isdefined( level.attachmentmap_conflicts[var_33] ) )
                    level.attachmentmap_conflicts[var_33] = [];

                if ( var_36 != "" )
                    level.attachmentmap_conflicts[var_33][var_34] = var_36;
            }
...
relic_punchbullets_fire_fists()
{
    ...
    for (;;)
    {
        ...
        level._id_135B3[var_3] = var_2;
        var_0++;
    }
}

scr_br_collection_findpath( var_0 )
{
    if ( !isdefined( level._id_135B3 ) )
        relic_punchbullets_fire_fists();

    var_1 = level._id_135B3[var_0];

    if ( isdefined( var_1 ) && ( var_1 == "t9" || var_1 == "s4" ) )
    {
        if ( !istrue( level._id_1347B ) )
            return 0;
...
buildweaponmap()
{
    level.weaponmapdata = [];
    level._id_135A5 = [];
    var_0 = scripts\mp\utility\game::rotate_silo_gyro_lights();
    ...
    for ( var_2 = 0; var_2 < var_1; var_2++ )
    {
        ...
        if ( var_4 != "" && ( var_0 || scripts\cp_mp\utility\weapon_utility::isweaponnameiw8( var_4 ) || level.mapname == "mp_firingrange" ) )
        {
            ...
            if ( var_6 != "" )
            {
                ...
                if ( var_7 != "" )
                {
                    ...
                    if ( var_8 > -1 && scr_br_collection_findpath( var_4 ) && !var_9 )
                    {
                        if ( !isdefined( level._id_135A5[var_6] ) )
                            level._id_135A5[var_6] = [];

                        level._id_135A5[var_6][level._id_135A5[var_6].size] = var_4;
                    }
                    else
                        level.weaponmapdata[var_4]._id_12FB6 = 1;
                }
            }
            ...
            if ( var_11 != "" )
                level.weaponmapdata[var_4].startuseweapon = var_11;
    ...
    for (;;)
    {
        ...
        if ( !var_0 && !scripts\cp_mp\utility\weapon_utility::isweaponnameiw8( var_4 ) && level.mapname != "mp_firingrange" )
            continue;
        ...
        level.weaponlootmapdata[var_22].rpg_shoot_at_trigs = int( var_21 ) != 0 && int( var_24 ) == 99;
        level.weaponlootmapdata[var_22].retreat_vehicle_collision_clear = scr_br_collection_findpath( var_4 );
    ...
    foreach ( var_48, var_39 in var_17 )
    {
        var_40 = 1;
        var_41 = 0;
        var_42 = [];
        ...
        for (;;)
        {
            var_43 = var_48 + "|" + var_40;

            if ( isdefined( level.weaponlootmapdata[var_43] ) )
            {
                var_41++;

                if ( var_42.size )
                {
                    foreach ( var_45 in var_42 )
                    {
                        var_46 = var_48 + "|" + var_45;
                        level.weaponlootmapdata[var_46] = spawnstruct();
                        level.weaponlootmapdata[var_46].variantid = var_45;
                        level.weaponlootmapdata[var_46].rpg_shoot_at_trigs = 1;
                        level.weaponlootmapdata[var_46].retreat_vehicle_collision_clear = 0;
                    }

                    var_42 = [];
                }
            }
            else if ( var_41 == var_39 || var_42.size >= var_37 )
                break;
            else
                var_42[var_42.size] = var_40;
...
{
    if ( isdefined( var_0 ) )
        var_0 scripts\engine\utility::_id_13403( var_2, "death_or_disconnect" );
...
weaponhasselectableoptic( var_0 )
{
    ...
    foreach ( var_4 in var_2 )
    {
        ...
        if ( var_5 == "rail" )
        {
            ...
            if ( scripts\mp\utility\weapon::breakerstate( var_1, var_6 ) )
                return 1;
        }
    }
...
watchdropweapons()
{
    ...
    for (;;)
    {
        ...
        if ( isdefined( var_0 ) && isdefined( var_1 ) && !scripts\mp\utility\weapon::ismeleeonly( var_1 ) && !scripts\mp\utility\weapon::run_and_stop_group_after( var_1 ) && !scripts\mp\utility\weapon::isknifeonly( var_1 ) )
        {
...
_id_11D28()
{
    ...
    foreach ( var_1 in level.players )
    {
        if ( isdefined( var_1 ) && isdefined( var_1._id_11D2B ) )
        {
            var_1._id_11D2B = undefined;
            var_1 notify( "endReactiveCamoThread" );
        }
    }
}

_id_11D29( var_0, var_1, var_2 )
{
    var_0._id_128DB = var_1;

    if ( var_2 == 1 )
        self setscriptablepartstate( "activeCamo", "stage" + var_0._id_128DB );
    else
        self setscriptablepartstate( "activeCamo", "init_stage" + var_0._id_128DB );
}

_id_11D2A( var_0 )
{
    if ( !isdefined( self._id_11D2B ) )
        self._id_11D2B = [];

    if ( !isdefined( self._id_11D2B[var_0] ) )
    {
        self._id_11D2B[var_0] = spawnstruct();
        self._id_11D2B[var_0].kills = 0;
        self._id_11D2B[var_0]._id_128DB = 0;
    }
}
...
_id_11D25( var_0 )
{
    ...
    if ( !isdefined( level._id_11D27 ) && !scripts\mp\flags::gameflag( "prematch_done" ) )
    {
        level._id_11D27 = 1;
        level thread _id_11D28();
    }

    var_1 = _id_135A3( var_0 );
    _id_11D2A( var_1 );
    var_2 = self._id_11D2B[var_1];
    _id_11D29( var_2, var_2._id_128DB, 0 );
    ...
    while ( var_2._id_128DB < var_4 )
    {
        scripts\engine\utility::waittill_either( "got_a_kill", "scr_advancereactivecamo" );
        var_6 = int( var_5[var_2._id_128DB + 1] );
        var_2.kills++;

        if ( var_2.kills >= var_6 )
        {
            _id_11D29( var_2, var_2._id_128DB + 1, 1 );
...
run_elevator_spawners_event( var_0 )
{
...
{
    _id_11A58( "j_shoulder_ri", "teslaLimb" );
    _id_11A58( "j_shoulder_le", "teslaLimb" );
    _id_11A58( "j_elbow_ri", "teslaLimb" );
    _id_11A58( "j_elbow_le", "teslaLimb" );
    _id_11A58( "j_hip_ri", "teslaLimb" );
    _id_11A58( "j_hip_le", "teslaLimb" );
    _id_11A58( "j_knee_ri", "teslaLimb" );
    _id_11A58( "j_knee_le", "teslaLimb" );
    _id_11A58( "j_spineupper", "teslaTorso" );
    _id_11A58( "j_head", "teslaHead" );
...
{
    _id_11A58( "j_shoulder_ri", "8BitLimb" );
    _id_11A58( "j_shoulder_le", "8BitLimb" );
    _id_11A58( "j_elbow_ri", "8BitLimb" );
    _id_11A58( "j_elbow_le", "8BitLimb" );
    _id_11A58( "j_hip_ri", "8BitLimb" );
    _id_11A58( "j_hip_le", "8BitLimb" );
    _id_11A58( "j_knee_ri", "8BitLimb" );
    _id_11A58( "j_knee_le", "8BitLimb" );
    _id_11A58( "j_spineupper", "8BitTorso" );
    _id_11A58( "j_head", "8BitHead" );
    self playsound( "8bit_death_sfx" );
    var_0 hide();
}

_id_11A58( var_0, var_1 )
{
...
{
    if ( !isdefined( self.delayeddetachbreak ) )
        self.delayeddetachbreak = [];
    ...
    self.delayeddetachbreak[mp_backlot2_patch( var_1 )] = var_1;
    thread startburnfx();
}
...
enableburnsfx()
{
    ...
    if ( !isdefined( self.burnsfx ) )
    {
        ...
        thread delayeventfired( self.burnsfx );
        wait 0.05;
    }
...
disableburnfx( var_0, var_1 )
{
    if ( isdefined( var_1 ) )
    {
        if ( self.delayeddetachbreak[mp_backlot2_patch( var_1 )] == var_1 )
            self.delayeddetachbreak[mp_backlot2_patch( var_1 )] = undefined;
    }
    else
        self.delayeddetachbreak = [];

    if ( self.delayeddetachbreak.size > 0 )
        thread startburnfx();
    else
    {
        thread stopburnfx();
...
clearburnfx()
{
    ...
    self.delayeddetachbreak = undefined;
}
...
startburnfx()
{
    ...
    var_0 = nocrash( self.delayeddetachbreak );
...
_id_12252( var_0, var_1 )
{
    ...
    var_0 scripts\cp_mp\utility\damage_utility::little_bird_mg_handleflarefire();
    var_0 dodamage( var_0.maxhealth, var_1, self, undefined, "MOD_EXPLOSIVE", getcompleteweaponname( "semtex_mp" ) );
    var_0 scripts\cp_mp\utility\damage_utility::little_bird_mg_handleflarerecharge();
}
