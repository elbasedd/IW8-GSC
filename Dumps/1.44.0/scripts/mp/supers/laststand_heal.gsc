...
laststandheal_onset()
{
    ...
    for (;;)
    {
        if ( self.laststandheal_hassuperweapon && ( !istrue( self.inlaststand ) || istrue( self.stuckinlaststand ) || istrue( scripts\mp\playerlogic::movenavobstaclemonitor() ) ) )
        {
        ...
        else if ( !self.laststandheal_hassuperweapon && istrue( self.inlaststand ) && !istrue( self.stuckinlaststand ) && !istrue( scripts\mp\playerlogic::movenavobstaclemonitor() ) )
        {
            var_0 = scripts\mp\supers::getcurrentsuper().staticdata.weapon;
...
laststandheal_setinactivewhendone()
{
    level endon( "game_ended" );
    self endon( "disconnect" );
    scripts\engine\utility::_id_133FE( "death", "last_stand_finished" );
