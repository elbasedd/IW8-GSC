...
init()
{
    ...
    level.airdrop_crushchicken = [];
    level.airdrop_givecrateuseweapon = [];
    level.accessoryattachment = [];
    level.accessorylogic = [];
    level.accessoryfullweapon = [];
    level.accessoryweaponbyindex = [];
    level.aigroundturretstate = [];
    level.airdrop_applyimmediatejuggernaut = [];
    level.aigroundturretref = [];
    level.airdop_brloadoutcratefirstactivation = [];
    var_0 = 0;
    ...
    for (;;)
    {
        ...
        if ( var_6 != "" )
        {
            ...
            level.airdrop_crushchicken[var_5] = var_6;
        }

        var_7 = tablelookupbyrow( "mp/accessorytable.csv", var_0, 16 );

        if ( var_7 != "" )
            level.airdop_brloadoutcratefirstactivation[var_3] = var_7;

        var_8 = tablelookupbyrow( "mp/accessorytable.csv", var_0, 17 );

        if ( var_8 != "" && var_7 != "" )
            level.aigroundturretstate[var_3 + "+" + var_7] = var_3 + "+" + var_8;

        var_9 = tablelookupbyrow( "mp/accessorytable.csv", var_0, 18 );

        if ( var_9 != "" && var_7 != "" )
            level.airdrop_applyimmediatejuggernaut[var_3 + "+" + var_7] = var_3 + "+" + var_9;

        var_10 = tablelookupbyrow( "mp/accessorytable.csv", var_0, 21 );

        if ( var_10 != "" )
            level.airdrop_bronloadoutcratedestroyed[var_3] = var_10;

        var_11 = tablelookup( "mp/itemsourcetable.csv", 2, var_1, 3 );

        if ( var_11 != "" )
            level.airdrop_givecrateuseweapon[var_2] = var_11;
...
{
    if ( isdefined( level.airdrop_crushchicken ) )
        return level.airdrop_crushchicken[var_0];

    return "none";
}

mortars_fire_projectile( var_0 )
{
    if ( isdefined( level.airdrop_givecrateuseweapon ) )
        return level.airdrop_givecrateuseweapon[var_0];
    ...
    if ( isdefined( var_2 ) )
    {
        switch ( var_2 )
        {
            ...
            case "thermometer":
                ...
                break;
            case "holo":
                scripts\cp_mp\gestures::_id_129BC();
                break;
            case "holo2":
                scripts\cp_mp\gestures::_id_129BD();
                break;
            case "holo3":
                scripts\cp_mp\gestures::_id_129BE();
                break;
            default:
                break;
        }
...
obit_activation( var_0 )
{
    ...
    if ( isdefined( var_3 ) )
    {
        ...
        {
            case "fem":
                var_4 = level.airdrop_applyimmediatejuggernaut[var_0];
                var_5 = level.aigroundturretref[var_0];
                ...
                break;
            case "big":
                var_6 = level.aigroundturretstate[var_0];
...
{
    if ( var_0 == 1 )
        scripts\cp_mp\gestures::_id_129AB();
    else if ( var_0 == 2 )
...
relic_mythic_should_ai_play_pain()
{
    if ( !isdefined( self._id_12C48 ) )
    {
        self._id_12C48 = 1;
        var_0 = nuke_slamtoblack();
