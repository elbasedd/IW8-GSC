...
callback_playerdamage_internal( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9, var_10, var_11, var_12, var_13, var_14 )
{
    ...
    if ( istrue( var_2.unset_just_keep_moving ) && isdefined( var_6 ) )
    {
        if ( !removegasmaskbr( var_6.basename ) )
            return;
    }
    ...
    else
    {
        ...
        if ( isdefined( var_0 ) && istrue( var_0.delayedattach ) )
            var_20 = 0;
        ...
        {
            if ( istrue( game["inLiveLobby"] ) || !scripts\mp\flags::gameflag( "prematch_done" ) || istrue( level.proper_damage ) )
            {
        ...
        if ( isdefined( var_1 ) && istrue( var_1.cranked ) )
            var_1 thread scripts\mp\cranked::uav_getenemyplayersinrange( var_2 );
        ...
        if ( saw_pickup_think( var_6 ) && scripts\mp\utility\damage::validshotcheck( var_5, var_1 ) )
        {
            if ( isdefined( var_6.riskspawn_initialset ) )
                var_2 thread init_createfx( var_9, var_1, var_6 );

            if ( var_6.basename != "iw8_pi_t9pistolshot_mp" )
                var_3 = _id_1288D( var_2, var_1, var_6, var_3, var_4 );
            else if ( var_3 == 0 )
                return;
        }
        ...
        if ( router_use_obj( var_1, var_2, var_3, var_5, var_6, var_13 ) )
            return;
        ...
        if ( rocket_fuel_stability( var_0 ) )
            var_3 = 0;

        if ( isdefined( var_1 ) && ( isplayer( var_1 ) || istrue( var_1.blueprintextractchance ) ) && scripts\mp\utility\game::getgametype() == "br" )
        {
            if ( armorvest_washit( var_1 ) || helmet_washit( var_1 ) )
                var_4 = var_4 | level.prematchmusic;

            if ( armorvest_wasbroke( var_1 ) || helmet_wasbroke( var_1 ) )
                var_4 = var_4 | level.prematchloadouts;

            if ( isdefined( level.unset_relic_gas_martyr ) )
                var_4 = var_4 | [[ level.unset_relic_gas_martyr ]]( var_2 );
        }

        if ( sat_wait_for_antenna( var_1 ) )
            var_3 = _id_121C7( var_1, var_3, var_5 );

        if ( isdefined( level.unset_relic_focus_fire ) )
        {
            if ( ![[ level.unset_relic_focus_fire ]]( var_5 ) )
                return;
        }
        ...
        if ( isplayer( var_1 ) && var_1 scripts\mp\utility\perk::_hasperk( "specialty_br_ping_on_damage" ) && var_32 >= var_38 )
        {
            if ( isplayer( var_2 ) && var_2 scripts\mp\utility\perk::_hasperk( "specialty_coldblooded" ) && var_2 != var_1 )
                var_1 thread scripts\mp\perks\perkfunctions::vote_player_set();
            else
                var_1 scripts\mp\perks\perkfunctions::infil_lbravo_damage_monitor( var_2 );
        }
...
init_createfx( var_0, var_1, var_2 )
{
    self notify( "newDragonsBreathHitLoc" );
    self endon( "newDragonsBreathHitLoc" );

    if ( !isdefined( self.init_create_script_for_level ) )
        self.init_create_script_for_level = scripts\cp\killstreaks\nuke_cp::_id_127C3();

    self.init_create_script_for_level.trackriotshield_watchcancelswitchaway = scripts\engine\utility::array_add( self.init_create_script_for_level.trackriotshield_watchcancelswitchaway, var_0 );
    waittillframeend;
    var_3 = [];
    ...
    if ( isdefined( self.init_create_script_for_level ) )
    {
        var_3 = self.init_create_script_for_level.trackriotshield_watchcancelswitchaway;
        self.init_create_script_for_level.trackriotshield_watchcancelswitchaway = [];
    }

    if ( scripts\mp\utility\player::isreallyalive( self ) )
        scripts\cp\killstreaks\nuke_cp::_id_12372( var_3, var_1, var_2 );
    else
        scripts\cp\killstreaks\nuke_cp::_id_12373();
}
...
postplayerdamaged( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9, var_10, var_11, var_12, var_13, var_14, var_15, var_16 )
{
    ...
    else if ( isdefined( var_4 ) && var_4 & level.idflags_penetration && !( var_4 & level.prepare_mission_failed_text ) )
    {
...
meleestagger( var_0 )
{
    ...
    if ( isdefined( level.lootchopper_finddroplocation ) )
        self setviewkickscale( scripts\engine\utility::ter_op( var_0 scripts\mp\utility\perk::_hasperk( "specialty_hardmelee" ), level.lootchopper_finddroplocation * 2, level.lootchopper_finddroplocation ) );
    else
...
{
    scripts\engine\utility::_id_13403( 2.0, "death_or_disconnect" );
    var_0 destroy();
}
...
allowdamageflash( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    if ( _id_12AF3( var_0, var_1, var_2, var_3, var_4 ) )
        return 0;

    return 1;
}

_id_12AF3( var_0, var_1, var_2, var_3, var_4 )
{
    if ( isdefined( var_2 ) )
    {
...
_id_135A7( var_0, var_1, var_2, var_3, var_4, var_5, var_6 )
{
    var_7 = var_3.basename;

    if ( removegasmaskbr( var_7 ) )
        return 1;

    if ( isdefined( level.completesmokinggunquest ) && isdefined( var_7 ) )
    {
        if ( istrue( level.completesmokinggunquest._id_136CB ) && var_7 == "iw8_fists_mp_zmb" || istrue( level.completesmokinggunquest.open_this_door ) && var_7 == "iw8_fists_mp_gxp" )
            return 1;
    }
...
cac_modified_damage( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9, var_10, var_11 )
{
    ...
    if ( isplayer( var_1 ) || isdefined( var_1 ) && istrue( var_1.blueprintextractchance ) )
    {
    ...
    if ( scripts\engine\utility::isbulletdamage( var_3 ) )
    {
        ...
        if ( scripts\mp\utility\weapon::run_cleanup_funcs_for_unused_objectives( var_17 ) && scripts\mp\utility\game::getgametype() == "br" )
            var_2 = var_2 * scripts\mp\gametypes\br_jugg_common.gsc::nuke_vault_runner();
        ...
        var_23 = istrue( var_1.setheadicon_addclienttomask ) && scripts\mp\utility\weapon::getweaponrootname( var_4 ) == "iw8_sn_crossbow";
        ...
        if ( var_0 scripts\mp\utility\killstreak::isjuggernaut() )
            var_2 = survival_wave_struct( var_4, var_2, var_25, 0, var_1 );

        if ( isdefined( var_1 ) && isdefined( var_1._id_132C1 ) && var_1._id_132C1 == "loot_chopper" )
            var_2 = int( var_2 / 3 );
    }
    else if ( isexplosivedamagemod( var_3 ) )
    {
        ...
        {
            var_34 = survival_ai_manager( var_2, var_1, var_0 );
    ...
    else if ( var_3 == "MOD_FIRE" )
    {
        if ( var_0 scripts\mp\utility\killstreak::isjuggernaut() )
        {
            if ( isdefined( var_4.basename ) && var_4.basename == "thermite_ap_mp" || var_4.basename == "thermite_bolt_mp" )
                var_2 = survival_wave_struct( var_4, var_2, 0, 1, var_1 );
        }
    ...
    var_35 = _id_135A7( var_1, var_0, var_2, var_4, var_3, var_8, var_7 );
    var_36 = [ "MOD_GRENADE_SPLASH", "MOD_FIRE", "MOD_EXPLOSIVE", "MOD_PROJECTILE_SPLASH", "MOD_EXPLOSIVE_BULLET" ];
    ...
    {
        var_38 = var_0 scripts\mp\gametypes\br_public.gsc::damagearmor( var_2 );
        ...
        if ( isplayer( var_1 ) )
        {
            ...
            else if ( isdefined( level.tv_station_infil_enemy_combat_logic ) )
                var_1 [[ level.tv_station_infil_enemy_combat_logic ]]( var_39 );
        }
        else if ( isdefined( var_1 ) && istrue( var_1.blueprintextractchance ) )
            armorvest_sethit( var_1 );
    }
    ...
    if ( isplayer( var_1 ) || isdefined( var_1 ) && istrue( var_1.blueprintextractchance ) )
    {
        var_41 = var_0 scripts\mp\gametypes\br_public.gsc::hasarmor();
        var_42 = var_0 scripts\mp\gametypes\br_public.gsc::hashelmet();
    ...
    if ( istrue( var_0.inlaststand ) && getdvarfloat( "scr_player_lastStandHealthScalar", 0 ) > 0 )
    {
        ...
        if ( isdefined( level.support_box_update_hint_logic ) )
        {
            ...
            var_43 = self [[ level.support_box_update_hint_logic ]]( var_44 );
        }
...
survival_ai_manager( var_0, var_1, var_2 )
{
...
survival_wave_struct( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    if ( isdefined( var_4 ) && var_4 scripts\mp\utility\killstreak::isjuggernaut() && scripts\mp\utility\game::getgametype() == "br" )
        var_6 = scripts\mp\gametypes\br_jugg_common.gsc::notcanon();
    ...
    switch ( var_5 )
    {
        ...
        case "mg":
        case "rifle":
            if ( scripts\mp\utility\weapon::run_cleanup_funcs_for_unused_objectives( var_0.basename ) && scripts\mp\utility\game::getgametype() == "br" )
                var_1 = min( 50, var_1 );
            else
                var_1 = min( 20, var_1 );
...
router_use_obj( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    if ( isdefined( var_0 ) && isplayer( var_0 ) && ( !isalive( var_0 ) || scripts\mp\utility\player::round_spawn_enemies( var_0 ) ) )
    {
...
_id_1288D( var_0, var_1, var_2, var_3, var_4 )
{
    if ( isdefined( var_1 ) && isdefined( var_0 ) )
    {
        if ( var_4 & level.prepdoorsforunload )
            var_5 = "lHandWeap";
        else
            var_5 = "rHandWeap";

        if ( !isdefined( var_1.vehicle_travel_player_connect_monitor ) )
            var_1.vehicle_travel_player_connect_monitor = [];

        var_6 = "" + var_0 getentitynumber();
        var_7 = gettime();

        foreach ( var_13, var_9 in var_1.vehicle_travel_player_connect_monitor )
        {
            foreach ( var_12, var_11 in var_9 )
            {
                if ( ( var_7 - var_11.time ) / 1000 > 0.1 )
                {
                    var_1.vehicle_travel_player_connect_monitor[var_13] = scripts\engine\utility::array_remove_key( var_1.vehicle_travel_player_connect_monitor[var_13], var_12 );

                    if ( var_1.vehicle_travel_player_connect_monitor[var_13].size == 0 )
                        var_1.vehicle_travel_player_connect_monitor = scripts\engine\utility::array_remove_key( var_1.vehicle_travel_player_connect_monitor, var_13 );
                }
            }
        }

        if ( !isdefined( var_1.vehicle_travel_player_connect_monitor[var_5] ) )
            var_1.vehicle_travel_player_connect_monitor[var_5] = [];

        if ( !isdefined( var_1.vehicle_travel_player_connect_monitor[var_5][var_6] ) )
        {
            var_1.vehicle_travel_player_connect_monitor[var_5][var_6] = spawnstruct();
            var_1.vehicle_travel_player_connect_monitor[var_5][var_6].time = var_7;
            var_1.vehicle_travel_player_connect_monitor[var_5][var_6].vehicle_trackturretprojectile = [];
        }

        var_14 = ondamagerelicdoomslayer( var_1, var_2 );
        var_15 = var_1.vehicle_travel_player_connect_monitor[var_5][var_6].vehicle_trackturretprojectile;

        if ( var_1.vehicle_travel_player_connect_monitor[var_5][var_6].vehicle_trackturretprojectile.size >= var_14 )
        {
            if ( getdvarint( "scr_spread_use_highest_damage_pellets", 0 ) )
            {
                foreach ( var_18, var_17 in var_1.vehicle_travel_player_connect_monitor[var_5][var_6].vehicle_trackturretprojectile )
                {
                    if ( var_3 > var_17 )
                    {
                        var_1.vehicle_travel_player_connect_monitor[var_5][var_6].vehicle_trackturretprojectile[var_18] = var_3;
                        var_1.vehicle_travel_player_connect_monitor[var_5][var_6].vehicle_trackturretprojectile = scripts\engine\utility::array_sort_with_func( var_1.vehicle_travel_player_connect_monitor[var_5][var_6].vehicle_trackturretprojectile, ::required_tank_capacity );
                        return var_3 - var_17;
                    }
                }
            }
        ...
        {
            var_1.vehicle_travel_player_connect_monitor[var_5][var_6].vehicle_trackturretprojectile = scripts\engine\utility::array_add( var_1.vehicle_travel_player_connect_monitor[var_5][var_6].vehicle_trackturretprojectile, var_3 );
            var_1.vehicle_travel_player_connect_monitor[var_5][var_6].vehicle_trackturretprojectile = scripts\engine\utility::array_sort_with_func( var_1.vehicle_travel_player_connect_monitor[var_5][var_6].vehicle_trackturretprojectile, ::required_tank_capacity );
        }
    }
...
required_tank_capacity( var_0, var_1 )
{
    return var_0 < var_1;
}

ondamagerelicdoomslayer( var_0, var_1 )
{
...
applystoppingpower( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    ...
    if ( var_6 == 2 )
    {
        if ( _id_12A53( var_0, var_1, var_2, var_3 ) )
            return var_5;
        else
            return 0.9 * var_5;
    }
...
_id_12A53( var_0, var_1, var_2, var_3 )
{
...
killstreakdamagefilter( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    if ( isdefined( level.lootchopper_getattackerdata ) )
        var_5 = [[ level.lootchopper_getattackerdata ]]( var_0, var_1, var_2, var_3, var_4 );
...
handlefriendlyfiredamage( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9, var_10, var_11, var_12, var_13 )
{
    ...
    if ( level.friendlyfire != 0 )
    {
        if ( level.maxallowedteamkills == -1 )
            ...
            var_14 = istrue( var_2.roof_rpg_behavior ) || istrue( var_2.isdefusing ) || istrue( var_2.isplanting );

        var_15 = rooftop_objective( var_1 );
    ...
    else if ( level.friendlyfire == 1 )
    {
        ...
        spawn_elevators( var_2, var_1, var_3, var_5 );
...
suspensemusic_reset( var_0, var_1 )
{
    var_2 = scripts\mp\utility\weapon::getweaponrootname( var_1.basename );
...
switch_timer( var_0, var_1, var_2, var_3, var_4 )
{
    if ( isdefined( var_1 ) )
    {
        if ( respawn_waits( var_0, var_4 ) )
        {
            if ( var_2.health > 0 && var_3 >= var_2.health )
                var_3 = var_2.health - 1;
        }
        else if ( isdefined( var_0._id_12873 ) )
        {
            if ( !isdefined( var_1._id_1286E ) )
                var_1._id_1286E = [];

            var_5 = "" + var_2 getentitynumber();
            var_6 = gettime();
            var_1._id_1286E[var_5] = spawnstruct();
            var_1._id_1286E[var_5].time = var_6;
            var_1._id_1286E[var_5].print_punchcard = var_0._id_12873.basename;
        }
        else if ( isdefined( var_1._id_1286E ) )
        {
            var_6 = gettime();

            foreach ( var_9, var_8 in var_1._id_1286E )
            {
                if ( var_6 != var_8.time )
                    var_1._id_1286E = scripts\engine\utility::array_remove_key( var_1._id_1286E, var_9 );
            }

            var_9 = "" + var_2 getentitynumber();

            if ( scripts\engine\utility::array_contains_key( var_1._id_1286E, var_9 ) && var_1._id_1286E[var_9].print_punchcard == var_0.basename )
            {
                var_3 = 0;
                var_1._id_1286E = scripts\engine\utility::array_remove_key( var_1._id_1286E, var_9 );
            }

            if ( var_1._id_1286E.size == 0 )
                var_1._id_1286E = undefined;
        }
    }
...
suicidedfromspawnwait( var_0, var_1, var_2, var_3, var_4 )
{
    if ( scripts\mp\utility\game::isanymlgmatch() && isdefined( var_0 ) && isplayer( var_0 ) && !nullweapon( var_3 ) && scripts\mp\utility\weapon::isprimaryweapon( var_3 ) && scripts\engine\utility::isbulletdamage( var_2 ) && suicide_on_alive( var_4 ) )
    {
...
switch_disabled( var_0, var_1, var_2 )
{
    var_3 = var_1;
    ...
    {
        if ( var_0 scripts\mp\utility\killstreak::isjuggernaut() )
            var_3 = var_0 scripts\mp\juggernaut::search_turret_no_target_stop_delay();
    }

    return var_3;
}

switchents( var_0, var_1, var_2, var_3, var_4 )
{
    var_5 = var_3;
    ...
    if ( isdefined( var_0 ) && var_0 scripts\cp_mp\vehicles\vehicle::isvehicle() )
    {
        if ( var_2 scripts\mp\utility\killstreak::isjuggernaut() )
            var_5 = var_2 scripts\mp\juggernaut::searchcircleorigin( var_3, var_4 );
    }

    return var_5;
}

switchedtohardpointspawnlogic( var_0, var_1 )
{
    var_2 = var_1;
    var_3 = var_0 scripts\cp_mp\utility\player_utility::getvehicle();

    if ( isdefined( var_3 ) )
        var_2 = scripts\cp_mp\vehicles\vehicle_damage::_id_131D5( var_3, var_0, var_1 );

    return var_2;
}

swapped_car( var_0, var_1, var_2, var_3, var_4 )
{
    var_5 = var_3;
    ...
    if ( isdefined( var_4 ) && var_4 == "MOD_CRUSH" )
    {
        if ( isdefined( var_0 ) && isdefined( var_1 ) && var_0 == var_1 && var_1 scripts\mp\utility\killstreak::isjuggernaut() && var_2 scripts\mp\utility\killstreak::isjuggernaut() )
            var_5 = var_2 scripts\mp\juggernaut::search_update_delay( var_3 );
    }
...
{
    var_3 = survivorsupertwo( var_3, var_6 );
    var_3 = suspensemusic_reset( var_3, var_6 );
    var_3 = suicidedfromspawnwait( var_1, var_3, var_5, var_6, var_9 );
    var_3 = switch_disabled( var_2, var_3, var_5 );
    var_3 = switchents( var_0, var_1, var_2, var_3, var_5 );
    var_3 = switchedtohardpointspawnlogic( var_2, var_3 );
    var_3 = swapped_car( var_0, var_1, var_2, var_3, var_5 );
    var_3 = switch_timer( var_6, var_1, var_2, var_3, var_5 );
    ...
    {
        if ( istrue( var_6.isalternate ) && scripts\mp\weapons::restore_ai_weapon( var_6.underbarrel ) )
        {
...
filterdamage( var_0, var_1, var_2, var_3, var_4, var_5, var_6 )
{
    ...
    if ( scripts\cp_mp\vehicles\vehicle::_id_13270( var_0, var_2, var_4, var_5 ) )
...
logattacker( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9 )
{
    ...
    if ( isdefined( var_2 ) && isdefined( var_2.owner ) && isplayer( var_1.owner ) )
    {
        ...
        if ( var_10 && !istrue( var_2.owner._id_13111 ) && !isdefined( self.attackerdata[var_2.owner.guid] ) )
...
handledamagefeedback( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9, var_10 )
{
    ...
    if ( isdefined( var_12 ) && var_12 != var_2 && var_3 + var_8 + var_9 > 0 && ( !isdefined( var_6 ) || var_6 != "shield" ) )
    {
        ...
        var_18 = !saw_pickup_think( var_5 ) && scripts\mp\utility\damage::isheadshot( var_6, var_4, var_1 );
        var_19 = 1;
        var_20 = var_4 == "MOD_MELEE";
        var_21 = "" + var_2 getentitynumber();
        var_22 = 0;

        if ( !var_17 && saw_pickup_think( var_5 ) && isdefined( var_12.vehicle_travel_player_connect_monitor ) )
        {
            foreach ( var_24 in var_12.vehicle_travel_player_connect_monitor )
            {
                if ( isdefined( var_24[var_21] ) && var_24[var_21].vehicle_trackturretprojectile.size > 1 )
                    var_22 = 1;
            }
        }
...
lethalequipmentdamagemod( var_0, var_1, var_2, var_3, var_4, var_5, var_6 )
{
    ...
    if ( isdefined( level._id_1249F ) )
        var_3 = var_3 * [[ level._id_1249F.nuke_core_tug_of_war ]]( var_0 );

    loggrenadedata( var_0, var_1, var_2, var_3, var_5, var_6 );
    var_1 scripts\common\utility::_id_12ED6( level.stadium_three_death_func, scripts\mp\utility\weapon::getweaponrootname( var_6 ), "damage", min( var_2.health, var_3 ), -1, var_6 );
    return var_3;
}
...
playerkilled_fixupattacker( var_0 )
{
    ...
    if ( var_1 )
    {
        ...
        if ( isdefined( var_2 ) )
        {
            ...
            if ( istrue( var_0.victim._id_128BF ) && isdefined( var_0.victim.attackerdata[var_2.guid].inflictor ) )
...
playerkilled_precalc( var_0 )
{
    ...
    var_0.currentpoint = scripts\mp\utility\player::round_spawn_enemies( var_2 );
}
...
playerkilled_sharedlogic_early( var_0 )
{
    ...
    if ( scripts\mp\utility\weapon::router_placed( var_3 ) && isdefined( var_4 ) && var_4.basename == "none" )
    {
    ...
    var_2 scripts\mp\equipment\molotov::tablettype();
    var_2 scripts\mp\equipment\throwing_knife_mp::_id_12C5B();
    ...
    if ( var_1 != var_2 )
    {
        if ( isdefined( var_1.petwatch ) )
        {
            ...
            else if ( var_2 zombieloadout( var_6, var_3 ) )
                var_1 scripts\cp_mp\pet_watch::apcturret();
        }
        else if ( isdefined( var_2.vehomn_fadeoutcontrols ) )
    ...
    if ( scripts\mp\utility\game::isteamreviveenabled() && scripts\mp\flags::gameflag( "prematch_done" ) )
    {
        if ( var_6 == "MOD_FALLING" && var_2 scripts\mp\teamrevive::isvalidrevivetriggerspawnposition() )
            var_2 scripts\mp\teamrevive::_id_12487();
    ...
    if ( scripts\cp_mp\utility\script_utility::issharedfuncdefined( "game", "getGameType" ) )
    {
        ...
        {
            var_2 scripts\cp\helicopter\chopper_boss::_id_12A36( "alive_in_gas" );
            var_2 scripts\cp\helicopter\chopper_boss::_id_12A36( "alive_not_downed" );

            if ( istrue( level.br_prematchstarted ) && !scripts\mp\gametypes\br_public.gsc::isplayeringulag() )
                scripts\mp\gametypes\br_analytics.gsc::close_kioskgate( self, "player_death" );
        }
    }
    ...
    if ( !var_16 )
    {
        if ( !isdefined( level.superslotcleanup ) || [[ level.superslotcleanup ]]( var_2 ) )
    ...
    if ( isdefined( var_0.idflags ) && var_0.idflags & level.idflags_penetration && !( var_0.idflags & level.prepare_mission_failed_text ) )
    {
        if ( isdefined( var_1.bulletkillsinaframecount ) && var_1.bulletkillsinaframecount == 0 )
        {
            ...
            if ( isplayer( var_1 ) )
                var_1 thread scripts\mp\utility\points::patch_far_wait( "penetration_kill" );
        }
    }
    ...
    if ( isdefined( var_0.inflictor ) && ( istrue( var_0.inflictor.screenent_d ) || istrue( var_0.inflictor.screenent_c ) ) )
        var_17 = "MOD_CRUSH";
    ...
    if ( level.gametype == "br" )
    {
        ...
        if ( isdefined( var_2 ) )
        {
            var_19 = scripts\mp\gametypes\br_public.gsc::ondefuse( var_2.team, var_2.squadindex );
        ...
        if ( isdefined( var_1 ) && isplayer( var_1 ) )
        {
            var_20 = scripts\mp\gametypes\br_public.gsc::ondefuse( var_1.team, var_1.squadindex );
        ...
        if ( var_18.size > 0 )
        {
            var_21 = scripts\engine\utility::ter_op( scripts\cp_mp\vehicles\vehicle::_id_1317A( var_0 ), "MOD_EXPLOSIVE", var_17 );

            if ( getdvarint( "scr_br_alt_mode_zxp", 0 ) || getdvarint( "scr_br_alt_mode_gxp", 0 ) )
                var_21 = scripts\mp\gametypes\br_gametype_zxp.gsc::startcheck( var_0, var_21 );
...
playerkilled_logkill( var_0 )
{
    ...
    var_6 = var_0.meansofdeath;
    var_7 = var_0.lifeid;
    var_2 scripts\common\utility::_id_12ED6( level.fireoffsplashforplayer, var_1 );

    if ( !istrue( level.ignorescoring ) )
        var_2 scripts\common\utility::_id_12ED6( level.stack_patch_waittill_stack, var_7, var_1, var_0.damage, var_6, var_5, var_0.primaryweapon, var_0.hitloc, var_4 );
    ...
    if ( isdefined( level.matchrecording_logeventmsg ) && isdefined( var_3 ) && isplayer( var_3 ) && scripts\engine\utility::isbulletdamage( var_6 ) )
    {
        ...
        if ( !isdefined( var_2.is_station_track_available ) )
        {
            var_2.is_station_track_available = 0;
            var_2.is_station_active = 0;
        }

        var_2.is_station_track_available = var_2.is_station_track_available + var_16;
        var_2.is_station_active++;
...
playerkilled_handlecorpse( var_0 )
{
    ...
    if ( !isdefined( self.nocorpse ) && !istrue( var_2._id_125A6 ) )
        var_2.body = var_2 cloneplayer( var_0.deathanimduration, var_1 );

    if ( !isdefined( self.nocorpse ) && !istrue( var_2._id_125A6 ) && isdefined( var_2.body ) )
    {
    ...
    else if ( isdefined( var_2.nocorpse ) && !istrue( var_2._id_125A6 ) )
    {
...
_startragdoll( var_0, var_1, var_2 )
{
    ...
    var_7 = zombieloadout( var_1, var_2 );
    var_8 = undefined;
    ...
    if ( isdefined( var_6 ) )
    {
        ...
        if ( var_11 || var_12 )
        {
            ...
            var_0.signal_nag = scripts\cp_mp\vehicles\vehicle_occupancy::_id_13256( var_6, var_8 );
        }
    ...
    if ( isdefined( var_5 ) )
    {
        ...
        if ( isdefined( var_6 ) )
        {
            var_0 thread ai_dropgren_override_string( var_6, var_8, var_9, var_10, var_5, var_4 );
            var_0 notify( "cancel_delete_corpse" );
            scripts\cp_mp\vehicles\vehicle_occupancy::_id_1323B( var_0, var_6, var_8 );
        }
        else
        {
            if ( !var_0 isragdoll() )
            ...
            var_0 thread activate_track_timer( var_9, var_10, var_5, var_4 );
        }
    }
}

activate_track_timer( var_0, var_1, var_2, var_3 )
{
    var_4 = self;
...
ai_dropgren_override_string( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    var_6 = self;
    ...
    if ( isdefined( var_6 ) )
    {
        if ( isdefined( var_6.signal_nag ) && isdefined( var_0 ) )
        {
            var_11 = ( 0, 0, 0 );
            var_12 = scripts\cp_mp\vehicles\vehicle_occupancy::_id_13256( var_0, var_1 );
            var_11 = var_12.origin - var_6.signal_nag.origin;
        ...
        var_6 thread activate_track_timer( var_2, var_3, var_4, var_5 );
    }
}

agentclasscallback( var_0, var_1, var_2 )
{
    if ( !isdefined( var_0 ) )
        return;
...
playerkilled_deletecorpseoutofvehicle( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    var_1 scripts\engine\utility::_id_13403( var_3, "death" );

    if ( isdefined( var_0 ) )
        var_0 delete();
}

zombieloadout( var_0, var_1 )
{
    if ( var_0 != "MOD_CRUSH" )
        return 0;
...
playerkilled_killcam( var_0 )
{
    ...
    if ( getdvarint( "scr_killcam_on_down", 1 ) && isdefined( var_1.victim.setsoundsubmixfadetoblackamb ) )
    {
        var_1 = var_1.victim.setsoundsubmixfadetoblackamb;
        var_1.victim.setsoundsubmixfadetoblackamb = undefined;
    ...
    if ( !( isdefined( var_3.cancelkillcam ) && var_3.cancelkillcam ) && var_1.dokillcam && level.killcam && game["state"] == "playing" && !var_3 scripts\mp\utility\player::isusingremote() && !level.showingfinalkillcam )
    {
        ...
        if ( isdefined( level.unset_relic_no_ammo_mun ) )
            [[ level.unset_relic_no_ammo_mun ]]();
    }
}

playerkilled_spawn( var_0 )
{
    if ( isdefined( level.supply_station_direction ) && [[ level.supply_station_direction ]]( var_0, 1 ) )
        return;
...
handlefriendlyfiredeath( var_0, var_1 )
{
    ...
    if ( var_3 > 0 )
    {
        ...
        if ( var_0 scripts\mp\utility\player::isusingremote() )
            var_0 thread scripts\mp\utility\damage::_id_12ABD();
        else
            var_0 thread scripts\mp\utility\damage::_id_12ABC();
    }
...
handleinlaststanddeath( var_0 )
{
    ...
    if ( isdefined( self.setquestrewardtierteamomnvar ) )
        self.setquestrewardtierteamomnvar xyvelscale_low();

    if ( isdefined( self.setquestrewardtierteamomnvar ) && var_0.attacker != self.setquestrewardtierteamomnvar )
    {
        thread scripts\cp\helicopter\chopper_boss::onplayerkilled( self.setquestrewardtierteamomnvar, self.setquestrewardtierteamomnvar, 0, var_0.damageflags, self.setteamasextracted, self.setup_bot_hq, var_0.hitloc, self.setreduceregendelayonkill, self.setreduceregendelayonkills );
...
handlenormaldeath( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8 )
{
    ...
    if ( !istrue( level.disablestattracking ) )
    {
        ...
        var_5.setroundwinstreakspecialcamos = undefined;
        ...
        if ( isdefined( var_2 ) && istrue( var_2.isequipment ) )
        {
            switch ( var_2.equipmentref )
            {
                ...
                case "equip_throwing_knife":
                    ...
                    var_1 scripts\cp_mp\pet_watch::apc_rus_monitordriverturretfire();
                    break;
                case "equip_c4":
    ...
    if ( !istrue( var_11.pers["ignoreWeaponMatchBonus"] ) && ( scripts\mp\utility\weapon::iscacprimaryweapon( var_3 ) || scripts\mp\utility\weapon::iscacsecondaryweapon( var_3 ) ) )
    {
        ...
        if ( var_11.pers["weaponMatchBonusKills"] > scripts\mp\weaponrank::next_drone_cd() )
        {
        ...
        else
        {
            ...
            if ( !var_13 )
            {
                ...
                var_15.updateinstantclassswapallowedinternal = var_11.pers["killsPerWeapon"].size;
    ...
    if ( !var_6 && !scripts\mp\utility\weapon::router_placed( var_2 ) || scripts\mp\utility\points::rooftop_crate_usefunc( var_3 ) )
    {
    ...
    if ( istrue( var_1.inlaststand ) )
        var_1 thread scripts\mp\laststand::unset_force_aitype_shotgun( var_7 );
    ...
    if ( isdefined( self.attackers ) )
    {
        foreach ( var_24 in self.attackers )
        {
            ...
            if ( isdefined( self.setquestrewardtierteamomnvar ) && self.setquestrewardtierteamomnvar == var_24 )
                continue;
        ...
        if ( isdefined( var_31 ) && isdefined( level.uavmodels ) && isdefined( level.uavmodels[var_31] ) && level.uavmodels[var_31].size > 0 )
        {
            ...
            foreach ( var_35 in level.uavmodels[var_31] )
            {
                if ( isdefined( var_35 ) && isdefined( var_35.owner ) && var_35.owner != var_1 && !scripts\engine\utility::array_contains( var_33, var_35.uavtype ) )
                {
                    ...
                    if ( !isdefined( var_35.brbonusxpallowed ) )
                        var_35.brbonusxpallowed = 0;

                    var_38 = scripts\engine\utility::ter_op( var_35.uavtype == "directional_uav", 15, 10 );

                    if ( var_35.brbonusxpallowed < var_38 )
                        ...
                        var_35.owner thread scripts\mp\utility\points::patch_far_wait( var_36, var_37 );

                    var_35.brbonusxpallowed++;
            ...
            foreach ( var_41 in level.supportdrones )
            {
                if ( level.teambased && var_41.team == var_1.team && var_41.owner != var_1 )
                {
                    if ( isdefined( var_41.enemiesmarked ) && isdefined( var_41.enemiesmarked[self getentitynumber()] ) )
                    {
                        ...
                        var_1 scripts\cp\helicopter\chopper_boss::onreconmarkedkill();
                    }
                }
                ...
                if ( var_41.helperdronetype == "scrambler_drone_guard" )
                {
                    if ( var_1 != var_41.owner )
                    {
                        ...
                        if ( !isdefined( var_41.brbonusxpallowed ) )
                            var_41.brbonusxpallowed = 0;

                        if ( var_41.brbonusxpallowed < 10 )
                            ...
                            var_41.owner thread scripts\mp\utility\points::patch_far_wait( var_36 );

                        var_41.brbonusxpallowed++;
                    ...
                    {
                        if ( !istrue( var_1._id_11EF8 ) )
                        {
                            var_1 scripts\cp\helicopter\chopper_boss::_id_11EFA( "t9_ch_global_kill_scrambled_enemies_s1", 1 );
                            continue;
                        }

                        var_1._id_11EF8 = undefined;
                    }
                }
            }
        }

        _attachmentblocks::calloutmarkerpingvo_getmaxsoundaliaslength( var_1, self );
        ...
        if ( isdefined( var_1.team ) && isdefined( level.activewpzones ) && level.activewpzones.size > 0 )
        {
            ...
            if ( var_1.team == var_45 && var_1 != var_44 )
            {
                if ( istrue( var_5.wpdisorient ) || istrue( var_5.wpburning ) )
                {
                    ...
                    if ( !isdefined( var_43.brbonusxpallowed ) )
                        var_43.brbonusxpallowed = 0;

                    if ( var_43.brbonusxpallowed < 15 )
                        ...
                        var_44 thread scripts\mp\utility\points::patch_far_wait( var_36 );

                    var_43.brbonusxpallowed++;
...
resetplayervariables()
{
    ...
    self.get_track_setting = 0;
    self.startofxptime = 0;
    self._id_125DE = 0;
    self.playercleanupentondisconnect = 0;
    self.droptogroundmultitrace = undefined;

    if ( !isdefined( self.building_roof_chopper_reenforce_watch ) )
        self.uniform_suicide_truck_speed_manager = 0;

    self.set_carry_item = undefined;
    self.set_chopper_search_speed = undefined;
    self.set_chopper_circle_speed = undefined;
    self.set_chopper_spawn_speed = undefined;
    self.set_car_collision = undefined;
    self._id_11B8F = undefined;
    self._id_11B90 = undefined;
    self._id_135A9 = undefined;
    self.iscrossbowspecial = undefined;
    self.shouldburnfromdamage = undefined;
    self._id_12CDA = undefined;
    self.voting = undefined;
    self.branalytics_dropbagused = undefined;
    self._id_13106 = undefined;
    self.setquestrewardtierteamomnvar = undefined;
    self.setreduceregendelayonkill = undefined;
    self.setreduceregendelayonkills = undefined;
    self.rangedweaponkillsthislife = undefined;
...
addattacker( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9 )
{
    ...
    var_0.attackerdata[var_1.guid].suppressedgunner = var_1 scripts\mp\events::move_platform( 0, var_0, var_3, var_9, var_2 );
    var_0.attackerdata[var_1.guid].suppressedlaser = var_1 scripts\mp\events::move_player_from_under_heli_and_kill( 0, var_0, var_3, var_9, var_2 );
...
addattackerkillstreak( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9, var_10 )
{
    ...
    var_0.attackerdata[var_2.guid].suppressedgunner = 0;
    var_0.attackerdata[var_2.guid].suppressedlaser = 0;
...
finishplayerdamagewrapper( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9, var_10, var_11, var_12 )
{
    ...
    if ( isdefined( var_1 ) && isplayer( var_1 ) )
    {
        if ( _id_1250E( self ) )
        {
            ...
            if ( !isdefined( var_1.getemptyloadout ) )
                var_1.getemptyloadout = var_13;
            else
                var_1.getemptyloadout = var_1.getemptyloadout + var_13;

            if ( scripts\mp\codcasterclientmatchdata::shouldlogcodcasterclientmatchdata() )
            {
                if ( isplayer( var_1 ) )
                {
                    var_1 scripts\common\utility::_id_12ED6( level.stadium_three_death_func, scripts\mp\utility\weapon::getweaponrootname( var_5 ), "damage", min( self.health, var_2 ), -1, var_5 );
            ...
            var_1 [[ level.tv_station_infil_enemy_combat_logic ]]( var_13 );
    }

    if ( scripts\mp\utility\weapon::iskillstreakweapon( var_5 ) )
    {
        if ( scripts\mp\utility\killstreak::getkillstreaknamefromweapon( var_5 ) == "precision_airstrike" && istrue( level.script_deplay_post ) )
            return;
    }
    ...
    else
    {
        ...
        if ( isplayer( self ) )
        {
            ...
            var_16 = !( var_4 != "MOD_TRIGGER_HURT" && isdefined( self.rpg_building_guys ) );
...
callback_playerlaststand( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8 )
{
    ...
    if ( !istrue( var_9 ) )
    {
        ...
        if ( isdefined( level.superfultonbeginuse ) && !self [[ level.superfultonbeginuse ]]( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8 ) )
            return 0;
    }

    if ( isdefined( level.van ) )
        self [[ level.van ]]();
    ...
    if ( isdefined( var_1 ) && var_1.classname != "worldspawn" )
    {
        ...
        self.x1opsinfilsequenceend = 1;

        if ( isplayer( var_1 ) && var_1 != self )
        {
            self.setquestrewardtierteamomnvar = var_1;
            self.setteamasextracted = var_11.meansofdeath;
            self.setup_bot_hq = var_4;
            var_1 thread scripts\mp\events::branalytics_respawn( var_10, self, var_4, var_3, var_0, var_11 );
            self.setreduceregendelayonkill = var_1.modifiers["mask"];
            self.setreduceregendelayonkills = var_1.modifiers["mask2"];
        }
        ...
        if ( getdvarint( "scr_killcam_on_down", 1 ) )
            self.setsoundsubmixfadetoblackamb = var_11;
        ...
        if ( !var_11.isfriendlyfire && var_1 != self )
        {
            if ( scripts\mp\utility\game::getgametype() == "br" )
            {
                if ( isplayer( var_1 ) && var_1 != self )
                {
                    ...
                    if ( var_13 )
                    {
                        if ( isdefined( var_1.binoculars_onstatemarkpendingexit ) )
    ...
    scripts\common\utility::blink_wheelson_chosen_spawn( 0 );

    if ( getdvar( "scr_br_gametype", "" ) == "dmz" || getdvar( "scr_br_gametype", "" ) == "risk" )
        scripts\mp\gametypes\br_plunder.gsc::_id_11A9B( self, 0, 1 );
    ...
    {
        if ( [[ level.end_freight_lift ]]( self ) )
            return 0;
    }
...
damageshellshockandrumble( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    ...
    if ( !isai( self ) && scripts\common\utility::getdamagetype( var_2 ) != "bullet" )
    {
        ...
        if ( isdefined( var_0 ) && istrue( var_0.screenent_d ) )
        {
            self playsound( "train_veh_impact_body" );

            if ( isdefined( level._id_135FD._id_12D76 ) )
            {
                var_6 = level._id_135FD._id_12D76[0].showtutsplash;
...
gamemodemodifyplayerdamage( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9, var_10 )
{
    if ( isdefined( var_2 ) && isplayer( var_2 ) && isalive( var_2 ) )
    {
        ...
        if ( istrue( level.play_hud_reminder_vo ) && scripts\mp\utility\game::getgametype() == "infect" && isdefined( var_5 ) )
        {
...
monitordamageoneshot( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9, var_10, var_11, var_12, var_13, var_14 )
{
    ...
    if ( isdefined( var_1 ) && !scripts\mp\utility\entity::isgameparticipant( var_1 ) && !istrue( var_1.usbs_pulled_out ) )
        return 1;
    ...
    if ( !istrue( self._id_125B0 ) && scripts\mp\utility\damage::non_player_should_ignore_damage( var_1, var_9, var_10, var_4 ) )
        return 1;
...
onkillstreakkilled( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8 )
{
    ...
    thread scripts\mp\events::_id_11BB5( var_10 );
...
setdeathtimerlength( var_0 )
{
    ...
    if ( var_1 scripts\mp\playerlogic::mayspawn() && !level.helis_assault3_check_size )
    {
...
force_interrupt_all_current_combat_actions( var_0 )
{
    if ( !scripts\mp\utility\stats::canrecordcombatrecordstats() )
        return;
...
{
    if ( scripts\mp\weapons::run_elevator_spawners_event( var_3 ) )
        enqueuecorpsetablefunc( "8bitDeath", scripts\mp\weapons::toggleconnectpaths );
    else if ( scripts\mp\weapons::run_func_on_each_player( var_3 ) )
        enqueuecorpsetablefunc( "teslaDeath", scripts\mp\weapons::togglecpplayerbc );
}
...
_id_1250E( var_0 )
{
    if ( istrue( level.heliexplode ) )
        return 0;

    if ( isdefined( level.heliextractarrivewait ) && ![[ level.heliextractarrivewait ]]( var_0 ) )
        return 0;

    return 1;
}

spawn_elevators( var_0, var_1, var_2, var_3 )
{
    if ( !isdefined( var_0 ) || !isdefined( var_1 ) || !isdefined( var_3 ) )
        return;

    var_4 = var_2 >= var_0.health;
    var_1 scripts\common\utility::_id_12ED6( level.stadium_three_death_func, scripts\mp\utility\weapon::getweaponrootname( var_1.currentweapon ), "friendly_fire_damage", int( min( var_2, var_0.health ) ), -1, var_1.currentweapon );
