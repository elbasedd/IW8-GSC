...
{
    if ( istrue( level._id_12593 ) || var_0 == "MOD_EXECUTION" )
        return;
    ...
    else if ( istrue( self.iszombie ) || istrue( level.play_hud_reminder_vo ) && level.gametype == "infect" && self.team == "axis" )
...
ai_dismount_turret()
{
    if ( !isarray( level.adson ) )
        level.adson = [];

    if ( !isent( level.adsoff ) )
        level.adsoff = spawn( "sound_transient_soundbanks", ( 0, 0, 0 ) );
}
...
apc_target_enemies( var_0 )
{
    ...
    if ( isdefined( level.adson[var_0] ) )
        return;

    level.adsoff settransientsoundbank( var_0 + ".all", 1 );
    level.adson[var_0] = level.adson.size;
    return;
}
...
_id_11EEA( var_0 )
{
    ...
    if ( isdefined( level.adson[var_0] ) )
    {
        level.adsoff settransientsoundbank( var_0 + ".all", 0 );
        level.adson[var_0] = undefined;
    }
    else
    {
