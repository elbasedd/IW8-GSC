...
open_close_loop( var_0 )
{
    return "team_two_hundred";
}

script_model_spawn_and_use( var_0 )
{
    if ( var_0 == open_close_loop() && ( isdefined( level._id_13693 ) || clip_ent() ) )
        return 1;
...
clip_ent()
{
    return getdvarint( "scr_br_use_agents", 0 ) > 0 || istrue( level.compareteamscores );
}
...
nextroundisfinalround()
{
    ...
    {
        if ( level script_model_spawn_and_use( var_2 ) )
            continue;
...
waslastround()
{
    ...
    if ( istrue( level.get_players_inside_the_plane ) )
        return 1;
...
isanymlgmatch()
{
    ...
    return getdvarint( "LOMTKQTRTM" ) || istrue( level.dropmenu_tut );
}

ismlgsystemlink()
{
    if ( level.systemlink && ( getdvarint( "LOMTKQTRTM" ) || istrue( level.dropmenu_tut ) ) )
        return 1;
...
ismlgprivatematch()
{
    if ( privatematch() && ( getdvarint( "LOMTKQTRTM" ) || istrue( level.dropmenu_tut ) ) )
        return 1;
...
{
    var_0 = getgametype();
    var_1 = onfieldupgradeendbuffer();
    return var_0 != var_1;
}

scn_infil_hackney_heli_npc2()
{
    if ( onfieldupgradeendbuffer() == "br" )
        return 1;
    else if ( onfieldupgradeendbuffer() == "jugg" )
        return 1;
...
nuke_vault_door_sounds()
{
    level.updatesquadmemberlaststandreviveprogress = 5;
    return level.updatesquadmemberlaststandreviveprogress;
}

nuke_vault_jugg()
{
    if ( !isdefined( level.updateteambettermissionrewardsui ) )
        level.updateteambettermissionrewardsui = getdvarint( "scr_br_payload_retreat_time", 30 );

    return level.updateteambettermissionrewardsui;
}

nuke_vault_escape()
{
    if ( !isdefined( level.updatesuperuiprogress ) )
        level.updatesuperuiprogress = getdvarint( "scr_br_payload_redraw_time", 10 );

    return level.updatesuperuiprogress;
}
...
spawn_chopper_carepackage( var_0, var_1, var_2, var_3, var_4 )
{
...
sandbox_safe_area()
{
    return istrue( level.sandbox_combat_area_bits );
}

rotateviptoplayer()
{
    return istrue( level.rotationids ) || istrue( level.rotationrefsbyseatandweapon ) || istrue( level.rotationentangles );
}
...
display_relics_splash( var_0 )
{
    ...
    if ( ( getgametype() == "arm" || rotateviptoplayer() ) && scout_drone() || getdvarint( "scr_game_forceEnableParachuteCut", 0 ) == 1 )
    {
        if ( var_3 != "mp_aniyah" )
        {
            ...
            if ( istrue( level.validautoassignquests ) && !istrue( var_0.helidrivableenablesiteonflyaway ) )
                var_2 = 1;
        }
    }
...
extractlocale_islocaleavailable()
{
    if ( ring() )
    {
    ...
    else if ( isdefined( level.pitcher_lights ) && level.pitcher_lights > 0 )
        return 1;
...
safesetalpha()
{
    switch ( getgametype() )
    {
        case "brtdm":
            return 0;
        case "br":
            switch ( onfieldupgradeendbuffer() )
            {
                case "x2":
                case "payload":
                case "kingslayer":
