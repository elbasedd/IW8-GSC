...
init()
{
    ...
    scripts\mp\utility\spawn_event_aggregator::registeronplayerspawncallback( ::rundomplateskybeam );
}
...
getspawnpoint()
{
    ...
    if ( isdefined( self.selectedspawnarea ) )
    {
        if ( issubstr( self.selectedspawnarea, "squad" ) )
        {
            ...
            if ( isdefined( var_3.vehicle ) )
            {
                ...
                self._id_132CD = var_3.vehicle.vehiclename;
            }
    ...
    if ( istrue( level.usesquadspawn ) && istrue( self.squadspawnconfirmed ) )
    {
        ...
        if ( isdefined( var_14 ) && isdefined( self.squadindex ) && self.team == var_14.team && self.squadindex == var_14.squadindex )
        {
            ...
            var_14 thread scripts\mp\utility\points::patch_far_wait( "squad_spawn" );
            thread scripts\mp\utility\points::patch_far_wait( "squad_spawn_self" );
            self._id_13483 = 1;
        }
    }
    ...
    for (;;)
    {
        ...
        foreach ( var_3 in level.availablespawnlocations[self.team] )
        {
            ...
            if ( isdefined( var_6.objectivekey ) )
                var_8 = istrue( var_6.projectileunlink );
            else if ( issubstr( var_3, "vehicle" ) )
            {
                ...
                {
                    var_8 = istrue( var_11.projectileunlink );
                    var_9 = player_near_obit( var_11, self );
...
{
    return level.objectives[var_0].ownerteam != var_1 || level.objectives[var_0].stalemate || level.objectives[var_0].captureblocked || istrue( level.objectives[var_0]._id_12806 ) || level.objectives[var_0].claimteam != "none" && level.objectives[var_0].claimteam != var_1;
}
...
{
    var_0 endon( "death" );
    var_0.projectileunlink = 0;
    var_1 = getdvarfloat( "scr_gw_apc_ignore_damage_health_pct", 0.15 );

    for (;;)
    {
        var_2 = gettime();
        var_3 = istrue( var_0.delayedeventtypes );
        var_4 = scripts\cp_mp\utility\weapon_utility::islockedonto( var_0 );
        var_5 = scripts\cp_mp\vehicles\vehicle_occupancy::_id_13236( var_0 );
        ...
        var_14 = var_0.projectileunlink;

        if ( var_6 || var_8 || var_9 || var_3 || var_4 || !var_5 )
            var_0.projectileunlink = 1;
        else
            var_0.projectileunlink = 0;
...
spawnselectionthink( var_0 )
{
    ...
    if ( istrue( level._id_1255B ) )
        thread scripts\mp\gametypes\arm.gsc::_id_1327D();

    if ( istrue( level._id_1255A ) )
        thread scripts\mp\gametypes\arm.gsc::_id_1327E();
...
watchforafk()
{
    ...
    if ( isdefined( level._id_1281C ) )
        wait( level._id_1281C );
    else
        wait 60.0;
    ...
    for (;;)
    {
        ...
        if ( var_0 == "tac_ops_spawn_area_selected" )
        {
            ...
            foreach ( var_4 in level.spawnselectionlocations )
            {
                ...
                {
                    _id_130B8( var_4[self.team].typeref, self, self.team );

                    if ( isdefined( var_4[self.team].objectivekey ) && isobjectiveindanger( var_4[self.team].objectivekey, self.team ) )
                        break;

                    if ( isdefined( var_4[self.team].typeref ) && issubstr( var_4[self.team].typeref, "vehicle" ) && istrue( var_4[self.team].dynamicent.projectileunlink ) )
                        break;
            ...
            if ( !isdefined( var_2 ) )
            {
                var_6 = ondroprelics( self.team, self.squadindex );
                ...
                foreach ( var_4 in var_6 )
                {
                    if ( isdefined( var_4.dynamicent ) && var_4.dynamicent getentitynumber() == var_1 )
                    {
                        ...
                        var_9 = spawnstruct();
                        var_9 = scripts\mp\spawnscoring::do_wave_spawning_while_players_return_to_safehouse( var_9, var_4.dynamicent );

                        if ( !isdefined( var_9._id_1280A ) )
                        {
                            var_4.dynamicent.transitionplayerstoac130cinematic = gettime();
                            break;
                        }
...
_id_130A8( var_0 )
{
    if ( !isdefined( self.platform_array ) )
        self.platform_array = [];
    ...
    var_3 = 262144;
    var_4 = 524288;
    self.platform_array["rightmouseup"] = istrue( self.platform_array["rightmouseup"] ) || ( var_0 & var_1 ) > 0;
    self.platform_array["leftmouseup"] = istrue( self.platform_array["leftmouseup"] ) || ( var_0 & var_2 ) > 0;
    self.platform_array["activate"] = istrue( self.platform_array["activate"] ) || ( var_0 & var_3 ) > 0;
    self.platform_array["gostand"] = istrue( self.platform_array["gostand"] ) || ( var_0 & var_4 ) > 0;
...
watchendconditions()
{
    ...
    var_0 = scripts\engine\utility::_id_13402( "tac_ops_map_selection_valid", "tac_ops_map_changed", "tac_ops_map_cleared", "spawned_player", "tac_ops_map_game_ended", "tac_ops_map_nuke" );
...
endcondition_focuschanged( var_0 )
{
    ...
    foreach ( var_3 in level.spawnselectionlocations )
    {
        ...
        {
            thread select_stadium_one_spawners( var_3[self.team].anchorentity.origin );
    ...
    if ( !isdefined( var_1 ) )
    {
        var_5 = ondroprelics( self.team, self.squadindex );

        foreach ( var_3 in var_5 )
        {
            if ( isdefined( var_3.dynamicent ) && var_3.dynamicent getentitynumber() == var_0 )
            {
                thread select_stadium_one_spawners( var_3.dynamicent.origin );
                var_1 = var_3.typeref;
                break;
            }
        }
...
select_stadium_one_spawners( var_0 )
{
    self endon( "disconnect" );
    self endon( "tac_ops_map_selection_valid" );
    ...
    if ( !isdefined( level._id_135EE ) )
        level._id_135EE = max( getdvarint( "NOLPSOTORP", 20000 ) * 0.5 / 1000, 1.0 );

    for (;;)
    {
        self predictstreampos( var_0, 1 );
        wait( level._id_135EE );
    }
}
...
evaluatesquadspawn()
{
    ...
    for (;;)
    {
        ...
        var_11 = ondroprelics( self.team, self.squadindex );
        ...
        for ( var_4 = 0; var_4 < 4; var_4++ )
        {
            ...
            {
                var_15 = istrue( var_9.projectileunlink );
                var_16 = var_1[var_4];
                ...
                if ( !isdefined( var_3[var_4].prespawnspawn ) || var_3[var_4].prespawnspawn != var_16 )
                {
                    var_3[var_4].prespawnspawn = var_16;
            ...
            if ( isdefined( var_3[var_4].prespawnspawn ) && var_3[var_4].prespawnspawn != -1 )
            {
                var_3[var_4].prespawnspawn = -1;
...
handlesquadspawncycle()
{
    ...
    for (;;)
    {
        var_0 = scripts\engine\utility::_id_133F7( "cyclePos", "cycleNeg" );
...
rundomplateskybeam()
{
    thread remove_closest_chopper_boss_vandalize_node_down();

    if ( getdvarint( "scr_squad_spawn_debug", 0 ) == 1 )
        thread scripts\mp\spawnscoring::_id_128BD();
}

remove_closest_chopper_boss_vandalize_node_down()
{
    ...
    for (;;)
    {
        if ( issquadspawnable( self ) )
            self.projectileunlink = issquadmateindanger( self );
...
issquadmateindanger( var_0 )
{
    ...
    if ( isdefined( var_0.setup_jugg_maze_kill_trigger ) && var_0.setup_jugg_maze_kill_trigger + 3000 > var_1 )
        return 1;
    ...
    {
        if ( istrue( var_0.vehicle.delayedeventtypes ) )
            return 1;
    ...
    if ( isdefined( var_0 ) && isdefined( var_0.transitionplayerstoac130cinematic ) && var_0.transitionplayerstoac130cinematic + 1000 > var_1 )
        return 1;
...
timeuntilspawnmessaging( var_0, var_1 )
{
    ...
    if ( var_0 > 0 )
    {
        ...
        scripts\engine\utility::_id_13409( var_0, "force_spawn" );
    }
}
...
getclosestavailablespawnlocation()
{
    ...
    else
    {
        ...
        {
            var_7 = ondroprelics( self.team, self.squadindex );
        ...
        foreach ( var_1 in level.teamnamelist )
        {
            foreach ( var_3 in level.availablespawnlocations[var_1] )
            {
                ...
                if ( isdefined( var_4.objectivekey ) )
                    var_4.projectileunlink = isobjectiveindanger( var_4.objectivekey, var_1 );
            }
        }
        ...
        foreach ( var_1 in level.teamnamelist )
        {
            ...
            foreach ( var_4 in level.availablespawnlocations[var_1] )
            {
                if ( !issubstr( var_4, "HQ" ) && issubstr( var_4, "fob" ) )
                {
                    ...
                    level.objectives[var_17]._id_12806 = var_14;
                    level.objectives[var_17]._id_12807 = gettime();
                }
            }
        }
...
_id_130B8( var_0, var_1, var_2 )
{
    var_3 = level.spawnselectionlocations[var_0][var_2].objectivekey;

    if ( !isdefined( var_3 ) || !isdefined( level.objectives[var_3] ) || !isdefined( level.objectives[var_3]._id_12807 ) || level.objectives[var_3]._id_12807 == gettime() )
        return;
    ...
    level.objectives[var_3]._id_12806 = var_13;
    level.objectives[var_3]._id_12807 = gettime();
}
