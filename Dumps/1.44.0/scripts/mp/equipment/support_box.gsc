...
supportbox_init()
{
    ...
    scripts\common\interactive::interactive_addusedcallback( ::br_circlecenters, "equip_armorBox" );
    scripts\mp\utility\join_team_aggregator::registeronplayerjointeamcallback( ::_id_12AEE );
...
_id_12AEE( var_0 )
{
    foreach ( var_2 in level.bomb_carrier )
    {
        if ( isdefined( var_2 ) )
            var_2 _id_12AEC( var_0 );
    }
}
...
chopperexfil_skip_ascend2( var_0, var_1, var_2, var_3 )
{
    ...
    var_0 thread _id_12AEF( self );

    if ( !scripts\mp\flags::gameflag( "prematch_fade_done" ) )
        var_0 thread _id_12AF0();
    ...
    level.bomb_carrier[var_0 getentitynumber()] = var_0;
...
_id_12AEF( var_0 )
{
    ...
    var_1 = scripts\engine\utility::_id_13403( 2, "touching_platform" );
...
{
    var_0 endon( "death" );
    var_0._id_12C8F = "vfx/iw8_mp/equipment/vfx_offhand_wm_supportbox_timeout.vfx";
    chopperexfil_skip_ascend2( var_0, "equip_supportBox", "super_ammo_drop", "hud_icon_equipment_support_box" );
    var_0 thread supportbox_makeusable( "equip_supportBox", &"EQUIPMENT_HINTS/SUPPORT_BOX_USE" );
}

br_circle_init_func( var_0 )
{
    var_0 endon( "death" );
    var_0._id_12C8F = "vfx/iw8_mp/equipment/vfx_offhand_wm_armorbox_timeout.vfx";
    chopperexfil_skip_ascend2( var_0, "equip_armorBox", "super_armor_drop", "ui_mp_br_loot_icon_health_armor_box" );
...
supportbox_destroy( var_0 )
{
    ...
    if ( isdefined( self._id_12C8F ) )
        var_1 = self._id_12C8F;
...
supportbox_delete( var_0, var_1, var_2 )
{
    ...
    if ( isdefined( self.owner ) )
    {
        if ( isdefined( self.equipmentref ) && self.equipmentref == "equip_armorBox" )
            self.owner scripts\cp\helicopter\chopper_boss::unlink_on_ai_death( "super_armor_drop", self.usedcount, var_0, var_2 );
        else
            self.owner scripts\cp\helicopter\chopper_boss::unlink_on_ai_death( "super_ammo_drop", self.usedcount, var_0, var_2 );
    }
...
supportbox_playercanuse( var_0 )
{
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" && scripts\mp\utility\game::onfieldupgradeendbuffer() != "x2" )
    {
        if ( isdefined( self.owner ) && !scripts\engine\utility::array_contains( scripts\mp\gametypes\br_public.gsc::ondefuse( self.owner.team, self.owner.squadindex ), var_0 ) )
            return 0;
...
{
    self.player_vo_confirm_pickup = scripts\cp_mp\entityheadicons::setheadicon_singleimage( [], var_0, 20, 1, 1000, 100, undefined, 1 );
...
supportbox_givexpforuse( var_0 )
{
    if ( isdefined( self.owner ) && !scripts\cp_mp\utility\player_utility::playersareenemies( self.owner, var_0 ) )
    {
        if ( self.owner != var_0 )
        {
            ...
            self.owner scripts\cp\helicopter\chopper_boss::unuseweapon( self );
        }

        self.owner scripts\mp\utility\stats::incpersstat( "munitionsBoxUsed", 1 );
        self.owner scripts\mp\supers::force_kill_off_other_ai( "super_ammo_drop" );
        self.usedcount++;
    }
}
...
supportbox_handlemovingplatforms( var_0 )
{
    ...
    if ( isdefined( var_0 ) && scripts\mp\equipment\claymore::respawn_after_death( var_0 ) )
    {
        var_1.wait_to_reset_wave_loadout = 1;
        self _meth_87B9( 1 );
    }
...
_id_12AEC( var_0 )
{
    ...
    var_1 = self.player_vo_confirm_pickup;

    if ( !isdefined( var_1 ) )
        return;

    var_2 = self.player_waitforlanded;

    if ( !isdefined( var_2 ) )
        return;

    if ( scripts\mp\utility\game::getgametype() == "br" && scripts\mp\menus::_id_128AB() && isdefined( self.owner ) )
        var_3 = scripts\engine\utility::array_contains( scripts\mp\gametypes\br_public.gsc::ondefuse( self.owner.team, self.owner.squadindex ), var_0 );
    else
    {
        if ( isdefined( self.owner ) )
    ...
    if ( var_3 && !var_5 )
    {
        scripts\cp_mp\entityheadicons::_id_12384( var_1, var_0 );
        scripts\cp_mp\entityheadicons::_id_12385( var_2, var_0 );
    }
    else if ( var_3 )
    {
        scripts\cp_mp\entityheadicons::_id_12385( var_1, var_0 );
        scripts\cp_mp\entityheadicons::_id_12384( var_2, var_0 );
    }
    else
    {
        scripts\cp_mp\entityheadicons::_id_12385( var_1, var_0 );
...
supportbox_usedcallback( var_0, var_1 )
{
    ...
    {
        if ( isdefined( level.update2v2progress ) )
            var_2 = var_0 [[ level.update2v2progress ]]( var_1 );
        else
        ...
        if ( var_2 )
        {
            ...
            var_0 _id_12AEC( var_1 );
            var_0 supportbox_givexpforuse( var_1 );
        }
    }
}

br_circlecenters( var_0, var_1 )
{
    ...
    if ( var_0 supportbox_playercanuse( var_1 ) )
    {
        if ( isdefined( level.tv_station_get_bomb_icon_on_cell_phone ) )
            var_2 = var_0 [[ level.tv_station_get_bomb_icon_on_cell_phone ]]( var_1 );
        else
            var_2 = 0;

        if ( var_2 && !istrue( var_0._id_12FC6 ) )
        {
            var_0.playersused[var_1 getentitynumber()] = var_1;
            var_0 _id_12AEC( var_1 );
            var_0 supportbox_givexpforuse( var_1 );
