...
{
    level._id_12B49 = [];
    scripts\mp\utility\join_team_aggregator::registeronplayerjointeamcallback( ::_id_12B43 );

    if ( scripts\mp\utility\game::getgametype() == "br" )
        level thread _id_12B3B();
}

_id_12B43( var_0 )
{
    foreach ( var_2 in level._id_12B49 )
    {
        var_2 _id_12B48( var_0 );
        var_2 _id_12B47( var_0 );
    }
}
...
_id_12B42()
{
    ...
    for (;;)
    {
        ...
        scripts\engine\utility::_id_13403( var_2, "offhand_pullback" );
    }
}
...
tacinsert_used( var_0 )
{
    ...
    if ( !_id_12B3C( var_0, var_3, var_4 ) )
    {
        scripts\mp\hud_message::showerrormessage( "MP/TAC_INSERT_CANNOT_PLACE" );
        thread _id_12B45();
        return 0;
    ...
    var_17 thread _id_12B3D();
    var_17 setotherent( self );
    var_17.weapon_name = "tac_insert_trigger";
    var_18 = var_7["entity"];

    if ( isdefined( var_18 ) && ( scripts\mp\vehicles\vehicle_compass_mp::respawn_after_death( var_18 ) || scripts\mp\gametypes\br_gondola.gsc::repair_grill_fixing_short_sfx( var_18 ) ) )
    {
        var_17 linkto( var_18 );
        var_19 = getdvarint( "scr_br_tac_insert_moving_platform_valid", 1 );

        if ( var_19 )
            var_17 thread _id_12B41( self );
    }
    ...
    _id_12804( var_1, var_16, var_17, 1 );
    var_21 = var_17 getentitynumber();
    level._id_12B49[var_21] = var_17;

    if ( var_17.issuper && isdefined( level._id_12B3F ) )
        self [[ level._id_12B3F ]]();
...
_id_12B44( var_0 )
{
    ...
    level endon( "game_ended" );
    var_0 endon( "disconnect" );
    self.vo_enemy_heli = spawn( "script_origin", self.origin );
    self.vo_enemy_heli makeusable();
    self.vo_enemy_heli setcursorhint( "HINT_NOICON" );
    self.vo_enemy_heli sethintstring( &"MP_PATCH/PICKUP_TI" );
    self.vo_enemy_heli linkto( self );
    ...
    for (;;)
    {
        self.vo_enemy_heli waittill( "trigger", var_2 );
...
_id_12B48( var_0 )
{
    ...
    if ( !isdefined( self ) || !isdefined( self.vo_enemy_heli ) )
        return;
    ...
    if ( var_1 )
        self.vo_enemy_heli enableplayeruse( var_0 );
    else
        self.vo_enemy_heli disableplayeruse( var_0 );
}
...
tacinsert_destroy( var_0, var_1, var_2, var_3 )
{
    ...
    if ( isdefined( self.owner ) )
    {
        if ( istrue( var_2 ) )
        {
            ...
            self.owner scripts\mp\supers::force_kill_off_other_ai( "super_tac_insert" );
            self.owner scripts\cp\helicopter\chopper_boss::unlink_on_ai_death( "super_tac_insert", 1, var_0, var_1 );
            scripts\mp\analyticslog::logevent_fieldupgradeexpired( self.owner, self.superid, 1, istrue( var_1 ) );
        }
        else
        {
            self.owner scripts\cp\helicopter\chopper_boss::unlink_on_ai_death( "super_tac_insert", 0, var_0, var_1 );
            scripts\mp\analyticslog::logevent_fieldupgradeexpired( self.owner, self.superid, 0, istrue( var_1 ) );
        }

        if ( !istrue( var_2 ) && !istrue( var_3 ) )
            self.owner thread _id_12B46();
...
_id_12B3C( var_0, var_1, var_2 )
{
    ...
    if ( isdefined( var_6 ) )
    {
        if ( isdefined( var_6["hittype"] ) && var_6["hittype"] != "hittype_none" )
        {
            ...
            if ( isdefined( var_8 ) )
            {
                if ( var_8 scripts\cp_mp\vehicles\vehicle::isvehicle() && !scripts\mp\vehicles\vehicle_compass_mp::respawn_after_death( var_8 ) )
                    var_3 = 0;

                var_9 = getdvarint( "scr_br_tac_insert_moving_platform_valid", 1 );

                if ( !var_9 && ( scripts\mp\vehicles\vehicle_compass_mp::respawn_after_death( var_8 ) || scripts\mp\gametypes\br_gondola.gsc::repair_grill_fixing_short_sfx( var_8 ) ) )
                    var_3 = 0;
...
_id_12B45()
{
    ...
    if ( istrue( level.rotationids ) )
    {
        ...
        var_1 = scripts\engine\utility::_id_13403( 5, "super_use_finished_lb" );
...
_id_12804( var_0, var_1, var_2, var_3 )
{
    _id_12801();
    var_4 = undefined;
