...
init()
{
    ...
    level.nuke_expl_struct = scripts\cp_mp\utility\game_utility::nuke_hostmigration_waittillhostmigrationdone( "nuke_expl_pos" );
}
...
tryusenukefromstruct( var_0 )
{
    ...
    if ( !isdefined( level.nukeincoming ) )
    {
        ...
        level.trial_spawn_vehicle = self;
    }
    else
    {
    ...
    else if ( !istrue( level.staticcircle ) )
    ...
    if ( !istrue( var_4 ) || level.gameended )
    {
        ...
        level.trial_spawn_vehicle = undefined;
        var_0 notify( "killstreak_finished_with_deploy_weapon" );
        return 0;
    }
    ...
    if ( var_1 != 1 && !istrue( level.staticcircle ) )
        scripts\common\utility::_id_12ED6( level.stack_patch_waittill_leaf, "nuke", self.origin );
...
{
    if ( !isdefined( level.trial_hitmarker ) )
    {
        createheadiconatorigin( "nuke" );
        level.trial_hitmarker = 1;
    }
...
nuke_starttimer( var_0 )
{
    ...
    if ( istrue( level.staticcircle ) )
        scripts\mp\trials\trial_utility::_id_123B0( "ui_nuke_data", 9, 2, 2 );
    else
    ...
    if ( isdefined( level.nukeinfo.player ) )
    {
        ...
        if ( !istrue( level.staticcircle ) )
        {
            if ( isdefined( level.nukeinfo.player ) && isdefined( var_1 ) )
                level.nukeinfo.player scripts\cp_mp\utility\killstreak_utility::_id_11D86( var_1 );
        }
    }
...
{
    level endon( "game_ended" );
    level thread trial_flare_destruct_missile( 1 );
...
trial_fobs_cleared()
{
    ...
    level thread trial_flare_watcher();
}
