...
checkpoint_name( var_0, var_1, var_2 )
{
    var_3 = checkpoint_next_fx_set( "attacker", var_0 );

    foreach ( var_5 in level.players )
    {
        if ( !isai( var_5 ) && isdefined( var_5.team ) && var_5.team == var_0 )
        {
            if ( var_5 checkpoint_objective_id() || distancesquared( var_1, var_5.origin ) > squared( var_2 ) )
...
checkpoint_next_fx( var_0, var_1, var_2 )
{
    var_3 = checkpoint_next_fx_set( "defender", var_0 );

    foreach ( var_5 in level.players )
    {
        if ( !isai( var_5 ) && isdefined( var_5.team ) && var_5.team == var_0 )
        {
            if ( var_5 checkpoint_overwatch() || distancesquared( var_1, var_5.origin ) <= squared( var_2 ) )
                var_3 = scripts\engine\utility::array_add( var_3, var_5 );
        }
    }
...
{
    if ( isdefined( level.checkpoint_objective ) )
        return self [[ level.checkpoint_objective ]]();
...
checkpoint_overwatch()
{
    if ( isdefined( level.checkpoint_player_spawns ) )
        return self [[ level.checkpoint_player_spawns ]]();
...
{
    var_0 = [[ level.checkpoint_edit_munitions_limiter ]]( self.team );
    var_1 = [[ level.checkpoint_edit_role ]]( self.team );
    var_2 = [[ level.checkpoint_fx_set ]]( self.team );
    var_3 = [[ level.checkpoint_get_item ]]( self.team );
    ...
    if ( var_4 == "active" )
    {
        if ( var_0.size >= var_2 )
        {
            ...
            {
                if ( isai( var_7 ) && level.bot_personality_type[var_7.personality] == "stationary" && var_7 checkforcorrectinstance() )
                {
            ...
            if ( var_5 )
            {
                checkpoint_release_spawnpoint( "attacker" );
                return;
            }

            checkpoint_release_spawnpoint( "defender" );
            return;
        }
        else
            checkpoint_release_spawnpoint( "attacker" );
    ...
    else if ( var_4 == "stationary" )
    {
        if ( var_1.size >= var_3 )
        {
            ...
            {
                if ( isai( var_10 ) && level.bot_personality_type[var_10.personality] == "active" && var_10 checkforarenaloadoutoverride() )
                {
...
checkpoint_fx_play()
{
    ...
    for (;;)
    {
        if ( gettime() > var_1 )
        {
            ...
            {
                var_4 = [[ level.checkpoint_edit_munitions_limiter ]]( var_3 );
                var_5 = [[ level.checkpoint_edit_role ]]( var_3 );
                var_6 = [[ level.checkpoint_fx_set ]]( var_3 );
                var_7 = [[ level.checkpoint_get_item ]]( var_3 );
                ...
                if ( var_4.size > var_6 )
                {
                    ...
                    {
                        if ( isai( var_11 ) && var_11 checkforcorrectinstance() )
                        {
                            if ( level.bot_personality_type[var_11.personality] == "stationary" )
                            {
                                var_11 checkpoint_release_spawnpoint( "defender" );
                                var_9 = 1;
                                break;
                            }
                            else
                    ...
                    if ( !var_9 && var_8.size > 0 )
                        scripts\engine\utility::random( var_8 ) checkpoint_release_spawnpoint( "defender" );
                }
                ...
                if ( var_5.size > var_7 )
                {
                    ...
                    {
                        if ( isai( var_16 ) && var_16 checkforarenaloadoutoverride() )
                        {
                            if ( level.bot_personality_type[var_16.personality] == "active" )
                            {
                                var_16 checkpoint_release_spawnpoint( "attacker" );
                                var_14 = 1;
                                break;
                            }
                            else
                    ...
                    if ( !var_14 && var_13.size > 0 )
                        scripts\engine\utility::random( var_13 ) checkpoint_release_spawnpoint( "attacker" );
                }
            }
        }
...
checkforarenaloadoutoverride()
{
    if ( isdefined( level.checkpoint_edit_role_limiter ) )
        return self [[ level.checkpoint_edit_role_limiter ]]();

    return 1;
}

checkforcorrectinstance()
{
    if ( isdefined( level.checkpoint_fx ) )
        return self [[ level.checkpoint_fx ]]();

    return 1;
}

chopper_sound_fade_and_delete( var_0 )
{
    var_1 = 0;
...
checkforammoquickdrop( var_0 )
{
    ...
    foreach ( var_5 in var_0 )
    {
        ...
        var_5._id_1371D = deploy_suicide_truck_in_lumber_yard( var_5 );
        var_5.dropoff_point_spawner = _id_13721( var_5, 0, 0 );
        var_7 = [ ( 0, 0, 0 ), ( 1, 1, 0 ), ( 1, -1, 0 ), ( -1, 1, 0 ), ( -1, -1, 0 ) ];

        foreach ( var_9 in var_7 )
        {
            var_10 = _id_13721( var_5, var_9[0], var_9[1] );
...
deploy_suicide_truck_in_lumber_yard( var_0 )
{
    var_1 = spawnstruct();
    var_1.steam_valve_think = ( 999999, 999999, 999999 );
    var_1.start_conceal_add = ( -999999, -999999, -999999 );

    foreach ( var_3 in var_0.nodes )
    {
        var_1.steam_valve_think = ( min( var_3.origin[0], var_1.steam_valve_think[0] ), min( var_3.origin[1], var_1.steam_valve_think[1] ), min( var_3.origin[2], var_1.steam_valve_think[2] ) );
        var_1.start_conceal_add = ( max( var_3.origin[0], var_1.start_conceal_add[0] ), max( var_3.origin[1], var_1.start_conceal_add[1] ), max( var_3.origin[2], var_1.start_conceal_add[2] ) );
    }

    var_1.center = ( ( var_1.steam_valve_think[0] + var_1.start_conceal_add[0] ) / 2, ( var_1.steam_valve_think[1] + var_1.start_conceal_add[1] ) / 2, ( var_1.steam_valve_think[2] + var_1.start_conceal_add[2] ) / 2 );
    var_1.play_get_in_sequence = ( var_1.start_conceal_add[0] - var_1.center[0], var_1.start_conceal_add[1] - var_1.center[1], var_1.start_conceal_add[2] - var_1.center[2] );
    var_1.radius = max( var_1.play_get_in_sequence[0], var_1.play_get_in_sequence[1] );
    return var_1;
}

_id_13721( var_0, var_1, var_2 )
{
    var_3 = ( var_0._id_1371D.center[0] + var_1 * var_0._id_1371D.play_get_in_sequence[0], var_0._id_1371D.center[1] + var_2 * var_0._id_1371D.play_get_in_sequence[1], 0 );
    var_4 = undefined;
    var_5 = 9999999;
...
markdistanceoverride( var_0, var_1 )
{
...
chase( var_0, var_1, var_2, var_3 )
{
    ...
    {
        childthread triage_door_clip();
        childthread triage_ai_campers();
    }
