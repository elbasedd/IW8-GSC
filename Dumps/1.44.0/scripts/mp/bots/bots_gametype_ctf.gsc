...
setup_callbacks()
{
    ...
    level.bot_funcs["get_watch_node_chance"] = ::checklinktotraininternal;
}

setup_bot_ctf()
{
    level.checkpoint_fx_set = ::get_players_at_zone;
    level.checkpoint_get_item = ::get_players_in_mortar_range;
    level.checkpoint_edit_munitions_limiter = ::get_allied_attackers_for_team;
    level.checkpoint_edit_role = ::get_allied_defenders_for_team;
    level.checkpoint_fx = ::checkifvalidpropspectate;
    level.checkpoint_objective = ::checkpairs;
    ...
    thread checkifteamrewardsareenabled();
...
crate_can_use( var_0 )
{
    ...
    if ( checkpoint_add_carepackage() )
        return 0;

    return checklethalequipmentachievement( self.team );
}
...
bot_ctf_think()
{
    ...
    for (;;)
    {
        ...
        if ( !isdefined( self.role ) )
            scripts\mp\bots\bots_gametype_common.gsc::checkpoint_player_spawns_func();

        var_2 = scripts\mp\utility\game::getotherteam( self.team )[0];

        if ( checklethalequipmentachievement( self.team ) )
        {
            if ( isdefined( level.choosecrouchorstandtac ) && isdefined( level.choosecrouchorstandtac[var_2] ) )
                level.choosecrouchorstandtac[var_2] = undefined;
        }
        ...
        if ( self.role == "attacker" )
        {
            if ( checkpoint_add_carepackage() )
                var_3 = 1;
            else if ( !checkkillrespawn() )
                var_3 = distancesquared( self.origin, level.teamflags[var_2].curorigin ) < squared( get_flag_protect_radius() );
        }
        else if ( !checklethalequipmentachievement( self.team ) )
            var_3 = !checkmapflagangles();
        ...
        if ( checkpoint_add_carepackage() )
        {
            ...
            else if ( scripts\mp\bots\bots_gametype_common.gsc::checkpoint_next_fx_play( self.team ) == 1 )
...
checkpoint_add_carepackage_munitions()
{
    ...
    else if ( self botcanseeentity( var_2 ) )
    {
        ...
        if ( self botgetdifficultysetting( "strategyLevel" ) > 0 && !checklethalequipmentachievement( self.team ) )
        {
            if ( !isdefined( level.choosecrouchorstandtac ) )
                level.choosecrouchorstandtac = [];

            if ( !isdefined( level.choosecrouchorstandtac[var_2.team] ) )
                level.choosecrouchorstandtac[var_2.team] = [];

            level.choosecrouchorstandtac[var_2.team]["origin"] = var_0;
            level.choosecrouchorstandtac[var_2.team]["time"] = gettime();
        }
    }
    ...
    else
    {
        ...
        {
            if ( isdefined( level.choosecrouchorstandtac ) && isdefined( level.choosecrouchorstandtac[var_2.team] ) )
            {
                var_4 = gettime() - level.choosecrouchorstandtac[var_2.team]["time"];

                if ( var_4 < 10000 )
                    var_3 = level.choosecrouchorstandtac[var_2.team]["origin"];
            }
        }
...
{
    var_1 = scripts\mp\bots\bots_gametype_common.gsc::checkpoint_next_fx_play( var_0 );

    if ( var_1 == 1 )
        return 1;

    if ( checklethalequipmentachievement( var_0 ) )
        var_2 = var_1 * 0.67;
    else
    {
        var_3 = scripts\mp\utility\game::getotherteam( var_0 )[0];

        if ( checklethalequipmentachievement( var_3 ) )
            var_2 = var_1 * 0.5;
        else
            var_2 = var_1 * 0.4;
    }
...
{
    var_1 = scripts\mp\bots\bots_gametype_common.gsc::checkpoint_next_fx_play( var_0 );
    return var_1 - get_players_at_zone( var_0 );
}

get_allied_attackers_for_team( var_0 )
{
    return scripts\mp\bots\bots_gametype_common.gsc::checkpoint_name( var_0, level.capzones[var_0].curorigin, get_flag_protect_radius() );
}

get_allied_defenders_for_team( var_0 )
{
    return scripts\mp\bots\bots_gametype_common.gsc::checkpoint_next_fx( var_0, level.capzones[var_0].curorigin, get_flag_protect_radius() );
}
...
checkifteamrewardsareenabled()
{
    ...
    level endon( "game_ended" );
    thread scripts\mp\bots\bots_gametype_common.gsc::checkpoint_fx_play();
}
...
checkmapflagangles()
{
    return checklinktotrain( self.team );
}

checkkillrespawn()
{
    var_0 = scripts\mp\utility\game::getotherteam( self.team )[0];
    return checklinktotrain( var_0 );
}

checkpoint_add_carepackage()
{
