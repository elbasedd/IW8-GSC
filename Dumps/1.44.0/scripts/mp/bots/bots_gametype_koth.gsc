...
setup_callbacks()
{
    level.bot_funcs["gametype_think"] = ::choosedropbagmodel;
    level.bot_funcs["should_start_cautious_approach"] = ::_id_124D5;
}
...
setup_bot_koth()
{
    ...
    if ( !var_0 )
    {
        ...
        level.checkpoint_set[var_5.trigger getentitynumber()] = 1;
        level.bot_gametype_precaching_done = 1;
        thread checkforactiveobjicon( var_5 );
    }
}
...
checkforactiveobjicon( var_0 )
{
    for ( var_1 = scripts\engine\utility::array_remove( level.objectives, var_0 ); var_1.size > 0; var_1 = scripts\engine\utility::array_remove( var_1, var_2 ) )
    {
        ...
        level.checkpoint_set[var_2.trigger getentitynumber()] = 1;
    }
}
...
choosedropbagmodel()
{
    ...
    for (;;)
    {
        ...
        if ( !isdefined( level.zone ) || !isdefined( level.checkpoint_set[level.zone.trigger getentitynumber()] ) )
        {
        ...
        if ( !istrue( level.zone.trigger.trigger_off ) && isdefined( level.zoneendtime ) && !isdefined( var_0 ) && !level.zonerandomlocationorder && level.chestorigin )
        {
            ...
            if ( var_2 > 0 && var_2 < 10000 )
            {
                ...
                if ( !var_3 )
                {
                    var_4 = level.zone._id_1371D.radius * 6;

                    if ( var_2 < 5000 )
                        var_4 = level.zone._id_1371D.radius * 3;

                    var_5 = distance( level.zone._id_1371D.center, self.origin );

                    if ( var_5 > var_4 )
                        var_0 = chopper_gunner();
                }
                else
                {
                    ...
                    var_8 = checkshouldplaymusic( level.zone );
        ...
        if ( !choose_two_spots( var_9 ) )
            checkformatchend( var_9 );
    }
}
...
{
    return checkweaponswitch( var_0 ).size;
}
...
checkweaponswitch( var_0 )
{
    ...
    foreach ( var_3 in level.participants )
    {
        if ( var_3 != self && scripts\mp\utility\entity::isteamparticipant( var_3 ) && istestclient( self, var_3 ) )
        {
            ...
            {
                if ( !isai( var_3 ) || var_3 choose_two_spots( var_0 ) )
...
choose_two_spots( var_0 )
{
    if ( !scripts\mp\bots\bots_util::bot_is_capturing() )
        return 0;

    return self.get_spotlight_goal_node == var_0;
}
