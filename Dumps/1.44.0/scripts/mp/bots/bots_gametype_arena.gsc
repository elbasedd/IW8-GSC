...
setup_callbacks()
{
    ...
    {
        level.bot_funcs["dropped_weapon_think"] = ::botisonplayerteam;
        level.bot_funcs["dropped_weapon_cancel"] = ::botpickskinid;
    }

    level.bot_funcs["gametype_think"] = ::checkcarryobjectpickupcheck;
}
...
checkcarryobjectpickupcheck()
{
    ...
    if ( !scripts\mp\gametypes\arena.gsc::isnormalloadouts() )
    {
        if ( self botgetdifficultysetting( "advancedPersonality" ) && self botgetdifficultysetting( "strategyLevel" ) > 0 )
            scripts\mp\bots\bots_gametype_gun.gsc::cheeseprompt( self getcurrentweapon().basename, self.pers["bot_original_personality"] );
    }

    self.persistantgametypeteamassign = 0;
    ...
    for (;;)
    {
        if ( isdefined( level.arenaflag ) && level.arenaflag.visibleteam == "any" )
        {
            ...
            if ( !scripts\mp\bots\bots_util::bot_is_capturing() || !self.persistantgametypeteamassign )
            ...
            self.persistantgametypeteamassign = 1;
        }
        else
        {
...
botisonplayerteam()
{
    ...
    for (;;)
    {
        ...
        if ( bot_update_difficultysetttings() || scripts\mp\bots\bots_util::checkrequiredteamstreamcount( 0.33 ) )
        {
            if ( self [[ level.bot_funcs["should_pickup_weapons"] ]]() && !scripts\mp\bots\bots_util::bot_is_remote_or_linked() )
            {
                ...
                {
                    var_4 = var_3[0];
                    level.init_relic_mythic = var_4;
                    botdisablekillstreaks( var_4 );
                }
            }
        }
...
botdisablekillstreaks( var_0 )
{
    if ( scripts\mp\bots\bots_strategy::bot_has_tactical_goal( "seek_dropped_weapon", var_0 ) == 0 )
    {
        ...
        if ( var_0.targetname == "dropped_weapon" )
        {
            ...
            if ( var_6 )
                var_5 = ::bot_verify_and_cache_bombzones;
        }
        ...
        if ( isdefined( var_0.chopper_boss_players_connect_monitor ) )
            var_13 = var_0.chopper_boss_players_connect_monitor;
...
bot_update_difficultysetttings()
{
    var_0 = undefined;
