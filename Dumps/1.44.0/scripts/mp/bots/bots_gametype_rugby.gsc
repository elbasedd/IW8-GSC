...
setup_callbacks()
{
    level.bot_funcs["gametype_think"] = ::chopper_can_see;
    level.bot_funcs["crate_can_use"] = ::crate_can_use;
    level.bot_funcs["know_enemies_on_start"] = undefined;
    level.bot_funcs["jugg_picked_up_cancel"] = ::checkammopickup;
}

_id_123F4()
{
    chopper_death_callback();
    scripts\mp\bots\bots_util::bot_waittill_bots_enabled();
    var_0 = chopper_watch_death( [ "_allies", "_axis" ] );
...
chopper_can_see()
{
    ...
    for (;;)
    {
        ...
        if ( self == level._id_12072 )
        {
            ...
            var_4 = maxtimelimit( var_3.trigger );
            self botsetscriptgoal( var_4.origin, 0, "critical", var_4.angles[1] );
            var_5 = scripts\mp\bots\bots_util::bot_waittill_goal_or_fail();
            continue;
        }

        if ( level._id_12072.team == self.team )
        {
            if ( !scripts\mp\bots\bots_util::bot_is_defending() )
                scripts\mp\bots\bots_strategy::bot_guard_player( level._id_12072, 400 );

            continue;
        }

        if ( gettime() > self.next_time_hunt_carrier || sighttracepassed( self.origin + ( 0, 0, 77 ), level._id_12072.origin + ( 0, 0, 77 ), 0, self ) )
        {
            self botsetscriptgoal( level._id_12072.origin, 16, "hunt" );
...
_id_1206D( var_0 )
{
    var_1 = seen_recently_spawner_time( scripts\mp\gametypes\rugby.gsc::nostand() + 2, "jugg_captured", randomint( 100 ) > 50 );
...
seen_recently_spawner_time( var_0, var_1, var_2, var_3 )
{
    var_4 = 0;
...
{
    wait 1.0;
    chopper_carepackage_set_useable( level.rugby.endzones["allies"][0].trigger );
    chopper_carepackage_set_useable( level.rugby.endzones["axis"][0].trigger );
    level.bot_set_objective_bottargets = 1;
}

chopper_carepackage_set_useable( var_0 )
{
