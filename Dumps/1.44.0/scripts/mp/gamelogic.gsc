...
roundend_checkscorelimit( var_0, var_1 )
{
    ...
    else if ( level.roundscorelimit > 0 )
    {
        ...
        {
            if ( level scripts\mp\utility\game::script_model_spawn_and_use( var_8 ) )
                continue;
...
forceend( var_0 )
{
    ...
    if ( level.teambased )
    {
        ...
        {
            if ( level scripts\mp\utility\game::script_model_spawn_and_use( var_2 ) )
                continue;
...
onscorelimit( var_0 )
{
    ...
    if ( level.teambased )
    {
        var_2 = scripts\mp\gamescore::destroy_vehicle_on_pilot_death();
...
updategameevents()
{
    ...
    if ( level.teambased )
    {
        ...
        if ( !var_17 && !var_20 )
        {
            ...
            if ( _id_124FD() && game["roundsPlayed"] == 0 )
...
waitforplayers( var_0 )
{
    ...
    for (;;)
    {
        ...
        foreach ( var_12 in level.teamnamelist )
        {
            if ( !enemy_push_players_logic( var_12 ) )
            {
...
prematchperiod()
{
    ...
    if ( isdefined( level._id_11B69 ) && [[ level._id_11B69 ]]() )
        return;
    ...
    if ( getdvarint( "scr_live_lobby", 0 ) == 1 && !istrue( level._id_125B8 ) )
    {
        ...
        if ( scripts\mp\utility\game::getgametype() == "br" )
        {
            ...
            level thread scripts\mp\gametypes\br_public.gsc::civilians_killed_stat_row();
        ...
        if ( !istrue( level.blueprintextract_createtempobjective ) )
            game["blockJIP"] = 1;
    }
    else if ( !istrue( level._id_125B8 ) )
    {
        if ( scripts\mp\utility\game::getgametype() == "br" )
        {
            ...
            if ( var_2 != 0 )
            {
                level thread _id_134EF();
                level thread currentrewarddropindex();
                level waittill( "start_prematch" );
            }
...
updatematchbonusscores( var_0 )
{
    ...
    if ( level.teambased )
    {
        ...
        foreach ( var_3 in level.players )
        {
            ...
            var_3 destroy_vehicle_if_driver_dies( var_1 );
        }
    }
    else
    {
...
givematchbonus( var_0, var_1 )
{
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" && scripts\mp\utility\game::onfieldupgradeendbuffer() != "dmz" && scripts\mp\utility\game::onfieldupgradeendbuffer() != "risk" && scripts\mp\utility\game::onfieldupgradeendbuffer() != "kingslayer" && scripts\mp\utility\game::onfieldupgradeendbuffer() != "rumble" )
        return;
...
destroy_vehicle_if_driver_dies( var_0 )
{
    if ( istrue( self.pers["ignoreWeaponMatchBonus"] ) || !isdefined( self.pers["killsPerWeapon"] ) )
        return;

    var_1 = scripts\mp\weaponrank::newhitlocs() / 60;
    var_2 = var_0 / 60;
...
adjustmatchtimerpausedstatefromleadchange( var_0, var_1 )
{
    if ( !isdefined( var_0 ) )
        return;
    ...
    else if ( scripts\mp\utility\game::scn_infil_hackney_heli_npc2() )
    {
        var_3 = 1000 - var_1;
...
checktimelimit( var_0 )
{
    ...
    if ( var_1 > 0 && !isdefined( level.little_bird_mg_explode ) )
    {
        if ( var_2 )
        {
            if ( istrue( level._id_12353 ) )
...
runjiprules()
{
    ...
    if ( !level.nojip )
    {
        if ( scripts\mp\utility\game::isroundbased() && !level.nojip )
        {
            switch ( scripts\mp\utility\game::getgametype() )
            {
                ...
                case "dd":
                    ...
                    break;
                case "br":
                    var_0 = enable_keypad_interaction();
                    break;
                default:
        ...
        else if ( !istrue( level.blueprintextract_createtempobjective ) && scripts\mp\utility\game::gettimepassedpercentage() > level.timepercentagecutoff )
        {
            setnojiptime( 1, 1 );
            level.nojip = 1;
        }
...
{
    return !scripts\mp\gametypes\br_gametypes.gsc::roof_lander_spawn_func( "allowLateJoiners" );
}
...
checkteamscorelimitsoon( var_0 )
{
    ...
    if ( !level.playedmatchendingsoon && scripts\mp\utility\game::canplayhalfwayvo() )
    {
        if ( getteamscore( var_0 ) >= int( level.scorelimit * level.currentround - level.scorelimit / 2 ) )
        {
            ...
            thread scripts\mp\music_and_dialog::stealth_alert_music_index( var_0 );
...
checkscorelimit( var_0 )
{
    ...
    if ( level.teambased )
    {
        ...
        {
            if ( level scripts\mp\utility\game::script_model_spawn_and_use( level.teamnamelist[var_2] ) )
                continue;
    ...
    else
    {
        ...
        if ( self.score < level.roundscorelimit && !istrue( level._id_12176 ) )
            return 0;
    }
...
matchstarttimerperplayer_internal( var_0, var_1 )
{
    ...
    if ( var_1 )
        childthread start_bomb_vest_defusal( var_2 ) childthread clearvisionsettimer( var_2 );

    wait( var_2 );
}

start_bomb_vest_defusal( var_0 )
{
    level endon( "match_start_timer_beginning" );
    var_1 = spawn( "script_origin", ( 0, 0, 0 ) );
    var_1 hide();
...
teamstarttimer( var_0, var_1, var_2 )
{
    ...
    if ( !level.gameended )
    {
        if ( scripts\mp\utility\game::getgametype() == "br" )
        {
            thread scripts\mp\gametypes\br_public.gsc::_id_11B68( var_0 );
            thread scripts\mp\gametypes\br_public.gsc::_id_11B5F( var_0 );
            thread scripts\mp\gametypes\br_public.gsc::bot_known_flag_carrier_loc();
        }
...
roundendwait( var_0, var_1 )
{
    ...
    {
        level [[ level._id_12056 ]]( var_0, var_1 );
        return;
    }

    if ( !level.doeomcombat )
    {
        wait( level._id_11B11 + 0.1 );
...
waittokickoffapcescort( var_0 )
{
    if ( isdefined( var_0 ) )
    {
        foreach ( var_2 in level.players )
        {
            ...
            {
                if ( scripts\mp\utility\game::getgametype() == "br" )
                    var_4 = scripts\mp\gametypes\br_public.gsc::ondefuse( var_2.team, var_2.squadindex );
                else
                    var_4 = scripts\mp\utility\teams::getteamdata( var_2.team, "players" );

                if ( isdefined( var_2.waittoopenaltbunker ) )
                    var_2.waittoopenaltbunker = !var_2.waittoopenaltbunker;
                else
                    var_2.waittoopenaltbunker = 1;
                ...
                if ( istrue( scripts\mp\utility\game::matchmakinggame() ) )
                {
                    if ( isdefined( var_2 ) && var_2 isfireteamleader() )
                    {
                        ...
                        foreach ( var_10 in var_8 )
                        {
                            ...
                            if ( var_11 != var_3 )
                            {
                                if ( isdefined( var_10.waittoopenaltbunker ) )
                                    var_10.waittoopenaltbunker = !var_10.waittoopenaltbunker;
                                else
                                    var_10.waittoopenaltbunker = 1;
...
playbattlechattersoundexpensive()
{
    if ( scripts\mp\utility\game::getgametype() == "br" )
    {
        if ( !scripts\mp\utility\game::privatematch() && !scripts\mp\gametypes\br_public.gsc::revive_or_disconnect_monitor() )
        {
            var_0 = [];
            ...
            foreach ( var_2 in level.players )
            {
                ...
                if ( isdefined( var_2.waittoopenaltbunker ) && var_2.waittoopenaltbunker )
                {
                    var_3 = var_2.team;
                    var_4 = 0;
                    var_5 = var_0.size;
                    ...
                    {
                        var_0[var_5] = var_3;
                        var_7 = scripts\mp\gametypes\br_public.gsc::ondefuse( var_2.team, var_2.squadindex );
                        var_8 = [];
...
callback_startgametype()
{
    ...
    level.get_stay_at_station_time = 0;
    level.gameended = 0;
    level.forcedend = 0;
    ...
    level.dropmenu_tut = getdvarint( "scr_game_cdltuning", 0 );
    level.helipadscriptable = scripts\mp\utility\game::isanymlgmatch();
    ...
    level._id_135FA = getdvarint( "scr_game_wrapKillstreaks" );
    ...
    level.matchrules_vampirism = scripts\mp\utility\dvars::obj_caches_threaded_nags_vo( "scr_" + scripts\mp\utility\game::getgametype() + "_vampirism", "scr_game_vampirism" );

    if ( level.matchrules_vampirism )
        level.modifyplayerdamage = scripts\mp\damage::gamemodemodifyplayerdamage;

    level.finalkillcamtype = scripts\mp\utility\dvars::obj_caches_threaded_nags_vo( "scr_" + scripts\mp\utility\game::getgametype() + "_killcamType", "scr_game_killcamType" );

    if ( level.finalkillcamtype == 2 )
        level.skipfinalkillcam = 1;

    level.allowkillstreaks = scripts\mp\utility\dvars::obj_caches_threaded_nags_vo( "scr_" + scripts\mp\utility\game::getgametype() + "_allowKillstreaks", "scr_game_allowKillstreaks" );
    level.roundretainstreaks = scripts\mp\utility\dvars::obj_caches_threaded_nags_vo( "scr_" + scripts\mp\utility\game::getgametype() + "_roundRetainStreaks", "scr_game_roundRetainStreaks" );
    level.roundretainstreakprog = scripts\mp\utility\dvars::obj_caches_threaded_nags_vo( "scr_" + scripts\mp\utility\game::getgametype() + "_roundRetainStreakProg", "scr_game_roundRetainStreakProg" );
    level.deathretainstreaks = scripts\mp\utility\dvars::obj_caches_threaded_nags_vo( "scr_" + scripts\mp\utility\game::getgametype() + "_deathRetainStreaks", "scr_game_deathRetainStreaks" );
    level.allowperks = scripts\mp\utility\dvars::obj_caches_threaded_nags_vo( "scr_" + scripts\mp\utility\game::getgametype() + "_allowPerks", "scr_game_allowPerks" );
    level.allowsupers = scripts\mp\utility\dvars::obj_caches_threaded_nags_vo( "scr_" + scripts\mp\utility\game::getgametype() + "_allowSupers", "scr_game_allowSupers" );
    level.superfastchargerate = scripts\mp\utility\dvars::obj_caches_threaded_nags_vo( "scr_" + scripts\mp\utility\game::getgametype() + "_superFastChargeRate", "scr_game_superFastChargeRate" );
    level.superpointsmod = scripts\mp\utility\dvars::obj_caches_tanks( "scr_" + scripts\mp\utility\game::getgametype() + "_superPointsMod", "scr_game_superPointsMod" );
    level.spawnprotectiontimer = scripts\mp\utility\dvars::obj_caches_tanks( "scr_" + scripts\mp\utility\game::getgametype() + "_spawnProtectionTimer", "scr_game_spawnProtectionTimer" );
    level.lethaldelay = scripts\mp\utility\dvars::obj_caches_tanks( "scr_" + scripts\mp\utility\game::getgametype() + "_lethalDelay", "scr_game_lethalDelay" );
    level.equipmentmatchstartshieldms = scripts\mp\utility\dvars::obj_caches_tanks( "scr_" + scripts\mp\utility\game::getgametype() + "_equipmentMSProtect", "scr_game_equipmentMSProtect" ) * 1000;
    level.magcount = scripts\mp\utility\dvars::obj_caches_threaded_nags_vo( "scr_" + scripts\mp\utility\game::getgametype() + "_magcount", "scr_game_magcount" );

    if ( !isdefined( level.practiceround ) )
        level.practiceround = scripts\mp\utility\dvars::obj_caches_threaded_nags_vo( "scr_" + scripts\mp\utility\game::getgametype() + "_practiceRound", "scr_game_practiceRound" );

    level.postgameexfil = scripts\mp\utility\dvars::obj_caches_threaded_nags_vo( "scr_" + scripts\mp\utility\game::getgametype() + "_postGameExfil", "scr_game_postGameExfil" );
    level.exfilactivetimer = scripts\mp\utility\dvars::obj_caches_tanks( "scr_" + scripts\mp\utility\game::getgametype() + "_exfilActiveTimer", "scr_game_exfilActiveTimer" );
    level.exfilextracttimer = scripts\mp\utility\dvars::obj_caches_tanks( "scr_" + scripts\mp\utility\game::getgametype() + "_exfilExtractTimer", "scr_game_exfilExtractTimer" );
    level.useammorestocklocs = scripts\mp\utility\dvars::obj_caches_tanks( "scr_" + scripts\mp\utility\game::getgametype() + "_useAmmoRestockLocs", "scr_game_useAmmoRestockLocs" );
    level._id_12854 = scripts\mp\utility\dvars::obj_caches_threaded_nags_vo( "scr_" + scripts\mp\utility\game::getgametype() + "_allow3rdspectate", "scr_game_allow3rdspectate" );
    setdvar( "LKNNQKNTS", level._id_12854 );
    ...
    if ( scripts\mp\utility\game::scalertoscaleinfo() )
        end_game_win();
    ...
    level.radarhideshots = scripts\mp\utility\dvars::obj_caches_threaded_nags_vo( "scr_" + var_0 + "_radarHidePings", "scr_game_radarHidePings" );
    setomnvar( "ui_compass_hide_weapon_pings_minimap", level.radarhideshots );
    level.navbarhideshots = scripts\mp\utility\dvars::obj_caches_threaded_nags_vo( "scr_" + var_0 + "_navBarHidePings", "scr_game_navBarHidePings" );
    setomnvar( "ui_compass_hide_weapon_pings_navbar", level.navbarhideshots );
    level.navbarhideenemies = scripts\mp\utility\dvars::obj_caches_threaded_nags_vo( "scr_" + var_0 + "_navBarHideEnemy", "scr_game_navBarHideEnemy" );
    ...
    level.helipad = getdvarint( "scr_player_disableMount" );
    level.disablebattlechatter = getdvarint( "scr_game_disableBattleChatter" );
    level.helidestroyvehiclestouchtrace = getdvarint( "scr_game_disableAnnouncer" );
    level.scoremod = [];
    ...
    {
        if ( scripts\mp\utility\game_utility_mp::support_box_is_scriptable_model() )
        {
    ...
    scripts\cp_mp\utility\game_utility::regendelayspeedfunc();
    level.maxallowedteamkills = scripts\mp\utility\dvars::obj_caches_threaded_nags_vo( "scr_" + scripts\mp\utility\game::getgametype() + "_ffPunishLimit", "scr_game_ffPunishLimit" );
    thread scripts\mp\init_cp_mp::init();
    thread scripts\mp\teams::init();
    scripts\mp\vehicles\little_bird_mg_mp::_id_12BC9();
    scripts\cp_mp\utility\player_utility::register_jugg_maze_objectives();
    ...
    scripts\mp\utility\player_frame_update_aggregator::registerplayerframeupdatecallback( scripts\mp\weapons::phase_two_combat );

    if ( scripts\mp\utility\game::matchmakinggame() && !scripts\mp\utility\game::privatematch() )
        level scripts\common\ui::lui_registercallback( "play_again_squad_request", ::waittokickoffapcescort );
    ...
    if ( getdvarint( "scr_allow_custom_loadouts", 0 ) == 0 || scripts\mp\utility\game::ring() )
...
timelimitthread()
{
    ...
    while ( game["state"] == "playing" )
    {
        ...
        if ( isdefined( level.starttime ) )
        {
            if ( gettimeremaining() < 3000 || istrue( level.little_bird_mg_explode ) )
            {
                waitframe();
                continue;
            }
...
timelimitclock()
{
    ...
    if ( isdefined( level._id_12C86 ) )
    {
        thread [[ level._id_12C86 ]]();
        return;
    }
...
gametimer()
{
    ...
    if ( !isdefined( game["startTimeFromMatchStart"] ) )
    {
        ...
        scripts\common\utility::_id_12ED6( level.stadiumpuzzleactive );
    }
...
startgame()
{
    ...
    if ( getdvarint( "scr_game_updated_prematch_allows", 1 ) == 1 )
    {
        foreach ( var_1 in level.players )
        {
            ...
            if ( scripts\mp\utility\game::getgametype() == "br" )
                var_1 scripts\mp\persistence::relic_mythic_modifyplayerdamage();
        }
    }
...
cleanpatchablecollision()
{
    ...
    foreach ( var_3, var_2 in var_0 )
    {
        if ( isdefined( var_2 ) )
        {
            if ( isdefined( level.vehicle_collision ) && istrue( level.vehicle_collision[var_3] ) )
            {
                level.vehicle_collision[var_3] = undefined;
                continue;
            }
    ...
    level.vehicle_clearpreventplayercollisiondamagefortimeafterexit = 1;

    if ( isdefined( level.vehicle_collision ) )
    {
        foreach ( var_3, var_5 in level.vehicle_collision )
        {

        }
    }

    level.vehicle_collision = undefined;
}

_id_11F05( var_0 )
{
    if ( istrue( level.vehicle_clearpreventplayercollisiondamagefortimeafterexit ) )
        return;

    if ( !isdefined( level.vehicle_collision ) )
        level.vehicle_collision = [];

    level.vehicle_collision[var_0] = 1;
}
...
wavetimerwatcher()
{
    ...
    for (;;)
    {
        scripts\engine\utility::_id_133EF( "wave_respawn_allies", "wave_respawn_axis" );
...
displayroundend( var_0, var_1, var_2 )
{
    ...
    if ( scripts\mp\utility\game::waslastround() && scripts\mp\utility\game::iswinbytworulegametype() && scripts\mp\utility\game::_id_1251A() )
    {
        var_0 = "tie";
...
displayroundswitch()
{
    ...
    if ( isdefined( level.sunsamplesizenear ) )
        [[ level.sunsamplesizenear ]]();

    var_0 = nonbunkerdoors();
...
freezeallplayers( var_0, var_1, var_2, var_3 )
{
    ...
    var_4 = scripts\mp\utility\game::safesetalpha();
...
setendofroundsoundtimescalefactor()
{
    if ( !isdefined( level.is_a_player_near ) )
    {
        createheadiconatorigin( "end_of_round" );
        level.is_a_player_near = 1;
        level._id_11F19 = undefined;
    }
}

resetsoundtimescalefactor()
{
    if ( !isdefined( level._id_11F19 ) )
    {
        createheadiconatorigin( "reset" );
        level.is_a_player_near = undefined;
        level.trial_hitmarker = undefined;
        level._id_11F19 = 1;
    }
}
...
restart()
{
    if ( isdefined( level.unset_maze_ai_stealth_settings ) )
        [[ level.unset_maze_ai_stealth_settings ]]();
...
endgame_regularmp( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    scripts\common\utility::_id_12ED6( level.stage );
    var_5 = 0;
    ...
    level.get_players_inside_the_plane = istrue( var_3 ) && scripts\mp\utility\game::getgametype() == "ctf";

    if ( isdefined( var_0 ) && level.get_players_inside_the_plane )
    {
    ...
    foreach ( var_14 in level.players )
    {
        if ( !isbot( var_14 ) && isdefined( var_14.team ) && var_14.team != "spectator" && var_14.team != "follower" )
            spawn_compound_final_push( var_14 );
    }
...
endgame_showkillcam()
{
    ...
    if ( istrue( level.colorise_warnings ) )
        return;
...
interruptbombplanting( var_0 )
{
    if ( isdefined( var_0 ) )
    {
        if ( var_0 == 1 || var_0 == 2 )
        {
            level.ai_goal_update_population = 0;
            level.addjuggernautcharge = var_0;
...
endgame_endround( var_0, var_1, var_2, var_3 )
{
    ...
    if ( scripts\mp\utility\game::waslastround() )
    {
        if ( scripts\mp\utility\game::getgametype() == "arena" )
        {
            ...
            foreach ( var_7 in level.players )
            {
                ...
                {
                    if ( var_7 == var_4[0] )
                        var_7 scripts\cp_mp\pet_watch::appliednukethreads();
                }
            }
        }
    ...
    rankedmatchupdates( var_0 );
    setdvar( "SLLNLPRON", scripts\mp\utility\game::safesetalpha() );
    ...
    else
    {
        if ( level.teambased )
        {
            ...
            {
                if ( level scripts\mp\utility\game::script_model_spawn_and_use( var_25 ) )
                    continue;
            ...
            if ( !scripts\mp\utility\game::waslastround() )
            {
                ...
                foreach ( var_7 in level.players )
                {
                    ...
                    if ( isalive( var_7 ) && isdefined( var_7.matchdatalifeindex ) )
                        var_7 scripts\common\utility::_id_12ED6( level.stack_patch_waittill_stack, var_7.matchdatalifeindex, undefined, undefined, "MOD_ROUND_ENDED", "none", undefined, undefined, undefined );
                }
...
endgame_endgame( var_0, var_1, var_2, var_3 )
{
    ...
    if ( scripts\mp\gametypes\br_public.gsc::revive_or_disconnect_monitor() && !level.forcedend )
    {
        level.ai_goal_update_population = 1;
        level scripts\common\ui::lui_registercallback( "exit_squad_eliminated", ::interruptbombplanting );
    }

    if ( scripts\mp\utility\game::matchmakinggame() && isdefined( level._id_124F2 ) && [[ level._id_124F2 ]]() )
    ...
    updateleaderboardstats();
    level scripts\mp\accolades::isgroundwarsiege();
    level.doingbroshot = scripts\mp\broshot::initbroshot( var_0 );
    var_4 = scripts\mp\gametypes\br_gametypes.gsc::roof_lander_spawn_func( "endGameIntermission" );

    if ( !level.doingbroshot && istrue( var_4 ) )
    {
        level notify( "spawning_intermission" );
        var_5 = nonbunkerdoors();

        foreach ( var_7 in level.players )
        {
            var_7 thread scripts\mp\utility\game::setuipostgamefade( 0.0 );

            if ( !isdefined( level.supportbox_playusesound ) || !var_7 [[ level.supportbox_playusesound ]]() )
    ...
    if ( !level.doingbroshot )
    {
        if ( scripts\mp\utility\game::getgametype() == "br" )
        {
            scripts\mp\gametypes\br_circle.gsc::playerstartjailsetcontrols();
            ...
            if ( getdvarint( "post_game_play_again_enabled", 1 ) == 1 )
            {
                ...
                {
                    if ( istrue( level.ai_goal_update_population ) )
    ...
    if ( scripts\mp\utility\game::getgametype() != "br" || scripts\mp\utility\game::onfieldupgradeendbuffer() == "dmz" || scripts\mp\utility\game::onfieldupgradeendbuffer() == "risk" || scripts\mp\utility\game::onfieldupgradeendbuffer() == "kingslayer" || scripts\mp\utility\game::onfieldupgradeendbuffer() == "rumble" || scripts\mp\utility\game::sandbox_safe_area() )
        processlobbydata();
    ...
    foreach ( var_7 in level.players )
    {
        ...
        if ( isalive( var_7 ) && isdefined( var_7.matchdatalifeindex ) )
            var_7 scripts\common\utility::_id_12ED6( level.stack_patch_waittill_stack, var_7.matchdatalifeindex, undefined, undefined, "MOD_GAME_ENDED", "none", undefined, undefined, undefined );
        ...
        var_7 scripts\common\utility::_id_12ED6( level.stack_patch_waittill_root );
    }

    scripts\common\utility::_id_12ED6( level.stage1accradius );
    ...
    if ( scripts\mp\gametypes\br_public.gsc::revive_or_disconnect_monitor() && isdefined( level.addjuggernautcharge ) && level.addjuggernautcharge == 2 )
    {
...
nonbunkerdoors()
{
    ...
    {
        var_0 = spawnstruct();
        var_0.origin = level.currentcarrier._id_12858.origin;
        var_0.angles = level.currentcarrier._id_12858.angles;
    }
...
branalytics_missionend( var_0 )
{
    ...
    {
        var_0.clientmatchdataid = level.get_stay_at_station_time;
        level.get_stay_at_station_time++;
    }
}

_id_11BB1( var_0 )
{
    if ( istrue( var_0._id_11BB1 ) )
        return;
    ...
    scripts\mp\matchdata::_id_1237E( var_0 );
}
...
trackattackerleaderboarddeathstats( var_0, var_1, var_2, var_3 )
{
    if ( isdefined( var_0 ) && isplayer( var_0 ) )
    {
        if ( var_3 != "MOD_FALLING" )
        {
            ...
            {
                var_0 scripts\common\utility::_id_12ED6( level.stack_patch_thread_root, "tactical", "kills", 1, var_2 );
                var_0 scripts\common\utility::_id_12ED6( level.stack_patch_thread_root, "tactical", "hits", 1, var_2 );
            ...
            if ( var_3 == "MOD_MELEE" && !scripts\mp\riotshield::isriotshield( var_2 ) && !scripts\mp\utility\weapon::isknifeonly( var_2 ) && !scripts\mp\utility\weapon::respawns_on_failed_unload( var_2 ) && !scripts\mp\utility\weapon::isaxeweapon( var_2 ) )
            {
                var_0 scripts\common\utility::_id_12ED6( level.stack_patch_thread_root, "none", "kills", 1, var_2 );
                var_0 scripts\common\utility::_id_12ED6( level.stack_patch_thread_root, "none", "hits", 1, var_2 );
...
setweaponstat( var_0, var_1, var_2 )
{
    ...
    if ( isdefined( var_3.vandalize_attack_max_cooldown ) )
        var_3 = var_3.vandalize_attack_max_cooldown;

    if ( !isdefined( level.displayplunderloss ) )
        level.displayplunderloss = scripts\mp\utility\game::getgametype() == "br" && getdvarint( "scr_track_picked_up_weapon_stats", 1 ) == 1;

    if ( scripts\mp\utility\weapon::ispickedupweapon( var_3 ) && !istrue( level.displayplunderloss ) )
        return;
    ...
    if ( var_5 == "weapon_grenade" || var_5 == "weapon_explosive" || var_4 == "trophy_mp" || var_4 == "forcepush_mp" )
    {
        ...
        scripts\common\utility::_id_12ED6( level.stadium_three_death_func, var_4, var_2, var_1, var_6, var_3 );
        return;
    }
    ...
    if ( var_2 == "deaths" )
    {
        ...
        scripts\common\utility::_id_12ED6( level.stadium_three_death_func, var_9, "deaths", var_1, var_6, var_3 );

        foreach ( var_12 in var_10 )
        {
            scripts\mp\persistence::incrementattachmentstat( var_12, var_2, var_1, var_3 );
            scripts\common\utility::_id_12ED6( level.stack_patch_thread_root, var_12, var_2, var_1, var_3 );
        }
    }
}
...
blueprintextract_setunlockablelootid()
{
...
updateleaderboardstatscontinuous()
{
    ...
    if ( !blueprintextract_setunlockablelootid() )
        return;
...
_id_130A6()
{
    ...
    if ( isdefined( level.completesmokinggunquest ) && isdefined( level.completesmokinggunquest.name ) )
    {
        switch ( level.completesmokinggunquest.name )
        {
            ...
            case "kingslayer":
            case "dmz":
                var_0 = var_0 + ( "_" + level.completesmokinggunquest.name );
                break;
            case "dbd":
            case "brz":
            case "":
    ...
    var_1 = scripts\engine\utility::ter_op( level.teambased, self.score, self.setupx1timelimit["gamemodeScore"] );
    ...
    incrementleaderboardstat( "kills", var_0, self.setupx1timelimit["kills"] );
    incrementleaderboardstat( "deaths", var_0, self.setupx1timelimit["deaths"] );

    switch ( scripts\mp\utility\game::getgametype() )
    {
        case "war":
            incrementleaderboardstat( "assists", var_0, self.setupx1timelimit["assists"] );
            break;
        case "arena":
            incrementleaderboardstat( "damage", var_0, self.setupx1timelimit["damage"] );
            incrementleaderboardstat( "assists", var_0, self.setupx1timelimit["assists"] );
            break;
        case "arm":
            incrementleaderboardstat( "captures", var_0, self.setupx1timelimit["captures"] );
            incrementleaderboardstat( "defends", var_0, self.setupx1timelimit["defends"] );
            break;
        case "cyber":
            incrementleaderboardstat( "revives", var_0, self.setupx1timelimit["rescues"] );
            incrementleaderboardstat( "plants", var_0, self.setupx1timelimit["plants"] );
            break;
        case "dm":
            updateleaderboardstatmaximum( "streak", var_0, self.setupx1timelimit["killChains"] );
            break;
        case "dom":
            incrementleaderboardstat( "captures", var_0, self.setupx1timelimit["captures"] );
            incrementleaderboardstat( "defends", var_0, self.setupx1timelimit["defends"] );
        ...
        case "sd":
            incrementleaderboardstat( "plants", var_0, self.setupx1timelimit["plants"] );
            incrementleaderboardstat( "defuses", var_0, self.setupx1timelimit["defuses"] );
            break;
        case "conf":
            incrementleaderboardstat( "confirms", var_0, self.setupx1timelimit["confirmed"] );
            incrementleaderboardstat( "denies", var_0, self.setupx1timelimit["denied"] );
            break;
        case "hq":
            incrementleaderboardstat( "captures", var_0, self.setupx1timelimit["captures"] );
            incrementleaderboardstat( "defends", var_0, self.setupx1timelimit["defends"] );
            break;
        case "koth":
            incrementleaderboardstat( "objTime", var_0, self.setupx1timelimit["objTime"] );
            incrementleaderboardstat( "defends", var_0, self.setupx1timelimit["defends"] );
            break;
        case "ctf":
            incrementleaderboardstat( "captures", var_0, self.setupx1timelimit["captures"] );
            incrementleaderboardstat( "returns", var_0, self.setupx1timelimit["returns"] );
            break;
        case "sr":
            incrementleaderboardstat( "plants", var_0, self.setupx1timelimit["plants"] );
            incrementleaderboardstat( "rescues", var_0, self.setupx1timelimit["rescues"] );
            break;
        case "siege":
            incrementleaderboardstat( "captures", var_0, self.setupx1timelimit["captures"] );
            incrementleaderboardstat( "revives", var_0, self.setupx1timelimit["rescues"] );
            break;
        case "grind":
            incrementleaderboardstat( "banks", var_0, self.setupx1timelimit["confirmed"] );
            incrementleaderboardstat( "denies", var_0, self.setupx1timelimit["denied"] );
            break;
        case "infect":
            incrementleaderboardstat( "time", var_0, scripts\mp\utility\stats::getpersstat( "extrascore0" ) );
            incrementleaderboardstat( "infected", var_0, self.setupx1timelimit["killsAsInfected"] );
            break;
        case "gun":
            incrementleaderboardstat( "stabs", var_0, self.setupx1timelimit["stabs"] );
            incrementleaderboardstat( "setBacks", var_0, self.setupx1timelimit["setbacks"] );
            break;
        case "grnd":
            incrementleaderboardstat( "objTime", var_0, self.setupx1timelimit["objTime"] );
            incrementleaderboardstat( "defends", var_0, self.setupx1timelimit["defends"] );
        ...
        case "pill":
            ...
            break;
        case "br":
            incrementleaderboardstat( "downs", var_0, self.setupx1timelimit["downs"] );
            incrementleaderboardstat( "contracts", var_0, self.setupx1timelimit["contracts"] );
            incrementleaderboardstat( "wins", var_0, self.setupx1timelimit["wins"] );
            incrementleaderboardstat( "topFive", var_0, self.setupx1timelimit["topFive"] );
            incrementleaderboardstat( "topTen", var_0, self.setupx1timelimit["topTen"] );
            incrementleaderboardstat( "topTwentyFive", var_0, self.setupx1timelimit["topTwentyFive"] );
            incrementleaderboardstat( "gamesPlayed", var_0, self.setupx1timelimit["gamesPlayed"] );
            incrementleaderboardstat( "revives", var_0, self.setupx1timelimit["rescues"] );
            incrementleaderboardstat( "cash", var_0, self.setupx1timelimit["cash"] );
            incrementleaderboardstat( "objTime", var_0, self.setupx1timelimit["objTime"] );
            break;
    }
}
...
checkforbestweapon()
{
    var_0 = scripts\common\utility::_id_12ED6( level.stack_patch_thread_leaf );
    var_1 = "";
    var_2 = -1;
...
_id_134EF( var_0 )
{
    if ( !istrue( level.spawn_tugofwar_tank ) )
        return;
    ...
    if ( var_6 <= var_8 )
    {
        ...
        if ( isdefined( level.unset_ignoreall_after_notify ) )
            [[ level.unset_ignoreall_after_notify ]]();
    }
...
_id_11ED7()
{
    ...
    foreach ( var_2 in var_0 )
        scripts\cp_mp\utility\game_utility::_id_11ED3( var_2, "targetname" );
}

moving_platform_offset()
{
    return 9;
}
...
end_game_win()
{
    level.allowsupers = getdvarint( "scr_" + scripts\mp\utility\game::onfieldupgradeendbuffer() + "_allowSupers", level.allowsupers );
}
...
spawn_compound_final_push( var_0 )
{
    ...
    if ( isdefined( var_0.getemptyloadout ) )
        var_6 = var_0.getemptyloadout;
