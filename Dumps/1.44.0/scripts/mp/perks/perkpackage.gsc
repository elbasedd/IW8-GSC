        ...
        if ( !self.perkpackagedata.isusing && scripts\mp\supers::issuperready() )
        {
            if ( self.perkpackagedata.istwomode )
            {
                ...
                if ( 1 )
                {
                    ...
                    if ( isdefined( level.vehicleindangertracking ) )
                        [[ level.vehicleindangertracking ]]();
                }
            }

            if ( var_0 && !istrue( self._id_125BE ) )
...
perkpackage_updateifchanged()
{
    ...
    if ( !var_2 || var_3 || var_4 || self.perkpackagedata.forcereset )
    {
        ...
        if ( scripts\mp\supers::issuperready() && !istrue( self._id_125BE ) )
            thread scripts\mp\supers::showsuperremindersplash();
...
perkpackage_getsecondfieldupgrade()
{
    ...
    if ( isdefined( self.use_airdrop_fx ) )
        var_0 = self.use_airdrop_fx;
...
perkpackagemenu_menuthink()
{
    ...
    wait 0.3;
    var_0 = scripts\engine\utility::_id_133F9( "perkPackageMenu_option1", "perkPackageMenu_option2", "perkPackageMenu_close", "death" );
    var_1 = 0;
...
vehiclelinkto( var_0, var_1 )
{
    ...
    self.usbwm = var_0;
    self.use_airdrop_fx = var_1;
    perkpackage_updateifchanged();
}

vehicleoccupantdeathcustomcallback()
{
    self.usbwm = undefined;
    self.use_airdrop_fx = undefined;
}
