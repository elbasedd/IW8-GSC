...
setupvfxobjs( var_0 )
{
    foreach ( var_2 in var_0 )
    {
        if ( isdefined( var_2.script_noteworthy ) && scripts\engine\utility::string_starts_with( var_2.script_noteworthy, "vfx_" ) )
        {
            ...
            if ( !score_event_kill( "setupVFXObjs(): obj", var_2 ) )
                continue;

            if ( !score_event_kill( "setupVFXObjs(): model", var_3 ) )
                continue;
...
delayfxcall( var_0, var_1, var_2 )
{
    ...
    if ( !score_event_kill( "delayFXCall()", var_1 ) )
        return;
...
animsuite_translation_pingpong()
{
    ...
    for (;;)
    {
        ...
        if ( !score_event_kill( "animSuite_Translation_PingPong()", self ) )
            return;
...
animsuite_playthreadedsound( var_0, var_1 )
{
    ...
    if ( !score_event_kill( "animSuite_playThreadedSound()", self ) )
        return;
...
animsuite_translation_once()
{
    ...
    for (;;)
    {
        ...
        if ( !score_event_kill( "animSuite_Translation_Once()", self ) )
            return;
    }
}
...
animsuite_rotation_pingpong()
{
    ...
    for (;;)
    {
        ...
        if ( !score_event_kill( "animSuite_Rotation_PingPong()", self ) )
            return;
...
animsuite_rotation_continuous()
{
    ...
    for (;;)
    {
        ...
        if ( !score_event_kill( "animSuite_Rotation_Continuous()", self ) )
            return;
    }
}

score_event_kill( var_0, var_1 )
{
    if ( !isdefined( var_1 ) )
        return 0;
