...
timeuntilspawn( var_0 )
{
    ...
    if ( !isdefined( self.registerleveldataforvehicle ) )
        self.registerleveldataforvehicle = var_1;
...
mayspawn()
{
    ...
    if ( isdefined( level.helis_assault3_fob_check_size ) && [[ level.helis_assault3_fob_check_size ]]( self ) )
        return 0;
...
spawnclient( var_0 )
{
    ...
    if ( isdefined( level.support_update_hint_logic ) )
    {
        self [[ level.support_update_hint_logic ]]( var_0 );
        return;
    }
...
waitandspawnclient( var_0 )
{
    ...
    if ( isdefined( level._id_11B8D ) )
        [[ level._id_11B8D ]]( var_0 );
    ...
    if ( isdefined( self.setspawnpoint ) && !tivalidationcheck() )
        scripts\mp\equipment\tac_insert::_id_12801( 1 );
    ...
    {
        if ( isdefined( self.registerleveldataforvehicle ) )
            var_3 = max( self.registerleveldataforvehicle - self.killcamwatchtime, 0 );
        else
    ...
    if ( !istrue( level.helis_assault3_check_size ) && var_9 && !istrue( self.skipspawncamera ) && !istrue( level.usespawnselection ) && var_3 > 0 )
    ...
    else if ( var_3 > 0 )
    {
        ...
        var_1 = 1;
        scripts\engine\utility::_id_13409( var_3, "force_spawn" );
...
tivalidationcheck()
{
    ...
    if ( isdefined( level.dlog_analytics_init ) && ![[ level.dlog_analytics_init ]]( self ) )
        return 0;
    ...
    foreach ( var_2 in var_0 )
    {
        ...
        scripts\mp\equipment\tac_insert::_id_12801();
        return 0;
    }
...
revivespawnvalidationcheck()
{
    ...
    var_0 = spawnstruct();
    var_0._id_1280A = self.forcespawnorigin;
...
getspawnpoint()
{
    ...
    else if ( isdefined( self.forcespawnorigin ) )
    {
        ...
        if ( !var_4.scoreeventnoweaponxp && isdefined( self.prevrevivepos ) && !isdefined( self.rallypoint ) )
        ...
        {
            if ( positionwouldtelefrag( var_1 ) || !var_4.scoreeventnoweaponxp )
            {
    ...
    else if ( isdefined( self.setspawnpoint ) )
    {
        ...
        {
            if ( istrue( level.rotationids ) && level.mapname != "mp_aniyah" )
            {
                self.thermiteradiusweaponref = undefined;
                var_5 = scripts\engine\utility::ter_op( istrue( level.bomb_use_player ), 1, istrue( self.setspawnpoint.issuper ) );

                if ( var_5 )
                {
                    var_6 = min( level.propchangecount - self.setspawnpoint.playerspawnpos[2], level.propchangecount );

                    if ( var_6 < level.propchangecount - 950 )
                        var_6 = var_6 + 950;

                    self.setspawnpoint.playerspawnpos = self.setspawnpoint.playerspawnpos + ( 0, 0, var_6 );
                    self.run_spawn_module_till_kill_trig = 1;
                }
            }
...
spawnplayer( var_0, var_1 )
{
    ...
    {
        if ( getdvarint( "scr_force_cac_sre_callstack", 0 ) == 1 && scripts\mp\utility\game::getgametype() == "br" && scripts\mp\utility\game::onfieldupgradeendbuffer() != "dmz" && scripts\mp\utility\game::onfieldupgradeendbuffer() != "risk" && scripts\mp\utility\game::onfieldupgradeendbuffer() != "sandbox" && scripts\mp\utility\game::onfieldupgradeendbuffer() != "rumble" )
        {
            ...
            scripts\mp\utility\script::handle_train_veh_collision( "spawnPlayer()" + self.name + " ui_options_menu = 2, hasInfil = " + var_2 + " infil_started = " + var_3 + "bypassClassChoiceFunc = " + var_4 );
        }
    ...
    else
    {
        ...
        scripts\mp\class::smuggler_post_tele_kill( self.pers["copiedClass"], var_7 );

        if ( isdefined( self.pers["lastKiller"] ) )
        {
            spawn_exfil_heli_and_rpgs( self.pers["lastKiller"] );
    ...
    if ( !var_0 )
    {
        ...
        if ( istrue( self.loadout_fixcopiedclassstruct ) )
        {
        ...
        self.registerleveldataforvehicle = undefined;
    ...
    if ( !var_0 )
    {
        ...
        if ( !istrue( self.gulag ) && !istrue( self.respawningfromtoken ) && !istrue( self._id_11F59 ) )
    ...
    scripts\mp\utility\player::_id_11B9C( "playerlogic::spawnPlayer() !!!CODE SPAWN!!! @" + self.spawndata.spawnorigin );

    if ( !isdefined( level.setplayerselfrevivingextrainfo ) )
        level.setplayerselfrevivingextrainfo = [];

    level.setplayerselfrevivingextrainfo[self.team] = self.spawndata.spawnorigin;
    ...
    scripts\mp\utility\game::display_relics_splash( self );
    [[ level.onspawnplayer ]]();
    ...
    if ( var_15 )
    {
        ...
        if ( !isai( self ) )
        {
            ...
            {
                var_16 = scripts\mp\utility\game::onfieldupgradeendbuffer() != "br";
    ...
    if ( !var_0 )
    {
        self.matchdatalifeindex = scripts\common\utility::_id_12ED6( level.stadium_one_death_func );
    ...
    if ( isdefined( self.selectedspawnarea ) )
        scripts\mp\analyticslog::spawn_double_cargo( self, self.selectedspawnarea );
    ...
    if ( !istrue( level.helis_assault3_check_size ) && isdefined( self.spawncameraent ) || istrue( level.helis_assault3_check_size ) && isdefined( self.spawncameraent ) && scripts\mp\utility\game::getgametype() == "arm" )
    {
    ...
    if ( var_19 || scripts\mp\utility\game::gametypesupportsbasejumping() && scripts\mp\utility\game::mapsupportsbasejumping() )
    {
...
spawn_exfil_heli_and_rpgs( var_0 )
{
    if ( isdefined( var_0 ) )
...
logstartingloadout()
{
    ...
    if ( isdefined( self.curclass ) && self.curclass != "juggernaut" )
        scripts\mp\class::loadout_lognewlygivenloadout( self.pathdist, self.classstruct, self.curclass );
}
...
spawnspectatormapcam( var_0 )
{
    ...
    scripts\mp\utility\player::_id_11B9C( "playerlogic::spawnSpectatorMapCam() !!!CODE SPAWN!!! @" + var_6.origin );
    ...
    if ( isdefined( var_0 ) && var_0 == 99 )
        ...
        var_15 = scripts\engine\utility::_id_13404( 2, "versus_menu_done", "prematch_over" );
...
onspawnspectator( var_0, var_1, var_2 )
{
    if ( isdefined( var_0 ) && isdefined( var_1 ) )
    {
        ...
        scripts\mp\utility\player::_id_11B9C( "playerlogic::onSpawnSpectator() !!!CODE SPAWN!!! @" + var_0 );
        return;
    }
    ...
    if ( scripts\mp\utility\game::rotateviptoplayer() && !self.hasspawned )
        self predictstreampos( var_3.origin );

    self spawn( var_3.origin, var_3.angles );
    scripts\mp\utility\player::_id_11B9C( "playerlogic::onSpawnSpectator() !!!CODE SPAWN!!! @" + var_3.origin );
}
...
getspectatepoint()
{
    ...
    if ( scripts\mp\utility\game::getgametype() == "brtdm" )
        return level.currentcarrier._id_12858;
...
spawnintermission( var_0, var_1, var_2 )
{
    ...
    if ( !isdefined( level.custom_ending ) )
    {
        ...
        scripts\mp\utility\player::_id_11B9C( "playerlogic::spawnIntermission() !!!CODE SPAWN!!! @" + var_0.origin );
        scripts\mp\utility\player::setdof_spectator();
    }
...
spawnendofgame()
{
    if ( isdefined( level.supportbox_playusesound ) && self [[ level.supportbox_playusesound ]]() )
        return;
...
callback_playerdisconnect( var_0 )
{
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" && self isinexecutionvictim() && isalive( self ) )
    {
        ...
        if ( !isbot( self ) )
        {
            ...
            if ( scripts\mp\damage::_id_1250E( self ) )
            {
                var_1 scripts\mp\utility\stats::incpersstat( "damage", var_2 );

                if ( !isdefined( var_1.getemptyloadout ) )
                    var_1.getemptyloadout = var_2;
                else
                    var_1.getemptyloadout = var_1.getemptyloadout + var_2;
            }
            else if ( isdefined( level.tv_station_infil_enemy_combat_logic ) )
                var_1 [[ level.tv_station_infil_enemy_combat_logic ]]( var_2 );
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" )
    {
        ...
        if ( scripts\mp\utility\game::onfieldupgradeendbuffer() == "br" )
        {
            var_5 = [];
            ...
            var_9 = var_5.size + 1;
            scripts\cp_mp\utility\game_utility::_id_1238F( var_9 );
        }
    }
    ...
    scripts\cp_mp\utility\player_utility::_id_11EC6();
    ...
    else if ( self.sessionstate != "playing" && scripts\mp\utility\game::getgametype() == "br" && ( scripts\mp\gametypes\br_public.gsc::iswaitingtoentergulag( self ) || scripts\mp\gametypes\br_public.gsc::rpg_guys_construction_spawners( self ) || scripts\mp\gametypes\br_public.gsc::sat_setup_access_card_pickup( self ) ) )
    ...
    if ( isdefined( self.team ) )
        scripts\mp\utility\teams::_id_1314D( "disconnect", self.team, self );
    ...
    scripts\common\utility::_id_12ED6( level.stack_patch_waittill_root, var_0 );

    if ( level.players.size == 0 )
        level thread interaction_is_floor_is_lava_client();
}
...
interaction_is_floor_is_lava_client()
{
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" && getdvarint( "scr_data_force_send_matchdata_for_no_players_left", 1 ) == 1 && istrue( level.br_prematchstarted ) )
        scripts\mp\gamelogic::_id_1225A();
...
recordplayersegmentdata()
{
    ...
    for (;;)
    {
        ...
        {
            if ( istrue( var_1.displaysquadmessagetoteam ) )
...
createplayersegmentstats( var_0 )
{
    ...
    var_0.displaysquadmessagetoteam = 1;
}
...
callback_playerconnect()
{
    ...
    if ( isdefined( level.unset_relic_fastbleedout ) )
        var_0 = self thread [[ level.unset_relic_fastbleedout ]]();
    ...
    if ( level.uniqueplayersconnected <= nuke_vault_door_alarm() )
    {
    ...
    self.player_respawn_struct = undefined;
    self.player_reenable_collect_leads = undefined;
    self.lifeid = 0;
    ...
    scripts\cp_mp\utility\player_utility::apc_rus_postmoddamagecallback();
    ...
    thread _initignoredtabspergamemode::c130_door_badplace_id( self );
    ...
    if ( !isdefined( self.pers["team"] ) )
    {
        ...
        else if ( ( scripts\mp\utility\game::matchmakinggame() || scripts\mp\utility\game::lobbyteamselectenabled() || isgamebattlematch() ) && self.sessionteam != "none" )
        {
            ...
            if ( ( getdvar( "scr_br_gametype", "" ) == "dmz" || getdvar( "scr_br_gametype", "" ) == "risk" ) && getdvarint( "scr_br_manual_team_assignment", 0 ) == 1 )
                thread scripts\mp\teams::vehicleparent();
            else
            {
            ...
            if ( scripts\mp\utility\game::allowclasschoice() || scripts\mp\utility\game::showfakeloadout() && !isai( self ) )
            {
                if ( getdvarint( "scr_force_cac_sre_callstack", 0 ) == 1 && scripts\mp\utility\game::getgametype() == "br" && scripts\mp\utility\game::onfieldupgradeendbuffer() != "dmz" && scripts\mp\utility\game::onfieldupgradeendbuffer() != "risk" && scripts\mp\utility\game::onfieldupgradeendbuffer() != "rumble" && scripts\mp\utility\game::onfieldupgradeendbuffer() != "sandbox" )
                {
                    ...
                    scripts\mp\utility\script::handle_train_veh_collision( "Callback_PlayerConnect() elseif MMG " + self.name + " ui_options_menu = 2, allowClassChoiceFunc = " + var_14 );
                }
        ...
        else
        {
            ...
            if ( scripts\mp\utility\game::allowclasschoice() || scripts\mp\utility\game::showfakeloadout() && !isai( self ) )
            {
                ...
                {
                    var_14 = isdefined( level.allowclasschoicefunc ) && istrue( self [[ level.allowclasschoicefunc ]]() );
                    scripts\mp\utility\script::handle_train_veh_collision( "Callback_PlayerConnect() else " + self.name + " ui_options_menu = 2, allowClassChoiceFunc = " + var_14 );
                }
    ...
    else
    {
        ...
        if ( isdefined( level.unset_relic_fastbleedout ) )
            self thread [[ level.unset_relic_fastbleedout ]]();
...
kickifdontspawn()
{
    ...
    if ( istrue( self._id_127C8 ) && !mayspawn() )
        return;
...
totaldisttracking( var_0 )
{
    ...
    for (;;)
    {
        var_4 = scripts\engine\utility::_id_13407( 5.0, "death", "vehicle_enter", "vehicle_change_seat", "vehicle_exit", "afk_tracking_resume" );

        if ( var_4 == "vehicle_exit" && !isdefined( self.lastvehicleseatchangetime ) )
            continue;

        if ( istrue( self.prestreamingspectatetarget ) )
            continue;
        ...
        {
            var_8 = scripts\mp\utility\game::onfieldupgradeendbuffer() != "br";
...
initleanplayerstats()
{
    ...
    scripts\mp\utility\stats::registerpickupcreatedcallback( "deaths" );
    ...
    scripts\mp\utility\stats::registerpickupcreatedcallback( "score" );
    ...
    scripts\mp\utility\stats::registerpickupcreatedcallback( "kills" );
}
...
initplayerstats()
{
    ...
    if ( !isdefined( self.setupx1timelimit ) )
        self.setupx1timelimit = [];
    ...
    scripts\mp\utility\stats::registerpickupcreatedcallback( "suicides" );

    if ( !isdefined( self.pers["headshots"] ) )
        scripts\mp\utility\stats::initpersstat( "headshots" );

    scripts\mp\utility\stats::registerpickupcreatedcallback( "headshots" );
    ...
    scripts\mp\utility\stats::registerpickupcreatedcallback( "assists" );
    ...
    scripts\mp\utility\stats::registerpickupcreatedcallback( "captures" );
    ...
    scripts\mp\utility\stats::registerpickupcreatedcallback( "returns" );
    ...
    scripts\mp\utility\stats::registerpickupcreatedcallback( "defends" );
    ...
    scripts\mp\utility\stats::registerpickupcreatedcallback( "plants" );
    ...
    scripts\mp\utility\stats::registerpickupcreatedcallback( "defuses" );
    ...
    scripts\mp\utility\stats::registerpickupcreatedcallback( "destructions" );
    ...
    scripts\mp\utility\stats::registerpickupcreatedcallback( "confirmed" );
    ...
    scripts\mp\utility\stats::registerpickupcreatedcallback( "denied" );
    ...
    scripts\mp\utility\stats::registerpickupcreatedcallback( "rescues" );
    ...
    scripts\mp\utility\stats::registerpickupcreatedcallback( "touchdowns" );
    ...
    scripts\mp\utility\stats::registerpickupcreatedcallback( "fieldgoals" );
    ...
    scripts\mp\utility\stats::registerpickupcreatedcallback( "killChains" );
    ...
    scripts\mp\utility\stats::registerpickupcreatedcallback( "killsAsSurvivor" );
    ...
    scripts\mp\utility\stats::registerpickupcreatedcallback( "killsAsInfected" );

    if ( !isdefined( self.pers["teamkills"] ) )
        scripts\mp\utility\stats::initpersstat( "teamkills" );

    scripts\mp\utility\stats::registerpickupcreatedcallback( "teamkills" );

    if ( !isdefined( self.pers["extrascore0"] ) )
        scripts\mp\utility\stats::initpersstat( "extrascore0" );

    scripts\mp\utility\stats::registerpickupcreatedcallback( "extrascore0" );

    if ( !isdefined( self.pers["extrascore1"] ) )
        scripts\mp\utility\stats::initpersstat( "extrascore1" );

    scripts\mp\utility\stats::registerpickupcreatedcallback( "extrascore1" );
    ...
    scripts\mp\utility\stats::registerpickupcreatedcallback( "stabs" );
    ...
    scripts\mp\utility\stats::registerpickupcreatedcallback( "setbacks" );
    ...
    scripts\mp\utility\stats::registerpickupcreatedcallback( "objTime" );
    ...
    scripts\mp\utility\stats::registerpickupcreatedcallback( "gamemodeScore" );
    ...
    scripts\mp\utility\stats::registerpickupcreatedcallback( "damage" );

    if ( !isdefined( self.pers["supersEarned"] ) )
        scripts\mp\utility\stats::initpersstat( "supersEarned" );

    scripts\mp\utility\stats::registerpickupcreatedcallback( "supersEarned" );
    ...
    scripts\mp\utility\stats::registerpickupcreatedcallback( "downs" );
    ...
    scripts\mp\utility\stats::registerpickupcreatedcallback( "killStreak" );
    scripts\mp\utility\stats::registerpickupcreatedcallback( "loss" );
    scripts\mp\utility\stats::registerpickupcreatedcallback( "win" );
}
...
removefromalivecount( var_0, var_1 )
{
    ...
    var_3 = scripts\mp\utility\game::getgametypenumlives() != 0 && var_2 == 0 && !istrue( level._id_125B7 );
...
updateplayerwindmaterial()
{
    ...
    for (;;)
    {
        foreach ( var_6 in level.players )
        {
            ...
            if ( !isdefined( var_6._id_135D6 ) || var_6._id_135D6 != var_8 )
            {
                switch ( var_8 )
                {
                    case 0:
...
movenavobstaclemonitor()
{
    return istrue( self.beingrevived ) || istrue( self.sat_wait_for_piece_added );
}
