...
isinvehicle( var_0 )
{
    ...
    if ( istrue( var_0 ) )
    {
        if ( isdefined( self._id_132C5 ) && isdefined( self._id_132C5.vehiclename ) )
            return 1;
    }
...
watchthermalinputchange()
{
    ...
    for (;;)
    {
        ...
        var_1 = scripts\engine\utility::_id_133FE( "input_type_changed", "thermal_handling_ended" );
...
apc_rus_postmoddamagecallback()
{
    if ( !self isdismembermentenabledforplayer() )
        level._id_119EE[self getxuid()] = self;
}

_id_11EC6()
{
    level._id_119EE[self getxuid()] = undefined;
}

nag_radius()
{
    return level._id_119EE;
}

addspawnlocation()
{
    if ( !isdefined( self.x1playertransition ) )
        self.x1playertransition = 0;

    if ( self.x1playertransition == 0 )
        self playerhide();

    self.x1playertransition++;
}

addspecialistbonus()
{
    if ( self.x1playertransition == 1 )
        self playershow();

    self.x1playertransition--;

    if ( self.x1playertransition == 0 )
        self.x1playertransition = undefined;
}
...
x1spyplane()
{
    ...
    if ( !isdefined( level._id_119C2 ) )
        level._id_119C2 = getdvarint( "scr_playerShowOnReset", 0 );
