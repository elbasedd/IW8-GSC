...
initparachutedvars()
{
    ...
    level.validateprojectileent = getdvarint( "scr_parachute_autodeploy_cut", 1 );
    level.validautoassignquests = getdvarint( "scr_parachute_cut", 1 );
    ...
    if ( !isdefined( level.van ) )
        level.van = ::van_blocker_moves;

    level._id_125D0 = spawnstruct();
    level._id_125D0._id_12A74 = getdvarint( "scr_parachute_hint_enabled", 1 );
    level._id_125D0._id_13687 = getdvarfloat( "scr_parachute_hint_xyvelscale_high", 0.0 );
    level._id_125D0._id_13688 = getdvarfloat( "scr_parachute_hint_xyvelscale_low", 0.0 );
    level._id_125D0._id_13689 = getdvarfloat( "scr_parachute_hint_xyvelscale_maxheight", 0.0 );
    level._id_125D0._id_13686 = getdvarfloat( "scr_parachute_hint_xylimit", 0.0 );
    level._id_125D0._id_1369B = getdvarfloat( "scr_parachute_hint_zdrop", 0.0 );
    level._id_125D0.zoffset = getdvarfloat( "scr_parachute_hint_zoffset", 0.0 );
    level._id_125D0._id_13735 = getdvarfloat( "scr_parachute_hint_zvelscale", 0.0 );
    level._id_125D0._id_1369C = getdvarfloat( "scr_parachute_hint_zlimit", 0.0 );
    level._id_125D0.killstreaks = getdvarfloat( "scr_parachute_hint_falling_xyratio", 0.0 );
}
...
startfreefall( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    ...
    {
        thread little_bird_mg_mp_waitandspawn( var_0, var_1, var_2, var_3, var_4, var_5 );
        return;
    }
    ...
    if ( level.gametype == "br" && ( !istrue( var_4 ) || ( getdvar( "scr_br_gametype", "" ) == "dmz" || getdvar( "scr_br_gametype", "" ) == "risk" ) ) && getdvarint( "scr_infil_parachute_vfx", 1 ) )
        thread propindex( getdvarint( "scr_bmo_parachuteTouchdownVFX", 1 ) == 1 && ( getdvar( "scr_br_gametype", "" ) == "dmz" || getdvar( "scr_br_gametype", "" ) == "risk" ) );
...
propindex( var_0 )
{
    ...
    if ( isdefined( self.operatorcustomization ) && isdefined( self.operatorcustomization.computermakingnose ) )
        var_2 = var_2 + self.operatorcustomization.computermakingnose;
...
{
    if ( isdefined( level.delete_race ) )
        return level.delete_race;
...
savedexecutionref()
{
    return isdefined( self.laser_trap_combat ) && ( self.laser_trap_combat == 5 || self.laser_trap_combat == 6 );
}
...
startparachute()
{
    ...
    if ( isparachutegametype() )
    {
        if ( isdefined( level.unload_all_vehicles_in_module ) )
            self [[ level.unload_all_vehicles_in_module ]]( self );
    }
}
...
van_blocker_moves()
{
    ...
    {
        if ( isdefined( var_0.laser_trap_combat ) && ( var_0.laser_trap_combat == 1 || var_0.laser_trap_combat == 2 ) )
        {
            var_0 laser_shut_down_button();
            var_0.laser_trap_combat = 3;
        }
    }
    else if ( level.gametype == "cp_survival" )
    {
        var_0 setclientomnvar( "ui_br_altimeter_state", 0 );
        var_0 laser_shut_down_button( 1 );
    }
}
...
_id_130A1()
{
    ...
    var_2 = clamp( var_0.origin[2], 0, level._id_125D0._id_13689 ) / level._id_125D0._id_13689;
    var_3 = level._id_125D0._id_13688 + ( level._id_125D0._id_13687 - level._id_125D0._id_13688 ) * var_2;
    var_4 = var_1 * ( 1, 1, 0 );
    var_5 = length( var_4 );
    var_6 = var_4 * var_3;
    var_7 = var_5 * var_3;

    if ( var_7 > level._id_125D0._id_13686 )
        var_6 = var_6 * ( level._id_125D0._id_13686 / var_7 );

    var_8 = 0;
    var_8 = var_8 - var_0.origin[2] * level._id_125D0._id_1369B;
    var_8 = var_8 + level._id_125D0.zoffset;
    var_8 = var_8 + var_1[2] * level._id_125D0._id_13735;
    var_8 = clamp( var_8, -1 * level._id_125D0._id_1369C, 0 );
    var_9 = 1;

    if ( var_5 < level._id_125D0.killstreaks * var_1[2] )
        var_9 = 2;

    var_0 setadditionalstreampos( var_0.origin + ( var_6[0], var_6[1], var_8 ), 0, var_9 );
}

little_bird_mg_mp_waitandspawn( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    self endon( "disconnect" );
    level endon( "game_ended" );
    ...
    var_6 = self;
    var_6.laser_trap_combat = 1;
    var_6.laser_show_position = istrue( var_4 );
    var_6.laser_shut_down_interact_monitor = 0;
    var_6 laser_reset_position( var_0, var_1, var_2, var_3, var_5 );
    var_6 laser_switch_manager();
    var_7 = gettime();

    for (;;)
    {
        if ( var_6 isskydiving() || var_7 + 2000 < gettime() || var_6 savedexecutionref() )
            break;
    ...
    for (;;)
    {
        if ( level._id_125D0._id_12A74 && var_6.laser_shut_down_interact_monitor < gettime() )
        {
            var_6 _id_130A1();
            var_6.laser_shut_down_interact_monitor = gettime() + 500;
        }

        if ( var_6 isinfreefall() && var_6.laser_trap_combat != 1 )
        {
            var_6 laser_switch_manager();
            var_6.laser_trap_combat = 1;
        }

        if ( var_6 isparachuting() && var_6.laser_trap_combat != 2 )
        {
            var_6 laser_start_ent_thermal();
            var_6.laser_trap_combat = 2;
        }

        if ( !var_6 isskydiving() && var_6.laser_trap_combat != 3 && var_6.laser_trap_combat != 4 && var_6.laser_trap_combat != 6 || var_6.laser_trap_combat == 5 )
        {
            var_6 laser_shut_down_button();

            if ( var_6.laser_trap_combat != 5 )
                var_6.laser_trap_combat = 3;
            else
                var_6.laser_trap_combat = 6;
        }

        var_8 = istrue( var_6.inlaststand );
        var_9 = var_6 isonground() && ( var_6.laser_trap_combat == 3 || var_6 savedexecutionref() );
        ...
        {
            var_6 laser_sights();
            var_6.laser_trap_combat = undefined;
            var_6.laser_show_position = undefined;
            var_6.laser_shut_down_interact_monitor = undefined;
...
laser_reset_position( var_0, var_1, var_2, var_3, var_4 )
{
    self skydive_interrupt();
    ...
    if ( level.gametype == "br" && ( !istrue( self.laser_show_position ) || getdvar( "scr_br_gametype", "" ) == "dmz" ) || getdvar( "scr_br_gametype", "" ) == "risk" )
        thread propindex( getdvarint( "scr_bmo_parachuteTouchdownVFX", 1 ) == 1 && ( getdvar( "scr_br_gametype", "" ) == "dmz" || getdvar( "scr_br_gametype", "" ) == "risk" ) );
...
laser_switch_manager()
{
    ...
    {
        self setclientomnvar( "ui_br_altimeter_state", 1 );
        level.allowmodestructs[self getxuid()] = self;
    }

    if ( !self.laser_show_position )
    {
        var_0 = "enabled";

        if ( isdefined( self.operatorcustomization ) && isdefined( self.operatorcustomization.computermakingnose ) )
            var_0 = var_0 + self.operatorcustomization.computermakingnose;
    }
}
...
laser_start_ent_thermal()
{
    ...
    if ( isdefined( self.select_patrol_eight_spawners ) && self.select_patrol_eight_spawners == 1 && var_0 != "truckwar" && var_0 != "reveal" && var_0 != "brdov" )
    {
        ...
        self.select_patrol_eight_spawners = 0;
    }
    ...
    if ( isdefined( level.unset_relic_bang_and_boom ) )
        self [[ level.unset_relic_bang_and_boom ]]();
}

laser_shut_down_button( var_0 )
{
    self.ignorefalldamagetime = gettime() + 5000;
    self.set_stealth_enabled = gettime();
    ...
    level.allowmodestructs[self getxuid()] = undefined;
    self notify( "parachute_complete" );
    self animscriptsetinputparamreplicationstatus( 0 );
}
