...
little_bird_init()
{
    level.setup_trap_consoles = getdvarfloat( "scr_little_bird_dmg_factor_fuselage", 1.0 );
    level.setup_wave_spawn_zone_disable = getdvarfloat( "scr_little_bird_dmg_factor_tail_stabilizer", 1.0 );
    level.setup_vehicle_wave_by_player_count = getdvarfloat( "scr_little_bird_dmg_factor_main_rotor", 1.2 );
    level.setup_volumes = getdvarfloat( "scr_little_bird_dmg_factor_tail_rotor", 1.0 );
    level.setup_tut_zones = getdvarfloat( "scr_little_bird_dmg_factor_landing_gear", 0.5 );
    level.setup_train_entarray_composite = getdvarfloat( "scr_little_bird_dmg_factor_driverless_collision", 3.0 );
    level.setupbrsquadleader = getdvarfloat( "scr_little_bird_impulse_dmg_threshold_high", 0.9 );
    level.setupbobbingboatmultiple = getdvarfloat( "scr_little_bird_impulse_dmg_threshold_mid", 0.9 );
    level.setupboardroomcode = getdvarfloat( "scr_little_bird_impulse_dmg_threshold_low", 0.1 );
    level.setup_weapon_spawns = getdvarfloat( "scr_little_bird_impulse_dmg_factor_low", 0.1 );
    level.setupblueprintpickupweapons = getdvarfloat( "scr_little_bird_impulse_dmg_factor_mid_low", 0.2 );
    level.setup_weapons_at_pos = getdvarfloat( "scr_little_bird_impulse_dmg_factor_mid_high", 0.75 );
    level.setuphqs = getdvarfloat( "scr_little_bird_dmg_pitch_roll_threshold", 55.0 );
    level.setuphelitimer = getdvarfloat( "scr_little_bird_dmg_pitch_roll_factor", 10.0 );
    level.setuphudelemninfilblack = getdvarfloat( "scr_little_bird_wood_surf_dmg_scalar", 0.6 );
...
little_bird_initinteract()
{
    ...
    scripts\cp_mp\vehicles\vehicle_interact::_id_1320F( "little_bird", "single", [ "pilot", "fl_platform", "fr_platform", "bl_platform", "br_platform" ] );
}

silo_door_runners()
{
    var_0 = scripts\cp_mp\utility\vehicle_omnvar_utility::_id_132E1( "little_bird", 1 );
...
silo_door_right()
{
    ...
    scripts\cp_mp\vehicles\vehicle_damage::_id_131DF( "little_bird", var_0 );
    var_1 = scripts\cp_mp\vehicles\vehicle_damage::vehicle_damage_getleveldataforvehicle( "little_bird" );
    var_1.class = "medium_heavy";
    var_2 = scripts\cp_mp\vehicles\vehicle_damage::_id_131C2( "little_bird", "heavy" );
    var_2.unfreeze_controls_after_spawn = ::skipweapondropondeath;
    var_2.uniquelootitemid = ::skydivehintnotify;
    scripts\cp_mp\vehicles\vehicle_damage::_id_131B3( "little_bird" );
    scripts\cp_mp\vehicles\vehicle_damage::_id_131EB( "little_bird", 12 );
    scripts\cp_mp\vehicles\vehicle_damage::_id_131E4( "little_bird", ::little_bird_deathcallback );
    scripts\cp_mp\vehicles\vehicle_damage::_id_131EE( "little_bird_mp", 5 );
}

silo_door_left()
{
    var_0 = _applydvarstosettings::_id_13185( "little_bird", 1 );
    var_0.bridge_one_death_func = 1.5;
    var_0.givebmodevloadouts = 0.875;
    var_0.getcombatrecordsupermisc = 350;
    var_0.getcoremapdropzones = 525;
    var_0.getcodecomputerdisplaycode = 875;
    var_0.getcrossbowammotype = 8.75;
    var_0.getcrossbowimpactfunc = 35;
    var_0.getcrateusetime = 70;
...
little_bird_create( var_0, var_1 )
{
    ...
    scripts\cp_mp\vehicles\vehicle::_id_131AE( var_2, "little_bird", var_0 );
...
fob_think()
{
    ...
    for (;;)
    {
        ...
        {
            case 0:
                var_9 = level.setup_trap_consoles;
                break;
            case 1:
                var_9 = level.setup_wave_spawn_zone_disable;
                break;
            case 2:
                var_9 = level.setup_vehicle_wave_by_player_count;
                break;
            case 3:
                var_9 = level.setup_volumes;
                break;
            case 4:
                var_9 = level.setup_tut_zones;
                break;
        }
        ...
        if ( !isdefined( var_11 ) )
            var_10 = var_10 * level.setup_train_entarray_composite;
        ...
        if ( abs( var_12 > level.setuphqs ) )
            var_10 = var_10 * level.setuphelitimer;
        ...
        if ( abs( var_13 > level.setuphqs ) )
            var_10 = var_10 * level.setuphelitimer;

        var_14 = 0;

        if ( var_10 > level.setupbrsquadleader )
            var_14 = self.maxhealth;
        else if ( var_10 > level.setupbobbingboatmultiple )
        {
            var_15 = level.setupbrsquadleader - level.setupbobbingboatmultiple;
            var_16 = ( var_10 - level.setupbobbingboatmultiple ) / var_15;
            var_17 = self.maxhealth * level.setupblueprintpickupweapons;
            var_18 = self.maxhealth * level.setup_weapons_at_pos;
            var_14 = scripts\engine\math::lerp( var_17, var_18, var_16 );
        }
        else if ( var_10 > level.setupboardroomcode )
            var_14 = self.maxhealth * level.setup_weapon_spawns;

        if ( var_14 > 0 )
        {
            if ( isdefined( var_11 ) && var_3 == 11534336 )
                var_14 = var_14 * level.setuphudelemninfilblack;
...
skipweapondropondeath( var_0, var_1 )
{
...
_id_13206( var_0 )
{
    ...
    scripts\cp_mp\utility\vehicle_omnvar_utility::vehomn_setammo( "little_bird", "flares", scripts\engine\utility::ter_op( self.lbravo_spawn_after_level_restart, 1, 0 ), var_0 );
