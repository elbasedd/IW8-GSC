...
init()
{
    ...
    if ( !isdefined( level.explosive_barrels ) )
        level.explosive_barrels = ::choppersupport_movetolocation;
    ...
    level.heli_structs_entrances = scripts\cp_mp\utility\game_utility::nuke_killplayer( "ks_heli_entrance" );
    level.heli_structs_goals = scripts\cp_mp\utility\game_utility::nuke_killplayer( "ks_heli_goal" );
    level.heli_structs_paths = scripts\cp_mp\utility\game_utility::nuke_killplayer( "ks_heli_path" );
...
spawnchopper( var_0, var_1 )
{
    ...
    var_22.gates_combat = var_2[2] - 750;
    var_22._id_128DC = 50;
    var_22._id_128DD = 25;
    var_22._id_128DE = undefined;
    var_22.stringtovec3 = 7;
    var_22.strip_node_flag_wait = 12;
...
choppersupport_monitorowner()
{
    ...
    self.owner scripts\engine\utility::_id_133EF( "joined_team", "disconnect" );
    thread choppersupport_leave();
}
...
choppersupport_engageturrettarget( var_0 )
{
    ...
    for (;;)
    {
        if ( !istrue( var_0.turreton ) || istrue( var_0._id_12F45 ) )
        {
            waitframe();
            continue;
        }
        ...
        if ( isdefined( var_1 ) && var_1.size > 0 )
        {
            ...
            {
                if ( istrue( self.brtruck_ontimelimit ) && isdefined( self.get_vehicle_riders ) )
                    self thread [[ self.get_vehicle_riders ]]();
                else if ( isdefined( self.vehicle_compass_friendlystatuschangedcallback ) )
                    self thread [[ self.vehicle_compass_friendlystatuschangedcallback ]]();
                else
...
choppersupport_acquireturrettarget( var_0, var_1 )
{
    ...
    if ( isdefined( var_4 ) )
    {
        ...
        if ( istrue( var_5 ) && self.currenttarget == var_4 && !istrue( self.brtruck_ontimelimit ) )
            self thread [[ level.explosive_barrels ]]( var_4, 1 );
...
choppersupport_clearcurrenttarget( var_0 )
{
    ...
    if ( istrue( var_0._id_12BA5 ) )
        var_0._id_12BA5 = undefined;
...
choppersupport_fireonturrettarget( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    thread explosionspots( var_0 );
    thread choppersupport_watchtargetlos( var_0, var_2 );
    thread choppersupport_watchtargettimeout( var_0 );
    thread exploder_ref( var_0 );
    var_5 = weaponfiretime( "chopper_support_turret_mp" );
    var_6 = 0;
    var_7 = 100;

    if ( isdefined( self._id_128DC ) )
        var_7 = self._id_128DC;

    var_8 = 20;

    if ( isdefined( self.stringtovec3 ) )
        var_8 = self.stringtovec3;

    var_9 = 40;

    if ( isdefined( self.strip_node_flag_wait ) )
        var_9 = self.strip_node_flag_wait;

    if ( istrue( var_4 ) )
    {
        var_10 = 3750;

        while ( istrue( expiredlootleadermarks( var_0, var_1 ) ) )
        {
    ...
    thread explosive_barrel( var_0 );

    while ( istrue( expiredlootleadermarks( var_0, var_1 ) ) )
    {
        if ( istrue( var_0._id_12BA5 ) )
        {
            if ( isdefined( self.currenttarget ) && var_1 == self.currenttarget && !istrue( self.brtruck_ontimelimit ) )
                self [[ level.explosive_barrels ]]( self.currenttarget, 1 );
            else
        ...
        else if ( var_0 choppersupport_turretlookingattarget() )
        {
            ...
            if ( var_6 == var_8 )
            {
                ...
                if ( isdefined( self._id_128DD ) )
                    var_7 = self._id_128DD;
            }
            else if ( var_6 == var_9 )
            {
                var_7 = undefined;

                if ( isdefined( self._id_128DE ) )
                    var_7 = self._id_128DE;
            }
...
expiredlootleadermarks( var_0, var_1 )
{
...
explosionspots( var_0 )
{
    ...
    if ( !scripts\cp_mp\utility\game_utility::rpg_attack_apc() )
        return;
    ...
    for (;;)
    {
        if ( isdefined( var_0.attackingtarget ) )
        {
            ...
            {
                if ( !istrue( self.playerzombielaststandrevive ) )
                    self.playerzombielaststandrevive = 1;
...
choppersupport_watchtargetlos( var_0, var_1 )
{
    ...
    for (;;)
    {
        if ( !istrue( var_0._id_12BA5 ) && isdefined( var_0.attackingtarget ) )
        {
...
choppersupport_patrolfield( var_0 )
{
    ...
    if ( isdefined( self.vehicle_compass_friendlystatuschangedcallback ) )
        self [[ self.vehicle_compass_friendlystatuschangedcallback ]]( var_0 );
    else
    {
        if ( self.currentaction != "patrol" )
        ...
        for (;;)
        {
            ...
            if ( isdefined( var_3 ) && !istrue( self.brtruck_ontimelimit ) )
                self [[ level.explosive_barrels ]]( var_3, 1 );
            else
            {
                var_5 = [];
                var_6 = ( 0, 0, 0 );
                ...
                if ( isdefined( var_6 ) && var_5.size > 0 && !istrue( self.brtruck_ontimelimit ) )
                {
                    var_11 = var_6 / var_5.size;
                    self [[ level.explosive_barrels ]]( var_11 );
                }
            }
...
circleorigin( var_0, var_1 )
{
    ...
    for (;;)
    {
        ...
        if ( istrue( self.brtruck_ontimelimit ) && isdefined( self.get_vehicle_node_to_wait_for_lbravo_based_on_group ) )
            break;
        else
        {
            var_7 = 20;
...
choppersupport_cleanup( var_0 )
{
    ...
    if ( isdefined( self.failstringsetup ) )
        self [[ self.failstringsetup ]]();

    if ( isdefined( self.streakinfo ) )
        self.streakinfo.isverdansksubmap = istrue( var_0 );
    ...
    for (;;)
    {
        if ( istrue( self.playerzombielaststandrevive ) )
        {
            ...
            for (;;)
            {
                ...
                {
                    self.playerzombielaststandrevive = undefined;
...
choppersupport_gettargets( var_0, var_1, var_2, var_3 )
{
    ...
    for ( var_7 = 0; var_7 < var_5.size; var_7++ )
    {
        ...
        if ( istrue( var_10 ) )
        {
            var_13 = spawnstruct();
            var_13.player = var_8;
            var_13._id_121E2 = var_11;
            var_13._id_12BBF = var_12;
...
choppersupport_istarget( var_0, var_1, var_2, var_3 )
{
    ...
    if ( isdefined( self.school_guards_behavior_internal ) && ![[ self.school_guards_behavior_internal ]]( var_1 ) )
        return [ 0, 0, 0 ];

    if ( !expiredlootleaders( var_1 ) )
        return [ 0, 0, 0 ];
...
choppersupport_getbesttarget( var_0, var_1 )
{
    ...
    {
        if ( !expiredlootleaders( var_7.player ) )
            continue;

        if ( choppersupport_isactivetarget( var_7.player ) && !istrue( var_7._id_12BBF ) )
            continue;
        ...
        if ( istrue( var_7._id_121E2 ) )
        {
            var_8 = 1;
            var_10 = var_10 + 40;
        }

        if ( istrue( var_7._id_12BBF ) )
        {
            var_9 = 1;
            var_10 = var_10 + 20;
        }
...
choppersupport_explode()
{
    ...
    if ( isdefined( self.get_vehicle_ai_spawner ) )
        [[ self.get_vehicle_ai_spawner ]]();
...
choppersupport_crash( var_0 )
{
    ...
    if ( istrue( self.brtruck_ontimelimit ) && isdefined( self.get_turret_target_pos ) )
    {
        self setneargoalnotifydist( 300 );
        var_1 = 1;
        var_2 = [[ self.get_turret_target_pos ]]();
    }
    else
...
beginevasivemaneuvers()
{
    ...
    var_0 = scripts\engine\utility::_id_13403( 3.0, "death" );
...
choppersupport_handledeathdamage( var_0 )
{
    ...
    if ( istrue( self.saw_3_angles ) && isdefined( level._id_12618 ) )
    {
        if ( isdefined( var_0.objweapon ) )
            [[ level._id_12618._id_1262B ]]( var_0.objweapon.basename );
        else
            [[ level._id_12618._id_1262B ]]( "none" );
    }
