...
init()
{
    ...
    level._id_12F9A = getuavstrengthmin();
    level._id_12F95 = getuavstrengthmax();
    level._id_12F96 = getuavstrengthlevelshowenemydirectional();
    level._id_12F98 = getuavstrengthlevelneutral();
    level._id_12F97 = getuavstrengthlevelshowenemyfastsweep();

    if ( !isdefined( level._id_12F99 ) && scripts\cp_mp\utility\script_utility::issharedfuncdefined( "game", "squadAsTeamEnabled" ) )
        level._id_12F99 = level [[ scripts\cp_mp\utility\script_utility::getsharedfunc( "game", "squadAsTeamEnabled" ) ]]() && getdvarint( "scr_uav_for_squad_only", 1 );

    if ( level.teambased && !istrue( level._id_12F99 ) )
    {
    ...
    level.allowassassinationdamage = 0;
...
onplayerconnect()
{
    ...
    var_1 = level.teambased && istrue( level._id_12F99 );
    ...
    for (;;)
    {
        ...
        if ( var_1 )
        {
            var_2 thread unsetchainkillstreaks();
            continue;
        }
...
useuav( var_0, var_1 )
{
    ...
    scripts\cp\helicopter\chopper_boss::_id_11EFA( "t9_ch_global_call_in_uav_for_operator_mission", 1 );
    scripts\cp\helicopter\chopper_boss::_id_11EFA( "t9_ch_global_call_in_uav_for_operator_mission_op2", 1 );

    if ( getdvarint( "LRTSSKLKPK", 1 ) >= 8 )
        scripts\cp\helicopter\chopper_boss::_id_11EFA( "t9_ch_global_call_in_uav_for_operator_mission_op3", 1 );
        ...
        case "directional_uav":
            ...
            if ( level.teambased )
            {
                foreach ( var_7 in level.players )
                {
                    ...
                    {
                        if ( istrue( level._id_12F99 ) && var_7.squadindex != var_3 )
                            continue;
...
launchuav( var_0, var_1, var_2, var_3 )
{
    ...
    if ( level.gametype == "br" )
        var_0 _id_12F91( var_4, 15000, var_1 );
    ...
    if ( isdefined( var_7 ) && var_7.damagetaken < var_7.maxhealth )
    {
        ...
        {
            var_7 unlink();
            var_7.handlerelicmartyrdom_common = var_7.origin + anglestoforward( var_7.angles ) * 50000;
            var_7 moveto( var_7.handlerelicmartyrdom_common, 50 );
        ...
        if ( isdefined( var_7 ) && var_7.damagetaken < var_7.maxhealth )
        {
            ...
            if ( isent( var_7 ) )
                var_7 moveto( var_7.handlerelicmartyrdom_common, 15 );
    ...
    if ( isdefined( var_7 ) )
    {
        ...
        var_7.streakinfo.isverdansksubmap = !istrue( var_7.leftplayspace );

        if ( !istrue( self._id_11D83 ) )
            var_7.owner scripts\cp_mp\utility\killstreak_utility::_id_11D86( var_7.streakinfo );
    ...
    if ( var_1 == "directional_uav" )
    {
        ...
        if ( level.teambased )
        {
            foreach ( var_34 in level.players )
            {
                ...
                {
                    if ( istrue( level._id_12F99 ) && var_34.squadindex != var_5 )
                        continue;
...
{
    self endon( "death" );
    self.owner scripts\engine\utility::_id_133EF( "disconnect", "joined_team" );
...
restorestrengthafterhostmigration()
{
    ...
    for (;;)
    {
        ...
        if ( level.teambased )
        {
            ...
            {
                if ( istrue( level._id_12F99 ) )
                {
                    foreach ( var_6, var_3 in level.squaddata[var_1] )
                    {
                        var_4 = var_1 + var_6;
                        var_5 = add_to_emp_drone_target_list( var_4 );
                        adjustmatchtimerpausedstatefromscore( var_1, var_6, var_5 );
                    }

                    continue;
                }

                var_5 = add_to_ents_to_clean_up( var_1 );
...
adjustmatchtimerpausedstatefromscore( var_0, var_1, var_2 )
{
    _id_130BB( var_0, var_1, var_2 );
}
...
updateteamuavstatus( var_0, var_1 )
{
    if ( isdefined( var_1 ) )
        ...
        var_2 = add_to_ents_to_clean_up( var_0 );

    _id_13084( var_2, var_0 );
}
...
_id_130BB( var_0, var_1, var_2 )
{
    ...
    {
        var_4 = var_0 + var_1;
        var_3 = add_to_emp_drone_target_list( var_4 );
    }

    _id_13084( var_3, var_0, var_1 );
}

_id_13084( var_0, var_1, var_2 )
{
    var_0 = int( max( min( var_0, level._id_12F95 ), level._id_12F9A ) );
    var_3 = var_0 == level._id_12F9A;
    var_4 = !var_3;
    var_5 = var_0 >= level._id_12F96;
    var_6 = !var_3;

    if ( var_0 == level._id_12F98 )
    {
        var_7 = "normal_radar";
        var_4 = 0;
    }
    else if ( var_0 == level._id_12F95 || var_5 )
        var_7 = "constant_radar";
    else if ( var_0 == level._id_12F97 )
        var_7 = "fast_radar";
    else
        var_7 = "normal_radar";
    ...
    foreach ( var_10 in var_8 )
    {
        ...
        if ( istrue( var_10.relic_squadlink_remove_visionset ) )
            continue;

        if ( istrue( var_10._id_125C0 ) )
            continue;
...
updateplayersuavstatus()
{
    foreach ( var_1 in level.players )
    {
        if ( istrue( var_1.relic_squadlink_remove_visionset ) )
            continue;
        ...
        if ( var_3 > 0 )
            var_2 = level._id_12F9A;

        var_2 = int( max( min( var_2, level._id_12F95 ), level._id_12F9A ) );
        var_1.radarstrength = var_2;
        var_4 = var_1.team == "spectator" || var_1.team == "follower" || var_1.team == "free";

        if ( var_2 <= level._id_12F98 || var_4 )
        {
            var_1.hasradar = 0;
        ...
        if ( var_2 >= level._id_12F97 )
            var_1.radarmode = "fast_radar";
        else
            var_1.radarmode = "normal_radar";

        var_1.radarshowenemydirection = var_2 >= level._id_12F96;
...
addactiveuav()
{
    ...
    if ( level.teambased )
    {
        ...
        if ( self.uavtype == "directional_uav" || self.uavtype == "harp" )
        {
            ...
            level.allowassassinationdamage++;
            return;
        }
    }
    else
...
watchhighlightfadetime( var_0, var_1, var_2, var_3 )
{
    ...
    self endon( "disconnect" );
    level endon( "game_ended" );
    scripts\engine\utility::_id_13409( var_2, "leave" );
        ...
        if ( istrue( level._id_12F99 ) )
        {
            ...
            foreach ( var_8 in level.teamnamelist )
            {
                foreach ( var_12, var_10 in level.squaddata[var_8] )
                {
                    ...
                    var_6++;
                    adjustmatchtimerpausedstatefromscore( var_8, var_12, var_2 );
                }
            }

            level.allowassassinationdamage = var_6;
        }
        else
        {
            foreach ( var_15 in level.teamnamelist )
            {
                ...
                level.allowassassinationdamage = level.teamnamelist.size;
...
_id_12F91( var_0, var_1, var_2 )
{
    if ( !isdefined( var_2 ) )
        return;

    var_3 = _id_12F92( var_0, var_1 );
