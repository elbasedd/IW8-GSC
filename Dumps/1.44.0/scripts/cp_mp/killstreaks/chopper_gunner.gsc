...
startchoppergunnerintro( var_0, var_1, var_2 )
{
    ...
    else if ( istrue( var_0.propminigameupdates ) && level.script == "cp_so_aniyah" )
    {
        ...
        while ( level.try_add_targetmarkergroup.size == 0 )
            waitframe();

        var_20 = scripts\engine\utility::getclosest( var_19, level.try_add_targetmarkergroup );
    ...
    if ( istrue( var_0.propminigameupdates ) && level.script == "cp_so_aniyah" )
    ...
    var_24.propminigameupdates = istrue( var_0.propminigameupdates );
    var_24 setvehicleteam( var_24.team );
    var_24 thread expirationtimer( "disconnect", var_1 );
    var_24 thread expirationtimer( "joined_team", var_1 );
    var_24 thread expirationtimer( "joined_spectator", var_1 );
    var_27 = 2000;
    ...
    var_29 = var_24 exitvehicledelay( 4.25 );
...
choppergunner_startremotecontrol( var_0 )
{
    ...
    thread expiredbydeath( "disconnect" );
    thread expiredbydeath( "joined_team" );
    thread expiredbydeath( "joined_spectator" );
    thread expiredbydeath( "team_kill_punish" );
    thread choppergunner_watchkills();
    thread choppergunner_handledestroyed( var_0 );
    thread choppergunner_notifyonkillstreakover();

    if ( !self.propminigameupdates )
    {
...
choppergunner_updatetargetmarkergroups( var_0 )
{
    ...
    if ( istrue( level.helistoreplunder ) )
        return;
...
{
    level endon( "game_ended" );
    scripts\engine\utility::_id_133F0( "leaving", "death", "explode" );
...
choppergunner_watchendstrobefx( var_0 )
{
    self endon( "death" );
    level endon( "game_ended" );
    var_0 scripts\engine\utility::_id_133EF( "death", "leaving" );
    self delete();
}
...
choppergunner_handlemissilefire( var_0 )
{
    ...
    for (;;)
    {
        ...
        if ( self.propminigameupdates && self.missilesleft == 8 )
            childthread exitvehicle_oldturnrate();
        ...
        var_17 thread expected_card( self.owner, 2, 300, 300 );
...
choppergunner_watchturretfire( var_0 )
{
    ...
    for (;;)
    {
        ...
        self.streakinfo.shots_fired++;
        var_1 thread expected_card( self.owner, 0.5, 256, 200 );
    }
}
...
choppergunner_returnplayer( var_0, var_1 )
{
    ...
    if ( isdefined( var_2 ) )
    {
        if ( !istrue( var_1 ) )
        {
            ...
            level thread scripts\cp_mp\utility\killstreak_utility::_id_11F70( var_2 );
            var_2 setclientomnvar( "ui_apache_screens_state", 0 );
            var_2 painvisionon();
            var_2 scripts\cp_mp\utility\killstreak_utility::killstreak_restorenvgstate();
...
{
    thread scripts\cp_mp\emp_debuff::waittill_num_and_all_spawns_dead_and_time_and_player_reset( self.owner, 5 );
    choppergunner_updatetargetmarkergroups();
}
...
expected_card( var_0, var_1, var_2, var_3 )
{
    level endon( "game_ended" );
