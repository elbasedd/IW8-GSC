...
brolaride_endgame( var_0, var_1 )
{
    ...
    foreach ( var_8 in level.players )
    {
        if ( isdefined( var_8 ) )
        {
            ...
            var_8 _calloutmarkerping_handleluinotify_added::_id_1313E( "ui_br_olaride_points", 31, 1, var_1 );
        }
    }
...
kiosk_onpurchase( var_0, var_1 )
{
    if ( var_1 == "contribute_heroes" )
        var_0 kiosk_onherocontribution( getdvarint( "scr_br_hvv_kiosk_contribution_bundle", 200 ) );
    else if ( var_1 == "br_defusekit" )
        var_0 kiosk_onherocontribution( getdvarint( "scr_br_hvv_kiosk_contribution_item", 100 ) );
    else if ( var_1 == "contribute_villains" )
        var_0 kiosk_onvillaincontribution( getdvarint( "scr_br_hvv_kiosk_contribution_bundle", 200 ) );
    else if ( var_1 == "br_bomb" )
        var_0 kiosk_onvillaincontribution( getdvarint( "scr_br_hvv_kiosk_contribution_item", 100 ) );
...
kiosk_onherocontribution( var_0 )
{
    giveheropoints( var_0 );
...
kiosk_onvillaincontribution( var_0 )
{
    givevillainpoints( var_0 );
...
setuplavatriggers()
{
    ...
    var_1 = [];
    var_2 = getallvehiclesinstances();
    level.lavawatchedentities = [];

    foreach ( var_5, var_4 in var_0 )
    {
        var_4.touchingentities = [];
        thread watchlavatrigger( var_4 );
        thread watchentitycollisionwithlavatriggers( var_4, var_5 * 0.1 );

        if ( var_4.dmg >= 9999 && getdvarint( "scr_br_lava_triggers_affect_vehicles", 1 ) > 0 )
            thread watchvehiclecollisionwithlavatriggers( var_4, var_2, var_5 * 0.1 );
    }

    level waittill( "prematch_fade_done" );

    foreach ( var_7 in level.players )
        var_7 thread watchthrownentities();
}
...
watchvehiclecollisionwithlavatriggers( var_0, var_1, var_2 )
{
    level endon( "game_ended" );
    wait( var_2 );
...
watchentitycollisionwithlavatriggers( var_0, var_1 )
{
    level endon( "game_ended" );
    wait( var_1 );
    ...
    for (;;)
    {
        foreach ( var_3 in level.lavawatchedentities )
        {
            if ( isdefined( var_3 ) && var_3 istouching( var_0 ) )
            {
                if ( isdefined( var_3.deletefunc ) )
                {
                    ...
                    var_3 notify( "deleted_equipment" );
                    continue;
                }

                if ( isdefined( var_3.animname ) && var_3.animname == "slinger" )
                {
                    if ( isdefined( var_3.scenenode ) && var_3.scenenode istouching( var_0 ) && isdefined( var_3.laststandplayers ) )
                        var_3 notify( "slinger_destroyed" );
        ...
        wait 1;
        level.lavawatchedentities = scripts\engine\utility::array_removeundefined( level.lavawatchedentities );
    }
}
...
watchthrownentities()
{
    ...
    for (;;)
    {
        ...
        if ( isdefined( var_0 ) )
        {
            if ( isdefined( var_0.equipmentref ) && var_0.equipmentref == "equip_slinger" )
            {
                var_0 waittill( "missile_stuck" );
                waitframe();

                if ( isdefined( self.slingers ) && self.slingers.size > 0 )
                    var_0 = self.slingers[self.slingers.size - 1];
                else
                    var_0 = undefined;
            }

            if ( isdefined( var_0 ) && !scripts\engine\utility::array_contains( level.lavawatchedentities, var_0 ) )
                level.lavawatchedentities[level.lavawatchedentities.size] = var_0;
        }
    }
}
