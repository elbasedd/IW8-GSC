// IW8 GSC SOURCE
// Generated by https://github.com/xensik/gsc-tool

main()
{
    scripts\mp\maps\mp_broadcast2\mp_broadcast2_precache.gsc::main();
    scripts\mp\maps\mp_broadcast2\gen\mp_broadcast2_art.gsc::main();
    scripts\mp\maps\mp_broadcast2\mp_broadcast2_fx.gsc::main();
    scripts\mp\maps\mp_broadcast2\mp_broadcast2_lighting.gsc::main();
    scripts\mp\load::main();
    scripts\cp_mp\utility\game_utility::_id_12B18();
    level.outofboundstriggers = getentarray( "OutOfBounds", "targetname" );
    level.kill_border_triggers = getentarray( "kill_border_trigger", "targetname" );
    scripts\mp\compass::setupminimap( "compass_map_mp_broadcast2", "codcaster_compass_map_mp_broadcast2" );
    setdvar( "NKLMONNPNN", 512 );
    setdvar( "PKKMTTRQO", 8 );
    game["attackers"] = "allies";
    game["defenders"] = "axis";
    game["allies_outfit"] = "urban";
    game["axis_outfit"] = "woodland";
    level.music_style = "eastern_europe";
    thread scripts\mp\animation_suite::animationsuite();
    level.chopper_gunner_assignedtargetmarkers_onnewai = getnodesinradius( ( 12590.3, 17293.2, 330.066 ), 250, 0, 100 );
}
