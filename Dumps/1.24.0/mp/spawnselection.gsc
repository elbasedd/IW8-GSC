...
init()
{
    ...
    scripts\mp\utility\spawn_event_aggregator::registeronplayerspawncallback( ::headlesscustomizationops );
}
...
getspawnpoint()
{
    ...
    var_1 = undefined;
    self.start_coop_escort_protect_hvi = 0;
    ...
    if ( isdefined( self.selectedspawnarea ) )
    {
        if ( issubstr( self.selectedspawnarea, "squad" ) )
        {
            ...
            self.start_coop_escort_protect_hvi = 1;

            if ( isdefined( var_3.vehicle ) )
            {
                self.spawningintovehicle = 1;
                self.spawn_fulton_rope_mdl = var_3.vehicle.vehiclename;
            }
    ...
    if ( istrue( level.usesquadspawn ) && istrue( self.squadspawnconfirmed ) )
    {
        ...
        if ( isdefined( var_14 ) && isdefined( self.squadindex ) && self.team == var_14.team && self.squadindex == var_14.squadindex )
        {
            ...
            var_14 thread scripts\mp\utility\points::freight_lift_run( "squad_spawn" );
            thread scripts\mp\utility\points::freight_lift_run( "squad_spawn_self" );
            self.start_coop_escort_protect_hvi = 1;
        }
    }
    ...
    for (;;)
    {
        ...
        foreach ( var_3 in level.availablespawnlocations[self.team] )
        {
            ...
            if ( isdefined( var_6.objectivekey ) )
                var_8 = istrue( var_6.getcpscriptedhelidropheight );
            else if ( issubstr( var_3, "vehicle" ) )
            {
                ...
                {
                    var_8 = istrue( var_11.getcpscriptedhelidropheight );
                    var_9 = get_closest_available_player_new( var_11, self );
...
{
    return level.objectives[var_0].ownerteam != var_1 || level.objectives[var_0].stalemate || level.objectives[var_0].captureblocked || istrue( level.objectives[var_0].relic_ammo_drain_take_ammo ) || level.objectives[var_0].claimteam != "none" && level.objectives[var_0].claimteam != var_1;
}
...
{
    var_0 endon( "death" );
    var_0.getcpscriptedhelidropheight = 0;

    for (;;)
    {
        var_1 = gettime();
        var_2 = istrue( var_0.bots_create_landing_spots );
        var_3 = scripts\cp_mp\utility\weapon_utility::islockedonto( var_0 );
        var_4 = scripts\cp_mp\vehicles\vehicle_occupancy::soundorg_int( var_0 );
        ...
        var_12 = var_0.getcpscriptedhelidropheight;

        if ( var_5 || var_6 || var_7 || var_2 || var_3 || !var_4 )
            var_0.getcpscriptedhelidropheight = 1;
        else
            var_0.getcpscriptedhelidropheight = 0;
...
spawnselectionthink( var_0 )
{
    ...
    if ( istrue( level.propsetchangesleft ) )
        thread scripts\mp\gametypes\arm.gsc::spawn_custom_type_array();

    if ( istrue( level.proprotate ) )
        thread scripts\mp\gametypes\arm.gsc::spawn_cypher_monitor_model();
...
watchforafk()
{
    ...
    if ( isdefined( level.relic_amped_monitor ) )
        wait( level.relic_amped_monitor );
    else
        wait 60.0;
...
selectionmade()
{
    ...
    for (;;)
    {
        ...
        {
            var_1 = should_save_debug_info( var_1 );
            var_2 = undefined;
            ...
            foreach ( var_4 in level.spawnselectionlocations )
            {
                ...
                {
                    should_start_cautious_approach_koth( var_4[self.team].typeref, self, self.team );

                    if ( isdefined( var_4[self.team].objectivekey ) && isobjectiveindanger( var_4[self.team].objectivekey, self.team ) )
                        break;

                    if ( isdefined( var_4[self.team].typeref ) && issubstr( var_4[self.team].typeref, "vehicle" ) && istrue( var_4[self.team].dynamicent.getcpscriptedhelidropheight ) )
                        break;
            ...
            if ( !isdefined( var_2 ) )
            {
                var_6 = floor2lights( self.team, self.squadindex );
                ...
                foreach ( var_4 in var_6 )
                {
                    if ( isdefined( var_4.dynamicent ) && var_4.dynamicent getentitynumber() == var_1 )
                    {
                        ...
                        var_9 = spawnstruct();
                        var_9 = scripts\mp\spawnscoring::brhandleinvulnerability( var_9, var_4.dynamicent );

                        if ( !isdefined( var_9.relic_amped_debug_explode_after_reviving ) )
                        {
                            var_4.dynamicent.key_drop_on_client_death_disconnect = gettime();
                            break;
                        }
...
should_save_debug_info( var_0 )
{
    if ( !isdefined( self.gas_flyby_plane ) )
        self.gas_flyby_plane = [];

    var_1 = 65536;
    var_2 = 131072;
    var_3 = 262144;
    var_4 = 524288;
    self.gas_flyby_plane["rightmouseup"] = istrue( self.gas_flyby_plane["rightmouseup"] ) || ( var_0 & var_1 ) > 0;
    self.gas_flyby_plane["leftmouseup"] = istrue( self.gas_flyby_plane["leftmouseup"] ) || ( var_0 & var_2 ) > 0;
    self.gas_flyby_plane["activate"] = istrue( self.gas_flyby_plane["activate"] ) || ( var_0 & var_3 ) > 0;
    self.gas_flyby_plane["gostand"] = istrue( self.gas_flyby_plane["gostand"] ) || ( var_0 & var_4 ) > 0;
...
watchendconditions()
{
    ...
    var_0 = scripts\engine\utility::spotlight_max_dist_sq_from_node( "tac_ops_map_selection_valid", "tac_ops_map_changed", "tac_ops_map_cleared", "spawned_player", "tac_ops_map_game_ended", "tac_ops_map_nuke" );
...
endcondition_focuschanged( var_0 )
{
    ...
    foreach ( var_3 in level.spawnselectionlocations )
    {
        ...
        {
            thread hideextractionobjectivefromteam( var_3[self.team].anchorentity.origin );
    ...
    if ( !isdefined( var_1 ) )
    {
        var_5 = floor2lights( self.team, self.squadindex );

        foreach ( var_3 in var_5 )
        {
            if ( isdefined( var_3.dynamicent ) && var_3.dynamicent getentitynumber() == var_0 )
            {
                thread hideextractionobjectivefromteam( var_3.dynamicent.origin );
                var_1 = var_3.typeref;
                break;
            }
        }
    ...
    if ( issubstr( var_1, "vehicle" ) )
    {
        ...
        thread initialize_water_trap( var_1 );
        var_8 = tablelookuprownum( "mp/spawnSelectionMapData.csv", 0, var_1 );
        self setclientomnvar( "ui_tom_previous_selection", var_8 );
        return;
    }
...
hideextractionobjectivefromteam( var_0 )
{
    ...
    if ( !isdefined( level.suicideonend ) )
        level.suicideonend = max( getdvarint( "NOLPSOTORP", 20000 ) * 0.5 / 1000, 1.0 );

    for (;;)
    {
        self predictstreampos( var_0, 1 );
        wait( level.suicideonend );
    }
}
...
evaluatesquadspawn()
{
    ...
    for ( var_4 = 0; var_4 < 4; var_4++ )
    {
        ...
        var_5.getattractionomnvarbitpackinginfo = undefined;
        var_3[var_4] = var_5;
    }
    ...
    for (;;)
    {
        ...
        var_11 = floor2lights( self.team, self.squadindex );
        ...
        for ( var_4 = 0; var_4 < 4; var_4++ )
        {
            ...
            {
                var_15 = istrue( var_9.getcpscriptedhelidropheight );
                var_16 = var_1[var_4];
                ...
                if ( !isdefined( var_3[var_4].getattractionomnvarbitpackinginfo ) || var_3[var_4].getattractionomnvarbitpackinginfo != var_16 )
                {
                    var_3[var_4].getattractionomnvarbitpackinginfo = var_16;
            ...
            if ( isdefined( var_3[var_4].getattractionomnvarbitpackinginfo ) && var_3[var_4].getattractionomnvarbitpackinginfo != -1 )
            {
                var_3[var_4].getattractionomnvarbitpackinginfo = -1;
...
handlesquadspawncycle()
{
    ...
    for (;;)
    {
        var_0 = scripts\engine\utility::spin_light( "cyclePos", "cycleNeg" );
...
{
    thread gulagfadetoblackspectatorsofplayer();

    if ( getdvarint( "scr_squad_spawn_debug", 0 ) == 1 )
        thread scripts\mp\spawnscoring::relic_shieldsonly_set_player_stats_after_spawn();
}
...
gulagfadetoblackspectatorsofplayer()
{
    ...
    for (;;)
    {
        if ( issquadspawnable( self ) )
            self.getcpscriptedhelidropheight = issquadmateindanger( self );
...
issquadmateindanger( var_0 )
{
    ...
    if ( isdefined( var_0.ignoredeathsdoor ) && var_0.ignoredeathsdoor + 3000 > var_1 )
        return 1;

    if ( isdefined( var_0.ignoredeathsdoor ) && isdefined( var_0.ignoreattractions ) && var_0.ignoredeathsdoor > var_0.ignoreattractions || isdefined( var_0.ignoredeathsdoor ) && !isdefined( var_0.ignoreattractions ) )
        return 1;
    ...
    {
        if ( istrue( var_0.vehicle.bots_create_landing_spots ) )
            return 1;
    ...
    if ( isdefined( var_0 ) && isdefined( var_0.key_drop_on_client_death_disconnect ) && var_0.key_drop_on_client_death_disconnect + 1000 > var_1 )
        return 1;
...
timeuntilspawnmessaging( var_0, var_1 )
{
    ...
    if ( var_0 > 0 )
    {
        ...
        scripts\engine\utility::spotlight_sweep_to_loc( var_0, "force_spawn" );
    }
}
...
getclosestavailablespawnlocation()
{
    ...
    else
    {
        ...
        {
            var_7 = floor2lights( self.team, self.squadindex );
...
{
    level thread should_ai_take_damage();
        ...
        foreach ( var_1 in level.teamnamelist )
        {
            foreach ( var_3 in level.availablespawnlocations[var_1] )
            {
                ...
                if ( isdefined( var_4.objectivekey ) )
                    var_4.getcpscriptedhelidropheight = isobjectiveindanger( var_4.objectivekey, var_1 );
            }
        }
        ...
        foreach ( var_1 in level.teamnamelist )
        {
            ...
            foreach ( var_4 in level.availablespawnlocations[var_1] )
            {
                if ( !issubstr( var_4, "HQ" ) && issubstr( var_4, "fob" ) )
                {
                    ...
                    level.objectives[var_17].relic_ammo_drain_take_ammo = var_14;
                    level.objectives[var_17].relic_amped_clear_victim = gettime();
                }
            }
        }
...
should_start_cautious_approach_koth( var_0, var_1, var_2 )
{
    var_3 = level.spawnselectionlocations[var_0][var_2].objectivekey;

    if ( !isdefined( var_3 ) || !isdefined( level.objectives[var_3] ) || !isdefined( level.objectives[var_3].relic_amped_clear_victim ) || level.objectives[var_3].relic_amped_clear_victim == gettime() )
        return;
    ...
    level.objectives[var_3].relic_ammo_drain_take_ammo = var_13;
    level.objectives[var_3].relic_amped_clear_victim = gettime();
}
