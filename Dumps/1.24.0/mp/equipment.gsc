...
inititems()
{
    ...
    var_0.callbacks["equip_binoculars"]["onGive"] = scripts\mp\equipment\binoculars.gsc::binoculars_ongive;
    var_0.callbacks["equip_binoculars"]["onTake"] = scripts\mp\equipment\binoculars.gsc::binoculars_ontake;
...
loadtable()
{
    ...
    for (;;)
    {
        ...
        var_3.heli_assault3_hangar_death_watcher = tablelookupbyrow( "mp/equipment.csv", var_0, 8 ) != "-1";
...
getequipmentmaxammo( var_0 )
{
    ...
    else
    {
        ...
        {
            if ( scripts\mp\gametypes\br_public.gsc::get_chopper_minigun_start_node() )
...
{
    if ( isdefined( level.equipment.table[var_0] ) )
        return istrue( level.equipment.table[var_0].heli_assault3_hangar_death_watcher );
    else
        return 0;
}
...
executeoffhandfired( var_0 )
{
    foreach ( var_6, var_2 in self.equipment )
    {
        ...
        if ( isdefined( var_3.objweapon ) && var_0 == var_3.objweapon )
        {
            ...
            scripts\mp\damage::cargo_truck_mg_initinteract( var_4 );
...
watchlethaldelayplayer( var_0, var_1 )
{
    ...
    if ( !isdefined( self.infil_chopper_dialogue ) || !istrue( self.infil_chopper_dialogue[var_1] ) )
    {
        if ( !isdefined( self.infil_chopper_dialogue ) )
            self.infil_chopper_dialogue = [];

        self.infil_chopper_dialogue[var_1] = 1;
    ...
    if ( isdefined( self.infil_chopper_dialogue ) && istrue( self.infil_chopper_dialogue[var_1] ) )
    {
        self.infil_chopper_dialogue[var_1] = undefined;

        if ( self.infil_chopper_dialogue.size == 0 )
            self.infil_chopper_dialogue = undefined;
...
hackequipment( var_0 )
{
    ...
    var_0 scripts\cp\so_laststand::latejoinersanimstruct( self.equipmentref );
    changeowner( var_0 );
...
initlethalmaxoffsetmap()
{
    ...
    foreach ( var_2 in level.equipment.table )
    {
        if ( !var_2.heli_assault3_hangar_death_watcher )
            continue;

        if ( var_2.id <= 0 )
            continue;

        if ( var_2.defaultslot == "" )
            continue;

        level.tr_vis_radius_override_lod1[var_2.ref] = 1 << var_0;
        var_0++;
    }
}
