...
waslastround()
{
    ...
    if ( istrue( level.chopper_kill_person ) )
        return 1;
...
isanymlgmatch()
{
    ...
    return getdvarint( "LOMTKQTRTM" ) || istrue( level.brloadoutcratedestroycallback );
}

ismlgsystemlink()
{
    if ( level.systemlink && ( getdvarint( "LOMTKQTRTM" ) || istrue( level.brloadoutcratedestroycallback ) ) )
        return 1;
...
ismlgprivatematch()
{
    if ( privatematch() && ( getdvarint( "LOMTKQTRTM" ) || istrue( level.brloadoutcratedestroycallback ) ) )
        return 1;
...
{
    level.light_tank_gunnerdamagemodignorefunc = 5;
    return level.light_tank_gunnerdamagemodignorefunc;
}
...
init_reach_wind_room( var_0, var_1, var_2, var_3, var_4 )
{
...
healthpool()
{
    return istrue( level.healthpack_health );
}

isgroundwarcoremode()
{
    return istrue( level.hasanykillstreak ) || istrue( level.tut_popup_listener ) || istrue( level.train_tag_array );
}
