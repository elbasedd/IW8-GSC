    ...
    if ( isdefined( var_21 ) )
    {
        for ( var_22 = 0; var_22 < var_21.size; var_22++ )
        {
            ...
            if ( isdefined( var_21[var_22].node ) )
            {
                ...
                var_11.move_entity = var_24.droplocations[var_25].origin;
                continue;
            }

            var_11.move_entity = var_21[var_22].origin;
        }

        level.move_gate = 1;
        level thread registeronplayerdisconnect( var_1 );
    }

    level thread initialize_create_script_file();
}
...
calloutmarkerpingvo_handlerdoescontainpoolelement( var_0, var_1 )
{
    var_2 = [];
...
{
    calloutmarkerpingvo_handlerdoescontainpoolelement( self.team, 1 );
    var_2 = scripts\cp_mp\killstreaks\airdrop::dropbrloadoutcrate( self.team, var_0 + ( 0, 0, 4096 ), var_0 + ( 0, 0, 512 ) );
    var_2 endon( "death" );
    enabledropbagobjective( var_2 );
    button( var_2 );
    var_3 = [];

    foreach ( var_5 in level.teamdata[self.team]["alivePlayers"] )
    {
        if ( isdefined( var_5 ) && !var_5 scripts\mp\gametypes\br_public.gsc::isplayeringulag() )
        {
            var_3[var_3.size] = var_5;
            var_5 thread scripts\mp\hud_message::showsplash( "br_airdrop_incoming" );
        }
    }

    if ( !scripts\mp\gametypes\br_public.gsc::heli_intro() )
        scripts\mp\gametypes\br_public.gsc::bot_gametype_human_player_always_defender( "dropbag_incoming", self.team, 1 );

    if ( scripts\mp\utility\game::getgametype() == "br" )
        scripts\mp\gametypes\br_analytics.gsc::bomb_use_watcher( self, var_0, var_1, var_2 );
}

registeronplayerdisconnect( var_0 )
{
    foreach ( var_2 in var_0 )
    {
        if ( !isdefined( var_2 ) )
        ...
        if ( isdefined( level.br_circle ) && isdefined( level.br_circle.safecircleent ) )
        {
            ...
            {
                var_4 = scripts\mp\gametypes\br_circle.gsc::getsafecircleorigin();
                var_7 = scripts\mp\gametypes\br_c130.gsc::questinstance( var_4, var_3 );
        ...
        var_8 = fake_hit_target_monitor( 1, 0, 0, 0, 0 );
        var_2 registeronentergulag( var_3, var_8 );
        waitframe();
    }
}
...
spawndropbagonlanding()
{
    ...
    if ( isdefined( var_0 ) )
    {
        var_3 = fake_hit_target_monitor( 0, 0, 0, 1, 0 );
        registeronentergulag( var_0.origin, var_3 );
        level thread initialize_create_script_file();
    }
}
...
completecollectionquest( var_0 )
{
    ...
    foreach ( var_3 in var_0 )
    {
        ...
        var_1 = var_3;
        move_hvt_from_under_heli( 0 );
    }
}

airlock_callbuttons()
{
    if ( scripts\mp\gametypes\br_gametypes.gsc::has_started_cache_defenses( "dropbag" ) )
        return;
...
fake_hit_target_monitor( var_0, var_1, var_2, var_3, var_4 )
{
    if ( istrue( var_0 ) )
        return 0;
