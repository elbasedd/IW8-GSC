...
assignchevrons( var_0, var_1 )
{
    ...
    var_2 = patch_far_wait( var_2, var_1 );
    var_2 = mortars_fire_logic( var_2, var_1 );
    var_3 = [];
...
activatezone()
{
    ...
    if ( isdefined( level.ffsm_nextstreamhinttime ) )
        var_0 = [[ level.ffsm_nextstreamhinttime ]]();

    var_1 = 1024;

    if ( isdefined( level.ffsm_onground_stateenter ) )
        var_1 = [[ level.ffsm_onground_stateenter ]]();
    ...
    {
        self.resetvisionsetnighttodefault = ::support_update_hint_logic;
...
deactivatezone()
{
    ...
    foreach ( var_1 in self.cleanupfunc )
        scripts\mp\spawnlogic::removespawndangerzone( var_1 );

    self.cleanupfunc = undefined;
...
cleanupgametypevips()
{
    level scripts\engine\utility::spectatecommands( "game_ended", "zone_moved" );
...
zone_onuse( var_0 )
{
    ...
    if ( level.usehqrules && !level.usehprules && self.ownerteam != "neutral" )
    {
        ...
        level.zone thread spawnx1stashlootcache();
        ...
        if ( isdefined( var_3 ) )
        {
            ...
            foreach ( var_11 in var_9 )
            {
                ...
                var_7 scripts\cp\so_laststand::laststand_player_in_focus();
            }
        }

        level thread scripts\mp\gametypes\hq.gsc::give_capture_credit( var_3, var_2, var_1, undefined, var_0 );
    ...
    else
    {
        ...
        if ( scripts\mp\utility\game::getgametype() == "hq" && !level.usehprules )
        {
            ...
            if ( var_18 > var_19 )
                level.player_retry_thread = 1;

            add_scriptable_setup( var_1 );
        }
        else if ( level.little_bird_trail )
        ...
        if ( !isdefined( level.lastcaptureteam ) || level.lastcaptureteam != var_1 )
        {
            ...
            if ( scripts\mp\utility\game::getgametype() == "koth" || scripts\mp\utility\game::getgametype() == "grnd" )
                level thread scripts\mp\gametypes\koth.gsc::give_capture_credit( var_3, var_2, var_1, level.lastcaptureteam, var_0 );
            else
                level thread scripts\mp\gametypes\hq.gsc::give_capture_credit( var_3, var_2, var_1, level.lastcaptureteam, var_0 );
        }
...
zone_onuseupdate( var_0, var_1, var_2, var_3 )
{
    ...
    if ( var_4 == "neutral" && self.claimteam != var_4 )
        add_scriptable_setup( self.claimteam );
    else if ( var_4 != "neutral" && self.claimteam != var_4 )
        add_scriptable_setup( var_4, 1 );
    else if ( level.little_bird_trail )
        level scripts\mp\gamelogic::pausetimer();
...
zone_onunoccupied()
{
    ...
    {
        level.zone scripts\mp\gameobjects::setobjectivestatusicons( level.icondefend, level.iconcapture );
        add_scriptable_setup( self.ownerteam );
    }
    else
    {
        level notify( "zone_destroyed" );
...
zone_oncontested()
{
    ...
    self.carryobjects_onjuggernaut = gettime();
    ...
    level.zone thread scripts\common\utility::set_showing_bomb_wire_pair_to_player( level.inside_bush, "hill_contested", level.zone.trigger.origin );
    self.didstatusnotify = 1;
}
...
zone_onuncontested( var_0 )
{
    ...
    if ( var_0 == "none" || var_1 == "neutral" )
    {
        ...
        level.zone thread scripts\common\utility::set_showing_bomb_wire_pair_to_player( level.inside_bush, "hill_empty", level.zone.trigger.origin );
    }
    else
    {
        if ( scripts\mp\utility\game::getgametype() == "koth" )
        ...
        level.zone thread scripts\common\utility::set_showing_bomb_wire_pair_to_player( level.inside_bush, "hill_uncontested", level.zone.trigger.origin );
    }

    var_7 = ( gettime() - self.carryobjects_onjuggernaut ) * 0.001;
    scripts\mp\utility\game::init_reach_wind_room( undefined, undefined, "Zone Contested", level.zone.trigger.origin, var_7 + " seconds" );
    self.carryobjects_onjuggernaut = undefined;
...
support_update_hint_logic( var_0 )
{
...
mortars_fire_logic( var_0, var_1 )
{
    var_2 = [];
    var_3 = [];
...
chopper_boss_explosion( var_0, var_1, var_2 )
{
    var_0.origin = var_1;
    var_0.angles = var_2;
    return var_0;
