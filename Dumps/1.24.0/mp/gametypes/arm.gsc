...
main()
{
    ...
    level.runcontrolledcallback = getdvarint( "scr_arm_tankRespawnTime", 120 );
    level.ally_spawns = getdvarint( "scr_arm_apcRespawnTime", 120 );
    level.longdialoguecooldown = getdvarint( "scr_arm_longDialogueCooldown", 1 );
    level.is_a_player_near = getdvarint( "scr_arm_mercyEndingNuke", 1 );
    level.is_ai_facing_point = getdvarint( "scr_arm_mercyEndingTime", 30 );
    level.kothtotaltime = getdvarint( "scr_arm_numNonRallyVehicles", 25 );
    level.creategulagarenaloadout = getdvarint( "scr_arm_disablePersonalNuke", 0 );
    level.loadout_updateclassdefault_weapons = getdvarint( "scr_personalNukeCostOverride", 30 );
    level.propsetchangesleft = getdvarint( "scr_arm_showTeamTanks", 1 );
    level.proprotate = getdvarint( "scr_arm_showTeamLittleBirds", 1 );
    level.relic_amped_monitor = getdvarint( "scr_arm_spawnSelectionAFKTime", 120 );
    level.initlocs_keypads = getdvarint( "scr_arm_manualAdjustLittleBirdLocs", 1 );
    level.customspeed = getdvarint( "scr_arm_dontSpawnJeep", 0 );
    level.custompassengerwaitfunc = getdvarint( "scr_arm_dontSpawnCargoTruck", 0 );
    level.showhint = getdvarint( "scr_arm_usePingSystem", 1 );

    if ( istrue( level.propsetchangesleft ) )
    {
        level.select_lobby_patrol_spawners = [];
        level.select_lobby_patrol_spawners["axis"] = [];
        level.select_lobby_patrol_spawners["allies"] = [];
    }

    if ( istrue( level.proprotate ) )
    {
        level.select_lobby_door_two_spawners = [];
        level.select_lobby_door_two_spawners["axis"] = [];
        level.select_lobby_door_two_spawners["allies"] = [];
        level.select_lobby_door_two_spawners["untouched"] = [];
    }
    ...
    level.relic_amped_explosion_time = ::relic_amped_explosion_time;
}
...
onstartgametype()
{
    ...
    if ( istrue( level.is_a_player_near ) )
        level thread kill_drone_turret();

    debug_show2dvotext();

    if ( istrue( level.showhint ) )
...
debug_show2dvotext()
{
    ...
    scripts\cp_mp\utility\game_utility::path_timeout( "vehicle_volume", "script_noteworthy" );
    scripts\cp_mp\utility\game_utility::path_timeout( "vehicle_volume_simplified", "script_noteworthy" );
    scripts\cp_mp\utility\game_utility::path_timeout( "super", "script_noteworthy" );
    scripts\cp_mp\utility\game_utility::path_timeout( "militarybase", "script_noteworthy" );
    scripts\cp_mp\utility\game_utility::path_timeout( "location_volume", "targetname" );
    scripts\cp_mp\utility\game_utility::path_timeout( "locale_area_trigger", "targetname" );
    scripts\cp_mp\utility\game_utility::path_timeout( "bank_entry_detector", "targetname" );
    scripts\cp_mp\utility\game_utility::path_timeout( "shadow_blocker", "targetname" );
    scripts\cp_mp\utility\game_utility::pathblocker( "door_prison_cell_metal_mp", 1 );
    scripts\cp_mp\utility\game_utility::pathblocker( "veh8_mil_air_acharlie130", 1 );
    ...
    scripts\cp_mp\utility\game_utility::path_loop( "light" );
    scripts\cp_mp\utility\game_utility::path_loop( "trigger_use_touch" );

    if ( isdefined( level.localeid ) && level.localeid == "locale_6" )
        scripts\cp_mp\utility\game_utility::path_timeout( "locale_8", "script_noteworthy" );
...
updatedomscores()
{
    ...
    level thread scripts\mp\spawnselection::shot_to_miss_for_dialog();
...
updatescores()
{
    ...
    var_5 = scripts\mp\gamescore::br_pe_chopper_crates();
    ...
    var_8 = scripts\mp\gamescore::br_pe_chopper_crates();

    if ( var_5 != var_8 )
        scripts\mp\gamescore::module_has_data_for_call_count( var_8, 1, var_5 );
}
...
objective_manageobjectivesintrovisibility()
{
    ...
    scripts\mp\objidpoolmanager::objective_playermask_hidefromall( level.gw_objstruct.axishqloc.delete_furthest_respawn_enemy.objidnum );
    scripts\mp\objidpoolmanager::objective_playermask_hidefromall( level.gw_objstruct.allieshqloc.delete_furthest_respawn_enemy.objidnum );
...
sethqmarkerobjective()
{
    ...
    var_2.lockupdatingicons = 1;
    level.gw_objstruct.axishqloc.delete_furthest_respawn_enemy = var_2;
    level.uncapturableobjectives[level.uncapturableobjectives.size] = var_2;
    ...
    scripts\mp\objidpoolmanager::update_objective_setbackground( var_2.objidnum, 7 );
    var_2.lockupdatingicons = 1;
    level.gw_objstruct.allieshqloc.delete_furthest_respawn_enemy = var_2;
    level.uncapturableobjectives[level.uncapturableobjectives.size] = var_2;
...
spawnselection_showenemyhq()
{
    ...
    if ( self.team == "allies" )
    {
        ...
        scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( level.gw_objstruct.axishqloc.delete_furthest_respawn_enemy.objidnum, self );
    }
    else
    {
        scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( level.gw_objstruct.allieshqloc.delete_furthest_respawn_enemy.objidnum, self );
    ...
    if ( self.team == "axis" )
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( level.gw_objstruct.allieshqloc.delete_furthest_respawn_enemy.objidnum, self );
    else
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( level.gw_objstruct.axishqloc.delete_furthest_respawn_enemy.objidnum, self );
}
...
docratedropsmoke( var_0, var_1, var_2 )
{
    ...
    if ( isdefined( var_0 ) )
        var_0 scripts\engine\utility::spotlight_min_dist_sq_from_node( var_2, "crate_dropped" );
    else
        wait( var_2 );
...
kill_drone_turret()
{
    ...
    if ( istrue( level.useobjectives ) )
        level thread kill_frequency_watcher();
}
...
kill_frequency_watcher()
{
    ...
    for (;;)
    {
        if ( br_movingcirclemovedistmin( "axis" ) == level.objectives.size )
            level thread kill_rate_too_slow( "axis" );
        else if ( br_movingcirclemovedistmin( "allies" ) == level.objectives.size )
            level thread kill_rate_too_slow( "allies" );

        waitframe();
    }
}

kill_rate_too_slow( var_0 )
{
    ...
    setomnvar( "ui_nuke_timer_time", level.is_ai_facing_point );
    setomnvar( "ui_nuke_countdown_active", 1 );
    var_1 = 0;
    var_2 = gettime();
    var_3 = level.is_ai_facing_point * 1000 + var_2;
    setomnvar( "ui_nuke_end_milliseconds", level.is_ai_facing_point * 1000 + var_2 );

    while ( br_movingcirclemovedistmin( var_0 ) == level.objectives.size )
    {
        waitframe();

        if ( gettime() > var_3 )
        {
            kill_spawngroups_near_safehouse( var_0 );
...
kill_spawngroups_near_safehouse( var_0 )
{
    ...
    foreach ( var_5 in level.players )
    {
        ...
        {
            if ( isdefined( var_5.level_carepackage_give_player_killstreak_incendiary_launcher ) )
            {
                var_5 clearsoundsubmix( "iw8_mp_spawn_camera" );
                var_5.level_carepackage_give_player_killstreak_incendiary_launcher stoploopsound( var_5.level_carepackage_player_used_logic );
                var_5.level_carepackage_give_player_killstreak_incendiary_launcher delete();
                var_5.level_carepackage_give_player_killstreak_incendiary_launcher = undefined;
                var_5.level_carepackage_player_used_logic = undefined;
            }
        }
    }
...
{
    thread scripts\mp\spawnselection::ondefuse( var_0.team, var_0.squadindex );
}
    ...
    foreach ( var_1 in level.vehicleinfo )
    {
        ...
        foreach ( var_9 in var_1.vehiclespawns )
        {
            if ( isdefined( scripts\cp_mp\utility\game_utility::getlocaleid() ) && isdefined( var_9.script_noteworthy ) && var_9.script_noteworthy == level.localeid )
            {
                ...
                if ( istrue( level.customspeed ) && var_1.refname == "jeep" || istrue( level.custompassengerwaitfunc ) && var_1.refname == "cargo_truck" )
                    continue;
                else
                {
...
vehiclespawn_tank( var_0, var_1 )
{
    ...
    if ( istrue( level.propsetchangesleft ) )
        skipfirstraise( var_3, var_3.team );

    return var_3;
}

skipfirstraise( var_0, var_1 )
{
    wait 1;
    ...
    var_0.relic_amped_monitor_beeps = var_2;
    thread spawn_exfil_heli( var_0 );
    level.select_lobby_patrol_spawners[var_1][level.select_lobby_patrol_spawners[var_1].size] = var_0;
}

spawn_exfil_heli( var_0 )
{
    var_1 = var_0.relic_amped_monitor_beeps.objidnum;
    var_2 = var_0.team;
    var_0 waittill( "death" );
    scripts\mp\objidpoolmanager::returnobjectiveid( var_1 );
    level.select_lobby_patrol_spawners[var_2] = scripts\engine\utility::array_remove( level.select_lobby_patrol_spawners[var_2], var_0 );
}
...
spawn_custom_type_array()
{
    ...
    foreach ( var_1 in level.select_lobby_patrol_spawners[self.team] )
        scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( var_1.relic_amped_monitor_beeps.objidnum, self );

    while ( self.inspawnselection )
        waitframe();

    foreach ( var_1 in level.select_lobby_patrol_spawners["axis"] )
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( var_1.relic_amped_monitor_beeps.objidnum, self );

    foreach ( var_1 in level.select_lobby_patrol_spawners["allies"] )
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( var_1.relic_amped_monitor_beeps.objidnum, self );
}
...
skipfinalkillcamfx( var_0, var_1 )
{
    wait 1;
    var_2 = scripts\mp\gameobjects::createobjidobject( var_0.origin, var_1, ( 0, 0, 0 ), undefined, 0, 0 );
    scripts\mp\objidpoolmanager::update_objective_ownerteam( var_2.objidnum, var_1 );
    ...
    var_0.relic_amped_monitor_beeps = var_2;
    thread spawn_exfil_enemies( var_0 );
    level.select_lobby_door_two_spawners["untouched"][level.select_lobby_door_two_spawners["untouched"].size] = var_0;
}
...
spawn_exfil_enemies( var_0 )
{
    ...
    if ( !isdefined( var_0.idmask ) )
        var_2 = "untouched";

    level.select_lobby_door_two_spawners[var_2] = scripts\engine\utility::array_remove( level.select_lobby_door_two_spawners[var_2], var_0 );
}
...
spawn_cypher_monitor_model()
{
    ...
    foreach ( var_1 in level.select_lobby_door_two_spawners[self.team] )
        scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( var_1.relic_amped_monitor_beeps.objidnum, self );

    foreach ( var_1 in level.select_lobby_door_two_spawners["untouched"] )
        scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( var_1.relic_amped_monitor_beeps.objidnum, self );

    while ( self.inspawnselection )
        waitframe();

    foreach ( var_1 in level.select_lobby_door_two_spawners["axis"] )
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( var_1.relic_amped_monitor_beeps.objidnum, self );

    foreach ( var_1 in level.select_lobby_door_two_spawners["allies"] )
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( var_1.relic_amped_monitor_beeps.objidnum, self );
...
spawn_claymore_group( var_0 )
{
    if ( isdefined( self.idmask ) )
        level.select_lobby_door_two_spawners[self.idmask] = scripts\engine\utility::array_remove( level.select_lobby_door_two_spawners[self.idmask], self );
    else
        level.select_lobby_door_two_spawners["untouched"] = scripts\engine\utility::array_remove( level.select_lobby_door_two_spawners["untouched"], self );

    self.idmask = var_0;
    level.select_lobby_door_two_spawners[var_0][level.select_lobby_door_two_spawners[var_0].size] = self;
    scripts\mp\objidpoolmanager::update_objective_icon( self.relic_amped_monitor_beeps.objidnum, "icon_minimap_littlebird_spawn_selection" );

    foreach ( var_2 in level.players )
    {
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( self.relic_amped_monitor_beeps.objidnum, var_2 );

        if ( isdefined( var_2 ) && isdefined( var_2.team ) && var_2.team == var_0 && istrue( var_2.inspawnselection ) )
        {
            scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( self.relic_amped_monitor_beeps.objidnum, var_2 );
            continue;
        }
...
init_rallyvehicles()
{
    ...
    {
        if ( level.localeid == "locale_6" && level.mapname == "mp_downtown_gw" && istrue( level.initlocs_keypads ) )
        {
            var_10 = [];
        ...
        foreach ( var_2 in var_0 )
        {
            ...
            level thread skipfinalkillcamfx( var_6, "neutral" );
        }
    }
...
arm_initoutofbounds()
{
    ...
    foreach ( var_2 in var_0 )
    {
        if ( isdefined( scripts\cp_mp\utility\game_utility::getlocaleid() ) && isdefined( var_2.script_noteworthy ) && var_2.script_noteworthy == level.localeid && scripts\mp\utility\game_utility_mp::isdragonsbreathweapon( var_2 ) )
        {
