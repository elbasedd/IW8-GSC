...
main()
{
    ...
    if ( scripts\mp\trials\mp_euphrates_create_script::trial_is_event() )
    ...
    while ( !isdefined( level.server_structs ) )
        waitframe();
    ...
    switch ( level.trial["missionScript"] )
    {
        case "gun_nonlinear":
        case "gun":
            thread [[ level.server_structs["gun"] ]]();
            break;
        case "clear":
            thread [[ level.server_structs["clear"] ]]();
            break;
        case "sniper":
            thread [[ level.server_structs["sniper"] ]]();
            break;
        case "jugg":
            thread [[ level.server_structs["jugg"] ]]();
            break;
        case "lava":
            thread [[ level.server_structs["lava"] ]]();
            break;
        case "pitcher":
            thread [[ level.server_structs["pitcher"] ]]();
            break;
        case "gunslinger":
            thread [[ level.server_structs["gunslinger"] ]]();
            break;
        case "race":
            thread [[ level.server_structs["race"] ]]();
            break;
        case "arm_course":
            thread [[ level.server_structs["arm_course"] ]]();
            break;
        default:
            break;
    }

    scripts\mp\equipment\binoculars::init();
...
set_no_crash( var_0, var_1 )
{
    level notify( "exitLevel_called" );
    processlobbydata();

    if ( isdefined( level.seq3_reset_switch ) )
        [[ level.seq3_reset_switch ]]();
...
trial_mission_data_init()
{
    var_0 = scripts\mp\trials\mp_euphrates_create_script::trial_fetch_mission_table();
...
getspawnpoint()
{
    while ( istrue( level.set_cp_vehicle_health_values ) )
        waitframe();
    ...
    {
        var_3 = scripts\cp_mp\vehicles\vehicle_occupancy::vehicle_occupancy_getavailablevehicleseats( level.set_corpse_detect_ranges, 1 );
        var_4 = spawnstruct();
        var_4.useonspawn = 1;
        var_4.enterstartwaitmsg = "spawned_player";
        thread scripts\cp_mp\vehicles\vehicle_occupancy::vehicle_occupancy_enter( level.set_corpse_detect_ranges, var_3[0], self, var_4 );
...
onplayerconnect( var_0 )
{
    ...
    level.player scripts\mp\gametypes\br.gsc::mid_air_explode();
    wait 1;
    ...
    thread set_chopper_circle_speed();
}
...
weapon_think()
{
    ...
    for (;;)
    {
        ...
        if ( isdefined( level.player.primaryinventory[0] ) )
            var_14 = var_13 == getcompleteweaponname( level.player.primaryinventory[0] );

        if ( isdefined( level.player.primaryinventory[1] ) )
            var_15 = var_13 == getcompleteweaponname( level.player.primaryinventory[1] );
        ...
        if ( !isdefined( self.spawned_weapon ) && !var_14 && !var_15 )
        {
            ...
            if ( istrue( level.matchdata_onroundend ) )
        ...
        if ( getcompleteweaponname( var_19 ) == var_13 )
            var_18 delete();
    }
}
...
trial_end_score_dialogue()
{
    ...
    scripts\mp\trials\mp_euphrates_create_script::trial_ui_waittill_retry();

    if ( !istrue( level.set_chopper_search_speed ) )
...
set_chopper_circle_speed()
{
    for (;;)
    {
        ...
        if ( var_0 == "trial_restart" )
            scripts\mp\trials\mp_euphrates_create_script::set_carry_item();
    }
}

processlobbydata()
{
    mp_m_trench_patch_giveplayer_c4( level.player );
