...
init()
{
    scripts\cp_mp\utility\script_utility::registersharedfunc( "br_c130Airdrop", "c130Airdrop_onCrateUse", ::br_danger_circle_closing_music );
    level.br_domheight = [];
    level.br_ending = [];
    level.br_circleshowdelaysafe = getdvarint( "scr_bmo_airdropCrateHeightOverride", 12000 );
}
...
br_contractxpearned()
{
    ...
    for (;;)
    {
        ...
        if ( istrue( level.showextractiontime ) && istrue( var_0 ) )
        ...
        if ( var_7 < var_3 )
        {
            ...
            for ( var_12 = 0; var_12 < var_10; var_12++ )
            {
                ...
                var_14 = br_checkforlaststandfinish( var_12 );
                var_15 = distance( var_14.startpt, var_14.endpt );
                var_16 = scripts\mp\gametypes\br_c130.gsc::getc130speed();
                var_17 = var_15 / var_16;
                var_18 = br_delaynojip( var_14, var_15, var_16, var_17 );

                if ( var_12 == 0 )
                    scripts\mp\gametypes\br_gametype_dmz.gsc::proprange( "br_c130airdrop_incoming" );

                var_18 br_displayperkinfo( var_13 );
...
br_checkforlaststandfinish( var_0, var_1, var_2 )
{
    if ( isdefined( level.br_level.br_mapcenter ) && isdefined( level.br_level.br_mapsize ) )
        var_3 = br_checkforlaststandwipe( var_0, var_1, var_2 );
    else
...
br_checkforlaststandwipe( var_0, var_1, var_2 )
{
    var_3 = undefined;
    ...
    var_5 = scripts\mp\gametypes\br_c130.gsc::first_convoy();
    ...
    if ( !isdefined( var_0 ) || var_0 == 0 )
    {
        if ( !istrue( var_2 ) )
        {
            var_7 = br_circleshowdelaydanger();
            var_8 = br_circleradiizero( var_7 );
            var_6 = vectortoangles( var_4 - var_8 * ( 1, 1, 0 ) );
            level.br_endgamesplashcallback = var_6;
        }

        var_4 = var_4 + ( 0, 0, scripts\cp_mp\parachute::f14_current_inputamt() );
    }
    else
    {
        if ( isdefined( level.br_endgamesplashcallback ) )
        {
            var_6 = level.br_endgamesplashcallback;
            var_9 = 1;
        ...
        var_10 = var_10 + ( 0, 0, scripts\cp_mp\parachute::f14_current_inputamt() );
        var_4 = var_4 + var_10;
    }

    var_3 = scripts\mp\gametypes\br_c130.gsc::registerhandlecommand( var_4, var_6 );
    return var_3;
}
...
br_delaynojip( var_0, var_1, var_2, var_3 )
{
    ...
    var_4.little_bird_mg_initspawning = var_1;
    var_4.speed = var_2;
    var_4.lifetime = var_3;
    var_4.brneverlanded = spawn( "script_model", var_4.startpt );
    var_4.brneverlanded setmodel( "veh8_mil_air_acharlie130_magma_rigid" );
    var_4.brneverlanded linkto( var_4, "", ( 0, 0, 0 ), ( 0, 0, 0 ) );
    ...
    level.br_domheight[level.br_domheight.size] = var_4;
    return var_4;
}

br_displayperkinfo( var_0, var_1, var_2, var_3 )
{
    self setscriptablepartstate( "audio_lp_dmz", "on", 0 );
    self moveto( self.endpt, self.lifetime );
    thread br_circle_closing_music();
    thread br_circle_init_func( var_0, var_1, var_2, var_3 );
}
...
br_circle_closing_music()
{
    ...
    wait 1;
    level.br_domheight = scripts\engine\utility::array_remove( level.br_domheight, self );
    self setscriptablepartstate( "audio_lp_dmz", "off", 0 );

    if ( isdefined( self.minimapid ) )
        scripts\mp\objidpoolmanager::returnobjectiveid( self.minimapid );

    if ( isdefined( self.brneverlanded ) )
        self.brneverlanded delete();

    if ( isdefined( self ) )
        self delete();
}

br_circle_init_func( var_0, var_1, var_2, var_3 )
{
    self endon( "death" );

    if ( isdefined( self.deadblue ) )
        self [[ self.deadblue ]]( var_0, var_1, var_2, var_3 );
    else
    {
        var_4 = self.lifetime;
        var_5 = var_4 * 0.4;
        ...
        while ( var_12 < var_0 )
        {
            ...
            var_13 = br_circlecenters( self.origin + anglestoforward( self.angles ) * 500 );
            ...
            var_14 = scripts\cp_mp\killstreaks\airdrop::deactivate_front_trigger_hurt( var_13 + ( 0, 0, level.br_circleshowdelaysafe - 100 ), var_13, self.angles, var_1, var_2 );
            ...
            var_15.showmaphinttilused = 5;
            level.br_ending[level.br_ending.size] = var_14;
        }
    }
}
...
br_circlecenters( var_0, var_1 )
{
    ...
    {
        var_7 = scripts\mp\gametypes\br_c130.gsc::ispointinbounds( var_2 ) && !br_clearinventory( var_2 );
...
br_clearinventory( var_0 )
{
    var_1 = 0;
    var_2 = level.br_ending;
...
br_danger_circle_closing_music( var_0 )
{
    ...
    if ( !scripts\mp\gametypes\br_public.gsc::has_focus_fire_headicon() && getdvarint( "scr_dmz_airdrop_drop_tablet", 0 ) == 1 )
    {
    ...
    if ( isdefined( level.ai_ascender_animout ) )
        var_5 = level.ai_ascender_animout;

    if ( istrue( level.audio_heli_fade_up ) )
        var_5 = int( var_5 * level.lightsreset );

    var_3 = scripts\mp\gametypes\br_plunder.gsc::dropplunderbyrarity( var_5, 10 );
    var_2 = scripts\engine\utility::array_combine( var_2, var_3 );

    foreach ( var_7 in var_2 )
        var_7.init_turrets = "c130_box";
