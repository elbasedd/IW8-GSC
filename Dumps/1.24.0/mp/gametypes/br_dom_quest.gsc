...
init()
{
    ...
    scripts\mp\gametypes\br_quest_util.gsc::onunlockitem( "domination", ::custom_damageshield );
    ...
    scripts\mp\gametypes\br_quest_util.gsc::onspecialistbonusavailable( "dom_locale", ::currentvalue );
    scripts\mp\gametypes\br_quest_util.gsc::onstim( "dom_locale", ::currlocation );
    scripts\mp\gametypes\br_quest_util.gsc::getquestdata( "dom_locale" ).nextid = 0;
    scripts\mp\gametypes\br_quest_util.gsc::num_nags( "domination", 1 );
    scripts\mp\gametypes\br_quest_util.gsc::onstompeenemyprogressupdate( "domination", ::currentmultikill );
...
domquest_removequestinstance()
{
    ...
    self.rewardtotype thread currenttime();
}
...
custom_damageshield()
{
    var_0 = flashentercode( self );
    var_1 = scripts\mp\gametypes\br_quest_util.gsc::display_ready_for_sequence( "domination", var_0 );
...
domlocale_createquestlocale( var_0 )
{
    ...
    var_3 = scripts\mp\gametypes\br_quest_util.gsc::nuke_vault_suicidebomber_internal( var_0 );
    var_4 = spawn( "trigger_radius", var_2, 0, int( var_3 ), int( level.blinking_light_thread ) );
...
domlocale_circletick( var_0, var_1 )
{
    ...
    if ( var_3 > var_1 )
    {
        foreach ( var_5 in self.subscribedinstances )
        {
            ...
            level thread scripts\mp\gametypes\br_public.gsc::bot_gametype_human_player_always_defender( "mission_obj_circle_fail", var_5.team, 1 );
            var_5.result = "circle";
        }
...
takequestitem( var_0 )
{
    ...
    var_1 scripts\mp\gametypes\br_quest_util.gsc::num_nodes_random_search( var_2, 4 );
    var_3 = flashentercode( var_0 );
    ...
    scripts\mp\gametypes\br_quest_util.gsc::reset_restock_flag( "domination", self, self.team );
    var_5 = spawnstruct();
    var_5.excludedplayers = [];
    var_5.excludedplayers[0] = self;
    var_5.monitoringimpact = scripts\mp\gametypes\br_quest_util.gsc::fix_door_clip( "domination", scripts\mp\gametypes\br_quest_util.gsc::flag_bot_defender_limit_for_team( self.team ) );
    ...
    scripts\mp\gametypes\br_quest_util.gsc::crushing_players( var_1.team, self, 6, scripts\mp\gametypes\br_quest_util.gsc::getquestindex( "domination" ) );
    level thread scripts\mp\gametypes\br_public.gsc::bot_gametype_human_player_always_defender( "mission_dom_accept", var_1.team, 1 );
}
...
precomputeddropbagpos()
{
    if ( isdefined( level.blinking_light_thread ) )
        return;

    level.disableinitplayergameobjects = 0;
    level.blinking_light_thread = 120;
    ...
    level.getallspawninstances = "icon_waypoint_ot";
...
domflag_onuseupdate( var_0, var_1, var_2, var_3 )
{
    if ( var_1 < 1.0 && !level.gameended )
        make_intel_model_usable( var_1, var_0 );

    if ( var_1 > 0.05 && var_2 && !istrue( self.didstatusnotify ) )
        self.didstatusnotify = 1;
}

currentrewarddropindex( var_0 )
{
    if ( !isdefined( self.large_transport_initomnvars ) || !self.large_transport_initomnvars )
    {
        self.large_transport_initomnvars = 1;
        level thread scripts\mp\gametypes\br_quest_util.gsc::showquestcircletoall( self.curorigin, "dom" );
...
domflag_onuse( var_0 )
{
    foreach ( var_2 in self.locale.subscribedinstances )
    {
        ...
        {
            var_3 = spawnstruct();
            var_4 = scripts\mp\gametypes\br_quest_util.gsc::flag_bot_defender_limit_for_team( var_0.team );
            var_5 = scripts\mp\gametypes\br_quest_util.gsc::getquestindex( "domination" );
            var_6 = scripts\mp\gametypes\br_quest_util.gsc::fixuppickuporigin( scripts\mp\gametypes\br_quest_util.gsc::fixsuperforbr( "domination" ) );
            var_3.linker = scripts\mp\gametypes\br_quest_util.gsc::linktoplayer_linklogic( var_5, var_4, var_6 );
            scripts\mp\gametypes\br_quest_util.gsc::displayteamsplash( var_2.team, "br_domination_quest_complete", var_3 );
            scripts\mp\gametypes\br_quest_util.gsc::crushing_players( var_2.team, var_0, 8, var_5 );
            level thread scripts\mp\gametypes\br_public.gsc::bot_gametype_human_player_always_defender( "mission_dom_success", var_2.team, 1, 1 );
            var_2.place_weapons_at_pos = self.flagmodel.origin;
            var_2.place_traversal_badplace = self.flagmodel.angles;
            ...
            if ( isdefined( self.assisttouchlist[var_2.team] ) )
            {
                ...
                foreach ( var_9 in var_7 )
                {
                    ...
                    var_2 scripts\mp\gametypes\br_quest_util.gsc::onplayerrespawn( var_10 );
                }
            }
        ...
        level thread scripts\mp\gametypes\br_public.gsc::bot_gametype_human_player_always_defender( "mission_gen_fail", var_2.team, 1 );
        var_2.result = "fail";
    }
...
currentmultikill()
{
    while ( self.rewardtotype.domflag.numtouching[self.id] )
        waitframe();
...
flashentercode( var_0 )
{
    var_1 = spawnstruct();
    var_1.playergetspawnpoint = "questPointsArray";
    var_1.playergetbestrespawnmissionorigin = ( var_0.origin[0], var_0.origin[1], 0 );
    var_1.playergetzombiespawnlocation = 12000;
    var_1.playergoingintols = 0;
    var_1.playergetspectatingplayer = 8000;
    var_1.playergettotaltime = 6000;
    var_1.num_of_frame_frozen = 7;
    ...
    {
        if ( var_1.playergetzombiespawnlocation < level.nuke_vault_oilfire_vfx_end_on_death )
            var_1.playergetzombiespawnlocation = level.nuke_vault_oilfire_vfx_end_on_death;

        var_1.playergetspectatingplayer = level.nuke_vault_oilfire_vfx_end_on_death;
        var_1.playergettotaltime = level.nuke_vault_oilfire_vision;
    }
