...
updategametypedvars()
{
    ...
    level.little_bird_trail = scripts\mp\utility\dvars::dvarintvalue( "pauseTime", 1, 0, 1 );
...
hardpointmainloop()
{
    ...
    scripts\mp\utility\game::init_reach_wind_room( undefined, undefined, "New Hardpoint", level.zone.trigger.origin );
    level.zone thread scripts\common\utility::set_showing_bomb_wire_pair_to_player( level.inside_bush, "hill_moved", level.zone.trigger.origin );
    scripts\mp\flags::gameflagwait( "prematch_done" );

    if ( level.little_bird_trail )
    ...
    if ( level.firstzoneactivationdelay )
    {
        ...
        level.supportbox_playusesound = 1;
        level thread scripts\mp\hud_message::updatematchstatushintforallplayers( undefined, 36, 36 );
        level thread waitthenplaynewobj();
        level.zone thread scripts\mp\gametypes\obj_zonecapture.gsc::init_range_targets( level.firstzoneactivationdelay, level.zone.curorigin + level.zone.offset3d );
        wait( level.firstzoneactivationdelay );
        level.supportbox_playusesound = 0;
...
overrideviewkickscalek98()
{
    switch ( level.mapname )
    {
        ...
        case "mp_hackney_am":
            level.overtime_music = [ "1", "4", "3", "9", "8" ];
            break;
        case "mp_spear_pm":
        case "mp_spear":
            level.overtime_music = [ "5", "4", "3", "2", "6" ];
            break;
        case "mp_cave":
        case "mp_cave_am":
            level.overtime_music = [ "1", "5", "2", "3", "4" ];
            break;
        case "mp_petrograd":
            level.overtime_music = [ "1", "9", "4", "5", "2" ];
            break;
        case "mp_deadzone":
            level.overtime_music = [ "12", "11", "3", "1", "2" ];
            break;
        case "mp_raid":
            level.overtime_music = [ "20", "2", "8", "9", "5" ];
            break;
        case "mp_piccadilly":
            level.overtime_music = [ "1", "5", "7", "3", "8" ];
            break;
        case "mp_crash2_pm":
        case "mp_crash2":
            level.overtime_music = [ "1", "20", "5", "4", "19" ];
            break;
        case "mp_emporium":
            level.overtime_music = [ "1", "2", "3", "4", "5" ];
            break;
        default:
            break;
    }
...
getfirstzone()
{
    if ( isdefined( level.overtime_music ) )
    {
        var_0 = level.objectives[level.overtime_music[0]];
...
getnextzone()
{
    ...
    else if ( isdefined( level.overtime_music ) )
    {
        level.prevzoneindex++;

        if ( level.prevzoneindex > level.overtime_music.size - 1 )
            level.prevzoneindex = 0;

        var_23 = level.objectives[level.overtime_music[level.prevzoneindex]];
    }
    else
    {
        level.prevzoneindex++;
...
hqactivatenextzone( var_0, var_1 )
{
    ...
    else if ( level.zoneactivationdelay )
    {
        ...
        level.supportbox_playusesound = 1;
        level thread scripts\mp\hud_message::updatematchstatushintforallplayers( undefined, 36, 36 );
        level.zone thread scripts\mp\gametypes\obj_zonecapture.gsc::init_range_targets( level.zoneactivationdelay, level.zone.curorigin + level.zone.offset3d );
...
hpcaptureloop()
{
    ...
    for (;;)
    {
        ...
        var_0 = level scripts\engine\utility::spin_light( "zone_captured", "zone_destroyed" );
...
initspawns()
{
    ...
    if ( !isdefined( level.spawnpoints ) || istrue( level.add_spawn_disable_struct ) )
    {
    ...
    if ( istrue( level.add_spawn_disable_struct ) )
        return;
...
getspawnpoint()
{
    ...
    {
        if ( !isdefined( level.rooftop_crate_usefunc ) )
        {
            level.rooftop_crate_usefunc = 1;
...
give_capture_credit( var_0, var_1, var_2, var_3, var_4 )
{
    level endon( "game_ended" );
    level.lastcaptime = gettime();
    scripts\mp\utility\game::init_reach_wind_room( var_4, undefined, "Hardpoint Captured", var_4.origin );
    var_5 = var_4;
    ...
    if ( isplayer( var_5 ) )
    {
        var_5 thread scripts\common\utility::set_showing_bomb_wire_pair_to_player( level.inside_bush, "capture", var_5.origin );

        if ( has_sequence_been_entered_correctly( var_5 ) && !scripts\mp\utility\game::isanymlgmatch() )
        {

        }
        else if ( !isscoreboosting( var_5 ) )
...
updatematchstatushintonspawn()
{
    ...
    if ( istrue( level.supportbox_playusesound ) )
