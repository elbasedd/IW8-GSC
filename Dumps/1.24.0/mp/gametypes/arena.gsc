...
main()
{
    ...
    if ( scripts\mp\utility\game::matchmakinggame() )
    {
        ...
        level.pavelow_boss_explodes = 1;
        level.requiredplayercount["allies"] = var_1;
        level.requiredplayercount["axis"] = var_1;
    }

    scripts\mp\utility\join_team_aggregator::registeronplayerjointeamcallback( ::onjoinedteamcb );
    ...
    level.anglesoffset = getknivesoutsetting();

    if ( level.anglesoffset )
    {
        level.dwell_aggro = 0.5;
        level.dyn_door = 0.05;
    }
    ...
    {
        level.queue_spawn = 1;
        level.airstrike_watchdeployweaponchange = getdvarint( "scr_always_snowfight", 0 );

        if ( level.airstrike_watchdeployweaponchange )
    ...
    {
        level.anim_offset = 1;
        level.relic_amped_reset_deathshield_on_revived = 1;
    }
    else if ( getdvarint( "scr_arena_stpday" ) == 1 )
    {
        level.anim_guy_keep_hidden = 1;
        level.relic_amped_reset_deathshield_on_revived = 1;
    }
...
onstartgametype()
{
    ...
    {
        if ( !level.airstrike_watchdeployweaponchange && game["roundsPlayed"] == 0 && istrue( game["practiceRound"] ) )
        {
            level.extratime = 30;
            game["didSnowFight"] = 1;
        ...
        else if ( level.airstrike_watchdeployweaponchange )
        {
            level.arenaloadouts = 3;
            var_5 = 0;
        }

        level.quickdropaddtoexisting["vanish"] = loadfx( "vfx/core/impacts/small_snowhit" );
        level.quickdropaddtoexisting["screen"] = loadfx( "vfx/iw8/weap/_impact/snowball/vfx_imp_snowball_scrn.vfx" );
        queue_spawn();
    }

    if ( var_5 )
    {
        if ( israndomloadouts() || israndompreviewloadouts() || heavydamageaward() || israndomcustomblueprintsloadouts() )
        ...
        else if ( ispickuploadouts() )
        {
            ...
            if ( !isdefined( game["roundsPlayed"] ) || isdefined( game["roundsPlayed"] ) && game["roundsPlayed"] == 0 )
            {
                ...
                level.startweapon.variantid = eyeonnotehandler( level.startweapon.weapon );
                level.arenaweapont1.variantid = eyeonnotehandler( level.arenaweapont1.weapon );
                level.arenaweapont2.variantid = eyeonnotehandler( level.arenaweapont2.weapon );
                level.arenaweapont3.variantid = eyeonnotehandler( level.arenaweapont3.weapon );
                level.arenaweapont4.variantid = eyeonnotehandler( level.arenaweapont4.weapon );
                level.arenaweapont5.variantid = eyeonnotehandler( level.arenaweapont5.weapon );
                level.arenaweapont6.variantid = eyeonnotehandler( level.arenaweapont6.weapon );
                level.arenaweapont7.variantid = eyeonnotehandler( level.arenaweapont7.weapon );
                level.arenaweapont8.variantid = eyeonnotehandler( level.arenaweapont8.weapon );
            }
    ...
    if ( istrue( game["pingEnabled"] ) )
        scripts\cp_mp\challenges::calloutmarkerping_init();
}
...
updategametypedvars()
{
    ...
    level.arenaloadouts = scripts\mp\utility\dvars::dvarintvalue( "arenaLoadouts", 1, 1, 15 );
    ...
    level.arenapickupattachments = getdvarint( "scr_arena_arenaAttachments", 0 );
}
    ...
    if ( scripts\mp\spawnlogic::shoulduseteamstartspawn() || istrue( level.alwaysusestartspawns ) || isintournament() )
    {
        ...
        if ( !isdefined( var_0 ) )
        {
            if ( istrue( self.switching_teams_arena ) && isdefined( self.pers["arena_spawn_pos"] ) )
            {
                ...
                if ( isdefined( var_0 ) )
                    self.release_player_on_damage = var_0.angles;
            }
            else
            {
                var_0 = getspawnpoint_startspawn( var_3 );
            ...
            if ( !isdefined( var_0 ) )
            {
                ...
                if ( var_4 > 3 )
                {
                    ...
                    foreach ( var_6 in scripts\mp\utility\teams::getteamdata( self.pers["team"], "players" ) )
                    {
                        if ( istrue( var_6.bot_loadout_team ) )
                        {
                            var_9++;
                            continue;
                        }
                    ...
                    if ( isdefined( var_8 ) )
                    {
                        ...
                        if ( isdefined( var_8.release_player_on_damage ) )
                            var_0.angles = var_8.release_player_on_damage;
                    }
                }
            }
...
onplayerconnect( var_0 )
{
    ...
    if ( !isnormalloadouts() )
    {
        ...
        foreach ( var_2 in level.teamnamelist )
        {
            if ( isdefined( var_0.pers["team"] ) && game["roundWinStreak"][var_2] > 0 && var_0.pers["team"] == var_2 )
            {
                ...
                var_0.pers["gamemodeLoadout"]["roundWinStreakPrimaryCamo"] = setroundwinstreakspecialcamos();
                var_0.pers["gamemodeLoadout"]["roundWinStreakecondaryCamoTeam"] = var_2;
                var_0.pers["gamemodeLoadout"]["roundWinStreakSecondaryCamo"] = setroundwinstreakspecialcamos();
            }
        }
    }
    ...
    if ( istrue( game["pingEnabled"] ) )
        var_0 scripts\cp\so_globallogic::calloutmarkerping_initplayer();
}

onplayerdisconnect( var_0 )
{
    poke_the_player_after_faux_death( 1 );
...
onjoinedteam()
{
    ...
    for (;;)
    {
        scripts\engine\utility::spectatecommands( "joined_team", "joined_spectators" );
...
onspawnfinished()
{
    ...
    if ( istrue( game["practiceRound"] ) || istrue( level.airstrike_watchdeployweaponchange ) )
        thread quickdropaddtocache();
    ...
    if ( !scripts\mp\flags::gameflag( "prematch_done" ) && game["roundsPlayed"] == 0 )
    {
        ...
        level scripts\engine\utility::spectatecommands( "prematch_done", "removeArenaOutlines" );
    }

    self setclientomnvar( "ui_player_notify_loadout", gettime() );
}

poke_the_player_after_faux_death( var_0 )
{
...
onplayerdamaged( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9, var_10, var_11, var_12, var_13 )
{
    ...
    {
        if ( istrue( level.queue_spawn ) && ( var_6.basename == "snowball_mp" || var_6.basename == "pball_mp" ) )
            playfxontagforclients( level.quickdropaddtoexisting["screen"], var_2, "tag_eye", var_2 );
...
damagewatcher()
{
    ...
    for (;;)
    {
        scripts\engine\utility::spectatetestonprematchfadedone( "damage", "force_regeneration", "removeAdrenaline", "healed", "healhRegenThink", "vampirism", "spawned_player" );
...
buildloadoutsforweaponstreaming()
{
    ...
    if ( ispickuploadouts() )
    {
        ...
        else
        {
            ...
            if ( istrue( level.relic_amped_reset_deathshield_on_revived ) )
            {
            ...
            if ( headicon_time_left() )
                var_1 = scripts\mp\class::bot_should_cap_next_zone( var_2, var_4, var_5, level.startweapon.variantid, var_7, var_8, var_9 );
            else
...
pressure_stability_event_init()
{
    level.arenaweapont1 = bot_shotguns( level.arenaweapont1 );
    level.arenaweapont2 = bot_shotguns( level.arenaweapont2 );
    level.arenaweapont3 = bot_shotguns( level.arenaweapont3 );
    level.arenaweapont4 = bot_shotguns( level.arenaweapont4 );
    level.arenaweapont5 = bot_shotguns( level.arenaweapont5 );
    level.arenaweapont6 = bot_shotguns( level.arenaweapont6 );
    level.arenaweapont7 = bot_shotguns( level.arenaweapont7 );
    level.arenaweapont8 = bot_shotguns( level.arenaweapont8 );
}
...
bot_shotguns( var_0 )
{
    ...
    if ( !issubstr( var_0.weapon, "equip" ) && var_0.weapon != "none" )
    {
        ...
        var_2 = flightyaw();
        var_0.brcirclecoughnexttime = var_2;

        if ( shut_down_station( var_0.weapon ) )
        {
            var_6 = flip_target();
            var_0.change_yaw_angle = var_6;
        }

        if ( shutdownattractionicontrigger( var_0.weapon ) )
        {
            var_7 = flip_time();
            var_0.resetstuckthermite[0] = var_7[0];
            var_0.resetstuckthermite[1] = var_7[1];
            var_0.resetstuckthermite[2] = var_7[2];
            var_0.resetstuckthermite[3] = var_7[3];
        }
...
pre_laserpanel_weapon()
{
    level.arenaweapont1 = bot_nags( level.arenaweapont1 );
    level.arenaweapont2 = bot_nags( level.arenaweapont2 );
    level.arenaweapont3 = bot_nags( level.arenaweapont3 );
    level.arenaweapont4 = bot_nags( level.arenaweapont4 );
    level.arenaweapont5 = bot_nags( level.arenaweapont5 );
    level.arenaweapont6 = bot_nags( level.arenaweapont6 );
    level.arenaweapont7 = bot_nags( level.arenaweapont7 );
    level.arenaweapont8 = bot_nags( level.arenaweapont8 );
}
...
bot_nags( var_0 )
{
    ...
    if ( !issubstr( var_0.weapon, "equip" ) && var_0.weapon != "none" )
    {
        ...
        var_7[3] = "none";
        var_8 = scripts\mp\utility\weapon::getweaponrootname( var_0.weapon );
        var_0.weaponobj = scripts\mp\class::bot_should_cap_next_zone( var_8, undefined, undefined, var_4, undefined, undefined, scripts\cp_mp\utility\game_utility::isnightmap() );
    }
...
setupweaponattachmentoverrides( var_0 )
{
    ...
    if ( !issubstr( var_0.weapon, "equip" ) && var_0.weapon != "none" )
    {
        ...
        var_1 = getarenapickupattachmentoverrides( var_0.weapon + "_mp" );
...
{
    var_1 = getarenapickupattachmentoverrides( var_0 + "_mp" );
    return var_1[0];
}
...
defineplayerloadout()
{
    ...
    var_6[2] = "none";
    var_6[3] = "none";
    var_7 = [ "specialty_blastshield" ];
    var_8 = "none";
    var_9 = "none";

    if ( isdefined( level.startweapon.weapon ) && level.startweapon.weapon != "none" && !issubstr( level.startweapon.weapon, "equip" ) )
    {
        var_10 = scripts\mp\utility\weapon::getweaponrootname( level.startweapon.weapon );

        if ( headicon_time_left() )
            var_3 = level.startweapon.variantid;
        ...
        if ( istrue( level.relic_amped_reset_deathshield_on_revived ) )
        {
            ...
            if ( shut_down_station( level.startweapon.weapon ) )
            {
                var_5 = flip_target();
                game["arenaRandomLoadout"][0]["loadoutPrimaryCosmeticAttachment"] = var_5;
            }

            if ( shutdownattractionicontrigger( level.startweapon.weapon ) )
            {
                var_6 = flip_time();
            ...
            var_0[0] = getattachmentoverride( level.startweapon.weapon );
    }
    else
    {
        var_10 = "none";
        ...
        {
            if ( level.startweapon.weapon == "equip_throwing_knife" || level.startweapon.weapon == "equip_throwing_knife_fire" )
            {
                var_7[var_7.size] = "specialty_extra_deadly";

                if ( level.anglesoffset )
                {
                    var_7[var_7.size] = "specialty_restock";

                    if ( level.anglesoffset == 2 )
                    {
                        var_11 = scripts\engine\utility::ter_op( heavydamageaward(), "mp/classtable_arena_blueprints.csv", "mp/classTable_arena.csv" );
                        var_3 = int( scripts\mp\gametypes\gun.gsc::farms2_gw_ambient_sound_load( "iw8_knife" ) );

                        if ( var_3 != 0 )
                            setomnvar( "ui_arena_primaryVariantID", var_3 );
            ...
            var_12 = scripts\mp\equipment::getdefaultslot( level.startweapon.weapon );

            if ( var_12 == "primary" )
                var_8 = level.startweapon.weapon;
            else
    ...
    level.arena_loadouts["default"]["loadoutPrimary"] = var_10;
    level.arena_loadouts["default"]["loadoutPrimaryAttachment"] = var_0[0];
    level.arena_loadouts["default"]["loadoutPrimaryAttachment2"] = var_0[1];
    level.arena_loadouts["default"]["loadoutPrimaryAttachment3"] = var_0[2];
    level.arena_loadouts["default"]["loadoutPrimaryAttachment4"] = var_0[3];
    level.arena_loadouts["default"]["loadoutPrimaryAttachment5"] = var_0[4];
    ...
    level.arena_loadouts["default"]["loadoutPrimaryVariantID"] = var_3;
    ...
    level.arena_loadouts["default"]["loadoutEquipmentPrimary"] = var_8;
    level.arena_loadouts["default"]["loadoutEquipmentSecondary"] = var_9;
    ...
    level.arena_loadouts["default"]["loadoutPerks"] = var_7;
    ...
    if ( !istrue( level.relic_amped_reset_deathshield_on_revived ) )
    {
        ...
        {
            level.arena_loadouts[var_14]["loadoutPrimaryCamo"] = setroundwinstreakspecialcamos();
            level.arena_loadouts[var_14]["loadoutSecondaryCamo"] = setroundwinstreakspecialcamos();
        }
    }
}
...
cacherandomloadouts()
{
    ...
    var_0 = [];
    var_1 = 0;
    var_2 = scripts\engine\utility::ter_op( heavydamageaward(), "mp/classtable_arena_blueprints.csv", "mp/classTable_arena.csv" );
    ...
    if ( israndompreviewloadouts() )
        var_1 = 144;
    else if ( heavy_enemy_spawning_pause_monitor() )
        var_1 = 0;
    else if ( heavystatehealthadd() )
        var_1 = 43;
    else if ( heavydamagescoreevent() )
        var_1 = 71;
    else if ( heavydamagescorelaunchonly() )
        var_1 = 101;
    else if ( heli_activate_instruct() )
        var_1 = 121;
    else if ( heavydamageawardlaunchonly() )
        var_1 = 149;
    ...
    while ( scripts\mp\class::table_getloadoutname( var_2, var_1 ) != "" )
    {
        ...
        if ( israndomloadouts() || heavydamageaward() || israndomcustomblueprintsloadouts() )
        {
            if ( var_1 == 144 )
                break;
        }
    ...
    if ( var_5 != "" && var_6 > 0.0 )
    {
        ...
        foreach ( var_9 in var_7 )
        {
            ...
            if ( var_10.size == 2 )
            {
                var_11 = var_10[0];
                var_12 = int( var_10[1] );
                var_13 = scripts\mp\utility\weapon::stream_time( var_11, var_12 );
                ...
                if ( var_13 )
                {
                    ...
                    var_14 = spawnstruct();
                    var_14.play_death_sound_with_global_cooldown = var_11;
...
{
    var_2 = [];
    var_2["loadoutPrimaryAddBlueprintAttachments"] = scripts\mp\class::rotationrefsbyseatandweapon( var_0, var_1, 0 );
    ...
    if ( heavydamageaward() || israndomcustomblueprintsloadouts() )
        var_2["loadoutPrimaryVariantID"] = scripts\mp\class::round_at_max( var_0, var_1, 0, var_2["loadoutPrimary"] );
    else
    ...
    var_2["loadoutSecondaryAddBlueprintAttachments"] = scripts\mp\class::rotationrefsbyseatandweapon( var_0, var_1, 1 );
    ...
    if ( heavydamageaward() || israndomcustomblueprintsloadouts() )
        var_2["loadoutSecondaryVariantID"] = scripts\mp\class::round_at_max( var_0, var_1, 1, var_2["loadoutSecondary"] );
    else
        ...
        if ( isdefined( var_2 ) && isdefined( var_3 ) )
        {
            ...
            if ( var_10.size > 0 )
            {
                if ( randomfloat( 1.0 ) < var_3 )
                {
                    ...
                    var_7["loadoutPrimary"] = var_12.play_death_sound_with_global_cooldown;
...
angvels( var_0, var_1 )
{
    ...
    foreach ( var_4 in var_0 )
    {
        if ( arenaloadouts_getweapongroup( var_4.play_death_sound_with_global_cooldown ) == var_1 )
...
{
    if ( headicon_time_left() && !issubstr( var_0, "equip" ) && var_0 != "none" )
    {
        ...
        if ( var_4 != "" )
        {
            var_5 = scripts\mp\class::round_at_max( "mp/classtable_arena_blueprints.csv", var_2, 0, var_4 );
            return var_5;
        }
...
spawnweapon( var_0, var_1, var_2 )
{
    ...
    if ( var_9 != "" )
    {
        if ( istrue( level.queue_spawn ) && var_3 == "equip_snowball" )
        {
            if ( isplayer( var_0 ) )
                var_8 = "single";
            ...
            if ( !isplayer( var_0 ) && var_0.script_label == "6" && !level.lock_player_stance )
            {
                if ( distance( var_0.origin, ( 0, 0, 138 ) ) < 10 )
                {
                    level.lock_player_stance = 1;
                    var_3 = "equip_pball";
                    var_8 = "single";
                }
            }
            else if ( !isplayer( var_0 ) && var_0.script_label == "8" && !level.lock_player_stance )
            {
        ...
        var_10.notify_planter_on_whizby = spawn( "trigger_radius", var_10.origin, 0, 32, 32 );
        var_10 thread helibankplunder();

        if ( istrue( level.queue_spawn ) )
        {
            var_10 thread startplayer( var_0, var_8 );
    ...
    else
    {
        ...
        if ( istrue( level.relic_amped_reset_deathshield_on_revived ) || headicon_time_left() )
            var_17 = var_1.weaponobj;
        else
        {
            var_16 = getarenapickupattachmentoverrides( var_3 + "_mp" );
        ...
        if ( isdefined( var_0.script_noteworthy ) && var_0.script_noteworthy == "wall" )
        {
            ...
            var_19.binoculars_onstateunmarkedupdate = var_22;
        }
...
helibankplunder()
{
    ...
    {
        wait 0.05;
        self.notify_planter_on_whizby waittill( "trigger", var_1 );
...
deleteotpreview()
{
    level scripts\engine\utility::spectatecommands( "prematch_done", "start_mode_setup" );
    disableotflag();
}
...
giveflagcapturexp( var_0, var_1 )
{
    ...
    if ( isplayer( var_2 ) )
    {
        ...
        var_2 thread scripts\common\utility::set_showing_bomb_wire_pair_to_player( level.inside_bush, "capture", var_2.origin );
    }
...
propaddtocircle()
{
    ...
    if ( istrue( level.arenaflag.modeplayerskipdialog ) )
        return;
...
getoutlineasset( var_0, var_1 )
{
    ...
    if ( istrue( level.queue_spawn ) && isdefined( var_1 ) && var_1 == "equip_pball" )
        var_2 = "outline_depth_yellow";
...
heavydamagescoreevent()
{
    return level.arenaloadouts == 9;
}

heavydamagescorelaunchonly()
{
    return level.arenaloadouts == 10;
}

heli_activate_instruct()
{
    return level.arenaloadouts == 11;
}

heavydamageawardlaunchonly()
{
...
queue_spawn()
{
    ...
    initweaponmap();
    thread pressure_overload_threshold();
}

pressure_overload_threshold()
{
    level.arenaweapont1.weapon = "equip_snowball";

    if ( !istrue( level.airstrike_watchdeployweaponchange ) && !istrue( game["practiceRound"] ) )
    {
    ...
    level.lock_player_stance = 0;
    var_0 = scripts\engine\utility::getstructarray( "weapon_pickup", "targetname" );

    foreach ( var_2 in var_0 )
    {
        if ( istrue( level.airstrike_watchdeployweaponchange ) || istrue( game["practiceRound"] ) || level.arenaloadouts != 3 )
        {
            spawnweapon( var_2, level.arenaweapont1 );
            continue;
        }
...
startplayer( var_0, var_1 )
{
    ...
    else
    {
        ...
        playfx( level.quickdropaddtoexisting["vanish"], self.origin );
        clearweaponoutlines();

        if ( !isplayer( var_0 ) )
            level thread spawnvehicles( var_0 );
...
spawnvehicles( var_0 )
{
    level endon( "game_ended" );
    wait 15;
    playfx( level.quickdropaddtoexisting["vanish"], var_0.origin );
...
{
    if ( var_0 == "equip_throwing_knife" || var_0 == "equip_throwing_knife_fire" )
        return 1;
    else
        return 0;
}
