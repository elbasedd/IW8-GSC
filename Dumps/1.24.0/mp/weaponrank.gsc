...
getweaponrankxpmultipliertotal()
{
    ...
    var_2 = farah_says_cut_wire_green();
    var_3 = first_interaction();
    return var_0 * var_1 * var_2 * var_3;
}

farah_says_cut_wire_green()
{
    if ( !isdefined( level.eggsetup ) )
        level.eggsetup = float( tablelookup( "mp/gametypesTable.csv", 0, scripts\mp\utility\game::getgametype(), 19 ) );

    return level.eggsetup;
}

farah_nobraids_body()
{
    if ( !isdefined( level.eggdebug ) )
        level.eggdebug = int( tablelookup( "mp/gametypesTable.csv", 0, scripts\mp\utility\game::getgametype(), 20 ) );

    return level.eggdebug;
}

farah_nobraids_body_reset()
{
    if ( !isdefined( level.eggrolls ) )
    {
        ...
        level.eggrolls = farah_nobraids_body() / 60 * ( var_0 / 60 );
    }

    return level.eggrolls;
}
