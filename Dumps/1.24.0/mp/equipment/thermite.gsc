// IW8 GSC SOURCE
// Generated by https://github.com/xensik/gsc-tool

thermite_used( var_0, var_1 )
{
    if ( isdefined( var_1 ) )
    {
        var_2 = var_0;
        var_0 = scripts\mp\utility\weapon::_launchgrenade( "thermite_mp", var_2.origin, ( 0, 0, 0 ) );
        scripts\mp\weapons::grenadeinitialize( var_0, makeweapon( "thermite_mp" ) );
        var_0.freight_lift_waittill_occupied = var_2;
        var_0.angles = var_2.angles;
        var_0.freight_lift_waittill_progress = self getcurrentweapon();
        var_0 linkto( var_2 );
        var_0.exploding = 1;
        var_0 setscriptablepartstate( "visibility", "hide", 0 );
    }

    var_0 thread thermite_watchdisowned();
    var_0 thread thermite_watchstuck( var_1 );
}

thermite_watchstuck( var_0 )
{
    self endon( "death" );
    var_1 = undefined;

    if ( istrue( var_0 ) )
    {
        var_2 = scn_infil_hackney_heli_npc2();

        if ( !istrue( var_2 ) )
        {
            thread thermite_delete();
            return;
        }

        if ( isdefined( self.freight_lift_waittill_occupied ) )
            self.freight_lift_waittill_occupied delete();
    }
    else
        self waittill( "missile_stuck", var_1 );

    self.owner endon( "disconnect" );
    self.owner endon( "joined_team" );
    self.owner endon( "joined_spectators" );

    if ( !istrue( var_0 ) )
    {
        if ( isdefined( var_1 ) && isplayer( var_1 ) )
            thread scripts\mp\weapons::grenadestuckto( self, var_1 );
    }

    var_3 = self.weapon_object;

    if ( isdefined( self.freight_lift_waittill_progress ) )
        var_3.little_bird_mg_enterend = self.freight_lift_waittill_progress;

    thread scn_infil_hackney_heli_npc3();
    self setscriptablepartstate( "effects", "impact", 0 );
    school_guards_rpg_shoot_into_windows( var_3 );

    if ( !istrue( level.cleanupkeybindings ) )
        self.dangerzoneid = scripts\mp\spawnlogic::addspawndangerzone( self.origin, 175, 175, self.owner.team, 100, self.owner, 1, self, 1 );

    wait 0.5;
    var_4 = makeweapon( "thermite_av_mp" );
    var_5 = makeweapon( "thermite_ap_mp" );

    if ( isdefined( self.freight_lift_waittill_progress ) )
    {
        var_4.little_bird_mg_enterend = self.freight_lift_waittill_progress;
        var_5.little_bird_mg_enterend = self.freight_lift_waittill_progress;
    }

    var_6 = 1;

    while ( var_6 <= 10 )
    {
        var_7 = var_6 + 1;

        if ( scripts\engine\utility::mod( var_7, 2 ) > 0 )
            school_guards_rpg_shoot_into_windows( var_4 );
        else
            school_guards_rpg_shoot_into_windows( var_5 );

        var_6 = var_7;
        wait 0.5;
    }

    thread thermite_destroy();
}

scn_infil_hackney_heli_npc2()
{
    self.freight_lift_waittill_occupied endon( "death" );
    self.owner endon( "disconnect" );
    self.owner endon( "joined_team" );
    self.owner endon( "joined_spectators" );
    self.freight_lift_waittill_occupied waittill( "missile_stuck", var_0, var_1, var_2, var_3, var_4, var_5 );

    if ( isdefined( var_0 ) )
    {
        if ( isplayer( var_0 ) || isagent( var_0 ) )
        {
            if ( var_0 scripts\cp_mp\utility\player_utility::_isalive() )
            {
                if ( isplayer( var_0 ) )
                    thread scripts\mp\weapons::grenadestuckto( self, var_0 );

                if ( isdefined( var_1 ) )
                    self linkto( var_0, var_1 );
                else
                    self linkto( var_0, "j_spine4", ( 0, 0, 0 ) );
            }
        }
        else if ( isdefined( var_1 ) )
            self linkto( var_0, var_1 );
        else
            self linkto( var_0 );
    }

    return 1;
}

scn_infil_hackney_heli_npc3()
{
    self endon( "death" );
    var_0 = self getlinkedparent();

    while ( isdefined( var_0 ) )
        self waittill( "missile_stuck", var_0 );

    self.badplace = createnavbadplacebybounds( self.origin, ( 125, 125, 125 ), ( 0, 0, 0 ) );
}

school_guards_rpg_shoot_into_windows( var_0 )
{
    var_1 = 125;
    var_2 = 25;
    var_3 = 10;
    var_4 = "MOD_FIRE";

    if ( var_0.basename == "thermite_mp" )
    {
        var_1 = 125;
        var_2 = 30;
        var_3 = 30;
    }

    var_5 = self.stuckenemyentity;

    if ( isdefined( var_5 ) )
    {
        if ( isplayer( var_5 ) && var_5 scripts\cp_mp\utility\player_utility::_isalive() )
        {
            var_5 dodamage( var_2, self.origin, self.owner, self, var_4, var_0 );
            var_5 scripts\cp_mp\utility\damage_utility::adddamagemodifier( "thermiteStuck", 0, 0, ::school_guards_rpg_guys );
        }
        else
            var_5 = undefined;
    }

    self radiusdamage( self.origin, var_1, var_2, var_3, self.owner, var_4, var_0 );

    if ( isdefined( var_5 ) )
        var_5 scripts\cp_mp\utility\damage_utility::removedamagemodifier( "thermiteStuck", 0 );
}

school_guards_rpg_guys( var_0, var_1, var_2, var_3, var_4, var_5, var_6 )
{
    if ( !isdefined( var_0 ) )
        return 1;

    if ( !isdefined( var_0.weapon_name ) || var_0.weapon_name != "thermite_mp" )
        return 1;

    if ( !isdefined( var_0.stuckenemyentity ) || var_0.stuckenemyentity != var_2 )
        return 1;

    return 0;
}

thermite_watchdisowned()
{
    self endon( "death" );
    self.owner scripts\engine\utility::spectatekey( "joined_team", "joined_spectators", "disconnect" );
    thread thermite_destroy();
}

thermite_destroy()
{
    thread thermite_delete( 5 );
    self setscriptablepartstate( "effects", "burnEnd", 0 );
}

thermite_delete( var_0 )
{
    self notify( "death" );
    self.exploding = 1;

    if ( isdefined( self.dangerzoneid ) )
    {
        scripts\mp\spawnlogic::removespawndangerzone( self.dangerzoneid );
        self.dangerzoneid = undefined;
    }

    if ( isdefined( self.badplace ) )
    {
        destroynavobstacle( self.badplace );
        self.badplace = undefined;
    }

    self forcehidegrenadehudwarning( 1 );

    if ( isdefined( var_0 ) )
        wait( var_0 );

    self delete();
}

thermite_onplayerdamaged( var_0 )
{
    if ( var_0.meansofdeath == "MOD_IMPACT" )
        return 1;

    var_0.victim.lastburntime = gettime();
    var_0.victim thread scripts\mp\weapons::enableburnfxfortime( 0.6 );
    return 1;
}
