...
init()
{
    ...
    level.intel_guys = ::onmatchstart;
    level.intel_loc = ::onroundend;
    level.insidecuavbunker = ::logkillstreakevent;
    level.inside_bush = ::loggameevent;
    level.inner = ::logattackerkillevent;
    level.intel_collect_vo_func = ::logvictimkillevent;
    level.instance = ::logmultikill;
    level.instantbleedoutsquadwipe = ::logplayerlife;
    level.instant_revived = ::logplayerdeath;
    level.instant_revive_buffer = ::logplayerdata;
    level.intel_active = ::logscoreevent;
    level.intel_location_vo = ::oncrateactivate;
    level.intel_collected = ::logweaponstat;
    level.inmapbounds = ::logattachmentstat;
    level.initvehicles = ::buildweaponrootlist;
    level.insertingarmorplate = ::logchallenge;
    level.inneurotoxintimestamp = ::logaward;
...
onmatchstart()
{
    ...
    if ( var_0 == "br" )
        var_1 = level.bot_cache_entrances_to_other_radios.name;
    ...
    if ( var_0 == "br" )
        scripts\common\utility::set_showing_bomb_wire_pair_to_player( level.initusage );
...
init_relic_lfo( var_0 )
{
    ...
    if ( scripts\mp\utility\game::rankingenabled() && self hasplayerdata() )
    {
        ...
        var_24 = self getplayerdata( "rankedloadouts", "squadMembers", "season_rank" );
        var_25 = self getplayerdata( "rankedloadouts", "squadMembers", "season_rank" );
    }
    else
    {
        var_12 = 0;
    ...
    if ( scripts\mp\utility\game::getgametype() == "arm" && isdefined( self.gas_flyby_plane ) )
    {
        var_36["press_rclick"] = istrue( self.gas_flyby_plane["rightmouseup"] );
        var_36["press_lclick"] = istrue( self.gas_flyby_plane["leftmouseup"] );
        var_36["press_activate"] = istrue( self.gas_flyby_plane["activate"] );
        var_36["press_gostand"] = istrue( self.gas_flyby_plane["gostand"] );
    }
...
logplayerdata( var_0 )
{
    ...
    {
        var_2 = scripts\mp\rank::chopper_kill_vehicle( self );
        scripts\mp\rank::chopper_lights( self.chopper_sound_fade_and_delete, var_2 );
    }

    self sendclientnetworktelemetry();
    self getuseholdkbmprofile();
    var_3 = 0;
    var_4 = 0;
