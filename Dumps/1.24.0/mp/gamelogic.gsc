...
default_ondeadevent( var_0 )
{
    ...
    if ( var_0 == "all" )
    {
        ...
        foreach ( var_6 in level.teamnamelist )
            logstring( var_6 + ": " + game["teamScores"][var_6] );
...
onscorelimit( var_0 )
{
    ...
    if ( level.teambased )
    {
        var_2 = scripts\mp\gamescore::br_pe_chopper_crates();
    ...
    else
    {
        ...
        {
            level.isdmzbotpracticematch = var_2;
...
waitforplayers( var_0 )
{
    ...
    for (;;)
    {
        ...
        else if ( var_9 && var_8 && var_7 > var_2 || var_7 > var_15 )
            break;
...
prematchperiod()
{
    ...
    if ( getdvarint( "scr_live_lobby", 0 ) == 1 && !istrue( level.quadgridradius2dsq ) )
    {
        if ( scripts\mp\utility\game::getgametype() != "br" )
        ...
        level thread startdeadsilence( 1 );
        ...
        if ( scripts\mp\utility\game::getgametype() == "br" )
        {
            ...
            level thread scripts\mp\gametypes\br_public.gsc::blink_wheelson_chosen_spawn();
    ...
    else if ( !istrue( level.quadgridradius2dsq ) )
    {
        if ( scripts\mp\utility\game::getgametype() == "br" )
        {
            ...
            if ( var_2 != 0 )
            {
                level thread startdeadsilence();
                level thread bot_loadout();
                level waittill( "start_prematch" );
            }
    ...
    if ( istrue( level.quadgridradius2dsq ) )
    {
...
updatematchbonusscores( var_0 )
{
    ...
    if ( level.teambased )
    {
        ...
        foreach ( var_3 in level.players )
        {
            ...
            var_3 br_pe_chopper_crate_index( var_1 );
        }
    }
    else
    {
...
givematchbonus( var_0, var_1 )
{
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" && scripts\mp\utility\game::fly_over_path() != "dmz" && scripts\mp\utility\game::fly_over_path() != "risk" && scripts\mp\utility\game::fly_over_path() != "kingslayer" )
        return;
...
br_pe_chopper_crate_index( var_0 )
{
    ...
    var_1 = scripts\mp\weaponrank::farah_nobraids_body() / 60;
    var_2 = var_0 / 60;
...
_getrandomlocations( var_0, var_1 )
{
    ...
    else if ( scripts\mp\utility\game::heli_go_to_exfil_point() )
    {
        var_3 = 1000 - var_1;
...
checktimelimit( var_0 )
{
    ...
    if ( var_1 > 0 && !isdefined( level.double_wood_stack ) )
    {
        if ( var_2 )
        {
            if ( istrue( level.plunder_getleveldataforrepository ) )
...
checkteamscorelimitsoon( var_0 )
{
    ...
    if ( !level.playedmatchendingsoon && scripts\mp\utility\game::canplayhalfwayvo() )
    {
        if ( getteamscore( var_0 ) >= int( level.scorelimit * level.currentround - level.scorelimit / 2 ) )
        {
            ...
            thread scripts\mp\music_and_dialog::is_attack_available( var_0 );
...
checkscorelimit( var_0 )
{
    ...
    else
    {
        ...
        if ( self.score < level.roundscorelimit && !istrue( level.player_rig_create ) )
            return 0;
    }
...
teamstarttimer( var_0, var_1 )
{
    ...
    if ( !level.gameended )
    {
        if ( scripts\mp\utility\game::getgametype() == "br" )
        {
            thread scripts\mp\gametypes\br_public.gsc::move_to_track_end( var_0 );
            thread scripts\mp\gametypes\br_public.gsc::move_payload_to_back_of_super( var_0 );
            thread scripts\mp\gametypes\br_public.gsc::ammo_cache_delete();
        }
...
roundendwait( var_0, var_1 )
{
    if ( !level.doeomcombat )
    {
        wait( level.mortar_remove_fov_user_scale + 0.1 );
...
callback_startgametype()
{
    ...
    level.brloadoutcratedestroycallback = getdvarint( "scr_game_cdltuning", 0 );
    level.creategulagarenaloadout = scripts\mp\utility\game::isanymlgmatch();
    ...
    level.matchrules_vampirism = scripts\mp\utility\dvars::firestation_jugg_test( "scr_" + scripts\mp\utility\game::getgametype() + "_vampirism", "scr_game_vampirism" );

    if ( level.matchrules_vampirism )
        level.modifyplayerdamage = scripts\mp\damage::gamemodemodifyplayerdamage;

    level.finalkillcamtype = scripts\mp\utility\dvars::firestation_jugg_test( "scr_" + scripts\mp\utility\game::getgametype() + "_killcamType", "scr_game_killcamType" );

    if ( level.finalkillcamtype == 2 )
        level.skipfinalkillcam = 1;

    level.allowkillstreaks = scripts\mp\utility\dvars::firestation_jugg_test( "scr_" + scripts\mp\utility\game::getgametype() + "_allowKillstreaks", "scr_game_allowKillstreaks" );
    level.roundretainstreaks = scripts\mp\utility\dvars::firestation_jugg_test( "scr_" + scripts\mp\utility\game::getgametype() + "_roundRetainStreaks", "scr_game_roundRetainStreaks" );
    level.roundretainstreakprog = scripts\mp\utility\dvars::firestation_jugg_test( "scr_" + scripts\mp\utility\game::getgametype() + "_roundRetainStreakProg", "scr_game_roundRetainStreakProg" );
    level.deathretainstreaks = scripts\mp\utility\dvars::firestation_jugg_test( "scr_" + scripts\mp\utility\game::getgametype() + "_deathRetainStreaks", "scr_game_deathRetainStreaks" );
    level.allowperks = scripts\mp\utility\dvars::firestation_jugg_test( "scr_" + scripts\mp\utility\game::getgametype() + "_allowPerks", "scr_game_allowPerks" );
    level.allowsupers = scripts\mp\utility\dvars::firestation_jugg_test( "scr_" + scripts\mp\utility\game::getgametype() + "_allowSupers", "scr_game_allowSupers" );
    level.superfastchargerate = scripts\mp\utility\dvars::firestation_jugg_test( "scr_" + scripts\mp\utility\game::getgametype() + "_superFastChargeRate", "scr_game_superFastChargeRate" );
    level.superpointsmod = scripts\mp\utility\dvars::firestation_jugg_spawn( "scr_" + scripts\mp\utility\game::getgametype() + "_superPointsMod", "scr_game_superPointsMod" );
    level.spawnprotectiontimer = scripts\mp\utility\dvars::firestation_jugg_spawn( "scr_" + scripts\mp\utility\game::getgametype() + "_spawnProtectionTimer", "scr_game_spawnProtectionTimer" );
    level.lethaldelay = scripts\mp\utility\dvars::firestation_jugg_spawn( "scr_" + scripts\mp\utility\game::getgametype() + "_lethalDelay", "scr_game_lethalDelay" );
    level.equipmentmatchstartshieldms = scripts\mp\utility\dvars::firestation_jugg_spawn( "scr_" + scripts\mp\utility\game::getgametype() + "_equipmentMSProtect", "scr_game_equipmentMSProtect" ) * 1000;
    level.magcount = scripts\mp\utility\dvars::firestation_jugg_test( "scr_" + scripts\mp\utility\game::getgametype() + "_magcount", "scr_game_magcount" );

    if ( !isdefined( level.practiceround ) )
        level.practiceround = scripts\mp\utility\dvars::firestation_jugg_test( "scr_" + scripts\mp\utility\game::getgametype() + "_practiceRound", "scr_game_practiceRound" );

    level.postgameexfil = scripts\mp\utility\dvars::firestation_jugg_test( "scr_" + scripts\mp\utility\game::getgametype() + "_postGameExfil", "scr_game_postGameExfil" );
    level.exfilactivetimer = scripts\mp\utility\dvars::firestation_jugg_spawn( "scr_" + scripts\mp\utility\game::getgametype() + "_exfilActiveTimer", "scr_game_exfilActiveTimer" );
    level.exfilextracttimer = scripts\mp\utility\dvars::firestation_jugg_spawn( "scr_" + scripts\mp\utility\game::getgametype() + "_exfilExtractTimer", "scr_game_exfilExtractTimer" );
    level.useammorestocklocs = scripts\mp\utility\dvars::firestation_jugg_spawn( "scr_" + scripts\mp\utility\game::getgametype() + "_useAmmoRestockLocs", "scr_game_useAmmoRestockLocs" );
    level.relic_amped_test_explode_other_player = scripts\mp\utility\dvars::firestation_jugg_test( "scr_" + scripts\mp\utility\game::getgametype() + "_allow3rdspectate", "scr_game_allow3rdspectate" );
    setdvar( "LKNNQKNTS", level.relic_amped_test_explode_other_player );
    ...
    if ( scripts\mp\utility\game::heli_deposit_instruct() )
        brush();
    ...
    level.radarhideshots = scripts\mp\utility\dvars::firestation_jugg_test( "scr_" + var_0 + "_radarHidePings", "scr_game_radarHidePings" );
    setomnvar( "ui_compass_hide_weapon_pings_minimap", level.radarhideshots );
    level.navbarhideshots = scripts\mp\utility\dvars::firestation_jugg_test( "scr_" + var_0 + "_navBarHidePings", "scr_game_navBarHidePings" );
    setomnvar( "ui_compass_hide_weapon_pings_navbar", level.navbarhideshots );
    level.navbarhideenemies = scripts\mp\utility\dvars::firestation_jugg_test( "scr_" + var_0 + "_navBarHideEnemy", "scr_game_navBarHideEnemy" );
    ...
    level.createdevguientryforkidnapper = getdvarint( "scr_player_disableMount" );
    level.disablebattlechatter = getdvarint( "scr_game_disableBattleChatter" );
    level.create_vehicle_vehicledata = getdvarint( "scr_game_disableAnnouncer" );
    level.scoremod = [];
    ...
    if ( scripts\cp_mp\utility\game_utility::isarenamap() && scripts\mp\utility\game_utility_mp::isdisconnectplayer() )
    {
    ...
    scripts\cp_mp\utility\game_utility::give_xp_to_all_players_hack();
    level.maxallowedteamkills = scripts\mp\utility\dvars::firestation_jugg_test( "scr_" + scripts\mp\utility\game::getgametype() + "_ffPunishLimit", "scr_game_ffPunishLimit" );
    thread scripts\mp\init_cp_mp::init();
    thread scripts\mp\teams::init();
    scripts\cp_mp\utility\player_utility::givebmodevloadouts();
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" )
    {
        thread scripts\cp_mp\calloutmarkerping::init();
    ...
    scripts\mp\utility\player_frame_update_aggregator::registerplayerframeupdatecallback( scripts\mp\weapons::fulton_hostageent );
    ...
    if ( !istrue( game["isLaunchChunk"] ) )
    {
        ...
        thread scripts\cp\execution::init();
    ...
    foreach ( var_5 in level.teamnamelist )
    {
        ...
        if ( !istrue( level.pavelow_boss_explodes ) )
    ...
    level.mortar_remove_fov_user_scale = 0.25;
...
timelimitthread()
{
    ...
    while ( game["state"] == "playing" )
    {
        ...
        if ( isdefined( level.starttime ) )
        {
            if ( gettimeremaining() < 3000 || istrue( level.double_wood_stack ) )
            {
                waitframe();
                continue;
            }
...
fail_on_transmission_timeout( var_0 )
{
    if ( var_0 > 20 )
    ...
    while ( game["state"] == "playing" )
    {
        if ( !level.timerstopped && scripts\mp\utility\game::gettimelimit() && !istrue( level.bombsplanted ) )
        {
            ...
            if ( var_6 <= 10 || var_6 <= 30 && var_6 % var_4 == var_7 )
            {
                ...
                if ( var_8 )
                {
                    var_9 = fail_on_transmission_timeout( var_5 );
...
gametimer()
{
    ...
    if ( !isdefined( game["startTimeFromMatchStart"] ) )
    {
        ...
        scripts\common\utility::set_showing_bomb_wire_pair_to_player( level.intel_guys );
    }
...
startgame()
{
    ...
    thread graceperiod();
    thread scripts\cp\so_laststand::roundbegin();
...
wavetimerwatcher()
{
    ...
    for (;;)
    {
        scripts\engine\utility::spectatecommands( "wave_respawn_allies", "wave_respawn_axis" );
...
setendofroundsoundtimescalefactor()
{
    if ( !isdefined( level.delay_put_vehicles_on_compass ) )
    {
        soundsettimescalefactorfromtable( "end_of_round" );
        level.delay_put_vehicles_on_compass = 1;
        level.personalscorecount = undefined;
    }
}

resetsoundtimescalefactor()
{
    if ( !isdefined( level.personalscorecount ) )
    {
        soundsettimescalefactorfromtable( "reset" );
        level.delay_put_vehicles_on_compass = undefined;
        level.kill_sentries = undefined;
        level.personalscorecount = 1;
    }
}
...
restart()
{
    if ( isdefined( level.lb_impulse_dmg_threshold_low ) )
        [[ level.lb_impulse_dmg_threshold_low ]]();
...
{
    if ( isdefined( level.isdmzbotpracticematch ) )
        var_0 = level.isdmzbotpracticematch;
...
endgame_regularmp( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    scripts\common\utility::set_showing_bomb_wire_pair_to_player( level.intel_loc );
    var_5 = 0;
    ...
    level.chopper_kill_person = istrue( var_3 ) && scripts\mp\utility\game::getgametype() == "ctf";

    if ( isdefined( var_0 ) && level.chopper_kill_person )
    {
...
endgame_showkillcam()
{
    ...
    if ( istrue( level.bonusobjectivescorecharge ) )
        return;
...
postroundfadenokillcam()
{
    foreach ( var_1 in level.players )
    {
        ...
        var_1 thread move_molotov_mortar();
    }
...
delay_delete_reinforcement_called_icon( var_0 )
{
    if ( isdefined( var_0 ) )
    {
        if ( var_0 == 1 || var_0 == 2 )
        {
            level._luidecision = 0;
            level._closepurchasemenuwithresponse = var_0;
            ...
            if ( !scripts\mp\utility\game::waslastround() )
            {
                ...
                foreach ( var_5 in level.players )
                {
                    ...
                    if ( isalive( var_5 ) && isdefined( var_5.matchdatalifeindex ) )
                        var_5 scripts\common\utility::set_showing_bomb_wire_pair_to_player( level.instant_revived, var_5.matchdatalifeindex, undefined, undefined, "MOD_ROUND_ENDED", "none", undefined, undefined );
                }
...
endgame_endgame( var_0, var_1, var_2, var_3 )
{
    ...
    if ( scripts\mp\gametypes\br_public.gsc::handlerelicmartyrdomgas() && !level.forcedend )
    {
        level._luidecision = 1;
        level scripts\common\ui::lui_registercallback( "exit_squad_eliminated", ::delay_delete_reinforcement_called_icon );
    }

    if ( scripts\mp\utility\game::matchmakinggame() && isdefined( level.projectileunlink ) && [[ level.projectileunlink ]]() )
        requestgamelobbyremainintact();

    if ( !istrue( level.processedwinloss ) && ( istrue( level.forfeitinprogress ) || level.forcedend ) )
        updatewinlossstats( var_0 );

    scripts\cp\so_laststand::roundend( var_0 );
    checkforpersonalbests();
    updatespmstats();
    scripts\mp\persistence::writebestscores();
    level notify( "stop_leaderboard_stats" );
    updateleaderboardstats();
    level scripts\mp\accolades::deletescriptableinstanceaftertime_proc();
    level.doingbroshot = scripts\mp\broshot::initbroshot( var_0 );

    if ( !level.doingbroshot )
    {
        level notify( "spawning_intermission" );
        var_4 = fillmaxarmorplate();

        foreach ( var_6 in level.players )
        {
            var_6 thread scripts\mp\utility\game::setuipostgamefade( 0.0 );

            if ( !isdefined( level.isdonetsksubmap ) || !var_6 [[ level.isdonetsksubmap ]]() )
    ...
    if ( !level.doingbroshot )
    {
        if ( scripts\mp\utility\game::getgametype() == "br" )
        {
            scripts\mp\gametypes\br_circle.gsc::get_push_force_direction();
            setomnvarforallclients( "ui_br_transition_type", 0 );
            setomnvarforallclients( "post_game_state", 14 );

            if ( !scripts\mp\gametypes\br_public.gsc::handlerelicmartyrdomgas() )
            {
    ...
    if ( scripts\mp\utility\game::getgametype() != "br" || scripts\mp\utility\game::fly_over_path() == "dmz" || scripts\mp\utility\game::fly_over_path() == "risk" || scripts\mp\utility\game::fly_over_path() == "kingslayer" || scripts\mp\utility\game::healthpool() )
        processlobbydata();
    ...
    foreach ( var_6 in level.players )
    {
        ...
        if ( isalive( var_6 ) && isdefined( var_6.matchdatalifeindex ) )
            var_6 scripts\common\utility::set_showing_bomb_wire_pair_to_player( level.instant_revived, var_6.matchdatalifeindex, undefined, undefined, "MOD_GAME_ENDED", "none", undefined, undefined );
        ...
        if ( var_11 )
        {
            if ( var_6 == var_8[0] && scripts\mp\utility\game::getgametype() == "arena" )
                var_6 scripts\cp_mp\pet_watch::add_pack_fx();
            else
        ...
        var_6 scripts\common\utility::set_showing_bomb_wire_pair_to_player( level.instant_revive_buffer );
    }

    scripts\common\utility::set_showing_bomb_wire_pair_to_player( level.intel_location_vo );
    ...
    if ( scripts\mp\gametypes\br_public.gsc::handlerelicmartyrdomgas() && isdefined( level._closepurchasemenuwithresponse ) && level._closepurchasemenuwithresponse == 2 )
    {
...
playerpowerscleanupkeybindings()
{
    if ( !isdefined( level.delay_notify_player_target_hit ) )
    {
    ...
    level.delay_notify_player_target_hit = 1;
}
...
{
    wait( level.mortar_remove_fov_user_scale );
...
fillmaxarmorplate()
{
    ...
    {
        var_0 = spawnstruct();
        var_0.origin = level.bot_known_flag_carrier_loc.relic_award_one_bullet.origin;
        var_0.angles = level.bot_known_flag_carrier_loc.relic_award_one_bullet.angles;
    }
...
processlobbydata()
{
    foreach ( var_1 in level.players )
    {
        ...
        mp_m_trench_patch_giveplayer_c4( var_1 );
    }
...
aq_ontimerupdate( var_0 )
{
    if ( isdefined( var_0 ) && !isdefined( var_0.clientmatchdataid ) )
    {
        var_0.clientmatchdataid = level.choppersupport_watchleash;
        level.choppersupport_watchleash++;
    }
}

mp_m_trench_patch_giveplayer_c4( var_0 )
{
    if ( istrue( var_0.mp_m_trench_patch_giveplayer_c4 ) )
        return;

    var_0.mp_m_trench_patch_giveplayer_c4 = 1;
    aq_ontimerupdate( var_0 );
    var_1 = var_0.name;
...
trackattackerleaderboarddeathstats( var_0, var_1, var_2, var_3 )
{
    if ( isdefined( var_0 ) && isplayer( var_0 ) )
    {
        if ( var_3 != "MOD_FALLING" )
        {
            ...
            {
                var_0 scripts\common\utility::set_showing_bomb_wire_pair_to_player( level.inmapbounds, "tactical", "kills", 1, var_2 );
                var_0 scripts\common\utility::set_showing_bomb_wire_pair_to_player( level.inmapbounds, "tactical", "hits", 1, var_2 );
            ...
            if ( var_3 == "MOD_MELEE" && !scripts\mp\riotshield::isriotshield( var_2 ) && !scripts\mp\utility\weapon::isknifeonly( var_2 ) && !scripts\mp\utility\weapon::isakimbomeleeweapon( var_2 ) && !scripts\mp\utility\weapon::isaxeweapon( var_2 ) )
            {
                var_0 scripts\common\utility::set_showing_bomb_wire_pair_to_player( level.inmapbounds, "none", "kills", 1, var_2 );
                var_0 scripts\common\utility::set_showing_bomb_wire_pair_to_player( level.inmapbounds, "none", "hits", 1, var_2 );
...
setweaponstat( var_0, var_1, var_2 )
{
    ...
    if ( var_5 == "weapon_grenade" || var_5 == "weapon_explosive" || var_4 == "trophy_mp" || var_4 == "forcepush_mp" )
    {
        ...
        scripts\common\utility::set_showing_bomb_wire_pair_to_player( level.intel_collected, var_4, var_2, var_1, var_6 );
        return;
    }
    ...
    if ( var_2 == "deaths" )
    {
        ...
        scripts\common\utility::set_showing_bomb_wire_pair_to_player( level.intel_collected, var_9, "deaths", var_1, var_6 );

        foreach ( var_12 in var_10 )
        {
            scripts\mp\persistence::incrementattachmentstat( var_12, var_2, var_1, var_3 );
            scripts\common\utility::set_showing_bomb_wire_pair_to_player( level.inmapbounds, var_12, var_2, var_1, var_3 );
        }
    }
}
...
updateleaderboardstatscontinuous()
{
    ...
    for (;;)
    {
        ...
        {
            var_1 should_play_player_infil();
            wait 0.1;
        }
...
should_play_player_infil()
{
    ...
    if ( isdefined( level.bot_cache_entrances_to_other_radios ) && isdefined( level.bot_cache_entrances_to_other_radios.name ) )
    {
        switch ( level.bot_cache_entrances_to_other_radios.name )
        {
            ...
            case "kingslayer":
            case "dmz":
                var_0 = var_0 + ( "_" + level.bot_cache_entrances_to_other_radios.name );
                break;
            case "":
                break;
            default:
    ...
    var_1 = scripts\engine\utility::ter_op( level.teambased, self.score, self.incrementobjectiveachievementkill["gamemodeScore"] );
    ...
    incrementleaderboardstat( "kills", var_0, self.incrementobjectiveachievementkill["kills"] );
    incrementleaderboardstat( "deaths", var_0, self.incrementobjectiveachievementkill["deaths"] );

    switch ( scripts\mp\utility\game::getgametype() )
    {
        case "war":
            incrementleaderboardstat( "assists", var_0, self.incrementobjectiveachievementkill["assists"] );
            break;
        case "arena":
            incrementleaderboardstat( "damage", var_0, self.incrementobjectiveachievementkill["damage"] );
            incrementleaderboardstat( "assists", var_0, self.incrementobjectiveachievementkill["assists"] );
            break;
        case "arm":
            incrementleaderboardstat( "captures", var_0, self.incrementobjectiveachievementkill["captures"] );
            incrementleaderboardstat( "defends", var_0, self.incrementobjectiveachievementkill["defends"] );
            break;
        case "cyber":
            incrementleaderboardstat( "revives", var_0, self.incrementobjectiveachievementkill["rescues"] );
            incrementleaderboardstat( "plants", var_0, self.incrementobjectiveachievementkill["plants"] );
            break;
        case "dm":
            updateleaderboardstatmaximum( "streak", var_0, self.incrementobjectiveachievementkill["killChains"] );
            break;
        case "dom":
            incrementleaderboardstat( "captures", var_0, self.incrementobjectiveachievementkill["captures"] );
            incrementleaderboardstat( "defends", var_0, self.incrementobjectiveachievementkill["defends"] );
        ...
        case "sd":
            incrementleaderboardstat( "plants", var_0, self.incrementobjectiveachievementkill["plants"] );
            incrementleaderboardstat( "defuses", var_0, self.incrementobjectiveachievementkill["defuses"] );
            break;
        case "conf":
            incrementleaderboardstat( "confirms", var_0, self.incrementobjectiveachievementkill["confirmed"] );
            incrementleaderboardstat( "denies", var_0, self.incrementobjectiveachievementkill["denied"] );
            break;
        case "hq":
            incrementleaderboardstat( "captures", var_0, self.incrementobjectiveachievementkill["captures"] );
            incrementleaderboardstat( "defends", var_0, self.incrementobjectiveachievementkill["defends"] );
            break;
        case "koth":
            incrementleaderboardstat( "objTime", var_0, self.incrementobjectiveachievementkill["objTime"] );
            incrementleaderboardstat( "defends", var_0, self.incrementobjectiveachievementkill["defends"] );
            break;
        case "ctf":
            incrementleaderboardstat( "captures", var_0, self.incrementobjectiveachievementkill["captures"] );
            incrementleaderboardstat( "returns", var_0, self.incrementobjectiveachievementkill["returns"] );
            break;
        case "sr":
            incrementleaderboardstat( "plants", var_0, self.incrementobjectiveachievementkill["plants"] );
            incrementleaderboardstat( "rescues", var_0, self.incrementobjectiveachievementkill["rescues"] );
            break;
        case "siege":
            incrementleaderboardstat( "captures", var_0, self.incrementobjectiveachievementkill["captures"] );
            incrementleaderboardstat( "revives", var_0, self.incrementobjectiveachievementkill["rescues"] );
            break;
        case "grind":
            incrementleaderboardstat( "banks", var_0, self.incrementobjectiveachievementkill["confirmed"] );
            incrementleaderboardstat( "denies", var_0, self.incrementobjectiveachievementkill["denied"] );
            break;
        case "infect":
            incrementleaderboardstat( "time", var_0, scripts\mp\utility\stats::getpersstat( "extrascore0" ) );
            incrementleaderboardstat( "infected", var_0, self.incrementobjectiveachievementkill["killsAsInfected"] );
            break;
        case "gun":
            incrementleaderboardstat( "stabs", var_0, self.incrementobjectiveachievementkill["stabs"] );
            incrementleaderboardstat( "setBacks", var_0, self.incrementobjectiveachievementkill["setbacks"] );
            break;
        case "grnd":
            incrementleaderboardstat( "objTime", var_0, self.incrementobjectiveachievementkill["objTime"] );
            incrementleaderboardstat( "defends", var_0, self.incrementobjectiveachievementkill["defends"] );
        ...
        case "pill":
            ...
            break;
        case "br":
            incrementleaderboardstat( "downs", var_0, self.incrementobjectiveachievementkill["downs"] );
            incrementleaderboardstat( "contracts", var_0, self.incrementobjectiveachievementkill["contracts"] );
            incrementleaderboardstat( "wins", var_0, self.incrementobjectiveachievementkill["wins"] );
            incrementleaderboardstat( "topFive", var_0, self.incrementobjectiveachievementkill["topFive"] );
            incrementleaderboardstat( "topTen", var_0, self.incrementobjectiveachievementkill["topTen"] );
            incrementleaderboardstat( "topTwentyFive", var_0, self.incrementobjectiveachievementkill["topTwentyFive"] );
            incrementleaderboardstat( "gamesPlayed", var_0, self.incrementobjectiveachievementkill["gamesPlayed"] );
            incrementleaderboardstat( "revives", var_0, self.incrementobjectiveachievementkill["rescues"] );
            incrementleaderboardstat( "cash", var_0, self.incrementobjectiveachievementkill["cash"] );
            break;
    }
}
...
checkforbestweapon()
{
    var_0 = scripts\common\utility::set_showing_bomb_wire_pair_to_player( level.initvehicles );
    var_1 = "";
    var_2 = -1;
...
startdeadsilence( var_0 )
{
    if ( !istrue( level.init_weapon_placements ) )
        return;
    ...
    if ( var_6 <= var_8 )
    {
        ...
        if ( isdefined( level.lb_impulse_dmg_factor_mid_high ) )
            [[ level.lb_impulse_dmg_factor_mid_high ]]();
    }
...
livelobbyrestart()
{
    ...
    game["blockJIP"] = 1;
    waitframe();
    plunder_repositoryusecallbackinternal( undefined );
    waitframe();
    setgameendtime( 0 );
    thread scripts\mp\analyticslog::logevent_sendplayerindexdata();
...
pathexit()
{
    ...
    foreach ( var_2 in var_0 )
        scripts\cp_mp\utility\game_utility::path_timeout( var_2, "targetname" );
}
...
brush()
{
    level.allowsupers = getdvarint( "scr_" + scripts\mp\utility\game::fly_over_path() + "_allowSupers", level.allowsupers );
}
...
init_relic_amped( var_0 )
{
    ...
    if ( isdefined( var_0.cleanupafterweaponswitch ) )
        var_6 = var_0.cleanupafterweaponswitch;
