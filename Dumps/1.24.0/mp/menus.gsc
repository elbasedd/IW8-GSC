...
handleclassedit( var_0 )
{
    ...
    var_3 = scripts\mp\class::init_intro_obj();
    var_4 = var_2 || var_3;
...
autoassign()
{
    ...
    else
    {
        ...
        if ( var_5.size > 0 )
        {
            ...
            if ( istrue( level.armsrace_c4_charge_think ) )
                var_9 = 0;
...
setteam( var_0 )
{
    ...
    {
        if ( getdvarint( "scr_force_cac_sre_callstack", 0 ) == 1 && scripts\mp\utility\game::getgametype() == "br" && scripts\mp\utility\game::fly_over_path() != "dmz" && scripts\mp\utility\game::fly_over_path() != "risk" && scripts\mp\utility\game::fly_over_path() != "sandbox" )
        {
            var_1 = isdefined( level.allowclasschoicefunc ) && istrue( self [[ level.allowclasschoicefunc ]]() );
            scripts\mp\utility\script::corner0to2dist( "setTeam() " + self.name + " ui_options_menu = 2, allowClassChoiceFunc = " + var_1 );
        }

        self setclientomnvar( "ui_options_menu", 2 );
    }

    if ( isdefined( level.lb_mg_dmg_factor_tail_rotor ) )
        self thread [[ level.lb_mg_dmg_factor_tail_rotor ]]();

    waitforclassselect();
    endrespawnnotify();

    if ( ( self.sessionstate == "spectator" || mine_caves_turret_1_support() ) && !istrue( self.suicideswitched ) )
    {
...
{
    return isdefined( level.mine_caves_turret_1_support ) && self [[ level.mine_caves_turret_1_support ]]();
}
...
leavesquad( var_0, var_1 )
{
    if ( isdefined( var_0 ) && isdefined( var_1 ) )
        thread scripts\mp\spawnselection::ondefuse( var_0, var_1 );
...
waitforclassselect()
{
    ...
    for (;;)
    {
        var_0 = scripts\mp\utility\game::getgametype() == "br" && scripts\mp\utility\game::allowclasschoice() && ( !scripts\mp\flags::gameflag( "prematch_done" ) || istrue( level.quadgridradius2dsq ) );
...
beginclasschoice( var_0 )
{
    ...
    if ( scripts\mp\utility\game::allowclasschoice() || scripts\mp\utility\game::showfakeloadout() && !isai( self ) )
    {
        if ( getdvarint( "scr_force_cac_sre_callstack", 0 ) == 1 && scripts\mp\utility\game::getgametype() == "br" && scripts\mp\utility\game::fly_over_path() != "dmz" && scripts\mp\utility\game::fly_over_path() != "risk" && scripts\mp\utility\game::fly_over_path() != "sandbox" )
        {
            ...
            scripts\mp\utility\script::corner0to2dist( "beginClassChoice() " + self.name + " ui_options_menu = 2, allowClassChoiceFunc = " + var_2 );
        }
...
addtoteam( var_0, var_1, var_2 )
{
    ...
    if ( var_0 == "spectator" || var_0 == "follower" )
    {
        ...
        scripts\cp_mp\pet_watch::launcher_second_techo();
    }
    else
