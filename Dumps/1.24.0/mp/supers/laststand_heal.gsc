...
laststandheal_onset()
{
    ...
    for (;;)
    {
        if ( self.laststandheal_hassuperweapon && ( !istrue( self.inlaststand ) || istrue( self.stuckinlaststand ) || istrue( scripts\mp\playerlogic::extractupdateiconsframeend() ) ) )
        {
        ...
        else if ( !self.laststandheal_hassuperweapon && istrue( self.inlaststand ) && !istrue( self.stuckinlaststand ) && !istrue( scripts\mp\playerlogic::extractupdateiconsframeend() ) )
        {
            var_0 = scripts\mp\supers::getcurrentsuper().staticdata.weapon;
...
laststandheal_setinactivewhendone()
{
    level endon( "game_ended" );
    self endon( "disconnect" );
    scripts\engine\utility::splashtime_vip( "death", "last_stand_finished" );
