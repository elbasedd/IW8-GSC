...
superdeadsilence_endsuper( var_0 )
{
    ...
    scripts\cp\so_laststand::lastweaponfiretimeend( "super_deadsilence", self.deadsilencekills );
...
superdeadsilence_onkill()
{
    if ( scripts\mp\utility\game::getgametype() != "infect" )
    {
        ...
        var_0 = scripts\mp\supers::failedmission( "super_deadsilence" );

        if ( self.deadsilencekills > var_0 )
        {
            var_1 = self.deadsilencekills - var_0;
            scripts\mp\supers::cargo_truck_mg_initoccupancy( "super_deadsilence", var_1 );
        }
    }
...
applyfovpresentation()
{
    ...
    var_1 = var_0 - 2;
    scripts\engine\utility::spotlight_sweep_to_loc( var_1, "super_use_finished" );
    self lerpfovbypreset( "default_2seconds" );
    self playlocalsound( "deadsilence_end" );
}

risk_currentlocsinuse()
{
    self endon( "death_or_disconnect" );
    self endon( "super_use_finished" );
    self notify( "superDeadsilence_watchForGameEnded" );
    self endon( "superDeadsilence_watchForGameEnded" );
    level scripts\engine\utility::spectatecommands( "game_ended", "prematch_cleanup" );
