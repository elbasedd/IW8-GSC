...
main()
{
    ...
    if ( isdefined( level.binoculars_iswithinprojectiondistance ) )
        [[ level.binoculars_iswithinprojectiondistance ]]();
    else
    ...
    foreach ( var_1 in level.players )
    {
        if ( isbot( var_1 ) )
        {
            if ( isalive( var_1 ) )
            {
                ...
                var_1.binoculars_processtargetlos = 1;
...
{
    var_0 scripts\engine\utility::spectatecommands( "bots_human_spawned", "bots_human_disconnected" );
    ...
    for (;;)
    {
        ...
        if ( var_46 )
        {
            ...
            if ( var_45 > 0 && var_44 > 0 )
                level scripts\engine\utility::spawnzombiedogtags( "spawned_enemies", "spawned_allies" );
            else if ( var_45 > 0 )
                level waittill( "spawned_enemies" );
            else if ( var_44 > 0 )
...
bot_think()
{
    ...
    if ( !isdefined( self.binoculars_processtargetlos ) )
    {
        var_1 = 1;
        self.binoculars_processtargetlos = 1;
    ...
    for (;;)
    {
        ...
        scripts\mp\bots\bots_loadout::binoculars_onstatelospendingexit();
...
bot_think_seek_dropped_weapons()
{
    ...
    for (;;)
    {
        ...
        if ( scripts\mp\bots\bots_util::binoculars_addtolosqueue( 0.33 ) )
        {
...
bot_crate_valid( var_0 )
{
    ...
    else if ( binoculars_getpendingendtime( var_0 ) )
        return 0;
...
bot_think_crate_blocking_path()
{
    ...
    for (;;)
    {
        ...
        for ( var_2 = 0; var_2 < var_1.size; var_2++ )
        {
            ...
            {
                if ( !binoculars_getpendingendtime( var_3 ) )
                {
...
bot_think_revive()
{
    ...
    for (;;)
    {
        ...
        if ( !bot_can_revive() )
        {
            self.handlerelicmartyrdomfrag = 0;
            continue;
        }
        ...
        for ( var_4 = 0; var_4 < var_1.size; var_4++ )
        {
            ...
            if ( distancesquared( self.origin, var_8 ) < 4194304 )
            {
                ...
                self.handlerelicmartyrdomfrag = 1;
...
revive_watch_for_finished( var_0 )
{
    ...
    self endon( "bad_path" );
    self endon( "goal" );
    var_0 scripts\engine\utility::spectatecommands( "death", "revived" );
...
handle_smoke()
{
    ...
    thread checkpoint_objective( var_0 );
}
