...
setup_callbacks()
{
    ...
    level.bot_funcs["get_watch_node_chance"] = ::battle_tracks_updateexternallisteningzone;
}

setup_bot_ctf()
{
    level.begin_vo = ::chopper_gunner_assigntargetmarkers;
    level.being_hacked = ::chopper_kill;
    level.bears = ::get_allied_attackers_for_team;
    level.bearsetup = ::get_allied_defenders_for_team;
    level.bearwatch = ::battle_tracks_tryplayingbattletrackswhenstandingonvehicle;
    level.besttime = ::battle_tracks_vehicleallowlisteningoutsideoccupancy;
    ...
    thread battle_tracks_tryinittogglestate();
...
crate_can_use( var_0 )
{
    ...
    if ( battle_tracks_vehicleoccupancyenter() )
        return 0;

    return battle_tracks_updatebattletracks( self.team );
}
...
bot_ctf_think()
{
    ...
    for (;;)
    {
        ...
        if ( !isdefined( self.role ) )
            scripts\mp\bots\bots_gametype_common.gsc::bhasriotshieldattached();

        var_2 = scripts\mp\utility\game::getotherteam( self.team )[0];

        if ( battle_tracks_updatebattletracks( self.team ) )
        {
            if ( isdefined( level.binoculars_iswithinprojectiondistance_compute ) && isdefined( level.binoculars_iswithinprojectiondistance_compute[var_2] ) )
                level.binoculars_iswithinprojectiondistance_compute[var_2] = undefined;
        }
        ...
        if ( self.role == "attacker" )
        {
            if ( battle_tracks_vehicleoccupancyenter() )
                var_3 = 1;
            else if ( !battle_tracks_trystopdrivertogglethink() )
                var_3 = distancesquared( self.origin, level.teamflags[var_2].curorigin ) < squared( get_flag_protect_radius() );
        }
        else if ( !battle_tracks_updatebattletracks( self.team ) )
            var_3 = !battle_tracks_updatetogglestate();
        ...
        if ( battle_tracks_vehicleoccupancyenter() )
        {
            ...
            else if ( scripts\mp\bots\bots_gametype_common.gsc::below_player_eye_allowance( self.team ) == 1 )
        ...
        {
            if ( battle_tracks_trystopdrivertogglethink() )
            {
...
battlepassxpmultipliers()
{
    ...
    else if ( self botcanseeentity( var_2 ) )
    {
        ...
        if ( self botgetdifficultysetting( "strategyLevel" ) > 0 && !battle_tracks_updatebattletracks( self.team ) )
        {
            if ( !isdefined( level.binoculars_iswithinprojectiondistance_compute ) )
                level.binoculars_iswithinprojectiondistance_compute = [];

            if ( !isdefined( level.binoculars_iswithinprojectiondistance_compute[var_2.team] ) )
                level.binoculars_iswithinprojectiondistance_compute[var_2.team] = [];

            level.binoculars_iswithinprojectiondistance_compute[var_2.team]["origin"] = var_0;
            level.binoculars_iswithinprojectiondistance_compute[var_2.team]["time"] = gettime();
        }
    }
    ...
    else
    {
        ...
        {
            if ( isdefined( level.binoculars_iswithinprojectiondistance_compute ) && isdefined( level.binoculars_iswithinprojectiondistance_compute[var_2.team] ) )
            {
                var_4 = gettime() - level.binoculars_iswithinprojectiondistance_compute[var_2.team]["time"];

                if ( var_4 < 10000 )
                    var_3 = level.binoculars_iswithinprojectiondistance_compute[var_2.team]["origin"];
            }
        }
...
{
    var_1 = scripts\mp\bots\bots_gametype_common.gsc::below_player_eye_allowance( var_0 );
    ...
    else
    {
        ...
        if ( battle_tracks_updatebattletracks( var_3 ) )
            var_2 = var_1 * 0.5;
        else
            var_2 = var_1 * 0.4;
    }
...
{
    var_1 = scripts\mp\bots\bots_gametype_common.gsc::below_player_eye_allowance( var_0 );
    return var_1 - chopper_gunner_assigntargetmarkers( var_0 );
}

get_allied_attackers_for_team( var_0 )
{
    return scripts\mp\bots\bots_gametype_common.gsc::being_kicked_from_inactivity( var_0, level.capzones[var_0].curorigin, get_flag_protect_radius() );
}

get_allied_defenders_for_team( var_0 )
{
    return scripts\mp\bots\bots_gametype_common.gsc::being_pickedup( var_0, level.capzones[var_0].curorigin, get_flag_protect_radius() );
}
...
battle_tracks_tryinittogglestate()
{
    ...
    level endon( "game_ended" );
    thread scripts\mp\bots\bots_gametype_common.gsc::begin_spectate();
}
...
{
    return battle_tracks_updatebattletracksfromplayerdata( self.team );
}

battle_tracks_trystopdrivertogglethink()
{
    var_0 = scripts\mp\utility\game::getotherteam( self.team )[0];
    return battle_tracks_updatebattletracksfromplayerdata( var_0 );
}
