...
setup_callbacks()
{
    ...
    level.bot_funcs["emp_picked_up_cancel"] = ::battle_tracks_onexitvehicle;
    level.bot_funcs["tactical_revive_override"] = ::battle_tracks_playbattletrackstoalloccupants;
}
...
setup_bot_cyber()
{
    ...
    var_0 = scripts\mp\bots\bots_gametype_common.gsc::binoculars_setpendingtimer( [ "_allies", "_axis" ] );
...
bot_cyber_think()
{
    ...
    for (;;)
    {
        ...
        if ( !isdefined( level.sandbox_combat_area ) )
        {
            if ( !isdefined( level.audio_shf_kill_hangar_lights ) )
            {
                if ( scripts\mp\bots\bots_strategy::bot_has_tactical_goal( "seek_cyber_emp", undefined ) == 0 )
                {
                    var_0 = ::claymore_crate_player_at_max_ammo;
                    var_1 = spawnstruct();
            ...
            else if ( self.isbombcarrier )
            {
                ...
                var_4 = scripts\mp\bots\bots_gametype_common.gsc::end_reach_exhaust_waste( var_3, 1 );
                ...
                if ( var_5 == "goal" )
                {
                    ...
                    level scripts\engine\utility::spotlight_min_dist_sq_from_node( level.planttime + 2, "bomb_planted" );
                }
            }
            else if ( level.audio_shf_kill_hangar_lights.team == self.team )
            {
                if ( !scripts\mp\bots\bots_util::bot_is_defending() )
                    scripts\mp\bots\bots_strategy::bot_guard_player( level.audio_shf_kill_hangar_lights, 400 );
            }
            else if ( gettime() > self.next_time_hunt_carrier || sighttracepassed( self.origin + ( 0, 0, 77 ), level.audio_shf_kill_hangar_lights.origin + ( 0, 0, 77 ), 0, self ) )
            {
                self botsetscriptgoal( level.audio_shf_kill_hangar_lights.origin, 16, "hunt" );
        ...
        var_3 = level.objectives[scripts\engine\utility::get_enemy_team( level.sandbox_combat_area )];

        if ( self.team == level.sandbox_combat_area )
        {
            if ( !gwperifvfx_plumes() )
        ...
        {
            var_6 = scripts\mp\bots\bots_gametype_common.gsc::end_pipe_room( var_3 ).origin;
            ...
            if ( var_7 == "goal" )
            {
                ...
                stack_patch_thread_node( level.defusetime + 2 );
            }
        }
...
setupminimapmaze()
{
    if ( !level.bombplanted )
    {
        if ( isdefined( level.sandbox_combat_area ) )
        {
            level.sandbox_combat_area = undefined;
            level.audio_shf_kill_hangar_lights = undefined;
        }

        var_0 = level.audio_shf_kill_hangar_lights;
        level.audio_shf_kill_hangar_lights = undefined;

        foreach ( var_2 in level.participants )
        {
            if ( isalive( var_2 ) && var_2.isbombcarrier )
                level.audio_shf_kill_hangar_lights = var_2;
        }

        var_4 = 0;

        if ( !isdefined( var_0 ) && isdefined( level.audio_shf_kill_hangar_lights ) )
        {
            var_4 = 1;

            if ( isai( level.audio_shf_kill_hangar_lights ) )
                level.audio_shf_kill_hangar_lights thread balloon_deposit_cash_vo_explanation();
        }
        else if ( isdefined( var_0 ) && !isdefined( level.audio_shf_kill_hangar_lights ) )
            var_4 = 1;
    ...
    {
        if ( isdefined( level.bombowner ) && !isdefined( level.sandbox_combat_area ) )
        {
            level.sandbox_combat_area = level.bombowner.team;
            level.hudglobalkillcount = gettime();
        }

        if ( !isdefined( level.bomb_defuser ) || !isalive( level.bomb_defuser ) || gettime() > level.hudglobalkillcount + 1000 )
        {
            var_7 = [];

            foreach ( var_2 in level.participants )
            {
                if ( isalive( var_2 ) && scripts\mp\utility\entity::isaiteamparticipant( var_2 ) && var_2.team != level.sandbox_combat_area )
                    var_7[var_7.size] = var_2;
            }

            if ( var_7.size > 0 )
            {
                var_10 = level.objectives[scripts\engine\utility::get_enemy_team( level.sandbox_combat_area )];
...
battle_tracks_playbattletrackstoalloccupants()
{
    ...
    if ( istrue( self.handlerelicmartyrdomfrag ) )
        return 0;
