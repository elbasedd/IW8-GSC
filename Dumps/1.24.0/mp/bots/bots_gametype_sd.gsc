...
atk_bomber_update()
{
    ...
    else
    {
        ...
        if ( !isdefined( level.initial_bomb_pickup_time ) || gettime() - level.initial_bomb_pickup_time < level.initial_pickup_wait_time )
        {
            ...
            thread badpathnodes( level.initial_pickup_wait_time / 1000 );
        ...
        {
            self botsetpathingstyle( "scripted" );
            var_11 = scripts\mp\bots\bots_gametype_common.gsc::end_reach_exhaust_waste( var_10, 1 );
        ...
        if ( var_12 == "goal" )
        {
            ...
            var_17 = scripts\mp\bots\bots_gametype_common.gsc::barkov_execution_path( level.planttime + 2, "bomb_planted", var_16 );
            self botclearscriptgoal();
...
bomb_defuser_update()
{
    ...
    if ( var_4 == "goal" )
    {
        ...
        var_13 = scripts\mp\bots\bots_gametype_common.gsc::barkov_execution_path( level.defusetime + 2, "bomb_defused", var_11, var_12 > 40 );
...
backstabber_update()
{
    ...
    if ( self.backstabber_stage == "3_move_to_bombzone" )
    {
        if ( !isdefined( self.barkov_crawl_scene_nostab ) )
            self.barkov_crawl_scene_nostab = scripts\engine\utility::random( level.objectives );

        self botsetpathingstyle( undefined );
        var_9 = self botsetscriptgoal( scripts\engine\utility::random( self.barkov_crawl_scene_nostab.bottargets ).origin, 160, "objective" );
        ...
        if ( var_9 )
        {
            ...
            if ( var_10 == "goal" )
            {
                ...
                {
                    if ( var_14 != self.barkov_crawl_scene_nostab )
                    {
                        self.barkov_crawl_scene_nostab = var_14;
                        break;
                    }
                }
            }
...
{
    var_1 = scripts\mp\bots\bots_gametype_common.gsc::enable_spawner_after_vehicle_death( var_0 );
