...
main()
{
    if ( scripts\mp\utility\game::fly_over_path() == "dmz" && scripts\mp\gametypes\br_public.gsc::has_filled_amped_bar() )
        [[ level.ctgs_compareweaponxp ]]();
    else
    {
        setup_callbacks();
        setup_bot_br();
...
bot_br_think()
{
    ...
    thread all_but_one_player_downed();
    self botsetflag( "ignore_nodes", 1 );

    for (;;)
    {
        if ( scripts\mp\gametypes\br_public.gsc::handlerelicmartyrdomgas() && !scripts\mp\flags::gameflag( "graceperiod_done" ) )
        {
            self.ignoreall = 0;
            wait 0.05;
            continue;
        ...
        {
            if ( scripts\mp\gametypes\br_public.gsc::handlerelicmartyrdomgas() && !isdefined( self.getemptyloadout ) )
            {
                self.ignoreall = 1;
            ...
            if ( ( scripts\mp\gametypes\br_public.gsc::headiconfaction() || var_0 ) && istrue( level.c130inbounds ) )
            {
                ...
                if ( scripts\mp\gametypes\br_public.gsc::headiconfaction() )
        ...
        {
            thread binoculars_endadslogic();
            scripts\engine\utility::spectatecommands( "death_or_disconnect", "gulag_end" );
            wait 3;
        }
        else
        ...
        if ( isdefined( level.br_circle ) && isnavmeshloaded() )
        {
            ...
            {
                if ( ( istrue( self isskydiving() ) || istrue( self isparachuting() ) ) && istrue( self.hide_plunderboxes ) && istrue( scripts\mp\gametypes\br_public.gsc::handlerelicmartyrdomgas() ) )
                {
...
binoculars_onstatemarkpendingexit()
{
    self endon( "death_or_disconnect" );
    self.ignoreall = 1;
    self.cp_dwn_twn_2_sound_load = binoculars_addmarkpoints();
    var_0 = gettime() + randomfloatrange( 5, 10 ) * 1000;
    var_1 = 0;

    while ( istrue( self isskydiving() ) || istrue( self isparachuting() ) )
    {
        if ( level.br_circle.circleindex > 0 && istrue( level.callback_subscribe ) && !scripts\mp\gametypes\br_circle.gsc::ispointincurrentsafecircle( self.cp_dwn_twn_2_sound_load ) )
            self.cp_dwn_twn_2_sound_load = binoculars_addmarkpoints();

        var_2 = bindingpc( self, self.cp_dwn_twn_2_sound_load );
        var_3 = 1;
        self botlookatpoint( self.cp_dwn_twn_2_sound_load, 0.05, "script_forced" );
...
{
    if ( !isdefined( level.dragonsbreathdamage ) || level.dragonsbreathdamage.size < 1 )
    {
        level.dragonsbreathdamage = engagementtime();
        level.dragonsbreathdamage = scripts\engine\utility::array_randomize( level.dragonsbreathdamage );
    }

    if ( isdefined( level.br_circle ) && isnavmeshloaded() )
    {
        var_1 = scripts\engine\utility::random( level.dragonsbreathdamage );

        if ( isdefined( var_1 ) )
        {
            var_2 = var_1.origin;
            level.dragonsbreathdamage = scripts\engine\utility::array_remove( level.dragonsbreathdamage, var_1 );
        }
        else
...
engagementtime()
{
    ...
    return scripts\engine\utility::get_array_of_closest( var_1, level.binoculars_onstateenterfunc, undefined, undefined, var_0 );
}
...
{
    if ( !isdefined( level.battle_tracks_playerinsidezone ) )
        level.battle_tracks_playerinsidezone = [ "iw8_sm_papa90_mp", "iw8_sh_charlie725_mp", "iw8_ar_akilo47_mp+acog", "iw8_lm_mgolf34_mp", "iw8_sn_kilo98_mp+scope", "iw8_sm_beta_mp+reflexmini2", "iw8_sm_augolf_mp+acog", "iw8_sm_mpapa7_mp+acog", "iw8_ar_falima_mp+reflexmini", "iw8_ar_kilo433_mp+acog", "iw8_ar_scharlie_mp+reflexmini2", "iw8_lm_lima86_mp+acog" ];

    var_0 = scripts\engine\utility::random( level.battle_tracks_playerinsidezone );

    switch ( var_0 )
    {
        case "iw8_sh_charlie725_mp":
            if ( !isdefined( level.binoculars_processlosqueuehigh ) )
                level.binoculars_processlosqueuehigh = 0;

            level.binoculars_processlosqueuehigh++;

            if ( level.binoculars_processlosqueuehigh >= 1 )
                level.battle_tracks_playerinsidezone = scripts\engine\utility::array_remove( level.battle_tracks_playerinsidezone, "iw8_sh_charlie725_mp" );

            break;
        case "iw8_sn_kilo98_mp+scope":
            if ( !isdefined( level.binoculars_processtargetdata ) )
                level.binoculars_processtargetdata = 0;

            level.binoculars_processtargetdata++;

            if ( level.binoculars_processtargetdata >= 1 )
                level.battle_tracks_playerinsidezone = scripts\engine\utility::array_remove( level.battle_tracks_playerinsidezone, "iw8_sn_kilo98_mp+scope" );
...
binoculars_clearuidata()
{
    ...
    {
        var_0 = spawnstruct();
        var_0.origin = flash_crate_player_at_max_ammo();
    }
    ...
    scripts\engine\utility::spectatecommands( "goal", "last_stand_start" );
    var_0.claimed = undefined;

    if ( !istrue( self.inlaststand ) )
        binoculars_clearpendingtimer();

    battle_tracks_setmusicstate();
}
...
enemy_ai_enter_alert_due_to_grenade_explode()
{
    ...
    var_1 = endgame_stars( var_0 );

    if ( !isdefined( var_1 ) )
        var_1 = endcameradebug_think( var_0 );
...
enemy_claymore_watchfortrigger()
{
    var_0 = scripts\mp\gametypes\br_circle.gsc::getdangercircleradius();
    var_1 = scripts\mp\gametypes\br_circle.gsc::getdangercircleorigin();
    var_2 = scripts\engine\utility::get_array_of_closest( var_1, level.beacon, undefined, undefined, var_0 );
    return var_2;
}
...
battle_tracks_setmusicstate()
{
    ...
    {
        var_2 = flareready();
        var_1 = binoculars_isads() || istrue( level.callback_subscribe );

        if ( var_1 )
            var_2 = flash_crate_player_at_max_ammo();

        if ( isdefined( var_2 ) )
        {
            var_3 = binoculars_checkexpirationtimer();

            if ( istrue( level.binoculars_updateheadiconvisibility ) && isdefined( var_3 ) && !var_1 )
            {
                ...
                if ( !isdefined( self.manualturret_toggleallowuseactions ) )
                {
                    ...
                    self.manualturret_toggleallowuseactions = var_3;
                }
            }
            else
            {
                self.manualturret_toggleallowuseactions = undefined;
                ...
                thread little_bird_mg_initoccupancy();
...
binoculars_endadslogic()
{
    ...
    for (;;)
    {
        ...
        foreach ( var_3 in var_1.arenaplayers )
        {
            ...
            if ( istrue( var_1.overtime ) && isdefined( var_1.currentlabel ) && isdefined( var_1.currentlabel.arenaflag ) && isdefined( var_1.currentlabel.arenaflag.flagmodel ) )
            {
                self botsetscriptgoal( var_1.currentlabel.arenaflag.flagmodel.origin, 64, "objective" );
                self botclearscriptenemy();
                continue;
            }
...
binoculars_checkexpirationtimer()
{
    if ( isdefined( level.manualturret_watchdeathongameend ) && gettime() <= level.manualturret_watchdeathongameend )
        return undefined;

    var_0 = evaluatefobspawns();
...
binoculars_updateheadiconvisibilityforplayer()
{
    ...
    foreach ( var_2 in level.players )
    {
        ...
        if ( var_2 binoculars_getmaxrange() )
            var_0++;
    }
...
{
    return isdefined( self.manualturret_toggleallowuseactions );
}
...
evaluatefobspawns()
{
    ...
    if ( istrue( self.inlaststand ) || scripts\mp\gametypes\br_public.gsc::headlessinfilplayers() )
        return undefined;

    if ( !isdefined( var_0 ) || istrue( var_0.inlaststand ) || !isalive( var_0 ) || var_0 scripts\mp\gametypes\br_public.gsc::headlessinfilplayers() )
        return undefined;

    if ( binoculars_isads() )
        return undefined;

    var_2 = enabledminimapdisable();
    ...
    if ( var_2 >= 3 )
    {
        ...
        var_3 = binoculars_updateheadiconvisibilityforplayer();

        if ( binoculars_getmaxrange() )
            return var_0;
...
sfx_br_flare_phosphorus()
{
    ...
    scripts\engine\utility::spectatekey( "death", "death_or_disconnect", "last_stand_start" );
    self.manualturret_toggleallowuseactions = undefined;
    level.manualturret_watchdeathongameend = gettime() + 7;
}
...
binoculars_isads()
{
    ...
    if ( istrue( level.callback_subscribe ) )
    {
...
headlightright( var_0 )
{
    var_1 = scripts\mp\gametypes\br_circle.gsc::getdangercircleorigin();
    var_2 = scripts\mp\gametypes\br_circle.gsc::getdangercircleradius();
    return scripts\engine\utility::headshot_distance( var_0, var_1, var_2 );
}
