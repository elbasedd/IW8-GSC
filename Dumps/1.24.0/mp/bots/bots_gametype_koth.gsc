...
setup_callbacks()
{
    level.bot_funcs["gametype_think"] = ::binoculars_ongive;
    level.bot_funcs["should_start_cautious_approach"] = ::progression_main;
}
...
setup_bot_koth()
{
    ...
    if ( !var_0 )
    {
        level.binoculars_getpendingtime = 1;
        var_5 = level.zone;
        ...
        level.bigtimer[var_5.trigger getentitynumber()] = 1;
        level.bot_gametype_precaching_done = 1;
        thread battle_tracks_shouldstartbattletracks( var_5 );
    }
}
...
battle_tracks_shouldstartbattletracks( var_0 )
{
    for ( var_1 = scripts\engine\utility::array_remove( level.objectives, var_0 ); var_1.size > 0; var_1 = scripts\engine\utility::array_remove( var_1, var_2 ) )
    {
        ...
        level.bigtimer[var_2.trigger getentitynumber()] = 1;
    }
}
...
binoculars_ongive()
{
    ...
    for (;;)
    {
        ...
        if ( !isdefined( level.zone ) || !isdefined( level.bigtimer[level.zone.trigger getentitynumber()] ) )
        {
        ...
        if ( !istrue( level.zone.trigger.trigger_off ) && isdefined( level.zoneendtime ) && !isdefined( var_0 ) && !level.zonerandomlocationorder && level.binoculars_getpendingtime )
        {
            ...
            if ( var_2 > 0 && var_2 < 10000 )
            {
                ...
                if ( !var_3 )
                {
                    var_4 = level.zone.support_box_update_hint_logic.radius * 6;

                    if ( var_2 < 5000 )
                        var_4 = level.zone.support_box_update_hint_logic.radius * 3;

                    var_5 = distance( level.zone.support_box_update_hint_logic.center, self.origin );

                    if ( var_5 > var_4 )
                        var_0 = binoculars_processlosqueuelow();
                }
                else
                {
                    ...
                    var_8 = binoculars_cansee( level.zone );
        ...
        if ( !binoculars_init( var_9 ) )
            battle_tracks_stopbattletrackswhenstandingonvehicle( var_9 );
    }
}
...
{
    return binoculars_clearexpirationtimer( var_0 ).size;
}
...
binoculars_clearexpirationtimer( var_0 )
{
    ...
    foreach ( var_3 in level.participants )
    {
        if ( var_3 != self && scripts\mp\utility\entity::isteamparticipant( var_3 ) && isalliedsentient( self, var_3 ) )
        {
            ...
            {
                if ( !isai( var_3 ) || var_3 binoculars_init( var_0 ) )
...
binoculars_init( var_0 )
{
    ...
    return self.choppersupport_isplayeractive == var_0;
}
