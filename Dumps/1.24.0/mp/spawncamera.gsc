...
init()
{
    if ( !isdefined( level.createhistorydestination ) )
        level.createhistorydestination = getdvarint( "scr_game_disablespawncamera", 0 ) == 1;
    ...
    if ( !istrue( level.createhistorydestination ) )
        initcameras();
}
...
startspawncamera( var_0, var_1, var_2 )
{
    ...
    scripts\mp\utility\player::mp_euphrates_gunnonlinear_opendoor( "spawnCamera::startSpawnCamera() START" );
    scripts\mp\utility\player::hideminimap( 1 );
    chopper_boss_destroyed_func();

    if ( !isdefined( self.spawncameraent ) )
    {
        scripts\mp\utility\player::mp_euphrates_gunnonlinear_opendoor( "spawnCamera::startSpawnCamera() UNDEFINED SPAWNCAMERAENT!!!" );
        thread playslamzoomflash( 0.0, getspawncamerawaittime(), 0.5 );
        return;
    }

    if ( istrue( level.createhistorydestination ) && scripts\mp\utility\game::getgametype() != "arm" )
        return;
    ...
    scripts\engine\utility::spotlight_sweep_to_loc( 0.1, "force_spawn" );
    thread removecameraondisconnect( self.spawncameraent );
    self cameralinkto( self.spawncameraent, "tag_origin", 1, 1 );
    scripts\mp\utility\player::mp_euphrates_gunnonlinear_opendoor( "spawnCamera::startSpawnCamera() CameraLinkTo()" );
...
getspawncamera()
{
    ...
    else if ( istrue( level.evaluatespawnforcameraselection ) && isdefined( level.hvt_key_picked_up ) && isdefined( level.hvt_key_picked_up[self.team] ) )
    {
        var_14 = level.hvt_key_picked_up[self.team];
        var_15 = undefined;
        var_16 = undefined;
...
snaptospawncamera()
{
    ...
    while ( !istrue( self.brgetloadoutammomax ) )
        waitframe();
    ...
    scripts\mp\utility\player::mp_euphrates_gunnonlinear_opendoor( "spawnCamera::snapToSpawnCamera()" );
...
endspawncamera()
{
    if ( isdefined( self.level_carepackage_give_player_killstreak_incendiary_launcher ) )
    {
        self clearsoundsubmix( "iw8_mp_spawn_camera" );
        self.level_carepackage_give_player_killstreak_incendiary_launcher stoploopsound( self.level_carepackage_player_used_logic );
        self.level_carepackage_give_player_killstreak_incendiary_launcher delete();
        self.level_carepackage_give_player_killstreak_incendiary_launcher = undefined;
        self.level_carepackage_player_used_logic = undefined;
    }
...
{
    self endon( "disconnect" );
    scripts\mp\utility\player::mp_euphrates_gunnonlinear_opendoor( "spawnCamera::runSlamZoomOnSpawn() START" );
    self waittill( "spawncamera_start" );
    scripts\mp\utility\player::mp_euphrates_gunnonlinear_opendoor( "spawnCamera::runSlamZoomOnSpawn() spawncamera_start PASSED" );
    ...
    scripts\mp\utility\player::mp_euphrates_gunnonlinear_opendoor( "spawnCamera::runSlamZoomOnSpawn() CameraLinkTo()" );
    handlemovetoblended();
    deletespawncamera();
    ...
    if ( istrue( self.projectileunlinkonnote ) )
    {
        ...
        self.projectileunlinkonnote = undefined;
    ...
    scripts\mp\utility\player::mp_euphrates_gunnonlinear_opendoor( "spawnCamera::runSlamZoomOnSpawn() COMPLETE" );
}
...
handlemovetoblended( var_0 )
{
    ...
    scripts\mp\utility\player::mp_euphrates_gunnonlinear_opendoor( "spawnCamera::handleMoveTo() START" );
    ...
    if ( !var_3 || !var_4 )
    {
        ...
        scripts\mp\utility\player::mp_euphrates_gunnonlinear_opendoor( "spawnCamera::handleMoveTo() FULL Blend Set" );
    ...
    else
    {
        ...
        scripts\mp\utility\player::mp_euphrates_gunnonlinear_opendoor( "spawnCamera::handleMoveTo() FIRST HALF Blend Set" );
        ...
        scripts\mp\utility\player::mp_euphrates_gunnonlinear_opendoor( "spawnCamera::handleMoveTo() SECOND HALF Blend Set" );
        wait 0.3;
        wait 0.2;
    }

    scripts\mp\utility\player::mp_euphrates_gunnonlinear_opendoor( "spawnCamera::handleMoveTo() COMPLETE" );
...
fadeblackforgeo( var_0 )
{
    ...
    if ( istrue( self.spawningintovehicle ) || scripts\cp_mp\utility\game_utility::isnightmap() )
    {
        ...
        if ( isdefined( self.spawn_fulton_rope_mdl ) && self.spawn_fulton_rope_mdl == "light_tank" )
        {
...
startoperatorsound()
{
    ...
    self.level_carepackage_give_player_killstreak_incendiary_launcher = spawn( "script_origin", ( 0, 0, 0 ) );
    self.level_carepackage_give_player_killstreak_incendiary_launcher showonlytoplayer( self );
    ...
    self.level_carepackage_player_used_logic = var_1;
    self.level_carepackage_give_player_killstreak_incendiary_launcher playloopsound( var_1 );
    thread delay_play_depart_vo();
}

delay_play_depart_vo()
{
    self endon( "spawned_player" );
    var_0 = self.level_carepackage_give_player_killstreak_incendiary_launcher;
    var_1 = self.level_carepackage_player_used_logic;
    self waittill( "disconnect" );
