...
setupvfxobjs( var_0 )
{
    foreach ( var_2 in var_0 )
    {
        if ( isdefined( var_2.script_noteworthy ) && scripts\engine\utility::string_starts_with( var_2.script_noteworthy, "vfx_" ) )
        {
            ...
            if ( !heli_leaving_monitor( "setupVFXObjs(): obj", var_2 ) )
                continue;

            if ( !heli_leaving_monitor( "setupVFXObjs(): model", var_3 ) )
                continue;
...
delayfxcall( var_0, var_1, var_2 )
{
    ...
    if ( !heli_leaving_monitor( "delayFXCall()", var_1 ) )
        return;
...
animsuite_translation_pingpong()
{
    ...
    for (;;)
    {
        ...
        if ( !heli_leaving_monitor( "animSuite_Translation_PingPong()", self ) )
            return;
...
animsuite_playthreadedsound( var_0, var_1 )
{
    ...
    if ( !heli_leaving_monitor( "animSuite_playThreadedSound()", self ) )
        return;
...
animsuite_translation_once()
{
    ...
    for (;;)
    {
        ...
        if ( !heli_leaving_monitor( "animSuite_Translation_Once()", self ) )
            return;
    }
}
...
animsuite_rotation_pingpong()
{
    ...
    for (;;)
    {
        ...
        if ( !heli_leaving_monitor( "animSuite_Rotation_PingPong()", self ) )
            return;
...
animsuite_rotation_continuous()
{
    ...
    for (;;)
    {
        ...
        if ( !heli_leaving_monitor( "animSuite_Rotation_Continuous()", self ) )
            return;
    }
}
