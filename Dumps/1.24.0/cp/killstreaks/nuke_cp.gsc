...
init()
{
    ...
    level.nuke_expl_struct = scripts\cp_mp\utility\game_utility::find_free_drop_location( "nuke_expl_pos" );
}
...
tryusenukefromstruct( var_0 )
{
    ...
    else if ( !istrue( level.is_ai_in_stealth ) )
    ...
    if ( var_1 != 1 && !istrue( level.is_ai_in_stealth ) )
        scripts\common\utility::set_showing_bomb_wire_pair_to_player( level.insidecuavbunker, "nuke", self.origin );
...
nuke_startlaunchsequence( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7 )
{
    ...
    if ( var_1.streakname != "nuke_select_location" )
    {
        ...
        level thread keypadscriptableused();
    ...
    if ( isdefined( level.nukeinfo.player ) )
    {
                ...
                if ( [[ scripts\cp_mp\utility\script_utility::getsharedfunc( "nuke", "cankill" ) ]]( var_5, var_3 ) )
                {
                    if ( isplayer( var_5 ) )
                    {
                        ...
                        if ( scripts\cp_mp\utility\script_utility::issharedfuncdefined( "player", "isReallyAlive" ) )
                        {
                            ...
                            {
                                if ( !istrue( var_5.playedheliintrovo ) )
                                {
        ...
        if ( !istrue( level.is_ai_in_stealth ) )
        {
            if ( isdefined( level.nukeinfo.player ) && isdefined( var_1 ) )
                level.nukeinfo.player scripts\cp_mp\utility\killstreak_utility::oncooldown( var_1 );
        }
    }
...
{
    level endon( "game_ended" );
    level thread kill_furthest_enemy( 1 );
...
kill_off_non_essential_ai()
{
    ...
    level thread kill_off_all_escort_friendly_ai_riders();
}

kill_off_all_escort_friendly_ai_riders()
{
    level endon( "game_ended" );
...
nuke_startnukedeathfx()
{
    ...
    if ( !istrue( self.playedheliintrovo ) )
    {
