...
isinvehicle( var_0 )
{
    ...
    if ( istrue( var_0 ) )
    {
        if ( isdefined( self.spawn_first_leads_early ) && isdefined( self.spawn_first_leads_early.vehiclename ) )
            return 1;
    }
...
watchthermalinputchange()
{
    ...
    for (;;)
    {
        ...
        var_1 = scripts\engine\utility::splashtime_vip( "input_type_changed", "thermal_handling_ended" );
...
givebmodevloadouts()
{
    level.modeignorevehicleexplosivedamage = [];
}

add_neurotoxin_damage_area()
{
    if ( !self calloutmarkerping_getcreatedtime() )
        level.modeignorevehicleexplosivedamage[self getxuid()] = self;
}

patch_weapons_on_rack_cleararea()
{
    level.modeignorevehicleexplosivedamage[self getxuid()] = undefined;
}

fake_agent_model_setup()
{
    return level.modeignorevehicleexplosivedamage;
}
