...
tryreenablescriptablevfx()
{
    ...
    if ( isdefined( self.accessorylogic ) && self.accessorylogic == "holo" && ( !isdefined( self.quarry_mortars ) || istrue( self.quarry_mortars ) ) )
    {
        ...
        if ( isdefined( var_2 ) )
        {
            if ( var_2 == "iw8_ges_plyr_gesture024" )
            {
                if ( !isdefined( self.quarry_mortars ) )
                    self.quarry_mortars = 0;

                self.quarry_mortars = !self.quarry_mortars;
                self setclientomnvar( "ui_smart_watch_interact", self.quarry_mortars );

                if ( isdefined( self.accessorylogic ) && self.accessorylogic == "holo" )
                {
                    if ( !self.quarry_mortars )
                        startholowatchvfx();
                    else
                        self setscriptablepartstate( "watchVFXPlayer", "off" );
                }
            ...
            {
                if ( !isdefined( self.start_timed_event_on_detection ) )
                    self.start_timed_event_on_detection = 0;

                self.start_timed_event_on_detection = !self.start_timed_event_on_detection;
                self setclientomnvar( "ui_smart_watch_check", self.start_timed_event_on_detection );
            }
