...
init_dvars()
{
    ...
    level.numbers_grenade.f_stun_duration = getdvarfloat( "scr_br_numbers_grenade_stun_duration", 3.0 );
    level.numbers_grenade.f_debuff_duration = getdvarfloat( "scr_br_numbers_grenade_debuff_duration", 10.0 );
...
numbers_grenade_used( var_0 )
{
    ...
    if ( isdefined( var_0.damaged_players ) )
    {
        ...
        foreach ( var_4 in var_0.damaged_players )
            var_4 thread player_effect( var_2 );
    }
...
player_effect( var_0 )
{
    ...
    thread player_effect_end_watcher( var_0 );

    if ( level.numbers_grenade.b_adrenaline_cancel )
        childthread adrenaline_watcher();

    scripts\cp_mp\utility\shellshock_utility::_shellshock( "numbers_grenade_mp", "stun", level.numbers_grenade.f_stun_duration, 1, calculate_interrupt_delay( level.numbers_grenade.f_stun_duration ) );
    self.b_has_numbers_stun = 1;
    self playloopsound( "br_plunder_kiosk_active_idle_lp" );
    self setscriptablepartstate( "headVFX", "numbersVision" );
    self.b_has_numbers_debuff = 1;
    wait( level.numbers_grenade.f_stun_duration );
    remove_stun();
    wait( level.numbers_grenade.f_debuff_duration - level.numbers_grenade.f_stun_duration );
    remove_debuff();
...
player_effect_end_watcher( var_0 )
{
    scripts\engine\utility::_id_143A6( "death_or_disconnect", "numbers_debuff_end_early", "numbers_effect_end" );
