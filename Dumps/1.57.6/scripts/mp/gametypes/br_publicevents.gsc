...
give_intel_data( var_0, var_1 )
{
    ...
    allsupportboxes( "Selected event: " + ai_ignore_all_until_goal( var_5 ) );
    var_6 = getdvarint( "scr_br_pe_force_type", 0 );

    if ( var_6 != 0 )
    {
        allsupportboxes( "Overriding selected event with: " + ai_ignore_all_until_goal( var_6 ) );
        var_5 = var_6;
    }
...
_id_140CD( var_0 )
{
    ...
    foreach ( var_9, var_3 in level.delayedeventtypes )
    {
        if ( var_3.weight <= 0.0 )
        {
            allsupportboxes( ai_ignore_all_until_goal( var_9 ) + " was invalidated due to 0 weight" );
            continue;
        }

        if ( isdefined( var_3._id_140CF ) && ![[ var_3._id_140CF ]]() )
        {
            allsupportboxes( ai_ignore_all_until_goal( var_9 ) + " was invalidated due to failing validate function" );
            continue;
        }
        ...
        if ( isdefined( level.delete_bad_trucks ) && isdefined( level.delete_bad_trucks[var_9] ) )
        {
            ...
            {
                allsupportboxes( ai_ignore_all_until_goal( var_9 ) + " was invalidated due to already activating max times" );
                continue;
            }
        }
        ...
        {
            allsupportboxes( ai_ignore_all_until_goal( var_9 ) + " was invalidated due to this event being on the blacklist" );
            continue;
        }
        ...
        if ( isdefined( var_3.compass ) )
        {
            ...
            if ( var_4 )
            {
                allsupportboxes( ai_ignore_all_until_goal( var_9 ) + " was invalidated due to a previously run event to being on this event's blacklist" );
                continue;
            }
        }
        ...
        if ( isdefined( var_3.guard_door_clip ) )
        {
            ...
            {
                allsupportboxes( ai_ignore_all_until_goal( var_9 ) + " was invalidated due to missing circle event weight for circle" + var_0 );
                continue;
            }
        }
        ...
        if ( isdefined( level.delay_vehicle_to_push_explode_sequence ) && isdefined( level.delay_vehicle_to_push_explode_sequence[var_9] ) )
        {
            ...
            {
                allsupportboxes( ai_ignore_all_until_goal( var_9 ) + " was invalidated due to being a previously ran event within the span of the event repetition buffer." );
                continue;
            }
        }

        allsupportboxes( ai_ignore_all_until_goal( var_9 ) + " was validated" );
