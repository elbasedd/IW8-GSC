...
bot_arena_think()
{
    ...
    if ( !scripts\mp\gametypes\arena.gsc::isnormalloadouts() )
    {
        if ( self botgetdifficultysetting( "advancedPersonality" ) && self botgetdifficultysetting( "strategyLevel" ) > 0 )
            scripts\mp\bots\bots_gametype_gun.gsc::bot_gun_pick_personality_from_weapon( self getcurrentweapon().basename, self.pers["bot_original_personality"] );
    }

    self.gototflagcall = 0;
    ...
    for (;;)
    {
        if ( isdefined( level.arenaflag ) && level.arenaflag.visibleteam == "any" )
        {
            ...
            if ( !scripts\mp\bots\bots_util::bot_is_capturing() || !self.gototflagcall )
