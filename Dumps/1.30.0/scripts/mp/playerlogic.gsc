...
timeuntilspawn( var_0 )
{
    ...
    if ( !isdefined( self.initialrespawndelay ) )
        self.initialrespawndelay = var_1;
...
waitandspawnclient( var_0 )
{
    ...
    {
        if ( isdefined( self.initialrespawndelay ) )
            var_3 = max( self.initialrespawndelay - self.killcamwatchtime, 0 );
        else
...
spawnplayer( var_0, var_1 )
{
    ...
    if ( scripts\mp\utility\game::teamhasinfil( self.team ) && !scripts\mp\flags::gameflag( "infil_started" ) && !isdefined( level.bypassclasschoicefunc ) )
    {
        if ( getdvarint( "scr_force_cac_sre_callstack", 0 ) == 1 && scripts\mp\utility\game::getgametype() == "br" && scripts\mp\utility\game::getsubgametype() != "dmz" && scripts\mp\utility\game::getsubgametype() != "risk" && scripts\mp\utility\game::getsubgametype() != "sandbox" )
        {
            ...
            scripts\mp\utility\script::demoforcesre( "spawnPlayer()" + self.name + " ui_options_menu = 2, hasInfil = " + var_2 + " infil_started = " + var_3 + "bypassClassChoiceFunc = " + var_4 );
        }
    ...
    else
    {
        ...
        scripts\mp\class::loadout_fixcopiedclassstruct( self.pers["copiedClass"], var_7 );
...
spawnspectatormapcam( var_0 )
{
    ...
    scripts\mp\utility\player::printspawnmessage( "playerlogic::spawnSpectatorMapCam() !!!CODE SPAWN!!! @" + var_6.origin );
    ...
    if ( isdefined( var_0 ) && var_0 == 99 )
        ...
        var_15 = scripts\engine\utility::waittill_any_timeout_2( 2, "versus_menu_done", "prematch_over" );
...
callback_playerdisconnect( var_0 )
{
    ...
    if ( isdefined( self.kills ) && isdefined( self.deaths ) )
    {
        if ( scripts\mp\utility\game::getgametype() == "arena" && 3 > self.timeplayed["total"] )
        {
            ...
            var_5 = self.arenadamage;
            setplayerteamrank( self, self.clientid, var_5 );
        }
        else if ( 120 < self.timeplayed["total"] )
        {
            var_6 = ( self.kills - self.deaths ) / ( self.timeplayed["total"] / 60 );
            setplayerteamrank( self, self.clientid, var_6 );
        }
    }
    else
    {
    ...
    var_8 = self.guid;
    logprint( "Q;" + var_8 + ";" + var_7 + ";" + self.name + "\n" );

    if ( analyticsstreamerislogfilestreamingenabled() )
        analyticsstreamerlogfilewritetobuffer( "Q;" + var_8 + ";" + var_7 + ";" + self.name + "\n" );
    ...
    else if ( self.sessionstate != "playing" && scripts\mp\utility\game::getgametype() == "br" && ( scripts\mp\gametypes\br_public.gsc::iswaitingtoentergulag( self ) || scripts\mp\gametypes\br_public.gsc::isloadinggulag( self ) || scripts\mp\gametypes\br_public.gsc::isrespawningfromtoken( self ) ) )
        removefromalivecount( 1 );
...
callback_playerconnect()
{
    ...
    self.hastriggeredwakizashichallenge = undefined;
    self.hastriggeredstreesweeperchallenge = undefined;
    self.lifeid = 0;
    ...
    if ( !isdefined( self.pers["team"] ) )
    {
        ...
        else if ( ( scripts\mp\utility\game::matchmakinggame() || scripts\mp\utility\game::lobbyteamselectenabled() || isgamebattlematch() ) && self.sessionteam != "none" )
        {
            ...
            if ( scripts\mp\utility\game::allowclasschoice() || scripts\mp\utility\game::showfakeloadout() && !isai( self ) )
            {
                if ( getdvarint( "scr_force_cac_sre_callstack", 0 ) == 1 && scripts\mp\utility\game::getgametype() == "br" && scripts\mp\utility\game::getsubgametype() != "dmz" && scripts\mp\utility\game::getsubgametype() != "risk" && scripts\mp\utility\game::getsubgametype() != "sandbox" )
                {
                    ...
                    scripts\mp\utility\script::demoforcesre( "Callback_PlayerConnect() elseif MMG " + self.name + " ui_options_menu = 2, allowClassChoiceFunc = " + var_14 );
                }
        ...
        else
        {
            ...
            if ( scripts\mp\utility\game::allowclasschoice() || scripts\mp\utility\game::showfakeloadout() && !isai( self ) )
            {
                ...
                {
                    var_14 = isdefined( level.allowclasschoicefunc ) && istrue( self [[ level.allowclasschoicefunc ]]() );
                    scripts\mp\utility\script::demoforcesre( "Callback_PlayerConnect() else " + self.name + " ui_options_menu = 2, allowClassChoiceFunc = " + var_14 );
                }
...
totaldisttracking( var_0 )
{
    ...
    for (;;)
    {
        var_4 = scripts\engine\utility::waittill_any_timeout_5( 5.0, "death", "vehicle_enter", "vehicle_change_seat", "vehicle_exit", "afk_tracking_resume" );
...
removefromalivecount( var_0 )
{
    ...
    var_2 = scripts\mp\utility\game::getgametypenumlives() != 0 && var_1 == 0 && !istrue( level.skipplaybodycountsound );
