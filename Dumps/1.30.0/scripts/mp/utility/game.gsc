...
{
    return level.systemlink || privatematch() && getdvarint( "NQORMNOQQM", 0 ) || _func_41F();
}
...
isgroundwarcoremode()
{
    return istrue( level.isgroundwarinfected ) || istrue( level.isgroundwarsiege ) || istrue( level.isgroundwardom );
}
...
canparachutebecut( var_0 )
{
    var_1 = 0;
    var_2 = 0;
    var_3 = scripts\cp_mp\utility\game_utility::getmapname();

    if ( ( getgametype() == "arm" || isgroundwarcoremode() ) && isverdansksubmap() || getdvarint( "scr_game_forceEnableParachuteCut", 0 ) == 1 )
    {
        if ( var_3 != "mp_aniyah" )
        {
            ...
            if ( istrue( level.parachutecancutparachute ) && !istrue( var_0.disablecutchute ) )
                var_2 = 1;
        }
    }
...
classoverrideremovesfists()
{
    ...
    else if ( isdefined( level.gungameweapons ) && level.gungameweapons > 0 )
        return 1;

    return 0;
}

isspecialeventobjhalloween()
{
