...
waitforplayers( var_0 )
{
    ...
    for (;;)
    {
        ...
        var_7 = getstreamedinplayercount();
        ...
        foreach ( var_12 in level.teamnamelist )
        {
            if ( !checkrequiredteamstreamcount( var_12 ) )
            {
...
prematchperiod()
{
    ...
    if ( getdvarint( "scr_live_lobby", 0 ) == 1 && !istrue( level.skipprematch ) )
    {
        ...
        if ( scripts\mp\utility\game::getgametype() == "br" )
        {
            ...
            level thread scripts\mp\gametypes\br_public.gsc::br_delaynojip();
...
teamstarttimer( var_0, var_1 )
{
    ...
    if ( !level.gameended )
    {
        if ( scripts\mp\utility\game::getgametype() == "br" )
        {
            thread scripts\mp\gametypes\br_public.gsc::prematchmusic( var_0 );
            thread scripts\mp\gametypes\br_public.gsc::prematchdialog( var_0 );
            thread scripts\mp\gametypes\br_public.gsc::applyprematchplotarmor();
        }
...
checkroundswitch( var_0 )
{
    ...
    if ( doadjustedsideswitchmodeoverride() )
    {
        if ( roundswitchforfairsides() )
            return 1;
    }
...
doadjustedsideswitchmodeoverride()
{
    ...
    if ( isdefined( var_0 ) )
    {
        foreach ( var_2 in level.players )
        {
            ...
            if ( var_3 == var_0 )
            {
                ...
                if ( isdefined( var_2.playagainsquadconfirmed ) )
                    var_2.playagainsquadconfirmed = !var_2.playagainsquadconfirmed;
                else
                    var_2.playagainsquadconfirmed = 1;

                foreach ( var_2 in var_4 )
                {
                    var_2 setclientomnvar( "ui_br_rocket_ammo", var_0 );
                    waitframe();
                }

                if ( istrue( scripts\mp\utility\game::matchmakinggame() ) )
                {
                    if ( var_2 isfireteamleader() )
                    {
                        var_7 = var_2 getfireteammembers();
                        ...
                        foreach ( var_9 in var_7 )
                        {
                            ...
                            if ( var_10 != var_3 )
                            {
                                if ( isdefined( var_9.playagainsquadconfirmed ) )
                                    var_9.playagainsquadconfirmed = !var_9.playagainsquadconfirmed;
                                else
                                    var_9.playagainsquadconfirmed = 1;

                                foreach ( var_2 in var_4 )
                                {
                                    var_2 setclientomnvar( "ui_br_rocket_ammo", var_10 );
                                    waitframe();
                                }
                            }
                        }
...
handleplayagainsquadstate()
{
    if ( scripts\mp\utility\game::getgametype() == "br" )
    {
        if ( !scripts\mp\utility\game::privatematch() && !scripts\mp\gametypes\br_public.gsc::isbotpracticematch() )
        {
            var_0 = [];

            foreach ( var_2 in level.players )
            {
                if ( isdefined( var_2.playagainsquadconfirmed ) && var_2.playagainsquadconfirmed )
                {
                    var_3 = var_2.team;
                    var_4 = 0;
                    var_5 = var_0.size;
...
callback_startgametype()
{
    ...
    scripts\cp_mp\utility\team_utility.gsc::team_utility_init();
    ...
    thread scripts\mp\challenges_mp::init();
    ...
    if ( scripts\mp\utility\game::matchmakinggame() && !scripts\mp\utility\game::privatematch() )
        level scripts\common\ui::lui_registercallback( "play_again_squad_request", ::playagainsquad_requestreceived );
    ...
    if ( getdvarint( "scr_allow_custom_loadouts", 0 ) == 0 || scripts\mp\utility\game::isclasstableoverridemode() )
...
startgame()
{
    ...
    if ( getdvarint( "scr_game_updated_prematch_allows", 1 ) == 1 )
    {
        foreach ( var_1 in level.players )
        {
            ...
            if ( scripts\mp\utility\game::getgametype() == "br" )
                var_1 scripts\mp\persistence::initteammatecounters();
        }
    }
...
cleanpatchablecollision()
{
    ...
    foreach ( var_3, var_2 in var_0 )
    {
        if ( isdefined( var_2 ) )
        {
            if ( isdefined( level.patchablecollisionreservations ) && istrue( level.patchablecollisionreservations[var_3] ) )
            {
                level.patchablecollisionreservations[var_3] = undefined;
                continue;
            }
    ...
    level.patchablecollisioncleanedup = 1;

    if ( isdefined( level.patchablecollisionreservations ) )
    {
        foreach ( var_3, var_5 in level.patchablecollisionreservations )
        {

        }
    }

    level.patchablecollisionreservations = undefined;
}

reservepatchablecollisionbynumber( var_0 )
{
    if ( istrue( level.patchablecollisioncleanedup ) )
        return;

    if ( !isdefined( level.patchablecollisionreservations ) )
        level.patchablecollisionreservations = [];

    level.patchablecollisionreservations[var_0] = 1;
}
...
displayroundend( var_0, var_1, var_2 )
{
    ...
    if ( scripts\mp\utility\game::waslastround() && scripts\mp\utility\game::iswinbytworulegametype() && scripts\mp\utility\game::shouldtiefinalscoreinwinbytwo() )
    {
        var_0 = "tie";
...
endgame_regularmp( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    if ( isdefined( var_0 ) && level.ctfnukeended )
    {
    ...
    if ( scripts\mp\utility\game::waslastround() )
    {
        if ( scripts\mp\utility\game::getgametype() == "arena" )
        {
            ...
            foreach ( var_7 in level.players )
            {
                ...
                {
                    if ( var_7 == var_4[0] )
                        var_7 scripts\cp_mp\pet_watch::addwatch2v2topscore();
                }
            }
        }
            ...
            if ( !scripts\mp\utility\game::waslastround() )
            {
                ...
                foreach ( var_7 in level.players )
                {
                    ...
                    if ( isalive( var_7 ) && isdefined( var_7.matchdatalifeindex ) )
                        var_7 scripts\common\utility::trycall( level.matchdata_logplayerdeath, var_7.matchdatalifeindex, undefined, undefined, "MOD_ROUND_ENDED", "none", undefined, undefined );
                }
...
endgame_endgame( var_0, var_1, var_2, var_3 )
{
    ...
    if ( scripts\mp\gametypes\br_public.gsc::isbotpracticematch() && !level.forcedend )
    {
        level._waitforlui = 1;
    ...
    if ( !level.doingbroshot )
    {
        if ( scripts\mp\utility\game::getgametype() == "br" )
        {
            scripts\mp\gametypes\br_circle.gsc::hidedangercircle();
    ...
    if ( scripts\mp\utility\game::getgametype() != "br" || scripts\mp\utility\game::getsubgametype() == "dmz" || scripts\mp\utility\game::getsubgametype() == "risk" || scripts\mp\utility\game::getsubgametype() == "kingslayer" || scripts\mp\utility\game::isprophuntgametype() )
        processlobbydata();
    ...
    if ( scripts\mp\gametypes\br_public.gsc::isbotpracticematch() && isdefined( level._luidecision ) && level._luidecision == 2 )
    {
...
getintermissionspawnpointoverride()
{
    ...
    {
        var_0 = spawnstruct();
        var_0.origin = level.brtdm_config.spectatepoint.origin;
        var_0.angles = level.brtdm_config.spectatepoint.angles;
    }
...
processlobbydataforclient( var_0 )
{
    ...
    scripts\mp\matchdata::setgamemodemodifierinplayerdata( var_0 );
}
...
setweaponstat( var_0, var_1, var_2 )
{
    ...
    if ( !isdefined( level.canprocesspickedupweapon ) )
        level.canprocesspickedupweapon = scripts\mp\utility\game::getgametype() == "br" && getdvarint( "scr_track_picked_up_weapon_stats", 1 ) == 1;

    if ( scripts\mp\utility\weapon::ispickedupweapon( var_3 ) && !istrue( level.canprocesspickedupweapon ) )
        return;
    ...
    if ( var_5 == "weapon_grenade" || var_5 == "weapon_explosive" || var_4 == "trophy_mp" || var_4 == "forcepush_mp" )
    {
        ...
        scripts\common\utility::trycall( level.matchdata_logweaponstat, var_4, var_2, var_1, var_6, var_3 );
        return;
    }
    ...
    if ( var_2 == "deaths" )
    {
        ...
        scripts\common\utility::trycall( level.matchdata_logweaponstat, var_9, "deaths", var_1, var_6, var_3 );
