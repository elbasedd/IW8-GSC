...
setupobjective( var_0, var_1, var_2, var_3 )
{
    ...
    if ( istrue( level.halloweendvar ) )
    {
        if ( scripts\mp\utility\game::getgametype() == "hq" )
            var_4 thread waitthenplaycat( var_4 );
        else if ( scripts\mp\utility\game::getgametype() == "grnd" )
            var_4 thread waitthenplayblood( var_4 );
    }

    return var_4;
}

waitthenplaycat( var_0 )
{
    scripts\mp\flags::gameflagwait( "prematch_done" );
    var_1 = anglestoforward( var_0.visuals[0].angles );
    var_2 = var_1 * 5;
    var_3 = var_0.visuals[0] gettagorigin( "j_laptop_tray" );
    playfx( level.hween_fx["ghostcat_hw"], var_3 + var_2 );
}

waitthenplayblood( var_0 )
{
    scripts\mp\flags::gameflagwait( "prematch_done" );
    playfx( level.hween_fx["blood_floor_hw"], getgroundposition( var_0.trigger.origin, 4 ) + ( 0, 0, 2 ) );
}
...
zone_onuse( var_0 )
{
    ...
    if ( istrue( level.halloweendvar ) )
        playfx( level.hween_fx["vanish_hw_fr"], level.zone.trigger.origin + ( 0, 0, 40 ) );
}
...
watchfuturehardpointtouchlist()
{
    ...
    foreach ( var_2 in level.teamnamelist )
    {
        self.futurenumtouching[var_2] = 0;
        self.futuretouchlist[var_2] = [];
    }

    self.futurenumtouching["neutral"] = 0;
    self.futuretouchlist["neutral"] = [];
    self.futuretouchlist["none"] = [];
    self.checkfuturelist = 1;
    ...
    for (;;)
    {
        ...
        if ( scripts\mp\utility\player::isreallyalive( var_4 ) && !isdefined( var_4.touchtriggers[var_0] ) )
        {
            ...
            self.futurenumtouching[var_2]++;
            var_6 = var_4.guid;
            var_7 = spawnstruct();
            var_7.player = var_4;
            var_7.starttime = gettime();
            self.futuretouchlist[var_2][var_6] = var_7;
            var_4.hpscorecount = 0;
        }

        if ( scripts\mp\utility\player::isreallyalive( var_4 ) && !isdefined( var_4.touchtriggers[var_0] ) )
            var_4 thread futuretriggertouchthink( self );
    }
}

futuretriggertouchthink( var_0 )
{
    level endon( "stop_watching_trigger" );
    ...
    if ( isdefined( self ) )
    {
        var_0.futuretouchlist[var_1][self.guid] = undefined;
        self.hpscorecount = 0;
    }
    else
    {
        ...
        foreach ( var_5, var_4 in var_0.futuretouchlist[var_1] )
        {
        ...
        foreach ( var_5 in var_2 )
            var_0.futuretouchlist[var_1][var_5] = undefined;
    }

    var_0.futurenumtouching[var_1]--;
}
