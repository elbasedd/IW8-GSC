...
main()
{
    ...
    level.modeusecondition = ::canusejuggcrate;
...
initrules()
{
    ...
    updategametypedvars();
    level.ontimelimitgraceperiod = getjuggovertimetimeout();
...
getjuggcratecapturetime()
{
    return getdvarfloat( "scr_rugby_juggCaptureTime" );
}

getjuggovertimetimeout()
{
...
onstartgametype()
{
    ...
    setupwaypointicons();
    reserveobjectiveidforjugg();
    thread initrugbyents();
    thread setupinitialstate();
    initspawns();
    initjugg();
    level thread splashgetjugg();
...
initrugbyents()
{
    level.invaliddroppointtriggers = scripts\engine\utility::array_combine( level.outofboundstriggers, level.minetriggers, level.hurttriggers, level.radtriggers );
    initendzoneents();
...
initendzoneent( var_0 )
{
    ...
    foreach ( var_3 in var_1 )
    {
        if ( !isdefined( var_0.chevrons ) )
            level thread setupchevrons( var_0 );
    }
}
...
initjuggcratesetupents()
{
    ...
    foreach ( var_3 in var_1 )
    {
        var_3 = fixupcratespawnpos( var_3 );
...
modeonteamchangedeath( var_0 )
{
    if ( isdefined( level.rugbyjugg ) )
        var_0 setclientomnvar( "ui_rugby_jugg_friendly", scripts\engine\utility::ter_op( var_0.team == level.rugbyjugg.team, 0, 1 ) );
}
...
onplayerspawned( var_0 )
{
    ...
    if ( isdefined( level.rugbyjugg ) && isdefined( level.rugbyjugg.team ) && isdefined( var_0.team ) )
        var_0 setclientomnvar( "ui_rugby_jugg_friendly", scripts\engine\utility::ter_op( var_0.team == level.rugbyjugg.team, 0, 1 ) );
}
...
awardgenericmedals( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9 )
{
    ...
    if ( isdefined( level.rugbyjugg ) )
    {
        ...
        {
            if ( var_1 == level.rugbyjugg )
                var_1 thread scripts\mp\utility\points::giveunifiedpoints( "kill_as_juggernaut" );
            else if ( var_12 == level.rugbyjugg )
                var_1 thread scripts\mp\utility\points::giveunifiedpoints( "kill_juggernaut" );
            else if ( var_1.team == level.rugbyjugg.team && var_1 != level.rugbyjugg )
            {
                var_16 = distancesquared( level.rugbyjugg.origin, var_14 );
            ...
            else if ( var_1.team != level.rugbyjugg.team && var_12.team == level.rugbyjugg.team )
            {
                var_16 = distancesquared( level.rugbyjugg.origin, var_13 );
...
ontimelimitot()
{
    if ( level.rugby.activejuggernauts.size > 0 )
    {
        ...
        {
            setomnvar( "ui_overtime_time", gettime() + getjuggovertimetimeout() * 1000 );
            level.rugby.inot = 1;
            level.canprocessot = 1;
            setjuggovertimetimeout();
            level thread juggaliveovertimewatcher();
        }
    }
    else
    {
...
setupchevrons( var_0 )
{
    ...
    var_0 thread assignrugbyendzonechevrons( var_1, var_0.trigger.targetname );
    var_0 thread updatechevrons( "idle" );
}

assignrugbyendzonechevrons( var_0, var_1 )
{
    wait 1.0;
    var_2 = getentitylessscriptablearray( var_0, "targetname" );
    var_2 = postshipmodifiedrugbychevrons( var_2, var_1 );
    var_3 = [];
...
removerugbychevronsfromarray( var_0, var_1 )
{
    var_2 = [];
    var_3 = [];
...
postshipmodifiedrugbychevrons( var_0, var_1 )
{
    var_2 = [];
    var_3 = [];
...
activatenewjuggernaut( var_0 )
{
    ...
    level thread playattackorfallbackvo( var_0.team, var_6 );
...
playattackorfallbackvo( var_0, var_1 )
{
    wait 3;
...
deactivatejuggernaut( var_0 )
{
    ...
    level.rugby.doprogressvo = undefined;
    level.rugby.progressvotime = undefined;
    level.rugby.juggprogress = undefined;
    level.rugby.juggprevprogress = undefined;
...
watchjuggprogress()
{
    ...
    for (;;)
    {
        ...
        if ( var_9 > 0.8 )
            level thread checkprogressvo( var_9, var_0, var_2 );
        ...
        if ( isdefined( level.rugbyjugg ) )
        {
            ...
            {
                var_7 = var_9;
                level.rugbyjugg thread scripts\mp\utility\points::giveunifiedpoints( "rugby_obj_push" );
            }
        }
...
checkprogressvo( var_0, var_1, var_2 )
{
    if ( !isdefined( level.rugby.doprogressvo ) )
    {
        level.rugby.doprogressvo = 1;
        level.rugby.progressvotime = gettime();
        level.rugby.juggprogress = var_0;
        level.rugby.juggprevprogress = var_0;
    }
    else if ( isdefined( level.rugby.progressvotime ) && level.rugby.progressvotime + 30000 < gettime() )
    {
        level.rugby.doprogressvo = 1;
        level.rugby.progressvotime = gettime();
        level.rugby.juggprogress = var_0;
    ...
    if ( istrue( level.rugby.doprogressvo ) )
    {
...
watchjuggtimeout()
{
    ...
    self endon( "rugby_jugg_end" );
    setjuggtimeout();
    level thread runjuggtimeout( self );
    wait( getjuggtimeout() );
    setomnvar( "ui_rugby_jugg_timer", 0 );
    setomnvar( "ui_rugby_jugg_radial", 0 );
    self suicide();
...
handlejuggjumpspam()
{
    ...
    for (;;)
    {
        ...
        for (;;)
        {
            var_2 = scripts\engine\utility::waittill_any_timeout_1( 1, "jugg_jumped" );
...
{
    var_0 = 0;
    level.juggreservedidstruct = spawnstruct();
    level.juggreservedidstruct.objidnum = scripts\mp\objidpoolmanager::requestreservedid( var_0 );
}
...
activatejuggcrate( var_0, var_1, var_2 )
{
    ...
    if ( !isvaliddroppoint( var_0 ) )
    {
        var_0 = level.crateresetposition;
        var_1 = level.crateresetangles;
    }
...
isvaliddroppoint( var_0 )
{
    var_1 = 1;

    foreach ( var_3 in level.invaliddroppointtriggers )
    {
...
createjuggcrate( var_0, var_1, var_2 )
{
    ...
    if ( istrue( var_2 ) )
    {
        level.crateresetposition = var_0;
        level.crateresetangles = var_1;
...
juggcratemanageuse( var_0 )
{
    ...
    var_1.isuseobject = 1;
    self.useent = scripts\mp\gameobjects::createholduseobject( "neutral", var_1, self.visuals, ( 0, 0, 64 ) );

    foreach ( var_3 in level.teamnamelist )
        self.useent.teamprogress = undefined;

    self.useent.trigger.useobjdistancelimit = 16900;
    ...
    self.useent.trigger setusepriority( -3 );
    self.useent.trigger sethintonobstruction( "hide" );
    self.useent scripts\mp\gameobjects::setusetime( getjuggcratecapturetime() );
    ...
    self.useent.onbeginuse = ::rugbycratetriggerholdonusebegin;
    self.useent.onuse = ::rugbycratetriggerholdonuse;
    self.useent.onenduse = ::rugbycratetriggerholdonuseend;
    self.useent.oncantuse = ::rugbycratetriggeroncantuse;
    self.useent.inuse = 0;

    foreach ( var_6 in level.teamnamelist )
    {
        self.useent.numtouching[var_6] = 0;
        self.useent.touchlist[var_6] = [];
    }
...
rugbycratetriggerholdonusebegin( var_0 )
{
...
rugbycratetriggerholdonuse( var_0 )
{
...
rugbycratetriggerholdonuseend( var_0, var_1, var_2 )
{
...
rugbycratetriggeroncantuse( var_0 )
{
...
canusejuggcrate( var_0 )
{
    ...
    var_2 = var_0 scripts\mp\juggernaut::jugg_getvalidstance( var_1 );
