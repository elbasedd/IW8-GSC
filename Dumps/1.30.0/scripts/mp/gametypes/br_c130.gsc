...
spawnc130pathstructnew( var_0, var_1 )
{
    ...
    var_5 = var_5 + ( 0, 0, getc130knownsafeheight() );

    if ( !scripts\mp\gametypes\br_gametypes.gsc::isfeaturedisabled( "randomizeCircleCenter" ) )
        var_2 = var_2 + var_5;
    ...
    if ( scripts\mp\gametypes\br_gametypes.gsc::isfeatureenabled( "planeConstrainedFromMapCenter" ) )
        var_2 = constraincenterpointinsideoob( var_2 );
    ...
    if ( scripts\mp\gametypes\br_public.gsc::isbotpracticematch() )
        ...
        var_6 = ( 0, getdvarfloat( "scr_br_c130PathAngle", var_6[1] ), 0 );

    var_10 = level.br_level.br_circleradii[0];
    ...
    if ( scripts\mp\gametypes\br_gametypes.gsc::isfeatureenabled( "planeUseCircleRadius" ) )
        var_11 = var_10;

    var_12 = spawnc130pathstructnewinternal( var_2, var_6, var_11 );
    var_13 = scripts\cp_mp\parachute::getc130height();
    var_12.startpt = ( var_12.startpt[0], var_12.startpt[1], var_13 );
    var_12.endpt = ( var_12.endpt[0], var_12.endpt[1], var_13 );
    return var_12;
}

constraincenterpointinsideoob( var_0 )
{
    var_1 = physics_createcontents( [ "physicscontents_playertrigger" ] );
    var_2 = scripts\engine\trace::ray_trace_ents( level.br_level.br_mapcenter, var_0, level.outofboundstriggersplanetrace, var_1 );
...
spawnc130pathstructnewinternal( var_0, var_1, var_2 )
{
    ...
    if ( !scripts\mp\gametypes\br_gametypes.gsc::isfeaturedisabled( "planeSnapToOOB" ) )
    {
    ...
    if ( scripts\mp\gametypes\br_gametypes.gsc::isfeaturedisabled( "planeSnapToOOB" ) )
    {
        if ( !ispointinbounds( var_4 ) )
            var_6 = snappointtooutofboundstriggertrace( var_0, var_4 );

        if ( !ispointinbounds( var_5 ) )
            var_7 = snappointtooutofboundstriggertrace( var_0, var_5 );
    }
...
updatec130pathomnvars( var_0 )
{
    ...
    if ( scripts\mp\gametypes\br_gametypes.gsc::isfeaturedisabled( "c130PlaneLine" ) )
        return;
...
createtestc130path( var_0, var_1 )
{
    ...
    if ( isdefined( level.br_level.br_mapcenter ) && isdefined( level.br_level.br_mapsize ) )
    {
        ...
        level thread scripts\mp\gametypes\br_analytics.gsc::branalytics_planepath( var_2.centerpt, var_2.angle[1], var_2.startptui, var_2.endptui );
    }
    else
...
spawnplayertoc130()
{
    ...
    scripts\mp\gametypes\br_infils.gsc::endprematchskydiving();
    thread playerputinc130( level.br_ac130 );
    self.brneverlanded = 1;

    if ( istrue( self.isbrsquadleader ) && getdvar( "scr_br_gametype", "" ) != "dmz" && getdvar( "scr_br_gametype", "" ) != "risk" )
        level scripts\mp\gametypes\br_public.gsc::brleaderdialogplayer( "deploy_squad_leader", self, 1, 0 );
}
...
gunship_spawn( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    var_6 = "veh8_mil_air_acharlie130_magma_rigid";

    if ( isdefined( level.br_c130_model_override ) )
        var_6 = level.br_c130_model_override;

    var_5.innards = spawn( "script_model", var_0 );
    var_5.innards setmodel( var_6 );
...
gunship_spawnvfx()
{
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" )
    {
        ...
        if ( !isdefined( level.br_c130_scriptable_fx_part ) )
            level.br_c130_scriptable_fx_part = "br_fx";

        self setscriptablepartstate( level.br_c130_scriptable_fx_part, "clouds" );
    }
    else
    {
...
inmapbounds( var_0 )
{
    ...
    if ( ( scripts\mp\gametypes\br_public.gsc::isdmzbotpracticematch() || scripts\mp\gametypes\br_public.gsc::isbotpracticematch() ) && isdefined( level.br_ac130 ) )
...
ispointinbounds( var_0, var_1 )
{
    ...
    if ( ( scripts\mp\gametypes\br_public.gsc::isbotpracticematch() || scripts\mp\gametypes\br_public.gsc::isdmzbotpracticematch() ) && isdefined( level.br_ac130 ) )
        return var_2;
...
killaftertime( var_0, var_1 )
{
    ...
    if ( isdefined( self.animstruct ) )
    {
        ...
        scripts\mp\gametypes\br_public.gsc::cleanac130struct( self.animstruct );
    }
    else
    {
...
listenjump( var_0, var_1 )
{
    ...
    for (;;)
    {
        ...
        var_6 = scripts\mp\gametypes\br_public.gsc::isplayerbrsquadleader() || isdefined( self.pers["squadMemberIndex"] ) && self.pers["squadMemberIndex"] == 0;
...
leaveplane( var_0, var_1, var_2, var_3 )
{
    ...
    thread scripts\mp\gametypes\br_gametypes.gsc::runbrgametypefunc( "onLeaveAC130" );
...
{
    var_3 = scripts\engine\utility::ter_op( var_0 scripts\mp\gametypes\br_public.gsc::isplayerbrsquadleader(), 2, var_2 );
