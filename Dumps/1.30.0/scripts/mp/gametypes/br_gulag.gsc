...
initgulag()
{
    ...
    if ( scripts\mp\gametypes\br_gametypes.gsc::isfeaturedisabled( "gulag" ) )
    {
        level.usegulag = 0;
        return;
    }
    ...
    level.gulag.nuketown = getdvarint( "scr_br_gulag_nuketown", 1 );
    level.gulag.rebirth = getdvarint( "scr_br_gulag_rebirth", 0 );
    level.gulag.countdowntime = getdvarint( "scr_br_fc_countdownTime", 3 );
    level.gulag.defaultplunder = getdvarint( "scr_br_fc_defaultPlunder", 5 );
    level.gulag.chairbreakout = getdvarint( "scr_br_gulag_chair", 1 );
    level.gulag.voices = getdvarint( "scr_br_gulag_voices", 1 );
    ...
    if ( istrue( level.gulag.arenaflag ) )
        scripts\mp\gametypes\br_dom_quest.gsc::setupdom();
    ...
    if ( level.gulag.betting )
    {
        ...
        if ( getdvarint( "scr_br_fc_countdownTime", 3 ) == 3 )
            level.gulag.countdowntime = 15;
    }

    if ( !isdefined( level.gulag.arenas[0].jailspawns[0].chair ) )
        level.gulag.chairbreakout = 0;
...
getfilteredarenastructs()
{
    ...
    foreach ( var_3 in var_0 )
    {
        ...
        if ( istrue( level.gulag.nuketown ) && var_4 || !istrue( level.gulag.nuketown ) && !var_4 )
...
setuparena( var_0 )
{
    ...
    foreach ( var_4 in var_2 )
    {
        ...
        if ( var_4.script_noteworthy == "voices" )
            var_0.rooms[var_0.rooms.size] = var_4;
    }
...
spawnlootweapons( var_0 )
{
    ...
    foreach ( var_16 in var_0 )
    {
        ...
        {
            var_7.part = var_17;
            scripts\mp\gametypes\br_pickups.gsc::loot_setitemcount( var_7, var_3[var_17] );
            var_7.keepinmap = 1;
...
gulagfixuparena( var_0 )
{
    var_1 = 5000;

    if ( !isdefined( var_0.nextfixupcheckms ) )
    {
        var_0.nextfixupcheckms = gettime() + var_1;
        return;
    }

    if ( gettime() > var_0.nextfixupcheckms )
    {
        var_0.jailedplayers = scripts\engine\utility::array_removeundefined( var_0.jailedplayers );
        updatematchqueuepositions( var_0 );

        if ( var_0.gulagindex == 0 )
        {
            level.gulag.laststandplayers = scripts\engine\utility::array_removeundefined( level.gulag.laststandplayers );

            foreach ( var_3 in level.gulag.laststandplayers )
            {
                if ( !istrue( var_3.inlaststand ) )
                    level.gulag.laststandplayers = scripts\engine\utility::array_remove( level.gulag.laststandplayers, var_3 );
            }
        }
    ...
    for (;;)
    {
        ...
        if ( istrue( level.br_prematchstarted ) )
        {
            if ( istrue( level.gulag.shutdown ) )
            {
                ...
                else if ( level.gulag.endonshutdown == 3 && ( istrue( var_0.shutdown ) || !ismatchpending( var_0 ) && !anyplayersinlaststandhold() ) )
                {
                    foreach ( var_3 in var_0.jailedplayers )
                    {
                        if ( isalive( var_3 ) )
...
gulaglaststandholdremove( var_0 )
{
    if ( istrue( level.gulag.shutdown ) && anyplayersinlaststandhold() )
    {
        ...
        {
            level.gulag.laststandplayers = scripts\engine\utility::array_remove( level.gulag.laststandplayers, var_0 );
            return 1;
        }
        else
            level.gulag.laststandplayers = scripts\engine\utility::array_removeundefined( level.gulag.laststandplayers );
...
playergulagdonesplash()
{
    ...
    self.gulagdone = 1;
    scripts\mp\gametypes\br_killstreaks.gsc::dangernotifyplayer( self, "gulag_closed", undefined, 2 );
    scripts\mp\gametypes\br_public.gsc::brleaderdialogplayer( "gulag_gulag_close", self );

    if ( istrue( level.br_pickups.respawntokenclosewithgulag ) && scripts\mp\gametypes\br_public.gsc::hasrespawntoken() )
    {
...
watchlethaldelay( var_0 )
{
    ...
    var_0.lethaldelayendtime = var_0.lethaldelaystarttime + level.gulag.lethaldelay * 1000 + level.gulag.countdowntime * 1000 + 2000;
...
startfightvo( var_0 )
{
    ...
    foreach ( var_2 in var_0.jailedplayers )
    {
        ...
        {
            scripts\mp\gametypes\br_public.gsc::brleaderdialogplayer( "gulag_next", var_2, 0 );
            continue;
        }

        if ( var_2.gulagposition == 2 )
            scripts\mp\gametypes\br_public.gsc::brleaderdialogplayer( "gulag_taunt", var_2, 0 );
    }
}
...
{
    var_3 = level.gulag.countdowntime;
...
playerpulloutofgulagwin( var_0, var_1 )
{
    level endon( "game_ended" );
    self endon( "disconnect" );
...
playergetnextarena()
{
    ...
    foreach ( var_4 in level.gulag.arenas )
    {
        ...
        {
            if ( var_6.size == 1 && isdefined( var_6[0] ) && var_6[0].team != self.team && ( !isdefined( var_1 ) || var_6[0].jailtime < var_2 ) )
            {
                var_1 = var_4;
...
playerjailtimeout( var_0 )
{
    ...
    if ( isplayermatched( var_0, self ) )
    {
        var_2 = self.gulagposition * ( level.gulag.timelimit + getovertimelength() + level.gulag.countdowntime + 2 + 2.0 + 1.0 + 1 );
        var_3 = gettime() + var_2 * 1000;

        while ( var_3 > gettime() && isplayermatched( var_0, self ) )
            waitframe();
    }
...
initplayerjail( var_0 )
{
    ...
    scripts\mp\gametypes\br_analytics.gsc::branalytics_gulagstart( self, scripts\engine\utility::ter_op( istrue( var_0 ), "default", "debug" ) );
    ...
    scripts\mp\gametypes\br_quest_util.gsc::onplayerentergulag();
    scripts\mp\gametypes\br_alt_mode_escape.gsc::escaperadioonentergulag();
    var_6 = gettime();
    ...
    if ( level.gulag.betting )
    {
        var_7 = level.gulag.defaultplunder;
    ...
    if ( scripts\mp\gametypes\br_gametypes.gsc::isbrgametypefuncdefined( "playerPreSpawnGulagJail" ) )
        scripts\mp\gametypes\br_gametypes.gsc::runbrgametypefunc( "playerPreSpawnGulagJail" );
    ...
    self setclientomnvar( "ui_br_infil_started", 1 );
    var_8 = var_1.fightover && playerismatchedplayerready( var_1 );
    var_9 = undefined;

    if ( level.gulag.chairbreakout )
        var_9 = 0.5;
    ...
    if ( !var_8 )
    {
        scripts\mp\gametypes\br_public.gsc::brleaderdialogplayer( "gulag_spawn", self, 0 );
        thread playerchairbreakout( var_1, var_2 );
    }

    foreach ( var_11 in level.teamdata[self.team]["players"] )
    {
        ...
        if ( var_11 != self )
        {
            var_11 thread scripts\mp\hud_message::showsplash( "br_gulag_teammate_in", undefined, self );
            scripts\mp\gametypes\br_public.gsc::brleaderdialogplayer( "gulag_teammate_gulag", var_11 );
        }
    }
...
playerjailwaitvo( var_0 )
{
    ...
    scripts\mp\gametypes\br_public.gsc::brleaderdialogplayer( "gulag_obj_wait", self, 0 );
}
...
{
    thread playercinematicfadeout( var_0 );
    gulagfadefromblackspectatorsofplayer();
}

gulagfadetoblackspectatorsofplayer()
{
    var_0 = scripts\mp\gametypes\br_spectate.gsc::getspectatorsofplayer( self );
...
initplayerarena( var_0, var_1, var_2 )
{
    ...
    scripts\mp\gametypes\br_public.gsc::brleaderdialogplayer( "gulag_spawn_rules", self, 0 );
    var_3 = gulagisfaded();
    ...
    scripts\mp\gametypes\br_armor.gsc::givearmorvalue( level.gulag.numarmorhealth );
    ...
    self allowprone( 1 );
    self allowcrouch( 1 );
    wait( level.gulag.countdowntime - 1 );
    wait 1;
...
gulagvictory( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7 )
{
    ...
    var_1 thread playerchairbreakoutinterrupt();
    ...
    foreach ( var_9 in level.teamdata[var_1.team]["players"] )
    {
        ...
        if ( var_9 != var_1 )
        {
            var_9 thread scripts\mp\hud_message::showsplash( "br_gulag_teammate_out", undefined, var_1 );
            scripts\mp\gametypes\br_public.gsc::brleaderdialogplayer( "gulag_teammate_win", var_9 );
        }
    }
    ...
    if ( var_2 )
    {
        ...
        {
            if ( var_4 == "timeout" )
                scripts\mp\gametypes\br_public.gsc::brleaderdialogplayer( "gulag_timeout", var_1, 0 );
            else
                scripts\mp\gametypes\br_public.gsc::brleaderdialogplayer( "gulag_noenemy", var_1, 0 );
        }
        ...
        scripts\mp\gametypes\br_public.gsc::brleaderdialogplayer( "gulag_win", var_1, 0 );
    ...
    var_12 = scripts\mp\gametypes\br_public.gsc::getdefaultstreamhinttimeoutms() / 1000;
    var_13 = var_1 playergulaggetrespawnpoint( 0, var_12 );
    var_14 = var_1 playerprestreamrespawnorigin( var_13 );
    wait 2.0;
    ...
    wait 1;
    var_1 gulagwinnerrespawn( var_5, var_4, var_13, 1, var_14, undefined, var_6, var_3, var_7 );
}

playerprestreamrespawnorigin( var_0 )
{
    var_1 = scripts\mp\gametypes\br_public.gsc::playerstreamhintdroptoground( var_0.origin, var_0.height );
...
gulagplayerlost( var_0 )
{
    ...
    scripts\mp\gametypes\br.gsc::markplayeraseliminated( self );
    level thread scripts\mp\gametypes\br.gsc::updateplayerandteamcountui();
    scripts\mp\gametypes\br_public.gsc::brleaderdialogplayer( "gulag_lose", self, 0, 1 );
    scripts\mp\gametypes\br_public.gsc::brleaderdialogteamexcludeplayer( "gulag_teammate_lose", self.team, self, 0, 0 );
    ...
    scripts\mp\gametypes\br_analytics.gsc::branalytics_gulagend( self, "loser" );
    ...
    if ( isalive( self ) )
    {
        if ( !scripts\mp\gametypes\br_gametypes.gsc::isfeaturedisabled( "teamSpectate" ) )
            scripts\mp\gametypes\br_spectate.gsc::migratespectators( self, undefined, 1 );
...
playerspawningasspectator()
{
    if ( istrue( level.usegulag ) && istrue( self.wasingulag ) && level.teamdata[self.team]["aliveCount"] > 0 )
        scripts\mp\gametypes\br_public.gsc::brleaderdialogplayer( "gulag_buyback", self, 0, 1 );
}
...
isplayervalidrespawntarget( var_0, var_1 )
{
    ...
    if ( !isalive( var_0 ) || var_0 scripts\mp\gametypes\br_public.gsc::isplayerinorgoingtogulag() || istrue( var_0.br_iseliminated ) )
        return 0;
...
playergetbestrespawnteammate( var_0, var_1 )
{
    ...
    foreach ( var_20 in var_18 )
    {
        ...
        if ( istrue( var_20 scripts\mp\gametypes\br_public.gsc::isplayerbrsquadleader() ) )
            break;
    }

    if ( isdefined( var_9 ) )
        return var_9;
    else if ( scripts\mp\utility\game::getsubgametype() == "dmz" || scripts\mp\utility\game::getsubgametype() == "risk" )
    {
        var_22 = scripts\mp\utility\teams::getteamdata( self.team, "teamCount" );

        if ( var_22 == 1 && !istrue( self.disconnecting ) && !scripts\mp\outofbounds::ispointinoutofbounds( self.origin ) )
            return self;
    }
...
playergetbestrespawnmissionorigin()
{
    ...
    foreach ( var_5, var_1 in level.questinfo.quests )
    {
        ...
        {
            if ( var_4 == self.team && !scripts\mp\gametypes\br_quest_util.gsc::isquestinstancealocale( var_3 ) )
            {
...
playergulaggetrespawnpoint( var_0, var_1, var_2 )
{
    var_3 = undefined;
    var_4 = undefined;
    var_5 = var_2;
    ...
    {
        var_8 = getsafeoriginaroundpoint( self.lastdeathpos, var_7, var_1 );

        if ( isdefined( var_8 ) )
        {
            var_3 = scripts\mp\gametypes\br_public.gsc::droptogroundmultitrace( var_8 );
            var_4 = getanglesfacingorigin( var_3, self.lastdeathpos );
        }
    }
    ...
    {
        var_9 = playergetbestrespawnteammate( var_0, var_1 );
        ...
        if ( isdefined( var_9 ) )
        {
            ...
            {
                var_3 = getsafeoriginaroundpoint( var_9.origin, var_7, var_1 );

                if ( isdefined( var_3 ) )
                {
                    var_3 = scripts\mp\gametypes\br_public.gsc::droptogroundmultitrace( var_3 );
                    var_4 = getanglesfacingorigin( var_3, var_9.origin );
    ...
    if ( !isdefined( var_3 ) && var_7 >= 0 )
    {
        var_12 = playergetbestrespawnmissionorigin();

        if ( isdefined( var_12 ) )
        {
            var_3 = getsafeoriginaroundpoint( var_12, var_7, var_1 );
            var_4 = getanglesfacingorigin( var_3, var_12 );
        }
    }
    ...
    if ( !isdefined( var_3 ) && var_7 >= 0 )
    {
        var_13 = playergetbestdropbagorigin( var_1 );

        if ( isdefined( var_13 ) )
        {
            var_3 = getsafeoriginaroundpoint( var_13, var_7, var_1 );
            var_4 = getanglesfacingorigin( var_3, var_13 );
        }
    }
    ...
    if ( !isdefined( var_3 ) )
    {
        if ( isdefined( level.br_circle ) && isdefined( level.br_circle.safecircleent ) )
        {
            if ( getdvarint( "scr_br_useClosestSafePointFromSquadmate", 1 ) )
            {
                ...
                var_3 = scripts\mp\gametypes\br_circle.gsc::closestsafepointfromsquadmate( self, var_1, var_14 );
            }
            ...
            if ( !isdefined( var_3 ) )
            {
                ...
                var_3 = scripts\mp\gametypes\br_circle.gsc::getrandompointinboundscircle( var_15, var_16, var_18, var_18, var_19, var_20, 0, var_1 );
            }
        }
    ...
    if ( var_6 )
    {
        ...
        if ( isdefined( level.br_circle ) )
        {
            ...
            var_28 = getgulagclosedcircleindex();
        ...
        if ( isdefined( level.respawnheightoverride ) )
            var_5 = level.respawnheightoverride;

        var_30 = ( 0, 0, var_5 );
        var_3 = scripts\mp\gametypes\br.gsc::getoffsetspawnoriginmultitrace( var_3, var_30 );
    }
...
gulagwinnerrespawn( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9 )
{
    ...
    if ( isdefined( var_1 ) )
        scripts\mp\gametypes\br_analytics.gsc::branalytics_gulagend( self, var_1 );
    ...
    gulagstreamexit();
    playerteleportgulag( var_13, var_12 );
    ...
    self playerhide();
    thread playercleanupentondisconnect( var_14 );
    waitframe();
    ...
    if ( isdefined( level.parachutedeploydelay ) )
        var_15 = level.parachutedeploydelay;
    ...
    scripts\mp\gametypes\br_alt_mode_escape.gsc::escaperadioonleavegulag();
    scripts\mp\gametypes\br_gametypes.gsc::runbrgametypefunc( "gulagWinnerRespawn", self );
    wait 0.5;
...
{
    self endon( "disconnect" );
    var_1 = scripts\mp\gametypes\br_gametypes.gsc::runbrgametypefunc( "triggerRespawnOverlay" );
...
{
    if ( !istrue( level.usegulag ) && !scripts\mp\gametypes\br_gametypes.gsc::isfeaturedisabled( "gulagWinnerRestoreLoadoutUseGulag" ) )
        return 0;
...
gulagwinnertableloadout()
{
    if ( istrue( self.gulagtableloadout ) )
    {
        ...
        if ( var_0 > -1 )
            self.pers["gamemodeLoadout"] = level.gulagloadouts[var_0];
        else
...
trygulagspawn()
{
    ...
    if ( scripts\mp\gametypes\br_public.gsc::hasrespawntoken() && !scripts\mp\gametypes\br_pickups.gsc::respawntokendisabled() )
    {
...
showbettinghud( var_0, var_1 )
{
    ...
    var_2.hudclear = var_1 scripts\mp\hud_util::createfontstring( "default", 1.2 );
    var_2.hudclear scripts\mp\hud_util::setpoint( "TOP", "TOP", var_3, var_4 - 15 );
    var_2.hudclear.label = &"MP_GULAG_BETTING/BET_CLEAR";
    var_2.hudclear.alpha = 0.0;
    var_5 = -90;
    var_6 = 65;
    var_2.hudbetonplayerleft = var_1 scripts\mp\hud_util::createfontstring( "default", 1.2 );
    var_2.hudbetonplayerleft scripts\mp\hud_util::setpoint( "TOP", "TOP", var_5, var_6 );
    var_2.hudbetonplayerleft.label = &"MP_GULAG_BETTING/BET_INCREASE_LEFT";
    var_2.hudbetonplayerleft.alpha = 1.0;
    var_2.hudoddspercentleft = var_1 scripts\mp\hud_util::createfontstring( "default", 1.25 );
    var_2.hudoddspercentleft scripts\mp\hud_util::setpoint( "TOP", "TOP", var_5 - 20, var_6 + 12 );
    var_2.hudoddspercentleft.label = &"MP_GULAG_BETTING/ODDS_PERCENT";
    var_2.hudoddspercentleft setvalue( randomint( 60 ) + 20 );
    var_2.hudoddspercentleft.alpha = 1.0;
    var_2.hudoddsratioleft = var_1 scripts\mp\hud_util::createfontstring( "default", 1.25 );
    var_2.hudoddsratioleft scripts\mp\hud_util::setpoint( "TOP", "TOP", var_5 + 20, var_6 + 12 );
    var_2.hudoddsratioleft.label = &"MP_GULAG_BETTING/ODDS_RATIO";
    var_2.hudoddsratioleft setvalue( randomint( 10 ) + 1 );
    var_2.hudoddsratioleft.alpha = 1.0;
    var_2.hudplayerleft = var_1 scripts\mp\hud_util::createfontstring( "default", 1.5 );
    var_2.hudplayerleft scripts\mp\hud_util::setpoint( "TOP", "TOP", var_5, var_6 + 25 );
    var_2.hudplayerleft.label = &"";

    if ( isdefined( self.fighters[var_2.playerleft] ) )
        var_2.hudplayerleft setplayernamestring( self.fighters[var_2.playerleft] );

    var_2.hudplayerleft.alpha = 1.0;
    var_2.hudamountleft = var_1 scripts\mp\hud_util::createfontstring( "default", 1.5 );
    var_2.hudamountleft scripts\mp\hud_util::setpoint( "TOP", "TOP", var_5, var_6 + 40 );
    var_2.hudamountleft.label = &"MP_GULAG_BETTING/CURRENT_BET";
    var_2.hudamountleft.alpha = 0.0;
    var_2.hudamountleft scripts\mp\hud::fontpulseinit();
    var_7 = 90;
    var_8 = 65;
    var_2.hudbetonplayerright = var_1 scripts\mp\hud_util::createfontstring( "default", 1.2 );
    var_2.hudbetonplayerright scripts\mp\hud_util::setpoint( "TOP", "TOP", var_7, var_8 );
    var_2.hudbetonplayerright.label = &"MP_GULAG_BETTING/BET_INCREASE_RIGHT";
    var_2.hudbetonplayerright.alpha = 1.0;
    var_2.hudoddspercentright = var_1 scripts\mp\hud_util::createfontstring( "default", 1.25 );
    var_2.hudoddspercentright scripts\mp\hud_util::setpoint( "TOP", "TOP", var_7 - 20, var_8 + 12 );
    var_2.hudoddspercentright.label = &"MP_GULAG_BETTING/ODDS_PERCENT";
    var_2.hudoddspercentright setvalue( randomint( 60 ) + 20 );
    var_2.hudoddspercentright.alpha = 1.0;
    var_2.hudoddsratioright = var_1 scripts\mp\hud_util::createfontstring( "default", 1.25 );
    var_2.hudoddsratioright scripts\mp\hud_util::setpoint( "TOP", "TOP", var_7 + 20, var_8 + 12 );
    var_2.hudoddsratioright.label = &"MP_GULAG_BETTING/ODDS_RATIO";
    var_2.hudoddsratioright setvalue( randomint( 10 ) + 1 );
    var_2.hudoddsratioright.alpha = 1.0;
    var_2.hudplayerright = var_1 scripts\mp\hud_util::createfontstring( "default", 1.5 );
    var_2.hudplayerright scripts\mp\hud_util::setpoint( "TOP", "TOP", var_7, var_8 + 25 );
    var_2.hudplayerright.label = &"";

    if ( isdefined( self.fighters[var_2.playerright] ) )
        var_2.hudplayerright setplayernamestring( self.fighters[var_2.playerright] );

    var_2.hudplayerright.alpha = 1.0;
    var_2.hudamountright = var_1 scripts\mp\hud_util::createfontstring( "default", 1.5 );
    var_2.hudamountright scripts\mp\hud_util::setpoint( "TOP", "TOP", var_7, var_8 + 40 );
    var_2.hudamountright.label = &"MP_GULAG_BETTING/CURRENT_BET";
    var_2.hudamountright.alpha = 0.0;
    var_2.hudamountright scripts\mp\hud::fontpulseinit();
    var_2.playerbeton = -1;
    var_2.amount = 0;
    updatebethud( var_2 );
...
watchbetplaced( var_0 )
{
    ...
    for (;;)
    {
        var_3 = var_0.owner scripts\engine\utility::waittill_any_return_2( var_1, var_2 );
        var_4 = -1;

        if ( var_3 == var_1 )
            var_4 = var_0.playerleft;
        else if ( var_3 == var_2 )
            var_4 = var_0.playerright;
...
updatebethud( var_0 )
{
    ...
    if ( var_0.playerbeton == -1 )
    {
        var_0.hudbetonplayerleft.label = &"MP_GULAG_BETTING/BET_CHANGE_LEFT";
        var_0.hudbetonplayerright.label = &"MP_GULAG_BETTING/BET_CHANGE_RIGHT";
        var_0.hudclear.alpha = 0.0;
        var_0.hudamountleft setvalue( 0 );
        var_0.hudamountright setvalue( 0 );
        var_0.hudamountleft.alpha = 0.0;
        var_0.hudamountright.alpha = 0.0;
    }
    else if ( var_0.playerbeton == var_0.playerleft )
    {
        var_0.hudbetonplayerleft.label = &"MP_GULAG_BETTING/BET_INCREASE_LEFT";
        var_0.hudbetonplayerright.label = &"MP_GULAG_BETTING/BET_CHANGE_RIGHT";
        var_0.hudclear.alpha = 1.0;
        var_0.hudamountleft setvalue( var_0.amount * var_1 );
        var_0.hudamountleft.alpha = 1.0;
        var_0.hudamountright.alpha = 0.0;
    }
    else if ( var_0.playerbeton == var_0.playerright )
    {
        var_0.hudbetonplayerleft.label = &"MP_GULAG_BETTING/BET_CHANGE_LEFT";
        var_0.hudbetonplayerright.label = &"MP_GULAG_BETTING/BET_INCREASE_RIGHT";
        var_0.hudclear.alpha = 1.0;
        var_0.hudamountright setvalue( var_0.amount * var_1 );
        var_0.hudamountright.alpha = 1.0;
        var_0.hudamountleft.alpha = 0.0;
    }
}
...
cleanupbethud( var_0, var_1 )
{
    ...
    var_2 = [ var_0.hudbetonplayerleft, var_0.hudbetonplayerright, var_0.hudamountleft, var_0.hudamountright, var_0.hudplayerleft, var_0.hudplayerright, var_0.hudoddspercentleft, var_0.hudoddsratioleft, var_0.hudoddspercentright, var_0.hudoddsratioright, var_0.hudavailable, var_0.hudclear ];
...
endbetting( var_0, var_1 )
{
    ...
    {
        var_4 = [ var_3.hudclear, var_3.hudavailable, var_3.hudoddspercentright, var_3.hudoddspercentleft, var_3.hudoddsratioright, var_3.hudoddsratioleft, var_3.hudbetonplayerleft, var_3.hudbetonplayerright ];

        if ( var_3.playerbeton != -1 )
        {
            if ( var_3.playerbeton == var_3.playerleft )
            {
                var_4[var_4.size] = var_3.hudplayerright;
                var_4[var_4.size] = var_3.hudbetonplayerright;
                var_4[var_4.size] = var_3.hudamountright;
            }
            else if ( var_3.playerbeton == var_3.playerright )
            {
                var_4[var_4.size] = var_3.hudplayerleft;
                var_4[var_4.size] = var_3.hudbetonplayerleft;
                var_4[var_4.size] = var_3.hudamountleft;
            }
        }
        else
...
_completebet( var_0, var_1, var_2 )
{
    ...
    if ( var_2 )
    {
        ...
        if ( var_1.playerbeton == var_1.playerleft )
        {
            var_1.hudplayerleft.color = var_3;
            var_1.hudplayerleft.label = &"MP_GULAG_BETTING/FIGHER_WINNER";
            var_1.hudamountleft.color = var_3;
            var_1.hudamountleft.label = &"MP_GULAG_BETTING/AMOUNT_WON";
            var_1.owner playlocalsound( "ammo_crate_use" );
            var_1.hudamountleft thread scripts\mp\hud::fontpulse( var_1.owner );
        }
        else if ( var_1.playerbeton == var_1.playerright )
        {
            var_1.hudplayerright.color = var_3;
            var_1.hudplayerright.label = &"MP_GULAG_BETTING/FIGHER_WINNER";
            var_1.hudamountright.color = var_3;
            var_1.hudamountright.label = &"MP_GULAG_BETTING/AMOUNT_WON";
            var_1.owner playlocalsound( "ammo_crate_use" );
            var_1.hudamountright thread scripts\mp\hud::fontpulse( var_1.owner );
        }
    }
    else
    {
        ...
        if ( var_1.playerbeton == var_1.playerleft )
        {
            var_1.hudplayerleft.color = var_4;
            var_1.hudplayerleft.label = &"MP_GULAG_BETTING/FIGHER_LOSER";
            var_1.hudamountleft.color = var_4;
            var_1.hudamountleft.label = &"MP_GULAG_BETTING/AMOUNT_LOST";
            var_1.hudamountleft thread scripts\mp\hud::fontpulse( var_1.owner );
        }
        else if ( var_1.playerbeton == var_1.playerright )
        {
            var_1.hudplayerright.color = var_4;
            var_1.hudplayerright.label = &"MP_GULAG_BETTING/FIGHER_LOSER";
            var_1.hudamountright.color = var_4;
            var_1.hudamountright.label = &"MP_GULAG_BETTING/AMOUNT_LOST";
            var_1.hudamountright thread scripts\mp\hud::fontpulse( var_1.owner );
        }
    }
...
playergulagautowin( var_0, var_1, var_2, var_3 )
{
    ...
    [var_6, var_7] = var_4 playerhandlesponsor( var_0, var_1, var_3 );
    var_4.respawningfromtoken = 1;
    var_8 = var_4 playerwaitforprestreaming();
    var_9 = scripts\mp\gametypes\br_public.gsc::getdefaultstreamhinttimeoutms() / 1000;
    var_10 = var_4 playergulaggetrespawnpoint( 0, var_9 );
    var_11 = var_4 playerprestreamrespawnorigin( var_10 );
    self.forcespawnorigin = var_11;
    var_12 = scripts\mp\gametypes\br_gametypes.gsc::runbrgametypefunc( "playerGulagAutoWinWait", var_0, var_1 );
    ...
    wait( var_14 );
    var_4 scripts\mp\hud_message::clearsplashqueue();
    ...
    var_4 scripts\mp\gametypes\br_spectate.gsc::playerclearspectatekillchainsystem();
    var_4.respawningfromtoken = undefined;

    if ( !isdefined( var_0 ) && !istrue( var_2 ) )
        var_4 thread triggerrespawnoverlay( 4 );

    var_4 gulagwinnerrespawn( 1, var_7, var_10, 1, var_11, undefined, var_6, var_3 );
}

playerhandlesponsor( var_0, var_1, var_2, var_3 )
{
    var_4 = self;
    var_5 = var_4;
    var_6 = "token";
    ...
    if ( isdefined( var_0 ) )
    {
        var_7 = istrue( var_4.br_iseliminated ) && !isalive( var_4 ) && istrue( var_4.wasingulag );
        var_5 = var_0;
        ...
        if ( !istrue( var_3 ) )
        {
            ...
            var_4 thread triggerrespawnoverlay( var_8 );
        }
    }
    ...
    if ( isdefined( var_0 ) || istrue( var_2 ) )
    {
        ...
        scripts\mp\gametypes\br.gsc::unmarkplayeraseliminated( var_4 );
    }

    scripts\mp\gametypes\br_analytics.gsc::branalytics_gulagstart( var_4, var_6 );
    scripts\mp\gametypes\br_analytics.gsc::branalytics_respawn( self, int( isdefined( var_0 ) ) );
...
{
    var_0 = self;
    var_1 = istrue( var_0.prestreamingspectatetarget );

    if ( var_1 )
    {
        var_0 scripts\mp\utility\lower_message::setlowermessageomnvar( 76 );

        while ( istrue( var_0.prestreamingspectatetarget ) )
            waitframe();
    }
...
creategulagloadoutarry()
{
    ...
    var_0 = tablelookupgetnumcols( level.gulagloadouttable ) - 1;

    for ( var_1 = 0; var_1 < var_0; var_1++ )
        level.gulagloadouts[level.gulagloadouts.size] = creategulagarenaloadout( var_1 );
}
...
creategulagarenaloadout( var_0 )
{
    var_1["loadoutArchetype"] = "archetype_assault";
    var_1["loadoutPrimary"] = tablelookup( level.gulagloadouttable, 0, "loadoutPrimary", var_0 + 1 );
    var_1["loadoutPrimaryAttachment"] = tablelookup( level.gulagloadouttable, 0, "loadoutPrimaryAttachment1", var_0 + 1 );
    var_1["loadoutPrimaryAttachment2"] = tablelookup( level.gulagloadouttable, 0, "loadoutPrimaryAttachment2", var_0 + 1 );
    var_1["loadoutPrimaryAttachment3"] = tablelookup( level.gulagloadouttable, 0, "loadoutPrimaryAttachment3", var_0 + 1 );
    var_1["loadoutPrimaryAttachment4"] = tablelookup( level.gulagloadouttable, 0, "loadoutPrimaryAttachment4", var_0 + 1 );
    var_1["loadoutPrimaryAttachment5"] = tablelookup( level.gulagloadouttable, 0, "loadoutPrimaryAttachment5", var_0 + 1 );
    var_1["loadoutPrimaryCamo"] = tablelookup( level.gulagloadouttable, 0, "loadoutPrimaryCamo", var_0 + 1 );
    var_1["loadoutPrimaryReticle"] = tablelookup( level.gulagloadouttable, 0, "loadoutPrimaryReticle", var_0 + 1 );
    var_1["loadoutSecondary"] = tablelookup( level.gulagloadouttable, 0, "loadoutSecondary", var_0 + 1 );
    var_1["loadoutSecondaryAttachment"] = tablelookup( level.gulagloadouttable, 0, "loadoutSecondaryAttachment1", var_0 + 1 );
    var_1["loadoutSecondaryAttachment2"] = tablelookup( level.gulagloadouttable, 0, "loadoutSecondaryAttachment2", var_0 + 1 );
    var_1["loadoutSecondaryAttachment3"] = tablelookup( level.gulagloadouttable, 0, "loadoutSecondaryAttachment3", var_0 + 1 );
    var_1["loadoutSecondaryAttachment4"] = tablelookup( level.gulagloadouttable, 0, "loadoutSecondaryAttachment4", var_0 + 1 );
    var_1["loadoutSecondaryAttachment5"] = tablelookup( level.gulagloadouttable, 0, "loadoutSecondaryAttachment5", var_0 + 1 );
    var_1["loadoutSecondaryCamo"] = tablelookup( level.gulagloadouttable, 0, "loadoutSecondaryCamo", var_0 + 1 );
    var_1["loadoutSecondaryReticle"] = tablelookup( level.gulagloadouttable, 0, "loadoutSecondaryReticle", var_0 + 1 );
    var_1["loadoutMeleeSlot"] = "none";
    var_1["loadoutEquipmentPrimary"] = tablelookup( level.gulagloadouttable, 0, "loadoutEquipmentPrimary", var_0 + 1 );
    var_1["loadoutEquipmentSecondary"] = tablelookup( level.gulagloadouttable, 0, "loadoutEquipmentSecondary", var_0 + 1 );
    ...
    var_1["loadoutPerks"] = [ tablelookup( level.gulagloadouttable, 0, "loadoutPerk1", var_0 + 1 ), tablelookup( level.gulagloadouttable, 0, "loadoutPerk2", var_0 + 1 ), tablelookup( level.gulagloadouttable, 0, "loadoutPerk3", var_0 + 1 ), tablelookup( level.gulagloadouttable, 0, "loadoutExtraPerk1", var_0 + 1 ), tablelookup( level.gulagloadouttable, 0, "loadoutExtraPerk2", var_0 + 1 ), tablelookup( level.gulagloadouttable, 0, "loadoutExtraPerk3", var_0 + 1 ) ];
...
playergivearenaloadout( var_0, var_1 )
{
    ...
    self.pers["gamemodeLoadout"] = level.gulagloadouts[var_1];
    ...
    self.gulagloadoutindex = level.gulagloadouts[var_1]["tableColumn"];
...
getloadoutindex()
{
    ...
    if ( var_0 > -1 && var_0 < level.gulagloadouts.size )
        return var_0;

    var_1 = randomint( level.gulagloadouts.size );
    return var_1;
}
...
setupdomendflag( var_0 )
{
    ...
    var_3 = scripts\mp\gametypes\obj_dom.gsc::setupobjective( var_0.dom.trigger, "neutral", undefined, 1 );
    ...
    if ( !self.neutralized )
    {
        foreach ( var_3 in self.arena.arenaplayers )
        {
            if ( isalive( var_3 ) && var_3.team != var_1 )
                var_3 thread gulagplayerlost( self.arena );
        }
...
getcinematicref()
{
    if ( istrue( level.gulag.rebirth ) )
        return "rebirth_gulag_intro";
    else if ( istrue( level.gulag.nuketown ) )
...
getcinematiclength()
{
    if ( istrue( level.gulag.nuketown ) )
        return 35000;
    else
        return 17000;
}
...
playerchairbreakout( var_0, var_1 )
{
    if ( !level.gulag.chairbreakout )
        return;
    ...
    playerchairbreakoutcontrols( 1 );
    thread playerchairbreakoutdisconnect( var_3, var_4 );
    ...
    thread playerchairbreakoutinput();
    self playerlinkto( var_3, "tag_player", 0, 30, 30, 45, 60, 0 );
    var_3 hide();
    var_3 showtoplayer( self );
    playerloopbreakoutanimations( var_3, var_4, var_2 );
    ...
    var_3 waittillmatch( "chair", "end" );
    thread playerchairbreakoutcleanup( var_3, var_4 );
}

playerloopbreakoutanimations( var_0, var_1, var_2 )
{
    self endon( "playerChairBreakoutCleanup" );
...
playerchairbreakoutinput()
{
    ...
    self playerlinktoabsolute( self.playerrig, "tag_player" );
    self.playerrig hide();
    self.playerrig showtoplayer( self );
    self.straps hide();
...
playerchairbreakoutinterrupt()
{
    playerchairbreakoutcleanup( self.playerrig, self.straps );
}

playerchairbreakoutcleanup( var_0, var_1 )
{
    if ( !level.gulag.chairbreakout )
        return;

    if ( !isdefined( self.playerrig ) )
        return;
    ...
    if ( isdefined( self ) )
    {
        ...
        self playershow( 1 );
        playerchairbreakoutcontrols( 0 );
        self.playerrig = undefined;
...
playerchairbreakoutdisconnect( var_0, var_1 )
{
    self endon( "playerChairBreakoutCleanup" );
    self waittill( "death_or_disconnect" );
    thread playerchairbreakoutcleanup( var_0, var_1 );
}

playerchairbreakoutcontrols( var_0 )
{
    if ( var_0 )
        self disableweapons();
    else
...
getnextchair( var_0 )
{
    if ( isdefined( var_0.chaircounter ) )
    {
        var_0.chaircounter++;
        var_0.chaircounter = var_0.chaircounter % var_0.chairs.size;
    }
    else
        var_0.chaircounter = 0;

    var_1 = var_0.chairs[var_0.chaircounter];
    return var_1;
}

setupvoices()
{
    if ( !istrue( level.gulag.voices ) )
        return;

    level.gulag.conversations = [];
    var_0 = 0;
    level.gulag.conversations[var_0] = spawnstruct();
    level.gulag.conversations[var_0].aliasprefix = "dx_brm_rm1_gulag_muffled_chatter_";
    level.gulag.conversations[var_0].aliases = [ 10, 20, 30, 40 ];
    level.gulag.conversations[var_0].delays = [ 1, 1, 1, 0 ];
    var_0++;
    level.gulag.conversations[var_0] = spawnstruct();
    level.gulag.conversations[var_0].aliasprefix = "dx_brm_rm1_gulag_muffled_chatter_";
    level.gulag.conversations[var_0].aliases = [ 90, 100, 110 ];
    level.gulag.conversations[var_0].delays = [ 1, 1, 0 ];
    var_0++;
    level.gulag.conversations[var_0] = spawnstruct();
    level.gulag.conversations[var_0].aliasprefix = "dx_brm_rm2_gulag_announcement_";
    level.gulag.conversations[var_0].aliases = [ 10 ];
    level.gulag.conversations[var_0].delays = [ 0 ];
    var_0++;
    level.gulag.conversations[var_0] = spawnstruct();
    level.gulag.conversations[var_0].aliasprefix = "dx_brm_rm2_gulag_announcement_";
    level.gulag.conversations[var_0].aliases = [ 20 ];
    level.gulag.conversations[var_0].delays = [ 0 ];
    var_0++;
    level.gulag.conversations[var_0] = spawnstruct();
    level.gulag.conversations[var_0].aliasprefix = "dx_brm_rm2_gulag_announcement_";
    level.gulag.conversations[var_0].aliases = [ 30 ];
    level.gulag.conversations[var_0].delays = [ 0 ];
    var_0++;
    level.gulag.conversations[var_0] = spawnstruct();
    level.gulag.conversations[var_0].aliasprefix = "dx_brm_rm2_gulag_announcement_";
    level.gulag.conversations[var_0].aliases = [ 40 ];
    level.gulag.conversations[var_0].delays = [ 0 ];
    var_0++;
    level.gulag.conversations[var_0] = spawnstruct();
    level.gulag.conversations[var_0].aliasprefix = "dx_brm_rm2_gulag_recording_";
    level.gulag.conversations[var_0].aliases = [ 10 ];
    level.gulag.conversations[var_0].delays = [ 0 ];
    var_0++;
    level.gulag.conversations[var_0] = spawnstruct();
    level.gulag.conversations[var_0].aliasprefix = "dx_brm_rm2_gulag_recording_";
    level.gulag.conversations[var_0].aliases = [ 20, 30, 40, 50, 60, 20, 70, 80, 90, 80, 20, 30, 100, 110 ];
    level.gulag.conversations[var_0].delays = [ 0.5, 1.0, 1.5, 0.7, 0.3, 1.0, 0.4, 0.8, 1.0, 0.5, 1.0, 1.5, 2.0, 0 ];
    var_0++;
    level.gulag.conversations = scripts\engine\utility::array_randomize( level.gulag.conversations );
}

startvoices( var_0 )
{
    if ( !istrue( level.gulag.voices ) || var_0.rooms.size == 0 )
        return;

    var_0.voiceent = spawn( "script_model", var_0.origin );
    var_0.voiceent setmodel( "tag_origin" );
    var_1 = randomint( level.gulag.conversations.size );
    var_2 = randomint( var_0.rooms.size );
    ...
    for (;;)
    {
        ...
        var_5 = var_0.rooms[var_2];
        var_6 = level.gulag.conversations[var_1];
        var_0.voiceent.origin = var_5.origin;
        var_0.voiceent dontinterpolate();
        waitframe();

        for ( var_7 = 0; var_7 < var_6.aliases.size; var_7++ )
        {
            var_8 = var_6.aliasprefix + var_6.aliases[var_7];
            var_0.voiceent playsoundonmovingent( var_8 );
            var_9 = lookupsoundlength( var_8, 1 ) / 1000.0;
            var_10 = var_6.delays[var_7] + var_9;
            wait( var_10 );
        }
        ...
        if ( var_1 >= level.gulag.conversations.size )
            var_1 = 0;
    }
}
