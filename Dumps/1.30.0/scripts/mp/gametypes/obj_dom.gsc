...
setupobjective( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    if ( istrue( level.gamesofsummercallbacks ) )
        var_9.specialeventonuse = ::specialeventonusegos;
    else if ( istrue( level.halloweendvar ) && scripts\mp\utility\game::getgametype() != "br" )
        var_9.specialeventonuse = ::specialeventonusehw;
    ...
    if ( istrue( level.halloweendvar ) && scripts\mp\utility\game::getgametype() != "br" )
    {
        var_19 = "decor_halloween_scarecrow";
        var_9.flagmodel.isscarecrow = 1;
    }
    else
        var_19 = "military_dom_flag_neutral";

    var_9.flagmodel setmodel( var_19 );
    var_9.flagmodel.angles = checkmapflagangles( var_9 );
    var_9.outlineent = var_9.flagmodel;

    if ( istrue( level.halloweendvar ) && scripts\mp\utility\game::getgametype() != "br" )
    {
        var_9 thread halloweenbats();
        var_9 thread domhalloweenstalker();
    }
...
dompoint_onuse( var_0 )
{
    ...
    if ( istrue( level.flagneutralization ) )
    {
        ...
        if ( var_3 == "neutral" )
        {
            ...
            if ( isdefined( self.specialeventonuse ) )
                [[ self.specialeventonuse ]]();
        }
        else
        {
...
specialeventonusegos()
{
    ...
    if ( var_7["fraction"] != 1 )
    {
        ...
        if ( var_7["fraction"] != 1 )
        {
            playfx( level.gos_fx["gos_fireworks"], var_7["position"] );
            playfx( level.gos_fx["gos_fireworks"], var_7["position"], anglestoforward( self.flagmodel.angles ) + ( -10, -10, 0 ) );
            playfx( level.gos_fx["gos_fireworks"], var_7["position"], anglestoforward( self.flagmodel.angles ) + ( 10, 10, 0 ) );
            level thread scripts\mp\gametypes\common.gsc::waitthenplayfireworkexplode( var_7["position"] );
        }
    }
    else
    {
        playfx( level.gos_fx["gos_fireworks"], var_2 );
        playfx( level.gos_fx["gos_fireworks"], var_2, anglestoforward( self.flagmodel.angles ) + ( -10, -10, 0 ) );
        playfx( level.gos_fx["gos_fireworks"], var_2, anglestoforward( self.flagmodel.angles ) + ( 10, 10, 0 ) );
        level thread scripts\mp\gametypes\common.gsc::waitthenplayfireworkexplode( var_2 );
    }
}

specialeventonusehw()
{
    if ( isdefined( self.hwvfx_active ) )
        return;
    else
    {
        self.hwvfx_active = 1;
        ...
        if ( var_7["fraction"] != 1 )
            playfx( level.hween_fx["blood_ceil_hw"], var_7["position"] - ( 0, 0, 5 ), ( -180, -180, 0 ), ( -180, -180, -180 ) );

        playfx( level.hween_fx["blood_floor_hw"], self.flagmodel.origin + ( 0, 0, 2 ) );
        var_0 = 4000;
        var_1 = 20;
        var_2 = self.flagmodel.origin;
        var_3 = var_2 + ( 0, 0, var_0 );
        ...
        if ( var_7["fraction"] != 1 )
            playfx( level.hween_fx["bats_fly_hw"], var_7["position"] + ( 0, 0, 50 ) );
        else
            playfx( level.hween_fx["bats_fly_hw"], self.flagmodel.origin + ( 0, 0, 50 ) );
    }
}
...
halloweenbats()
{
    ...
    if ( var_7["fraction"] != 1 )
        playfx( level.hween_fx["bats_hw"], var_7["position"] );
    else
        playfx( level.hween_fx["bats_hw"], self.flagmodel.origin + ( 0, 0, 200 ) );
}
...
updateflagstate( var_0, var_1, var_2, var_3 )
{
    ...
    if ( istrue( level.halloweendvar ) && scripts\mp\utility\game::getgametype() != "br" )
    {
        if ( istrue( var_3 ) )
            ...
            self.flagmodel thread scarecrowsuprise();
        else if ( isdefined( var_2 ) )
            playfx( level.hween_fx["vanish_hw_fr"], self.flagmodel.origin + ( 0, 0, 60 ) );
    }
    else if ( isdefined( var_2 ) )
    {
        if ( var_2 == "allies" )
...
scarecrowsuprise()
{
    ...
    wait 0.55;
    playfx( level.hween_fx["vanish_hw_fr"], self.origin + ( 0, 0, 60 ) );
...
domhalloweenstalker()
{
    ...
    for (;;)
    {
        ...
        if ( isplayer( var_0 ) )
        {
            if ( !isdefined( self.stalkertarget ) )
            {
                self.stalkertarget = var_0;
                thread watchstalker( self.flagmodel, var_0 );
            }
        }
...
watchstalker( var_0, var_1 )
{
    level endon( "game_ended" );
    self notify( "new_stalker_target" );
    self endon( "new_stalker_target" );

    while ( isdefined( self.stalkertarget ) )
    {
        if ( !validatestalked( var_0, var_1 ) )
        {
            self.stalkertarget = undefined;
            self.stalkerdamage = 0;
            self.eyesdamageon = undefined;
            self.angrydamaged = undefined;
            self.reflectdamage = undefined;
            self.flagmodel rotateto( ( 0, self.flagmodel.angles[1], self.flagmodel.angles[2] ), 0.2, 0.1, 0.1 );
        }
        else if ( isdefined( self.stalkertarget ) && scripts\mp\utility\player::isreallyalive( self.stalkertarget ) )
        {
            ...
            {
                self.flagmodel.fwd = anglestoforward( self.flagmodel.angles );
                var_2 = self.stalkertarget.origin;
...
watchscarecrowdamage()
{
    ...
    for (;;)
    {
        ...
        if ( isdefined( var_1 ) && isdefined( self.stalkertarget ) && self.stalkertarget == var_1 )
            self.stalkerdamage = self.stalkerdamage + var_0;

        if ( isdefined( self.stalkerdamage ) )
        {
            if ( self.stalkerdamage > 100 && !isdefined( self.eyesdamageon ) )
                self.eyesdamageon = 1;

            if ( self.stalkerdamage > 200 && !isdefined( self.angrydamaged ) )
            {
                self.angrydamaged = 1;
                playfx( level.hween_fx["vanish_hw_en"], self.flagmodel.origin + ( 0, 0, 80 ) );
            }

            if ( self.stalkerdamage > 300 && !isdefined( self.reflectdamage ) )
            {
                self.reflectdamage = 1;
                thread retaliateonattacker( var_1 );
            }
        }
...
validatestalked( var_0, var_1 )
{
    var_2 = 0.05;
    var_3 = var_1 getstalkeeorigin();
    var_4 = var_0 getscarecroworigin();
