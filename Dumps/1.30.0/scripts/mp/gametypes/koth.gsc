...
hardpointmainloop()
{
    ...
    if ( level.firstzoneactivationdelay )
    {
        ...
        level.zone thread scripts\mp\gametypes\obj_zonecapture.gsc::lockedradialunfill( level.firstzoneactivationdelay, level.zone.curorigin + level.zone.offset3d );
...
getfirstzone()
{
    ...
    var_0 thread scripts\mp\gametypes\obj_zonecapture.gsc::watchfuturehardpointtouchlist();
    level.futurezone thread scripts\mp\gametypes\obj_zonecapture.gsc::watchfuturehardpointtouchlist();
    return var_0;
}
...
getnextzone()
{
    ...
    else
    {
        if ( isdefined( level.remappedhpzoneorder ) )
        {
            ...
            if ( var_30 > level.remappedhpzoneorder.size - 1 )
                var_30 = 1;

            level.futurezone = level.objectives[level.remappedhpzoneorder[var_30]];
        }
        else
        {
            level.prevzoneindex++;
            ...
            level.futurezone = level.objectives[scripts\engine\utility::string( var_30 )];
        }
...
hqactivatenextzone( var_0, var_1 )
{
    ...
    else if ( level.zoneactivationdelay )
    {
        ...
        level.zoneislocked = 1;
        level thread scripts\mp\hud_message::updatematchstatushintforallplayers( undefined, 36, 36 );
        level.zone thread scripts\mp\gametypes\obj_zonecapture.gsc::lockedradialunfill( level.zoneactivationdelay, level.zone.curorigin + level.zone.offset3d );
...
give_capture_credit( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    ...
    if ( isplayer( var_6 ) )
    {
        var_6 thread scripts\common\utility::trycall( level.matchdata_loggameevent, "capture", var_6.origin );

        if ( isexcessivescoreboosting( var_6 ) && !scripts\mp\utility\game::isanymlgmatch() )
        {
    ...
    while ( !level.gameended )
    {
        ...
        if ( !level.zone.stalemate && !level.gameended )
        {
            ...
            foreach ( var_5 in level.zone.touchlist[var_3] )
            {
                ...
                if ( isdefined( var_5.player.hpscorecount ) && var_5.player.hpscorecount > 3 )
                {
                    var_5.player.hpscorecount = 0;
                ...
                if ( !isdefined( var_5.player.hpscorecount ) )
                    var_5.player.hpscorecount = 0;
                else
                    var_5.player.hpscorecount++;
