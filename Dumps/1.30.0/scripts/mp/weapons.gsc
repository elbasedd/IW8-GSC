...
scriptedmountdisable( var_0 )
{
    ...
    if ( shoulddisablemount( var_0 ) )
    {
        ...
        thread scriptedmountdisable_cleanup();
    }
}

scriptedmountdisable_cleanup()
{
...
mtxvfx_manage( var_0 )
{
    ...
    var_1 = "cancel_" + var_0;
    thread mtxvfx_cleanup( var_1 );
    childthread mtxvfx_on( var_0, var_1 );
    childthread mtxvfx_watchweaponswitch( var_0, var_1 );
    childthread mtxvfx_watchmantle( var_0, var_1 );
}

mtxvfx_watchweaponswitch( var_0, var_1 )
{
    for (;;)
    {
        ...
        if ( self isonladder() )
            mtxvfx_ladderhack( var_0, var_1 );
    }
}

mtxvfx_watchmantle( var_0, var_1 )
{
    for (;;)
    {
...
mtxvfx_ladderhack( var_0, var_1 )
{
    ...
    childthread mtxvfx_on_ladderhack( var_0, var_1 );
}

mtxvfx_on( var_0, var_1 )
{
    self notify( var_1 );
    self endon( var_1 );
    self.playingmtxvfx = 1;
...
mtxvfx_on_ladderhack( var_0, var_1 )
{
    self notify( var_1 );
    self endon( var_1 );
    self.playingmtxvfx = 1;
...
mtxvfx_off( var_0 )
{
    ...
    if ( istrue( self.playingmtxvfx ) )
    {
        self setscriptablepartstate( "weaponVFXViewmodel", "neutral" );
        self setscriptablepartstate( "weaponVFXWorldModel", "neutral" );
    }

    self.playingmtxvfx = undefined;
}

mtxvfx_cleanup( var_0 )
{
    self endon( "disconnect" );
    self endon( "newMTXVFXStateSet" );
    scripts\engine\utility::waittill_any_2( "death", "weapon_change" );
...
updatelauncherusage()
{
    ...
    switch ( var_1 )
    {
        ...
        case "iw8_sh_aalpha12":
            thread scripts\cp_mp\aalpha12.gsc::initusage();
            break;
    }
    ...
    switch ( var_1 )
    {
        ...
        case "iw8_sn_crossbow":
            ...
            break;
        case "iw8_sn_xmike109":
            thread scripts\cp_mp\xmike109::usageloop( var_0 );
            break;
        case "iw8_sh_aalpha12":
            thread scripts\cp_mp\aalpha12.gsc::usageloop( var_0 );
            break;
    }
}
...
savetogglescopestates()
{
    ...
    foreach ( var_2 in var_0 )
    {
        if ( !isundefinedweapon( self.primaryweaponobj ) && var_2 == self.primaryweaponobj || !isundefinedweapon( self.secondaryweaponobj ) && var_2 == self.secondaryweaponobj )
        {
            if ( isdefined( var_2.scope ) && istogglescope( var_2.scope ) && !stockblockstoggle( var_2.backpiece ) )
            {
                var_3 = scripts\mp\utility\weapon::getcompleteweaponnamenoalt( var_2 );
...
{
    self endon( "death" );
    level.onweapondropcreated scripts\cp_mp\utility\callback_group::callback_trigger( self, var_0, self.objweapon );
    ...
    for (;;)
    {
        ...
        if ( var_6 )
        {
            ...
            if ( var_7.size > 2 )
            {
                ...
                foreach ( var_12 in var_7 )
                {
                    ...
                    if ( scripts\mp\utility\weapon::ismeleeoverrideweapon( var_12 ) )
                        var_9 = 1;
        ...
        level.onweapondroppickedup scripts\cp_mp\utility\callback_group::callback_trigger( self, var_2, self.objweapon );
...
watchgrenadethrows()
{
    ...
    switch ( var_0.weapon_name )
    {
        ...
        case "geiger_counter_mp":
            thread scripts\mp\equipment\geiger_counter.gsc::geiger_counter_used( var_0 );
            break;
        case "offhand_spotter_scope_mp":
            thread scripts\mp\equipment\binoculars::binoculars_used( var_0 );
            break;
        case "tac_cover_mp":
        ...
        case "advanced_vehicle_drop_marker_mp":
            thread scripts\mp\equipment\advanced_supply_drop::advanced_vehicle_drop_marker_used( var_0 );
            break;
...
{
    return [ "apc_russian", "atv", "big_bird", "cargo_truck", "cargo_truck_mg", "cop_car", "hoopty", "hoopty_truck", "jeep", "large_transport", "light_tank", "little_bird", "little_bird_mg", "medium_transport", "pickup_truck", "tac_rover", "technical", "van", "loot_chopper" ];
}
...
updateviewkickscale( var_0 )
{
    ...
    else if ( isdefined( self.overrideviewkickscale ) )
    {
        ...
        if ( var_1 == 1 )
            ...
            var_0 = self.overrideviewkickscaleromeo700;
        else if ( var_1 == 5 )
...
getplayerspeedbyweapon( var_0 )
{
    ...
    else
    {
        ...
        {
            if ( var_2.inventorytype != "primary" && var_2.inventorytype != "altmode" || scripts\mp\utility\weapon::ismeleeoverrideweapon( var_2 ) )
            {
...
initweaponsourcetable()
{
    level.weaponsourcetable = [];
    var_0 = 0;
    ...
    for (;;)
    {
        ...
        level.weaponsourcetable[var_3] = var_2;
        var_0++;
    }
}

isweaponavailable( var_0 )
{
    if ( !isdefined( level.weaponsourcetable ) )
        initweaponsourcetable();

    var_1 = level.weaponsourcetable[var_0];

    if ( isdefined( var_1 ) && var_1 == "t9" )
    {
        if ( !istrue( level.warzone ) )
            return 0;
...
buildweaponmap()
{
        ...
        if ( var_3 != "" )
        {
            ...
            if ( var_5 != "" )
            {
                ...
                if ( var_6 != "" )
                {
                    ...
                    if ( var_7 > -1 && isweaponavailable( var_3 ) )
                    {
                        if ( !isdefined( level.weapongroupdata[var_5] ) )
                            level.weapongroupdata[var_5] = [];

                        level.weapongroupdata[var_5][level.weapongroupdata[var_5].size] = var_3;
                    }
                    else
                        level.weaponmapdata[var_3].uihidden = 1;
                }
            }
    ...
    {
        var_1++;
        var_14 = tablelookupbyrow( "loot/weapon_ids.csv", var_1, 0 );

        if ( var_14 == "" )
            break;

        var_3 = tablelookupbyrow( "loot/weapon_ids.csv", var_1, 1 );
        var_15 = tablelookupbyrow( "loot/weapon_ids.csv", var_1, 6 );
        var_16 = scripts\mp\utility\weapon::getweaponvarianttablename( var_3 );
        var_17 = tablelookup( var_16, 1, var_15, 0 );
        ...
        var_18 = var_3 + "|" + var_17;
        level.weaponlootmapdata[var_18] = spawnstruct();
        level.weaponlootmapdata[var_18].variantid = int( var_17 );
        var_19 = tablelookup( var_16, 1, var_15, 3 );

        if ( var_19 != "" )
            level.weaponlootmapdata[var_18].assetoverridename = var_19;

        var_20 = tablelookup( "loot/weapon_ids.csv", 6, var_15, 5 );
        level.weaponlootmapdata[var_18].islocked = int( var_17 ) != 0 && int( var_20 ) == 99;
        level.weaponlootmapdata[var_18].isavailabletocoremp = isweaponavailable( var_3 );
        var_21 = tablelookup( var_16, 1, var_15, 4 );
        var_11 = parseattachdefaulttoidmap( var_21 );
        ...
        if ( isdefined( var_11 ) )
        {
            ...
            level.weaponlootmapdata[var_18].attachdefaulttoidmap = var_11;
        }

        var_22 = [];

        for ( var_23 = 5; var_23 <= 15; var_23++ )
        {
            var_24 = tablelookup( var_16, 1, var_15, var_23 );

            if ( var_24 != "" )
            {
                var_25 = strtok( var_24, "|" );

                if ( var_25.size == 2 )
                    var_22[var_25[0]] = int( var_25[1] );
                else
                    var_22[var_25[0]] = 0;
            }
        }
        ...
        if ( var_22.size > 0 )
            level.weaponlootmapdata[var_18].attachcustomtoidmap = var_22;

        var_26 = tablelookup( var_16, 1, var_15, 16 );

        if ( var_26 != "" )
        {
            var_27 = [];
            var_28 = strtok( var_26, " " );

            foreach ( var_30 in var_28 )
            {
                var_31 = strtok( var_30, "|" );

                if ( var_31.size != 2 )
                    continue;

                var_27[var_31[0]] = int( var_31[1] );
            }

            if ( var_27.size > 0 )
                level.weaponlootmapdata[var_18].attachextratoidmap = var_27;
        }
    }

    var_33 = 2;

    foreach ( var_44, var_35 in var_13 )
    {
        var_36 = 1;
        var_37 = 0;
        var_38 = [];
        ...
        for (;;)
        {
            var_39 = var_44 + "|" + var_36;

            if ( isdefined( level.weaponlootmapdata[var_39] ) )
            {
                var_37++;

                if ( var_38.size )
                {
                    foreach ( var_41 in var_38 )
                    {
                        var_42 = var_44 + "|" + var_41;
                        level.weaponlootmapdata[var_42] = spawnstruct();
                        level.weaponlootmapdata[var_42].variantid = var_41;
                        level.weaponlootmapdata[var_42].islocked = 1;
                        level.weaponlootmapdata[var_42].isavailabletocoremp = 0;
                    }

                    var_38 = [];
                }
            }
            else if ( var_37 == var_35 || var_38.size >= var_33 )
                break;
            else
                var_38[var_38.size] = var_36;
...
watchdropweapons()
{
    ...
    for (;;)
    {
        ...
        if ( isdefined( var_0 ) && isdefined( var_1 ) && !scripts\mp\utility\weapon::ismeleeonly( var_1 ) && !scripts\mp\utility\weapon::ismeleeoverrideweapon( var_1 ) && !scripts\mp\utility\weapon::isknifeonly( var_1 ) )
        {
...
runcamoscripts( var_0 )
{
    ...
    if ( issubstr( var_0, "reactiveCamo" ) )
        thread reactivecamo();
}
...
reactivecamo()
{
    self endon( "disconnect" );
    self endon( "death" );
    self endon( "reactiveCamo" );

    if ( !isdefined( self.reactivecamokillcount ) )
        self.reactivecamokillcount = 0;
    ...
    for (;;)
    {
        ...
        self.reactivecamokillcount = self.reactivecamokillcount + 1;

        if ( self.reactivecamokillcount / 3 == var_0 )
        {
            var_1 = int( self.reactivecamokillcount / 3 );

            if ( var_1 > 5 )
                break;

            self setscriptablepartstate( "activeCamo", "stage" + var_1 );
            var_0++;
        }
    }
}
...
mtxtesladeath( var_0 )
{
    playfxfortag( "j_shoulder_ri", "teslaLimb" );
    playfxfortag( "j_shoulder_le", "teslaLimb" );
    playfxfortag( "j_elbow_ri", "teslaLimb" );
    playfxfortag( "j_elbow_le", "teslaLimb" );
    playfxfortag( "j_hip_ri", "teslaLimb" );
    playfxfortag( "j_hip_le", "teslaLimb" );
    playfxfortag( "j_knee_ri", "teslaLimb" );
    playfxfortag( "j_knee_le", "teslaLimb" );
    playfxfortag( "j_spineupper", "teslaTorso" );
    playfxfortag( "j_head", "teslaHead" );
...
mtx8bitdeath( var_0 )
{
    playfxfortag( "j_shoulder_ri", "8BitLimb" );
    playfxfortag( "j_shoulder_le", "8BitLimb" );
    playfxfortag( "j_elbow_ri", "8BitLimb" );
    playfxfortag( "j_elbow_le", "8BitLimb" );
    playfxfortag( "j_hip_ri", "8BitLimb" );
    playfxfortag( "j_hip_le", "8BitLimb" );
    playfxfortag( "j_knee_ri", "8BitLimb" );
    playfxfortag( "j_knee_le", "8BitLimb" );
    playfxfortag( "j_spineupper", "8BitTorso" );
    playfxfortag( "j_head", "8BitHead" );
    self playsound( "8bit_death_sfx" );
    var_0 hide();
}

playfxfortag( var_0, var_1 )
{
...
enableburnsfx()
{
    ...
    if ( !isdefined( self.burnsfx ) )
    {
        ...
        thread burnsfx_cleanup( self.burnsfx );
        wait 0.05;
    }
