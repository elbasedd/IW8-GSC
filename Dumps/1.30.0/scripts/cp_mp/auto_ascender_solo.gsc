...
init()
{
    ...
    foreach ( var_1 in level.ascendsolostarts )
        scripts\cp_mp\auto_ascender::markupascenderstruct( var_1, 1 );

    foreach ( var_4 in level.descendsolostarts )
    ...
    foreach ( var_1 in level.ascendsolostarts )
    {
        foreach ( var_4 in level.descendsolostarts )
        {
            if ( !isdefined( var_4.sololink ) && distance2dsquared( var_1.origin, var_4.origin ) < 10 )
            {
                var_1.sololink = var_4;
                var_4.sololink = var_1;
                break;
            }
        }
    }
...
ascendersoloscriptableused( var_0, var_1, var_2, var_3, var_4 )
{
    if ( var_2 == "on" )
    {
...
startascenderanim( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    if ( var_4 > 0 )
        thread spinafterdelay( self.angles + ( 0, var_4 * -1, 0 ) );
...
ascenderuse( var_0, var_1 )
{
    ...
    if ( !scripts\cp_mp\auto_ascender::canplayeruseascender( var_0, var_1 ) )
        return;
    ...
    var_1.ascendermodelview = var_12;
    var_1.ascendermodelworld = var_13;
    ...
    if ( !var_2.dir )
    {
        if ( scripts\common\utility::iscp() )
            var_15 = getdvarfloat( "scr_descender_speed_cp", scripts\cp_mp\auto_ascender::getascenderdescendspeedcp() );
        else
            var_15 = getdvarfloat( "scr_descender_speed", scripts\cp_mp\auto_ascender::getascenderdescendspeed() );
    }
    else
        var_15 = getdvarfloat( "scr_ascender_speed", scripts\cp_mp\auto_ascender::getascenderascendspeed() );

    var_16 = var_14 / var_15;
    var_17 = scripts\cp_mp\auto_ascender::getascenderaccelpercent() * var_16;
    var_18 = scripts\cp_mp\auto_ascender::getascenderdecelpercent() * var_16;
...
ispointnearsoloascenderline( var_0 )
{
    foreach ( var_2 in level.ascendsolostarts )
    {
        if ( distance2dsquared( var_2.origin, var_0 ) < scripts\cp_mp\auto_ascender::getascendernearradiussq() )
        {
            if ( isdefined( var_2.sololink ) )
            {
                if ( abs( var_0[2] - var_2.origin[2] ) < scripts\cp_mp\auto_ascender::getascendernearheight() || abs( var_0[2] - var_2.sololink.origin[2] ) < scripts\cp_mp\auto_ascender::getascendernearheight() )
                    return 1;
            }
        }
    }
